            ...
            while (rs.next()) {
                ...
                if (rs.getInt("blocker_id") == 0 && rs.getInt("auto_blocked") == 0) {
                    ...
                    if (event_id > 0) {
                        ...
                        try {
                            pstmt2 = con.prepareStatement("SELECT ltype, color FROM lessonbook5 WHERE recid = ?");
                            pstmt2.clearParameters();
                            pstmt2.setInt(1, lesson_id);
                            ...
                            if (rs2.next()) {
                                ltype = rs2.getString("ltype");
                                bgcolor = rs2.getString("color");

                                out.println("<td class=timesheetTD nowrap colspan=\"" + rs.getInt("max_players") + "\" style=\"text-align:center;;background-color:" + bgcolor + ";\"><i>" + ltype + "</i></td>");
                            } else {

...
public class Proshop_report_pos extends HttpServlet {
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   if (!excel.equals("yes")) {                // if not Excel 
      ...
      out.println("<form method=\"post\" action=\"/" +rev+ "/servlet/Proshop_report_pos\">");
   ...
   if (sent_count > 0) {
      ...
      try {
         ...
         if (course.equals("") || course.equals("-ALL-")) {
            ...
            queryString = "SELECT * FROM pos_hist WHERE date = ? ORDER BY time, course, fb, member_id";
            
         } else {
              
            queryString = "SELECT * FROM pos_hist WHERE date = ? AND course = ? ORDER BY time, course, fb, member_id";
         }

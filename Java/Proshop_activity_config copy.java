...
public class Proshop_activity_config extends HttpServlet {
    ...
    if (activity_id > 0) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                out.println("<tr><td align=center><a href=\"/"+rev+"/servlet/Proshop_activity_config?parent_id=" + activity_id + "&activity_id=" + rs.getInt("activity_id") + "\" class=linkA>" + rs.getString("activity_name") + "</a></td></tr>");
        ...
        if (found) out.println("<tr><td><br><a href=\"/"+rev+"/servlet/Proshop_activity_config?parent_id=" + activity_id + "&new\" class=linkA>Create New " + activity_name + " Activity w/ Time Sheet</a></td></tr>");
        ...
        try {
            ...
            while (rs.next()) {
                ...
                out.println("<tr><td>asdf <a href=\"/"+rev+"/servlet/Proshop_activity_config?parent_id=" + rs.getInt("activity_id") + "\" class=linkA>" + rs.getString("activity_name") + "</a></td></tr>");
        ...
        out.println("<br><br>");
        out.println("<center><a href=\"/"+rev+"/servlet/Proshop_activity_config?newroot\">New Activity</a></center>");
        ...
        try {
            ...
            while ( rs.next() ) {
                ...
                out.println("<tr><td align=center><a href=\"/"+rev+"/servlet/Proshop_activity_config?activity_id=" + rs.getInt("activity_id") + "&edit\" class=linkA>" + rs.getString("activity_name") + "</a></td></tr>");
        ...
        out.println("<tr><td><br><a href=\"/"+rev+"/servlet/Proshop_activity_config?parent_id=" + parent_id + "&new\" class=linkA>Create New " + activity_name + " Sub Activity</a></td></tr>");
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    String sid = req.getParameter("emailOpt");
    try {
        emailOpt = Integer.parseInt(sid);
    } catch (NumberFormatException ignore) {}
    
    sid = req.getParameter("allow_x");
    try {
    ...
    try {
        ...
        if (activity_id == 0) {
            ...
            pstmt = con.prepareStatement("" +
                    "INSERT INTO activities " +
                        "(activity_name, contact, email, emailOpt, allow_x, xhrs, " +
                         "forceg, unacompGuest, hndcpProSheet, hndcpMemSheet, rndsperday, " +
                         "minutesbtwn, max_players, allow_guests, common_name, " +
                         "first_time, last_time, `interval`, alt_interval, disallow_joins, " +
                         "use_hdcp_equiv, hdcp_equiv_name, hdcp_joining_range, enabled, " +
                         "parent_id, consec_mem, consec_pro) " +
                    "VALUES " +
                        "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            pstmt = con.prepareStatement("" +
                    "UPDATE activities " +
                    "SET " +
                        "activity_name = ?, contact = ?, email = ?, emailOpt = ?, allow_x = ?, xhrs = ?, forceg = ?, " +
                        "unacompGuest = ?, hndcpProSheet = ?, hndcpMemSheet = ?, rndsperday = ?, minutesbtwn = ?, " +
                        "max_players = ?, allow_guests = ?, common_name = ?, first_time = ?, last_time = ?, `interval` = ?, " +
                        "alt_interval = ?, disallow_joins = ?, use_hdcp_equiv = ?, hdcp_equiv_name = ?, " +
        ...
        pstmt.setInt(4, emailOpt);
        pstmt.setInt(5, allow_x);
        pstmt.setInt(6, xhrs);
        pstmt.setInt(7, forceg);
        pstmt.setInt(8, unacompGuest);
        pstmt.setInt(9, hndcpProSheet);
        pstmt.setInt(10, hndcpMemSheet);
        pstmt.setInt(11, rndsperday);
        pstmt.setInt(12, minutesbtwn);
        pstmt.setInt(13, max_players);
        pstmt.setInt(14, allow_guests);

        pstmt.setString(15, common_name);
        pstmt.setInt(16, first_time);
        pstmt.setInt(17, last_time);
        pstmt.setInt(18, interval);
        pstmt.setInt(19, alt_interval);
        pstmt.setInt(20, disallow_joins);
 ...
 private void buildForm(int activity_id, int parent_id, Connection con, PrintWriter out) {
    ...
    try {
        ...
        // See if the club is configured to use the guest database
        ...
        out.println("<BR><BR>If problem persists, contact customer support.");
        out.println("<BR><BR>");
        out.println("<a href=\"/" +rev+ "/servlet/Proshop_announce\" target=\"bot\">Return</a>");
        out.println("</CENTER></BODY></HTML>");
        out.close();
    ...
    out.println("<form action=\"/" +rev+ "/servlet/Proshop_activity_config\" method=\"post\" target=\"bot\" name=\"f\">");
    ...
    out.println("<tr>");
      ...
      out.println("<td align=\"right\" style=\"padding-right: 15px\">");
         out.println("<font size=\"2\">");
         out.println("Can <b>members join other members'</b> reservations?");
         out.println("</font>");
    ...
    if (parent_id == 0) {
        out.println("<tr>");
          out.println("<td align=\"center\"></td>");    // No help link at the moment

          out.println("<td align=\"right\" style=\"padding-right: 15px\">");
             out.println("<font size=\"2\">");
             out.println("Do you allow <b>Do you wish to use the <b>Guest Database</b> system?</b>?");
             out.println("</font>");

          out.println("</td><td align=\"left\" style=\"padding-left: 15px\"><font size=\"2\">");
             out.println("<select size=\"1\" name=\"guestdb\">");

             Common_Config.buildOption(0, "No" , guestdb, out);
             Common_Config.buildOption(1, "Yes" , guestdb, out);
...
*/
    ...
    out.println("<tr>");
      ...
      out.println("<td align=\"right\" style=\"padding-right: 15px\">");
         out.println("<font size=\"2\">");
         out.println("How many <b>consecutive times can members</b> reserve at one time?<br>(Zero = None, just one at a time):");
         out.println("</font>");
    ...
    out.println("<tr>");
      ...
      out.println("<td align=\"right\" style=\"padding-right: 15px\">");
         out.println("<font size=\"2\">");
         out.println("How manu <b>consecutive times can proshop users</b> reserve at one time?<br>(Zero = None, just one at a time):");
         out.println("</font>");
    ...
    out.println("<br><center><button onclick=\"location.href='/" + rev + "/servlet/Proshop_announce'\"> Home </button></center><br>");

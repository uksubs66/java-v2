...
public class Common_guestdb extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp)
            ...
            if (caller.equals("sheet") || caller.equals("modal") || caller.equals("mobile")) {
                ...
                if (caller.equals("sheet")) {
                    ...
                    if (tempTok.countTokens() == 2) {
                        ...
                        out.println(result_msg);
                        out.println("<br><br><button onclick=\"passguest('" + target_guest + "');\">Continue</button>");
        ...
        if (req.getParameter("mergeGuest") != null) {
            ...
            if (target_id != 0 && merged_id != 0 && !target_unique_id.equals("")) {
...
*/
        ...
        if (req.getParameter("modal") != null || caller.equals("modal")) {
            loadModal(req, sess_activity_id, user, out, con);
            return;
        }
...
*/
        ...
        if (!Utilities.isGuestTrackingConfigured(activity_id, con)) {
            ...
            if (isProshop) {
                ...
                out.println("<br><br><center><button onclick=\"location.href='/" + rev + "/servlet/Proshop_announce'\">Home</button></center>");
            } else {
                out.println("<br><br><center><button onclick=\"location.href='/" + rev + "/servlet/Member_announce'\">Home</button></center>");
            }
...
/*  Don't use alphabet filtering box for now.  Will re-enable later if requested
            if (isProshop) {
                ...
                out.print("<form action=\"/" + rev + "/servlet/Common_guestdb\" method=\"POST\">");
...
*/
            ...
            out.println("<table align=\"center\" bgcolor=\"#CCCCAA\">");

            if (!result_msg.equals("")) {
                out.println("<tr><td align=\"center\"><h3>" + result_msg + "</h3></td></tr>");
            }
            out.println("<form action=\"/" + rev + "/servlet/Common_guestdb\" method=\"POST\" name=\"guestBtnForm\">");
            out.println("<tr><td align=\"center\">");
            out.println("<input type=\"hidden\" name=\"caller\" value=\"mgmt\">");
            if (isProshop) {
                out.println("<input type=\"button\" class=\"btnNorm\" name=\"btnHome\" value=\"Home\" title=\"Return to announcement page\" onclick=\"location.href='/" + rev + "/servlet/Proshop_announce'\">&nbsp;&nbsp;");
            } else {
                out.println("<input type=\"button\" class=\"btnNorm\" name=\"btnHome\" value=\"Home\" title=\"Return to announcement page\" onclick=\"location.href='/" + rev + "/servlet/Member_announce'\">&nbsp;&nbsp;");
            }
            ...
            if (!isProshop) {
                ...
                out.println("<input type=\"button\" class=\"btnNorm\" value=\"Partner List\" onclick=\"location.href='/" + rev + "/servlet/Member_partner'\">");
            }
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    out.println("<td class=\"mainTd\"" + textStyle + ">" + (name_last.equals("") ? "&nbsp;" : name_last) + "</td>");
                    ...
                    out.println("<a class=\"rwDtaAct\" href=\"/" + rev + "/servlet/Common_guestdb?guestForm&caller=mgmt&guest_id=" + guest_id + "\" title=\"Edit this guest's information\">Edit</a>");
                    out.println("<a class=\"rwDtaAct\" href=\"/" + rev + "/servlet/Common_guestdb?hostForm&guest_id=" + guest_id + "\" title=\"Select which members will be able to see this guest during the booking process\">Hosts</a>");

                    // If a member, also display a link to remove the host binding between the user and this guest.
                    if (!isProshop && host_id > 0) {
                        out.println("<a class=\"rwDtaAct\" href=\"/" + rev + "/servlet/Common_guestdb?removeHost=" + host_id + "\" title=\"Remove yourself as a host for this guest\" onclick=\"return confirm('Warning: If you remove yourself as a host for this guest, you will no longer be able to select them when booking a reservation!');\">Remove</a>");
                    } /*else if (isProshop) {
                        out.println("<a class=\"rwDtaAct\" href=\"/" + rev + "/servlet/Common_guestdb?removeGuest&guest_id=" + guest_id + "\" title=\"Remove this guest from the system.\" onclick=\"confirm('Warning: This guest will be permanently removed, and any currently booked rounds with this guest will no longer be associated with them!')\">Remove</a>");
                    }*/
    ...
    private void displayHostForm (HttpServletRequest req, HttpSession session, boolean isProshop, String user, PrintWriter out, Connection con) {
        ...
        out.println(" fr.src = '/" + rev + "/servlet/Common_guestdb?hostList&guest_id=" + guest_id + "&addHost=' + username;");
        out.println("}");
        out.println("// -->");
        out.println("</script>");
        ...
        out.println("<iframe id=\"hostlistiframe\" src=\"/" + rev + "/servlet/Common_guestdb?hostList&guest_id=" + guest_id + "\" width=\"500px\" scrolling=no frameborder=no></iframe>");
        out.println("</td>");

        
        // Set up form for use with adding a new host by clicking the memeber name-list
        out.println("<form action=\"/" + rev + "/servlet/Common_guestdb\" method=\"POST\" name=\"playerform\" onsubmit=\"false\">");
        ...
        out.println("</tr>");
        out.println("<tr><td align=\"center\" colspan=\"2\"><br><button class=\"btnNorm\" onclick=\"location.href='/" + rev + "/servlet/Common_guestdb'\">Return</button></td></tr>");
        out.println("</table>");
    ...
    private void displayHostList (HttpServletRequest req, boolean isProshop, PrintWriter out, Connection con) {
        ...
        out.println("<form action=\"/" + rev + "/servlet/Common_guestdb\" method=\"POST\" name=\"removeHostForm\">");
    ...
    public static void displayGuestForm (HttpServletRequest req, boolean isProshop, int activity_id, PrintWriter out, Connection con) {
        ...
        if (req.getParameter("caller") != null) caller = req.getParameter("caller");
        if (req.getParameter("guestForm") != null && req.getParameter("guestForm").equals("Add New Guest")) submitText = "Add Guest";
        ...
        if (!Utilities.isGuestTrackingConfigured(activity_id, con)) {
            ...
            out.println("<form action=\"/" + rev + "/servlet/Common_guestdb\" method=\"POST\" name=\"guestInfo\">");
            ...
            if (caller.equals("sheet")) {   // came from tee sheet, meaning form is currently in a window that should be closed
                ...
                out.println("<input type=\"button\" class=\"btnNorm\" name=\"btnCancel\" value=\"Cancel\" onclick=\"location.href='/" + rev + "/servlet/Common_guestdb'\">");
            }
    ...
    public static void printGuestInfoFields(HttpServletRequest req, boolean isProshop, boolean reqOnly, int activity_id, PrintWriter out, Connection con) {
        ...
        if (req.getParameter("guest_id") != null && !req.getParameter("guest_id").equals("0") && req.getParameter("return") == null) {
            ...
            try {
                ...
                if (rs.next()) {
                    if (rs.getString("unique_id") != null) unique_id = rs.getString("unique_id");
                    uid_type = rs.getString("uid_type");
            ...
            if (req.getParameter("unique_id") != null) unique_id = req.getParameter("unique_id");
            if (req.getParameter("uid_type") != null) uid_type = req.getParameter("uid_type");
            if (req.getParameter("name_pre") != null) name_pre = req.getParameter("name_pre");
            if (req.getParameter("name_first") != null) name_first = req.getParameter("name_first");
            if (req.getParameter("name_mi") != null) name_mi = req.getParameter("name_mi");
            if (req.getParameter("name_last") != null) name_last = req.getParameter("name_last");
            if (req.getParameter("name_suf") != null) name_suf = req.getParameter("name_suf");
            ...
            if (req.getParameter("email_bounced1") != null) email_bounced1 = Integer.parseInt(req.getParameter("email_bounced1"));
            if (req.getParameter("email_bounced2") != null) email_bounced2 = Integer.parseInt(req.getParameter("email_bounced2"));
            ...
            if (req.getParameter("address1") != null) address1 = req.getParameter("address1");
            if (req.getParameter("address2") != null) address2 = req.getParameter("address2");
            if (req.getParameter("city") != null) city = req.getParameter("city");
            if (req.getParameter("state") != null) state = req.getParameter("state");
            if (req.getParameter("zip") != null) zip = req.getParameter("zip");
            if (req.getParameter("gender") != null) gender = req.getParameter("gender");
            if (req.getParameter("hdcp_num") != null) hdcp_num = req.getParameter("hdcp_num");
            if (req.getParameter("home_club") != null) home_club = req.getParameter("home_club");
            if (req.getParameter("inact") != null) inact = Integer.parseInt(req.getParameter("inact"));

            try {
                if (req.getParameter("hdcp_index") != null && !req.getParameter("hdcp_index").equals("")) hdcp_index = Double.parseDouble(req.getParameter("hdcp_index"));
        ...
        if (!use_phone.equals("N") && (!reqOnly || use_phone.equals("R"))) {
            out.println("<tr><td align=\"right\">Phone 1:</td><td align=\"left\"><input type=\"text\" name=\"phone1\" value=\"" + phone1 + "\" size=\"24\" maxlength=\"24\">" + (use_phone.equals("R") ? (phone1.equals("") ? "<font color=red> *</font>" : " *") : "") + "</td></tr>");
    ...
    private static String updateGuest(HttpServletRequest req, boolean isProshop, int activity_id, String user, PrintWriter out, Connection con) {
        ...
        if (req.getParameter("skip") != null) skip = true;
        if (req.getParameter("caller") != null) caller = req.getParameter("caller");
        if (req.getParameter("unique_id") != null) unique_id = req.getParameter("unique_id").trim();
        if (req.getParameter("uid_type") != null) uid_type = req.getParameter("uid_type").trim();
        if (req.getParameter("name_pre") != null) name_pre = req.getParameter("name_pre").trim();
        if (req.getParameter("name_first") != null) name_first = req.getParameter("name_first");
        if (req.getParameter("name_mi") != null) name_mi = req.getParameter("name_mi").trim();
        if (req.getParameter("name_last") != null) name_last = req.getParameter("name_last").trim();
        if (req.getParameter("name_suf") != null) name_suf = req.getParameter("name_suf").trim();
        if (req.getParameter("email1") != null) email1 = req.getParameter("email1").trim();
        if (req.getParameter("email2") != null) email2 = req.getParameter("email2").trim();
        if (req.getParameter("email_bounced1") != null) email_bounced1 = Integer.parseInt(req.getParameter("email_bounced1"));
        if (req.getParameter("email_bounced2") != null) email_bounced2 = Integer.parseInt(req.getParameter("email_bounced2"));
        if (req.getParameter("emailOpt") != null) emailOpt = Integer.parseInt(req.getParameter("emailOpt"));
        if (req.getParameter("phone1") != null) phone1 = req.getParameter("phone1").trim();
        if (req.getParameter("phone2") != null) phone2 = req.getParameter("phone2").trim();
        if (req.getParameter("address1") != null) address1 = req.getParameter("address1").trim();
        if (req.getParameter("address2") != null) address2 = req.getParameter("address2").trim();
        if (req.getParameter("city") != null) city = req.getParameter("city").trim();
        if (req.getParameter("state") != null) state = req.getParameter("state").trim();
        if (req.getParameter("zip") != null) zip = req.getParameter("zip").trim();
        if (req.getParameter("gender") != null) gender = req.getParameter("gender").trim();
        if (req.getParameter("hdcp_num") != null) hdcp_num = req.getParameter("hdcp_num").trim();
        if (req.getParameter("home_club") != null) home_club = req.getParameter("home_club").trim();
        if (req.getParameter("inact") != null) inact = Integer.parseInt(req.getParameter("inact"));
        try {
            if (req.getParameter("hdcp_index") != null && !req.getParameter("hdcp_index").equals("")) hdcp_index = Double.parseDouble(req.getParameter("hdcp_index"));
        } catch (Exception ignore) { }
        ...
        if (!skip &&
            ...
            out.println("<form action=\"/" + rev + "/servlet/Common_guestdb\" method=\"POST\" name=\"returnForm\">");
            ...
            out.println("<input type=\"hidden\" name=\"return\" value=\"1\">");
            if (!caller.equals("")) out.println("<input type=\"hidden\" name=\"caller\" value=\"" + caller + "\">");
            if (!unique_id.equals("")) out.println("<input type=\"hidden\" name=\"unique_id\" value=\"" + unique_id + "\">");
            if (!uid_type.equals("")) out.println("<input type=\"hidden\" name=\"uid_type\" value=\"" + uid_type + "\">");
            if (!name_pre.equals("")) out.println("<input type=\"hidden\" name=\"name_pre\" value=\"" + name_pre + "\">");
            if (!name_first.equals("")) out.println("<input type=\"hidden\" name=\"name_first\" value=\"" + name_first + "\">");
            if (!name_mi.equals("")) out.println("<input type=\"hidden\" name=\"name_mi\" value=\"" + name_mi + "\">");
            if (!name_last.equals("")) out.println("<input type=\"hidden\" name=\"name_last\" value=\"" + name_last + "\">");
            if (!name_suf.equals("")) out.println("<input type=\"hidden\" name=\"name_suf\" value=\"" + name_suf + "\">");
            if (!email1.equals("")) out.println("<input type=\"hidden\" name=\"email1\" value=\"" + email1 + "\">");
            if (!email2.equals("")) out.println("<input type=\"hidden\" name=\"email2\" value=\"" + email2 + "\">");
            if (email_bounced1 != 0) out.println("<input type=\"hidden\" name=\"email_bounced1\" value=\"" + email_bounced1 + "\">");
            if (email_bounced2 != 0) out.println("<input type=\"hidden\" name=\"email_bounced2\" value=\"" + email_bounced2 + "\">");
            if (emailOpt != 0) out.println("<input type=\"hidden\" name=\"emailOpt\" value=\"" + emailOpt + "\">");
            if (!phone1.equals("")) out.println("<input type=\"hidden\" name=\"phone1\" value=\"" + phone1 + "\">");
            if (!phone2.equals("")) out.println("<input type=\"hidden\" name=\"phone2\" value=\"" + phone2 + "\">");
            if (!address1.equals("")) out.println("<input type=\"hidden\" name=\"address1\" value=\"" + address1 + "\">");
            if (!address2.equals("")) out.println("<input type=\"hidden\" name=\"address2\" value=\"" + address2 + "\">");
            if (!city.equals("")) out.println("<input type=\"hidden\" name=\"city\" value=\"" + city + "\">");
            if (!state.equals("")) out.println("<input type=\"hidden\" name=\"state\" value=\"" + state + "\">");
            if (!zip.equals("")) out.println("<input type=\"hidden\" name=\"zip\" value=\"" + zip + "\">");
            if (!gender.equals("")) out.println("<input type=\"hidden\" name=\"gender\" value=\"" + gender + "\">");
            if (!hdcp_num.equals("")) out.println("<input type=\"hidden\" name=\"hdcp_num\" value=\"" + hdcp_num + "\">");
            if (hdcp_index != -99) out.println("<input type=\"hidden\" name=\"hdcp_index\" value=\"" + hdcp_index + "\">");
            if (!home_club.equals("")) out.println("<input type=\"hidden\" name=\"home_club\" value=\"" + home_club + "\">");
            if (inact == 1) out.println("<input type=\"hidden\" name=\"inact\" value=\"" + inact + "\">");
            ...
            if (!isProshop || !allowOverride) {
                ...
                out.println("<form action=\"/" + rev + "/servlet/Common_guestdb\" method=\"POST\" name=\"overrideForm\">");
            ...
            if (existingUser) {
                ...
                try {
                    ...
                    if (!guest_name.equals(guest_name_old)) nameChanged = true;
    ...
    public static void displayDuplicateGuests(HttpServletRequest req, String user, int activity_id, PrintWriter out, Connection con) {
        ...
        if (req.getParameter("unique_id") != null) {
            ...
            if (req.getParameter("caller") != null) caller = req.getParameter("caller");
            if (req.getParameter("uid_type") != null) dup_uid_type = req.getParameter("uid_type");
            if (req.getParameter("name_pre") != null) dup_name_pre = req.getParameter("name_pre");
            if (req.getParameter("name_first") != null) dup_name_first = req.getParameter("name_first");
            if (req.getParameter("name_mi") != null) dup_name_mi = req.getParameter("name_mi");
            if (req.getParameter("name_last") != null) dup_name_last = req.getParameter("name_last");
            if (req.getParameter("name_suf") != null) dup_name_suf = req.getParameter("name_suf");
            if (req.getParameter("email1") != null) dup_email1 = req.getParameter("email1");
            if (req.getParameter("email2") != null) dup_email2 = req.getParameter("email2");
            if (req.getParameter("email_bounced1") != null) dup_email_bounced1 = Integer.parseInt(req.getParameter("email_bounced1"));
            if (req.getParameter("email_bounced2") != null) dup_email_bounced2 = Integer.parseInt(req.getParameter("email_bounced2"));
            if (req.getParameter("emailOpt") != null) dup_emailOpt = Integer.parseInt(req.getParameter("emailOpt"));
            if (req.getParameter("phone1") != null) dup_phone1 = req.getParameter("phone1");
            if (req.getParameter("phone2") != null) dup_phone2 = req.getParameter("phone2");
            if (req.getParameter("address1") != null) dup_address1 = req.getParameter("address1");
            if (req.getParameter("address2") != null) dup_address2 = req.getParameter("address2");
            if (req.getParameter("city") != null) dup_city = req.getParameter("city");
            if (req.getParameter("state") != null) dup_state = req.getParameter("state");
            if (req.getParameter("zip") != null) dup_zip = req.getParameter("zip");
            if (req.getParameter("gender") != null) dup_gender = req.getParameter("gender");
            if (req.getParameter("hdcp_num") != null) dup_hdcp_num = req.getParameter("hdcp_num");
            if (req.getParameter("hdcp_index") != null && !req.getParameter("hdcp_index").equals("")) dup_hdcp_index = Double.parseDouble(req.getParameter("hdcp_index"));
            if (req.getParameter("home_club") != null) dup_home_club = req.getParameter("home_club");
            if (req.getParameter("inact") != null) dup_inact = Integer.parseInt(req.getParameter("inact"));
            ...
            if (existingGuest) {
                ...
                out.println("<form action=\"/" + rev + "/servlet/Common_guestdb\" method=\"POST\" name=\"mergeToExistingForm\">");
                out.println("<input type=\"hidden\" name=\"caller\" value=\"" + caller + "\">");
                ...
                // Handicap Index
                if (!use_hdcp_index.equals("N")) {
                    out.println("<tr><td align=\"left\">Handicap Number: " + (dup_hdcp_index == -99 ? "" : hdcp_index) + (use_hdcp_index.equals("R") ? (dup_hdcp_index == -99 ? "<font color=red> *</font>" : " *") : "") + "</td></tr>");
                }
                ...
                // Print form and submit button to merge to this guest
                out.println("<form action=\"/" + rev + "/servlet/Common_guestdb\" method=\"POST\" name=\"mergeToEditedForm\">");
                out.println("<input type=\"hidden\" name=\"caller\" value=\"" + caller + "\">");
                out.println("<input type=\"hidden\" name=\"target_unique_id\" value=\"" + dup_unique_id + "\">");
                ...
                if (!caller.equals("")) out.println("<input type=\"hidden\" name=\"caller\" value=\"" + caller + "\">");
                if (!dup_unique_id.equals("")) out.println("<input type=\"hidden\" name=\"unique_id\" value=\"" + dup_unique_id + "\">");
                if (!dup_uid_type.equals("")) out.println("<input type=\"hidden\" name=\"uid_type\" value=\"" + dup_uid_type + "\">");
                if (!dup_name_pre.equals("")) out.println("<input type=\"hidden\" name=\"name_pre\" value=\"" + dup_name_pre + "\">");
                if (!dup_name_first.equals("")) out.println("<input type=\"hidden\" name=\"name_first\" value=\"" + dup_name_first + "\">");
                if (!dup_name_mi.equals("")) out.println("<input type=\"hidden\" name=\"name_mi\" value=\"" + dup_name_mi + "\">");
                if (!dup_name_last.equals("")) out.println("<input type=\"hidden\" name=\"name_last\" value=\"" + dup_name_last + "\">");
                if (!dup_name_suf.equals("")) out.println("<input type=\"hidden\" name=\"name_suf\" value=\"" + dup_name_suf + "\">");
                if (!dup_email1.equals("")) out.println("<input type=\"hidden\" name=\"email1\" value=\"" + dup_email1 + "\">");
                if (!dup_email2.equals("")) out.println("<input type=\"hidden\" name=\"email2\" value=\"" + dup_email2 + "\">");
                if (dup_email_bounced1 != 0) out.println("<input type=\"hidden\" name=\"email_bounced1\" value=\"" + dup_email_bounced1 + "\">");
                if (dup_email_bounced2 != 0) out.println("<input type=\"hidden\" name=\"email_bounced2\" value=\"" + dup_email_bounced2 + "\">");
                if (dup_emailOpt != 0) out.println("<input type=\"hidden\" name=\"emailOpt\" value=\"" + dup_emailOpt + "\">");
                if (!dup_phone1.equals("")) out.println("<input type=\"hidden\" name=\"phone1\" value=\"" + dup_phone1 + "\">");
                if (!dup_phone2.equals("")) out.println("<input type=\"hidden\" name=\"phone2\" value=\"" + dup_phone2 + "\">");
                if (!dup_address1.equals("")) out.println("<input type=\"hidden\" name=\"address1\" value=\"" + dup_address1 + "\">");
                if (!dup_address2.equals("")) out.println("<input type=\"hidden\" name=\"address2\" value=\"" + dup_address2 + "\">");
                if (!dup_city.equals("")) out.println("<input type=\"hidden\" name=\"city\" value=\"" + dup_city + "\">");
                if (!dup_state.equals("")) out.println("<input type=\"hidden\" name=\"state\" value=\"" + dup_state + "\">");
                if (!dup_zip.equals("")) out.println("<input type=\"hidden\" name=\"zip\" value=\"" + dup_zip + "\">");
                if (!dup_gender.equals("")) out.println("<input type=\"hidden\" name=\"gender\" value=\"" + dup_gender + "\">");
                if (!dup_hdcp_num.equals("")) out.println("<input type=\"hidden\" name=\"hdcp_num\" value=\"" + dup_hdcp_num + "\">");
                if (dup_hdcp_index != -99) out.println("<input type=\"hidden\" name=\"hdcp_index\" value=\"" + dup_hdcp_index + "\">");
                if (!dup_home_club.equals("")) out.println("<input type=\"hidden\" name=\"home_club\" value=\"" + dup_home_club + "\">");
                if (dup_inact == 1) out.println("<input type=\"hidden\" name=\"inact\" value=\"" + dup_inact + "\">");
                ...
                if (caller.equals("modal")) {
                    ...
                    out.println("<input class=\"btnNorm\" type=\"button\" name=\"useGuestBtn\" value=\"Use Existing Record\" onclick=\"location.href='/" + rev + "/servlet/Common_guestdb'\">&nbsp;&nbsp;");
                    out.println("<input class=\"btnNorm\" type=\"submit\" name=\"guestForm\" value=\"Return to Guest Info Form\">");
                }
    ...
    public static boolean mergeGuestID(String target_unique_id, int target_id, int merged_id, Connection con) {
        ...
        // First, get the player names for both IDs to see if any changes occurred.
        try {
        ...
        // Update teecurr2
        if (!error) error = convertGuestId("teecurr2", "guest_id1", target_id, merged_id, con);
        if (!error) error = convertGuestId("teecurr2", "guest_id2", target_id, merged_id, con);
        if (!error) error = convertGuestId("teecurr2", "guest_id3", target_id, merged_id, con);
        if (!error) error = convertGuestId("teecurr2", "guest_id4", target_id, merged_id, con);
        if (!error) error = convertGuestId("teecurr2", "guest_id5", target_id, merged_id, con);

        // Update teepast2
        if (!error) error = convertGuestId("teepast2", "guest_id1", target_id, merged_id, con);
        if (!error) error = convertGuestId("teepast2", "guest_id2", target_id, merged_id, con);
        if (!error) error = convertGuestId("teepast2", "guest_id3", target_id, merged_id, con);
        if (!error) error = convertGuestId("teepast2", "guest_id4", target_id, merged_id, con);
        if (!error) error = convertGuestId("teepast2", "guest_id5", target_id, merged_id, con);

        // Update evntsup2b
        if (!error) error = convertGuestId("evntsup2b", "guest_id1", target_id, merged_id, con);
        if (!error) error = convertGuestId("evntsup2b", "guest_id2", target_id, merged_id, con);
        if (!error) error = convertGuestId("evntsup2b", "guest_id3", target_id, merged_id, con);
        if (!error) error = convertGuestId("evntsup2b", "guest_id4", target_id, merged_id, con);
        if (!error) error = convertGuestId("evntsup2b", "guest_id5", target_id, merged_id, con);

        // Update lreqs3
        if (!error) error = convertGuestId("lreqs3", "guest_id1", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id2", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id3", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id4", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id5", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id6", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id7", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id8", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id9", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id10", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id11", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id12", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id13", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id14", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id15", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id16", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id17", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id18", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id19", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id20", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id21", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id22", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id23", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id24", target_id, merged_id, con);
        if (!error) error = convertGuestId("lreqs3", "guest_id25", target_id, merged_id, con);

        // Update activity_sheets_players
        if (!error) error = convertGuestId("activity_sheets_players", "guest_id", target_id, merged_id, con);

        // Update lreqs3
        if (!error) error = convertGuestId("wait_list_signups_players", "guest_id", target_id, merged_id, con);

        // Delete any duplicate host entries between the two guest_ids in guestdb_hosts
        if (!error) error = removeDuplicateHosts(target_id, merged_id, con);

        // Update guestdb_hosts
        if (!error) error = convertGuestId("guestdb_hosts", "guest_id", target_id, merged_id, con);

        // Remove guestdb_data entry for merged_id
        if (!error) error = removeGuestId(merged_id, con);

        // Finally, update unique_id to the new value
        if (!error) error = convertUniqueId("guestdb_data", "unique_id", "guest_id", target_unique_id, target_id, con);
    ...
    public static boolean updateGuestNames(int guest_id, String name_new, String name_old, Connection con) {
        ...
        // Update teecurr2
        if (!error) error = convertGuestName("teecurr2", "teecurr_id", "guest_id1", "player1", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("teecurr2", "teecurr_id", "guest_id2", "player2", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("teecurr2", "teecurr_id", "guest_id3", "player3", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("teecurr2", "teecurr_id", "guest_id4", "player4", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("teecurr2", "teecurr_id", "guest_id5", "player5", name_new, name_old, guest_id, con);

        // Update teepast2
        if (!error) error = convertGuestName("teepast2", "teepast_id", "guest_id1", "player1", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("teepast2", "teepast_id", "guest_id2", "player2", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("teepast2", "teepast_id", "guest_id3", "player3", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("teepast2", "teepast_id", "guest_id4", "player4", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("teepast2", "teepast_id", "guest_id5", "player5", name_new, name_old, guest_id, con);

        // Update evntsup2b
        if (!error) error = convertGuestName("evntsup2b", "id", "guest_id1", "player1", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("evntsup2b", "id", "guest_id2", "player2", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("evntsup2b", "id", "guest_id3", "player3", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("evntsup2b", "id", "guest_id4", "player4", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("evntsup2b", "id", "guest_id5", "player5", name_new, name_old, guest_id, con);

        // Update lreqs3
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id1", "player1", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id2", "player2", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id3", "player3", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id4", "player4", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id5", "player5", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id6", "player6", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id7", "player7", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id8", "player8", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id9", "player9", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id10", "player10", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id11", "player11", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id12", "player12", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id13", "player13", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id14", "player14", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id15", "player15", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id16", "player16", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id17", "player17", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id18", "player18", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id19", "player19", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id20", "player20", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id21", "player21", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id22", "player22", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id23", "player23", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id24", "player24", name_new, name_old, guest_id, con);
        if (!error) error = convertGuestName("lreqs3", "id", "guest_id25", "player25", name_new, name_old, guest_id, con);

        // Update activity_sheets_players
        if (!error) error = convertGuestName("activity_sheets_players", "activity_sheets_player_id", "guest_id", "player_name", name_new, name_old, guest_id, con);

        // Update lreqs3
        if (!error) error = convertGuestName("wait_list_signups_players", "wait_list_signup_player_id", "guest_id", "player_name", name_new, name_old, guest_id, con);
    ...
    private static void loadModal(HttpServletRequest req, int activity_id, String user, PrintWriter out, Connection con) {
        ...
        if (Utilities.isGuestTrackingTbaAllowed(activity_id, isProshop, con)) out.println("<br>If you do not know the name of your guest at this time, please select 'TBA' from the list.");
        out.println("<br><br>");
        out.println("</td></tr>");
        ...
        alphaTable.guestdbList(user, 0, activity_id, out, con);
        out.println("</td>");
    ...
    public static String getGuestName(int guest_id, Connection con) {
        ...
        } catch (Exception exc) { guest_name = exc.getMessage() + " " + guest_id; } // return blank name if error encountered

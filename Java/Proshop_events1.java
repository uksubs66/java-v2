      ...
      out.println("<table border=\"1\" bgcolor=\"#F5F5DC\" cellpadding=\"5\" valign=\"top\">");
         ...
         if (sess_activity_id != 0) {
            ...
            */
            ...
            out.println("<td align=\"center\">");
               out.println("<font color=\"#FFFFFF\" size=\"2\"><b>");
               out.println("<a href='?sortBy=date" + ((sortBy.equals("date") && !sortDesc) ? "&desc" : "") + ((showOld) ? "&showOld" : "") + "' style='color:white'>Date</a>");
               out.println("</b></font></td>");
   ...
   try {
      ...
      String sql = "" +
         "SELECT name, date, year, month, day, color, act_hr, act_min, courseName, " +
            ...
            "IF(season=1, c_date, date) AS date2, season, activity_id " +
         "FROM events2b e " +
      ...
      while (rs.next()) {
         ...
         if (signUp != 0 || max != 0 || gstOnly != 0) {
            ...
            if (skip == false) {                // if ok to display
               ...
               //
               out.println("<form action=\"/" +rev+ "/servlet/Proshop_events2\" method=\"post\" target=\"bot\">");

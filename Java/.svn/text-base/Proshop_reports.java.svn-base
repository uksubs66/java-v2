...
public class Proshop_reports extends HttpServlet {
 ...
 private void guestQuota(HttpServletRequest req, PrintWriter out, Connection con) {
    ...
    if (user.equals("")) {
         ...
         out.println("<BR><BR>Please try again.");
         out.println("<BR><BR>If problem persists, contact customer support.");
         out.println("<br><br><a href=\"/" +rev+ "/servlet/Proshop_reports?subtee=cal\">Return</a>");
         out.println("</CENTER></BODY></HTML>");
         out.close();
         return;
    }
...
*/      
         ...
         PreparedStatement pstmt_mtype = con.prepareStatement (
            "SELECT IF(m_type='', IF(null_user=1,'Unknown','Removed from Member Database'), m_type) AS m_type, SUM(p_9), SUM(p_18) " +
            "FROM ( " +
...
*/
         ...
         PreparedStatement pstmt_gtype = con.prepareStatement (
               ...
               ") AS d_table GROUP BY guest_type WITH ROLLUP;");
  ...
  */
   ...
   try {
      ...
      sql = ((sess_activity_id == 0) ?
            ...
            "SELECT  SUM(f1) AS pronew, SUM(f2) AS promod, " +
            ...
            if (hotel > 0) {
               ...
               out.println("<td>");
                  ...
                  if (memNewlyr < 1 || totLastyr < 1) {
                     ...
                     out.println("<p align=\"right\">" + memNewlyr + " (" + (memNewlyr * 100)/totLastyr + "%)</p>");
                  }
                  out.println("</font></td>");
      ...
      out.println("<table border=\"1\" bgcolor=\"#F5F5DC\" cellpadding=\"5\">");
         ...
         out.println("<input type=\"hidden\" name=\"noshow2\" value=\"yes\"></input>");
            ...
            out.println("<tr><td>");
               out.println("<font size=\"2\">");
               out.println("<div id=\"awmobject1\">");        // allow menus to show over this box
               Common_Config.displayStartDate(0, 0, 0, true, out);
               out.println("</div><br>");

               out.println("<div id=\"awmobject2\">");        // allow menus to show over this box
               Common_Config.displayEndDate(0, 0, 0, true, out);
 ...
 private void teeTime(HttpServletRequest req, PrintWriter out, Connection con) {
   ...
   if (subtee.equalsIgnoreCase( "cal" )) {      // if calendar year requested
      ...
      try {
         ...
         rs = stmt.executeQuery("SELECT MIN(yy) " +
                                "FROM teepast2");

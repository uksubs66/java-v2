...
public class Support_courses extends HttpServlet {
 ...
 private void defaultCourseConfig(HttpServletRequest req, String user, String club, Connection con, PrintWriter out) {
     ...
     //
     String [] courseA = new String [20];            // max of 20 course
     
     String courseName = "";                         // to hold course name
     String defaultPro = "";
     String defaultMem = "";
     int courseCount = 0;                            // to be set to the total # of courses
     ...
     if (req.getParameter("memCourse") != null && req.getParameter("proCourse") != null) {
         ...
         try {
             ...
             if (user.startsWith( "sales" )) {
                 ...
                 out.println("<BR><A HREF=\"/" +rev+ "/support_main.htm\">Return</A>");
             }
             out.println("</CENTER></BODY></HTML>");
             out.close();
         }
         ...
         try {
             ...
             courseCount = i;       // save the total # of courses for later

             stmt2 = con.createStatement();

             rs2 = stmt2.executeQuery("SELECT default_course_mem, default_course_pro FROM club5");

             if (rs2.next()) {
                 defaultMem = rs2.getString("default_course_mem");
                 defaultPro = rs2.getString("default_course_pro");
             }
             ...
             //
             out.println("<br>");
             out.println("<form action=\"/" + rev + "/servlet/Support_courses?default\" method=\"post\" name=\"defCourse\">");
             out.println("<p><b>Default Member Course: </b>");
             out.println("<select size=\"1\" name=\"memCourse\">");
             out.println("<option value=\"\"></option>");
             if (defaultMem.equals("-ALL-")) {
             ...
             while ((!courseName.equals( "" )) && (i < 20)) {
                 ...
                 if (courseName.equals( defaultMem )) {
             ...
             while ((!courseName.equals( "" )) && (i < 20)) {
                 ...
                 if (courseName.equals( defaultPro )) {
                     out.println("<option selected value=\"" + courseName + "\">" + courseName + "</option>");
                 } else {
                     out.println("<option value=\"" + courseName + "\">" + courseName + "</option>");
                 }
     ...
     if (user.equals("support")) {
         out.println("<A HREF=\"/" +rev+ "/support_main.htm\">Return</A>");
     } else {

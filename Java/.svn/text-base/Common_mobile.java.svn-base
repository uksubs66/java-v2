...
public class Common_mobile {
 ...
 public static void doSlot(parmClub parm, parmCourse parmc, parmMobile parmM, PrintWriter out, Connection con) {
   ...
   if (parmM.type.equals("lottery")) {     // if lottery request      
      ...
      x = 0;           // X not allowed on lottery requests
   ...
   //
   //  Build the page that solicits the plaer info for the tee time request
   //
   out.println("<HTML>");
   out.println("<HEAD>");
   ...
   if (parmM.type.equals("lottery")) {     // if lottery request      
      out.println("<form action=\"/" +rev+ "/servlet/Member_lott\" method=\"post\" name=\"playerform\" onsubmit=\"scrubOptions()\">");
   } else {            // tee time    
      out.println("<form action=\"/" +rev+ "/servlet/Member_slot\" method=\"post\" name=\"playerform\" onsubmit=\"scrubOptions()\">");
   }
   ...
   while (i < max) {
      ...
      if (!parmM.playerA[i].equals("") && parmM.blockPA[i] == true) {    // IF option to not allow mems to change player
         ...
         if (gstCount > 0) {
            ...
            i2 = 0;
            guest = nonProGuests[i2];        // get first Guest Type (use guest string!!)
            
            while (i2 < gstCount) {

               if (parmM.playerA[i].equals(guest)) {
                  out.println("<option selected value=\"" +guest+ "\">" +guest+ "</option>");
               } else {
   ...
   //   Notes
   //
   if (parmM.hide == 0) {      // if proshop does Not wants to hide the notes, display the text box or notes
      ...
      out.println("Notes:&nbsp;<textarea name=\"notes\" value=\"\" id=\"notes\" cols=\"16\" rows=\"2\">");
   ...
   if (parmM.lottid > 0 || !parmM.type.equals("lottery")) {    // if lottery AND id has been assigned, OR NOT a lottery request
      ...
      if (parmM.index.equals( "995" )) {         // if came from Member_teelist_list
         ...
         out.println("<input type=\"hidden\" name=\"index\" value=\"" + parmM.index + "\">");
         if (!parmM.returnCourse.equals( "" )) {
            out.println("<input type=\"hidden\" name=\"course\" value=\"" + parmM.returnCourse + "\">");
         } else {
            out.println("<input type=\"hidden\" name=\"course\" value=\"" + parmM.course + "\">");
         }
      }
 ...
 public static void namePrompt(String caller, String club, String user, HttpServletRequest req, PrintWriter out, Connection con) {
   ...
   if (!letter.equals("")) {      // if letter
      ...
      if (caller.equals("lottery")) {     // if lottery request      
         ...
         out.println("<form action=\"/" +rev+ "/servlet/Member_slot\" method=\"post\" name=\"playerform\">");
      }
      ...
      try {
         ...
         out.println("<option disabled value=\" \">-select a name-</option>");
      ...
      if (use_guestdb) {
          ...
          out.println("<BR><p align=\"center\">Select an existing guest from the list, or click Add New Guest to create a new guest.<BR><BR>");
          ...
          alphaTable.guestdbList(user, 1, sess_activity_id, out, con);

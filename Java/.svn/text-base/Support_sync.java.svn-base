...
public class Support_sync extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   String support = "support";             // valid username for normal use
   String sales = "sales";                 // valid username for rtype="rosters"
   String rtype = "";             
   ...
   if (!user.equals( support ) && (rtype.equals("rosters") && !user.startsWith( sales ))) {
   ...
   if (req.getParameter("posted_scores") != null && req.getParameter("posted_scores").equals("yes")) {
      ...
      if (req.getParameter("all_clubs") == null) {
          ...
          // do all clubs
          out.println("<BR><BR>Running GHIN Handicap Download For ALL Clubs...<BR><BR>");
        ...
        if (rosters.length > 0){
            for (int i=0; i < rosters.length; i++){
                ...
                if (!displayName.startsWith("0") && !displayName.startsWith("1") && !displayName.startsWith("2") &&
                    ...
                    out.println("<tr><td>");
                    out.println("<a href=\"/" + rev + "/servlet/Support_sync?rtype=rosters&file=" + path + rosters[i] + "\" style=\"font-size:10pt; color:black\" target=\"_blank\">" + displayName + "</a>");
                    out.println("</td></tr>");
                }
            }
        }
    ...
    private void displayErrorLogs(HttpServletRequest req, HttpServletResponse resp, PrintWriter out) {
        ...
        if (req.getParameter("file") != null) { 
            try {
                ...
                out.println("<CENTER><a href=\"/" + rev + "/servlet/Support_sync?rtype=errorlogs\">Return</a></CENTER><br>");
                out.println("<pre>");
                ...
                out.println("</pre>");
                out.println("<br><CENTER><a href=\"/" + rev + "/servlet/Support_sync?rtype=errorlogs\">Return</a></CENTER><br>");
            ...
            try {
                ...
                while (rs.next()) {
                    try {
                        ...
                        if (f.exists()) {
                            out.println("<tr><td>");
                            out.println("<a href=\"/" + rev + "/servlet/Support_sync?rtype=errorlogs&file=" + f.getPath() + "\" style=\"font-size:10pt; color:black\">" + rs.getString("clubname") + "-rsync-log.txt" + "</a>");
                            out.println("</td></tr>");
                        }
    ...
    private void setAllActive(HttpServletRequest req, String user, String club, Connection con, PrintWriter out) {
        ...
        if (user.equals("support") || user.startsWith("sales")) {
            ...
            if (req.getParameter("rtype") != null && req.getParameter("rtype").equals("active")) {
                ...
                out.println("<br><br><button onclick=\"location.href='/" +rev+ "/servlet/Support_sync?rtype=active2'\">Continue</button>");

    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        configure_slot:
        {
            if (((req.getParameter("letter") != null) || (req.getParameter("buddy") != null)
                ...
                //
                Map<String, Object> avail_tee_times_map = getLotteryTimesByCourse(date, course, lottName, bln_hideFrontBack, club, out, con, (new_skin && mobile == 0));
                ...
                if (courseCount > 0 && courseL.equals("-ALL-")) {
                    try {
                        ...
                        while (rs.next()) {
                            ...
                            Map<String, Object> course_avail_tee_times_map = getLotteryTimesByCourse(date, avail_course, lottName, bln_hideFrontBack, club, out, con, (new_skin && mobile == 0));
                ...
                if (new_skin && mobile == 0) {
                    ...
                    if (lottid == 0) {
                        ...
                        // editing a non-recurring request
                        if (user.equalsIgnoreCase(orig_by)) {
                        ...
                        if (user.equalsIgnoreCase(orig_by)) {
                            slotPageParms.page_start_instructions.add("[options.lottery.intructionsOwnerRecur]");
                            
                        } else {
                            slotPageParms.page_start_instructions.add("[options.lottery.intructionsRecur]");
                            slotPageParms.page_start_instructions.add("This request was created by " + 
                                    (orig_by.startsWith("proshop") ? "the Proshop." : Utilities.getFullNameFromUsername(orig_by, con) + "."));
                        }
                    }
                    ...
                    if (lottid == 0 || user.equalsIgnoreCase(orig_by)) {
                        ...
                        if (recur_count > 0) {
                            slotPageParms.callback_form_map.put("update_recur",
                                formUtil.makeFieldMap("[options.lottery.recurUpdateMode]", "select", "", 1, 0, "", false, recur_update_map));
                        }
                    } else {
                    ...
                    if (lottid > 0 && user.equalsIgnoreCase(orig_by) && skipCancel == false && ( recur_count == 0 || recur_count > 0 && user.equalsIgnoreCase(orig_by)) ) {     
                        
                        slotPageParms.callback_button_map.put("remove", new LinkedHashMap<String, Object>());
                        slotPageParms.callback_button_map.get("remove").put("value", "[modalOptions.slotPageLoadNotification.cancelButton]");
                        slotPageParms.callback_button_map.get("remove").put("action","cancelSlot");
                    }
                    // don't show Continue button if it's a recurring request and the user did not originate
                    if ( recur_count == 0 || (recur_count > 0 && user.equalsIgnoreCase(orig_by)) ) {     
        ...
        slotPageParms.slot_submit_map.put("update_recur", "update_recur");
        ...
        if (mobile == 0) {                 // if NOT a Mobile user, must be new skin -- so use json.
            ...
            if (!skipReturns) {     // if we haven't prompted the user for additional acction
                ...
                if (updateReq == true && orig_by.equalsIgnoreCase(user)) {
                   ...
                   if (recur_count > 0) {
                       messages.add("<HR></HR>"); 
                       messages.add("<strong>ATTENTION:</strong> &nbsp;Your changes have been applied to " +recur_count+ " recurring request(s) scheduled after this one. &nbsp;");
                       messages.add("Please note that those requests have been updated to be identical to this one, including any other previous changes made to this request.");
...
*/
    ...
    private Map<String, Object> getLotteryTimesByCourse(long date, String course, String lottName, boolean bln_hideFrontBack, String club, PrintWriter out, Connection con, boolean new_skin){

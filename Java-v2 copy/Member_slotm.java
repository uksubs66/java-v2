    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        configure_slot:
        {
            ...
            if ((req.getParameter("letter") != null) || (req.getParameter("buddy") != null) || (req.getParameter("return") != null)
                ...
                slotParms.setParameterByName("time%", formUtil.getIntArrayFromReq(req, "time%", 15, 0));
                slotParms.time1 = time;
                slotParms.setParameterByName("player%", formUtil.getStringArrayFromReq(req, "player%", 25, ""));
                slotParms.setParameterByName("pcw%", formUtil.getStringArrayFromReq(req, "p%cw", 25, ""));
                slotParms.setParameterByName("p9%", formUtil.getIntArrayFromReq(req, "p9%", 25, 0));
                slotParms.setParameterByName("guest_id%", formUtil.getIntArrayFromReq(req, "guest_id%", 25, 0));
                ...
                if (slots > 5 || ((club.equals("desertmountain") || club.equals("elcaminocc")) && slots > 1)) {
            ...
            //  If Wisconsin Club (brynwoodcc), remove all mode of trans aside from 'GC', 'CAD', 'C/C', 'FC' on Sat between 7-9am from 5/1-9/30
            if (club.equals("brynwoodcc")) {
                
                if (shortDate >= 512 && shortDate <= 922 && time >= 700 && time < 900 && day_name.equals("Saturday")) {
            ...
            if (first_call == true && !slotParms.player1.equals(name) && !slotParms.player2.equals(name) && !slotParms.player3.equals(name) && !slotParms.player4.equals(name) && !slotParms.player5.equals(name)) {
                for (int i2 = 0; i2 < players_per_group; i2++) {
                    if (slotParms.getStringByName("player%", i2).equals("")) {
                        slotParms.setParameterByName("player%", name, i2);
                        slotParms.setParameterByName("pcw%", pcw, i2);
                        break;
                    }
                }
            }
            ...
            if ((club.equals("stclaircc") && course.equals("Terrace"))
                ...
                slotParms.setParameterByName("p9%", formUtil.getIntArrayFromReq(req, "_nonexistant_parameter_", 25, 1));
        ...
        */
        ...
        slotPageParms.guest_id_a = slotParms.getIntArrayByName("guest_id%");
        slotPageParms.p9_a = slotParms.getIntArrayByName("p9%");
        slotPageParms.time_a = slotParms.getIntArrayByName("time%");         // get all the tee times

        slotPageParms.player_a = slotParms.getStringArrayByName("player%");  // get all the players
        slotPageParms.pcw_a = slotParms.getStringArrayByName("pcw%");
...
//   }
            ...
            for (int i2 = 0; i2 < players; i2++) {
                ...
                out.println("&nbsp;<input type=\"text\" name=\"player" + (i2 + 1) + "\" value=\"" + slotParms.getStringByName("player%", i2) + "\" size=\"20\" maxlength=\"43\">");
                out.println("&nbsp;&nbsp;&nbsp;<select size=\"1\" name=\"p" + (i2 + 1) + "cw\">");

                out.println("<option selected value=" + slotParms.getStringByName("pcw%", i2) + ">" + slotParms.getStringByName("pcw%", i2) + "</option>");
                for (int i3 = 0; i3 < 16; i3++) {

                    if (!parmc.tmodea[i3].equals("") && !parmc.tmodea[i3].equals(slotParms.getStringByName("pcw%", i2)) && parmc.tOpt[i3] == 0) {
                        out.println("<option value=\"" + parmc.tmodea[i3] + "\">" + parmc.tmodea[i3] + "</option>");
                    }
                }
                out.println("</select>");
                if (slotParms.getIntByName("p9%", i2) == 1) {
                ...
                if (parm.members > 0) {             // if at least one member
                    ...
                    if (parm.members > 1 || !parm.gstA[0].equals("")) {  // if slot 1 is a guest or multiple members
                        ...
                        if (new_skin) {
                            ...
                            String[] player_a = parm.getStringArrayByName("player%");
                            String[] pcw_a = parm.getStringArrayByName("pcw%");
                            int[] p9_a = parm.getIntArrayByName("p9%");
                            String[] userg_a = parm.userg;
                            int[] guest_id_a = parm.getIntArrayByName("guest_id%");
                            ...
                            hidden_field_map.put("time%", parm.getIntArrayByName("time%"));
    ...
    private void sendMail(Connection con, parmSlotm parms, String user) {
        ...
        } else if (parms.club.equals("pradera") || parms.club.equals("pinery") || parms.club.equals("snoqualmieridge") || parms.club.equals("tartanfields")) {

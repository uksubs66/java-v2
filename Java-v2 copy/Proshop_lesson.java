...
//            }
 ...
 private void grpLesson(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session,
   ...
   // If we came from the Select Date page, reorganize the calDate string into the typical dates string.
   if (jump.equals("999")) {
       
       if (!calDate.equals("")) {
           String [] tmpStr = calDate.split("/");
           
           if (tmpStr.length == 3) {
               dates = tmpStr[2] + tmpStr[1] + tmpStr[0];
           }
       }
       
   } else {
       dates = req.getParameter("date");           // date of lesson book
       times = req.getParameter("time");           // time requested   
   }
   ...
   out.println("<tr><td align=\"center\"><br>");
    ...
    out.println("<table border=\"0\" bgcolor=\"#FFFFFF\" valign=\"top\">");      // buttons
      ...
      out.println("<input type=\"hidden\" name=\"proid\" value=" +id+ ">");
      if (j == 999) {
          out.println("<input type=\"hidden\" name=\"group\"\">");
      } else {
          out.println("<input type=\"hidden\" name=\"calDate\" value=\"" +calDate+ "\">");
          out.println("<input type=\"hidden\" name=\"jump\" value=\"" +j+ "\">");
      }
...
//      emailMod = 1;                   // set modified lesson
   ...
   if (skipDining.equalsIgnoreCase("yes") || msgPlayerCount == 0) {
       ...
       // If we came from the Group Lessons page (j=999) return there instead of the lesson book
       if (j == 999) {
           out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_lesson?proid=" +id+ "&group\">");
       } else {
 ...
 private void grpSelectDate(HttpServletRequest req, HttpSession session, PrintWriter out, Connection con) {
        ...
        try {
            ...
            if (diff > 0) {
                ...
                //
                out.println("<form action=\"Proshop_lesson\" method=\"post\" target=\"bot\" name=\"frmLoadDay\">");
                out.println("<input type=\"hidden\" name=\"groupLesson\" value=\"" + lname + "\">");
                out.println("<input type=\"hidden\" name=\"calDate\" value=\"\">");
                out.println("<input type=\"hidden\" name=\"proid\" value=\"" + id + "\">");
                out.println("<input type=\"hidden\" name=\"lesson_id\" value=\"" + lesson_id + "\">");
                out.println("<input type=\"hidden\" name=\"jump\" value=\"999\">");
...
//         out.println("<input type=\"hidden\" name=\"date\" value=\"" +dates+ "\">");        // shouldn't need this
 ...
 private void displayGroup(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session,
   ...
   int etime = 0;
   int count = 0;
   int count2 = 0;                     // init column counter
   int lesson_id = 0;
   int clinic = 0;
   int advDays = 180;
   ...
   //  Build the html page to request the Group Lesson
   //
   out.println(SystemUtils.HeadTitle("Proshop - Select Group Lesson"));
   out.println("<body bgcolor=\"#FFFFFF\" text=\"#000000\">");
   SystemUtils.getProshopSubMenu(req, out, lottery);            // required to allow submenus on this page
   out.println("<font face=\"Verdana, Arial, Helvetica, Sans-serif\">");
   out.println("<center>");
   ...
   try {
      ...
      if (count == 0) {       // if none found
         out.println("<p>Sorry, no Group Lessons were found for this pro during the next " + advDays + " days.</p>");
         out.println("<br>");
      }

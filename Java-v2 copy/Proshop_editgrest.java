...
public class Proshop_editgrest extends HttpServlet {
 ...
 private void updateConf(HttpServletRequest req, PrintWriter out, HttpSession sess) {
   ...
   if (!oldName.equals( name )) {    // if name has changed
      ...
      try {
         ...
         pstmt = con.prepareStatement (
                 "SELECT sdate FROM guestres2 WHERE name = ? AND activity_id = ?");
   ...
   // IF THIS IS A NEW RESTRICTION (NO ID PASSED) THEN LETS ADD IT NOW
   ...
   if (grest_id == 0) {
       ...
       try {
          ...
          pstmt = con.prepareStatement (
            "INSERT INTO guestres2 (" +
                "name, sdate, start_mm, start_dd, start_yy, start_hr, start_min, " +
                "stime, edate, end_mm, end_dd, end_yy, end_hr, end_min, etime, recurr, num_guests, per, " +
                "courseName, fb, color, activity_id, locations) " +
            "VALUES (" +
                "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
          ...
          pstmt.setInt(17, Integer.parseInt(guests));
          pstmt.setString(18, per);
          pstmt.setString(19, course);
          pstmt.setString(20, fb);
          pstmt.setString(21, color);
          ...
          rs = pstmt.executeQuery();
          if (rs.next()) grest_id = rs.getInt(1);

          pstmt = con.prepareStatement (
              "INSERT INTO guestres2_gtypes (id, guestres_id, guest_type) VALUES (NULL, ?, ?)");
       ...
       out.println("<BR><BR><H3>Guest Restriction Has Been Added</H3>");
       out.println("<BR><BR>Thank you, the restriction has been added to the system database.");
       out.println("<BR><BR>");
       out.println("<font size=\"2\">");
       out.println("<form method=\"get\" action=\"Proshop_grest?new\">");
       out.println("<input type=\"submit\" value=\"Return\" style=\"text-decoration:underline; background:#8B8970\">");
       out.println("</form></font>");
       out.println("</CENTER></BODY></HTML>");
       out.close();
       ...
       out.println("Please confirm the following parameters for the restriction:<br><font size=3><b>" + name + "</b></font><br><br>");
       ...
       out.println("<form action=\"Proshop_editgrest\" method=\"post\" target=\"bot\">");
       ...
       out.println("<form method=\"get\" action=\"Proshop_grest\">");
 ...
 private void removeGrest(HttpServletRequest req, PrintWriter out, HttpSession sess) {
   ...
   try {
      ...
      //
      pstmt = con.prepareStatement (
               "DELETE FROM guestres2 WHERE id = ?");
      ...
      if (count > 0) {      // if the restriction was deleted
          ...
          //
          pstmt = con.prepareStatement (
                  "DELETE FROM guestres2_gtypes WHERE guestres_id = ?");
          pstmt.clearParameters();
          pstmt.setInt(1, grest_id);
          pstmt.executeUpdate();
   ...
   out.println("<BR><BR>Thank you, the restriction \"" + name + "\" has been removed from the database.");
   out.println("<BR><BR>");
   out.println("<font size=\"2\">");
   ...
   try {
      ...
      pstmt = con.prepareStatement (
        ...
        "SET sdate = ?, start_mm = ?, start_dd = ?, start_yy = ?, " +
            ...
            "end_hr = ?, end_min = ?, etime = ?, recurr = ?, num_guests = ?, per = ?, " +
            "courseName = ?, fb = ?, color = ?, name = ?, locations = ? " +
        "WHERE id = ?"); 

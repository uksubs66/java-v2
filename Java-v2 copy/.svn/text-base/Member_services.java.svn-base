...
public class Member_services extends HttpServlet {
 ...
 //  boolean new_skin = (sess_activity_id == ProcessConstants.DINING_ACTIVITY_ID);
   ...
   if (!club.equals( "interlachenspa" ) && foretees_mode != 0) {  // skip if Interlachen Spa
    ...
    if (!dining) {      // these components are built in Dining_home for the dining system
      ...
      if (caller.equals( "none" ) || caller.startsWith("foretees")) {
            ...
            out.println("	$('#password').keyup(function(){");
            out.println("		$('#result').html(passwordStrength($('#password').val(),$('#username').val(),$('#lastname').val())) ;");
            out.println("		perc = passwordStrengthPercent($('#password').val(),$('#username').val(),$('#lastname').val());");
   ...
   //      out.println("	}");
      ...
      out.println("<div class=\"main_instructions\"><strong>Instructions</strong>:  Use this page to change your personal settings. Only change the fields you want to update. Click the 'Update' button to save the changes.</div>");          
    }
    ...
    out.println("<form action=\"Member_services\" method=\"post\" name=\"f\" id=\"f\">");
    ...
    if (caller.equals( "none" ) || caller.startsWith("foretees")) {   // if user did not come from MemFirst or other web site
        ...
        out.println("<td><input type=\"password\" name=\"oldpw\" id=\"oldpw\" size=\"20\" maxlength=\"15\"></td>");
        out.println("<td></td>");
        out.println("</tr><tr>");
        out.println("<td>New Password:</td>");
        out.println("<td nowrap><input type=\"password\" name=\"newpw\" id=\"password\" size=\"20\" maxlength=\"15\"></td>");
        out.println("<td>");
        ...
        out.println("<td><input type=\"password\" name=\"newpwc\" id=\"newpwc\" size=\"20\" maxlength=\"15\"></td>");
        out.println("<td></td>");
        out.println("</tr>");
        out.println("</table>");
    ...
    out.println("<tr><td>"); // <font size=\"2\">
        ...
        boolean static_emails = (rsync == 1 &&
         ...
         if (static_emails) {
               out.println("<td><strong>" + email +"<strong><input type=\"hidden\" name=\"email\" value=\"" + email + "\">&nbsp;&nbsp;</td>");
         } else {
               out.println("<td><input type=\"text\" name=\"email\" size=\"28\" maxlength=\"50\" value=\"" + email +"\"" + ((email_bounced!=0) ? " style=\"background-color: red\"" : "") + ">&nbsp;&nbsp;</td>");
         }
         if (static_emails) {
               out.println("<td><strong>" + email2 +"<strong><input type=\"hidden\" name=\"email2\" value=\"" + email2 + "\">&nbsp;&nbsp;</td>");
         } else {
               out.println("<td><input type=\"text\" name=\"email2\" size=\"28\" maxlength=\"50\" value=\"" + email2 +"\"" + ((email2_bounced!=0) ? " style=\"background-color: red\"" : "") + ">&nbsp;&nbsp;</td>");
         }
    ...
    out.println("<tr><td>");
      ...
      if (club.equals("hawkslandinggolfclub") || caller.equals("FLEXWEBFT")) {
         ...
         out.println("<td><input type=\"text\" name=\"phone1\" size=\"14\" maxlength=\"24\" value=\"" + phone1 +"\"></td>");
         out.println("</tr><tr>");
         out.println("<td>Alternate Number:&nbsp;</td>");
         out.println("<td><input type=\"text\" name=\"phone2\" size=\"14\" maxlength=\"24\" value=\"" + phone2 +"\"></td>");
    ...
    out.println("<tr><td><hr width=\"75%\" align=\"center\"><BR></td></tr>"); 
         ...
         if (foretees_mode != 0 && !dining) {            // if foretees golf in system and NOT dining
            ...
            if ( activity_system == true ) {  // AND Activities - add row for group title
                  ...
                  out.println("<tr><td><br><strong>Golf Options:</strong></td></tr>");
            }
            ...
            if (IS_TLT == false && sess_activity_id == 0) {      // DO NOT do this for Notification systems
                 ...
                 out.println("<select size=\"1\" name=\"tee_sheet_jump\">");
      ...
      //  if (IS_TLT == false && allowMobile == true) {      // DO NOT do this for Notification systems
        if (IS_TLT == false && allowMobile == true && (sess_activity_id == 0 || club.equals("philcricket"))) {      // TEMP - remove later (use above if)
...
/*
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   //}
   out.println("<input type=\"submit\" value=\"Return\" id=\"submit\">");
   out.println("</form>");

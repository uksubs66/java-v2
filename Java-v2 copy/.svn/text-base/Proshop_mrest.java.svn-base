 ...
 *   last updated:
 *
 *        1/13/14   Add 'Delete' button to event list.  Also, change 'Select' button to 'Edit'.
 *       12/19/13   Updated Start Year and End Year select objects to always print this year and then the following 5 years as options, as well as that mrest's set year, if prior to the current year.
 *        1/21/13   Fixed page displaying vertical alignment oddly on IE.
 *       11/27/12   Tweak iframe resize code
 *       10/14/10   Enable suspensions for FlxRez
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   try {
    ...
    out.println("<tr><td align=\"center\">");
      ...
      out.println("<br>To change or remove a restriction, click on the Edit or Delete button within the restriction.");
   ...
   try {
      ...
      while (rs.next()) {
         ...
         out.println("<form method=\"post\" action=\"Proshop_mrest\" target=\"bot\">");
         ...
         out.println("<td align=\"center\" valign=\"center\"><input type=\"submit\" name=\"Delete\" value=\"Delete\" onclick=\"return confirmDelete()\"></form></td></tr></table>");
               out.println("<script type='text/javascript'>");
               out.println("function confirmDelete() {");
               out.println("ret = confirm('WARNING! You are about to PERMANENTLY delete a Member Restriction!\\n\\nAre you sure you want to continue?');");
...
*/
    ...
    out.println("<p align=\"left\">&nbsp;&nbsp;Membership Types to be Restricted (select all that apply):<br>");
       ...
       out.println("<br><br>&nbsp;&nbsp;Show this restriction on the " + ((sess_activity_id == 0) ? "Tee" : "Time") + " Sheets?:&nbsp;&nbsp;");
       ...
       if (id > 0) {
           ...
           out.println("<iframe id=\"suspiframe\" src=\"Proshop_mrest?susp&mrestid=" + id + "&name=" + name + "&avail_locations=" + locations_csv + "\" style=\"width:800px\" scrolling=no frameborder=no></iframe>");
       }
   ...
   out.println("document.getElementById('suspiframe').height = divHeight;"); // was style.height
   out.println("}");
   out.println("// -->");
 ...
 private void doCopy(HttpServletRequest req, PrintWriter out, Connection con, int lottery, int activity_id) {
   ...
   try {
      ...
      if (b == true) {
         ...
         out.println("<form action=\"Proshop_mrest\" method=\"post\" target=\"bot\">");
 ...
 private void printSuspensions(HttpServletRequest req, int sess_activity_id, Connection con, PrintWriter out) {
     ...
     String eyear = "";
     String locations_csv = "";
     String avail_locations = (req.getParameter("avail_locations") != null) ? req.getParameter("avail_locations") : "";
     
     boolean suspExists = false;
     
     if (req.getParameter("updateSusp") != null) {
         updateSuspension(req, sess_activity_id, con, out);
         return;
     }
     ...
     out.println("<td align=\"center\"><b>" + ((sess_activity_id == 0) ? "Course" : "Locations") + "</b></td>");
...
/*         pstmt = con.prepareStatement("" +
                 "SELECT rs.*, r.locations AS avail_locations " +
                 "FROM rest_suspend rs " +
                 "LEFT OUTER JOIN restriction2 r ON r.id = rs.mrest_id " +
                 "WHERE rs.mrest_id = ? " +
                 "ORDER BY id");*/
         ...
         while (rs.next()) {
             ...
             courseName = rs.getString("courseName");
             locations_csv = rs.getString("locations");
             //avail_locations = rs.getString("avail_locations");
             ...
             out.println("<form method=\"post\" action=\"Proshop_mrest?susp\">");
         ...
         out.println("<!-- avail_locations=" + avail_locations + " -->"); 
         ...
         if (req.getParameter("id") != null && req.getParameter("returning") == null) {
             ...
             // user just clicked on a suspension to edit it
...
//                   "SELECT * FROM rest_suspend WHERE id = ?");
             ...
             locations_csv = (req.getParameter("locations_csv") != null) ? req.getParameter("locations_csv") : "";
             //avail_locations = (req.getParameter("avail_locations") != null) ? req.getParameter("avail_locations") : "";
         ...
         if (sess_activity_id == 0) {
            ...
            out.println("&nbsp;&nbsp;Choose the locations to suspend this Restriction:&nbsp;&nbsp;");
            Common_Config.displayActivitySheetSelect(locations_csv, avail_locations, sess_activity_id, false, con, out);

            out.println("<input type=\"hidden\" name=\"avail_locations\" value=\"" + avail_locations + "\">");
         ...
         out.println("&nbsp;&nbsp;&nbsp;<a href=\"Proshop_mrest?susp&mrestid=" + mrest_id + "&avail_locations=" + avail_locations + "\" style=\"text-decoration: none;\"><button type=\"button\">Reset</button></a>");
 ...
 // *********************************************************
 private void updateSuspension(HttpServletRequest req, int sess_activity_id, Connection con, PrintWriter out) {
     ...
     if (sess_activity_id != 0) {
         ...
         avail_locations = req.getParameter("avail_locations");
     }

     name = req.getParameter("name");
     courseName = (req.getParameter("course") == null) ? "" : req.getParameter("course");
         ...
         try {
             if (id >= 0) {         // suspension exists, perform an update
                 ...
                 pstmt = con.prepareStatement("UPDATE rest_suspend SET " +
                         ...
                         "thursday = ?, friday = ?, saturday = ?, locations = ? " +
                         "WHERE id = ?");
                 pstmt.clearParameters();
                 ...
                 pstmt.setString(14, locations_csv);
                 pstmt.setInt(15, id);
                 ...
                 pstmt = con.prepareStatement("INSERT INTO rest_suspend " +
                         ...
                         "wednesday, thursday, friday, saturday, locations) " +
                         "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                 pstmt.clearParameters();
                 pstmt.setInt(1, mrest_id);
                 ...
                 pstmt.setString(15, locations_csv);
                 count = pstmt.executeUpdate();

             }

             String url = "Proshop_mrest?susp&mrestid=" + mrest_id + "&name=" + name + "&avail_locations=" + avail_locations;
 ...
 private void deleteSuspension(HttpServletRequest req, Connection con, PrintWriter out) {
     ...
     if (req.getParameter("id") != null) {
         ...
         try {
             ...
             String url = "Proshop_mrest?susp&mrestid=" + mrest_id + "&name=" + name;
 ...
 private String getRecurrence(ResultSet rs, PrintWriter out) {
     ...
     try {
         ...
         if (rs.getInt("sunday") != 0 && rs.getInt("monday") != 0 && rs.getInt("tuesday") != 0 &&
             rs.getInt("wednesday") != 0 && rs.getInt("thursday") != 0 && rs.getInt("friday") != 0 &&
             rs.getInt("saturday") != 0) {

             recurr = "Every Day";

         } else if (rs.getInt("monday") != 0 && rs.getInt("tuesday") != 0 &&
             rs.getInt("wednesday") != 0 && rs.getInt("thursday") != 0 && rs.getInt("friday") != 0) {

             recurr = "Every Weekend";
             
         } else if (rs.getInt("saturday") != 0 && rs.getInt("sunday") != 0) {

...
*/
   ...
   if (sess_activity_id != 0) {
       ...
       out.println("<font size=\"2\">");
       out.println("http://<input type=\"text\" name=\"website_url\" value=\"" + website_url + "\" size=\"35\" maxlength=\"255\">");
       out.println("</font>");
       out.println("</td>");
       out.println("</tr><tr>");
...
*/
      ...
      try {
          ...
          // NOTE: Support_cluboptions now builds the initial entry at init time - so just update and let it fail if nothing to update
            ...
            PreparedStatement pstmt = con.prepareStatement (
               ...
               "website_url = ?, allow_mobile = ?, peer_review = ?, hdcp_memview = ?, boxgroove = ?, boxUser = ?, boxPW = ?" +
            ...
            pstmt.setInt(32, dining);
            pstmt.setInt(33, guestdb);
            pstmt.setString(34, website_url);
            pstmt.setInt(35, allow_mobile);
            pstmt.setInt(36, peer_review);
            pstmt.setInt(37, hdcp_memview);
            pstmt.setInt(38, boxgroove);
            pstmt.setString(39, boxUser);
            pstmt.setString(40, boxPW);
            if (updatePass) pstmt.setString(41, newPass);
   ...
   if (req.getParameter("step2") != null) {
     ...
     //     (posType.equals( "TAI Club Management" ) && club.equals("bhamcc"))) {         // if TAI, IBS or CSG, check for '$' in fee values (not allowed)
      if (posType.equals( "IBS" ) || posType.equals( "ClubSystems Group" )) { 
            ...
            for (i = 0; i < guests; i++) {
               ...
               if (!guestN[i].equals( "" )) {     // if guest type specified
                  ...
                  if (found == true) {       // if found, update it, else add it
                     ...
                     stmt = con.prepareStatement (
                        "UPDATE guest5 SET " +
                        "gOpt = ?, gpos = ?, g9pos = ?, gstItem = ?, gst9Item = ?, revenue = ?, use_guestdb = ?, salestax = ? " +
                     ...
                     stmt.setString(9, guestN[i]);      // for this guest type
                     stmt.setInt(10, sess_activity_id);
                     ...
                     stmt = con.prepareStatement (
                        ...
                        "gpos, g9pos, gstItem, gst9Item, revenue, use_guestdb, salestax) " +
                        "VALUES (?,?,?, " +
                        "?,?,?,?,?,?,?)");
   ...
   if (req.getParameter("step4") != null) {
     ...
     // if (posType.equals( "IBS" ) || posType.equals( "ClubSystems Group" ) || 
     //     (posType.equals( "TAI Club Management" ) && club.equals("bhamcc"))) {         // if IBS or CSG, check for '$' in fee values (not allowed)
      if (posType.equals( "IBS" ) || posType.equals( "ClubSystems Group" )) { 

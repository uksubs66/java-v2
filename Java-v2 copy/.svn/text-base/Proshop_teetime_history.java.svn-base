 ...
 *   Revisions:     
 *
 *       10/22/13   Added an additional column to display whether the tee time was made private, and which option was selected.
 *        7/12/13   Added an additional column to display whether emails were suppressed for each update to the tee time.
 *        8/25/10   Change the order to use sdate instead of time when querying teehist.  The time value is that
 *                  of the tee time, so it is always the same value.
...
public class Proshop_teetime_history extends HttpServlet {
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    out.println("<b>Tee Time Date:</b>&nbsp;&nbsp;" + month + "/" + day + "/" + year);
    ...
    try {
        ...
        PreparedStatement pstmtc = con.prepareStatement (
            "SELECT type, sdate, mname, user, player1, player2, player3, player4, player5, email_suppressed, make_private " + 
            "FROM teehist " +
...
//            "ORDER BY mdate DESC, time DESC");
        ...
        while (rs.next()) {
           ...
           if (h == false) {
              out.println("<tr bgcolor=\"#336633\" style=\"color: white\"><td nowrap><b>Date & Time</b></td><td><b>Name</b></td><td><b>User</b></td><td nowrap><b>Player 1</b></td><td nowrap><b>Player 2</b></td><td nowrap><b>Player 3</b></td><td nowrap><b>Player 4</b></td><td nowrap><b>Player 5</b></td><td nowrap><b>Emails Suppressed?</b></td><td nowrap><b>Private?</b></td></tr>");
              h = true;
           }
           ...
           if (rs.getInt("make_private") == 1) {
               make_private = "Hide Names";
           } else if (rs.getInt("make_private") == 2) {
               make_private = "Hide Tee Time";
           } else {
               if (ProcessConstants.isProshopUser(rs.getString("user"))) {
                   make_private = "No";
               } else {
                   make_private = "N/A";
               }
           ...
           out.println("<tr bgcolor=\"" + bgcolor + "\">");
            ...
            out.print("<td nowrap>" + rs.getString("mname") + "</td>"); // mname
            out.print("<td nowrap>" + rs.getString("user") + "</td>"); // username
            out.print("<td nowrap>" + rs.getString("player1") + "</td>"); // player1
            out.print("<td nowrap>" + rs.getString("player2") + "</td>"); // player2
            out.print("<td nowrap>" + rs.getString("player3") + "</td>"); // player3
            out.print("<td nowrap>" + rs.getString("player4") + "</td>"); // player4
            out.print("<td nowrap>" + rs.getString("player5") + "</td>"); // player5
            out.print("<td nowrap>" + email_suppressed + "</td>"); // email suppressed?
            out.print("<td nowrap>" + make_private + "</td>"); // tee time private?
           out.println("</tr>");
        ...
        if (i==0 && !IS_TLT) {
            ...
            out.println("<p align=center><i><b>"+ i +" historical " + ((i>1) ? "entries" : "entry") + " found for this tee time.</b></i></p>");

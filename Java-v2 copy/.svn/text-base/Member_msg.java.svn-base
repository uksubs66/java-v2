...
public class Member_msg extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (req.getParameter("user") != null) {
      ...
      try {
         ...
         pstmt = con.prepareStatement (
            ...
            "mobile_count, mobile_iphone, msub_type " +
    ...
    //session.setAttribute("new_skin", (new_skin) ? "1" : "0");  // new skin flag
      ...
      if (mobile == 0) {
          ...
          if (new_skin) {
             ...
             if ((!email.equals( "" ) || rsync == 1) && message.equals( "" ) && email_bounced == 0 && email2_bounced == 0) {
                if (default_activity_id == dining_activity_id) {     // if dining
                   out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Dining_home\">");
                } else {
          ...
          if (email_bounced == 1 || email2_bounced == 1) {
         ...
         } else if (email.equals( "" ) && rsync == 0 && (!club.equals("rehobothbeachcc") || count <= 2)) {
            ...
            if (new_skin) out.println("</div>");     // close out the sub_instructions above

            out.println("</td></tr></table><br>");

            if (last_message.equals( "" ) && (!aolEmailUser || !displayAolNotice)) {        // if no message sent to this user yet 

               addMsg(new_skin, out, con);                    // send msg #1
            }
         ...
         //
         Login.promptMobileUser(club, name, session, out, con);   // prompt mobile user to see how they want to proceed
      ...
      out.close();
    ...
    } else if (req.getParameter("useStandard") != null) {     // if switching an iPad user
        ...
        if (new_skin) {
            ...
            // new skin will skip this page and simply diect the user to the next page
          ...
          out.println("<HEAD>");
             ...
             if (default_activity_id == dining_activity_id) {     // if dining
                ...
                out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Member_announce\">");
             } else {
                out.println("<meta http-equiv=\"Refresh\" content=\"1; url=/" + rev + "/member_welcome.htm\">");
             }
          out.println("</head>");
          ...
          out.println("<br>Thank you.  We will now route you to your standard ForeTees site.");

          if (new_skin) out.println(" Using the new skin.");

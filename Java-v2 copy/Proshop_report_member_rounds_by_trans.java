/***************************************************************************************
 *   Proshop_report_member_rounds_by_trans: This servlet will ouput the member rounds report
 *
 *
 *   Called by:     manually - custom report but can be run for any club !!!!!!
 *
 * 
 *                  This report would take up too many server resources, so we do not want to make it
 *                  available to all clubs!!!!
 * 
 *
 *   Created:       1/14/2009 by Bob
 *
 *
 *   Last Updated:
 *
 *   2/09/12   Added the 'email' parm to include the members' email and phone info.
 *  10/20/11   Added an option to breakdown each mode of transportation by 9 and 18 hole rounds.  Pass parameter "breakdown" to activate this.
 *   9/29/11   Fixed processing to look for first matching MoT instead of first non-matching when looping through the MoTs for a given round.
 *
 * 
 * 
 *  Below are the fields that are displayed as a default.  Specifying &breakdown will split the MOTS into 9 & 18 hole counts.
 * 
 *    Member Name   Membership   Member Type   	Member Number 	9 Hole 	18 Hole   CAD 	MCT 	GCT 	PC 	WLK 
 * 
 *                  
 ***************************************************************************************
 */
...
public class Proshop_report_member_rounds_by_trans extends HttpServlet {
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    boolean add_email = false;
    
    if (req.getParameter("breakdown") != null) {     // Pass breakdownMoT parameter to breakdown modes of transportation into 9 and 18 hole rounds
        breakdown = true;
    }
    
    if (req.getParameter("email") != null) {     // Include email and phone info?
        add_email = true;
    }
    ...
    if (req.getParameter("continue") == null) {
       ...
       displayWarning(period, breakdown, add_email, out);              // display warning message
    ...
    try {
       ...
       getParms.getTmodes(con, parmcs, no_course);        // get the course parms (modes of trans)
    }
    catch (Exception ignore) {
    }
         
   tmloop1:
   for (i=0; i<parmcs.tmode_limit; i++) {         // count the # of modes specified for this club
   ...
   int [] modecA = new int [modesc];         // array to hold the counts for each mode
    
   int [] modecTA = new int [modesc];        // array to hold the total counts for each mode
   
   int [] modec9A = new int [modesc];        // array to hold 9 hole counts for each mode
   
   int [] modec18A = new int [modesc];       // array to hold 18 hole counts for each mode
   ...
   /*
    ...
    sdate = 20130101;          // change these dates as needed and un-comment this block - the rest will work as is.
    edate = 20131231;
    */
    //                   END OF CUSTOM
    ...
    if (!excel.equals("yes")) {                // if not Excel Spreadsheet Format
       ...
       out.println("</td>");
       out.println("<form method=\"post\" action=\"Proshop_report_member_rounds_by_trans\" target=\"_blank\">");
       out.println("<input type=\"hidden\" name=\"excel\" value=\"yes\">");
       out.println("<input type=\"hidden\" name=\"period\" value=\"" + period + "\">");
       if (breakdown) out.println("<input type=\"hidden\" name=\"breakdown\">");
    ...
    out.println("<tr bgcolor=\"#336633\" style=\"font-family:verdana;color:white;font-size:.9em\">" +
                ...
                out.println("<td><b>9 Hole&nbsp;</b></td>" +
                "<td><b>18 Hole&nbsp;</b></td>");
                
                  for (i=0; i<modesc; i++) {     // add hdng for each mode

                     if (!parmcs.tmodea[i].equals( "" )) {
                        
                        out.println("<td><b>" + parmcs.tmodea[i] + "&nbsp;</b></td>");
                        
                        if (breakdown) {
                            out.println("<td><b>" + parmcs.tmodea[i] + " - 9h&nbsp;</b></td>");
                            out.println("<td><b>" + parmcs.tmodea[i] + " - 18h&nbsp;</b></td>");
                        }
                     }
                  }             
                out.println("</tr>");
    ...
    try {
        ...
        sql = "SELECT username, name_last, name_first, name_mi, m_ship, m_type, memNum, email, phone1 FROM member2b ORDER BY name_last, name_first, name_mi";
        ...
        while (rs1.next()) {         // check each member
            ...
            for (i=0; i<modesc; i++) {     // init counter for each mode
            ...
            while (rs2.next()) {
               ...
               if (user1.equals(username) && show1 == 1) {       // if this user and checked in
                  ...
                  loop1:
                  for (i=0; i<modesc; i++) {         // locate the matching mode of trans

                     if (parmcs.tmodea[i].equals( p1cw )) {
               ...
               if (user2.equals(username) && show2 == 1) {       // if this user and checked in
                  ...
                  for (i=0; i<modesc; i++) {         // locate the matching mode of trans
                     ...
                     if (parmcs.tmodea[i].equals( p2cw )) {
               ...
               if (user3.equals(username) && show3 == 1) {       // if this user and checked in
                  ...
                  for (i=0; i<modesc; i++) {         // locate the matching mode of trans
                     ...
                     if (parmcs.tmodea[i].equals( p3cw )) {
               ...
               if (user4.equals(username) && show4 == 1) {       // if this user and checked in
                  ...
                  for (i=0; i<modesc; i++) {         // locate the matching mode of trans
                     ...
                     if (parmcs.tmodea[i].equals( p4cw )) {
               ...
               if (user5.equals(username) && show5 == 1) {       // if this user and checked in
                  ...
                  for (i=0; i<modesc; i++) {         // locate the matching mode of trans
                     ...
                     if (parmcs.tmodea[i].equals( p5cw )) {
            ...
            if (p9 > 0 || p18 > 0 ) {
                 ...
                 if (add_email == true) {
                     out.println("<td align=\"center\">" + email + "</td><td align=\"center\">" + phone + "</td>");
                 }

                 out.println("<td align=\"center\">" + p9 + "</td><td align=\"center\">" + p18 + "</td>");

                  for (i=0; i<modesc; i++) {     // add hdng for each mode

                     out.println("<td align=\"center\">" + modecA[i] + "</td>");
                     
                     if (breakdown) {                        
                         out.println("<td align=\"center\">" + modec9A[i] + "</td>");
                         out.println("<td align=\"center\">" + modec18A[i] + "</td>");
                     }
                  }                                         
    ...
    // output the Totals row
     ...
     out.println("<td>&nbsp;</td><td>&nbsp;</td><td align=\"center\">&nbsp;</td>" +
      ...
      for (i=0; i<modesc; i++) {     // add hdng for each mode
         ...
         out.println("<td align=\"center\">" + modecTA[i] + "</td>");
      }                                         
     out.println("</tr>");    // end of this row (one per member)
...
//   Warning Message
//
 private void displayWarning(String period, boolean breakdown, boolean email, PrintWriter out) {
    ...
    out.println("<BR><BR>");
    out.println("<form method=\"post\" action=\"Proshop_report_member_rounds_by_trans\">");
    out.println("<input type=\"hidden\" name=\"period\" value=\"" +period+ "\">");
    if (breakdown) out.println("<input type=\"hidden\" name=\"breakdown\">");
    if (email) out.println("<input type=\"hidden\" name=\"email\">");
    out.println("<input type=\"hidden\" name=\"continue\" value=\"yes\">");

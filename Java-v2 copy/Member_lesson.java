 ...
 *   last updated:
 *
 *      3/03/14  Remove old skin code, new_skin boolean, and add systemLingo for personal scheduling system.
...
public class Member_lesson extends HttpServlet {
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   if (availCount > 0) {      // if any available times
      ...
      out.println("    &nbsp;&nbsp;&nbsp;&nbsp;Date:&nbsp;&nbsp;<b>" + dayName + "&nbsp;&nbsp;" + month + "/" + day + "/" + year + "</b></p>");
      } else {
         ...
         if (earlyTimes == false) {
            ...
            if (club.equals( "deserthighlands" )) {
               ...
               if (sess_activity_id > 0) {
                  out.println("    Please contact the Pro Shop if you wish to schedule a " + sysLingo.TEXT_lesson + " for this date.<br /><br /></p>");
               } else {
                  out.println("    Please contact the Golf Shop if you wish to schedule a " + sysLingo.TEXT_lesson + " for this date.<br /><br /></p>");
               }
            }
         }
      }
   ...
   out.println("    <a class=\"standard_button\" href=\"Member_lesson?proid=" +id+ "\">Return</a>");
   out.println("  </div> ");
   Common_skin.outputPageEnd(club, sess_activity_id, out, req);
   out.close();
 ...
 private String promptProId(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session, Connection con)
   ...
   if (club.equals("championhills") || club.equals("avonoakscc") || ((club.equals("cranecreek") || club.equals("pointlake") || club.equals("fortcollins")) && sess_activity_id == 1)) {
      ...
      if (club.equals("championhills")) {
          ...
          custMsg = "Please call Fort Collins Country Club Tennis Shop to schedule a private or group lesson. (970) 482-1422";
      ...
      if (club.equals("fortcollins")) {
         out.println("  <br /><h3 class=\"lesson_ctr\">Online Scheduling Unavailable</h3>");
   ...
   if (club.equals("tcclub")) {
      ...
      try {
          ...
          if (rs.next()) {
              ...
              out.println("  <br /></p> <br />");
              out.println("  <p class=\"lesson_ctr\">");
              out.println("  <form method=\"post\" action=\"https://www.securedata-trans7.com/ap/thecountryclub/index.php?page=10\" target=\"_blank\">");
   ...
   if (club.equals( "interlachenspa" )) {
      ...
      if (sess_activity_id != 0) {
         if (req.getParameter("group") == null) {
            out.println("    <br />To schedule a " +sysLingo.TEXT_lesson_reservation+ ", click on the desired Professional from the list below.");
         } else {
            out.println("    <br />To view available Group Lessons and Clinics, click on the desired Professional from the list below.");
         }
      } else {
         if (req.getParameter("group") == null) {
            out.println("    <br />To schedule a " +sysLingo.TEXT_lesson_reservation+ ", click on the desired " +sysLingo.TEXT_Lesson_Pro+ " from the list below.");
         } else {
 ...
 private void promptType(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session,
   ...
   if (club.equals( "minikahda" ) || club.equals( "ncrcc" ) || club.equals( "tcclub" ) || club.equals("paloaltohills")/* || club.equals("lakesclub")*/) {
   ...
   //
   if (club.equals( "cherryhills" ) || club.equals( "royaloakscc" ) || club.equals("charlottecc")) {
   ...
   try {
         ...
         while (rs.next()) {
            ...
            date_map.put(record_key, new LinkedHashMap<String, Object>());
            //date_map.get(record_key).put("date", record_key);
            //date_map.get(record_key).put("tooltip", "This is a tooltip for "+record_key );
            date_map.get(record_key).put("status", rs.getInt("status"));
            //date_map.get(record_key).put("proid", id);
   ...
   out.println("    alert(\"Please select a " + sysLingo.TEXT_lesson + " type before clicking date.\");");
   out.println("  }");    
   out.println("}");               
   ...
   if (club.equals("columbine") && id == 7) {
      ...
      if (club.equals( "interlachenspa" )) {
         ...
         out.println("    <br />Select a " + sysLingo.TEXT_Lesson + " Type from the list below and then click on the day desired.");
      }
      out.println("  </div>");
      if (club.equals( "interlachenspa" )) {
   ...
   try {
      ...
      if (!club.equals( "interlachen" )) {
         ...
         out.println("      </tr>--><tr>");
         out.println("        <th class=\"individual_lesson_type\">" + sysLingo.TEXT_Individual_Lesson + " Type</th>");
         if (!club.equals("ballantyne") || sess_activity_id != 1) {
            out.println("        <th class=\"individual_lesson_desc\">Description</th>");
         }
         ...
         while (rs.next()) {
            ...
            out.println("      <tr>");
            out.println("        <td class=\"individual_lesson_type\">");
            out.println("        <input type=\"radio\" name=\"ltype\" value=\"" +ltname+ "\" onclick=\"setLessonSel()\"  /> " +ltname + "</td>");
            if (!club.equals("ballantyne") || sess_activity_id != 1) {
               out.println("        <td class=\"individual_lesson_desc\">" +desc+ "</td>");
            }
         ...
         out.println("    <table class=\"standard_list_table\">");
         out.println("      <thead><tr>");
         out.println("        <th>Individual Lesson Types  (Select One)</th>");
         out.println("      </tr><tr>");
         out.println("        <th  class=\"individual_lesson_type\">Individual Lesson Type</th>");
         out.println("      </tr><thead>");
         out.println("      <tbody>");
 ...
 private void displayGroup(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session,
   ...
   try {
      ...
      if (rs.next()) {
          ...
          // build the name
         StringBuffer pro_name = new StringBuffer(rs.getString("fname"));
   ...
   out.println("    </tr><tr>");
   out.println("      <th class=\"group_lesson_name\">Lesson Name</th>");
   if (sess_activity_id != 0) {
      out.println("      <th class=\"group_lesson_date\">Dates</th>");
   } else {
      out.println("      <th class=\"group_lesson_date\">Date</th>");
   }
   out.println("      <th class=\"group_lesson_start_end\">Start</th>");
   out.println("      <th class=\"group_lesson_start_end\">End</th>");
   out.println("      <th>Group Size</th>");
   out.println("      <th>Price</th>");
   out.println("      <th class=\"group_lesson_desc\">Description</th>");
   out.println("      <th></th>");
   out.println("    </tr>");
   out.println("    </thead>");
   out.println("    <tbody>");
   ...
   try {
      ...
      while (rs.next()) {
         ...
         out.println("      <td class=\"group_lesson_name\">" + lgname + "</td>");
         out.print("      <td class=\"group_lesson_date\">");
         ...
         out.println("</td>");
         out.println("      <td class=\"group_lesson_start_end\">" +ampms+ "</td>");
         out.println("      <td class=\"group_lesson_start_end\">" +ampme+ "</td>");
         out.println("      <td>" + max + "</td>");
         out.println("      <td>" + cost + "</td>");
         out.println("      <td class=\"group_lesson_desc\">" + desc + "</td>");
         out.println("      <td>");
         if (sess_activity_id == 0 || date_start == date_end || clinic == 1) {
 ...
 private void reqTime(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session,
   ...
   out.println("    <p class=\"lesson_left2\">You have approximately <b>6 minutes</b> to complete this task.");
   out.println("      If you want to return without making any changes, <b>do not ");
   out.println("      use your browser's BACK</b> button/option. ");
   out.println("      Instead select the <b>Go Back</b> option below.</p>");
   out.println("  </div>");

   out.println("  <p class=\"lesson_ctr\">Date:&nbsp;&nbsp;<b>" + dayName );
   out.println("    &nbsp;&nbsp;" + month + "/" + day + "/" + year + "</b>");

   if (club.equals( "interlachenspa" )) {
   out.println("      &nbsp;&nbsp;&nbsp;&nbsp;Time:");
   } else {
   out.println("      &nbsp;&nbsp;&nbsp;&nbsp;" + sysLingo.TEXT_Lesson + " Time:");
   }
   out.println("      &nbsp;&nbsp;<b>" +hr+ ":" + Utilities.ensureDoubleDigit(min) + " " +ampm+ "</b></p>");
   ...
   if (!index.equals( "" )) {          // if an edit
      ...
      if (date == todayDate && time < cantime) {
         ...
         if (club.equals( "interlachenspa" )) {
            out.println("      <input type=submit value=\"Cancel Service\" name=\"remove\" />");
         } else {
            out.println("      <input type=submit value=\"Cancel " + sysLingo.TEXT_Lesson + "\" name=\"remove\" />");
         }
      }
      out.println("      </div>");
   }
 ...
 private void reqTime2(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session,
   ...
   try {
      ...
      pstmt = con.prepareStatement (
         "SELECT memname, length, in_use, phone1, phone2, notes, recid, ltname " +
   ...
   if (req.getParameter("remove") != null) {
      ...
      if (activity_id != 0) {
          try {
              ...
              dbError_ns(req, out, exc, caller, con, sess_activity_id, club);
          }
      }
      ...
      if (activity_id != 0 && autoblock_times == 1) {
...
//      }
   ...
   //
   if (activity_id != 0 && sheet_ids.size() > 0 && autoblock_times == 1) {
 ...
 private void grpLesson(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session,
   ...
   if (clinic == 1 && sess_activity_id != 0) {
       ...
       out.println("  <p class=\"lesson_ctr\">Date:&nbsp;&nbsp;<b>" + month + "/" + day + "/" + year + "</b>&nbsp;&nbsp;&nbsp;&nbsp;Time:&nbsp;&nbsp;<b>" + Utilities.getSimpleTime(stime) + " &ndash; " + Utilities.getSimpleTime(etime) + "</b></p>");
   }
...
//         emailMod = 1;
 ...
 private void cancel(int id, int time, long date, String calDate, String ltype, PrintWriter out, HttpSession session,
   ...
   out.println("  <br /><br /><h3 class=\"lesson_ctr\">Return/Cancel Requested</h3>");
         ...
         while (rs.next()) {
            ...
            if (!f.isFile()) {
               ...
               out.println("<!-- couldn't file bio page file: " + req.getRealPath("") + "/announce/" +club+ "/" +club+ "_bio" +id+ ".htm -->");
                  ...
                  out.println("<a href=\"/" +rev+ "/servlet/Common_lesson?bioview&club=" +club+ "&proid=" +id+ "\" class=\"tu_iframe_1024x768 standard_button\">" + pro_name.toString() + "</a><br /><br />");   

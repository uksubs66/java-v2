  ...
  public ProshopMultipartRequest(HttpServletRequest request,
    ...
    int activity_id = (Integer)session.getAttribute("activity_id");
      ...
      try {
          ...
          SystemUtils.logError("Proshop Upload error in ProshopMultipartRequest: ERR=" + e1.getMessage());
    ...
    while ((part = parser.readNextPart()) != null) {
      ...
      if (part.isParam()) {
        ...
        if (fileName != null) {
          ...
          if (new_skin == true && (fileName.endsWith( ".htm" ) || fileName.endsWith( ".html" ))) { 
          ...
          } else if (fileName.endsWith( Fname1 )) {    // || fileName.endsWith( Fname2 )
               
             nameOK = true;          // name is ok
             
          } else if (fileName.startsWith( Fname3 ) && (fileName.endsWith ( ".htm" ))) {  // || fileName.endsWith ( ".html" )
          ...
          // do not backup or change the name if new skin and unique name - we save as a staging file instead
          if (new_skin == false || fileName.startsWith(club + "_announce" + ((activity_id == 0) ? "" : "_" + activity_id) + ".htm")) {    
          ...
          if (new_skin && !fileName.startsWith(club + "_announce" + ((activity_id == 0) ? "" : "_" + activity_id) + ".htm")) {   // if new skin and file name is different than actual announcement page for his club

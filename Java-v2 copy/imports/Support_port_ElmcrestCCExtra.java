...
*/
            ...
            if (mtype.startsWith("Primary")) {
                ...
                try {
                    PreparedStatement pstmtk = con.prepareStatement("SELECT SUBSTR(username, LENGTH(username)) as prim FROM member2b WHERE memNum = ? and SUBSTR(username, LENGTH(username)) > 1 ORDER BY username DESC LIMIT 1");
                    pstmtk.clearParameters();
                    pstmtk.setString(1, mNum);

                    ResultSet rsk = pstmtk.executeQuery();

                    if (rsk.next()) {
                        primary = rsk.getInt("prim") + 1;
                        mem_id += String.valueOf(primary);
                    } else {
                        primary = 2;
                        mem_id += "2";
                    }
                    ...
                    out.println("<br>Error while getting primary value  " + lname + ", " + fname + "(" + exc.getMessage() + ")");
                }

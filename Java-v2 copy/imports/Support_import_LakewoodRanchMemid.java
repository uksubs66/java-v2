...
*/               
 ...
 private void portWebid(HttpServletRequest req, PrintWriter out, Connection con, String club) {
    ...
    try {
        ...
        while ((line = bfrin.readLine()) != null) {            // get one line of text
            ...
            if (!webid.equals( "" )) { 
                ...
                if (byname) {
                    ...
                    //
                    tmp = 0;
                    sql = "SELECT username, name_first, name_mi, name_last FROM member2b WHERE memNum = ?";
                    ...
                    if (rs.next()) {
                        memidOld = rs.getString("username");
                        tmp = 1;
                        newName = rs.getString("name_first") + " " + (!rs.getString("name_mi").equals("") ? (rs.getString("name_mi") + "") : "") + rs.getString("name_last");
                    }
                    ...
                    if (tmp == 0) {
                        ...
                        logError("Member Not Found: name was " +lname+ ", " +fname+ " (" +memidNew+ ")", club);
                        err_nf++;
                    } else {
                        ...
                        if (tmp == 1) {
                            ...
                            try {
                                sql = "UPDATE member2b SET username = ? WHERE memNum = ?";

                                pstmt2 = con.prepareStatement (sql);
                                pstmt2.clearParameters();
                                pstmt2.setString(1, memidNew);
                         ...
                         Admin_editmem.updTeecurr(newName, memidNew, memidOld, con);      // update teecurr with new values

                         Admin_editmem.updTeepast(newName, memidNew, memidOld, con);      // update teepast with new values

                         Admin_editmem.updLreqs(newName, memidNew, memidOld, con);        // update lreqs with new values

                         Admin_editmem.updPartner(memidNew, memidOld, con);               // update partner with new values

                         Admin_editmem.updEvents(newName, memidNew, memidOld, con);        // update evntSignUp with new values

                         Admin_editmem.updLessons(newName, memidNew, memidOld, con);       // update the lesson books with new values
                                ...
                                logError("Error encountered: " + exc.getMessage() + " for " +lname+ ", " +fname+ " (" +webid+ ")", club);
                            }
               ...
               msg = "Webid Not Specified: mem_id was " +mem_id+ ", name was " +lname+ ", " +fname;
               logError(msg, club);                           // log it
               err_nf++;
        ...
        msg = "SQL Exception: " +mem_id+ ", " +lname+ ", " +fname;
        logError(msg, club);                           // log it

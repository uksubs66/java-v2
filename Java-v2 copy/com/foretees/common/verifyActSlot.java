 ...
 **/
  ...
  *      2 = Selected activity_id Only - Check selected court for the first opening of the requested amount of time.
  *      3 = All activity_ids in search_group - Check all courts in the selected court's group for the first opening of the requested amount of time.
  * @throws Exception
  */
        ...
        switch (run_mode) {
            ...
            case 2:
                ...
                try {
                    ...
                    pstmt = con.prepareStatement("SELECT activity_id FROM activities a WHERE parent_id = ? AND search_group = ? AND a.interval = ? AND enabled = 1 ORDER BY activity_id = ? DESC, sort_by ASC, activity_name ASC");
                    pstmt.clearParameters();
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        if (slotParms.club.equals("lakewoodranch") && curr_time < 1300 && curr_time != 830 && curr_time != 1000 && curr_time != 1130) {
                            continue time_loop;
                        }
 ...
 public static boolean checkMaxOrigBy(String user, long date, int max_originations, Connection con) {
     ...
     try {
         ...
         pstmt = con.prepareStatement("SELECT count(*) as 'orig_count' FROM (SELECT * FROM activity_sheets WHERE DATE_FORMAT(date_time, '%Y%m%d') = ? AND orig_by = ? GROUP BY related_ids) AS orig_query");
                ...
                if (!found) {
                    ...
                    for (int j = 0; j < slotParms.sheet_ids.size(); j++) {
                        ...
                        if (count != 0) {
                            ...
                            if ( parmAct.rndsperday != 0 && count >= parmAct.rndsperday ) {        // if we are over the allowed times
...
*/
  ...
  * @param con Connection to club database
  *
  * @return has - True if at least one player was found, false if no players were found
  */
 public static boolean checkMemberHasAccess(int sheet_id, String user, Connection con) {
     ...
     // Only procede if the slot has players
     if (checkSlotHasPlayers(sheet_id, con)) {
         ...
         try {
             ...
             if (disallow_joins) {
                 ...
                 try {
                     ...
                     while (rs2.next()) {
                         if (user.equalsIgnoreCase(rs2.getString(1))) hasAccess = true;
                     }

        ...
        onSelect: function(dateText,inst){
            ...
            if("date_map" in calendar_map){
                ...
                if(calendar_map.IS_TLT){
                    ...
                    ftChangePage(ftSetJsid("Member_sheet?calDate="+dateText+"&course="+coursename+(($("#select_jump").is(':checked')) ? "&select_jump" : "")+consec));
                }
            }
    ...
    var methods = {
        ...
        init: function(option){
            ...
            $(this).each(function(index){
                ...
                if(!calContainer.find(".foreTeesMemberCalendarObject").length){
                    calContainer.append('<div class="foreTeesMemberCalendarObject"></div>');
                }
                ...
                if(result == null){
                    ...
                    calObj.datepicker({disabled: true, defaultDate: defaultDate});
                ...
                if("selected_date" in pluginData){
                    selected_date = pluginData.selected_date;
                } else if("selected_date" in result){
                    selected_date = pluginData.result;
                } else if(typeof(calContainer.attr('data-ftdefaultdate')) != "undefined" && calContainer.attr('data-ftdefaultdate').length){
                ...
                calObj.datepicker("option",{
                    ...
                    beforeShowDay: function(date){
                        ...
                        if("date_map" in calendar_map){
                            ...
                            if((typeof dateArray == "object") && (typeof(dateArray[date.format('mm/dd/yyyy')]) == "object")){
                                if(dateArray[date.format('mm/dd/yyyy')].display){
                                    if(options.disableOnDayValue != null && dateArray[date.format('mm/dd/yyyy')].ft_code == options.disableOnDayValue){
                                        enable_day = false;
                                    }else{
                                        day_class = dateArray[date.format('mm/dd/yyyy')].ft_code;
                                        enable_day = true;
                                        tooltip = dateArray[date.format('mm/dd/yyyy')].ft_tool_tip;
                                    }
                                }else{
                                    enable_day = false;
                                }
                        ...
                        if((typeof($(this).data('todaysDate')) != "undefined") && (date.format('mm/dd/yyyy') == $(this).data('todaysDate').format('mm/dd/yyyy'))){

...
function ftSSOLink(uid, iv, o){
    if (o.base_url.length && o.url.length) {
        ...
        ftVars.iv = iv;
        /*
        var urlA1 = o.base_url.split("//");
        var urlA2 = urlA1[1].split("/");
        var setA = urlA2[urlA2.length-2].split("_");
        //var club = setA[0];
        //var activity = setA[1];
        if(setA[2]){
            var mode = parseInt(setA[2].split("m")[1],10);
        } else {
            var mode = 0;
        }
        setA[2] = "m"+mode;
        urlA2[urlA2.length-2] = setA.join("_");
        urlA1[1] = urlA2.join("/");
        o.base_url = urlA1.join("//");
        */

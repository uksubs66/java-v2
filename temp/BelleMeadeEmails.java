...
public class BelleMeadeEmails extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   //
   String club = "bellemeadecc";   // get club name
   ...
   try {
      ...
      fr = new FileReader("//usr//local//tomcat//webapps//" +club+ "//emails2.csv");
   ...
   if (failed == true) {
      ...
      try {
         ...
         fr = new FileReader("c:\\java\\tomcat\\webapps\\" + club + "\\emails2.csv");
         //fr = new FileReader("/Applications/apache-tomcat-5.5.26/webapps/" + club + "/emails2.csv");
      }
      catch (Exception e2) {
...
/*          
         mNum = tok.nextToken().trim();
         email = tok.nextToken().trim();
                  
         if (mNum.equalsIgnoreCase(last)) {
             continue;
         } else {
             last = mNum;
         }
         ...
         try {
             pstmt = con.prepareStatement("UPDATE member2b SET email = ?, gender = IF (gender='', 'M', gender), m_type = IF (gender='', 'Primary Male', 'Primary Female') WHERE username = ?");
             pstmt.clearParameters();
             pstmt.setString(1, email);
             pstmt.setString(2, mNum);
             pstmt.executeUpdate();
         } catch (Exception exc) {
             out.println("Error updating database!");
         }
*/         
      
         mNum = tok.nextToken().trim();
         depend = tok.nextToken().trim();
         relate = tok.nextToken().trim();
         email = tok.nextToken().trim();
         ...
         if (relate.equalsIgnoreCase("W") || relate.equalsIgnoreCase("D")) {
             gender = "F";
         } else {
             gender = "M";
         }
             
         if (relate.equalsIgnoreCase("")) {
             try {
                 pstmt = con.prepareStatement("UPDATE member2b SET email = ? WHERE memNum = ? AND pri_indicator = ?");
                 pstmt.clearParameters();
                 pstmt.setString(1, email);
                 pstmt.setString(3, mNum);
                 pstmt.setInt(4, Integer.parseInt(depend));
                 pstmt.executeUpdate();
             } catch (Exception exc) {
                 out.println("Error updating database!");
             }             
             ...
             try {
                 pstmt = con.prepareStatement("UPDATE member2b SET email = ?, gender = ? WHERE memNum = ? AND pri_indicator = ?");
                 pstmt.clearParameters();
                 pstmt.setString(1, email);
                 ...
                 out.println("Error updating database 2!");
             }
         }
...
//         mship = tok.nextToken();
         mship_num = tok.nextToken();       // number format to be translated later into actual mship value (FOR BELLE MEADE ONLY!)
         temp = tok.nextToken();
         temp2 = tok.nextToken();
         temp3 = tok.nextToken();
...
//         posid = tok.nextToken();
         ...
         // TEMP OPERATIONS :: USED ONLY FOR BELLE MEADE!!
         posid = mNum;
         mNum = mNum.trim();
         mship_num = mship_num.trim();
         if (mNum.equals(mNum_last) && mship_num.equals("?")) {
             mship_num = mship_last;        // Same family
         } else {
             mNum_last = mNum;          // New family
             mship_last = mship_num;
         }
         /////////////////////////////////////////////////
...
// BELLE MEADE        
            ...
            temp2 = temp2.trim();
            temp3 = temp3.trim();
            if (temp.equals( "?" ) || temp.equalsIgnoreCase( "N/A" )) {

               temp = "0";
            }
            if (temp2.equals( "?" ) || temp2.equalsIgnoreCase( "N/A" )) {

               temp2 = "0";
            }            
            if (temp3.equals( "?" ) || temp3.equalsIgnoreCase( "N/A" )) {
            ...
            if (temp2.length() == 1 && !temp2.equals("0")) {
                temp2 = "0" + temp2;
            }
            if (temp3.length() == 1 && !temp3.equals("0")) {
                temp3 = "0" + temp3;
            }
////            
/*            
...
/////////////BELLE MEADE///////////////////////       
            ...
            if (tok.countTokens() > 0) {
                primary = Integer.valueOf(tok.nextToken().trim());
            }
////////////////////////////////////////////////            
/*            
            if ( tok.countTokens() > 0 ) {
...
//// belle meade            
            switch(Integer.parseInt(mship_num)){
                case 1:
                    mship = "Resident";
                    break;
                case 2:
                    mship = "Associate under 30 years of age";
                    break;
                case 3:
                    mship = "Associate 30 to 34";
                    break;
                case 4:
                    mship = "Associate 35 to 39";
                    break;
                case 5:
                    mship = "Associate Resident";
                    break;
                case 6:
                    mship = "Honorary Resident";
                    break;
                case 7:
                    mship = "Ladies";
                    break;
                ...
                case 9:
                    ...
                    break;
                case 13:
                    mship = "Special Recovery";
                    break;
                case 14:
                    mship = "Ladies Provisional";
                    break;
                default:
                    mship = "Unknown";
                    break;
...
*/
            ...
            * 4: mm.dd.yy or mm/dd/yy (i.e. 01.12.56 or 01/12/56)
            ************************************************************/
...
///Belle Meade            
            ...
            if (prefix.equalsIgnoreCase("Miss") || prefix.equalsIgnoreCase("Mrs") || prefix.equalsIgnoreCase("Mrs.") || prefix.equalsIgnoreCase("Ms")) {
                gender = "F";
                prefix = "";
            ...
            if (gender.equalsIgnoreCase("M") && primary < 2) {
                mtype = "Primary Male";
            } else if (gender.equalsIgnoreCase("F") && primary < 2) {

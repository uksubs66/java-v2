...
public class Member_announce extends HttpServlet {
 ...
 int CONNECT_ID = ProcessConstants.CONNECT_ID;      // Act id for FT Connect - Home
 int CONNECT_NEWSLETTERS_ID = ProcessConstants.CONNECT_NEWSLETTERS_ID;    // Act id for FT Connect - Newsletters
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   //  IF HERE FOR MANAGERS PORTAL THEN JUMP OUT TO IT
   if (activity_id == ProcessConstants.MANAGERS_PORTAL){
           
        Manager_dashboard mpDash = new Manager_dashboard();

        mpDash.doGet(req, resp);
        return;
   }
   ...
   if (req.getParameter("override") == null) {   // skip if Home link in bread crumb set to override the following test
       ...
       if (((club.equals( "paloaltohills" ) || club.equals("capecodnational") || club.equals("mpccpb") || club.equals("terravita") 
               || (!club.equals("canyonoakscc") && !club.equals("gettysvuecc") && !club.equals("sunsethillscc") && Utilities.isClubInGroup(club, "AGC"))) && activity_id == 0) 
               || club.equals("philcricketrecip") || club.equals("idlewildcc") || club.equals("foresthighlands")) {
           ...
           return;
               ...
               || (club.equals("theranchcc") && activity_id == 10)) {
   ...
   if (req.getParameter("activity") != null) {   // if FT Connect site and user clicked the club name (see Common_skin.outputBanner)
       ...
       if (temp.equals("connect")) {
           activity_id = CONNECT_ID;    // Display the Connect Home page
       } else if (temp.equals("connect_newsletters")) {
           activity_id = CONNECT_NEWSLETTERS_ID;    // Display the Connect Newsletters page
       }
   }
   ...
   if (caller.equals(ProcessConstants.FT_PREMIER_CALLER)) {
        ...
        if (club.equals("tontoverde")) {
            ...
            if (rwd) {
                ...
                if (activity_id == 0) {
                    ...
                    // golf landing page
                    landing_page = "http://foretees2.flexdemo.com/members-only/golf/golf-17.html";

                } else if (activity_id == 1) {

                    // tennis landing page
                    landing_page = "http://foretees2.flexdemo.com/members-only/tennis/tennis-25.html";

                } else if (activity_id == 2) {

                    // don't set for fitness, it uses our announcement page

                } else if (activity_id == 30) {

                    // don't set for juniors

                } else if (activity_id == ProcessConstants.DINING_ACTIVITY_ID) {

                    // dining landing page
                    landing_page = "http://foretees2.flexdemo.com/members-only/dining/dining-19.html";
                ...
                if (activity_id == 0) {
                    ...
                    // tennis landing page
                    landing_page = "http://foretees.flexdemo.com/tennis-1364.html";

                } else if (activity_id == ProcessConstants.DINING_ACTIVITY_ID) {

                    // dining landing page
                    landing_page = "http://foretees.flexdemo.com/dining---social-1362.html";
        ...
        } else if (club.equals("***pinehurstcountryclub")) { // disable this for now
        ...
        } else if (club.equals("stalbans")) {
            
            if (activity_id == 0) {

                // golf landing page
                landing_page = "http://64.106.202.173/members-only/golf/courses-107.html";

            } else if (activity_id == ProcessConstants.DINING_ACTIVITY_ID) {

                // dining landing page
                landing_page = "http://64.106.202.173/members-only/club-dining-92.html";

            } else if (activity_id == 1) {
                
                // tennie landing page (not used yet)
                landing_page = "http://64.106.202.173/members-only/tennis-95.html";
            }

        } else if (club.equals("belairbayclub")) {
            
            if (activity_id == ProcessConstants.DINING_ACTIVITY_ID) {

                // dining landing page
                landing_page = "http://64.106.201.136/members/dining-34.html";
            ...
            if (activity_id == 0) {
                ...
                // golf landing page
                landing_page = "http://www.mediterraliving.com/golf/pro-shop-321.html";

            } else if (activity_id == ProcessConstants.DINING_ACTIVITY_ID) {

                // dining landing page
                landing_page = "http://www.mediterraliving.com/dining---events/the-clubhouse-122.html";

            } else if (activity_id == 1) {

                // tennis landing page
                landing_page = "http://www.mediterraliving.com/tennis/pro-shop-163.html";

            } else if (activity_id == 2) {

                // bocce landing page
                landing_page = "http://www.mediterraliving.com/lifestyle/lifestyle-wall-169.html";

            } else if (activity_id == 3) {

                // fitness landing page
                landing_page = "http://www.mediterraliving.com/wellness/pro-shop-177.html";
    ...
    if (new_skin) {
        ...
        if (Utilities.isFTConnectClub(req) > 0 && activity_id == ProcessConstants.CONNECT_DIRECTORY_ID) {
/*
            out.println("<div class=\"main_instructions\">"
                    ...
                    + "</div>");
*/
            out.println("<center><a href=\"Member_directory?viewRoster\" target=\"jonasPopup\" class=\"standard_button\" style=\"width:350px\">View Member Directory</a></center>");

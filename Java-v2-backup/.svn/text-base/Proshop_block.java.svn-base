 ...
 *   last updated: 
 *
 *        4/30/14   Get multi value from Utilities instead of getClub (more efficient and getClub was throwing an error).
...
public class Proshop_block extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   *     //  all we need is the multi value, so use Utilities to get it !!!
   */
   multi = Utilities.getMulti(con);  
   ...
   try {
      ...
      while ( rs.next() ) {
         ...
         out.println("<form method=\"post\" action=\"Proshop_block\" target=\"bot\">");
         out.println("<td align=\"center\">");
 ...
 private void doCopy(int lottery, HttpServletRequest req, PrintWriter out, Connection con, int sess_activity_id) {
   ...
   try {
      ...
      if (rs.getRow() > 0) {
          ...
          out.println("<form action=\"Proshop_block\" method=\"post\" target=\"bot\">");

 ...
 *       11/07/13   Updated the email search so there's no longer a need to strip off "mailto:" from copied email addresses.
 *        5/15/12   Updated the email search so there's no longer a need to strip out the <> from copied email addresses.
 *        4/18/12   When searching by email, the club name in results is now a button that, when clicked, will switch the support login to be 
 *                  logged in for that club (same as going into the Switch Clubs option on Support page 1).
...
public class Support_errorlog extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   out.println("<HTML><HEAD><TITLE>Display Error Log</TITLE>" +
           "<meta http-equiv='Refresh' content='900; url=Support_errorlog'>" +
           "</HEAD>");
 ...
 private void search(HttpServletRequest req, PrintWriter out) {
   ...
   if (req.getParameter("email") != null) {        // if 2nd call call
      ...
      if (email.contains("<")) {
          email = email.substring(email.indexOf("<") + 1, email.indexOf(">")).trim();
      ...
      try {
         ...
         while (rs2.next()) {
            ...
            while (rs.next()) {
               ...
               out.println("<tr><td>");
                  ...
                  out.println("<td>");
                     out.println("<font size=\"2\">");
                     out.println("<form action=\"Support_cluboptions\" method=\"get\">");
                     out.println("<input type=\"hidden\" name=\"switchClubs\">");
                     out.println("<input type=\"hidden\" name=\"newclub\" value=\"" + club + "\">");
                     out.println("<p align=\"center\"><input type=\"submit\" name=\"doSwitch\" value=\"" + club + "\"></p>");
                     out.println("</form>");
                     out.println("</font></td>");

                  out.println("<form action=\"Support_errorlog\" method=\"post\">");
                  out.println("<td>");
                     out.println("<font size=\"2\">");
         ...
         out.println("<form method=\"get\" action=\"Support_errorlog\">");
      ...
      out.println("<center>");
         ...
         out.println("<p>To locate a member, enter the email address,<br>");
            out.println("or any portion of it, as it may exist in the member database.<br>"
                    + "<br>Email addresses can be copy and pasted with \"mailto:\" or &lt;&gt; present, "
                    + "<br>as they will be stripped automatically.</p>");
            out.println("</font>");
         ...
         out.println("<form action=\"Support_errorlog\" method=\"post\" name=\"f\">");
         out.println("<input type=\"hidden\" name=\"search\" value=\"yes\">");
         ...
         out.println("<form action=\"Support_errorlog\" method=\"get\">");
 ...
 private void removeAddr(HttpServletRequest req, PrintWriter out) {
   ...
   try {
      ...
      out.println("</font>");
      out.println("<BR><BR>Please <A HREF=\"Logout\">Logout</A>");
    ...
    for (int x=0; x < email.length; x++) {
        ...
        if (!email[x].equals("")) {
            ...
            try {
               ...
               Connect.close(rs2,pstmt,con2);
               Connect.close(rs2,stmt2,con);
           }

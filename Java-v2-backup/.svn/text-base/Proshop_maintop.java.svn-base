 ...
 *   last updated:
 *
 *      6/05/14  Add check for proshop alerts - announcements from ForeTees.  Replace the club logo with a mailbox if message present for today.
 *      5/06/14  Oak Hill CC (oakhillcc) - add an Advance Guest Report link next to the Member View link (case 2361).
 *      4/24/14  Do not show the Mobile help icon as we now have RWD.
...
import com.foretees.api.records.ClubInvoicing;
import com.foretees.api.records.ForeTeesAnnouncement;
//import com.foretees.api.records.Invoice;
//import com.foretees.api.records.InvoiceDetail;
//import com.foretees.api.records.InvoiceItem;
...
public class Proshop_maintop extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    //String FT_announcement = "";             // FT Announcement File Name
    ...
    //
    //  Check if we have an announcement for the proshop users
    //
    //     To display an announcement add an entry to the v5.announcements table
    //
    //FT_announcement = Utilities.checkFTannouncement(sess_activity_id, club);  
    ...
    showMobile = false;   // do not show now that we have RWD
    ...
    if (club.equalsIgnoreCase("gallerygolf") && (sess_activity_id == 1 || sess_activity_id == 9)) {
        club_logo_name = "logo2.jpg";
    }
...
*/
    ...
    SystemUtils.getProshopMainMenu(req, out, lottery);
     ...
     } else if (club.equals("deserthighlands") || club.equalsIgnoreCase("bluehillscc") || club.equalsIgnoreCase("demowill")) {
     ...
     if (showMobile && IS_TLT == false && sess_activity_id == 0 && !club.equals("denvercc")) {    //skip Mobile image if Notification or FlxRez system 
     ...
     if (user.equals("proshopfb")) {
        ...
        out.println("<font size=\"5\">ForeTees Dining Integration Management</font><br>");
         ...
         if (sess_activity_id == ProcessConstants.CONNECT_ID) {
             
             tmp_title = "ForeTees Connect Management System";
     ...
     //
     if ((getActivity.isConfigured(con) && (!getActivity.isStagingMode(con) || sess_activity_id != 0 || club.equals("winchestercc")) && !club.equals("demosandbox")) || club.equals("wichita")) {
     ...
     }  else if (Utilities.isFTConnectClub(req) > 0 && Utilities.isFTConnectProUser(user, req)) { // end isConfigured()
     ...
     if (!club.equals("demosandbox")) {
         ...
         if (Utilities.isGuestTrackingConfigured(sess_activity_id, con) || club.startsWith("demo") || club.startsWith("tpc") || club.equals("desertforestgolfclub")) {
             out.println("&nbsp;<label class=\"gblSep\">|</label>&nbsp;<a class=\"gblHref\" href=\"Common_guestdb\" target=\"bot\" > Guests</a>");
         }
         //out.println("&nbsp;<label class=\"gblSep\">|</label>&nbsp;<a class=\"gblHref\" href=\"/" +rev+ "/proshop_help.htm\" target=\"_blank\" > Help</a>");  // no longer needed - see Support
     }
     ...
     if (club.startsWith("demo") && !club.equals("demosandbox")) {
       ...
       //out.println("&nbsp;&nbsp;<label class=\"gblSep\">|</label>&nbsp;<a class=\"gblHref\" href=\"Proshop_app_credentials\" target=\"bot\" > App Setup</a>");
     }
     
     
     if (club.equals("interlachen") || club.equals("kiawahislandclub")) {        // link to check members' guest quotas
         
        out.println("&nbsp;<label class=\"gblSep\">|</label>&nbsp;<a class=\"gblHref\" href=\"Proshop_custom_memlist\" target=\"bot\" > Quotas</a>");

     } else if (club.equals("oakhillcc")) {        // Oak Hill CC - add link for Advance Guest Report (case 2361)
         
        out.println("&nbsp;<label class=\"gblSep\">|</label>&nbsp;<a class=\"gblHref\" href=\"Proshop_report_custom\" target=\"bot\" > Advance Guest Report</a>");
     }
     
     if(ProcessConstants.isDevServer() || ProcessConstants.isTestingServer()){
         Club clubRecord = new Club(club);
         if(clubRecord.last_error == null){
             // Club has a record
             List<ClubInvoicing> clubInvoicing = ClubInvoicing.getVisibleToProshopByClubId(clubRecord.id);
             if(clubInvoicing != null && clubInvoicing.size() > 0){
                 // Club has ClubInvoicing record(s) -- create a menu option for viewing thier invoices
                 String invoiceMenuClass = "ftInvocingMenu";
                 String invoiceMenuName = "Invoices";
                 if(clubRecord.unpaid_invoices > 0){
                     invoiceMenuClass += " ftHasUnpaid";
                 }
                 if(clubRecord.max_days_past_due > 0){
                     invoiceMenuClass += " ftHasPastDue";
                 }
                 out.println("&nbsp;<label class=\"gblSep\">|</label>&nbsp;<a class=\"gblHref "+invoiceMenuClass+"\" href=\"Proshop_invoice_list\" target=\"bot\" >"+invoiceMenuName+"</a>");
             } else if(clubInvoicing == null){
                 Connect.logError("Proshop Invoice Menu Error:  Null clubInvoicing List");
             } else {
                 //Connect.logDebug("JGK","Proshop Invoice Menu:  Empty clubInvoicing List Size:"+clubInvoicing.size()+" club_id:"+clubRecord.id);
                 // Invoicing not enabled for club, or no invoicing records are visible to proshop.
             }
         } else {
             Connect.logError("Proshop Invoice Menu Error:"+clubRecord.last_error);
         }
     }
     ...
     out.println("<br><br><br></td>");
      ...
      //
      //  Display our mail box if we have an announcement for this date
      //
      if (!club.equals("demosandbox")) {       // if not our sandbox site
         
        out.print("<td width=\"12%\" align=\"center\">");
        
        // Get active announcments (today is between edate and sdate)
        //List<ForeTeesAnnouncement> active = ForeTeesAnnouncement.getActiveByUsername(ForeTeesAnnouncement.getModeByActivityId(sess_activity_id), club, user);  // Change to this after this code is in production.
        List<ForeTeesAnnouncement> active = ForeTeesAnnouncement.getActiveByUsername(club.startsWith("demo")?ForeTeesAnnouncement.MODE_ALL:ForeTeesAnnouncement.getModeByActivityId(sess_activity_id), club, user);
        //if (!FT_announcement.equals("")) {       // if announcement exists
        if(active != null && !active.isEmpty()){
            // Automatically show the first unread message (though there may be others)
            out.print("<a href=\"Proshop_announce_list?announcement_id=" +active.get(0).id + "\" target=\"bot\"><img src=\"../assets/images/ForeTees_Msg_sm.png\"></a>&nbsp;&nbsp;</td>");
            //out.print("<a href=\"Proshop_announcements?fileName=" +FT_announcement+ "\" target=\"bot\"><img src=\"/" +rev+ "/images/ForeTees_Msg_sm.png\"></a>&nbsp;&nbsp;</td>");
        } else {
            out.println("<br><a class=\"gblHref\" href=\"Proshop_announce_list\" target=\"bot\" >View Past<br>Announcements</a>");
            //out.println("<br><a class=\"gblHref\" href=\"Proshop_announcements\" target=\"bot\" >View Past<br>Announcements</a>");
        }
        
        out.print("<td width=\"12%\" align=\"center\" class=\"header_right_end\">"); 

...
public class Admin_mships extends HttpServlet {
  ...
  **/
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse resp)
      ...
      if (req.getParameter("generateMissingMships") != null) {
          generateMissingMships(req);
      }
      ...
      if (user.equals("admin4tea")) {
          out.println("<form action=\"Admin_mships\" method=\"POST\" name=\"generateMissingMshipsForm\">");
          out.println("<tr><td style=\"text-align:center\" colspan=\"2\">");
          out.println("<input type=\"submit\" name=\"generateMissingMships\" value=\"Generate Missing Mship Entries\">");
          out.println("</td></tr>");
          out.println("</form>");
      ...
      if (user.equals("admin4tea")) {
          ...
          for (int i=0; i<activity_ids.size(); i++) {
              ...
              out.println("<tr>");
              out.println("<td style=\"text-align:left\"><label><input type=\"checkbox\" " + checked + "name=\"act_" + activity_ids.get(i) + "\" value=\"" + activity_ids.get(i) + "\"> " + activity_names.get(i) + "</label></td>");
              out.println("</tr>");
          }
          out.println("</table>");
          out.println("</td></tr>");
  ...
  private boolean generateMissingMships(HttpServletRequest req) {
      ...
      // Load up all activities for this club
      if (getActivity.isGolfEnabled(con)) {
          activity_ids.add(0);
      }
      ...
      try {
          ...
          Utilities.logError("Admin_mships.generateMissingMships - Failed to gather activity_ids - Error=" + e.toString());
      } finally {
          Connect.close(rs, stmt);
      }
      
      
      // Get all mships for this club
      try {
          stmt = con.createStatement();
          
          rs = stmt.executeQuery("SELECT mship, GROUP_CONCAT(activity_id ORDER BY activity_id SEPARATOR ',') AS activity_csv FROM mship5 GROUP BY mship");
          
          while (rs.next()) {
            
              // For each mship, loop through the global list of activity_ids, and for each one that there isn't already an mship5 entry for, insert a new one to flesh out the entries
              String[] temp_ids = rs.getString("activity_csv").split(",");
              List<Integer> curr_activity_ids = new ArrayList<Integer>(temp_ids.length);
              
              for (int i = 0; i < temp_ids.length; i++) {
                  curr_activity_ids.add(Integer.parseInt(temp_ids[i]));
              }
              
              for (int activity_id : activity_ids) {
                  
                  if (!curr_activity_ids.contains(activity_id)) {
                      
                      try {
                          
                          pstmt = con.prepareStatement("INSERT INTO mship5 (mship, activity_id, mtimes, period, " +
                                      "days1, days2, days3, days4, days5, days6, days7, " +
                                      "advhrd1, advmind1, advamd1, advhrd2, advmind2, advamd2, advhrd3, advmind3, advamd3, " +
                                      "advhrd4, advmind4, advamd4, advhrd5, advmind5, advamd5, " +
                                      "advhrd6, advmind6, advamd6, advhrd7, advmind7, advamd7, " +
                                      ...
                                      "'','','','','',30,'')");
                          pstmt.clearParameters();
                          pstmt.setString(1, rs.getString("mship"));
                          pstmt.setInt(2, activity_id);
                          
                          pstmt.executeUpdate();
                          
                      } catch (Exception e) {
                          Utilities.logError("Admin_mships.generateMissingMships - Insert new mship5 entry for mship=" + rs.getString("mship") + ", activity_id=" + activity_id + " - Error=" + e.toString());
                      } finally {
                          Connect.close(pstmt);
                      }
                  }
          ...
          Utilities.logError("Admin_mships.generateMissingMships - Failed to gather mships - Error=" + e.toString());
      } finally {
          Connect.close(rs, stmt);
      }

...
public class Member_gensheets extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    String user_mNum = Utilities.getmNum(user, con);    // Current user's member number
  ...
  //int days = 0;                           // max days member can edit times for this day of the week
    ...
    boolean hideNames = false;    // This is only used in a custom at the moment. Should be turned into a full feature eventually.
    ...
    // Generate list of all guest types
    List<String> guestTypes = new ArrayList<String>();
    for (int i = 0; i < parm.MAX_Guests; i++) {
        if (parm.guest[i] != null && !parm.guest[i].isEmpty()) { 
            guestTypes.add(parm.guest[i]);
        }
    }
    ...
    if ((((club.equals("quecheeclubtennis") && activity_id == 1) || (club.equals("denvercc") && activity_id != 16 && curr_time >= 1200))&& index == 0)
            || (club.equals("marbellacc") && ((index == 0 && curr_time < 700) || (index == 1 && curr_time >= 1900)))
            || (club.equals("avonoakscc") && (index < 1 || (index == 1 && curr_time >= 2100)))) {
        restrictAll = true;
    }
   
    //treat tennis days in advance as 7
    if (club.equalsIgnoreCase("ccofbuffalo")) {
       int cur_time = Utilities.getTime(con);
       if (activity_id == 2 && (index > 7 || (index == 7 && cur_time < 700))) {
           restrictAll = true;
       }
    }
...
//    }
    ...
    jsonHashMap = gson_obj.toJson(calendar_map);
    ...
    out.println("<div id=\"member_sheet_calendar\" class=\"calendar flxsheet primary\" " +
            "data-ftdefaultdate=\"" + month + "/" + day + "/" + year + "\" data-ftjson=\"" + StringEscapeUtils.escapeHtml(jsonHashMap) + "\"></div>");
   ...
   out.println("<div class=\"tabular_cell\">");
    ...
    out.println("<div class=\"sub_instructions pageHelp\" data-ftHelpTitle=\"Instructions\">");

    out.println("<h3 class=\"altTitle\">Instructions:</h3>");

    if(!rwd){
        out.println("<p>Use the time sheet below to select the time and location you'd like to reserve.</p>");
        out.print("<p>To display a different day's time sheet, select the date from the calendar on the left.</p>");
    } else {
        out.println("<p>Use the time sheet below to select the time and location you'd like to reserve. Available times will appear in green.</p>");
        out.print("<p class=\"calUseFull\">To display a different day's time sheet, select the date from the calendar on the left.</p>");
        out.print("<p class=\"calUseCompact\">To display a different day's time sheet, select the date using the calendar icon below.</p>");
    }
    out.println("<p>Special Events and Restrictions, if any are colored according to the legend below.</p>");
...
//        }
    ...
    */
    ...
    if(rwd){
        out.print("<div class=\"rwdCourseDate\"><div class=\"rwdCourseSelect\"><label><b>Displaying</b>");
    }
    //
    // Display Full Activity Tree (only if subactivities found)
    //
    if (getActivity.getSubActivityCount(sess_activity_id, con) > 0) {
        if(rwd){ // If responsive, use first child as default, if no child selected
            try {
                int parent_id = getActivity.getParentIdFromActivityId(activity_id, con);
                if(parent_id == activity_id){
                    ArrayList<Integer> child_ids = getActivity.getChildrenForActivity(activity_id, con);
                    activity_id = child_ids.get(0);
                }
        ...
        out.print("<span>"+getActivity.getActivityName(activity_id, con)+" </span>");
    }
    if(rwd){
        out.print("<b>Time Sheets</b></label></div><div class=\"rwdDateSelect\"><label><b>For</b><span>" + day_name + "</span><input type=\"text\" class=\"ft_date_picker_field calendar flxsheet ftMemberSheetCalendar\" data-ftdefaultdate=\"" + month + "/" + day + "/" + year + "\"  value=\"" + month + "/" + day + "/" + year + "\" data-ftjson=\"" + StringEscapeUtils.escapeHtml(jsonHashMap) + "\"></label></div>");
        // We will close the final div down further.
    }
    ...
    if (view_count > 1 && rwd){
        // Responsive mode
        // Responsive cannot support "Summary", se we'll use responsive tabbed.
        StringBuilder viewBar = new StringBuilder();
        viewBar.append("<div class=\"rwdViewSelect\"><label><b>In</b>");
        viewBar.append("<select class=\"rwdGensheetModeSelect\" onchange=\"document.forms['frmLoadDay'].layout_mode.value=this[this.selectedIndex].value;document.forms['frmLoadDay'].submit()\">");
        if (allow_summary||allow_old_summary){ // Responsive "Tabbed" mode
            viewBar.append("<option value=\"");
            viewBar.append("4");
            viewBar.append("\"");
            if(layout_mode == 4){ 
                viewBar.append(" selected");
            }
            viewBar.append(">Summary</option>");
        }
        if (allow_tabbed){ // Responsive "Tabbed" mode
            viewBar.append("<option value=\"");
            viewBar.append("1");
            viewBar.append("\"");
            if(layout_mode == 1){ 
            ...
            viewBar.append(">Tabbed</option>");
        }
        if (allow_detail){ // Responsive "Detailed" mode
            viewBar.append("<option value=\"");
            viewBar.append("3");
            viewBar.append("\"");
            if(layout_mode == 3){ 
            ...
            viewBar.append(">Detailed</option>");
        }
        viewBar.append("</select>");
        viewBar.append(" <b>View</b></label></div>");
        out.print(viewBar.toString());
    }
    
    if (rwd) {
        out.println("<div class=\"rwdTimeDisplay\">");
        out.println("<span class=\"serverClock\"><span>The Server Time is: <b class=\"jquery_server_clock\" data-ftclub=\"" + club + "\"></b></span>");
        out.println("<a class=\"helpButton\" href=\"#\" data-fthelp=\"about_server_clock\" title=\"How is this clock used?\"><span>How is this clock used?</span></a></span>");
        out.println("</div>");
    }
    //
    // START THE FORM
    ...
    out.println("</form>");
    if (rwd){
        out.print("</div>"); // Close activity / date / (view) container
    }
    ...
    if (!getActivity.isChildlessActivity(sess_activity_id, con)) {
        ...
        if (group_id > 0) {
                    ...
                    if (showRest) {    // Only display on legend if not suspended for entire day
                        ...
                        if ((rest_recurr.equals( "Every " + day_name )) ||          // if this day
                            ...
                            if (found == false) {      // if first one
                                ...
                                found = true;
                                if(rwd){
                                    out.print("<fieldset class=\"standard_legend_list\"><legend>Today's Restrictions</legend><div>");
                                } else {
                            ...
                            if(rwd){
                                out.print("<div><p style=\"background-color:" + rs.getString("color") + "\">" + rs.getString("name") + "</p></div>");
                            } else {
                                out.println("&nbsp;" +
                                        "<button style=\"background-color:" + rs.getString("color") + "\">" + rs.getString("name") + "" +
                                        "</button>" +
            ...
            //**********************************************
            //
            String memNoticeMsg = verifySlot.checkMemNotice(date, 0, 0, 0, "", day_name, "teesheet", false, sess_activity_id, con);
            ...
            if (!memNoticeMsg.equals("")) {
                ...
                try {
                    ...
                    if (rwd) {
                        out.println("<br><div style=\"text-align:center;width:100%;margin:auto;\"><table align=\"center\" border=\"2\" cols=\"1\" bgcolor=\"#f5f5dc\" cellpadding=\"0\" cellspacing=\"0\">");
                    } else {
                    ...
                    while (rs.next()) {
                        ...
                        if ((notice_mon == 1 && day_name.equals( "Monday" )) || (notice_tue == 1 && day_name.equals( "Tuesday" )) || (notice_wed == 1 && day_name.equals( "Wednesday" )) ||
                            ...
                            if (rwd) {
                                out.println("<td width=\"100%\" " + (!notice_bgColor.equals("") ? "bgcolor=\"" + notice_bgColor + "\"" : "") + " align=\"center\">");
                            } else {
                                out.println("<td width=\"700\" " + (!notice_bgColor.equals("") ? "bgcolor=\"" + notice_bgColor + "\"" : "") + " align=\"center\">");
                            }
                            /*
            ...
            //
            if (view_count > 1 && !rwd) {
                // Desktop mode (Responsive mode view is above)
                String view_bar = "";
    ...
    */
                ...
                try {
                    ...
                    stmt = con.createStatement();
                    rs = stmt.executeQuery("" +
                            "SELECT max_players, consec_mem_csv " +
                            "FROM activities " +
                    ...
                    while (rs.next()) {
                        ...
                        String [] temp = rs.getString("consec_mem_csv").split(",");
                ...
                out.println("<tr class=timesheetTH>");
                out.println("<td class=headerTD>Time</td>");
                if (!restrictAll && disp_consec) out.println("<td class=headerTD>Min.</td>");
                ...
                try {
                    ...
                    pstmt = con.prepareStatement(
                              "SELECT t1.*, t2.max_players, t2.activity_name, t2.disallow_joins, t2.consec_mem, t2.consec_mem_csv, t2.interval, t2.alt_interval, "
                            + "e.name, e.color, t2.hndcpMemSheet, "
                            + "DATE_FORMAT(date_time, \"%l:%i %p\") AS time, "
                            + "DATE_FORMAT(date_time, \"%H%i\") AS intTime, "
                            + "DATE_FORMAT(date_time, \"%Y%m%d\") AS date, "
                            + "IFNULL(GROUP_CONCAT(IF(asp.username <> '', asp.username, NULL) SEPARATOR ','), '') AS player_usernames "
                            + "FROM activity_sheets t1 "
                            + "LEFT OUTER JOIN activities t2 ON t2.activity_id = t1.activity_id "
                            + "LEFT OUTER JOIN activity_sheets_players asp ON t1.sheet_id = asp.activity_sheet_id "
                            + "LEFT OUTER JOIN events2b e ON e.event_id = t1.event_id "
                            + "WHERE "
                            + "t1.activity_id IN (SELECT activity_id FROM activities WHERE parent_id = ?) AND date_time BETWEEN ? AND ? "
                    ...
                    while ( rs.next() ) {
                        ...
                        if (rs.getInt("hndcpMemSheet") == 1) {
                            dispHdcp = true;
                        }
                        
                        try {
                            
                            pstmt2 = con.prepareStatement("SELECT count(*) AS player_count FROM activity_sheets_players WHERE activity_sheet_id = ?");
                            pstmt2.clearParameters();
                            pstmt2.setInt(1, rs.getInt("t1.sheet_id"));
                            
                            rs2 = pstmt2.executeQuery();
                            
                            if (rs2.next()) {
                                player_count = rs2.getInt("player_count");
                            }
                            
                        } catch (Exception e) {
                            Utilities.logError("Member_gensheets.doGet - " + club + " - Failed looking up count of players from activity_sheet_players for sheet_id: " + rs.getInt("t1.activity_id") + " - Error = " + e.toString());
                        } finally {
                        ...
                        // Gallery Golf Club wants these two courts to be staff-viewable only
                        if ((club.equals("gallerygolf") && (curr_activity_id == 7 || curr_activity_id == 8))
                                || (club.equals("oakhurstcc") && (curr_activity_id == 5))) {
                            continue;
                        }
                        ...
                        // If Governors Club and Courts 1-6 (activity_ids 2-7), only display times every 90 minutes. Skip the rest! activity_ids 8&9 Skip 8:30&9:00PM!
                        if (club.equals("governorsclub")) {

                            int tempTime = rs.getInt("intTime");

                            if (curr_activity_id >= 2 && curr_activity_id <= 7) {
                            ...
                            if ((tempTime == 2030 || tempTime == 2100) && (curr_activity_id == 8 || curr_activity_id == 9)) {
                                continue;
                            }


                        } else if (club.equals("lakewoodranch") && group_id == 1) {
                            
                            // Prior to 1pm, only display times every 1.5 hrs. for tennis
                            int tempTime = rs.getInt("intTime");
                            
                            if (tempTime < 1300 && tempTime != 830 && tempTime != 1000 && tempTime != 1130) {
                                continue;
                            }
                        } else if (club.equals("dorsetfc") || club.equals("islandcc")) {

                            // Prior to 12:30pm, only display times every 1.5 hrs.
                            int tempTime = rs.getInt("intTime");
                            
                            if (tempTime < 1230 && tempTime != 800 && tempTime != 930 && tempTime != 1100) {
                                continue;
                            }
                        } else if (club.equals("theranchcc")) {
                            
                            if (dateShort >= 901 || dateShort <= 531) {
                            
                                int tempTime = rs.getInt("intTime");
                            
                                if (day_name.equalsIgnoreCase("Saturday")) {    // Saturday only processing
                                    
                                    if (tempTime < 1230 && tempTime != 800 && tempTime != 930 && tempTime != 1100) {
                                        continue;
                                    }
                                    
                                } else if (!day_name.equalsIgnoreCase("Sunday")) {    // Monday - Friday processing
                                    
                                    if (tempTime >= 1800 && tempTime < 2100 && tempTime != 1800 && tempTime != 1930) {
                                        continue;
                                    }
                                }
                            }
...
//                                }
//                            }
                        } else if (club.equalsIgnoreCase("mosscreek")) {

                            int tempTime = rs.getInt("intTime");
                            if (tempTime < 1200 && tempTime != 800 && tempTime != 1000 && curr_activity_id != 3) {
                                if (curr_activity_id != 2) {
        ...
        */
                            ...
                            if (!rs.getString("player_usernames").equals("")) {
                                
                                String[] player_usernames = rs.getString("player_usernames").split(",");

                                hidden_mem_loop:
                                for (int j = 0; j < player_usernames.length; j++) {
                                    
                                    // See if any hidden members are a part of this time
                                    if (!hideNames && !has_hidden_mem && hidden_mems.containsKey(player_usernames[j]) 
                                            && (!hidden_mems.get(player_usernames[j]).equalsIgnoreCase(user_mNum) || user_mNum.equals(""))) {
                                        
                                        has_hidden_mem = true;
                                        break hidden_mem_loop;
                                    }
                                }
                            }                            
                            ...
                            if (club.equals("lakewoodranch") && rs.getInt("intTime") < 1300 && group_id == 1) {
                                consec_csv = "3";
                            } else if ((club.equals("dorsetfc") || club.equals("islandcc")) && rs.getInt("intTime") < 1230) {
                                consec_csv = "3";
                            } else if (club.equals("mosscreek") && rs.getInt("intTime") < 1200) {
                                if (curr_activity_id != 3) {
                                    consec_csv = "4,3,2,1";
                                } else {
                                   consec_csv = "4"; 
                                }
                                ...
                                if (dateShort >= 901 || dateShort <= 531) {
                                    ...
                                    if (day_name.equalsIgnoreCase("Saturday") && tempTime < 1230) {    // Saturday only processing
                                        consec_csv = "3";
                                    } else if (!day_name.equalsIgnoreCase("Sunday") && !day_name.equalsIgnoreCase("Saturday")) {    // Monday - Friday processing

                                        if (tempTime >= 1800 && tempTime < 2100) {
                                            consec_csv = "3";
                                        } else if (tempTime == 1730) {    // Need to make sure no one books over the 6:00pm 90-minute interval!
                                            consec_csv = "1";
                                        } else if (tempTime == 1700) {    // Need to make sure no one books over the 6:00pm 90-minute interval!
                                            consec_csv = "2,1";
                            ...
                            } else if (!restrictAll && disp_consec) {
                            ...
                            if (rs.getInt("lesson_id") == 0 && rs.getInt("event_id") == 0) {
                                ...
                                try {
                                    ...
                                    pstmt2 = con.prepareStatement(
                                            "SELECT asp.username, player_name, IFNULL(ntrp_rating, -1.0) AS ntrp_rating, ("
                                            + "SELECT COUNT(*) AS players "
                                            + "FROM activity_sheets_players "
                                            + "WHERE activity_sheet_id = ? AND username = ?) AS part_of "
                                            + "FROM activity_sheets_players asp "
                                            + "LEFT JOIN member2b m ON asp.username = m.username "
                                            + "WHERE activity_sheet_id = ? "
                                            + "ORDER BY pos");
                                    pstmt2.clearParameters();
                                    pstmt2.setInt(1, rs.getInt("sheet_id"));
                                    ...
                                    while ( rs2.next() ) {
                                        ...
                                        if ( !started_row ) {
                                            ...
                                            // CUSTOM - if club requests this - make slot NOT selectable if user is not part of it
                                            if (selectable && rs2.getInt("part_of") != 1 && !rs.getString("orig_by").equalsIgnoreCase(user) 
                                                    && (disallow_joins || hideNames || has_hidden_mem || player_count >= rs.getInt("max_players") || rs.getInt("force_singles") == 1)) {
                                        ...
                                        if (club.equals("bwrc") && (rs2.getString("player_name").equalsIgnoreCase("Dave LESSON_with_Dave") || rs2.getString("player_name").equalsIgnoreCase("Tyler LESSON_with_Tyler"))) {
                                            has_hidden_mem = true;
                                        }

                                        out.print("<td class=timesheetTD nowrap" + (!color.equals("") ? " style=\"background-color:" + color + "\"" : "") + ">");
                                        
                                        if ((hideNames || has_hidden_mem) && rs2.getInt("part_of") != 1 && !rs.getString("orig_by").equalsIgnoreCase(user)) {
                                            
                                            if (!rs2.getString("asp.username").equals("")) { 
                                                out.print("&nbsp;Member</td>");
                                            } else if (rs2.getString("player_name").equalsIgnoreCase("X")) {
                                                out.print("&nbsp;X</td>");
                                            } else {
                                                out.print("&nbsp;Guest</td>");
                                            }
                                        } else {
                                            String player_disp = rs2.getString("player_name");
                                            
                                            if (dispHdcp && rs2.getDouble("ntrp_rating") > 0) {
                                                player_disp += " " + rs2.getDouble("ntrp_rating");
                                            }
                                            out.print("&nbsp;" + player_disp + "</td>");
                                        }
                ...
                try {
                    ...
                    while ( rs.next() ) {
                        ...
                        // Gallery Golf Club wants these two courts to be staff-viewable only
                        if ((club.equals("gallerygolf") && (rs.getInt("activity_id") == 7 || rs.getInt("activity_id") == 8))
                                ||(club.equals("oakhurstcc") && rs.getInt("activity_id") == 5)) {
                            continue;
                        }
...
//                                order_pos = i*activity_ids_per_row;
                            ...
                            try {
                                ...
                                pstmt2 = con.prepareStatement(""
                                        + "SELECT COUNT(*) AS players_found, username, SUBSTRING_INDEX(player_name, ' ', -1) AS last_name, ("
                                         ...
                                         + "FROM activity_sheets_players "
                                         + "WHERE activity_sheet_id = ? AND username = ?) AS part_of, "
                                        + "GROUP_CONCAT(username SEPARATOR ',') AS player_usernames "
                                        + "FROM activity_sheets_players "
                                        + "WHERE activity_sheet_id = ? "
                                ...
                                if (players_found > 0) {
                                    ...
                                    if (club.equals("bwrc")) {
                                        ...
                                        if (last_name.equalsIgnoreCase("LESSON_with_Dave") || last_name.equalsIgnoreCase("LESSON_with_Tyler")) {
                                            hideNames = true;
                                        }
                                    }
                                    
                                    if (!hideNames && part_of != 1 && !rs.getString("orig_by").equalsIgnoreCase(user)) {
                                        
                                        String [] player_usernames = rs2.getString("player_usernames").split(",");

                                        hidden_mem_loop:
                                        for (int j = 0; j < player_usernames.length; j++) {

                                            if (hidden_mems.containsKey(player_usernames[j]) && (!hidden_mems.get(player_usernames[j]).equalsIgnoreCase(user_mNum) || user_mNum.equals(""))) {
                                                has_hidden_mem = true;
                                                break hidden_mem_loop;
                                            }
                                        }
                                    ...
                                    if (restrictAll || ((disallow_joins || hideNames || has_hidden_mem) && part_of != 1 && !rs.getString("orig_by").equalsIgnoreCase(user))) {
                                    ...
                                    if (allow && (part_of == 1 || rs.getString("orig_by").equalsIgnoreCase(user) || (rs.getInt("force_singles") == 0 && players_found < rs.getInt("max_players")))) {
                                    ...
                                    // if time slot can be joined display the green bar
                                    if (allow && (part_of == 1 || rs.getString("orig_by").equalsIgnoreCase(user) || (rs.getInt("force_singles") == 0 && players_found < rs.getInt("max_players"))) && ((!is_today && !restrictAll) || (is_today && rs.getInt("intTime") > curr_time))) out.print("<span style=\"background-color:darkGreen\">&nbsp;</span>");
                                    out.print("<nobr>&nbsp;" + ((username.equals("")) ? ((last_name.equalsIgnoreCase("x")) ? "X" : "Guest") : ((hideNames || has_hidden_mem) ? "Member" : last_name)) + " (" + players_found + ")<nobr>");
             ...
             // end layout 4
            } else if (rwd && layout_mode == 1 || layout_mode == 3) {
                // RWD Detailed and Tabbed layout mode
            
                //
                //*******************************************************************************************************
                // RWD Layout Mode 1 & 3 (Detailed and Tabbed / RWD) - Display Time Sheet (Member View - multiple activities - vertical display)
                //*******************************************************************************************************
                //
                ...
                int max_players = 0;
                boolean disp_consec = false;
                boolean tabbedMode = (layout_mode == 1);
                ...
                try {
                    ...
                    sql.append("SELECT t1.*, t2.max_players, t2.activity_name, t2.disallow_joins, t2.consec_mem, t2.consec_mem_csv, t2.interval, t2.alt_interval, e.name, e.color, t2.hndcpMemSheet, "
                            + "DATE_FORMAT(date_time, \"%l:%i %p\") AS time, "
                            + "DATE_FORMAT(date_time, \"%H%i\") AS intTime, "
                            + "DATE_FORMAT(date_time, \"%Y%m%d\") AS date, "
                            + "IFNULL(GROUP_CONCAT(IF(asp.username <> '', asp.username, NULL) SEPARATOR ','), '') AS player_usernames "
                            ...
                            + "LEFT OUTER JOIN activity_sheets_players asp ON t1.sheet_id = asp.activity_sheet_id "
                            + "LEFT OUTER JOIN events2b e ON e.event_id = t1.event_id "
                            + "WHERE t1.activity_id IN (SELECT activity_id FROM activities WHERE parent_id = ?) AND date_time BETWEEN ? AND ? "
                            + "GROUP BY t1.sheet_id ");
                    
                    if(tabbedMode){
                        sql.append("ORDER BY sort_by, activity_name, date_time");
                    } else {
                        sql.append("ORDER BY date_time, sort_by, activity_name");
                    }
                    ...
                    boolean disallow_joins = false;
                    String last_time = "";
                    String activityCommonName = getActivity.getActivityCommonName(activity_id, con);
                    boolean firstPass = true;
                    boolean has_hidden_mem = false;
                    boolean dispHdcp = false;
                    
                    StringBuilder closeTableHtml = new StringBuilder();
                    ...
                    closeTableHtml.append("</"+tags.tbody+"></"+tags.table+">");
                    if(tabbedMode){
                        closeTableHtml.append("</div>");
                    }
                    ...
                    while ( rs.next() ) {
                        ...
                        if(last_activity_id == 0){
                            last_activity_id = curr_activity_id;
                        }
...
//                            }
                        ...
                        if((tabbedMode && last_activity_id != curr_activity_id) || firstPass){
                            if(!firstPass){
                                // Close previous tab
                                tablesHtml.append(closeTableHtml.toString());
                            }
                            if(tabbedMode){
                                tablesHtml.append("<div class=\"rwdTabContent");
                                tablesHtml.append(firstPass?" selected":"");
                                tablesHtml.append("\" id=\"genSheetTab");
                                tablesHtml.append(tabCount);
                                tablesHtml.append("\">");
                                tabsHtml.append("<li class=\"");
                                tabsHtml.append(firstPass?"selected":"");
                                tabsHtml.append("\"><a href=\"#genSheetTab");
                                tabsHtml.append(tabCount);
                                tabsHtml.append("\">");
                                tabsHtml.append(rs.getString("activity_name"));
                                tabsHtml.append("</a></li>");
                                selectHtml.append("<option");
                                selectHtml.append(firstPass?" selected":"");
                                selectHtml.append(" value=\"#genSheetTab");
                                selectHtml.append(tabCount);
                                selectHtml.append("\">");
                                selectHtml.append(rs.getString("activity_name"));
                                selectHtml.append("</option>");
                            }
                            tablesHtml.append("<"+tags.table+" class=\"rwdTable standard_list_table member_gensheet_detail rwdCompactible\">");
                            // header row
                            tablesHtml.append("<"+tags.thead+" class=\"rwdThead\">");
                            tablesHtml.append("<"+tags.tr+" class=\"rwdTr\">");
                            tablesHtml.append("<"+tags.th+" class=\"rwdTh sTime\">Time</"+tags.th+">");
                            if (!restrictAll && disp_consec) tablesHtml.append("<"+tags.th+" class=\"rwdTh sSelect\">Min.</"+tags.th+">");
                            tablesHtml.append("<"+tags.th+" class=\"rwdTh sName\">");
                            tablesHtml.append(activityCommonName );
                            tablesHtml.append("</"+tags.th+">");
                            tablesHtml.append("<"+tags.th+" class=\"rwdTh sPlayer\">Players</"+tags.th+">");
                            tablesHtml.append("</"+tags.tr+">");
                            tablesHtml.append("</"+tags.thead+">");
                            tablesHtml.append("<"+tags.tbody+" class=\"rwdTbody\">");
        ...
        */
                            ...
                            String TR_selectable = "<"+tags.tr+" class=\"rwdTr\"><"+tags.td+" class=\"rwdTd sT\"><a class=\"standard_button teetime_button\" href=\"#\" onclick=\"gotoTimeSlot(" + rs.getInt("sheet_id") + "," + group_id + "," + date + "," + rs.getInt("force_singles") + "," + layout_mode + ", " + (consec.size() > 0 ? consec.get(0) : "0") + "); return false;\">" + rs.getString("time") + "</a></"+tags.td+">";
                            String TR_nonselectable = "<"+tags.tr+" class=\"rwdTr\"" + ((color.equals("")) ? "" : " style=\"background-color:" + color + "\"") + "><"+tags.td+" class=\"rwdTd sT\"><div class=\"time_slot\">" + rs.getString("time") + "</div></"+tags.td+">";

                            if (!restrictAll && consec.size() > 1) {

                                TR_selectable += "" +
                                        "<"+tags.td+" class=\"rwdTd sS\">" +
                                        "<select name=consec size=1 " +
                                        " onchange=\"gotoTimeSlot(" + rs.getInt("sheet_id") + "," + group_id + "," + date + "," + rs.getInt("force_singles") + "," + layout_mode + ", this.options[this.selectedIndex].value)\">";
                                ...
                                //}
                                */
                                TR_selectable += "</select></"+tags.td+">";

                                TR_nonselectable += "<"+tags.td+" class=\"rwdTd sS\"></"+tags.td+">";

                            } else if (!restrictAll && disp_consec) {

                                // other activities being displayed allow consecutive times but not this one

                                TR_selectable += "<"+tags.td+" class=\"rwdTd sS\"></"+tags.td+">";
                                TR_nonselectable += "<"+tags.td+" class=\"rwdTd sS\"></"+tags.td+">";

                            }
                            
                            String TD_activityName = "<"+tags.td+" class=\"rwdTd " + ((alt) ? "timesheetTD3alt" : "timesheetTD3") + " sN\">" + rs.getString("activity_name") + "</"+tags.td+">";
                            String TD_playerStart = "<"+tags.td+" class=\"rwdTd sP\"" + (!color.equals("") ? " style=\"background-color:" + color + "\"" : "") + ">";
                            ...
                            if (rs.getInt("lesson_id") == 0 && rs.getInt("event_id") == 0) {
                                ...
                                try {
                                    ...
                                    while ( rs2.next() ) {
                                        ...
                                        if ( !started_row ) {
                                            ...
                                            tablesHtml.append( ( selectable) ? TR_selectable : TR_nonselectable );
                                            tablesHtml.append(TD_activityName);
                                            tablesHtml.append(TD_playerStart);
                                        }

                                        players_found++;

                                        tablesHtml.append("<div");
                                        if(user.equalsIgnoreCase(rs2.getString("username"))){
                                            tablesHtml.append(" class=\"me\"");
                                        }
                                        tablesHtml.append("><span>");
                                        ...
                                        if ((hideNames || has_hidden_mem) && rs2.getInt("part_of") != 1 && !rs.getString("orig_by").equalsIgnoreCase(user)) {
                                            ...
                                            if (!rs2.getString("asp.username").equals("")) { 
                                                tablesHtml.append("&nbsp;Member</td>");
                                            } else if (rs2.getString("player_name").equalsIgnoreCase("X")) {
                                                tablesHtml.append("&nbsp;X</td>");
                                            } else {
                                                tablesHtml.append("&nbsp;Guest</td>");
                                            }
                                        } else {
                                            tablesHtml.append(rs2.getString("player_name"));
                                            
                                            if (dispHdcp && rs2.getDouble("ntrp_rating") > 0) {
                                                tablesHtml.append(" " + rs2.getDouble("ntrp_rating"));
                                            }
                                        }
                                        tablesHtml.append("</span></div>");
                                    ...
                                    out.print("<p>ERROR LOADING PLAYER:" + exc.toString() + "</p>");
                                ...
                                if (players_found == 0 && !rname.equals("")) { //emptySlot &&
                                    ...
                                    tablesHtml.append(TD_activityName);
                                    tablesHtml.append(TD_playerStart);
                                    tablesHtml.append("<div class=\"resSlot\">");
                                    tablesHtml.append(rname);
                                    tablesHtml.append("</div>");
                                    started_row = true;
                                }
                                // Close players cell;
                                if(started_row){
                                    tablesHtml.append("</"+tags.td+">");
                                }
                                //

                            } else {

                                // this time slot is for either an event or a lesson

                                tablesHtml.append( TR_nonselectable );
                                tablesHtml.append(TD_activityName);
                                started_row = true;
                                ...
                                if (rs.getInt("event_id") != 0) {
                                    ...
                                    // time is covered by an event
                                    tablesHtml.append(TD_playerStart);
                                    tablesHtml.append("<div class=\"evntSlot\">" );
                                    tablesHtml.append(rs.getString("name"));
                                    tablesHtml.append("</div></"+tags.td+">"); // align=center class=timesheetTD

                                } else if (rs.getInt("lesson_id") != 0) {

                                    tablesHtml.append("<"+tags.td+" class=\"rwdTd sP\"><div class=\"clinicSlot\">");
                                    tablesHtml.append((rs.getInt("lesson_id") > 0) ? sysLingo.TEXT_Lesson : "Clinic");
                                    tablesHtml.append("</div></"+tags.td+">"); // align=center class=timesheetTD
                    ...
                    if(tabbedMode){
                        out.print("<div class=\"rwdTabOrSelect "+Common_skin.getSizeClass(tabCount)+"TabCount\"><div class=\"rwdTabs\"><ul>");
                        out.print(tabsHtml.toString());
                        out.print("</ul></div><div class=\"rwdTabSelect\"><label><b>"+activityCommonName+"</b><select>");
                        out.print(selectHtml.toString());
                        out.print("</select></label></div></div>");
                    }
                    out.print(tablesHtml.toString());
                ...
                //
                // RWD LAYOUT MODE 4 - RESPONSIVE SUMMARY VIEW
                //
...
//                for (int i=0; i<=((order.size()-1)/activity_ids_per_row); i++) {
                for (int i = 0; i < cols_per_row.size(); i++) {
                    ...
                    StringBuilder timeHtml = new StringBuilder();
                    StringBuilder chartHtml = new StringBuilder();

                    curr_activity_instring = "";

                    timeHtml.append("<div class=\"rwdLeftStaticColumn\"><table class=\"standard_list_table\">");
                    timeHtml.append("<thead><th><span>Time</span></th></thead>");
                    
                    chartHtml.append("<div class=\"rwdRightScrollTable\"><table class=\"standard_list_table\">");
                    chartHtml.append("<thead>");

//                    for (int j = (i*activity_ids_per_row); j < ((i+1)*activity_ids_per_row) && j < order.size(); j++) {
                    for (int j = col_count; j < (col_count + cols_per_row.get(i)) && j < order.size(); j++) {
                        curr_activity_instring += (!curr_activity_instring.equals("") ? "," : "") + order.get(j);   // Add activity_ids to a csv string to be used when pulling activity_sheets below
                        chartHtml.append("<th><span>");
                        chartHtml.append(activity_names.get(j));
                        chartHtml.append("</span></th>");
                    }
                    chartHtml.append("</thead>");
                    ...
                    try {
                        ...
                        pstmt = con.prepareStatement(
                                "SELECT t1.*, t2.activity_name, t2.max_players, t2.disallow_joins, t2.consec_mem_csv, t2.interval, t2.alt_interval, e.name, e.color, t2.hndcpMemSheet, "
                                + "DATE_FORMAT(date_time, \"%l:%i %p\") AS time, "
                                + "DATE_FORMAT(date_time, \"%H%i\") AS intTime, "
                                + "DATE_FORMAT(date_time, \"%Y%m%d\") AS date, "
                                + "IFNULL(GROUP_CONCAT(IF(asp.username <> '', asp.username, NULL) SEPARATOR ','), '') AS player_usernames "
                                ...
                                + "LEFT OUTER JOIN restriction2 r ON r.id = t1.rest_id "
                                + "WHERE "
                                + "t1.activity_id IN (" + curr_activity_instring + ") AND date_time BETWEEN ? AND ? "
                                + "GROUP BY t1.sheet_id "
                                + "ORDER BY date_time, sort_by, activity_name");
                        ...
                        boolean has_hidden_mem = false;
                        boolean dispHdcp = false;
                        timeHtml.append("<tbody>");
                        chartHtml.append("<tbody>");
...
//                                }
                            ...
                            if ( !last_time.equals(rs.getString("time")) ) {
                                last_time = rs.getString("time");
                                timeHtml.append("<tr><td><div class=\"time_slot\">");
                                timeHtml.append(last_time);
                                timeHtml.append("</div></td></tr>");
//                                order_pos = i*activity_ids_per_row;
                                order_pos = col_count;
                                if(row > 0){
                                    chartHtml.append("</tr>");
                            ...
                            if (!rs.getString("in_use_by").equals("")) {
                                ...
                                onclick = "";
                                td_style = "style=\"background: " + color + ";\"";   // use its color
                                tdClassList.add("isRestricted");

                            }

                            // see if we've skipped over any courts
                            while (order.get(order_pos) != curr_activity_id) {
                                chartHtml.append("<td class=\"isSkipped\"></td>");
                                order_pos++;
                            }
                            
                            if (rs.getString("orig_by").equalsIgnoreCase(user)) {    // If user is the orig, mark them as part of the time right away
                                part_of = 1;
                            }
                            
                            if (part_of != 1 && !rs.getString("player_usernames").equals("")) {
                                ...
                                for (int j = 0; j < player_usernames.length; j++) {
                                    ...
                                    // See if the member is a part of this time.
                                    if (player_usernames[j].equalsIgnoreCase(user)) {
                                        part_of = 1;
                                        has_hidden_mem = false;    // Don't want to hide member names for this time if the current user is in it
                                        break hidden_mem_loop;
                                    }
                            ...
                            try {
                                ...
                                while(rs2.next()){
                                    ...
                                    List<String> playerPartsL = Arrays.asList(playerName.split(" "));
                                    ArrayList<String> playerParts = new ArrayList<String>(playerPartsL); 
                                    String lastName = playerParts.get(playerParts.size()-1);
                                    username = rs2.getString("asp.username");
                                    boolean isGuest = false;
                                    
                                    if (club.equals("bwrc") && (lastName.equalsIgnoreCase("LESSON_with_Dave") || lastName.equalsIgnoreCase("LESSON_with_Tyler"))) {
                                        hideNames = true;
                                    }
                                    ...
                                    } else if (part_of != 1 && (hideNames || has_hidden_mem)) {
                                        lastName = "Member";
                                    }
                                    
                                    if(playerParts.size() > 0){
                                        playerParts.remove(playerParts.size()-1);
                                    }
                                    
                                    String firstName = Utilities.implode(playerParts, " ");
                                    String dispName = lastName;
                                                                   
                                    if (dispHdcp && rs2.getDouble("ntrp_rating") > 0) {
                                        dispName += " " + rs2.getDouble("ntrp_rating");
                                    }
                                                                  
                                    playerHtml.append(tags.getTag("span", tags.getTag("span", dispName), 
                                            user.equalsIgnoreCase(username)?"me":""));
                                }
                                
                                if (players_found > 0) {
                                    
                                    tdClassList.add("hasPlayers");
                                    // EXISTING PLAYERS

                                    if (((disallow_joins || hideNames || has_hidden_mem) && part_of != 1 && !rs.getString("orig_by").equalsIgnoreCase(user)) || restrictAll) {
                                        allow = false;
                                        onclick = "";
                                    }
                                    ...
                                    if (allow && (part_of == 1 || (rs.getInt("force_singles") == 0 && players_found < rs.getInt("max_players")))) {
                                        ...
                                        // time slot is full - change bgcolor to indicate it's not clickable
                                        //chartHtml.append("style=\"background: white;\"");
                                    }
                                    String closeLinkHtml = null;
                                    // if time slot can be joined display the green bar
                                    if (allow && (part_of == 1 || rs.getString("orig_by").equalsIgnoreCase(user) || (rs.getInt("force_singles") == 0 && players_found < rs.getInt("max_players"))) 
                                            && ((!is_today && !restrictAll) || (is_today && rs.getInt("intTime") > curr_time))) {
                                        //chartHtml.append("<span class=\"leftBlock\"></span>");
                                        tdClassList.add("canJoin");
                                        linkHtml.append("<a href=\"javascript:void(0)\" class=\"standard_button\" ");
                                        linkHtml.append(onclick);                                 
                                        linkHtml.append(">");
                                        closeLinkHtml = "</a>";
                                    } else {
                                        tdClassList.add("isFull");
                                        linkHtml.append("<span class=\"no_slot\" ");
                                        //linkHtml.append(onclick + " " + td_style);
                                        linkHtml.append(">");
                                        closeLinkHtml = "</span>";
                                    }
                                    linkHtml.append("<span>");
                                    linkHtml.append(playerHtml);
                                    linkHtml.append("</span>");
                                    linkHtml.append("<span>");
                                    linkHtml.append(players_found);
                                    linkHtml.append("</span>");
                                    linkHtml.append(closeLinkHtml);
                                    ...
                                    if ( rs.getString("in_use_by").equals("") && rs.getInt("lesson_id") == 0 && rs.getInt("event_id") == 0 &&
                                        ...
                                        if (rs.getInt("lesson_id") != 0) {
                                            ...
                                            if (rs.getInt("lesson_id") < 0) {
                                                ...
                                                String lname = tok.nextToken();       // get the name of this lesson
                                                String lcolor = tok.nextToken();      // get the color for this lesson
                                                tdClassList.add("isClinic");
                                                //chartHtml.append("<span title=\"");
                                                linkTitle = lname;
                                                linkName = "Clinic";
                                                //chartHtml.append("\" class=\"genSheetClinic\">Clinic</span>");

                                            } else {
                                                tdClassList.add("isLesson");
                                                //chartHtml.append("<span class=\"genSheetLesson\">");
                                                linkName = sysLingo.TEXT_Lesson;
                                                //chartHtml.append("</span>");
                                            }

                                        } else if (rs.getInt("event_id") > 0) {
                                            tdClassList.add("isEvent");
                                            linkName = "Event";
                                            //chartHtml.append("<span class=\"genSheetEvent\">Event</span>");

                                        } else if (rs.getInt("blocker_id") > 0) {
                                            tdClassList.add("isBlocked");
                                            linkName = "Blocked";
                                            //chartHtml.append("<span class=\"genSheetBlocked\">Blocked</span>");
                                        } else {
                                            tdClassList.add("isInUse");
                                            linkName = "In Use";
                                            //chartHtml.append("<span class=\"genSheetInUse\">in use</span>");
                                    ...
                                    if(onclick.equals("")){
                                        if(!linkName.equals("")){
                                            linkHtml.append("<span class=\"no_slot\" ");
                                            linkHtml.append(onclick);
                                            linkHtml.append(" title=\"");
                                            linkHtml.append(linkTitle);
                                            linkHtml.append("\">");
                                            linkHtml.append("<span>");
                                            linkHtml.append(linkName);
                                            linkHtml.append("</span>");
                                            linkHtml.append("</span>");
                                        }
                                    } else {
                            ...
                            chartHtml.append("<td class=\"");
                            chartHtml.append(Utilities.implode(tdClassList, " "));
                            chartHtml.append("\" ");
                            chartHtml.append(td_style);
                            chartHtml.append(">");
                            chartHtml.append(linkHtml.toString());
                            chartHtml.append("</td>");
                    ...
                    // end tables
                    chartHtml.append("</tbody></table></div>");
                    timeHtml.append("</tbody></table></div>");
                    
                    out.print("<div class=\"rwdTableGroup rwdGenSheets\"><div class=\"rwdTableGroupWrap\">");
                    out.print(timeHtml.toString());
                    out.print(chartHtml.toString());
                    out.print("</div></div>");

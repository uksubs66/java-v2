...
public class Proshop_event_list extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   List<Integer> activity_ids = new ArrayList<Integer>();
...
//   }
   ...
   // Add their current activity as the first one
   if (user.equals(DINING_USER)) {
       activity_ids.add(dining_activity_id);
   } else {
       activity_ids.add(sess_activity_id);
   }

   // Look up all other activities which have current events configured
   try {
       
       pstmt = con.prepareStatement("SELECT activity_id FROM events2b WHERE IF(season=1, c_date, date) >= ? AND activity_id <> ? AND inactive = 0 GROUP BY activity_id ORDER BY activity_id");
       ...
       Utilities.logError("Proshop_event_list.doGet - " + club + " - Failed looking up activity_ids with configured events - Error=" + e.toString());
   } finally {
       Connect.close(rs, pstmt);
   }
   
   // Add dining last, if it wasn't added earlier
   if (organization_id != 0 && !user.equals(DINING_USER)) {
       activity_ids.add(dining_activity_id);
   }
   ...
   for (Integer act_id : activity_ids) {
       
       if (act_id == 0) {    // Golf
           activity_name = "Golf";
       } else if (act_id == dining_activity_id) {    // Dining
           activity_name = "Dining";
       } else {    // FlxRez Activity
           activity_name = getActivity.getActivityName(act_id, con);
       }
       
       out.println("<tr><td align=\"center\" valign=\"top\"><BR /><BR />");
       
       // Print activity name header
       out.println("<span style=\"margin-left:auto; margin-right:auto; font-weight:bold; font-size:larger;\">" + activity_name + " Events</span><BR />");
       
       out.println("<font size=\"2\"><br>");
       out.println("<table border=\"1\" bgcolor=\"#F5F5DC\" cellpadding=\"5\" valign=\"top\">");
       out.println("<font size=\"2\" face=\"Arial, Helvetica, Sans-serif\">");

       if (act_id != dining_activity_id) {   // Not Dining      
       ...
       try {
          ...
          if (act_id != 9999) {   // if NOT Dining
             ...
             String sql = "" +
                ...
                "AND activity_id = " + act_id + " AND inactive = 0 ";
             ...
             while (rs.next()) {
                ...
                sfb = rs.getString("fb");
                c = rs.getInt("c");                     // total count of all signups
                c2 = rs.getInt("c2");                    // count of signups not moved, if zero then we'll hide this event
                date2 = rs.getLong("date2");
                ...
                if (signUp != 0 || max != 0 || gstOnly != 0) {
                   ...
                   if (act_id == 0 && edate < date && (signUp != 0 || gstOnly != 0)) {
                   ...
                   if (skip == false) {                // if ok to display
                      ...
                      if (shr > 12) {
                       ...
                       out.println("<input type=\"hidden\" name=\"activity_id\" value=\"" + act_id + "\">");
          ...
          if (found == false) {
              ...
              out.println("<p align=\"center\">There are no " + activity_name + " events scheduled at this time.</p>");
              out.println("</td></tr>");
          }    // end of if

          out.println("<tr><td><br><form><p align=center>");
          out.println("<input type=\"button\" value=\"Cancel - Close\" onClick='self.close();'>");
          out.println("</p></form></td></tr>");
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   int activity_id = 0;
     ...
     try { event_id = Integer.parseInt(req.getParameter("event_id")); } catch (Exception ignore) { }
     try { activity_id = Integer.parseInt(req.getParameter("activity_id")); } catch (Exception ignore) { }
   ...
   out.println("<BR><p align=\"center\">http://web.foretees.com/"+rev+"/servlet/Login?extlogin=yes&caller=event&act_id=" + activity_id + "&event=" + event_id + "&els=##ELS##</p>");

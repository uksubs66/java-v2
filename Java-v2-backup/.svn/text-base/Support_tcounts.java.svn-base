...
public class Support_tcounts extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   try {
      ...
      rs2 = stmt2.executeQuery("SELECT clubname, fullname, inactive FROM clubs ORDER BY fullname");

      while (rs2.next()) {

         club = rs2.getString(1);                 // get club name
         fullname = rs2.getString(2);             // get club's full name
         inact = rs2.getInt(3);                   // get inact flag

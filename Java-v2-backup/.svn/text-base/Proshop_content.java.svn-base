...
public class Proshop_content extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    } else if (req.getParameter("linkBuilder") != null) {

        linkBuilder(req, resp, out, con);
        out.close();
        return;
...
private String buildContent(int id, Connection con) {
 ...
 private void linkBuilder(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, Connection con) {
    ...
    // if activity id is being passed in then use it
    try { 
        int tmpId = Integer.parseInt(req.getParameter("activityId")); 
        activity_id = tmpId;
    } catch (Exception ignore) {}
    
    // get the request destination page
    String destination = (req.getParameter("destination") != null) ? req.getParameter("destination") : "";
    
    // if event id is being passed in then set it
    try { 
        event_id = Integer.parseInt(req.getParameter("eventId")); 
    } catch (Exception ignore) {}
    
    Map<String, Object> activity_map = new LinkedHashMap<String, Object>();
    Map<String, Object> destination_map = new LinkedHashMap<String, Object>();
    List<Map<String,Object>> event_list = new ArrayList<Map<String,Object>>();
    
    activity_map = mobileAPI.getActivityList(club, con);
    destination_map = mobileAPI.getDestinationList(activity_id, club, con);
    
    out.println("<form>");
    out.println("<input type=\"hidden\" name=\"linkBuilder\" value=\"\">");
    
    out.println("<select size=\"1\" name=\"activityId\" onChange=\"this.form.submit()\">");
    for (Map.Entry<String, Object> entry : activity_map.entrySet()) {
        Common_Config.buildOption((Integer)entry.getValue(), (String)entry.getKey(), activity_id, out);
    }
    out.println("</select>");
    
    
    out.println("<select size=\"1\" name=\"destination\" onChange=\"this.form.submit()\">");
    for (Map.Entry<String, Object> entry : destination_map.entrySet()) {
        Common_Config.buildOption(entry.getValue().toString(), entry.getKey().toString(), destination, out);
    }
    out.println("</select>");
    
    
    if (destination.contains("events")) {
        
        event_list = mobileAPI.getEventList(activity_id, 0, 0, club, con);
        
        out.println("<select size=\"1\" name=\"eventId\" onChange=\"\">");
        
        for (Map<String, Object> eventMap : (List<Map<String,Object>>) event_list) {
            
            if ( (eventMap.get("eventName") instanceof String) && (eventMap.get("eventDate") instanceof String) && (eventMap.get("eventId") instanceof Integer) ) {
                
                Common_Config.buildOption((Integer)eventMap.get("eventId"), (String)eventMap.get("eventName") + " " + eventMap.get("eventDate"), event_id, out);
            }
        }
    ...
    out.println("<input type=\"submit\" name=\"button_submit\" value=\"Build Link\">");
    ...
    if (req.getParameter("button_submit") != null) {
        ...
        boolean add_question = (!destination.contains("?"));
        
        link.append(Utilities.getExternalBaseUrl(req, activity_id, club, null));
        link.append(destination);
        
        if (event_id > 0) {
            if (add_question) link.append("?");
            link.append("event_id=" + event_id);
        }
        
        out.println("<p>" + link.toString() + "</p>");

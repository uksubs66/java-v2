...
public class Member_activity_slot extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        boolean blockP1 = true;
        boolean blockP2 = true;
        boolean blockP3 = true;
        boolean blockP4 = true;
        ...
        String[] userA = new String[parmAct.max_players + 1];
        Arrays.fill(userA, "");
        ...
        dateShort = date - ((date / 10000) * 10000);       // get mmdd (i.e.  20060512 - 20060000 = 512)
            ...
            if (req.getParameter("letter") != null || req.getParameter("return") != null
                ...
                if (parmAct.disallow_joins == 1) {
                    ...
                    for (int i=0; i < slotParms.sheet_ids.size(); i++) {
                        ...
                        if (!verifyActSlot.checkMemberHasAccess(slotParms.sheet_ids.get(i), user, con)) {    // Member does not have access to this time
                        ...
                        } else {    // Member has access to this time
                            
                            // Update in_use_at timestamp to the current time so that we don't free up their time early
                            try {
                                
                                pstmt = con.prepareStatement("UPDATE activity_sheets SET in_use_by = ?, in_use_at = now() WHERE sheet_id = ?");
                                pstmt.clearParameters();
                                ...
                                Utilities.logError("Member_activity_slot.doPost - " + club + " - Error updating in_use_at timestamp to current time. ERR: " + exc.toString());
                ...
                String memNotice = verifySlot.checkMemNotice(date, time, 0, 0, "", day_name, "teetime", false, slotParms.activity_id, con);
                ...
                if (!memNotice.equals("") && req.getParameter("skip_member_notice") == null) {      // if message to display
                    ...
                    if (club.equals("mediterra") && (sess_activity_id == 1 || sess_activity_id == 2 || sess_activity_id == 3)) {
                    ...
                    if (slot_id == 0) {
                        ...
                        if (slotParms.slots < 2) {
                            ...
                            if (in_use == 9 || ((in_use == 1 || in_use == 2 || slotParms.layout_mode == 4) && consec_csv.size() > 1 
                                    && (!club.equals("lakewoodranch") || slotParms.time >= 1300 || group_id != 1)
                                    && ((!club.equals("dorsetfc") && !club.equals("islandcc")) || slotParms.time >= 1230)
                                    //&& (!club.equals("mosscreek") || slotParms.time >= 1200 || sess_activity_id == 3)
                                    && (!club.equals("theranchcc") || (dateShort > 531 && dateShort < 901) 
                                       || (slotParms.day.equalsIgnoreCase("Saturday") && slotParms.time >= 1230)
                                       || ((!slotParms.day.equalsIgnoreCase("Sunday") && !slotParms.day.equalsIgnoreCase("Saturday")) && (slotParms.time < 1800 || slotParms.time >= 2100))))) {
                                
                                slotParms.group_id = group_id;
                                promptOtherTime(out, slotParms, slotPageParms, run_mode, con);
                ...
                if (slotParms.search_other_times == 1 && (req.getParameter("promptOtherTime") != null || ((in_use == 1 || in_use == 2) 
                        ...
                        || ((club.equals("dorsetfc") || club.equals("islandcc")) && slotParms.time < 1230)
                        || (club.equals("mosscreek") && slotParms.time < 1200 && sess_activity_id != 3)
                        || (club.equals("theranchcc") && (dateShort >= 901 || dateShort <= 531) 
                           && ((slotParms.day.equalsIgnoreCase("Saturday") && slotParms.time < 1230)
                           || (!slotParms.day.equalsIgnoreCase("Sunday") && !slotParms.day.equalsIgnoreCase("Saturday") && slotParms.time >= 1800 && slotParms.time < 2100))))))) {
                    ...
                    if (in_use == 1) {
                        ...
                        try {
                            ...
                            pstmt = con.prepareStatement(
                                    "SELECT activity_name, a.interval, DATE_FORMAT(date_time, '%H%i') as time, DATE_FORMAT(date_time, '%l:%i %p') AS display_time "
                            ...
                            if (rs.next()) {
                                ...
                                slotPageParms.page_start_instructions.add("[options.notify.altConsecutiveTimeSuccessPromptInstructions]<br><br>"
                                        ...
                                        + "Starting time: <span style=\"font-weight:bold;\">" + rs.getString("display_time") + "</span><br>"
                                ...
                                slotPageParms.callback_map.put("time", rs.getInt("time"));
                ...
                userA = slotParms.getUserArray(parmAct.max_players);
        ...
        slotPageParms.force_singles = slotParms.force_singles;
        ...
        if (((club.equals("denvercc") && slotPageParms.activity_id >= 12 && slotPageParms.activity_id <= 15) || club.equals("mg-cc")) && !new_reservation) {
        ...
        } else if (club.equals("bwrc") && !new_reservation && (slotPageParms.activity_id == 2 || slotPageParms.activity_id == 3)) {
            
            slotPageParms.allow_cancel = false;
        } else if (club.equals("lakewoodranch") && !new_reservation && slotPageParms.activity_id != 0) {
            try {

                pstmt = con.prepareStatement("select orig_by from activity_sheets where sheet_id = ?");
                pstmt.clearParameters();
                pstmt.setInt(1, slotParms.sheet_ids.get(0));
            ...
            if (!orig_by.equalsIgnoreCase(user)) {
                slotPageParms.allow_cancel = false;
            }


            if (slotParms.user1.equalsIgnoreCase(user) || orig_by.equalsIgnoreCase(user)) {     // if player is this member or originat

                blockP1 = false;               // allow them to change/erase this one
            }
            if (slotParms.user2.equalsIgnoreCase(user) || orig_by.equalsIgnoreCase(user)) {     // if player is this member or originat

                blockP2 = false;               // allow them to change/erase this one

            }
            if (slotParms.user3.equalsIgnoreCase(user) || orig_by.equalsIgnoreCase(user)) {     // if player is this member or originat

                blockP3 = false;               // allow them to change/erase this one

            }
            if (slotParms.user4.equalsIgnoreCase(user) || orig_by.equalsIgnoreCase(user)) {     // if player is this member or originator

                blockP4 = false;               // allow them to change/erase this one

            }

            slotPageParms.lock_player_a[0] = blockP1;
            slotPageParms.lock_player_a[1] = blockP2;
            slotPageParms.lock_player_a[2] = blockP3;
            slotPageParms.lock_player_a[3] = blockP4;

        }
        
        if (!new_reservation) {
            try {
                pstmt = con.prepareStatement("SELECT count(*) AS player_count FROM activity_sheets_players WHERE activity_sheet_id = ? AND username = ?");
                pstmt.clearParameters();
                pstmt.setInt(1, slot_id);
                ...
                if (rs.next() && rs.getInt("player_count") == 0) {
                    slotPageParms.allow_cancel = false;
                }

            } catch (Exception e) {
                Utilities.logError("Member_activity_slot.doPost - " + club + " - Failed looking up if user in reservation - Error=" + e.toString());
            } finally {
        ...
        slotPageParms.slot_submit_map.put("force_singles", "force_singles");
...
//  String index = req.getParameter("index");          //  day index value (needed by _sheet on return)
        ...
        slotParms.force_singles = reqUtil.getParameterInteger(req, "force_singles", 0);
...
//  slotParms.ind = ind;                      // index value
        ...
        if (req.getParameter("remove") != null) {
            ...
            if (parmAct.unacompGuest == 0) {      // if unaccompanied guests not supported
            ...
            // blackhawk- require members to include at least 2 players not including x's on all courts but the ball machine
            if ((club.equals("pinebrookcc") || (club.equals("islandcc") && sess_activity_id == 1) || (club.equals("blackhawk") && activity_id != 10 && sess_activity_id == 1)) && (slotParms.members + slotParms.guests) < 2) {
            ...
            if (club.equals("bwrc") && (slotParms.members + slotParms.guests) < 2) {
                errorMsg = "<BR><H3>Not Enough Players Found</H3><BR>"
                        + "<BR><BR>Sorry, all reservations must include at least 2 players."
                        + "<BR><BR>Please add additional players to this reservation or return to the time sheets.";
            }
            ...
            if (club.equalsIgnoreCase("fortcollins")) {    // reservations to have at least 2 player names(excluding x's) 
                
                int count = 0;
                
                if (!slotParms.player1.equals("") && !slotParms.player1.equalsIgnoreCase("X")) count++;
                if (!slotParms.player2.equals("") && !slotParms.player2.equalsIgnoreCase("X")) count++;
                if (!slotParms.player3.equals("") && !slotParms.player3.equalsIgnoreCase("X")) count++;
                if (!slotParms.player4.equals("") && !slotParms.player4.equalsIgnoreCase("X")) count++;
                if (!slotParms.player5.equals("") && !slotParms.player5.equalsIgnoreCase("X")) count++;
            ...
            if (club.equals("theranchcc") && sess_activity_id == 1) {    // reservations to have at least 2 player names(including x's) 
                
                int count = 0;
                
                if (!slotParms.player1.equals("")) count++;
                if (!slotParms.player2.equals("")) count++;
                if (!slotParms.player3.equals("")) count++;
                if (!slotParms.player4.equals("")) count++;
                if (!slotParms.player5.equals("")) count++;
            ...
            if (club.equals("pinebrookcc")) {
                ...
                if (error == true) {
                    ...
                    out.println("<BR>Sorry, <b>" + parm1.player + "</b> may only play twice per month on the indoor courts.<br><br>");
        ...
        if (parmAct.disallow_joins == 1) {
            ...
            for (int i=0; i < slotParms.sheet_ids.size(); i++) {
                ...
                if (!verifyActSlot.checkMemberHasAccess(slotParms.sheet_ids.get(i), user, con)) {
                    ...
                    out.println("<BR><BR>Sorry, but one of the selected time slots is already occupied by another reservation.");
                    out.println("<BR><BR>Please select another time or location for this reservation.");
                    out.println("<BR><BR>");
...
//bad out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Member_jump?index=" + index + "&course=" + slotParms.course + "&jump=" + slotParms.jump + "\">");
    ...
    private void promptOtherTime(PrintWriter out, parmSlot slotParms, parmSlotPage slotPageParms, int run_mode, Connection con) {
        ...
        long dateShort = slotParms.date - ((slotParms.date / 10000) * 10000);       // get mmdd (i.e.  20060512 - 20060000 = 512)
            ...
            if (interval > 0 && consec_csv.size() > 0) {
                ...
                if (slotParms.sheet_ids.size() == 0) {
                    ...
                    if (slotParms.layout_mode != 4) {
                        slotPageParms.page_start_notifications.add("[options.notify.consecutiveTimePromptNotAvailable]");
                    }
                    slotPageParms.page_start_instructions.add("[options.notify.consecutiveTimePromptIntructions]");
                    slotPageParms.page_start_button_go_back = true;
                    for (int i = 0; i < consec_csv.size(); i++) {

                        if (((slotParms.club.equals("lakewoodranch") && slotParms.time < 1300 && slotParms.group_id == 1)
                                || ((slotParms.club.equals("dorsetfc") || slotParms.club.equals("islandcc")) && slotParms.time < 1230))
                                && consec_csv.get(i) < 3) {
                            
                            continue;
                            
                        } else if (slotParms.club.equals("theranchcc")) {

                            if (dateShort >= 901 || dateShort <= 531) {

                                if ((consec_csv.get(i) < 3 && 
                                           ((slotParms.day.equalsIgnoreCase("Saturday") && slotParms.time < 1230)
                                        || (!slotParms.day.equalsIgnoreCase("Sunday") && !slotParms.day.equalsIgnoreCase("Saturday") && slotParms.time >= 1800 && slotParms.time < 2100)))
                                 || (consec_csv.get(i) > 2 && !slotParms.day.equalsIgnoreCase("Sunday") && !slotParms.day.equalsIgnoreCase("Saturday") && slotParms.time == 1700)
                                 || (consec_csv.get(i) > 1 && !slotParms.day.equalsIgnoreCase("Sunday") && !slotParms.day.equalsIgnoreCase("Saturday") && slotParms.time == 1730)) {
                                    
                                    continue;
                                }
                            }
                        } else if (slotParms.club.equals("mosscreek") && slotParms.time < 1200 && consec_csv.get(i) < 4 && slotParms.activity_id != 3) {
                            continue;
                        }
                ...
                if (slotParms.sheet_ids.size() < slotParms.slots && (slotParms.search_other_times == 1/* || slotParms.club.startsWith("demo")*/)) {
                    ...
                    for (int i = 0; i < consec_csv.size(); i++) {
                        ...
                        if (consec_csv.get(i) <= slotParms.sheet_ids.size() 
                                || (((slotParms.club.equals("lakewoodranch") && slotParms.time < 1300 && slotParms.group_id == 1) 
                                ||   ((slotParms.club.equals("dorsetfc") || slotParms.club.equals("islandcc")) && slotParms.time < 1230)) 
                                && consec_csv.get(i) < 3)) {
                        ...
                        } else if (slotParms.club.equals("mosscreek") && slotParms.time < 1200 && consec_csv.get(i) < 4) {
                            continue;
                        }

 ...
 *      4/24/13  Member list when viewing an existing distribution list will now be sorted alphabetically by last name, then first name.
...
public class Edit_distributionlist extends HttpServlet {
  ...
  **/
  ...
  private FormModel buildForm(HttpServletRequest req, HttpServletResponse resp, HttpSession session, Connection con, PrintWriter out)
  {
    ...
    String statement = "SELECT * FROM " + table_name + " WHERE owner = ? AND name = ?";
    
    if (isProshopUser) statement = "SELECT * FROM " + table_name + " WHERE owner = ? AND name = ? AND enabled = 1";
    ...
    try {
      ...
      if (rs.next()) {
        ...
        if (isProshopUser) dlist_id = rs.getInt("id");       // get dist list id
        ...
        if (isProshopUser == false) {     // if member
           ...
           for (int i=0; i<max_list_size;i++) {
             ...
             user_name = rs.getString(column_name);
        ...
        } else {     // proshop user - get list of members from distributuin_list_entries table
           
            PreparedStatement pstmt2 = con.prepareStatement (
                    "SELECT dle.username FROM distribution_lists_entries dle LEFT OUTER JOIN member2b m ON dle.username = m.username WHERE distribution_list_id = ? ORDER BY m.name_last, m.name_first");

            pstmt2.clearParameters();            
            pstmt2.setInt(1, dlist_id);      

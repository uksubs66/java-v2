 ...
 *   last updated: 
 *
 *       6/17/14  Remove the word 'message' from the default message '.... has sent you the following message:' as it was typically getting
 *                split in most emails for some unknown reason (mess age).
 *       5/22/14  Kansas City CC(kansascitycc)- added custome trailer and removed the header      
 ...
 *       3/13/13  Miamaai Valley GC (miamivalleygolfclub) - Changed the verbiage to read "Your Course Superintendent...has sent you the following message" for emails sent from the proshopsuper proshop user (case 2237).
...
public class Send_email extends HttpServlet {
   ...
   private ArrayList<String> excelRecipients = new ArrayList<String>();
  ...
  **/
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (req.getParameter("excelRecipients") != null) {
        excelRecipients(session, req, resp, con, out);
        return;
    }
    ...
    if (!ProcessConstants.isProshopUser(user) && !ProcessConstants.isAdminUser(user) && isDining == false)
    {
      ...
      if (!MemberHelper.hasEmailAddress(user, con, out))
      {
        ...
        // make sure club has the app (*** might need to change this to check for nextAction instead)
        if (req.getParameter("push") != null && !Utilities.allowMobileApp(club, con)) {
            out.println("<CENTER><BR><BR>The Push Notification service works in conjunction with our ClubCentral mobile app.<BR>" +
                    "<br>If you are interested in providing this service to your members, please contact sales at <a href=\"mailto:sales@foretees.com\">sales@foretees.com</a>.");
            out.print("</CENTER><BR><BR></BODY>");
    ...
    if (ServletFileUpload.isMultipartContent(req)) {
    ...
    } else if (req.getParameter("push") != null && reqUtil.getParameterString(req, ActionHelper.NEXT_ACTION, "").equals("sendPush") && req.getParameter("initialRequest") == null) {
      
        sendPushNotification(session, req, resp, con, out);
        ...
        // TEMP TO BLOCK PUSHES
        /*
        if (req.getParameter("push") != null && !club.startsWith("demo")) {
            
                    out.println(SystemUtils.HeadTitle("Push Inactive"));
                    out.println("<BODY>");
                    SystemUtils.getProshopSubMenu(req, out, 0);
                    out.println("<CENTER><BR><BR>");
                    out.println("<h3>Push Notification Feature is Currently Not Available</h3>");
                    out.println("<p>Sorry, we had to temporarily remove this feature. &nbsp;Please try again later today.</p>");
        ...
        */
        ...
        //out.println("<!-- doGet() calling addToList req.getParameter('push')=" + req.getParameter("push") + " -->");
          
        //out.println("<!-- doGet() calling addToList initialRequest=" + req.getParameter("initialRequest") + " -->");
          if (req.getParameter("initialRequest") != null) {
    ...
    throws IOException
  {
    ...
    boolean pushNotification = req.getParameter("push") != null; // || (reqUtil.getParameterString(req, ActionHelper.NEXT_ACTION, "").equals("sendPush") && reqUtil.getParameterString(req, "initialRequest", "").equals("yes")); 
    boolean disp_attach_notice = false;                    // Display Attachment Notice option (proshop user only)
    
    
    if (ProcessConstants.isProshopUser(user) && !pushNotification) {            // if Proshop user
    ...
    throws IOException
  {
    ...
    if (selected_names != null && selected_names.size()>0)
    {
        //Add selected names to global arraylist excelRecipients
        for (Object names : selected_names) {
            if (!excelRecipients.contains((String) names)) {
                excelRecipients.add((String) names);
            }
...
*/
  ...
  throws IOException
  {
      ...
      boolean pushNotification = req.getParameter("push") != null;        // if request is for a Push Notification rather than a Send Email
      ...
      try{
        ...
        if (course.equals( "-ALL-" ) || course == null || course.equals("")) {
            ...
            pstmt2p = con.prepareStatement (
              ...
              "FROM " +tableName+ " WHERE date = ? AND player1 != ''");
            ...
            pstmt2p = con.prepareStatement (
              "SELECT username1, username2, username3, username4, username5 " +
              "FROM " +tableName+ " WHERE date = ? AND courseName = ? AND player1 != ''");
        ...
        while (rs.next()) {
                ...
                if (!username[i].equals( "" )) {
                    ...
                    if (pushNotification == false) {
                        ...
                        if (MemberHelper.hasEmailAddress(username[i], con, out)) {
                            ...
                            selectedItems.add(username[i]);        // add member
                        }
                    
                    } else {
                      
                        long member_id = memberUtil.getMemberIdFromUsername(username[i], con);
                        
                        if (memberUtil.hasAppConfigured(member_id, club_id, con)) {        // if member has the app
            ...
            }   // end of FOR loop - check each member in tee time
            ...
            if (organization_id > 0) {
               ...
               if (con_d != null) {
                  ...
                  for (int i=0; i<selectedItems.size();i++) {
                    ...
                    if (tok.countTokens() > 1) {         // event_id, event_name (there could be colons in the event name too!)
                    ...
                    try {
                      ...
                      stmt = con_d.prepareStatement (""
                            + "SELECT person_id "
                            + "FROM reservations "
                            + "WHERE organization_id = ? AND event_id = ? AND person_id IS NOT null AND "
                                + "state <> 'cancelled'"
                                + ((club.equals("esterocc")) ? " AND state <> 'waitlisted';" : ""));  
                          ...
                          if (!username.equals( "" )) {
                            ...
                            if (pushNotification == false) {
                                ...
                                if (MemberHelper.hasEmailAddress(username, con, out)) {
                                    ...
                                    selectedItems.add(username);        // add member
                                }

                            } else {

                                long member_id = memberUtil.getMemberIdFromUsername(username, con);
  ...
  private void sendPushNotification(HttpSession session, HttpServletRequest req, HttpServletResponse resp, Connection con, PrintWriter out)
    throws IOException
  {
      
   String fb_message = "";
   String club = (String)session.getAttribute("club");
   String msg = reqUtil.getParameterString(req, "messageField", "");
   int club_id = Utilities.getClubId(club, con);
   
   FormModel form = retrieveFormFromSession(session, req, resp, con, out);
   ...
   if (msg.isEmpty()) {
     ...
     feedback.setPositive(false);
     fb_message = "You must specify a message for your push notification.  Please correct and try again.";
     feedback.addMessage(ScriptHelper.escapeSpecialCharacters(fb_message));

     showPage(feedback, session, req, resp, con, out, false);
     return;
   }
   ...
   ArrayList<Long> recipients = new ArrayList<Long>(); // recipient member ids
   
   StringBuilder recipients_csv = new StringBuilder();

   int i = 0;
   long member_id = 0;
   for (i = 0; i < names.size(); i++) {

       member_id = memberUtil.getMemberIdFromUsername(names.getRow(i).getId(), con);
       if (memberUtil.hasAppConfigured(member_id, club_id, con)) {
           
           recipients.add(member_id);
           if (recipients_csv.length() > 0) recipients_csv.append(",");
           recipients_csv.append(member_id);
       }
   } // end loop of names arraylist
   
   
/* SAMPLE FORM TO SEND A PUSH NOTIFICATION
<form name="ttform" action="http://clubcentral.foretees.com/ForeTeesServices/process1.php/SendPush" method="POST">
  Msg: <input name="Msg" type="text" value="ForeTees Test"><br>
  Type: <input name="Type" type="text" value="4"> (0 For Global ForeTees Notification , 1 For Club Notification, 4 For User Notification )<br>
  ClubId: <input name="ClubId" type="text" value="150"><br>
  MemberIds: <input name="MemberIds" type="text" value="593"><br>
  <input name="ent_submit" type="submit" value="Push">
</form>
*/
   
        List<String[]> params = new ArrayList<String[]>();
        params.add(httpConnect.keyVal("Msg", msg));
        params.add(httpConnect.keyVal("Type", "4")); // (0 For Global ForeTees Notification , 1 For Club Notification, 4 For User Notification )
        params.add(httpConnect.keyVal("ClubId", club_id));
        params.add(httpConnect.keyVal("MemberIds", recipients_csv.toString()));

        String result = httpConnect.post("http://clubcentral.foretees.com/ForeTeesServices/process1.php/SendPush", params);
        
   
   //
   //  Build the HTML page
   //
   out.println(SystemUtils.HeadTitle("Push Notification Confirmation Page"));
   ...
   out.println("<p><b>Thank you!</b></p><p>Your push notification is being queued for sending.&nbsp; ");
   
   
   out.println("Out of the " + names.size() + " members selected for your message, ");
   out.println("we were able send it to " + recipients.size() + " of them that have the ClubCentral app installed.&nbsp; ");
   out.println("They should receive the message soon.</p>");
   out.println("<!-- Recipients: " + recipients_csv.toString() + " -->");
   ...
   out.println("<table border=0><tr><td>");
    ...
    out.println("<input type=\"hidden\" name=\"push\" value=\"1\"><input type=\"submit\" value=\"Return\" style=\"text-decoration:underline;width:90px\">");
    out.println("</form>");
  ...
  {
   /*
   if( req.getParameter("push") != null) {
       
       sendPushNotification(session,req,res, con, out);
       return;
   }
   */
   ...
   String user = (String)session.getAttribute("user");     // get username ('proshop' or member's username)
   String club = (String)session.getAttribute("club");
   String dining_user_id = "";
   
if (club.equals("esterocc") || club.equals("demobrad")) Utilities.logDebug("BSK", "TEST Estero Dining User ID Check");   
   
   if ((String)session.getAttribute("dining_user") != null) {
       
       dining_user_id = (String)session.getAttribute("dining_user");
       
if (club.equals("esterocc") || club.equals("demobrad")) Utilities.logDebug("BSK", "TEST dining_user ID found: " + dining_user_id);       
   }
   ...
   if (isDining == false && sess_activity_id > 0) {
    ...
    servletFileUpload.setSizeMax(1024 * 1024); // 1024KB
    ...
    try {
        ...
        out.println("<h3 align=center>Attachment Size Limit Exceeded</h3><p>Your attachment was too large to send.  The maximum size for an attachment is 1024KB (one megabyte).</p>" +
   ...
   if (club.equalsIgnoreCase("winchestercc") && sess_activity_id == 1) {
       senderTitle = "Your Lifestyle Director";
   }
   ...
   if (user.equals("proshop4") && club.equals("foresthillsgc")) {
       ...
       if (!club.equals("rollinghillsgc") && !club.equals("ccdcranch") && !club.equals("pattersonclub")&& !club.equals("kansascitycc") && !club.equals("boulder")
               && (!club.equals("pinebrookcc") || sess_activity_id != 1) && (!club.equals("mirabel") || !user.equalsIgnoreCase("proshopcom")) 
               && (!club.equals("stcloudcc") || !user.equalsIgnoreCase("proshopnewslet"))) {      // Do not print header for Rolling Hills GC!

           if (club.equals("portage") && isDining == false) {

               message = "<p>Your Team of Golf Professionals has sent you the following message:</p>";

           } else if (club.equals("fortcollins")) {

               message = "<p>Your " + senderTitle + " has sent you the following message:</p>";

           } else if (club.equals("islandview")) {

                 message = "<p>Your golf club at Island View has sent you the following message:</p>";

           } else if (club.equals("bhamcc") && isDining == false) {

                 message = "<p>Your Professional Staff at " + clubName + " has sent you the following message:</p>";
                 
           } else if (club.equals("lakeshorecc") && user.equalsIgnoreCase("proshopgrounds")) {

                 message = "<p>Your Course Director at " + clubName + " has sent you the following message:</p>";
               
           } else if (club.equals("scarsdalegolfclub") && user.equalsIgnoreCase("proshopgrounds")) {

                 message = "<p>Your course superintendent at " + clubName + " has sent you the following message:</p>";
               
           } else if (club.equals("stcloudcc") && (user.equalsIgnoreCase("proshopmarketin") || user.equalsIgnoreCase("proshoppres") || user.equalsIgnoreCase("proshopgm"))) {
               
                 if (user.equalsIgnoreCase("proshopmarketin")) {
                     message = "<p>Rollie Carlson at the St. Cloud Country Club has sent you the following message:</p>";
                 } else if (user.equalsIgnoreCase("proshoppres")) {
                     message = "<p>The St. Cloud Country Club has sent you the following message:</p>";
                 } else if (user.equalsIgnoreCase("proshopgm")) {
                     message = "<p>The General Manager at St. Cloud Country Club has sent you the following message:</p>";
                 }
                 ...
                 message = "<p>    Your " + senderTitle + " at " + clubName + " has sent you the following message:</p>";
                 
           } else if (club.equals("coloradosga")) {

                 message = "<p>" + clubName + " has sent you the following message:</p>";
                 
           } else if (club.equals("deserthillsgolfclub")) {

                 message = "<p>From Desert Hills Golf Club:</p>";
                 
           } else if(club.equals("oldwarson")){
                 
                 message = "<p>The staff at " + clubName + " has sent you the following message:</p>";
               
           } else if (club.equals("oregonsrgolf")) {

                 message = "<p>Your Executive Director at " + clubName + " has sent you the following message:</p>";
                 
           } else if (club.equals("esterocc") && (dining_user_id.equals("155288") || user.equalsIgnoreCase("proshopcomm"))) {

               if (dining_user_id.equals("155288")) {
                   message = "<p>Your House and Entertainment Committee from Estero Country Club has sent you this message:</p>";
               } else if (user.equalsIgnoreCase("proshopcomm")) {
                   message = "<p>Your Communications Director at Estero Country Club has sent you the following:</p>";
               }
                 
           } else if (club.equals("bigsprings")) {
               
               if (user.equalsIgnoreCase("proshop2")) {
                   message = "<p>Your Golf Professional at Harmony Landing has sent you the following message:</p>";
               } else {
                   message = "<p>Your Golf Professional at Big Spring Country Club has sent you the following message:</p>";
               }
               
           } else if (club.equals("waynesborough")) {
             
               message = "<p>Greetings from the Golf Staff at Waynesborough Country Club.</p>";
               
           } else if (club.equals("rhillscc") && sess_activity_id == 0 && user.equalsIgnoreCase("proshopgrounds")) {
             
               message = "";
               
           } else if (club.equals("winchestercc") || club.equalsIgnoreCase("mirasolcc") || club.equalsIgnoreCase("hillwoodcc")) {
           
               message = "";
           
           } else if (club.equals("fortwayne")) {
           
               message = "<p>Your golf professional at FWCC Golf Shop has sent you the following message:</p>\n\n";
           
           }  else if (club.equalsIgnoreCase("albany") && user.equalsIgnoreCase("proshopsupe")) {
           
               message = "<p>Your Golf Course Superintendent has sent you the following message:</p>";
           
           } else if (club.equals("bloomfieldhillscc")) {
           
               message = "<p> Your Professional staff at BHCC has sent you the following message:</p>";
           } else if (club.equals("ballenisles")) {
           
               message = "<p> Your BallenIsles Golf Professional Staff has sent you the following:</p>";
           } else if (club.equals("carolinacc")) {
           
               message = "<p> The Professional Staff at Carolina Country Club has sent you the following message:</p>";
           } else {

              if (user.equals("proshop2") && club.equals("hillcrestwi")) {

                 message = "<p>Your Superintendant/General Manager at " + clubName + " has sent you the following message:</p>";

              } else if (club.equals("interlachen") && sess_activity_id == 1 && isDining == false) {

                 message = "<p>Your " + senderTitle + ", Steve Paulsen, at " + clubName + " has sent you the following message:</p>";
              } else if (club.equals("interlachen") && sess_activity_id == 11 && isDining == false) {

                 message = "<p>Your " + senderTitle + ", Kate Daly, at " + clubName + " has sent you the following message:</p>";
              } else {

                 message = "<p>Your " + senderTitle + " at " + clubName + " has sent you the following:</p>";
              }
           }
       }
   ...
   //
   if (!club.equals("fortcollins") && !club.equalsIgnoreCase("bigsprings")) {
       if (!clubName.equals( "" )) subject += " (" +clubName+ ")";
   }
   
   if (club.equalsIgnoreCase("bigsprings")) {
       if (user.equalsIgnoreCase("proshop1")) {
           subject += " (Big Spring Country Club)";
       } else if (user.equalsIgnoreCase("proshop2")) {
           subject += " (Harmony Landing)";
       }
   }
   ...
   if (club.equals("avonoakscc") && !user.startsWith("proshop") && email_orig_caller.equals("report_handicap")) {
       replyTo = "golfshop@avonoakscc.com";
   }
      ...
      if (!club.equals("rollinghillsgc")) {     // Do not print a footer for Rolling Hills GC
          ...
          if (user.startsWith("proshopfb") || isDining == true) {
              if (club.equalsIgnoreCase("mediterra")) {
                  efrom = "YourClubStaff@foretees.com";
              } else {
              ...
              if (club.equals("mirabel")) {
                  ...
                  efrom = "OldWarsonCC@foretees.com";
                  
              } else if (club.equals("boulder")){
                  
                  efrom = "BoulderCC@foretees.com";
              ...
              } else if (club.equals("stcloudcc") && (user.equalsIgnoreCase("proshopmarketin") || user.equalsIgnoreCase("proshoppres") 
                      || user.equalsIgnoreCase("proshopgm"))) {
                  ...
                  if (user.equalsIgnoreCase("proshopmarketin")) {
                      ...
                      efrom = "SCCCNewsletter@foretees.com";
                  }
                  ...
                  efrom = "WaynesboroughCountryClub@foretees.com";
                  
              } else if (club.equals("lagrangecc")) {
                  
                  efrom = "LaGrangeGolfShop@foretees.com";
                  
              } else if (club.equals("esterocc") && (dining_user_id.equals("155288") || user.equalsIgnoreCase("proshopcomm"))) {
                  
                  if (dining_user_id.equals("155288")) {
                      efrom = "YourH&ECommittee@foretees.com";
                  } else if (user.equalsIgnoreCase("proshopcomm")) {
                      efrom = "esterocc@foretees.com";
                  }
                  
              } else if (club.equals("winchestercc")) {
                  
                  efrom = "WinchesterCountryClub@foretees.com";
                  
              } else if (club.equals("hillwoodcc")) {
                  
                  efrom = "HillwoodGolfShop@foretees.com";
                  
              } else if (club.equals("carolinacc")) {
                  
                  efrom = "CarolinaCC@foretees.com";
                  
              } else if (club.equals("rhillscc") && sess_activity_id == 0 && user.equalsIgnoreCase("proshopgrounds")) {
                  
                  efrom = "yourgroundsdepartment@foretees.com";
                  
              } else if (club.equals("bloomfieldhillscc")) {
                  
                  efrom = "bhccgolfshop@foretees.com";
          ...
          if (club.equals( "hillcrestwi" ) || club.equals( "pinehills" ) || club.equals("roccdallas")) {        // if Hillcrest WI or Pine Hills
             ...
             trailer = "\n\n\n****************************************************************************************" +
                       "\nThis message has been sent by The Kansas City Country Club Golf via the ForeTees system.  ";
                       skipReplyto = true;
             ...
             trailer = "\n\n\n****************************************************************************************" +
                   "\nThis message has been sent by your Club Professional via the ForeTees system.  " +
                   "Should you prefer not to receive these email messages, " +
                   "please contact your Pool Director, Kate Daly, at ";
             ...
             skipReplyto = true;
                  ...
                  || user.equalsIgnoreCase("proshopgm") || user.equalsIgnoreCase("proshopnewslet"))) {
              ...
              } else if (user.equalsIgnoreCase("proshopnewslet")) {
                  trailer = "\n\n\n****************************************************************************************" +
                       "\nThis message has been sent via the ForeTees system.  " +
                       "Should you prefer not to receive these email messages, " +
                       "please contact the at ";
              ...
              trailer = "\n\n\n****************************************************************************************" +
                      ...
                      + "please contact your Club staff at michele.massambani@mirabel.com.</span>";
              ...
              trailer = "\n\n\n****************************************************************************************"
                      + "\nThis message has been sent by the CSGA via the ForeTees system.  "
                      + "Should you prefer not to receive these email messages, "
                      + "please contact the CSGA at ";
              ...
              if (dining_user_id.equals("155288")) {
                  ...
                  trailer = "\n\n\n****************************************************************************************"
                          + "\nThis message has been sent by your H&E Committee via the ForeTees system.  "
                          + "Should you prefer not to receive these email messages, "
                          + "please contact your H&E Committee at ";
                  
              } else if (user.equalsIgnoreCase("proshopcomm")) {
                  
                  trailer = "\n\n\n****************************************************************************************"
                          + "\nThis message has been sent by your Communications Director via the ForeTees system.  "
                          + "Should you prefer not to receive these email messages, "
              ...
              trailer = "\n\n\n****************************************************************************************"
                      + "\nThis message has been sent by your Superintendent via the ForeTees system.";
              ...
              if (club.equalsIgnoreCase("mediterra")){
                  ...
                  trailer = "\n\n\n****************************************************************************************" +
                   "\nThis message has been sent by your Club Staff via the ForeTees system. " +
          ...
          } else if (club.equalsIgnoreCase("hillwoodcc") && sess_activity_id == 0) {
              trailer = "\n\n\n*****************************************************************************************"
                      + "\nThis message has been sent by Mike Lathrop, Head Golf Professional, Hillwood Country Club, via the ForeTees system. "
                      + "Should you prefer not to receive these email messages, please contact your Club Professional at mike.lathrop@hillwoodcc.org.";
...
*/
   ...
   emailParm.club = club;
   emailParm.user = user;
   emailParm.dining_user_id = dining_user_id;
   emailParm.message = trailer;
   
   if (email_orig_caller.equals("report_handicap") && club.equals("superstitionmountain")) {
       emailParm.handicapChair = true;
   }
...
*/
  ...
  **/
  ...
  private void drawBeginningOfPageBeforeForm(FeedBack feedback, PrintWriter out, HttpServletRequest request, HttpSession session, Connection con)
  {
    ...
    boolean pushNotification = request.getParameter("push") != null; 
    //boolean pushNotification = request.getParameter("push") != null || (reqUtil.getParameterString(request, ActionHelper.NEXT_ACTION, "").equals("sendPush") && reqUtil.getParameterString(request, "initialRequest", "").equals("yes")); 
    ...
    if (pushNotification) {
        ...
        out.println(" $(\"#messageField\").keyup(function(event) {");
        out.println("  var currentString = $(\"#messageField\").val();");
        //out.println("  var count_err = false;");
        out.println("  $(\"#counter\").html('Characters: ' + currentString.length);");
        out.println("  if (currentString.length > 200 )  {");
        out.println("   $(\"#counter\").addClass('text_counter_error');");
        //out.println("   count_err = true;");
        out.println("  } else {");
        //out.println("   if (count_err == true) {");
        //out.println("    count_err = false;");
        out.println("    $(\"#counter\").removeClass('text_counter_error');");
        //out.println("   }");
        out.println("  }");
        out.println(" });");
        out.println("});");
    ...
    if (feedback != null) {
      ...
      out.print("<body onLoad=\"document.pgFrm." + Email.MESSAGE + ".focus()\">");
   ...
   // LayoutHelper.drawBeginPageContentWrapper(onLoad, null, out);
    ...
    if (pushNotification) {
        ...
        /*
        out.println("<style>");
        out.println("table.frm { width: auto}");
        out.println("");
        out.println("");
        out.println("");
        out.println("");
        out.println("</style>");
        */
        LayoutHelper.drawBeginMainBodyContentWrapper("Send Push Notification", pageActions, out);
    } else if (club.equals( "merrillhills" )) {
  ...
  **/
  ...
  private FormModel buildForm(HttpServletRequest req, HttpServletResponse resp, HttpSession session, Connection con, PrintWriter out) 
  {
    ...
    //form.addHiddenInput("push", (pushNotification) ? "1" : "0");
    if (pushNotification) form.addHiddenInput("push", "1");
    
    //create the action model for this form and add it to the form model
    ActionModel formActions = new ActionModel();

    String sendUrl = "javascript:" + ((pushNotification) ? "sendPush" : "sendEmail") + "('" + versionId + "servlet/Send_email', '" + ((pushNotification) ? "sendPush" : ActionHelper.SEND) + "')";
    Action sendAction = new Action(ActionHelper.SEND, Labels.SEND, "Send the " + ((pushNotification) ? "notification" : "email") + ".", sendUrl);
    ...
    String ExcelUrl = "javascript:openNewWindow('" + versionId + "servlet/Send_email?excelRecipients=yes" + "', '" + "excelRecipients" + "', '" + ActionHelper.EXCEL + "')";
    Action searchExcelAction = new Action(ActionHelper.EXCEL , "Export Recipients", "Export recipients list to Excel", ExcelUrl);
    formActions.add(searchExcelAction);
    ...
    if (pushNotification) {
        ...
        // add push notificaiton specific help steps here
        helpSteps.add("Keep messages brief.  Most devices only display the first 100 or so characters without going in and reading the full message.&nbsp; "
                + "Total message length should be kept to under 200 characters for maximum reach.");
        helpSteps.add("Do not over use this service or your members will block these communications.");
        helpSteps.add("<b><i>The value of these contextual notifications needs to outweigh the cost of your members' time and privacy.</i></b>");
        
    } else {

        // add email specific help steps here
        if (!ProcessConstants.isProshopUser(user))
        {
        ...
        if (ProcessConstants.isProshopUser(user))
        {
          ...
          helpSteps.add(Email.NOTE_PDF_UPLOAD);
        }
        else
        {
    ...
    if (!pushNotification) {
        ...
        // smaller box for push notifications - maybe just a textbox
        message.setMaxLength("50");
        message.setSize("3");
        //message.setOnChange("updateCounter()");
    }
    ...
    String searchMembersUrl = "javascript:openNewWindow('" + versionId + Member.SEARCH_WINDOW_URL + ((pushNotification) ? "?push=1" : "") + "', '" + Member.SEARCH_WINDOW_NAME + "', '" + Member.SEARCH_WINDOW_PARAMS + "')";
    ...
    //
    if (user.startsWith( "proshop" ) && !pushNotification) {
    ...
    //******************************************************************************************************
    //
    private void excelRecipients(HttpSession session, HttpServletRequest req, HttpServletResponse res, Connection con, PrintWriter out)
            throws IOException {
        ...
        try {              // if user requested Excel Spreadsheet Format
            ...
            res.setHeader("Content-Disposition", "attachment;filename=\"excelRecipients.xls\"");
        } catch (Exception ignore) {
        }
        ...
        for (String username : excelRecipients) {

            try {
                pstmt = con.prepareStatement("SELECT name_last, name_first from member2b WHERE username = ?");
                ...
                if (rs.next()) {
                    sortedMap.put(rs.getString("name_last")+rs.getString("name_first"), username);
                }

            } catch (Exception exc) {
                Utilities.logError("Send_email.excelRecipients -  - Error looking up name of member - ERR: " + exc.toString());
            } finally {
                Connect.close(rs, pstmt);
            }
        }
        
        for (Map.Entry<String,String> username : sortedMap.entrySet()) {
            ...
            try {
                ...
                Utilities.logError("Send_email.excelRecipients -  - Error looking up name & emails of member - ERR: " + exc.toString());
            } finally {
                Connect.close(rs, pstmt);
            }
        }
  ...
  **/
  ...
  private void drawEndOfPageAfterForm(PrintWriter out, boolean disp_attach_notice, boolean isDining)
  {
    ...
    if (disp_attach_notice == true) {
       ...
       out.println("<BR><BR>It is highly recommended that you upload your documents in PDF format and include links to them in the message content whenever possible.<br>"
               + "This will allow you to use files larger than 1 MB. &nbsp;Refer to the link to the right under Instructions for assistance with this.");
       out.println("<BR><BR>If you find it necesary to send attachments, we recommended that you send PDF type documents whenever possible.");
       ...
       out.println("<BR><BR>Also note that the file cannot exceed <b>1MB</b> in size.");

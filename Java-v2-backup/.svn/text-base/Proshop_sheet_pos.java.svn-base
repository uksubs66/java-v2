...
public class Proshop_sheet_pos {
 ...
 public static void sendCharges(String caller, String club, String course, String user, long date, 
   ...
   boolean extProcess = false;        // external processing (i.e. tee time charges processed in Proshop_sheet_checkin)
   boolean extCharges = false;        // external charges 'sent' indicator
  ...
  // if ((club.equals( "engineerscc" ) || club.equals( "metropoliscc" ) || club.equals( "sawgrass" )) && parmp.posType.equals( "Jonas" )) {   
   ...
   if (parmp.posType.equals( "Abacus21" ) || parmp.posType.startsWith( "Jonas" ) || parmp.posType.equals( "TAI Club Management" ) ||
      ...
      if (parmp.posType.equals( "Jonas Generic" )) {
         ...
         if (parmp.posType.equals( "ClubSoft" ) || parmp.posType.equals( "Jonas Generic" ) || parmp.posType.equals( "Jonas Encore" )) {
   ...
   try {
      ...
      while (rs.next()) {
         ...
         ttid = rs.getInt("teecurr_id");       // get tid for CSG or PCS Group Batch
         ...
         count++;             // count number of tee times found
         ...
         if (parmp.posType.equals( "PCS Group Batch" )) {
            
            //
            //  PCS Group Batch processing - do this in Proshop_sheet_checkin.checkPCScharges for each tee times
            //
            Proshop_sheet_checkin checkin = new Proshop_sheet_checkin();    // create an instance of Proshop_sheet_checkin so we can call it (static vs non-static)

            extCharges = checkin.checkPCScharges(ttid, 0, parm, parmp, club, caller, con);   // check for and send any charges
            
            if (extCharges) {
               
               masterDone = 1;     // charges sent
            }  
            
            extProcess = true;     // indicate charges to be processed in Proshop_sheet_checkin
         }

         
         if (extProcess == false) {       // if charges to be processed here
            ...
            if (!player1.equalsIgnoreCase( "x" ) && !player1.equals( "" ) && show1 == 1) {
               ...
               if (guest == 0) {        // if member
                  ...
                  if (!user1.equals( "" ) && pos1 == 0) {      // skip if no user name found or already processed
                     ...
                     if (parmp.posType.equals( "Jonas Encore" )) {

                        done = buildJonasEncore(parmp, out, con, resnum, ttidnum, club);
                     }
               ...
               if (guest == 0) {        // if member
                  ...
                  if (!user2.equals( "" ) && pos2 == 0) {      // skip if no user name found
                     ...
                     if (parmp.posType.equals( "Abacus21" ) || parmp.posType.startsWith( "Jonas" )) {
                        ...
                        } else if (parmp.posType.equals( "Jonas Encore" )) {
            ...
            //
            //  Charges were processed externally (Proshop_sheet_checkin) - set the POS flags if any were sent
            //
            if (pos1 == 0) pos1 = 1;      // set 'processed' if not previously processed or Pay Now set (= 2)
            if (pos2 == 0) pos2 = 1;
            if (pos3 == 0) pos3 = 1;
            if (pos4 == 0) pos4 = 1;
            if (pos5 == 0) pos5 = 1;
            
            if (extCharges) {        // if any charges were sent
               
               if (parmp.posSent1 == true) pos1 = 3;    // set the pos flags if charges were sent for each player
               if (parmp.posSent2 == true) pos2 = 3;
               if (parmp.posSent3 == true) pos3 = 3;
               if (parmp.posSent4 == true) pos4 = 3;
               if (parmp.posSent5 == true) pos5 = 3;
            }
   ...
   if (caller.equals("timer")) {      // if initiated by the custom timer (SystemUtils) 
      ...
      if (masterDone == 1) {   // if we built any records
         ...
         } else if (parmp.posType.equals( "IBS" ) || parmp.posType.equals( "PCS Group Batch" ) || (!caller.equals("timer") && club.equals("ironwood"))) {   // IBS or PCS/Ironwood

            //
            //  inform user the charges have been sent.  Display any errors received.
            //
            resp.setContentType("text/html");                   // normal html response
            resp.setHeader("Content-Disposition", "inline");    // undo attachment change from above
 ...
 // ********************************************************************
 //  Process the Jonas Encore POS charges for an individual member
 //
 //  Build a CSV file containing the following:
 //
 //   Columns:
 //       1 Member Number or POSID                           
 //       2 Reference Number (Reservation #)                     
 //       3 Sales Area (Always GOLF)                  
 //       4 Item Code                         
 // ********************************************************************

 private static int buildJonasEncore(parmPOS parmp, PrintWriter out, Connection con, long resnum, long ttidnum, String club) {
   ...
   try {
      ...
      String errorMsg1 = "Error1 in Proshop_sheet_pos.buildJonasEncore for club: " + club;
   ...
   //
   if (!posid.equals("")) {     // if posid provided
   ...
   if (!mnum.equals( "" ) && mnum != null) {
      ...
      try {
         ...
         if (!item.equals( "" ) && !item.equals( "0" ) && item != null) {   // if pos charge found for Mode of Trans selected
            ...
            tempSB.append(",");                           
            tempSB.append(resnum);                            // Res #
            tempSB.append(",");
            tempSB.append(salesArea);                         // Sales Area (Golf)
            tempSB.append(",");                     
         ...
         String errorMsg2 = "Error2 in Proshop_sheet_pos.buildJonasEncore for club: " +club;
      ...
      if (parmp.player.equals( "" )) {       // if member
         ...
         try {
            ...
            String errorMsg3 = "Error3 in Proshop_sheet_pos.buildJonasEncore for club: " +club;
         ...
         try {
            ...
            String errorMsg4 = "Error4 in Proshop_sheet_pos.buildJonasEncore for club: " +club;
 ...
 private static int buildJonas(parmPOS parmp, PrintWriter out, Connection con, long resnum, long ttidnum, String club) {
   ...
   //  some clubs use the posid field instead of the mnum
   //
   if ((club.equals( "lakewood" ) || club.equals( "pinnaclepeak" ) || club.equals("engineerscc") || club.equals("mesaverdecc") 
           || club.equals("austincountryclub") || club.equals("trooncc") || club.equals("kiawahislandclub")) & !posid.equals( "" )) {
...
//                            parmp.date == Hdate1 || parmp.date == Hdate2 || parmp.date == Hdate3) && parmp.time < 1200) {  // changed 6/05/07
 ...
 private static int buildCSG(parmPOS parmp, PrintWriter out, Connection con, String club, int ttid, String caller) {
   ...
   //
   if (club.equals("atlantacountryclub") || club.equals("tavistockcc") || club.equals("awbreyglen")) {
         ...
         if (parmp.player.equals( "" )) {     // if player is a member
            ...
            if (parmp.club.equals( "olyclub" )) {        // if Olympic Club
               ...
               if (mship.equals( "No Package" ) || mship.equals( "Non Golf" )) {
                ...
                //
                if (mship.equals("Intermediate") || mship.equals("Non-Resident House")) {
                    ...
                    } else if (parmp.course.equals("St Martins") && mship.equals("Non-Resident House")) {

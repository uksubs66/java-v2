 ...
 *   last updated:
 *
 *        1/13/14   CC of North Carolina (ccofnc) - Removed custom so members no longer get defaulted to a default_activity_id of 1.
 *       10/25/13   CC of North Carolina (ccofnc) - Added custom to default all new members to a default_activity_id of 1.
 *        4/12/13   Changed "Save" button to say "Save and New" to be more clear of what it's actually doing.
  ...
  **/
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    HttpSession session = SystemUtils.verifyAdmin(req, out);       // check for intruder
     ...
     if (club.equalsIgnoreCase("lehigh") && mshiptype.equalsIgnoreCase("Non Golf")) {
     ...
     try {
        ...
        stmt = con.prepareStatement (
                 "SELECT inact FROM member2b WHERE name_last = ? AND name_first = ? AND name_mi = ? AND deleted = 0");
        ...
        if (dupName == false) {
           ...
           if (rsync == 0) {            // if club does not use Roster Sync
              ...
              stmt = con.prepareStatement (
                "INSERT INTO member2b (username, password, name_last, name_first, name_mi, " +
                 ...
                 "name_pre, name_suf, webid, hdcp_club_num_id, hdcp_assoc_num_id, inact, gender, tflag, default_activity_id) " +
                 "VALUES (?,?,?,?,?,?,?,?,0,?,?,?,'',1,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
              ...
              stmt = con.prepareStatement (
                "INSERT INTO member2b (username, password, name_last, name_first, name_mi, " +
                 "m_ship, m_type, email, count, c_hancap, g_hancap, wc, message, emailOpt, " +
                 "memNum, ghin, locker, bag, birth, posid, msub_type, email2, phone1, phone2, " +
                 "name_pre, name_suf, webid, hdcp_club_num_id, hdcp_assoc_num_id, inact, gender, tflag, default_activity_id, billable) " +
                 "VALUES (?,?,?,?,?,?,?,?,0,?,?,?,'',1,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
           }
           ...
           stmt.setInt(30, default_activity_id);
  ...
  **/
  ...
  private FormModel buildNewForm(HttpServletRequest req,HttpServletResponse resp, HttpSession session, Connection con, PrintWriter out) throws Exception
  {
     ...
     try {
      ...
      Action applyAction = new Action("updateAndReturn", "Save and New", "Save the member and return to this page to add another.", applyUrl);

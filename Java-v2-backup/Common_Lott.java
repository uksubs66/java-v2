 ...
 *
 *       5/02/14   Set useBothFB option for Cold Spring CC (coldspringcc) - like Estero.
...
public class Common_Lott {
 ...
 public static void buildArrays(Connection con, parmLott parm, parmLottC parmc) {
   ...
   List<String> exclude_courses = new ArrayList<String>();
   
   if (parm.club.equals("rioverdecc")) {
       exclude_courses.add("Road Runner Executive");
   }
   ...
   try {
      if (parm.sfb.equals( "Both" )) {
         ...
         if (!parm.course.equals("") && !parm.course.equals("-ALL-")) {
             ...
             if (parm.club.equals("decordovabend") && (parm.lottName.equals("Tuesday Lottery") || parm.lottName.equals("Saturday Lottery"))) {
                 
                //  Custom - Order by fb first so members can start on the desired tee
     ...
     while (rs.next()) {
        ...
        if (!player1.equals("") || !player2.equals("") || !player3.equals("") || !player4.equals("") || 
            !player5.equals("") || !blocker.equals("") || !event.equals("") || exclude_courses.contains(parmc.courseA[i])) {  // if busy or blocked or an event
...
//   short save_fb = 0;
   ...
   if ((parm.club.equals("esterocc") || parm.club.equals("plantationgcc") || parm.club.equals("coldspringcc")) && parm.sfb.equals( "Both" )) useBothFB = true;   // do this for Estero if lottery defined for both tees
      ...
      if (rs.next()) {
         ...
         while (i < tee_count) {          // loop through arrays for a matching tee time (what was requested)
            ...
            if (ttime == time2 && tfb == parm.rfb && course.equals(req_course)) {   // if matching time/fb/course found       
               ...
               // Utilities.logDebug("BP", "Lottery assign1Time. Tee time = " +ttime+ ", i = " +i+ ", beforei = " +parm.beforei+ ".");                                   
               ...
               while (grps > 0) {
                  ...
                  if (astat == 0 || bstat == 0) {   // if before or after still open to check
                     ...
                     if (parmc.id2A[i3] == 0 && parmc.busyA[i3] == false && parmc.timeA[i3] > 0 && restricted == false) {  // if spot open (check associated entry in Id Array)
                        ...
                        if (grps > 0) {          // if still more groups - make sure there is at least one more matching time
                     ...
                     if (error > 0) {                         // if error - try another time slot
                        ...
                        if (astat == 0 || bstat == 0) {      // if before or after still open to check
                           ...
                           // Utilities.logDebug("BP", "Lottery assign1Time. Tee time = " +parmc.timeA[i3]+ ", course = " +parmc.courseA[i3]+ ", beforei = " +parm.beforei+ ", afteri = " +parm.afteri+ ", grps = " +grps+ ".");                                   
 ...
 public static int checkBefore(parmLott parm, parmLottC parmc, boolean useBothFB) {
    ...
    // Utilities.logDebug("BP", "Lottery checkBefore. Tee time = " +parmc.timeA[i3]+ ", course = " +parmc.courseA[i3]+ ", beforei = " +parm.beforei+ ", afteri = " +parm.afteri+ ".");                                   
               
    if (!parm.club.equals("gallerygolfTEMP")) {         // if NOT Gallery Golf - use existing logic (release this once they ok it!!!)
        ...
        //    Gallery Club
        // 
        //     4-21-2015 BP - Copied the above code and changed it so we only go back one tee time each time this method is called.
        //                    The above code goes backwards 'grps' number of tee times.  This was originally designed to go back one tee time per
        //                    call in order get the closest time(s) to that requested.
        // 
        if (!parmc.course.equals("-ALL-")) {          // if NOT multi course lottery

            if (i3 >= 1) {                     // if room to back up
                ...
                if (parm.sfb.equals( "Both" )) {   // if both tees used for this lottery
                    ...
                    //  Use both tees - switch tees if both are defined (double tees for this day)
                    //
                    if (parmc.fbA[i3] != parm.fb_before) {     // if different f/b found than last checked

                        parm.fb_before = parmc.fbA[i3];     // might be a new fb
                    }
                }
                ...
                if (ttime >= parm.ftime) {    // if tee time is acceptable
                    ...
                    if ((parmc.fbA[i3] == parm.fb_before || useBothFB == true)) {   // if matching f/b found
            ...
            if (i3 >= 1) {                      // if room to back up
                ...
                //
                loop2:
                while (i3 > 0) {                         // while room to back up     

                    i3--;                                // back up one time

                    if (parm.sfb.equals( "Both" )) {      // if both tees used for this lottery

                            parm.fb_before = parmc.fbA[i3];        // then use whatever the next one is
                    }
                    ...
                    new_course = parmc.courseA[i3];      // get the course from the next entry backwards (use these for each group)

                    if (parm.checkothers == 1 || new_course.equals(parm.courseReq)) {   // if member wants to check other courses or this is the same course

                        if (parmc.timeA[i3] < parm.ftime) {    // if tee time is NOT acceptable
                        ...
                        loop3:
                        while (index >= 0) {                  // while room to back up

                            ttime = parmc.timeA[index];      // get tee time from array

                            if (ttime >= parm.ftime) {    // if tee time is acceptable

                                if (parmc.fbA[index] == new_fb && parmc.courseA[index].equals(new_course)) {   // if matching f/b & course found
    ...
    }              // end of IF Gallery Golf

    // Utilities.logDebug("BP", "Lottery checkBefore. Tee time = " +parmc.timeA[i3]+ ", course = " +parmc.courseA[i3]+ ", beforei = " +parm.beforei+ ".");                                   
               
    parm.i3 = i3;           // set new values for return
    parm.grps = grps;       // restore/set # of groups
 ...
 public static int checkAfter(parmLott parm, parmLottC parmc, boolean useBothFB) {
    ...
    if (!parm.club.equals("gallerygolfTEMP")) {           // if NOT Gallery Golf - use existing logic
        ...
        //   Gallery Golf Custom
        // 
        //     4-21-2015 BP - Copied the above code and changed it so we only go ahead one tee time each time this method is called.
        //                    The above code goes ahead 'grps' number of tee times.  This was originally designed to go ahead one tee time per
        //                    call in order get the closest time(s) to that requested.
        // 
        if (!parmc.course.equals("-ALL-")) {          // if NOT multi course lottery

            if (i3 < tee_count) {                   // if room to go ahead
                ...
                if (parm.sfb.equals( "Both" )) {   // if both tees used for this lottery
                    ...
                    if (parmc.fbA[i3] != parm.fb_after) {     // if different f/b found than last checked
                        ...
                        parm.fb_after = parmc.fbA[i3];     // might be a new fb
                    }
                }

                loop3:
                while (i3 < tee_count) {          // while room to search ahead

                    ttime = parmc.timeA[i3];       // get tee time from array

                    if (ttime > 0 && ttime <= parm.ltime) {    // if tee time is acceptable

                        if (parmc.fbA[i3] == parm.fb_after || useBothFB == true) {     // if matching f/b found

                            found = true;                        // match found
                            parm.fb_after = parmc.fbA[i3];       // fb may have changed
                            parm.rfb = parm.fb_after;            // set new f/b if it changed
                            break loop3;                         // done
                        }
            ...
            if (i3 < tee_count) {                 // if room to go ahead
                ...
                if (parm.sfb.equals( "Both" )) {   // if both tees used for this lottery
                    ...
                    //  Use both tees - switch tees if both are defined (double tees for this day)
                    //
                    parm.fb_after = parmc.fbA[i3];        // then switch if its different
                }
                ...
                while (i3 < tee_count) {           // while room to search ahead
                    ...
                    if (ttime > 0 && ttime <= parm.ltime) {    // if tee time is acceptable
                        ...
                        if ((parmc.fbA[i3] == new_fb || useBothFB == true) && 
                            (parmc.courseA[i3].equals(new_course) || parm.checkothers > 0)) {              // if matching f/b & course found OR new course 
    ...
    // Utilities.logDebug("BP", "Lottery checkAfter. Tee time = " +parmc.timeA[i3]+ ", course = " +parmc.courseA[i3]+ ", beforei = " +parm.beforei+ ", afteri = " +parm.afteri+ ".");                                   
      ...
      while (rs.next()) {
         ...
         if (atime1 != 0) {          // only process if its assigned
            ...
            if (ok == true) {             // if ok to proceed (no tee times are in use)
               ...
               // Bind the newly inserted requests
               ...
               if((group_ids.size() > 1 || teecurr_group_id > 0) && member_id > 0 && group_ids.size() > 0){
                   ...
                   if(teecurr_group_id > 1){
                       ...
                       Utilities.logError("Common_Lott.moveReqs: Err=Unable to create teecurr binding.  Can't get teecurr_group_id for member_id:"+member_id+", lreq_id:"+id);
                   }
               }

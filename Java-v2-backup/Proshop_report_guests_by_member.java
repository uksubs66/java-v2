 ...
 *   Last Updated:
 *
 *             10/15/09 Add custom option to display counts for each individual guest type (only run as a custom report on dev server).
 *                      Refer to displayWarning method to turn this on.
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   int activity_id = (Integer)session.getAttribute("activity_id");
    ...
    if (opt1.equals("showtypes")) {
       ...
       showAll(period, lottery, excel, activity_id, req, resp, out, con);              // display warning message
       return;                                   // exit and wait for continue
    }
...
*/
 ...
 //**********************************************************************
 //
 private void showAll(String period, int lottery, String excel, int activity_id, HttpServletRequest req, HttpServletResponse resp, PrintWriter out, Connection con) {
   ...
   */
   ...
   try {
      ...
      getClub.getParms(con, parm, activity_id, true);   // get the guest types (all types - act & inact)
   }
   catch (Exception ignore) {
   }

   int [] gstRnds = new int [parm.MAX_Guests];       // use array for the guest types
   int [] show = new int [5];     
   ...
   for (i = 0; i < parm.MAX_Guests; i++) {
    ...
    try {
      ...
      //
      //   Custom for Merion - to run only when they ask !!!!!!!
      //
      //whereClause += " AND courseName='East'";

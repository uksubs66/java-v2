...
public void updAfterCreate(Connection con, int invNum, int blOverride, String item, String description)
        throws Exception {
   ...
   try {
     ...
     if (blOverride > 0) {
         ...
         pstmt1 = con.prepareStatement("UPDATE invoice SET invoiceNumber = ?, billableCnt = ?, amountDue = ?, " +
                                       "item = ?, description = ? " + 
                                       "WHERE id = ?"); 
          
         pstmt1.clearParameters();
         pstmt1.setInt(1, invoiceNumber);
         pstmt1.setInt(2, billableCnt);
         pstmt1.setInt(3, amountDue);
         pstmt1.setString(4, item);
         ...
         pstmt1 = con.prepareStatement("UPDATE invoice SET invoiceNumber = ?, item = ?, description = ? WHERE id = ?");     
...
//
public void updAfterCreate2(Connection con, int invNum, int setupFee, String item, String description)
        throws Exception {
   ...
   try {
         ...
         pstmt1 = con.prepareStatement("UPDATE invoice SET invoiceNumber = ?, amountDue = ?, " +
                                       "item = ?, description = ? " + 
                                       "WHERE id = ?"); 
          
         pstmt1.clearParameters();
         pstmt1.setInt(1, invoiceNumber);
         pstmt1.setInt(2, amountDue);
         pstmt1.setString(3, item);
...
public void retrieve (Connection con, int invId)  throws Exception {
   ...
   //
   //  Get invoice data for the ID specified.
   // 
   PreparedStatement pstmt = null;
   ResultSet rs = null;
...
public void insertInvoiceToDb(Connection con)  throws Exception {
   ...
   try {
     ...
     pstmt1 = con.prepareStatement("INSERT INTO invoice (invoiceNumber, invStatus, "
           ...
           + "payDueDate, paidDate, amountDue, checkNumber, activity_id, item, description)"
...
public void updateFullInvoiceToDb(Connection con) throws Exception {
   ...
   try {
     ...
     pstmt1 = con.prepareStatement("UPDATE invoice "
           ...
           + ", checkNumber = " + checkNumber + ", activity_id = " + activity_id + ", "
...
// Create date from Calendar to store in database.
//
private int getIntDate(Calendar calDate) {
...
public int setCheckNumber(int val) {
 ...
 public void displayClubInvoice(Connection con, PrintWriter out, String rev) throws Exception {
   ...
   int duedate = getIntDate(dueDate);
   int generatedate = getIntDate(generateDate);
   
   String duedateS = getDueDate(2);
   
   if (duedate < generatedate) duedateS = getGenerateDate(2);    // make sure that the due date is not earlier than the generate date
   ...
   try {
       ...
       if (rs.next()) {
        ...
        out.println("      </tr><tr>");
        out.println("        <td>" + duedateS + "</td>");          // mm/dd/yyyy
        out.println("      </tr>");
        out.println("    </tbody>");
        out.println("    </table>");
        ...
        out.println("    <tbody>");
        out.println("      <tr>");
        out.println("        <td class=\"item\">" + item + "</td>");

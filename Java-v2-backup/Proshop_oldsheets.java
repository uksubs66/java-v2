 ...
 *   last updated:
 *
 *        5/06/14   Allow for Jonas Generic POS interface.
 ...
 *       12/06/13   Fixed issue where slotParms.user wasn't getting populated with the current user in verify method.
 *       11/01/13   Add 'Send Email' option to the Control Panel.
 *        7/23/13   Dove Canyon Club (dovecanyonclub) - Added custom to display tflags corresponding to each player's mtype after their name (case 2287).
 ...
 *        5/08/13   Birmingham CC (bhamcc) - Added to custom to display mNums by player names.
 *        2/26/13   Editing past tee times will now utilize the Proshop Short Name list, and will now create history entries for modifications.
 *        2/21/13   Add 2 more event save areas and show shotgun info in tee time if appropriate.
 *        2/15/13   Added processing to make sure pos1-5 values are properly maintained within verify() and when shifting players due to old players being removed.
 *       11/29/12   Add hack to adjust the css display property of the likno modal elements
 ...
 *        3/21/12   Removed the new Print Options menu from the page, since it was not ready for release and accidentally got committed (it's commented out for now).
 *        3/13/12   Pinery CC (pinery) - Added custom to display custom_disp1-5 values after the player name on old tee sheets (case 2134).
 *        2/08/12   Added a control panel option to clear all POS flags for the current day. This is only available to those who have POS access.
 *       12/09/11   Club at Pradera (pradera) - Added custom to display custom_disp1-5 values after the player name on old tee sheets (case 2020).
 *       10/07/11   Updated slot page to use alphaTable.nameList and alphaTable.guestList in order for the quick search box to be available.
 *        9/23/11   Add PCS Group POS Interface for Ironwood.
 *        8/24/11   Olympic Club - add column to Print a Receipt.
 *        7/03/11   Add POS processing.
 *        6/22/11   Display the Pay Now "paid" image after a player's name if the Pay Now feature was used for them.
 *        6/16/11   tflag values will now be displayed after a player's name, if populated.
 *        5/03/11   Add the tee sheet summary and quick view tables.
 *       12/27/10   The Lakes CC - add mNums to the player names.
 *        9/01/10   Updated doInsert method to all it to be called from Proshop_sheet and insert times to teecurr2 too.
 *        8/23/10   Hide 'Print Sheet' & 'Excel' links in control panel if iPad user
...
public class Proshop_oldsheets extends HttpServlet {
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   //
   //  Check for Send POS selected from CP
   //
   if (req.getParameter("sendpos") != null) {

      Proshop_sheet_pos.promptPOS2("old", req, resp, out, session, con);      // go prompt user
      return;
   }
   ...
   if (req.getParameter("prtReceipt") != null) {
      ...
      Proshop_sheet_pos.printReceipt(req, resp, out, session, con, true);      // go print the receipt (true = from oldsheets)
      return;
   }

   if (req.getParameter("clearpos") != null) {

      Proshop_sheet_pos.clearPOSFlags(req, resp, out, session, true, con);
   }
   ...
   if (req.getParameter("print") != null) {
      ...
      if (!report_type.equals( "pos" )) {     // if NOT pos report type
         ...
         try {
            ...
            if (exp.getMessage().equals("null")) return;
            SystemUtils.logError("Proshop_oldsheets - exception from Proshop_sheet_reports. Error = " +exp.getMessage());
            return;
         }
      }
   }
   ...
   int fontSz = 2;                // font size for player names and MOTs on tee sheet - change this to change all
   ...
   String rest1 = "";       // for legend - max 4 events, 4 rest's
   String rcolor1 = "";
   String rest2 = "";
   String rcolor2 = "";
   String rest3 = "";
   ...
   if (req.getParameter("returnCourse") != null) {
      ...
      returnCourse = req.getParameter("returnCourse");         // if cournse returned
   }
   ...
   if (req.getParameter("gotodate") != null) {     // if user typed in the date in the "GoTo" box
      ...
      if (month < 1 || month > 12 || day < 1 || day > 31 || year < 2002 || year > thisyear) {
         ...
         out.println("<BR>Please try again later.");
         out.println("<BR><BR>If problem persists, contact customer support.");
         out.println("<BR><BR><a href=\"Proshop_oldsheets\">Return</a>");
         out.println("</CENTER></BODY></HTML>");
         out.close();
         return;
      }
   ...
   cal_date.set((int)yy, (int)mm-1, (int)dd);      // set Calendar to date of this tee sheet

   day_num = cal_date.get(Calendar.DAY_OF_WEEK);      // day of week (01 - 07)
   ...
   try {
      ...
      out.println("<BODY><CENTER>");
      out.println("<BR><BR><H2>Database Error E1</H2>");
   ...
   if (((req.getParameter("noShow") != null) || (req.getParameter("checkAll") != null)) && checkinAccess) {
      ...
      if (noShow == true) {
         ...
         try {
            ...
            pstmt1 = con.prepareStatement (updShow);  // use selected stmt
            ...
            out.println("<BR><BR><H3>Database Error E2</H3>");
   ...
   //
   if (club.equals( "loscoyotes" ) || club.equals( "minnetonkacc" ) || club.equals( "lakes" ) || club.equals("bhamcc")) {
      
      disp_mnum = true;
   }
   
   //  Override customs and default course settings if proshop user is set to display mnum
   if (dispMnumOption) disp_mnum = true;
   
   if (!returnCourse.equals("")) {
       
       course = returnCourse;         // change course to the course option that was last displayed before user selected a process
   }
   
   if (course.equals("-ALL-")) {
       
       tempCourse = "";      // use this if we can't pass -ALL-
   ...
   try {
      ...
      //
      getClub.getPOS(con, parmp, tempCourse);
      ...
      //
      //  Get the 5-some option of course we're viewing
      //
      temp = "SELECT fives FROM clubparm2 WHERE first_hr != 0 AND courseName = ?";
      
      if (tempCourse.equals("")) {
          
          temp = "SELECT fives FROM clubparm2 WHERE first_hr != 0";
      }
      
      PreparedStatement pstmtc = con.prepareStatement (temp);
      ...
      pstmt = con.prepareStatement (
         ...
         "FROM teepast2 " +
         "WHERE date = ? " +temp+ "AND (event != '' OR restriction != '') " +
      ...
      while (rs.next()) {
         ...
         if (!event.equals("")) {    // if event
             
             for (i=0; i<count_event; i++) {
                 
                 if (event.equals(eventA[i])) {      // if already saved this event
                     
                     i = count_event;                // force done
                     
                 } else {
                     
                     if (eventA[i].equals("")) break;   // continue if empty save area found
                 }
             }
         }

         if (!event.equals("") && i < count_event && eventA[i].equals( "" )) {
      ...
      //
      //  Gather event info if any events for this day
      //
      for (i=0; i<count_event; i++) {
         ...
         if (!eventA[i].equals("")) {
            ...
            pstmt = con.prepareStatement (
                ...
                "FROM events2b " +
                "WHERE name = ? AND activity_id = 0");

            pstmt.clearParameters();       
            pstmt.setString(1, eventA[i]);        
            rs = pstmt.executeQuery();      

            if (rs.next()) {

                event_start_hrA[i] = rs.getInt(1);
                event_start_minA[i] = rs.getInt(2);
      ...
      out.println(SystemUtils.HeadTitle("DB Error"));
      out.println("<BR><BR><H2>Database Error E3A</H2>");
      ...
      //
      //  Gather info for tee sheet summary and Quick View
      //
     String courseClause = (course.equals("-ALL-")) ? "" : "courseName = ? AND";
     String sqlQuery1 =  "";
     String sqlQuery1a =  "";
     String sqlQuery2 = "";
     String sqlQuery3 = "";
     ...
     if (fives == 0) {
         ...
         //sqlQuery1 =  "SELECT COUNT(date) AS total FROM teepast2 WHERE " + courseClause + " date = ?;"; 
         // query to count the total # of tee time player positions occupied
         sqlQuery2 = "SELECT SUM(subtotal) AS total FROM (" +
             "SELECT COUNT(date) AS subtotal FROM teepast2 WHERE " + courseClause + " date = ? AND player1 <> '' " +
             "UNION ALL " +
             "SELECT COUNT(date) AS subtotal FROM teepast2 WHERE " + courseClause + " date = ? AND player2 <> '' " +
             "UNION ALL " +
             "SELECT COUNT(date) AS subtotal FROM teepast2 WHERE " + courseClause + " date = ? AND player3 <> '' " +
             "UNION ALL " +
             "SELECT COUNT(date) AS subtotal FROM teepast2 WHERE " + courseClause + " date = ? AND player4 <> '' " +
             ") AS gtotal;";
         // query to count the # of occupied tee times (full or partially full)
         sqlQuery3 = "SELECT COUNT(date) AS total FROM teepast2 WHERE " + courseClause + " date = ? AND (player1 <> '' OR player2 <> '' OR player3 <> '' OR player4 <> '');"; 
     } else {
         // query  to count the total # of tee times available
         //sqlQuery1 =  "SELECT COUNT(date) AS total FROM teepast2 WHERE " + courseClause + " date = ?;";
         //sqlQuery1a =  "SELECT COUNT(date) AS total FROM teepast2 WHERE " + courseClause + " date = ?;";  
         // query to count the total # of tee time player positions occupied
        sqlQuery2 = "SELECT SUM(subtotal) AS total FROM (" +
             "SELECT COUNT(date) AS subtotal FROM teepast2 WHERE " + courseClause + " date = ? AND player1 <> '' " +
             "UNION ALL " +
             ...
             "SELECT COUNT(date) AS subtotal FROM teepast2 WHERE " + courseClause + " date = ? AND player4 <> '' " +
             "UNION ALL " +
             "SELECT COUNT(date) AS subtotal FROM teepast2 WHERE " + courseClause + " date = ? AND player5 <> '' " +
             ") AS gtotal;";
         // query to count the # of occupied tee times (full or partially full)
         sqlQuery3 = "SELECT COUNT(date) AS total FROM teepast2 WHERE " + courseClause + " date = ? AND (player1 <> '' OR player2 <> '' OR player3 <> '' OR player4 <> '' OR player5 <> '');"; 
   ...
   try {   
     ...
     if (course.equals("-ALL-")) {
         ...
         pstmt1.setString(7, course);
         pstmt1.setLong(8, (year * 10000) + (month * 100) + day);
         if (fives == 1) {
             pstmt1.setString(9, course);
     ...
     // done running tee time summary queries
      ...
      out.println(SystemUtils.HeadTitle("DB Error"));
      out.println("<BR><BR><H2>Database Error E3B</H2>");
      out.println("<BR><BR>Unable to access the Database.");
      out.println("<BR>Please try again later.");
      out.println("<BR><BR>If problem persists, contact customer support.");
      out.println("<BR><BR>" + e1.getMessage());
      ...
      if (!excel.equals("yes")) {
         ...
         out.println(" w = window.open ('Proshop_diary?scroll=1&index=" +num+ "&course=" +course+ "&year=" +year+ "&month=" +month+ "&day=" +day+ "','clientIdPopup','width=640,height=475,scrollbars=1,dependent=0,directories=0,location=0,menubar=0,resizable=0,status=0,toolbar=0');");
         out.println(" w.creator = self;");
         out.println("}");
         out.println("function openHistoryWindow(date, course, time, fb, tid) {");
         out.println(" w = window.open ('Proshop_teetime_history?calDate=' +date+ '&course=' +course+ '&time=' +time+ '&fb=' +fb+ '&tpid=' +tid+ '&history=yes','historyPopup','width=800,height=455,scrollbars=1,dependent=0,directories=0,location=0,menubar=0,resizable=1,status=0,toolbar=0');");
         out.println(" w.creator = self;");
         out.println("}");
         out.println("</script>");
        ...
        out.println("function doNoPost(tee_id, imgId) {");
        ...
        out.println(" } else {");
        out.println("  doc.location.href='Proshop_sheet_checkin?tid='+tee_id+'&imgId='+imgId+'&nopost&oldsheet';");
        out.println(" }");
        out.println("}");
        out.println("</script>");
    ...
    if (!excel.equals("yes")) {
      ...
      //
      out.println("<table align=center border=0 width=\"100%\"><tr valign=top>");   

      out.println("<td align=center width=\"20%\">");         // column for Control Panel
       if (SystemUtils.verifyProAccess(req, "TS_PRINT", con, out)) {
           out.println("<span id='xawmMenuPathImg-foreteesControlPanel' style='padding-left:100px'>");
         ...
         out.println("<form name=\"cpHlp\" method=\"GET\" action=\"/" +rev+ "/servlet/Proshop_oldsheets\">");
         out.println("<input type=\"hidden\" name=\"index\" value=\"" + num + "\">");
         out.println("<input type=\"hidden\" name=\"yyyymmdd\" value=\"" + date + "\">");
         ...
         out.println("<input type=\"hidden\" name=\"oldsheets\">");
         out.println("</form>");
         ...
         if (checkinAccess && !course.equals("-ALL-")) {

             out.println("<tr><td align=\"center\"><font color=\"#FFFFFF\" size=\"2\">");
             out.println("<a href=\"Proshop_oldsheets?checkallin=yes&course=" +course+ "&date=" +date+ "&calDate=" +calDate+ "\" title=\"Check In All Players\" alt=\"Check All In\" onclick=\"return confirm('Are you sure you want to check in all players on this tee-sheet?');\">");
             out.println("Check All In</a>");
             out.println("</td></tr>");
             /*
             out.println("<form action=\"Proshop_oldsheets\" method=\"post\" target=\"bot\">");
         ...
         if (updateAccess && !course.equals("-ALL-")) {
             ...
             out.println("<tr><td align=\"center\"><font color=\"#FFFFFF\" size=\"2\">");
             out.println("<a href=\"Proshop_oldsheets?insert=yes&course=" +course+ "&date=" +date+ "&calDate=" +calDate+ "\" title=\"Insert New Tee Time\" alt=\"Insert New Tee Time\">");
         ...
         if (!parmp.posType.equals( "" ) && !parmp.posType.equals( "None" )) {       // any POS type 
            ...
            if (posAccess && !course.equals("-ALL-")) {
               
                if (parmp.posType.equals( "Abacus21" ) || parmp.posType.startsWith( "Jonas" ) || parmp.posType.equals( "NorthStar" ) || 
                    parmp.posType.equals( "ClubSystems Group" ) || parmp.posType.equals( "TAI Club Management" ) || 
                    parmp.posType.equals( "IBS" ) || parmp.posType.equals( "ClubSoft" ) || parmp.posType.equals( "PCS Group" )) {

                    if (!course.equals( "-ALL-" )) {

                       out.println("<tr><td align=\"center\"><font size=\"2\">");
                       out.println("<a href=\"Proshop_oldsheets?date=" +date+ "&sendpos=yes&course=" +course+ "\" target=\"_blank\" title=\"Generate POS Charges\" alt=\"Send POS Charges\">");
                       ...
                       out.println("<tr><td align=\"center\"><font size=\"2\">");
                       out.println("<a href=\"/" +rev+ "/servlet/Proshop_oldsheets?clearpos&date=" +date+ "&course=" +course+ "\" target=\"bot\" title=\"Clear POS Flags\" alt=\"Clear POS Flags\" " + 
                ...
                if (!parmp.posType.equals( "PCS Group" )) {  // any not PCS (not needed for them)

                   //  add POS Report link
                   out.println("<tr><td align=\"center\"><font color=\"#FFFFFF\" size=\"2\">");
                   out.println("<a href=\"Proshop_report_pos?oldSheet=yes&course=" +course+ "&date=" +date+ "\" title=\"POS Report\" alt=\"POS Report\" target=\"_blank\">");
         ...
         if (reportAccess && !course.equals("-ALL-")) {
            
             out.println("<tr><td align=\"center\"><font color=\"#FFFFFF\" size=\"2\">");
             out.println("<a href=\"Proshop_report_handicap?todo=view2&course=" +course+ "&date=" +date+ "\" title=\"Posted Scores\" alt=\"Posted Scores\">");
         ...
         if (waitListAccess && !course.equals("-ALL-")) {
             ...
             for (int z = 0; z < parmWaitLists.count; z++) {
                ...
                if ( parmWaitLists.signups[z] > 0 ) {
                   ...
                   out.println("<tr><td align=\"center\"><font color=\"#FFFFFF\" size=\"2\">");
                   out.println("<a href=\"Proshop_oldsheets?viewWL=''&course=" +course+ "&date=" +date+ "&waitListId=" +parmWaitLists.id[z]+ "&start_time=" +parmWaitLists.stime[z]+ "&end_time=" +parmWaitLists.etime[z]+ "&name=" +parmWaitLists.name[z]+ "\" title=\"View Wait List\" alt=\"View Wait List\">");
                   out.println("View Wait List" + ((z>1) ? " " + z : "") + "</a>");
                   out.println("</td></tr>");
                    /*
                    out.println("<form method=\"POST\" action=\"Proshop_oldsheets\">");
                    out.println("</tr><td align=\"left\">");
         ...
         if (printAccess && enableAdvAssist && !course.equals("-ALL-")) {
             ...
             out.println("<tr><td align=\"center\"><font color=\"#FFFFFF\" size=\"2\">");
             out.println("<a href=\"Proshop_oldsheets?excel=yes&course=" +course+ "&calDate=" +calDate+ "\" title=\"Excel\" alt=\"Excel\" target=\"_blank\">");
             out.println("Excel</a>");
             out.println("</td></tr>");             
             /*
             ...
             out.println("<form action=\"Proshop_oldsheets\" method=\"post\" target=\"_blank\">");
             out.println("<tr><td align=\"left\">");
         ...
         if (paceofplay != 0 && popAccess == true && !course.equals("-ALL-")) {
             ...
             if (pace_count > 0) {
                ...
                out.println("<form action=\"Proshop_report_pace\" method=\"POST\" name=\"paceform\">");
   ...
   /*
      ...
      */
              ...
              out.println("<tr><td align=\"center\"><font color=\"#FFFFFF\" size=\"2\">");
              out.println("<a href=\"Proshop_dsheet?email=no&course=" +course+ "&index=" +tmp_index+ "\" title=\"Manage Notifications\" alt=\"Manage Notifications\" target=\"_top\">");
      ...
      out.println("</td><td align=right width=\"30%\">");         // column for Course Selection and Calendar
      
      //  form for calendar
      
      out.println("<form action=\"Proshop_oldsheets\" target=\"bot\" method=\"post\" name=\"frmLoadDay\">");
      ...
      out.println("</td><td align=left width=\"30%\">");         // column for Go To Date form
         ...
         out.println("<BR>");
         out.println("<form action=\"Proshop_oldsheets\" target=\"bot\" method=\"post\">");
      ...
      out.println("</td><td align=right width=\"20%\">");         // column for Tee Sheet Summary
      
         
         
         //
         //  Add Tee Sheet Summary and Quick View Table
         //
           //
           ...
           out.println("Tee Times Occupied:</td>");
           ...
           out.println("<td nowrap><font size=\"2\" color=\"#FFFFFF\">" + full_teetimes + "</font></td>");
           out.println("</tr>");

           out.println("<tr>");
           out.println("<td nowrap><font size=\"2\" color=\"#FFFFFF\">");
           out.println("Player Slots Occupied:</td>");
           ...
           out.println("<td nowrap><font size=\"2\" color=\"#FFFFFF\">" + occupied_slots + "</font></td>");
           out.println("</tr>");
           ...
           out.println("<a href=\"javascript:void(0)\" onClick=\"window.open('Proshop_sheet_reports?date=" + date + "&oldsheets=yes&course=" +course+ "', 'newwindow', 'height=600, width=970, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no directories=no, status=no');return false;\">");
         ...
         //  end of Tee Sheet Summary table
    ...
    for (i=0; i<count_event; i++) {     // check for events
        ...
        if (!eventA[i].equals( "" )) {
           ...
           out.println("<button type=\"button\" style=\"background:" + ecolorA[i] + "\">" + eventA[i] + "</button>");
           out.println("&nbsp;&nbsp;&nbsp;&nbsp;");
        }
    }
      ...
      if (!rest1.equals( "" )) {
         ...
         if (!rest2.equals( "" )) {
            ...
            if (!rest3.equals( "" )) {
               ...
               if ((!rest4.equals( "" )) && (eventA[0].equals( "" ))) {   // do 4 rest's if no events
         ...
         if (!eventA[0].equals( "" )) {     // if event but no rest, need br
   ...
   try {
      ...
      //  Get the tee sheet for this date
      //
      courseClause = (course.equals("-ALL-") || course.equals("")) ? "" : " AND courseName = ?";
      ...
      pstmt = con.prepareStatement (
         ...
         "pos1, pos2, pos3, pos4, pos5, tflag1, tflag2, tflag3, tflag4, tflag5, " + 
         "custom_disp1, custom_disp2, custom_disp3, custom_disp4, custom_disp5, " +
         "nopost1, nopost2, nopost3, nopost4, nopost5, event_type, hole, " +
         "mtype1, mtype2, mtype3, mtype4, mtype5, courseName " +
         "FROM teepast2 " +
         "WHERE date = ?" +courseClause+ " " + 
         "ORDER BY time, courseName, fb");

      pstmt.clearParameters();
      pstmt.setLong(1, date);
      if (!course.equals("-ALL-") && !course.equals("")) pstmt.setString(2, course);
      rs = pstmt.executeQuery();
      ...
      while (rs.next()) {
         ...
         pos4 = rs.getInt(44);
         pos5 = rs.getInt(45);
         tflag1 = rs.getString(46);
         tflag2 = rs.getString(47);
         tflag3 = rs.getString(48);
         tflag4 = rs.getString(49);
         tflag5 = rs.getString(50);
         ...
         String timeLabel = hr + ":" + SystemUtils.ensureDoubleDigit(min) + ampm;    // create label for time value
         ...
         if (type == shotgun) {                             // if Shotgun Event
            ...
            for (i=0; i<count_event; i++) {
                ...
                if (event.equals(eventA[i])) {
                    ...
                    timeLabel = SystemUtils.getSimpleTime(event_start_hrA[i], event_start_minA[i]);
                    break eventloop1;
                }
            }
            ...
            if (!timeLabel.equals( "" )) {       // if time gathered above

                StringTokenizer toks = new StringTokenizer(timeLabel, " " );     // delimiters are space (8:30 AM)

                timeLabel = toks.nextToken();                             // get the time only (8:30)
            }

            timeLabel += " Shotgun";            // i.e.  9:30 Shotgun
         }
         ...
         if (course.equals("-ALL-")) {
            ...
            for (tmp_i = 0; tmp_i < courseCount; tmp_i++) {
                if (courseName.equals(courseA.get(tmp_i))) break;      // find index value for this course
            }

            out.println("<td bgcolor=\"" + colors.course_color[tmp_i] + "\" align=\"center\">");  // assign a bg color
            out.println("<font size=\"1\">" +courseName);
            out.println("</font></td>");             
         }
         ...
         if (!player1.equals("")) {
            ...
            showpaid = "";                    // default to not showing Pay Now paid image
            ...
            if (excel.equals("yes")) {
                ...
                if (player1.equalsIgnoreCase("x")) {
                   ...
                   if (club.equals("lomassantafecc")) {
                       ...
                       if (player1.startsWith("Regular") || player1.startsWith("Family")) {
                           out.println("<td bgcolor=\"tomato\" align=\"center\">");
                       } else if (player1.startsWith("Prospective Member") || player1.startsWith("Comp") || player1.startsWith("PGA") || player1.startsWith("Employee")) {
                           out.println("<td bgcolor=\"dodgerblue\" align=\"center\">");
                       } else {
                           out.println("<td bgcolor=\"" + bgcolor + "\" align=\"center\">");
                       }
                   } else {
                   ...
                   // If Club at Pradera and a cart number was found for this player, display it behind their name.
                   if ((club.equals("pradera") || club.equals("pinery") || club.equals("canyonoakscc") || club.equals("beechmontcc") || club.equals("austincountryclub") || club.equalsIgnoreCase("rollinghillsccca") || club.equalsIgnoreCase("bloomfieldhillscc"))  && !custom_disp1.equals("")) {
                       p1 += " " + custom_disp1;
                   }
                   ...
                   if (club.equals("dovecanyonclub")) {
                       ...
                       if (mtype1.equalsIgnoreCase("Adult Male")) {
                           p1 += " AM";
                       } else if (mtype1.equalsIgnoreCase("Adult Female")) {
                           p1 += " AF";
                       } else if (mtype1.equalsIgnoreCase("Dependent")) {
                           p1 += " D";
                       }
                   }
                   ...
                   if (!showpaid.equals("")) {
                       out.println("&nbsp;<img src=\"/" +rev+ "/images/" + showpaid + "\" border=\"1\" title=\"Paid at counter\" alt=\"Paid at counter\">");
                   }
                   out.println("</font></td></form>");
                }
            ...
            if (excel.equals("yes")) {
                ...
                if (player2.equalsIgnoreCase("x")) {
                   ...
                   if (club.equals("lomassantafecc")) {
                       ...
                       if (player2.startsWith("Regular") || player2.startsWith("Family")) {
                           out.println("<td bgcolor=\"tomato\" align=\"center\">");
                       } else if (player2.startsWith("Prospective Member") || player2.startsWith("Comp") || player2.startsWith("PGA") || player2.startsWith("Employee")) {
                           out.println("<td bgcolor=\"dodgerblue\" align=\"center\">");
                       } else {
                           out.println("<td bgcolor=\"" + bgcolor + "\" align=\"center\">");
                       }
                   ...
                   // If Club at Pradera and a cart number was found for this player, display it behind their name.
                   if ((club.equals("pradera") || club.equals("pinery") || club.equals("canyonoakscc") || club.equals("beechmontcc") || club.equals("austincountryclub") || club.equalsIgnoreCase("rollinghillsccca") || club.equalsIgnoreCase("bloomfieldhillscc")) && !custom_disp2.equals("")) {
                       p2 += " " + custom_disp2;
                   }
                   ...
                   if (club.equals("dovecanyonclub")) {
                       ...
                       if (mtype2.equalsIgnoreCase("Adult Male")) {
                           p2 += " AM";
                       } else if (mtype2.equalsIgnoreCase("Adult Female")) {
                           p2 += " AF";
                       } else if (mtype2.equalsIgnoreCase("Dependent")) {
                           p2 += " D";
                       }
                   }
            ...
            if (excel.equals("yes")) {
                ...
                if (player3.equalsIgnoreCase("x")) {
                   ...
                   if (club.equals("lomassantafecc")) {
                       ...
                       if (player3.startsWith("Regular") || player3.startsWith("Family")) {
                           out.println("<td bgcolor=\"tomato\" align=\"center\">");
                       } else if (player3.startsWith("Prospective Member") || player3.startsWith("Comp") || player3.startsWith("PGA") || player3.startsWith("Employee")) {
                           out.println("<td bgcolor=\"dodgerblue\" align=\"center\">");
                       } else {
                           out.println("<td bgcolor=\"" + bgcolor + "\" align=\"center\">");
                       }
                   ...
                   // If Club at Pradera and a cart number was found for this player, display it behind their name.
                   if ((club.equals("pradera") || club.equals("pinery") || club.equals("canyonoakscc") || club.equals("beechmontcc") || club.equals("austincountryclub") || club.equalsIgnoreCase("rollinghillsccca") || club.equalsIgnoreCase("bloomfieldhillscc")) && !custom_disp3.equals("")) {
                       p3 += " " + custom_disp3;
                   }
                   ...
                   if (club.equals("dovecanyonclub")) {
                       ...
                       if (mtype3.equalsIgnoreCase("Adult Male")) {
                           p3 += " AM";
                       } else if (mtype3.equalsIgnoreCase("Adult Female")) {
                           p3 += " AF";
                       } else if (mtype3.equalsIgnoreCase("Dependent")) {
                           p3 += " D";
                       }
                   }
            ...
            if (excel.equals("yes")) {
                ...
                if (player4.equalsIgnoreCase("x")) {
                   ...
                   if (club.equals("lomassantafecc")) {
                       ...
                       if (player4.startsWith("Regular") || player4.startsWith("Family")) {
                           out.println("<td bgcolor=\"tomato\" align=\"center\">");
                       } else if (player4.startsWith("Prospective Member") || player4.startsWith("Comp") || player4.startsWith("PGA") || player4.startsWith("Employee")) {
                           out.println("<td bgcolor=\"dodgerblue\" align=\"center\">");
                       } else {
                           out.println("<td bgcolor=\"" + bgcolor + "\" align=\"center\">");
                       }
                   ...
                   // If Club at Pradera and a cart number was found for this player, display it behind their name.
                   if ((club.equals("pradera") || club.equals("pinery") || club.equals("canyonoakscc") || club.equals("beechmontcc") || club.equals("austincountryclub") || club.equalsIgnoreCase("rollinghillsccca") || club.equalsIgnoreCase("bloomfieldhillscc")) && !custom_disp4.equals("")) {
                       p4 += " " + custom_disp4;
                   }
                   ...
                   if (club.equals("dovecanyonclub")) {
                       ...
                       if (mtype4.equalsIgnoreCase("Adult Male")) {
                           p4 += " AM";
                       } else if (mtype4.equalsIgnoreCase("Adult Female")) {
                           p4 += " AF";
                       } else if (mtype4.equalsIgnoreCase("Dependent")) {
                           p4 += " D";
                       }
                   }
                ...
                if (excel.equals("yes")) {
                   ...
                   if (player5.equalsIgnoreCase("x")) {
                      ...
                      if (club.equals("lomassantafecc")) {
                          ...
                          if (player5.startsWith("Regular") || player5.startsWith("Family")) {
                              out.println("<td bgcolor=\"tomato\" align=\"center\">");
                          } else if (player5.startsWith("Prospective Member") || player5.startsWith("Comp") || player5.startsWith("PGA") || player5.startsWith("Employee")) {
                              out.println("<td bgcolor=\"dodgerblue\" align=\"center\">");
                          } else {
                              out.println("<td bgcolor=\"" + bgcolor + "\" align=\"center\">");
                          }
                      ...
                      // If Club at Pradera and a cart number was found for this player, display it behind their name.
                      if ((club.equals("pradera") || club.equals("pinery") || club.equals("canyonoakscc") || club.equals("beechmontcc") || club.equals("austincountryclub") || club.equalsIgnoreCase("rollinghillsccca") || club.equalsIgnoreCase("bloomfieldhillscc")) && !custom_disp5.equals("")) {
                          p5 += " " + custom_disp5;
                      }
                      ...
                      if (club.equals("dovecanyonclub")) {
                          ...
                          if (mtype5.equalsIgnoreCase("Adult Male")) {
                              p5 += " AM";
                          } else if (mtype5.equalsIgnoreCase("Adult Female")) {
                              p5 += " AF";
                          } else if (mtype5.equalsIgnoreCase("Dependent")) {
                              p5 += " D";
                          }
                      }
         ...
         if (!excel.equals("yes")) {
             ...
             //  col for notes
             out.println("<form method=\"post\" action=\"Proshop_oldsheets\" target=\"_blank\">");
             ...
             if (tsNotesView) {
                 ...
                 out.println("</font></td>");         // end of Notes col
             }
             ...
             //
             out.println("<td bgcolor=\"" + bgcolor + "\" align=\"center\">");
             out.println("<a href=\"javascript: void(0)\" onclick=\"openHistoryWindow('" + calDate + "','" + courseName + "', " + time + "," + fb + "," + teepast_id + ");return false;\"><img src=\"/" +rev+ "/images/history.gif\" width=\"12\" height=\"13\" border=\"0\" title=\"Click here to view tee time history.\"></a>");
             out.println("</td>");

             /*
             out.println("<form method=\"post\" action=\"Proshop_teetime_history\" target=\"_tthistory\">");
             out.println("<font size=\"2\">");
            ...
            if (club.equals( "olyclub" )) {        // if Olympic Club - add Print Receipt col

               out.println("<td bgcolor=\"#8B8970\" align=\"center\">");
               out.println("<font size=\"" +fontSz+ "\">");

               if (!player1.equals( "" ) || !player2.equals( "" ) || !player3.equals( "" ) || !player4.equals( "" )) {
                  out.println("<a href=\"javascript:void(0)\" onClick=\"window.open ('Proshop_oldsheets?prtReceipt=yes&teepast_id=" +teepast_id+ "', 'newwindow', 'Height=440, width=400, toolbar=no, menubar=no, scrollbars=auto, resizable=no, location=no directories=no, status=no')\">");
                  out.println("PR</a>");
               } else {
                  out.println("&nbsp;");
               }
            ...
            if (handicapSys) {       //  if handicap system configured, add column for No-Post option
               ...
               if (!user1.equals("") || !user2.equals("") || !user3.equals("") || !user4.equals("") || !user5.equals("") 
                       || (club.equals("bloomcc") && (!player1.equals("") || !player2.equals("") || !player3.equals("") || !player4.equals("") || !player5.equals("")))) {  // if any members in the tee time

                   out.print("<img onclick=\"doNoPost("+teepast_id+",this.id);\" id=\"nopostbox_"+teepast_id+"_0\" src=\"/" +rev+ "/images/");
                   ...
                   } else {      // if at least one No Post round
      ...
      out.println(SystemUtils.HeadTitle("DB Error"));
      out.println("<BR><BR><H2>Database Error E3</H2>");
      out.println("<BR><BR>Unable to access the Database.");
      out.println("<BR>Please try again later.");
      out.println("<BR><BR>If problem persists, contact customer support.");
      out.println("<BR><BR>" + e1.getMessage());
 ...
 private void editTime(long date, String course, int jump, HttpServletRequest req, PrintWriter out, HttpSession session, Connection con) {
   ...
   boolean handicapSys = Utilities.isHandicapSysConfigured(0, con);     // check for a Handicap System configured (GHIN, CDGA, etc)
   ...
   if ((req.getParameter("letter") != null) || (req.getParameter("return") != null)) { // if user clicked on a name letter
      ...
      if (req.getParameter("pos1") != null) pos1 = Integer.parseInt(req.getParameter("pos1"));
      if (req.getParameter("pos2") != null) pos2 = Integer.parseInt(req.getParameter("pos2"));
      if (req.getParameter("pos3") != null) pos3 = Integer.parseInt(req.getParameter("pos3"));
      if (req.getParameter("pos4") != null) pos4 = Integer.parseInt(req.getParameter("pos4"));
      if (req.getParameter("pos5") != null) pos5 = Integer.parseInt(req.getParameter("pos5"));
      ...
      try {
         ...
         out.println(SystemUtils.HeadTitle("DB Error"));
         out.println("<CENTER><BR><BR><H3>Database Error E4</H3>");
         out.println("<BR><BR>Unable to access the Database.");
         out.println("<BR>Please try again later.");
   ...
   try {
      ...
      out.println(SystemUtils.HeadTitle("DB Error"));
      out.println("<CENTER><BR><BR><H3>Database Error E5</H3>");
      out.println("<BR><BR>Unable to access the Database.");
      out.println("<BR>Please try again later.");
      out.println("<BR><BR>If problem persists, please contact customer support.");
      out.println("<BR><BR>" + e1.getMessage());
   ...
   if (assign == 0) {   // if normal tee time
      ...
      // hack to fix bug causing the modal not to display on this page
      // it seems that these two elements when created have their display
      // set to none on this page, unlike any other page they are created on
      out.println("  $(\"#foretees_modal-container\").css('display', 'block');");
      out.println("  $(\".liknomodal-wrap\").css('display', 'block');");
      out.println("}");
   ...
   out.println("<tr><td align=\"center\"><br>");
      ...
      out.println("<table border=\"0\" align=\"center\" cellpadding=\"5\" cellspacing=\"5\">"); // table to contain 4 tables below
         ...
         out.println("<form action=\"Proshop_oldsheets\" method=\"post\" name=\"can\">");
         out.println("<input type=\"hidden\" name=\"course\" value=\"" + course + "\">");
         out.println("<input type=\"hidden\" name=\"returnCourse\" value=\"" + returnCourse + "\">");
         out.println("<input type=\"hidden\" name=\"calDate\" value=" + calDate + ">");
         out.println("<input type=\"hidden\" name=\"jump\" value=" + jump + ">");
         ...
         out.println("<form action=\"Proshop_oldsheets\" method=\"post\" name=\"playerform\">");
         ...
         out.println("<input type=\"hidden\" name=\"pos1\" value=\"" + pos1 + "\">");
         out.println("<input type=\"hidden\" name=\"pos2\" value=\"" + pos2 + "\">");
         out.println("<input type=\"hidden\" name=\"pos3\" value=\"" + pos3 + "\">");
         out.println("<input type=\"hidden\" name=\"pos4\" value=\"" + pos4 + "\">");
         out.println("<input type=\"hidden\" name=\"pos5\" value=\"" + pos5 + "\">");

            if (assign == 0) {      // if normal tee time
                
               int tdwidth = 370;
               
               if (handicapSys == true) tdwidth = 450;
               ...
               if (tsNotesView) {
                   ...
                   out.println("Notes:&nbsp;<textarea name=\"notes\"" + (!tsNotesUpdate ? " disabled" : "") + " value=\"\" id=\"notes\" cols=\"28\" rows=\"2\">");
                   out.println(notes);
            ...
            out.println("<input type=submit value=\"Submit\" name=\"submit\"><br>");
         ...
         if (hybrid_slot) {
             ...
             if (assign == 0 || req.getParameter("letter") != null) {  // if normal tee time OR user clicked on a name letter
                 ...
                 //
                 alphaTable.nameList(club, letter, "", "", false, parmc, enableAdvAssist, true, out, con);
 ...
 private void cancel(HttpServletRequest req, PrintWriter out, Connection con) {
   ...
   out.println("<Title>Proshop Tee Slot Page</Title>");
   out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_oldsheets?calDate=" + calDate + "&course=" + course + "&returnCourse=" + returnCourse + "&post=yes\">");
   out.println("</HEAD>");
 ...
 private void verify(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
   ...
   if (req.getParameter("pos1") != null) slotParms.pos1 = Short.parseShort(req.getParameter("pos1"));
   if (req.getParameter("pos2") != null) slotParms.pos2 = Short.parseShort(req.getParameter("pos2"));
   if (req.getParameter("pos3") != null) slotParms.pos3 = Short.parseShort(req.getParameter("pos3"));
   if (req.getParameter("pos4") != null) slotParms.pos4 = Short.parseShort(req.getParameter("pos4"));
   if (req.getParameter("pos5") != null) slotParms.pos5 = Short.parseShort(req.getParameter("pos5"));
 ...
 /*        slotParms.pos1 = rs.getShort("pos1");
   ...
   if (req.getParameter("proNameReqd") != null) {           // if name is required
      ...
      if (proShortName.equals("") || proShortName == null) {
         ...
         returnToSlot(out, slotParms, calDate, overrideAccess, 1, nopost1, nopost2, nopost3, nopost4, nopost5);
         return;
      }
   }
   ...
   if (req.getParameter("remove") != null) {
      ...
      if ((slotParms.player1.equals( "" )) && (slotParms.player2.equals( "" )) && (slotParms.player3.equals( "" )) && (slotParms.player4.equals( "" )) && (slotParms.player5.equals( "" ))) {
         ...
         out.println("<BR><BR>");
            ...
            out.println("<input type=\"hidden\" name=\"nopost4\" value=\"" +nopost4+ "\">");
            out.println("<input type=\"hidden\" name=\"nopost5\" value=\"" +nopost5+ "\">");
            out.println("<input type=\"hidden\" name=\"pos1\" value=\"" +slotParms.pos1+ "\">");
            out.println("<input type=\"hidden\" name=\"pos2\" value=\"" +slotParms.pos2+ "\">");
            out.println("<input type=\"hidden\" name=\"pos3\" value=\"" +slotParms.pos3+ "\">");
            out.println("<input type=\"hidden\" name=\"pos4\" value=\"" +slotParms.pos4+ "\">");
            out.println("<input type=\"hidden\" name=\"pos5\" value=\"" +slotParms.pos5+ "\">");
            out.println("<input type=\"hidden\" name=\"proShortName\" value=\"" + proShortName + "\">");
            out.println("<input type=\"hidden\" name=\"edit\" value=\"yes\">");
      ...
      if (((!slotParms.player1.equals( "" )) && (!slotParms.player1.equalsIgnoreCase( "x" )) && (slotParms.g1.equals( "" ))) ||
         ...
         //
         if (!slotParms.player1.equals(slotParms.oldPlayer1) && !slotParms.player1.equals(slotParms.oldPlayer2) && !slotParms.player1.equals(slotParms.oldPlayer3) 
                 && !slotParms.player1.equals(slotParms.oldPlayer4) && !slotParms.player1.equals(slotParms.oldPlayer5)) {

            slotParms.show1 = 0;        // init no-show flag
            slotParms.pos1 = 0;
         }

         if (!slotParms.player2.equals(slotParms.oldPlayer2) && !slotParms.player2.equals(slotParms.oldPlayer3) && !slotParms.player2.equals(slotParms.oldPlayer4) 
         ...
         if (!slotParms.player3.equals(slotParms.oldPlayer3) && !slotParms.player3.equals(slotParms.oldPlayer4) && !slotParms.player3.equals(slotParms.oldPlayer5)) {

            slotParms.show3 = 0;        // init no-show flag
            slotParms.pos3 = 0;
         }

         if (!slotParms.player4.equals(slotParms.oldPlayer4) && !slotParms.player4.equals(slotParms.oldPlayer5)) {
      ...
      //
      if (slotParms.user1.equals("")) nopost1 = 0;
      if (slotParms.user2.equals("")) nopost2 = 0;
      if (slotParms.user3.equals("")) nopost3 = 0;
      if (slotParms.user4.equals("")) nopost4 = 0;
      if (slotParms.user5.equals("")) nopost5 = 0;
      ...
      try {
         ...
         PreparedStatement pstmt6 = con.prepareStatement (
            ...
            "guest_id1 = ?, guest_id2 = ?, guest_id3 = ?, guest_id4 = ?, guest_id5 = ?, " +
            "nopost1 = ?, nopost2 = ?, nopost3 = ?, nopost4 = ?, nopost5 = ?, " +
            "pos1 = ?, pos2 = ?, pos3 = ?, pos4 = ?, pos5 = ? " +
            "WHERE date = ? AND time = ? AND fb = ? AND courseName = ?");
         ...
         pstmt6.setInt(65, nopost1);
         pstmt6.setInt(66, nopost2);
         pstmt6.setInt(67, nopost3);
         pstmt6.setInt(68, nopost4);
         pstmt6.setInt(69, nopost5);

         pstmt6.setInt(70, slotParms.pos1);
         pstmt6.setInt(71, slotParms.pos2);
         pstmt6.setInt(72, slotParms.pos3);
         pstmt6.setInt(73, slotParms.pos4);
         pstmt6.setInt(74, slotParms.pos5);
         ...
         out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
         out.println("<center>");
         out.println("<BR><BR><H2>Database Error E6</H2>");
         out.println("<BR><BR>Unable to access the Database.");
         out.println("<BR>Please try again later.");
         out.println("<BR><BR>If problem persists, contact customer support.");
         out.println("<BR><BR>" + e1.getMessage());
   ...
   out.println("<link rel=\"stylesheet\" href=\"/" +rev+ "/web utilities/foretees2.css\" type=\"text/css\">");
   out.println("<Title>Proshop Tee Slot Page</Title>");
   out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_oldsheets?calDate=" +calDate+ "&course=" + slotParms.course + "&returnCourse=" + slotParms.returnCourse + "&jump=" + slotParms.jump + "&post=yes\">");
   out.println("</HEAD>");
 ...
 */ 
 ...
 public void doInsert(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
   ...
   if (req.getParameter("insertSubmit") == null) {      // if not an insert 'submit' request (from self)
      ...
      out.println("<form action=\"Proshop_oldsheets\" method=\"post\" target=\"bot\">");
      out.println("<input type=\"hidden\" name=\"calDate\" value=\"" +calDate+ "\">");
      if (date >= today) out.println("<input type=\"hidden\" name=\"index\" value=\"" +index+ "\">");
      out.println("<input type=\"hidden\" name=\"date\" value=\"" +date+ "\">");
      out.println("<input type=\"hidden\" name=\"course\" value=\"" +course+ "\">");
      out.println("<input type=\"hidden\" name=\"insert\" value=\"yes\">");
      ...
      try {
         ...
         PreparedStatement pstmt1 = con.prepareStatement (
            "SELECT mm FROM " + ((date >= today) ? "teecurr2" : "teepast2") + " WHERE date = ? AND time = ? AND fb = ? AND courseName = ?");
      ...
      if (date >= today) {
          ...
          try {
             ...
             out.println("<body bgcolor=\"#FFFFFF\" text=\"#000000\">");
             out.println("<font size=\"2\" face=\"Arial, Helvetica, Sans-serif\">");
             out.println("<center><BR><BR><H3>Database Error E7</H3>");
             out.println("<BR><BR>Unable to access the Database.");
             out.println("<BR>Please try again later.");
             out.println("<BR><BR>If problem persists, contact customer support.");
      ...
      out.println("<BR><BR>Date & Time:  <b>" + day_name + " " + mm + "/" + dd + "/" + yy + " " + hr + ":" + SystemUtils.ensureDoubleDigit(min) + " " + sampm + "</b>");
 ...
 private void dbError(HttpServletRequest req, PrintWriter out, Exception exc, int lottery) {
   ...
   out.println("<BR><BR><H3>Database Error E8</H3>");
   out.println("<BR><BR>Sorry, we are unable to access the database at this time.");
   out.println("<BR>Please try again later.");
   out.println("<BR><BR>If problem persists, contact customer support.");
...
private void displayDatabaseErrMsg(String pMessage, String pException, PrintWriter out) {
    out.println(SystemUtils.HeadTitle("Database Error"));
    out.println("<BODY><CENTER>");
    out.println("<BR><BR><H1>Database Error E9</H1>");
    out.println("<BR><BR>Sorry, we are unable to access the database at this time.");
    out.println("<BR>Please try again later.");
 ...
 private void viewSignups(HttpServletRequest req, PrintWriter out, Connection con) {
    ...
    String sdate = (req.getParameter("date") == null) ? "" : req.getParameter("date");    
    ...
    out.println("<form action=\"Proshop_oldsheets\" method=\"GET\">");
    out.println("<input type=\"hidden\" name=\"date\" value=\"" + date + "\">");
    //out.println("<input type=\"hidden\" name=\"course\" value=\"" + course + "\">");
    //out.println("<input type=\"hidden\" name=\"jump\" value=\"" + jump + "\">");
 ...
 private void returnToSlot(PrintWriter out, parmSlot slotParms, String calDate, boolean allowOverride, int skip, int nopost1, int nopost2, int nopost3, int nopost4, int nopost5) {

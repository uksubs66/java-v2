...
import com.foretees.api.records.MemberType;
...
//           out.println("<select size=\"1\" name=\"mtypeopt\" onChange=\"document.playerform.submit()\">");
    ...
    public static Map<String, Map<String, Object>> generatePartnerList(String user_id, int activity_id, int mode, Connection con, PrintWriter out, boolean jquery, boolean generateMap) {
        ...
        String listType = (activity_id == ProcessConstants.MANAGERS_PORTAL?"VIP":"Partner");
        /*
        if(!generateMap){
        out.println("<div class=\"sub_instructions\">");
        out.println("<strong>"+listType+" List</strong> <br />");
        ...
        if (partner_count > 0) {
            ...
            try {
                pstmt = con.prepareStatement(
                        ...
                        + "m.dining_id, m.wc, m.gender, m.ghin, m.username, m.m_ship, m.m_type, m.email, m.email2, m.g_hancap, "
                        + "p.partner_id, p.id "
                        + "FROM partner p "
                ...
                while (rs.next()) {
                    ...
                    if (club.equalsIgnoreCase("lacumbrecc")) {
                        if (rs.getString("m_ship").equalsIgnoreCase("Social Clubhouse") && activity_id != ProcessConstants.DINING_ACTIVITY_ID) {
                            skip = true;
                        }
                    } else if (club.equals("noaks")) {
                        if (!rs.getString("m_ship").equalsIgnoreCase("Golf") && activity_id == 0) {      // North Oaks - skip all non-Golf members if activity is Golf
                            skip = true;
                        }
                    } else if (club.equals("foxchapelgolfclub")) {
                        if (rs.getString("m_ship").equalsIgnoreCase("Unaccompanied") || rs.getString("m_ship").equalsIgnoreCase("Outing")) {
                            skip = true;
                        }
                    } else if (club.equals("lehigh")) {
                        if (activity_id == 0 && (rs.getString("m_ship").equalsIgnoreCase("Non Golf") || rs.getString("m_ship").equalsIgnoreCase("Family Sport & Soc"))) {
                            skip = true;
                        }
                    }
                    ...
                    if (!skip) {
                        ...
                        Map<String, Object> partnerItemMap = new LinkedHashMap<String, Object>();
                        if (generateMap) {
                            
                            
                            partnerItemMap.put("name", partner_name + h_index);
                            partnerItemMap.put("name_only", partner_name);
                            partnerItemMap.put("display", partner_display);
                            partnerItemMap.put("wc", partner_wc);
                            partnerItemMap.put("username", user_name);
                            partnerItemMap.put("gender", rs.getString("gender"));
                            partnerItemMap.put("ghin", partner_hdcp);
                            partnerItemMap.put("m_ship", rs.getString("m_ship"));
                            partnerItemMap.put("m_type", rs.getString("m_type"));
                            partnerItemMap.put("style", style);
                            partnerItemMap.put("id", rs.getString("id"));
                            partnerItemMap.put("partner_id", rs.getString("partner_id"));
                            
                        }

                        if (mode == 0) {
                            if (generateMap) {
                                partnerItemMap.put("dining_id", rs.getInt("dining_id"));
                            } else {
                        ...
                        partnerlist_map.put(user_key, partnerItemMap);
                    }
                }
            ...
            out.println("Select '"+listType+" List' <br>");
    ...
    public static Map<String, Map<String, Object>> getMemebersFromDistributionList(List<String> lists, int mode, HttpSession session, Connection con)
        ...
        int sess_activity_id = (Integer) session.getAttribute("activity_id");    // get activity_id
                        ...
                        for (int i2 = 1; i2 < list_size; i2++) {               // check all 30 (start with 1)
                            ...
                            if (!uname.equals("")) {
                                ...
                                if (rs.next()) {
                                    ...
                                    if (club.equalsIgnoreCase("lacumbrecc")) {
                                        if (rs.getString("m_ship").equalsIgnoreCase("Social Clubhouse") && sess_activity_id != ProcessConstants.DINING_ACTIVITY_ID) {
                                            skip = true;
                                        }
                                    } else if (club.equals("noaks")) {
                                        if (!rs.getString("m_ship").equalsIgnoreCase("Golf") && sess_activity_id == 0) {      // North Oaks - skip all non-Golf members if activity is Golf
                                            skip = true;
                                        }
                                    } else if (club.equals("foxchapelgolfclub")) {
                                        if (rs.getString("m_ship").equalsIgnoreCase("Unaccompanied") || rs.getString("m_ship").equalsIgnoreCase("Outing")) {
                                        ...
                                        if (sess_activity_id == 0 && (rs.getString("m_ship").equalsIgnoreCase("Non Golf") || rs.getString("m_ship").equalsIgnoreCase("Family Sport & Soc"))) {
                                            skip = true;
                                        }
                                    }                       
                                    ...
                                    nameMap.put("name", partner_name);
                                    nameMap.put("display", partner_display);
                                    nameMap.put("wc", partner_wc);
                                    nameMap.put("username", user_name);
                                    nameMap.put("ghin", rs.getString("ghin"));
                                    nameMap.put("m_ship", rs.getString("m_ship"));
                                    nameMap.put("m_type", rs.getString("m_type"));
                                    nameMap.put("gender", rs.getString("gender"));
                                    
                                    namelist_map.put(user_name, nameMap);
     ...
     * @param jquery Flag for indicating if caller is jquery
     */
    public static void displayNameList(String letter, String user, String club, int activity_id, int mode, Connection con, PrintWriter out, boolean jquery) {
        Map<String, Map<String, Object>> tempMap = generateNameList(letter, user, club, activity_id, mode, con, out, jquery, false);
    }
     ...
     * @param con Connection to club database
     */
    public static Map<String, Map<String, Object>> getNameList(String letter, String user, String club, int activity_id, int mode, Connection con, PrintWriter out) {
        return generateNameList(letter, user, club, activity_id, mode, con, out, false, true);
    }
     ...
     * @param generateMap Flag to return Map object of partner list, and not output HTML
     */
    public static Map<String, Map<String, Object>> generateNameList(String letter, String user, String club, int activity_id, int mode, Connection con, PrintWriter out, boolean jquery, boolean generateMap) {

        PreparedStatement pstmt = null;
        ResultSet rs = null;

        boolean skip = false;
        boolean restrictToFamily = false; //club.equals("martiscamp");   Martiscamp will now be using the Hide Member subtype functionality instead
        ...
        try {
            ...
            pstmt = con.prepareStatement(
                    ...
                    + "CONCAT(m.name_last, ', ', m.name_first, IF(m.name_mi <> '', CONCAT(' ', m.name_mi), '')) AS partner_name, "
                    + "CONCAT(m.name_first, IF(m.name_mi <> '', CONCAT(' ', m.name_mi, ' '), ' '), m.name_last) AS partner_display, "
                    + "m.email, m.email2, m.memEmailOpt1, m.memEmailOpt2 "
                    + "FROM member2b AS m "
                    + "WHERE inact = 0 AND billable = 1 AND "
                    + ((restrictToFamily) ? "memNum = ? " : "name_last LIKE ? ") 
                    + "AND (msub_type <> 'Hide Member' OR (memNum <> '' AND memNum = ?)) "
                    + "ORDER BY last_only, name_first, name_mi");
            
            pstmt.clearParameters();
            
            if (restrictToFamily) {
                pstmt.setString(1, Utilities.getmNum(user, con));
            } else {
                pstmt.setString(1, letter + "%");
            }
            
            pstmt.setString(2, Utilities.getmNum(user, con));
            ...
            while (rs.next()) {         
                ...
                if (!skip) {
                    ...
                    if (generateMap) {
                        ...
                        userMap.put("name", partner_name);
                        userMap.put("display", partner_display);
                        userMap.put("wc", partner_wc);
                        userMap.put("username", user_name);
                        userMap.put("ghin", rs.getString("ghin"));
                        userMap.put("m_ship", rs.getString("m_ship"));
                        userMap.put("m_type", rs.getString("m_type"));
                        userMap.put("gender", rs.getString("gender"));
                        userMap.put("style", style);

                        namelist_map.put(user_key, userMap);
                    } else {
    ...
    /**
     * searchNames  -- search for names by given string
     *
     * @param search Search term to use for filtering names ("doe", "john doe", "doe, john", )
     * @param club Club name for use in future customs
     * @param activity_id Activity id to get the partner list for
     * @param mode Run mode:  0 = normal, other values allow for special needs
     * @param con Connection to club database
     */
    public static Map<String, Object> searchNames(String search, String user, String club, int activity_id, int mode, int limit, Connection con, HttpServletRequest req) {
        return searchNames(search, user, club, activity_id, mode, limit, false, con, req);
    }
    public static Map<String, Object> searchNames(String search, String user, String club, int activity_id, int mode, int limit, boolean by_member_number, Connection con, HttpServletRequest req) {
        return searchNames(search, user, club, activity_id, mode, limit, by_member_number, null, con, req);
    }
    public static Map<String, Object> searchNames(String search, String user, String club, int activity_id, int mode, int limit, boolean by_member_number, String mship_filter, Connection con, HttpServletRequest req) {

        PreparedStatement pstmt = null;
        ResultSet rs = null;

        boolean skip = false;
        boolean exactLast = false;
        boolean restrictToFamily = false; //club.equals("martiscamp");   Martiscamp will now be using the Hide Member subtype functionality instead
        boolean proSearch = reqUtil.getSessionString(req, "user", "").toLowerCase().startsWith("proshop");
        
        if(mship_filter == null){
            // Try to get mship filter from querystring
            mship_filter = reqUtil.getParameterString(req, "mship_filter", null);
        }
        if(mship_filter != null && mship_filter.isEmpty()){
            // empty filter is null a filter.
            mship_filter = null;
        }
        
        String mtype_filter = reqUtil.getParameterString(req, "mtype_filter", null);
        if(mtype_filter != null && mtype_filter.isEmpty()){
            // empty filter is null a filter.
            mtype_filter = null;
        }
        ...
        int found = 0;
        int returned = 0;
        int mship_filtered = 0;
        int mtype_filtered = 0;
            
        Map<String, Object> result_map = new LinkedHashMap<String, Object>();
        Map<String, Map<String, Object>> namelist_map = new LinkedHashMap<String, Map<String, Object>>();

        String user_key, first, mid, last, user_name, partner_wc, email, email2, style, memnum;
        
        String firstSearch = "";
        String lastSearch = "";
        String midSearch = "";
        String orig_search = search;
        ...
        search = search.replaceAll("[^a-zA-Z\\u0400-\\u04FF0-9'\\u0027 ,]+","").trim();  // Allow only these characters in the search
        // If searching for names like O'connell and Oconnell becomes a problem,
        //   we'll have to create and maintain first/last/middle search index (using MySQL triggers)
        //   for the member2b table, with everything but A-z converted or stripped, 
        //   and then do the same here for the search words
        //
        //   We should probably do this regardless, since accented character matching 
        //  (allow a search for "a" to match a name with "á" for example) 
        //   also seems to be a problem with the current table structure
        
        //search = search.replaceAll("(.*)[,]+(.*)","$1,$2");
        ...
        // Parse names the same as we would in jquery.foreTeesMemberSelect("parseNames")
        
        // Break the search down to first and last name
        // We'll isolate middle as well, but we currently don't search with it
        String[] searchParts = search.split(",");
        if(searchParts.length > 1){
            debug.put("lastSep", ",");
            lastSearch = searchParts[0].trim();
            String[] midParts = searchParts[1].trim().split(" ");
            if(midParts.length > 1){
                firstSearch = midParts[0].trim();
                midSearch = midParts[1].trim();
            } else {
                firstSearch = searchParts[1].trim();
            }
            exactLast = true;
        } else {
            debug.put("lastSep", " ");
            ...
            if(searchParts.length > 2){
                lastSearch = searchParts[2].trim();
                midSearch = searchParts[1].trim();
                firstSearch = searchParts[0].trim();
            } else if(searchParts.length > 1) {
                lastSearch = searchParts[1].trim();
                firstSearch = searchParts[0].trim();
            } else {
        ...
        debug.put("firstSearch", firstSearch);
        debug.put("lastSearch", lastSearch);
        debug.put("exactLast", exactLast);
        debug.put("search", search);
        debug.put("orig_search", orig_search);
        
        result_map.put("_debug",debug);
        result_map.put("results",namelist_map);
        ...
        if(!lastSearch.equals("")){
            try {
                ...
                if(firstSearch.equals("")){
                    // Only a single search term.  Search both first and last name, placeing first name matches below last
                    // The "Limit 20000" is required to keep each select group (last name match and first name match) ordered.  Seems like a bug in Mysql, by Mysql devs seem to think it's intended behaviour.
                    // "20000" was chosen because search results should never reach a number that high.  If an individual letter will return
                    // more than 20,000 members, then this number my need to be bumped -- but in all likleyhood, if individual letters are returning that
                    // many results, we'll need to reevaluate this search logic, probably limiting all results to 1,000 or so.
                    if(by_member_number){
      
                        sql = "SELECT * FROM ("
                                + "(SELECT SUBSTRING_INDEX(name_last, '_', 1) AS last_only, name_last, name_first, name_mi, dining_id, wc, username, m_ship, m_type, ghin, gender, "
                                + " m.email, m.email2, memNum, m.id "
                                + "FROM member2b AS m "
                                + "WHERE name_last LIKE ? AND SUBSTRING_INDEX(name_last, '_', 1) LIKE ? AND inact = 0 " + (proSearch?"":" AND billable = 1 ")
                                + "AND (msub_type <> 'Hide Member' OR memNum = ?) "
                                + ((restrictToFamily) ? "AND memNum = ? " : "")
                                + "ORDER BY last_only, name_first, name_mi LIMIT 20000) "
                                + "UNION "
                                + "(SELECT SUBSTRING_INDEX(name_last, '_', 1) AS last_only, name_last, name_first, name_mi, dining_id, wc, username, m_ship, m_type, ghin, gender, "
                                + " m.email, m.email2, memNum, m.id "
                                + "FROM member2b AS m "
                                + "WHERE name_first LIKE ? AND inact = 0 " + (proSearch?"":" AND billable = 1 ")
                                + "AND (msub_type <> 'Hide Member' OR memNum = ?) "
                                + ((restrictToFamily) ? "AND memNum = ? " : "")
                                + "ORDER BY name_first, last_only, name_mi LIMIT 20000)"
                                + "UNION "
                                + "(SELECT SUBSTRING_INDEX(name_last, '_', 1) AS last_only, name_last, name_first, name_mi, dining_id, wc, username, m_ship, m_type, ghin, gender, "
                                + " m.email, m.email2, memNum, m.id "
                                + "FROM member2b AS m "
                                + "WHERE memNum LIKE ? AND inact = 0 " + (proSearch?"":" AND billable = 1 ")
                                + "AND (msub_type <> 'Hide Member' OR memNum = ?) "
                                + ((restrictToFamily) ? "AND memNum = ? " : "")
                                + "ORDER BY memNum, last_only, name_first, name_mi LIMIT 20000)"
                                + ") UT";
                    ...
                    pstmt.clearParameters();
                    int i = 1;
                    pstmt.setString(i++, lastSearch + "%");
                    pstmt.setString(i++, lastSearch + lastMatch);
                    pstmt.setString(i++, user_mNum);
                    if (restrictToFamily) {
                        pstmt.setString(i++, user_mNum);
                    }
                    pstmt.setString(i++, lastSearch + "%");
                    ...
                    sql = ""
                            ...
                            + " m.email, m.email2, memNum, m.id "
                            + "FROM member2b AS m "
                            + "WHERE name_last LIKE ? AND SUBSTRING_INDEX(name_last, '_', 1) LIKE ? AND name_first LIKE ? AND inact = 0 "
                            + "AND (msub_type <> 'Hide Member' OR memNum = ?) "
                            + ((restrictToFamily) ? "AND memNum = ? " : "")
                            + "ORDER BY last_only, name_first, name_mi";
                    pstmt = con.prepareStatement(sql);
                    ...
                    pstmt.clearParameters();
                    pstmt.setString(1, lastSearch + "%");
                    pstmt.setString(2, lastSearch + lastMatch);
                    pstmt.setString(3, firstSearch + "%");
                    pstmt.setString(4, user_mNum);
                    if (restrictToFamily) pstmt.setString(5, user_mNum);
                ...
                while (rs.next()) {         
                    ...
                    user_key = "user_" +found+ "_"+ user_name; // Prepend "user_" to the username key so numeric names are kept in order in javascript
                    //partner_name = rs.getString("partner_name");
                    //partner_display = rs.getString("partner_display");
                    partner_wc = rs.getString("wc");
                    email = rs.getString("email"); // Note, for security reasons, never add the email address to the response object
                    ...
                    if (mode == 2) {  // email-list and json mode
                        // Exclude this user if no email address, or other custom casea
                        if (
                                ((email == null || email.equals("")) && (email2 == null || email2.equals("")))
                                || (!proSearch && (club.equals("treesdalegolf") || user_name.equals("R0084")))
                                ) {
                            skip = true;
                        }
                    }
                    ...
                    if (club.equalsIgnoreCase("lacumbrecc")) {
                        if (mship.equalsIgnoreCase("Social Clubhouse") && activity_id != ProcessConstants.DINING_ACTIVITY_ID) {
                            skip = true;
                        }
                    } else if (club.equals("noaks")) {
                        if (!mship.equalsIgnoreCase("Golf") && activity_id == 0) {      // North Oaks - skip all non-Golf members if activity is Golf
                            skip = true;
                        }
                    } else if (club.equals("foxchapelgolfclub")) {
                        if (mship.equalsIgnoreCase("Unaccompanied") || mship.equalsIgnoreCase("Outing")) {
                            skip = true;
                        }
                    } else if (club.equals("lehigh")) {
                        if (activity_id == 0 && (mship.equalsIgnoreCase("Non Golf") || mship.equalsIgnoreCase("Family Sport & Soc"))) {
                            skip = true;
                        }
                    }
                    
                    if(!skip && mship_filter != null){
                        skip = !mship.equalsIgnoreCase(mship_filter);
                        mship_filtered ++;
                    }
                    if(!skip && mtype_filter != null){
                        skip = !mtype.equalsIgnoreCase(mtype_filter);
                        mtype_filtered ++;
                    }
                    ...
                    if (!skip) {
                        ...
                        if(found<=limit || limit ==0){
                            ...
                            member_type = new MemberType(club, mtype, user_name, gender);
                            Map<String, Object> userMap = new LinkedHashMap<String, Object>();
                            if(firstSearch.equals("")){
                                userMap.put("first_match", first.replaceAll("^((?i)"+lastSearch+"){0,1}(.*)$","<b>$1</b>$2"));
                            } else {
                                userMap.put("first_match", first.replaceAll("^((?i)"+firstSearch+"){0,1}(.*)$","<b>$1</b>$2"));
                            }
                            userMap.put("last_match", last.replaceAll("^((?i)"+lastSearch+"){0,1}(.*)$","<b>$1</b>$2"));
                            //userMap.put("name", partner_name);
                            //userMap.put("display", partner_display);
                            userMap.put("first", first);
                            userMap.put("middle", mid);
                            userMap.put("last", last);
                            userMap.put("wc", partner_wc);
                            userMap.put("id", rs.getInt("id"));
                            userMap.put("username", user_name); // !!! All code (much of it javascript) needs to be changed to use the member's id instead of username.  Using username here is a security issue for some clubs.
                            if(by_member_number){
                                userMap.put("memnum", memnum);
                                userMap.put("memnum_match", memnum.replaceAll("^((?i)"+lastSearch+"){0,1}(.*)$","<b>$1</b>$2"));
                            }
                            userMap.put("ghin", rs.getString("ghin"));
                            userMap.put("m_ship", mship);
                            userMap.put("m_type", mtype);
                            userMap.put("gender", gender);
                            //userMap.put("debug", member_type);
                            userMap.put("mtype_gender", member_type.gender); // Gender from member's specified gender, or gender derived from mtype if member's gender is not "M" or "F"
                            userMap.put("mtype_status_abrv", member_type.type_status_abrv);
                            userMap.put("dining_id", rs.getString("dining_id"));
                            userMap.put("class", style);
                ...
                Utilities.logError("nameLists.displayNameList Error: " + org.apache.commons.lang.exception.ExceptionUtils.getStackTrace(exc));
                debug.put("error", exc.toString());
        ...
        if(lastSearch.equals("")){
            ...
            result_map.put("messages",new String[]{"You must enter a valid name to search for."});
        }else if(found == 0){
            // Nothing found
            ...
            if(mship_filter != null && mship_filtered > 0){
                message.append("<b>");
                message.append(mship_filter);
                message.append("</b> ");
            }
            if(mtype_filter != null && mtype_filtered > 0){
                if(mship_filter != null && mship_filtered > 0){
                    message.append(" / ");
                }
                message.append("<b>");
                message.append(mtype_filter);
                message.append("</b> ");
            }
            ...
            if(mode == 2){
                message.append("with a valid email address found");
            } else if(mode == 3){
                message.append("with handicap number found");
            } else {
                message.append("found");
            }
            message.append(" having a");
            if(firstSearch.equals("")){
                if(by_member_number){
                    message.append(" first name, last name or member number");
                } else {
                    message.append(" first or");
                }
            }
            if(firstSearch.equals("") && !by_member_number){
                message.append(" last name");
            }
            if(!exactLast){
                message.append(" beginning with");
            } else {
                message.append(" of");
            }
            message.append(" \"");
            message.append(lastSearch);
            message.append("\"");
            if(!firstSearch.equals("")){
                message.append(" and a first name beginning with \"");
                message.append(firstSearch);
                message.append("\"");
            }
            
            message.append(".");
            result_map.put("messages",new String[]{message.toString()});
            ...
            result_map.put("messages",new String[]{
                "Showing " + returned + " of " + found + " found.",
                "Please narrow your search by entering more of the member's name" + end
            });
        }
        result_map.put("found",found);
        result_map.put("returned",returned);
        return result_map;

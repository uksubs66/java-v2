...
public static Map<String, Object> getAllActivitities(final String club, Connection con) {
 
    return getAllActivitities(club, false, con);
}


public static Map<String, Object> getAllActivitities(final String club, boolean allowStaging, Connection con) {
    ...
    //
    // add the activities in default order, we can customize further using clubname
    //
    
    if (foretees_mode > 0) {

        response_map.put("Golf", 0);
    }

    if (dining_mode > 0 && dining_staging != 1) {

        response_map.put("Dining", ProcessConstants.DINING_ACTIVITY_ID);
    }

    for (i = 0; i < activity_names.size(); i++) {

        response_map.put(activity_names.get(i), activity_ids.get(i));
    }
...
public static int getChildlessActivityCount(Connection con) {
 ...
 public static int getRootIdFromActivityId (int activity_id, HttpServletRequest req) {
     
     if(activity_id == 0 || activity_id == ProcessConstants.DINING_ACTIVITY_ID){
         return activity_id;
     }
     
    Map<Integer,Integer> cache = (Map<Integer,Integer>) reqUtil.getRequestMap(req, "FtRootActivityCache", null);
    if(cache == null){
        cache = new HashMap<Integer,Integer>();
        req.setAttribute("FtRootActivityCache", cache);
    }
    Integer result = cache.get(activity_id);
    if(result == null){
        try{
            result = getActivity.getRootIdFromActivityId (activity_id, Connect.getCon(req));
            cache.put(activity_id, result);
        } catch(Exception e){
            result = activity_id;
        }
 ...
 public static String getRootNameFromActivityId (int activity_id, HttpServletRequest req) {
     ...
     activity_id = getRootIdFromActivityId (activity_id, req);
     
     String result = "";
     
     Map<Integer,String> cache = (Map<Integer,String>) reqUtil.getRequestMap(req, "FtRootActivityNameCache", null);
    if(cache == null){
        cache = new HashMap<Integer,String>();
        req.setAttribute("FtRootActivityNameCache", cache);
    }
    result = cache.get(activity_id);
    if(result == null){
        try{
            result = getActivityName (activity_id, Connect.getCon(req));
            cache.put(activity_id, result);
        } catch(Exception e){
            result = "Unknown";
        }
 ...
 // Overloaded method call for instances where a root name is needed, but no Request object is available. Use req version where possible!!
 public static String getRootNameFromActivityId (int activity_id, Connection con) {
     ...
     try {
         result = getActivityName(getRootIdFromActivityId (activity_id, con), con);
     } catch (Exception e) {
         result = "Unknown";
     }
 ...
 public static int getActivityCount(Connection con) {
     ...
     // First check if they have FlxRez active.  If not, we know they only have golf and can return 1
     if (!isConfigured(con)) {
 ...
 public static boolean buildActivitySelect(int size, int selected_id, String custom_action, boolean include_all, Connection con, PrintWriter out) {
     ...
     boolean result = buildActivitySelect(size, selected_id, custom_action, include_all, con, sb);
     out.print(sb.toString());
     return result;
 }
 
 
  public static boolean buildActivitySelect(int size, int selected_id, String custom_action, boolean include_all, Connection con, StringBuilder sb) {
     ...
     try {
         ...
         if (rs.next() || golfEnabled) {
             ...
             sb.append("<select name=\"activity_id\" size=\"" + size + "\" " + custom_action + ">");

             if (golfEnabled) {
                 sb.append("<option " + (selected_id == 0 ? "selected " : "") + "value=\"0\">Golf</option>");
             }

             if (diningEnabled) {
                 sb.append("<option " + (selected_id == ProcessConstants.DINING_ACTIVITY_ID ? "selected " : "") + "value=\"" + ProcessConstants.DINING_ACTIVITY_ID + "\">Dining</option>");
             }
             ...
             while (rs.next()) {
                 ...
                 sb.append("<option " + selected + "value=\"" + rs.getInt("activity_id") + "\">" + rs.getString("activity_name") + "</option>");
             }

             if (include_all) {
                 sb.append("<option " + (selected_id == 999 ? "selected " : "") + "value=\"999\">ALL</option>");
             }
         ...
         sb.append("<!-- Error building activity select: " + exc.getMessage() + " -->");
         error = true;
 ...
 public static LinkedHashMap<Integer, String> getRootActivityMap(HttpServletRequest req) {
     ...
     try {
         ...
         if (rs.next() || golfEnabled) {
             ...
             if (diningEnabled) {
                 result.put(ProcessConstants.DINING_ACTIVITY_ID, "Dining" );
             }

             while (rs.next()) {
                 result.put(rs.getInt("activity_id"), rs.getString("activity_name"));
             }
 ...
 public static void buildActivityTree(int activity_id, int selected_activity_id, PrintWriter out, Connection con, HttpServletRequest req) {
     ...
     try {
         ...
         if (parent_id == activity_id) {
             ...
             buildActivityTree(parent_id, activity_id, out, con, req);
         }
         ...
         if (child_count > 0) {
             ...
             if (child_ids.size() == 1) {
                 ...
                 out.print("<select name=activity_id_select onchange=\"reloadPageActId(this.options[this.selectedIndex].value)\">");

                 if (selected_activity_id == 0) out.print("<option selected value=\"0\">CHOOSE...</option>");

                 for (int i=0; i < child_ids.size(); i++) {
                     out.print("<option value=\"" + child_ids.get(i) + "\"" + (child_ids.get(i) == selected_activity_id ? " selected" : "") + ">" + getActivity.getActivityName(child_ids.get(i), con) + "</option>");
                 }
 ...
 public static List<Integer> getColsPerRowCsv(int activity_id, Connection con) {
     
     PreparedStatement pstmt = null;
     ResultSet rs = null;
     
     List<Integer> cols_per_row = new ArrayList<Integer>();
     
     try {
         
         pstmt = con.prepareStatement("SELECT cols_per_row_csv FROM activities WHERE activity_id = ?");
         pstmt.clearParameters();
         pstmt.setInt(1, activity_id);
         
         rs = pstmt.executeQuery();
         
         if (rs.next() && !rs.getString("cols_per_row_csv").equals("")) {
             
             String[] cols_array = rs.getString("cols_per_row_csv").split(",");
             
             for (int i = 0; i < cols_array.length; i++) {
                 try {
                     cols_per_row.add(Integer.parseInt(cols_array[i]));
                 } catch (Exception ignore) { }
             }
         }
         
     } catch (Exception exc) {
         Utilities.logError("ERROR: getActivity.getColsPerRowCsv() club=" + Utilities.getClubName(con, true) + ", activity_id=" + activity_id + ", err=" + exc.toString());
     } finally {
         Connect.close(rs, pstmt);
     }
     
     if (cols_per_row.isEmpty()) {
         cols_per_row.add(10);
     }

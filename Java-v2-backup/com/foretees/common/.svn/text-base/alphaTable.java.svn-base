...
//           out.println("<select size=\"1\" name=\"mtypeopt\" onChange=\"document.playerform.submit()\">");
    ...
    public static void nameList(String club, String letter, String mshipOpt, String mtypeOpt, boolean ghinOnly,
        ...
        String[] mtypeA = new String[60];            // arrays to hold family names to put in order (Medinah)
        String[] mNameA = new String[60];
        String[] dNameA = new String[60];
        String[] userA = new String[60];
        
        boolean useMshipTflags = false;
        ...
        Map<String, String> mshipTflags = new HashMap<String, String>();
        ...
        if (useMshipTflags) {
            ...
            try {
                pstmt = con.prepareStatement("SELECT mship, tflag FROM mship5 WHERE activity_id = ?");
                pstmt.clearParameters();
                pstmt.setInt(1, 0);    // THIS SHOULD BE DYNAMIC!!! We don't have access to an activity ID here, so just hardcoding to 0 for now, since Waialae only has golf
                
                rs = pstmt.executeQuery();
                
                while (rs.next()) {
                    mshipTflags.put(rs.getString("mship"), rs.getString("tflag"));
                }
        ...
        try {
            ...
            if (club.equals("congressional") || club.equals("farmingtoncc")) {
            ...
            if (club.equalsIgnoreCase("interlachen")) {
                notMembers = " m_ship <> 'Social' AND ";
            } else if (club.equalsIgnoreCase("lacumbrecc")) {
                notMembers = " m_ship <> 'Social Clubhouse' AND ";
            } else if (club.equalsIgnoreCase("noaks")) {
                notMembers = " m_ship = 'Golf' AND ";          // ONLY 'Golf' members
            } else {
                notMembers = "";
            }
            ...
            while (rs.next()) {
                ...
                if (useMshipTflags) {
                    ...
                    if (mshipTflags.get(mship) != null && !mshipTflags.get(mship).equals("")) {
                        tflag = mshipTflags.get(mship);
                    }
                }
                ...
                if (club.equals("medinahcc") || club.equals("oswegolake") || club.equals("virginiacc") || club.equals("pinehurstcountryclub") || 
                    ...
                    if (!mnum.equals("")) {
                        ...
                        if (club.equals("charlottecc") || club.equals("loxahatchee") || club.equals("princessannecc") || club.equals("hillwoodcc") 
                                || club.equals("bearpath") || club.equals("ccyork") || club.equalsIgnoreCase("fortwayne") || club.equals("pattersonclub") 
                                || club.equals("tpcsugarloaf")) {
                    ...
                    } else if (club.equals("martiscamp") && (mship.equalsIgnoreCase("Social") || mship.equalsIgnoreCase("Extended Family"))) {

                        if (mship.equals("Social")) {
                            out.println("<option value=\"" + wname + "\" style=\"color:orange\">" + dname + "</option>");
                        } else if (mship.equals("Extended Family")) {
                            out.println("<option value=\"" + wname + "\" style=\"color:green\">" + dname + "</option>");
                        }
                        
                    } else if (club.equalsIgnoreCase("kiawahislandclub")) {
                        if (mship.equalsIgnoreCase("Golf")) {
                            out.println("<option value=\"" + wname + "\" style=\"color:green\">" + dname + "</option>");
                        } else if (mship.equalsIgnoreCase("Sports") || mship.equalsIgnoreCase("Sports Junior")) {
                            out.println("<option value=\"" + wname + "\" style=\"color:blue\">" + dname + "</option>");
                        } else if (mship.equalsIgnoreCase("Social")) {
                            out.println("<option value=\"" + wname + "\" style=\"color:red\">" + dname + "</option>");
                        } else if (mship.equalsIgnoreCase("Sub Sports") || mship.equalsIgnoreCase("Sub Golf") || mship.equalsIgnoreCase("Sub Sports Junior")) {
                            out.println("<option value=\"" + wname + "\" style=\"color:goldenrod\">" + dname + "</option>");
                        } else {
    ...
    public static Map<String, Map<String, Object>> guestdbList(String user, int caller, int activity_id, int cur_guest_id, PrintWriter out, Connection con, boolean generateMap) {
        ...
        Boolean tpcClub = false;                        //check to see if club is a TPC club
        if(Utilities.isClubInGroup(club, "TPC")) {
            tpcClub = true;
        }
        ...
        if (caller != 1 && (!generateMap)) {
            ...
            out.println("<tr>");
            out.println("<td align=\"center\">");
            out.println("<input style=\"width:165px;\" type=text name=DYN_search onkeyup=\"DYN_triggerChange()\" onkeypress=\"DYN_moveOnEnterKey(event); return DYN_disableEnterKey(event)\" onclick=\"this.focus();this.select();\" value=\"Quick Search Box\">"); // return DYN_disableEnterKey(event)
            out.println("</td></tr>");
        ...
        if (!generateMap) {
            if (caller != 1) {
                ...
                //} else {
                    out.println("<select size=\"15\" name=\"bname\" style=\"width:165px; cursor:hand;\" onchange=\"DYN_selectChange(this, passguest);\" onclick=\"DYN_selectClick(this, passguest);\" onblur=\"DYN_selectBlur(this, passguest);\" style=\"\">");
                //}
            ...
            result_map.get("guest_tba").put("last", "");
            result_map.get("guest_tba").put("middle", "");
            result_map.get("guest_tba").put("first", "");
        ...
        try {
            ...
            pstmt = con.prepareStatement(
                    "SELECT d.guest_id, d.name_last, d.name_first, d.name_mi, CONCAT(d.name_last, ', ', d.name_first, IF(d.name_mi != '', CONCAT(' ', d.name_mi), '')) as display_name, "
                    ...
                    + checkUsername2
                    + filterGuestId
                    + "ORDER BY d.name_last, d.name_first, d.name_mi");
            ...
            while (rs.next()) {
                ...
                email = rs.getString("d.email1");
                email2 = rs.getString("d.email2");
                ...
                if (!generateMap) {
                    if (caller != 1) {
                        if (tpcClub && (email.equalsIgnoreCase("tpcpassport@tpcpassport.com") || email2.equalsIgnoreCase("tpcpassport@tpcpassport.com"))) {
                            out.println("<option val=\"1\" value=\"" + guest_name + ":" + guest_id + "\">" + display_name + "</option>");
                        } else {
                        ...
                        if (tpcClub && (email.equalsIgnoreCase("tpcpassport@tpcpassport.com") || email2.equalsIgnoreCase("tpcpassport@tpcpassport.com"))) {
                            out.println("<option val=\"1\" value=\"" + guest_id + "\">" + display_name + "</option>");
                        } else {
                    ...
                    result_map.get("guest_" + guest_id).put("first", rs.getString("name_first"));
                    result_map.get("guest_" + guest_id).put("last", rs.getString("name_last"));
                    result_map.get("guest_" + guest_id).put("middle", rs.getString("name_mi"));

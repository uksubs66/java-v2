...
public class Logout extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (session == null) {
        // User is already logged out, or session has timed out
        // Try to get the club any other information so we can simulate a logout
        club = Utilities.getParameterString(req, ProcessConstants.RP_CLUB, "");
        user = Utilities.getParameterString(req, "u", "");
        if(!club.equals("") && !user.equals("")){
            caller = Utilities.getParameterString(req, "cl", "");
            new_skin = true;
            ftConnect = Utilities.getParameterInteger(req, "ftc", 0);
            ext_login = (Utilities.getParameterInteger(req, "ext", 0)>0);
            sess_activity_id = Utilities.getParameterInteger(req, ProcessConstants.RP_ACTIVITY, 0);
        ...
        // Note -- if you add any more, make sure it is passed in the Logout uri in CommonSkin as well
    }
    ...
    //
    if (club.equals( "fortcollins" ) && !user.startsWith("sales")) {        // if normal foretees user
    ...
    //  Exit
    //
    if (caller.equals( "none" ) && (ftConnect == 0 || !club.equals("democonnect"))) {        // if normal foretees user
        ...
        if (mobile == 0) {
            ...
            if (new_skin) {            // WILL BE FALSE FOR ADMIN AND PROSHOP USERS !!!!!!!!!!!!!!!!!
               ...
               out.println("<form action=\"#\" onsubmit=\"self.close()\"><input id=\"exitButton\" type=\"submit\" value=\"RETURN\" id=\"submit\" onClick='self.close();'>");
               out.println("</form></center></div></div>");

 ...
 *   Last Updated:  
 *
 *   7/24/13   Manufacturers now utilize activity ids to allow for different activities to have separate lists of manufacturers.
 *   4/10/13   Add teesheetLookUp method to process call from Proshop_sheet Control Panel to display any checked out demo clubs by members on that day's tee sheet.
 *   4/09/13   Add notes box to checkout page and reports (case 1878).
 *   3/07/13   Updated check-in page to only display checked out equipment for the current activity. Also added a message to be displayed if no equipment is currently checked out.
 *  12/15/11   Small update to make the Maintain Inventory page's HTML easier to read.
...
public class Proshop_demo_clubs extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (req.getParameter("printClubs") != null) {
        ...
        printClubsOut(req, session, resp, con, out, lottery, sess_activity_id);
        return;
        ...
        getCheckIn(req, session, con, out, lottery, sess_activity_id);
        return;
    ...
    } else if (req.getParameter("notesDisplay") != null) {
    
        dispNotes(req, session, con, out, lottery, sess_activity_id);    // Display the notes
        return;
        
    } else if (req.getParameter("teesheetLookup") != null) {
    
        teesheetLookUp(req, session, con, out, lottery, sess_activity_id);    // get members from tee sheet anc check for checked out clubs (from Proshop_sheet)
        return;
    ...
    if (req.getParameter("mfrCfg") != null) {
        ...
        try {
            ...
            out.println("<form method=\"get\" action=\"Proshop_demo_clubs\">");
            ...
            pstmt2 = con.prepareStatement("SELECT * FROM demo_clubs_mfr WHERE id > 2 AND (activity_id = ? OR activity_id = -99) ORDER BY mfr");
            ...
            try {pstmt2.close();} catch (Exception ignore) {}
        }
        ...
        if (club_id > 0) {
            ...
            try {
                ...
                if (rs.next()) {
                    ...
                    out.println("<br><a href=\"Proshop_demo_clubs?demoConfig&club_id=" + club_id + "\" style=\"text-decoration:underline\"><button style=\"width:80px\">Return</button></a>");
                    out.println("&nbsp;<a href=\"Proshop_demo_clubs?getCheckIn\" style=\"text-decoration:underline\"><button style=\"width:100px\">Club Check-In</button></a>");
...
//        out.println("<select name=club_id size=10 onchange=\"selectClub(this.options[this.selectedIndex].value)\" style=\"width:185px\">");
        ...
        try {
            ...
            while (rs.next()) {
                ...
                out.println(">" + rs.getString("dcm.mfr") + " - " + rs.getString("dct.type") + " - " + rs.getString("name") + "</option>");
            }
        ...
        if (club_id != 0 && req.getParameter("add") == null) {
            ...
            try {
                ...
                if (rs.next()) {
                    ...
                    pstmt2 = con.prepareStatement("SELECT * FROM demo_clubs_mfr WHERE id > 2 && (activity_id = ? OR activity_id = -99) ORDER BY mfr");
                    pstmt2.clearParameters();
                    pstmt2.setInt(1, sess_activity_id);
                    ...
                    out.println("</select>&nbsp;");
                    out.println("<font size=\"2\"><a href=\"javascript: void(0)\" onclick=\"window.open ('Proshop_demo_clubs?mfrCfg', 'mfrCfg', 'height=550, width=400, toolbar=no, menubar=no, scrollbars=auto, resizable=yes, location=no directories=no, status=no')\">Configure Manufacturers</a></font>");
                    out.println("</td></tr>");
                    ...
                    out.println("</select>&nbsp;");
                    out.println("<font size=\"2\"><a href=\"javascript: void(0)\" onclick=\"window.open ('Proshop_demo_clubs?typeCfg', 'typeCfg', 'height=550, width=400, toolbar=no, menubar=no, scrollbars=auto, resizable=yes, location=no directories=no, status=no')\">Configure " + (sess_activity_id == 0 ? "Club" : "Equipment") + " Types</a></font>");
                    out.println("</td></tr>");
                ...
                out.println("<a href=\"Proshop_demo_clubs?demoConfig\" style=\"text-decoration:underline;color:black\"><button style=\"width:80px\">Reset</button></a>");
                out.println("</td></tr>");
 ...
 private void printClubs(HttpServletRequest req, HttpServletResponse resp, Connection con, PrintWriter out, int lottery, int sess_activity_id) {
    ...
    if (!toExcel) {  // Don't print header if user requested Excel Spreadsheet Format
        ...
        out.println("<h2 align=center>Demo " + ((sess_activity_id == 0) ? "Club" : "Equipment") + " Listing</h2>");
        out.println("");
        out.println("");
        ...
        out.println("<form method=\"get\" action=\"Proshop_demo_clubs\" target=\"_blank\">");
 ...
 private void printClubUsage(HttpServletRequest req, HttpServletResponse resp, Connection con, PrintWriter out, int lottery, int sess_activity_id) {
    ...
    if (!toExcel) {  // Don't print header if user requested Excel Spreadsheet Format    
        ...
        out.println("<h2 align=center>Demo " + ((sess_activity_id == 0) ? "Club" : "Equipment") + " Usage Report</h2>");
        out.println("");
        out.println("");
 ...
 private void printClubsOut(HttpServletRequest req, HttpSession session, HttpServletResponse resp, Connection con, PrintWriter out, int lottery, int sess_activity_id) {
    ...
    if (!toExcel) {
        ...
        out.println("<script type=\"text/javascript\">");
        out.println("<!--");
        out.println("function openNotesWindow(id) {");
        out.println("w = window.open ('Proshop_demo_clubs?checkoutId=' +id+ '&notesDisplay=yes','notesyPopup','width=640,height=360,scrollbars=1,dependent=0,directories=0,location=0,menubar=0,resizable=1,status=0,toolbar=0');");
        ...
        out.println("<h2 align=center>Demo " + ((sess_activity_id == 0) ? "Clubs" : "Equipment") + " Checked Out</h2>");
        out.println("");
        out.println("");
        if (sendEmails == false) {
         ...
         if (!user.startsWith("proshop") || SystemUtils.verifyProAccess(req, "TOOLS_EMAIL", con, out)) {
             ...
             out.println("<center><form method=\"get\" action=\"Proshop_demo_clubs\">");
             out.println("<input type=\"hidden\" value=\"yes\" name=\"printClubsOut\">");
   ...
   // out.println("<table align=center border=1 width=\"75%\">");
    ...
    out.println("<tr align=center style=\"background:#336633; color:white; font-size:10pt; font-weight:bold\"><td>Manufacturer</td>"
            + "<td>" + ((sess_activity_id == 0) ? "Club " : "") + "Type</td><td>Demo " + ((sess_activity_id == 0) ? "Club" : "Equipment") + "</td>"
            + "<td>ICN</td><td>Name</td><td>Mem #</td>" + (sess_activity_id == 0 ? "<td>Bag #</td>" : "") + "<td>Date Out</td><td>Days Out</td>"
            + "<td>Notes</td><td>Send Email</td></tr>");
    }
    try {

        pstmt = con.prepareStatement(
                "SELECT dcu.*, dc.*, dcm.mfr, dct.type, CONCAT(m.name_first, ' ', m.name_last) AS memName, m.memNum, m.bag, m.email, " +
        ...
        while (rs.next()) {
            ...
            checkoutId = rs.getInt("dcu.id");
            notes = rs.getString("dcu.notes");
            ...
            out.println("<tr align=center><td align=left><font size =1>&nbsp;" + rs.getString("dcm.mfr") + "</font></td><td align=left><font size =1>&nbsp;" + rs.getString("dct.type") + "</font></td>" +
                    ...
                    "<td><font size =1>" + rs.getString("dateout") + "</font></td><td><font size =1>" + rs.getInt("days_out") + "</font></td>");

            if (!notes.equals("") && !toExcel) {

                out.println("<td align=\"center\"><form><font size=\"2\">");   // notes attached
                out.println("<input type=\"image\" src=\"/" + rev + "/images/notes.jpg\" border=\"0\" onclick=\"openNotesWindow(" + checkoutId + ");return false;\" title=\"Click here to view notes.\">");
                out.println("</font></form></td>");         // end of the notes col

            } else {

                out.println("<td>&nbsp;</td>");   // no notes attached
            ...
            if (!email.equals("")) {
                ...
                out.println("<td>&nbsp;</td>");   // no email address
            }
    ...
    if (!toExcel) {
     ...
     if (sendEmails == true) {     // if request to send emails to all members in the report
         ...
         if (countE > 0) {
             ...
             out.println("<center><font size=\"3\"><br><p align=center>You have requested to send an email to all " + countE + " members in this report (that have email addresses).</p>");
             ...
             out.println("<br><p align=center>");
             out.println("<form action=Proshop_demo_clubs method=get><input type=hidden name=printClubsOut value=yes><input type=submit value=\"No, Go Back!\" style=\"width: 90px; background:#8B8970\">");
 ...
 private void printMemberHistory(HttpServletRequest req, HttpServletResponse resp, int lottery, int sess_activity_id, String club, Connection con, PrintWriter out) {
    ...
    if (req.getParameter("member_username") == null || req.getParameter("member_username").equals("")) {
        ...
        out.println("<br><h2 align=center>Member Demo " + ((sess_activity_id == 0) ? "Clubs" : "Equipment") + " History</h2>");
        out.println("</td></tr>");
        out.println("</table>");


        out.println("<table align=center border=\"1\" cellpadding=\"10\" bgcolor=\"#F5F5DC\"><tr>");
        out.println("<form method=\"get\" name=\"frmSelectMem\" action=\"Proshop_demo_clubs\">");
        out.println("<input type=\"hidden\" name=\"printMemberHistory\">");
        out.println("<tr><td align=\"center\" valign=\"center\">");
        out.println("Please select a member from the list to the right.<br><br><br>");
        ...
        out.println("<br><br><br><input type=button value=\"Run Report\" onclick=\"submitForm()\" name=btnSubmit style=\"width:100px\">");
        out.println("<br><br><input type=\"button\" value=\"Home\" onclick=\"location.href='Proshop_announce'\">");
        ...
        if (!toExcel) {
            ...
            out.println("<h2 align=center>Demo " + ((sess_activity_id == 0) ? "Clubs" : "Equipment") + " Member History</h2>");
        ...
        out.println("<tr align=center style=\"background:#336633; color:white; font-size:10pt; font-weight:bold\"><td>Manufacturer</td><td>" + ((sess_activity_id == 0) ? "Club " : "") + "Type</td><td>Demo " + ((sess_activity_id == 0) ? "Club" : "Equipment") + "</td><td>ICN</td><td>Date Out</td><td>Date In</td><td>Days Out</td><td>Notes</td></tr>");
        ...
        try {
            ...
            while (rs.next()) {
                ...
                out.println("<tr align=center><td align=left><font size =1>&nbsp;" + rs.getString("dcm.mfr") + "</font></td><td align=left><font size =1>&nbsp;" + rs.getString("dct.type") + "</font></td>" +
                        ...
                        "<td><font size =1>" + (rs.getInt("days_out") >= 0 ? rs.getInt("days_out") : "N/A") + "</font></td>");
 ...
 private void printClubHistory(HttpServletRequest req, HttpServletResponse resp, int lottery, int sess_activity_id, Connection con, PrintWriter out) {
    ...
    if (req.getParameter("club_id") == null || req.getParameter("club_id").equals("")) {
        ...
        out.println("<table border=\"0\" bgcolor=\"#FFFFFF\">");
        out.println("<td align=\"center\">");
        out.println("<br><h2 align=center>Demo " + ((sess_activity_id == 0) ? "Clubs" : "Equipment") + " History</h2>");
        out.println("</td></tr>");
        out.println("</table>");
        ...
        if (!toExcel) {
            ...
            out.println("<h2 align=center>Demo " + ((sess_activity_id == 0) ? "Clubs" : "Equipment") + " History</h2>");
        ...
        try {
            ...
            //  Gather and display club/equipment information
        ...
        out.println("<tr align=center style=\"background:#336633; color:white; font-size:10pt; font-weight:bold\"><td>Member Name</td><td>Mem #</td><td>Date Out</td><td>Date In</td><td>Days Out</td><td>Notes</td></tr>");


        //
        //  Gather and list all past usage for this club/equipment
        //
        try {
 ...
 private void getCheckOut(HttpServletRequest req, HttpSession session, Connection con, PrintWriter out, int lottery, int sess_activity_id) {
    ...
    */
    ...
    out.println("<form name=\"sortForm\" method=\"get\" action=\"Proshop_demo_clubs\">");
    out.println("<tr><td align=\"center\">");
    ...
    try {
        pstmt = con.prepareStatement("SELECT * FROM demo_clubs_mfr WHERE id > 2 AND (activity_id = ? OR activity_id = -99) AND id IN (SELECT mfr_id FROM demo_clubs WHERE enabled = 1 AND inact = 0 GROUP BY mfr_id) ORDER BY mfr");
        pstmt.clearParameters();
        pstmt.setInt(1, sess_activity_id);
        rs = pstmt.executeQuery();
    ...
    out.println("&nbsp; <a href=\"Proshop_demo_clubs?checkInOut\" style=\"text-decoration:underline;color:black\"><button style=\"width:80px\">Reset</button></a>");
    out.println("</form>");
                    
    out.println("</td></tr>");
    out.println("</table>");
    
    if (count > 0) {       // if any demo clubs found
        ...
        out.println("<form action=Proshop_demo_clubs method=GET name=frmCheckInOut id=frmCheckInOut>");
        ...
        out.println("<tr><td align=\"center\">");
        out.println("<b>Member:</b> (select from Name List)&nbsp;<input type=text name=member value=\"" + member + "\" onchange=\"document.forms['playerform'].member.value=this.value\" onfocus=\"this.blur()\" ondblclick=\"alert('Choose a member using the lists on the right.');\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
        out.println("<br><br><b>Notes:</b>&nbsp;<textarea name=\"notes\" value=\"\" id=\"notes\" cols=\"38\" rows=\"1\"></textarea>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
        ...
        out.println("<tr><td>");
           ...
           try {
               ...
               while (rs.next()) {
                   ...
                   out.println("<tr>");  
                   out.println("<td align=\"right\"><input type=\"checkbox\" name=\"dc" + clubId + "\" id=\"dc" + clubId + "\" value=\"1\"></td>");                         
                   out.println("<td align=\"left\"><font size=\"1\"><label for=\"dc" + clubId + "\">&nbsp;" + clubType + " - " + icn + clubName + symbols + "&nbsp;&nbsp;</label></font></td>"); 
                   out.println("</tr>");
...
*/
           ...
           */
           ...
           out.println("<form action=\"Proshop_demo_clubs\" method=\"get\" name=\"playerform\" id=\"playerform\">");
           ...
           //
           alphaTable.nameList(club, letter, mshipOpt, mtypeOpt, false, null, enableAdvAssist, false, out, con);
        ...
        out.println("<table border=0 cellpadding=5 align=center>");
        out.println("<form name=\"resetForm\" method=\"get\" action=\"Proshop_demo_clubs\">");
 ...
 private void doCheckOut(HttpServletRequest req, HttpSession session, Connection con, PrintWriter out, int lottery, int sess_activity_id) {
    ...
    try {
        ...
        while (rs.next()) {
            ...
            out.println("<tr><!--<td>&nbsp;<a href=\"Proshop_demo_clubs?doCheckOut&doCheckIn="+ rs.getInt("club_id") + "&member=" + member + "&club_id=" + club_id + "\">Return Now</a>&nbsp;</td>-->");
    ...
    out.println("<input type=hidden name=doCheckOut>");
        ...
        out.println("&nbsp;<input type=\"button\" style=\"width:120px\" onclick=\"document.location.href='Proshop_demo_clubs?checkInOut'\" value=\"Back to Checkout\">");
        out.println("</form></center>");
 ...
 private void getCheckIn(HttpServletRequest req, HttpSession session, Connection con, PrintWriter out, int lottery, int sess_activity_id) {
    ...
    if (sendEmails == false) {
        ...
        if (!user.startsWith("proshop") || SystemUtils.verifyProAccess(req, "TOOLS_EMAIL", con, out)) {
         ...
         out.println("<form action=Proshop_demo_clubs method=GET name=frmCheckIn id=frmCheckIn>");
         out.println("<input type=hidden name=doCheckIn>");

         out.println("<tr align=center style=\"background:#336633; color:white; font-size:10pt; font-weight:bold\"><td><td>Demo " + ((sess_activity_id == 0) ? "Club" : "Equipment") + "</td><td>ICN</td><td>Member</td><td>Mem #</td><td>Date Out</td><td>Days</td><td>Notes</td><td>Send Email</td></tr>");
     }
    try {

        pstmt = con.prepareStatement("" +
                "SELECT dcu.*, dc.*, dcm.mfr, CONCAT(m.name_first, ' ', m.name_last) AS memName, m.memNum, m.email, " + 
                "DATE_FORMAT(datetime_out, '%m/%d/%Y %l:%i %p') AS dateout, " + 
                "DATEDIFF(now(), datetime_out) AS days_out " +
                "FROM demo_clubs_usage dcu " +
                "LEFT OUTER JOIN demo_clubs dc ON dc.id = dcu.club_id " +
        ...
        if (sendEmails == false){            
            ...
            out.println("<td align=left><font size=1>&nbsp;" + rs.getString("name") + "</font></td><td><font size=2>" + ((rs.getString("icn") == null) ? "N/A" : rs.getString("icn")) + "</font></td>" +
                    ...
                    "<td><font size=2>" + rs.getString("dateout") + "</font></td><td><font size=2>" + rs.getInt("days_out") + "</font></td>");  
            
            if (!notes.equals("")) {
                
                //
                //  use button tag since we are in the middle of a form
                //
                out.println("<td align=\"center\"><font size=\"2\">");   // notes attached
                out.println("<button type=\"button\" onclick=\"openNotesWindow(" + checkoutId + ");return false;\" title=\"Click here to view notes.\"><img src=\"/" +rev+ "/images/notes.jpg\" border=\"0\">");

                out.println("</button></font></td>");         // end of the notes col
    ...
    if (sendEmails == false) {
         ...
         if (count == 0) {
             out.println("<tr><td style=\"text-align:center;\" colspan=\"9\">No demo " + ((sess_activity_id == 0) ? "clubs" : "equipment") + " currently checked out.");
         }
    ...
    if (sendEmails == true) {     // if request to send emails to all members in the report
         ...
         if (countE > 0) {
             ...
             out.println("<br><p align=center>");
             out.println("<form action=Proshop_demo_clubs method=get><input type=hidden name=getCheckIn value=yes><input type=submit value=\"No, Go Back?\" style=\"width: 90px; background:#8B8970\">");
             out.println("</form></p></font></center>");
 ...
 private void doCheckIn(HttpServletRequest req, HttpSession session, Connection con, PrintWriter out, int lottery, int sess_activity_id) {
    ...
    out.println("<p align=center><BR>Thank you.<BR><BR>" + result_message + "</p>");
 ...
 private void dispNotes(HttpServletRequest req, HttpSession session, Connection con, PrintWriter out, int lottery, int sess_activity_id) {
    ...
    if (req.getParameter("checkoutId") != null) {

        id = Integer.parseInt(req.getParameter("checkoutId"));
    ...
    //
    //  Get the notes for the requested Demo Club Usage entry
    //
    try {
        ...
        SystemUtils.buildDatabaseErrMsg("Error loading up notes.", exc.getMessage(), out, false);
    } finally {
        try {rs.close();} catch (Exception ignore) {}
        try {pstmt.close();} catch (Exception ignore) {}
    }
    ...
    //
    //  display the notes in the window that was opened for this
    //
    out.println(SystemUtils.HeadTitle("Show Notes"));
    out.println("<BODY><CENTER><BR>");
    out.println("<H3>Demo Equipment Notes</H3><BR>");
    out.println("<font size=\"3\">");
        out.println("<b>Notes:</b> &nbsp;&nbsp;" + notes + "<BR><BR>");
    out.println("<form><input type=\"button\" style=\"text-decoration:underline; background:#8B8970; width:80px\" value=\"Close\" onClick='self.close()' alt=\"Close\">");
    out.println("</form>");
 ...
 //
 //  Called by Proshop_sheet to get all members from the tee sheet and check for outstanding demo clubs
 //
 private void teesheetLookUp(HttpServletRequest req, HttpSession session, Connection con, PrintWriter out, int lottery, int sess_activity_id) {
    ...
    //
    //   Start the page display - this resides in a pop-up window
    //
    out.println("<html>");
    out.println("<head>");
    out.println("<title>Demo Clubs Out</title>");
    out.println("</head>");
    ...
    out.println("<tr><td align=\"center\">");
    out.println("<H3>Outstanding Demo Clubs</H3>");
    out.println("<b>Members Playing Today That Have Demo Clubs Checked Out</b><BR><BR>");

    //
    //  Get each member on the tee sheet for the specified date and check them for demo clubs
    //
    try {

        pstmt = con.prepareStatement("SELECT hr, min, courseName, player1, player2, player3, player4, player5, " +
        ...
        while (rs.next()) {
           ...
           userA[0] = rs.getString("username1");
           userA[1] = rs.getString("username2");
           userA[2] = rs.getString("username3");
           userA[3] = rs.getString("username4");
           userA[4] = rs.getString("username5");
        
           //
           //  Check each player for member with demo club out
           //
           for (int i=0; i<5; i++) {
               ...
               if (!userA[i].equals("")) {       // if member
                    ...
                    pstmt2 = con.prepareStatement(
                            "SELECT dcu.*, dc.*, dcm.mfr, dct.type, " +
                            "DATE_FORMAT(datetime_out, '%c/%d/%Y') AS dateout, " +
                            "IF (dcu.datetime_in = '0000-00-00 00:00:00', DATEDIFF(now(), dcu.datetime_out), '-1') AS days_out " +
                            "FROM demo_clubs_usage dcu " +
                            "LEFT OUTER JOIN demo_clubs dc ON dc.id = dcu.club_id " +
                            "LEFT OUTER JOIN demo_clubs_mfr dcm ON dcm.id = dc.mfr_id " +
                            "LEFT OUTER JOIN demo_clubs_types dct ON dct.id = dc.type_id " +
                            "LEFT OUTER JOIN member2b m ON m.username = dcu.username " +
                            "WHERE dcu.username = ? AND dcu.datetime_in = '0000-00-00 00:00:00' AND dc.inact = 0 AND dc.activity_id = 0 " +
                            "ORDER BY datetime_out ASC");
                    pstmt2.clearParameters();
                    pstmt2.setString(1, userA[i]);

                    rs2 = pstmt2.executeQuery();

                    while (rs2.next()) {

                        checkoutId = rs2.getInt("dcu.id");
                        notes = rs2.getString("dcu.notes");
                        
                        if (count == 0) {       // if first one found
                            
                            // Start main output table
                            out.println("<table align=center border=1 cellpadding=\"5\" bgcolor=\"#F5F5DC\">");
                            out.println("<tr align=center style=\"background:#336633; color:white; font-size:10pt; font-weight:bold\">"
                                      + "<td align=left>Player</td><td>Time</td>");
                            if (!course.equals("")) {
                                out.println("<td align=left>Course</td>");
                            }
                            out.println("<td>Manufacturer</td><td>Club Type</td><td>Demo Club</td>"
                                      + "<td>ICN</td><td>Date Out</td><td>Days Out</td><td>Notes</td></tr>");
                        }

                        out.println("<tr align=center><td align=left><font size =2><b>" + playerA[i] + "</b></font></td><td align=center><font size =2>" +hr+ ":" +SystemUtils.ensureDoubleDigit(min)+ "</font></td>");

                        if (!course.equals("")) {
                            out.println("<td align=left><font size =2>" +course+ "</font></td>");
                        }
                        
                        out.println("<td align=left><font size =2>&nbsp;" + rs2.getString("dcm.mfr") + "</font></td><td align=left><font size =2>&nbsp;" + rs2.getString("dct.type") + "</font></td>" +
                                "<td align=left><font size =1>&nbsp;" + rs2.getString("name") + "</font></td><td><font size =2>" + ((rs2.getString("icn")==null) ? "N/A" : rs2.getString("icn")) + "</font></td>" +
                                "<td><font size =2>" + rs2.getString("dateout") + "</font></td>" +
                                "<td><font size =2><b>" + (rs2.getInt("days_out") >= 0 ? rs2.getInt("days_out") : "N/A") + "</b></font></td>");
                        ...
                        count++;            // count number of players with demo clubs out
                        
                    }     // end of WHILE demo clubs out for this member
               }
               
           }    // end of FOR loop - check each player
                
        }       // end of WHILE tee times 

    } catch (Exception exc) {
        SystemUtils.buildDatabaseErrMsg("Error gathering demo club data.", exc.getMessage(), out, false);
    } finally {
        try {rs.close();} catch (Exception ignore) {}
        try {rs2.close();} catch (Exception ignore) {}
        try {pstmt.close();} catch (Exception ignore) {}
    ...
    if (count == 0) {       // if no demo clubs found for these members
            
        out.println("There are no members on this date's tee sheet that have demo clubs checked out.");
    ...
    out.println("<BR><BR><form><input type=\"button\" style=\"text-decoration:underline; background:#8B8970; width:80px\" value=\"Close\" onClick='self.close()' alt=\"Close\">");
    out.println("</form>");
    
    out.println("<BR><form method=\"link\" action=\"javascript:self.print()\">");
    out.println("<button type=\"submit\" style=\"text-decoration:underline; background:#8B8970\">Print List</button>");
   out.println("</form>");

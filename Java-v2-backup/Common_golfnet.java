...
import com.ngn.services._2007._03._20.memberdata.MemberHandicapType;
 ...
 public static String[] postScore(String user, String club, long date, int tee_id, int score, int escScore, String type, int holes, int teecurr_id, PrintWriter out, Connection con) {
    ...
    try {
        ...
        if (rs.next()) {
            ...
            rating_dbl = rs.getDouble("tee_rating") * 10;
            rating = (int)rating_dbl;
  ...
  //scoreInfo.setSourceUserId(hdcp_num);
    ...
    scoreInfo.setRating(rating);
  //scoreInfo.setGross(score); as of 6-30-2015 we are only sending the esc score
...
*/
 ...
 public static void getHandicapsForClub(String club, Connection con, PrintWriter out) {
    ...
    try {
        ...
        for (int i = 0; i < members.size(); i++) {
            ...
            //hdcpInfo.setHandicapType(MemberHandicapType.TREND);

            hdcp = hdcpInfo.getValue();

            if (out != null) out.println("<br>" + friendlyId + ", " + hdcp + ", " + hdcpInfo.getDisplayHandicap() + ", " + hdcpInfo.getHandicapType());
 ...
 private static FetchMembersByClubRequest getFetchMembersByClubRequest(String club, String sourceClubId) {
    ...
    final int days_to_go_back = -15000;

...
class Minify_ClosureCompiler {
    ...
    {
        self::_prepare();
        if (! (($tmpFile = tempnam(self::$tempDir, 'cc_'))  && ($tmpErr = tempnam(self::$tempDir, 'cce_')))  ) {
            throw new Exception('Minify_ClosureCompiler : could not create temp file.');
        }
        file_put_contents($tmpFile, $js);
        exec(self::_getCmd($options, $tmpFile, $tmpErr), $output, $result_code);
        unlink($tmpFile);
        if ($result_code != 0) {
            $error = file_get_contents($tmpErr);
            unlink($tmpErr);
            throw new Exception('Minify_ClosureCompiler : Closure Compiler execution failed: ' . $error);
        }
        unlink($tmpErr);
        return implode("\n", $output);
    }

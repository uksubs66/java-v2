...
public class Proshop_editblock extends HttpServlet {
 ...
 private void updateConf(HttpServletRequest req, PrintWriter out, HttpSession sess) {
   ...
   //
   //  Scan name for special characters and replace with HTML supported chars (i.e. '>' = &gt) - THIS IS NO LONGER USED - scan name for illegal characters instead.
   //
//   name = SystemUtils.filter(name);
//   oldName = SystemUtils.filter(oldName);
   
   // reject names containing special characters that aren't allowed
   if (SystemUtils.scanName(name)) {

      out.println(SystemUtils.HeadTitle("Data Entry Error"));
      out.println("<BODY><CENTER>");
      out.println("<BR><BR><H3>Invalid Parameter Value Specified</H3>");
      out.println("<BR><BR>The name entered for this blocker contains special characters.");
      out.println("<BR>Please remove special characters from the name and try again.");
      out.println("<br><br><font size=\"2\">");
      out.println("<form method=\"get\" action=\"javascript:history.back(1)\">");
      out.println("<input type=\"submit\" value=\"Return\" style=\"text-decoration:underline; background:#8B8970\">");
      out.println("</form></font>");
   ...
   if (id == 0) {
       ...
       out.println("<BR><BR>");
       out.println("<font size=\"2\">");
       out.println("<form method=\"get\" action=\"Proshop_block?new\">");
       out.println("<input type=\"submit\" value=\"Return\" style=\"text-decoration:underline; background:#8B8970\">");
       out.println("</form></font>");
       out.println("</CENTER></BODY></HTML>");
       out.close();
       ...
       out.println("<form action=\"Proshop_editblock\" method=\"post\" target=\"bot\">");
       ...
       out.println("<form method=\"get\" action=\"Proshop_block\">");

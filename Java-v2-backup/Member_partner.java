...
public class Member_partner extends HttpServlet {
  ...
  **/
  ...
  public void doPost2(HttpServletRequest req, HttpServletResponse resp, boolean dining)
      ...
      boolean rwd = true; //Utilities.getRequestBoolean(req, ProcessConstants.RQA_RWD, false);
      ...
      boolean is_mp = false;
      if (sess_activity_id == ProcessConstants.MANAGERS_PORTAL) {
      ...
      String title = (is_mp?"Manage VIPs":"Manage Partners");
      if (new_skin) {

          if (!dining || rwd) {      // these components are built in Dining_home for the dining system for desktop mode
             //
             //  Build the top of the page
             //
             Common_skin.outputHeader(club, sess_activity_id, title, true, out, req);
             ...
             Common_skin.outputBreadCrumb(club, sess_activity_id, out, title, req);
      ...
      if(rwd){
          // Responsive mode partner management
          String list_type = (is_mp?"VIP":"partner");
          out.println("<div class=\"main_instructions pageHelp\" data-fthelptitle=\"Instructions\"><h2 class=\"altTitle\">Instructions:</h2> ");
          if (activity_ids.size() == 1) {
              out.println("To <b>add a new "+list_type+"</b>, click their name in the list to the right. ");
          } else {
              out.println("To <b>add a new "+list_type+"</b>, first use the checkboxes to select "
                      + "the activities for the "+list_type+" list which you would like to update. ");
              out.println("Next, click the member's name in the list to the right. ");
          }
          out.println(""
                  + "To <b>alter the ordering of a "+list_type+" list</b> grab an item by the <span class=\"ui-icon ui-icon-arrowthick-2-n-s\" style=\"display:inline-block\"></span> icon, and drag to the desired location. "
                  + "To <b>return the list to alphabetical order</b> click on the \"Alphabetize List\" button. "
                  + "To <b>remove a member from a "+list_type+" list</b>, click the <span class=\"deleteButtonSmall\" title=\"Remove partner\"><b>Remove</b></span> icon next to "
                  + "their name in the "+list_type+" list you'd like them removed from.");

          if (club.equals("denvercc") && !is_mp) {
              out.println("Names <b>displayed in green</b> indicate that they are Dependents.");
          }
          out.println("</div>");
          
          out.print("<div class=\"ftPlContainer\">");
          out.print("<div class=\"partnerlistcontainer\">");
          displayPartnerListsRwd(req, session, out, con);
          out.print("</div>");
          out.print("</div>");
      } else {
          // Desktop mode partner management
          
          if (is_mp) {

              activity_names.add("VIPs");
              activity_ids.add(ProcessConstants.MANAGERS_PORTAL);
  ...
  private String addPartner(HttpServletRequest req, HttpSession session, PrintWriter out, Connection con) {
      ...
      boolean jsonMode = (req.getParameter("json") != null);
      ...
      User ua = ApiAccess.getUser(req);
      boolean is_mp = sess_activity_id == ProcessConstants.MANAGERS_PORTAL && ua.access.manager_portal;
  ...
  private void displayPartnerListsRwd(HttpServletRequest req, HttpSession session, PrintWriter out, Connection con) {

      PreparedStatement pstmt = null;
      Statement stmt = null;
      ResultSet rs = null;

      //String user_id = Utilities.getSessionString(req, "user", "");
      ...
      boolean golfEnabled = getActivity.isGolfEnabled(con);
      boolean activitiesEnabled = getActivity.isConfigured(con);
      boolean displayHndcpIndex = getHandicapOption(user_id, activity_id, con); 
      int sess_activity_id = reqUtil.getSessionInteger(req,"activity_id",0); 
      User ua = ApiAccess.getUser(req);
      boolean is_mp = sess_activity_id == ProcessConstants.MANAGERS_PORTAL && ua.access.manager_portal;
      ...
      for (int i=0; i<activity_ids.size(); i++) {
          ...
          out.println("<div class=\"ftPl\">");

          activity_id = activity_ids.get(i);
          activity_name = activity_names.get(i);

          // Print activity name
          out.println("<div class=\"ftPlActivity\">" + activity_name + "</div>");
          // Print partnerlist management container
          out.println("<div class=\"ftPlManage ftPlActivityId_"+activity_id+"\" data-ftPlActivityId=\""+activity_id+"\" data-ftPlActivityName=\""+activity_name+"\" data-ftPlTrackHdcp=\""+(activity_id == 0 && handicapsTracked?"yes":"no")+"\" data-ftPlDisplayHndcp=\""+(displayHndcpIndex?"1":"0")+"\"></div>");
  ...
  private void displayHandicapOption(HttpServletRequest req, HttpSession session, String user_id, PrintWriter out, Connection con) {
      ...
      int activity_id = 0;
      boolean displayHndcpIndex = false;
      ...
      //
      displayHndcpIndex = getHandicapOption(user_id, activity_id, con); 
      ...
      if (req.getParameter("updatehndcp") != null) {
         ...
         if (!displayHndcpIndex) {
            
            displayHndcpIndex = true;     // toggle it
            
         } else {
            
            displayHndcpIndex = false;     // toggle it
         }
         ...
         try {
            ...
            pstmt.setInt(1, (displayHndcpIndex?1:0));
      ...
      if(jsonMode){
            out.print("{\"result\":" + (displayHndcpIndex ? "true " : "false") + "}");
      } else {
          ...
          out.println("<span style=\"font: 11px 'Trebuchet MS',Verdana,Arial,Helvetica,sans-serif;\"><BR><input type=\"checkbox\" " + (displayHndcpIndex ? "checked " : "") + "name=\"displayHndcpIndex\" id=\"displayHndcpIndex\" value=\"1\" onChange=\"document.frmHndcpOption.submit()\"><label for=\"displayHndcpIndex\">Show Handicap Indexes in Partner List when booking tee times</label></span>");
          out.println("</form>");
      }
  ...
  private boolean getHandicapOption(String user_id, int activity_id, Connection con) {
      
      if(activity_id != 0){
          return false;
      }
      
      boolean displayHdcp = false;
      ...
      try {
         ...
         if (rs.next()) {
            ...
            displayHdcp = rs.getInt("display_partner_hndcp") == 1;
         }

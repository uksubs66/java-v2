...
public class Member_searchsel extends HttpServlet {
  ...
  **/
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    out.println("<!-- doGet pushNotification=" + pushNotification + " -->");
    ...
    throws IOException
  {
      ...
      try
      {
          SelectionList theNames = MemberHelper.queryMembersWithEmailAddresses(req, con, club, letter, out, isPro);
      ...
      catch (Exception exc) // SQLException sqle
      {
          Utilities.logError("Member_searchsel.searchMembers() ERR: " + exc.toString());
      }
    ...
    throws IOException
  {
    ...
    //boolean pushNotification = reqUtil.getParameterString(req, "push", "0").equals("1");
    boolean pushNotification = req.getParameter("push") != null;
    //out.println("<!-- getUsersByMemberTypeMemberShip: pushNotification=" + pushNotification + " -->");
    ...
    if(pushNotification) {
        ...
        select_stmt = "SELECT username FROM member2b "
                + "LEFT OUTER JOIN v5.mobile_auth t2 ON member2b.id = t2.member_id "
                + "WHERE t2.club_id = " + club_id + " AND ";
    }
    ...
    {
        if(pushNotification) {
          select_stmt += " AND inact = 0 AND billable = 1 GROUP BY username ORDER BY name_last, name_first, name_mi";
        } else {
    ...
    //if(pushNotification) { out.println("<!-- " + select_stmt + " -->"); }

...
public class Proshop_reports extends HttpServlet {
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   if (req.getParameter("unknownRounds") != null) {
       
      unknownRounds(req, resp, sess, out, con);
      return;
   }
   
   if (req.getParameter("noShowRounds") != null) {
      noShowRounds(req, resp, sess, out, con);
      return;
   }
   
   if (req.getParameter("forcastReport") != null) {
       forcastReport(req, resp, sess, out, con);
       return;
   }
   
   if (req.getParameter("EventUtilReport") != null) {
       
      EventUtilReport(req, resp, sess, out, con);
      return;
   }
   
   if (req.getParameter("EventByMember") != null) {
      verifyCustom.gallerygolfemail(con,req);
      EventByMember(req, resp, sess, out, con);
      return;
   }
   
   if (req.getParameter("byUser") != null) {
       String username = req.getParameter("username");
       EventByUser(req, resp, sess, out, con, username);
       return;
   }
   
   
   
//   if (req.getParameter("EventByMember2") != null) {
//
//      EventByMember2(req, resp, sess, out, con);
//      return;
//   }
 ...
 private void goRounds(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession sess, Connection con) {
   ...
   try {
      ...
      getClub.getParms(con, parm, 0, true);  // get the club parms (get golf and ALL guest types)
   }
   ...
   try{
      if (req.getParameter("excel") != null) {     // if user requested Excel Spreadsheet Format
         ...
         resp.setHeader("Content-Disposition", "attachment;filename=\"Proshop_reports.xls\"");
      }
   }
   catch (Exception exc) {
   }
...
*/
 ...
 private void goToday(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, Connection con) {
   ...
   try {
      ...
      error = "Error in Proshop_reports.goToday getting club parms. Error = " +e.getMessage();
...
/*
 ...
 /**
  * Displays all unknown rounds for the selected date range
  ...
  */
 private void unknownRounds(HttpServletRequest req, HttpServletResponse resp, HttpSession sess, PrintWriter out, Connection con) {
     
     PreparedStatement pstmt = null;
     ResultSet rs = null;

     String club = (String) sess.getAttribute("club");    // get club name
     int lottery = Integer.parseInt((String) sess.getAttribute("lottery"));    // get lottery support indicator
     ...
     if (excel) {
         try {              // if user requested Excel Spreadsheet Format
             resp.setContentType("application/vnd.ms-excel");    // response in Excel Format
             resp.setHeader("Content-Disposition", "attachment;filename=\"Unknown Rounds_" + start_date + "_" + end_date + ".xls\"");
         } catch (Exception ignore) {}
     } else {
         out.println(SystemUtils.HeadTitle("Unknown Rounds Report"));
         out.println("<body bgcolor=\"#FFFFFF\" text=\"#000000\">");
     ...
     out.println("<br /><h2 style=\"margin-left:auto; margin-right:auto; text-align:center;\">Unknown Rounds Report</h2>");
     ...
     try {
         ...
         try {
             ...
             getDateSelection(req, club, "unknownRounds", "Proshop_reports", out, con);
             return;
         }
     ...
     out.println("<h3 style=\"margin-left:auto; margin-right:auto; text-align:center;\">" + Utilities.getDateFromYYYYMMDD((int)sdate,2) + " - " + Utilities.getDateFromYYYYMMDD((int)edate,2) + "</h3>");

     if (!excel) {
         out.println("<table align=\"center\" border=\"0\"><tr><td align=\"center\">");
         out.println("<form method=\"get\" action=\"Proshop_announce\">");
         out.println("<input type=\"button\" onclick=\"javascript:history.back()\" value=\"Back\" style=\"background:#8B8970\">");
     ...
     out.println("<tr bgcolor=\"" + bgrndcolor + "\">");
     if (!excel) out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">#</td>");
     ...
     try {
         ...
         pstmt = con.prepareStatement(
                 "(SELECT DATE_FORMAT(DATE, '%c/%e/%Y') AS date, time, day, IF(courseName<>'',courseName,'N/A') AS courseName, IF(fb = '0','Front','Back') AS fb, player1 AS player FROM teepast2 "
                 + "WHERE (player1<>'' AND username1='' AND userg1='' AND show1=1 AND gtype1='') AND date >= ? AND date <= ?) "
                 + "UNION ALL "
                 + "(SELECT DATE_FORMAT(DATE, '%c/%e/%Y') AS date, time, day, IF(courseName<>'',courseName,'N/A') AS courseName, IF(fb = '0','Front','Back') AS fb, player2 AS player FROM teepast2 "
                 + "WHERE (player2<>'' AND username2='' AND userg2='' AND show2=1 AND gtype2='') AND date >= ? AND date <= ?) "
                 + "UNION ALL "
                 + "(SELECT DATE_FORMAT(DATE, '%c/%e/%Y') AS date, time, day, IF(courseName<>'',courseName,'N/A') AS courseName, IF(fb = '0','Front','Back') AS fb, player3 AS player FROM teepast2 "
                 + "WHERE (player3<>'' AND username3='' AND userg3='' AND show3=1 AND gtype3='') AND date >= ? AND date <= ?) "
                 + "UNION ALL "
                 + "(SELECT DATE_FORMAT(DATE, '%c/%e/%Y') AS date, time, day, IF(courseName<>'',courseName,'N/A') AS courseName, IF(fb = '0','Front','Back') AS fb, player4 AS player FROM teepast2 "
                 + "WHERE (player4<>'' AND username4='' AND userg4='' AND show4=1 AND gtype4='') AND date >= ? AND date <= ?) "
                 + "UNION ALL "
                 + "(SELECT DATE_FORMAT(DATE, '%c/%e/%Y') AS date, time, day, IF(courseName<>'',courseName,'N/A') AS courseName, IF(fb = '0','Front','Back') AS fb, player5 AS player FROM teepast2 "
                 + "WHERE (player5<>'' AND username5='' AND userg5='' AND show5=1 AND gtype5='') AND date >= ? AND date <= ?)");
         ...
         while (rs.next()) {
             ...
             out.println("<tr>");
             if (!excel) out.println("<td style=\"margin-left:auto; margin-right:auto; text-align:center;\">" + total_count + "</td>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + rs.getString("date") + "</td>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + Utilities.getSimpleTime(rs.getInt("time")) + "</td>");
             ...
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + rs.getString("fb") + "</td>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + rs.getString("player") + "</td>");
             out.println("</tr>");
         }

     } catch (Exception exc) {
         Utilities.logError("Proshop_reports.unknownRounds - " + club + " - Error looking up unknown rounds data for club - ERR: " + exc.toString());
     } finally {
     ...
     if (total_count > 0) {
         out.println("<h3 style=\"margin-left:auto; margin-right:auto; text-align:center;\">Total Found: " + total_count + "</h3>");
     }
   ...
   try {
      ...
      if (req.getParameter("allmems") != null) {
         ...
         pstmt = con.prepareStatement (
            ...
            "mNum1, mNum2, mNum3, mNum4, mNum5, custom_int " +
            "FROM teepast2 " +
            "WHERE (" +whereClause+ ") " +
            "AND (date >= ? AND date <= ?) " +
            "ORDER BY date, time");
         ...
         while (rs.next()) {
            ...
            //  Build the HTML for each record found
            //
            out.println("<tr style=\"background-color:" + main_bgcolor + ";\">");
            out.println("<td align=\"center\">");
               out.println("<font size=\"2\">");
 ...
 private void getActivityTimes(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, Connection con, String name) {
    ...
    try{
        if (req.getParameter("excel") != null) {     // if user requested Excel Spreadsheet Format
            ...
            toExcel = true;
            resp.setContentType("application/vnd.ms-excel");    // response in Excel Format
            resp.setHeader("Content-Disposition", "attachment;filename=\"Proshop_report.xls\"");
        }
    } catch (Exception ignore) { }
...
*/
            ...
            pstmt = con.prepareStatement(
                    "SELECT a.sheet_id, a.activity_id, a.event_id, a.notes, ap.show, ap.player_name, ap.username, a.related_ids, act.interval, "
                    + "DATE_FORMAT(a.date_time, '%m/%d/%Y') AS pretty_date, "
                    + "DATE_FORMAT(a.date_time, '%l:%i %p') AS pretty_time "
                    + "FROM activity_sheets_players ap "
                    + "LEFT OUTER JOIN activity_sheets a ON a.sheet_id = ap.activity_sheet_id "
                    + "LEFT OUTER JOIN activities act ON a.activity_id = act.activity_id "
            ...
            if (rs.getRow() > 0) {
                ...
                out.println(((allmems) ? "<td><font color=white><b>Player Name</b></font></td>" : "") +
                            ...
                            "<td><font color=white><b>Length</b></font></td>" +
            ...
            while (rs.next()) {
                ...
                String[] tempStr = rs.getString("a.related_ids").split(",");
                length = tempStr.length * rs.getInt("interval");
                ...
                if (allmems) {
                    ...
                    out.println("<td>" + length + " Minutes</td>");
 ...
 /**
  * Used by reports to provide a standardized start date and end date selection page, which posts back to the specified return_location
  * 
  * @param req HTML Request object
  * @param club Club being handled
  * @param return_key Keyword to post as parameter back to the return_location. Could also be used if additional parameters are needed for specific reports. Blank if not needed.
  * @param return_location Servlet to post back to
  * @param out Output stream
  * @param con Connection to club database
  */
 public void getDateSelection(HttpServletRequest req, String club, String return_key, String return_location, PrintWriter out, Connection con) {

    Statement stmt = null;
    ResultSet rs = null;


    String report_title = "Date Range Selection";
    ...
    int oldest_yy = 0;
     ...
     try {
         ...
         SystemUtils.buildDatabaseErrMsg("Proshop_reports.getDateSelection - Error looking up oldest tee time.", e.toString(), out, false);
         return;
     }
    ...
    out.println("<form action=\"" + return_location + "\" method=\"post\">");
    ...
    // Print keyword input for page routing after submission, if provided
    if (!return_key.equals("")) {
        out.println("<input type=\"hidden\" name=\"" + return_key + "\">");
    }
 ...
 private void EventUtilReport(HttpServletRequest req, HttpServletResponse resp, HttpSession sess, PrintWriter out, Connection con) {
     ...
     long edate = 0;            //end date
     long sdate = 0;            //start date
     ...
     if (excel) {
         try {              // if user requested Excel Spreadsheet Format
             resp.setContentType("application/vnd.ms-excel");    // response in Excel Format
             resp.setHeader("Content-Disposition", "attachment;filename=\"EventUtilReport_" + start_date + "_" + end_date + ".xls\"");
         } catch (Exception ignore) {}
     } else {
         out.println(SystemUtils.HeadTitle("Event Util Report"));
         out.println("<body bgcolor=\"#FFFFFF\" text=\"#000000\">");
         SystemUtils.getProshopSubMenu(req, out, lottery);
     }
     ...
     out.println("<br /><h2 style=\"margin-left:auto; margin-right:auto; text-align:center;\">Event Utilization Report</h2>");
     ...
     try {
         ...
         try {
             ...
             // invalid dates here, bailout and call form again
             getDateSelection(req, club, "EventUtilReport", "Proshop_reports", out, con);
             return;
         }
     ...
     out.println("<h3 style=\"margin-left:auto; margin-right:auto; text-align:center;\">" 
             + Utilities.getDateFromYYYYMMDD((int)sdate,2) + " - " + Utilities.getDateFromYYYYMMDD((int)edate,2) + "</h3>");
     ...
     if (!excel) {
         ...
         out.println("<form method=\"post\" action=\"Proshop_reports\" name=frmRequestExcel id=frmRequestExcel target=\"_blank\">");
         out.println("<input type=\"hidden\" name=\"EventUtilReport\" value=\"yes\">");
         ...
         out.println("</form></td><td align=\"center\">");
         out.println("<form method=\"get\" action=\"Proshop_reports\">");
     ...
     out.println("<tr bgcolor=\"" + bgrndcolor + "\">");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Date</td>");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Event Name</td>");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Max Groups</td>");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Groups Registered</td>");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Groups on Waitlist</td>");
     out.println("</tr>");

     try {
         
         pstmt = con.prepareStatement(
                 "select e2b.date, e2b.name, e2b.max, count(if(sup.wait=0,1,null)) as registered, count(if(sup.wait=1,1,null)) as waitlist "
                         +"from evntsup2b as sup, events2b as e2b"
                 + " Where sup.player1 != '' AND sup.event_id = e2b.event_id AND e2b.date = sup.c_date AND e2b.date >= ? AND e2b.date <= ? "
                         +"AND e2b.activity_id = ? group by e2b.event_id");
         ...
         while (rs.next()) {
         date = rs.getInt("e2b.date");
         event_name = rs.getString("e2b.name");
         if (event_name.startsWith("*** ")) {   //if hidden event
             event_name = event_name.substring(4);
         }
             out.println("<tr>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + Utilities.getDateFromYYYYMMDD(date,2) + "</td>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + event_name + "</td>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + rs.getString("e2b.max") + "</td>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + rs.getString("registered") + "</td>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + rs.getString("waitlist") + "</td>");
             out.println("</tr>");
         }

     } catch (Exception exc) {
         Utilities.logError("Proshop_reports.EventUtilReport - " + club + " - Error looking up unknown rounds data for club - ERR: " + exc.toString());
     } finally {
         Connect.close(rs, pstmt);
     }
 ...
 private void EventByMember(HttpServletRequest req, HttpServletResponse resp, HttpSession sess, PrintWriter out, Connection con) { 
     ...
     if (excel) {
         try {              // if user requested Excel Spreadsheet Format
             resp.setContentType("application/vnd.ms-excel");    // response in Excel Format
             resp.setHeader("Content-Disposition", "attachment;filename=\"EventByMember_" + start_date + "_" + end_date + ".xls\"");
         } catch (Exception ignore) {}
     } else {
         out.println(SystemUtils.HeadTitle("Event Signups by Member Report"));
         out.println("<body bgcolor=\"#FFFFFF\" text=\"#000000\">");
         SystemUtils.getProshopSubMenu(req, out, lottery);
     }
     ...
     out.println("<br /><h2 style=\"margin-left:auto; margin-right:auto; text-align:center;\">Event Signups by Member Report</h2>");
     ...
     try {
         ...
         try {
             ...
             // invalid dates here, bailout and call form again
             getDateSelection(req, club, "EventByMember", "Proshop_reports", out, con);
             return;
         }
     ...
     if (!excel) {
         ...
         out.println("</form></td><td align=\"center\">");
         out.println("<form method=\"post\" action=\"Proshop_reports\" name=frmRequestExcel id=frmRequestExcel target=\"_blank\">");
         out.println("<input type=\"hidden\" name=\"EventByMember\" value=\"yes\">");
         out.println("<input type=\"hidden\" name=\"cal_box_0\" value="+start_date+" >");
         out.println("<input type=\"hidden\" name=\"cal_box_1\" value="+end_date+" >"); 
         out.println("<input type=\"hidden\" name=\"excel\" value=\"yes\">");
         out.println("<input type=\"submit\" value=\"Excel\" style=\"text-decoration:underline; background:#8B8970\">");
     ...
     out.println("<tr bgcolor=\"" + bgrndcolor + "\">");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Member Name</td>");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Membership</td>");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Member Number</td>");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Events Registered</td>");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Events on Wait List</td>");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Inactive</td>");
     out.println("</tr>");

     
     try {
         
         pstmt = con.prepareStatement( "Select mem.name_first, mem.name_mi, mem.name_last, mem.m_ship, mem.memNum, mem.username, "
                + "count(if(sup.wait=0,1,null)) as registered, count(if(sup.wait=1,1,null)) as waitlist , mem.inact from member2b as mem "
                + "LEFT JOIN evntsup2b as sup ON mem.username = sup.username1 or mem.username = sup.username2 or mem.username = sup.username3 "
                + "or mem.username = sup.username4 or mem.username = sup.username5 "
                + "LEFT JOIN events2b as e2b  ON sup.event_id = e2b.event_id WHERE sup.player1 != '' AND sup.c_date > ? AND sup.c_date < ? "
                + "AND e2b.activity_id = ? GROUP BY mem.username ORDER BY mem.name_last");
         ...
         while (rs.next()) {
             last = rs.getString("mem.name_last");
             first = rs.getString("mem.name_first");
             mi = rs.getString("mem.name_mi");
             username = rs.getString("mem.username");
             mNum = rs.getString("mem.memNum");
             name = last + ", " + first;
             inactive = rs.getString("mem.inact");
             registered = rs.getString("registered");
             waitlist = rs.getString("waitlist");
             out.println("<tr>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">"
                     + "<a href=\"javascript:void(0)\" onClick=\"window.open ('Proshop_reports?byUser=yes&username=" + username 
                     + "&sdate=" + sdate + "&edate=" + edate + "&first=" + first + "&last=" + last + "&registered="+registered
                     +"&waitlist="+waitlist+"&sAct="+sess_activity_id+"&mNum="+mNum+"&mi="+mi
                     +"', 'newwindow', 'Height=600, width=600, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no directories=no, status=no')\">" 
                     + name + "</a></td>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + rs.getString("mem.m_ship") + "</td>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + mNum + "</td>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + registered + "</td>");
             out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + waitlist + "</td>");
             if (inactive.equals("0")) {
                out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + "Active" + "</td>");
             } else {
                out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + "Inactive" + "</td>");             
             }
         ...
         Utilities.logError("Proshop_reports.EventByMember - " + club + " - Error looking up events by member data for club - ERR: " + exc.toString());
     } finally {
         Connect.close(rs, pstmt);
     }
 ...
 private void EventByUser(HttpServletRequest req, HttpServletResponse resp, HttpSession sess, PrintWriter out, Connection con, String user) {
     ...
     if (req.getParameter("last") != null) {
         lName = req.getParameter("last");
     }     
     if (req.getParameter("first") != null) {
         fName = req.getParameter("first");
     }
     if (req.getParameter("mi") != null) {
         mid = req.getParameter("mi");
     }
     if (req.getParameter("registered") != null) {
         reg = req.getParameter("registered");
     }
     if (req.getParameter("waitlist") != null) {
         wait = req.getParameter("waitlist");
     }
     if (req.getParameter("sAct") != null) {
         sess_activity_id = req.getParameter("sAct");
     }
     if (req.getParameter("mNum") != null) {
         memNum = req.getParameter("mNum");
     }
     
     if (mid.equals("")) {
         full = fName+" "+lName;
     } else {
         full = fName+" "+mid+" "+lName;
     }
     
     out.println("<p align=center><font size=5>Individual Member Report</font>");
     out.println("<br><br><font size=4><i><br>"+Utilities.getDateFromYYYYMMDD((int)sdate,2)+" - "+Utilities.getDateFromYYYYMMDD((int)edate,2)+"</i></font></p><br>");
//
//     out.println("<br><br><font size=4><i><br>Current Handicap Index is: check123</i></font></p><br>");
     ...
     out.println("<tr style=\"font-weight:bold;background-color:#336633;color:white;\">" + // #CCCCAA
             ...
             + "<td align=center>Wait list</td>"
             + "</tr>"
             + "<tr bgcolor=\"#F5F5DC\">"
             + "<td align=center>"+full+"</td>"
             + "<td align=center>"+memNum+"</td>"
     ...
     out.println("<tr style=\"font-weight:bold;background-color:#336633;color:white;\">" + // #CCCCAA
         ...
         pstmt = con.prepareStatement( "Select ev.name, ev.date, sup.wait from evntsup2b "
                 +"as sup LEFT JOIN events2b as ev ON sup.event_id = ev.event_id "
                 +"where (sup.username1 = ? or sup.username2 = ? or sup.username3 = ? or sup.username4 = ? or sup.username5 = ?) "
                 +"AND ev.date >= ? AND ev.date <= ? AND ev.activity_id = ? order by ev.date;");
         ...
         pstmt.setLong(6, sdate);
         pstmt.setLong(7, edate);
         pstmt.setString(8, sess_activity_id);
         ...
         while (rs.next()) {
             ...
             event_date = rs.getLong("ev.date");
             waitlist = rs.getString("sup.wait");
             out.println("<tr bgcolor=\"#F5F5DC\">"
             +"<td align=center>" + rs.getString("ev.name") + "</td>"
             +"<td align=center>" + Utilities.getDateFromYYYYMMDD((int)event_date,2) + "</td>");
             if (waitlist.equals("1")) {
                out.println("<td align=center>Wait list</td>");
             } else {
                out.println("<td align=center>Registered</td>");             
             }
         ...
         Utilities.logError("Proshop_reports.EventByUser - Error looking up events by user data for club - ERR: " + ex.toString());
     } finally {
         Connect.close(rs, pstmt);
     }
          out.println("</table>");
 ...
 /**
  * Displays all No Show rounds for the selected date range
  * @param req HTTP Request object
  * @param resp HTTP Response object
  * @param sess HTTP Session object
  * @param out Output stream
  * @param con Connection to club database
  */
 private void noShowRounds(HttpServletRequest req, HttpServletResponse resp, HttpSession sess, PrintWriter out, Connection con) {
     ...
     if (excel) {
         try {              // if user requested Excel Spreadsheet Format
             resp.setContentType("application/vnd.ms-excel");    // response in Excel Format
             resp.setHeader("Content-Disposition", "attachment;filename=\"No Show Rounds_" 
                     + start_date + "_" + end_date + ".xls\"");
         } catch (Exception ignore) {}
     } else {
         out.println(SystemUtils.HeadTitle("No Show Rounds Report"));
         out.println("<body bgcolor=\"#FFFFFF\" text=\"#000000\">");
         SystemUtils.getProshopSubMenu(req, out, lottery);
     }
     ...
     if (!excel) {
         out.println("<br /><h2 style=\"margin-left:auto; margin-right:auto; text-align:center;\">No Show Rounds Report</h2>");
         out.println("<tr><center>");
         out.println("<td align=\"center\">Please be patient. This report may take longer than expected to run. Only click once.</td>");
         out.println("</center><br></tr>");
     }
     ...
     try {
         ...
         try {
             ...
             // invalid dates here, bailout and call form again
             getDateSelection(req, club, "noShowRounds", "Proshop_reports", out, con);
             return;
         }
     ...
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Member Number</td>");
     out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">No Shows</td>");
     out.println("</tr>");

     

    sql = "SELECT count(*) as Count FROM teepast2 " +
          "WHERE ((username1 = ? OR username2 = ? OR username3 = ? OR username4 = ? OR username5 = ?) " +
          "AND (show1 <> 1 OR show2 <> 1 OR show3 <> 1 OR show4 <> 1 OR show5 <> 1) AND (date >= ? AND date <= ?)) ";

 
     try {

         stmt = con.createStatement();
         rs2 = stmt.executeQuery("SELECT username, memNum, name_first, name_last, name_mi from member2b order by name_last");
         while (rs2.next()) {
             
             username = rs2.getString("username");
             memberNumber = rs2.getString("memNum");
             nameLast = rs2.getString("name_last");
             nameFirst = rs2.getString("name_first");
             nameMi = rs2.getString("name_mi");
             
             if (nameMi == null || nameMi.equals("")) {
                 nameFull = nameLast + ", " + nameFirst;
             } else {
                 nameFull = nameLast + ", " + nameFirst + " " + nameMi;
             }
             ...
             try {
                 ...
                 while (rs.next()) {
                     ...
                     out.println("<tr>");
                     out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + nameFull + "</td>");
                     out.println("<td style=\"margin-left:auto; margin-right:auto;\"><center>" + memberNumber + "</center></td>");
                     out.println("<td style=\"margin-left:auto; margin-right:auto;\"><center>" + rs.getInt("Count") + "</center></td>");
                     out.println("</tr>");
                 }

             } catch (Exception exc) {
                 Utilities.logError("Proshop_reports.noShowRounds - " 
                         + club + " - Error looking up No Show rounds data for club - ERR: " + exc.toString());
             } finally {
                 Connect.close(rs, pstmt);
             }
         }
         ...
         Utilities.logError("Proshop_reports.noShowRounds - " + club + " - Error looking up username & name for club - ERR: " + exc.toString());
     } finally {
         Connect.close(rs2, stmt);
     }
 ...
 /**
  * Displays all forecast rounds report for the selected date range
  * @param req HTTP Request object
  * @param resp HTTP Response object
  * @param sess HTTP Session object
  * @param out Output stream
  * @param con Connection to club database
  */
 private void forcastReport(HttpServletRequest req, HttpServletResponse resp, HttpSession sess, PrintWriter out, Connection con) {
    ...
    //Holds the guest types for the club
    ArrayList<String> guestType = new ArrayList<String>();
    //Holds the membership types/guest types and their MOT
    Map<String, String> sortedMap = new TreeMap<String, String>();
    //Holds the membership types/guest types and the counts associated with their MOT
    HashMap<String, String> motCount = new HashMap<String, String>();
     
    String sql = "";           //String for sql querries 
    String nameHolder = "";
    String unique = "";
    String lastPlayerType = "";
    ...
    if (excel) {
        try {              // if user requested Excel Spreadsheet Format
            resp.setContentType("application/vnd.ms-excel");    // response in Excel Format
            resp.setHeader("Content-Disposition", "attachment;filename=\"Forcasting_Report_" 
                    + start_date + "_" + end_date + ".xls\"");
        } catch (Exception ignore) {}
    } else {
        out.println(SystemUtils.HeadTitle("Forcasting Report"));
        out.println("<body bgcolor=\"#FFFFFF\" text=\"#000000\">");
        SystemUtils.getProshopSubMenu(req, out, lottery);        
    }
    ...
    if (!excel) {
        out.println("<br /><h2 style=\"margin-left:auto; margin-right:auto; text-align:center;\">Forcasting Report</h2>");
        out.println("<tr>");
        out.println("<br></tr>");
    }
    ...
    try {
        ...
        try {
            ...
            // invalid dates here, bailout and call form again
            getDateSelection(req, club, "forcastReport", "Proshop_reports", out, con);
            return;
        }
    ...
    out.println("<tr bgcolor=\"" + bgrndcolor + "\">");
    out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Player Type</td>");
    out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">MOT</td>");
    out.println("<td align=\"center\" style=\"color:" + fontcolor + ";\">Rounds</td>");
    out.println("</tr>");
    
            //|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\\                              
           ///Search past and present tee time for membership type MOT counts\\\
          ////|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\\\\
    sql = "SELECT * FROM (SELECT m_ship, p1cw AS pcw, count(*) AS total "
            +"FROM teecurr2 t LEFT JOIN member2b m ON t.username1 = m.username "
            +"WHERE player1<>'' AND username1<>'' AND date >= ? AND date <= ? GROUP BY m_ship, p1cw\n"
        + "UNION ALL \n"
        + "SELECT m_ship, p2cw AS pcw, count(*) AS total "
            +"FROM teecurr2 t LEFT JOIN member2b m ON t.username2 = m.username "+
            "WHERE player2<>'' AND username2<>'' AND date >= ? AND date <= ? GROUP BY m_ship, p2cw\n"
        + "UNION ALL \n"            
        + "SELECT m_ship, p3cw AS pcw, count(*) AS total "
            +"FROM teecurr2 t LEFT JOIN member2b m ON t.username3 = m.username "
            +"WHERE player3<>'' AND username3<>'' AND date >= ? AND date <= ? GROUP BY m_ship, p3cw\n"
        + "UNION ALL \n"
        + "SELECT m_ship, p4cw AS pcw, count(*) AS total "
            +"FROM teecurr2 t LEFT JOIN member2b m ON t.username4 = m.username "
            +"WHERE player4<>'' AND username4<>'' AND date >= ? AND date <= ? GROUP BY m_ship, p4cw\n"
        + "UNION ALL\n"
        + "SELECT m_ship, p5cw AS pcw, count(*) AS total "
            +"FROM teecurr2 t LEFT JOIN member2b m ON t.username5 = m.username "
            +"WHERE player5<>'' AND username5<>'' AND date >= ? AND date <= ? GROUP BY m_ship, p5cw\n"
        + "UNION ALL\n"
        + "SELECT mship1 AS mship, p1cw AS pcw, count(*) AS total FROM teepast2 t "
            +"WHERE player1<>'' AND username1<>'' AND date >= ? AND date <= ? GROUP BY mship1, p1cw\n"
        + "UNION ALL\n"
        + "SELECT mship1 AS mship, p2cw AS pcw, count(*) AS total FROM teepast2 t "
            +"WHERE player1<>'' AND username2<>'' AND date >= ? AND date <= ? GROUP BY mship2, p2cw\n"
        + "UNION ALL\n"
        + "SELECT mship1 AS mship, p3cw AS pcw, count(*) AS total FROM teepast2 t "
            +"WHERE player1<>'' AND username3<>'' AND date >= ? AND date <= ? GROUP BY mship3, p3cw\n"
        + "UNION ALL\n"
        + "SELECT mship1 AS mship, p4cw AS pcw, count(*) AS total FROM teepast2 t "
            +"WHERE player1<>'' AND username4<>'' AND date >= ? AND date <= ? GROUP BY mship4, p4cw\n"
        + "UNION ALL\n"
        + "SELECT mship1 AS mship, p5cw AS pcw, count(*) AS total FROM teepast2 t "
            +"WHERE player1<>'' AND username5<>'' AND date >= ? AND date <= ? GROUP BY mship5, p5cw) AS r1\n"
        + "GROUP BY m_ship, pcw;";
    ...
    try {
        ...
        pstmt.setLong(9, sdate);
        pstmt.setLong(10, edate);
        pstmt.setLong(11, sdate);
        pstmt.setLong(12, edate);
        pstmt.setLong(13, sdate);
        pstmt.setLong(14, edate);
        pstmt.setLong(15, sdate);
        pstmt.setLong(16, edate);
        pstmt.setLong(17, sdate);
        pstmt.setLong(18, edate);
        pstmt.setLong(19, sdate);
        pstmt.setLong(20, edate);
        rs = pstmt.executeQuery();
        unique = "@";
        while (rs.next()) {

            nameHolder = rs.getString("m_ship");

            if (nameHolder.equals("")) {
                nameHolder = "Unknown";
            }

            sortedMap.put(nameHolder+unique, rs.getString("pcw"));          
            motCount.put(nameHolder+unique, rs.getString("total"));

            unique = unique + "@";     //string to concatinate with keys so each key is unique

        }

    } catch (Exception exc) {
        Utilities.logError("Proshop_reports.forcastReport - " + club 
                + " - Error running Forcast Report data for club - ERR: " + exc.toString());
    } finally {
        Connect.close(rs, pstmt);
    }
                          
            //||||||||||||||||||||||||||||||||||||||||||\\                              
           ///Search for all the guest types at the club\\\
          ////||||||||||||||||||||||||||||||||||||||||||\\\\
    try {
        pstmt = con.prepareStatement("select guest from guest5 where activity_id = 0 ORDER BY guest");
        pstmt.clearParameters();
        rs = pstmt.executeQuery();
        while (rs.next()) {
            guestType.add(rs.getString("guest"));
        }   
        ...
        Connect.close(rs, pstmt);
            ...
            //||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\\                              
           ///Search past and present tee time for guest type MOT counts\\\
          ////||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\\\\
    sql = "SELECT * FROM (SELECT p1cw AS pcw, count(*) AS total FROM teecurr2 "
            +"WHERE player1 REGEXP ? AND date >= ? AND date <= ? GROUP BY p1cw\n"
             + "UNION ALL \n"
             + "SELECT p2cw AS pcw, count(*) AS total FROM teecurr2 "
             + "WHERE player2 REGEXP ? AND date >= ? AND date <= ? GROUP BY p2cw\n"
             + "UNION ALL \n"
             + "SELECT p3cw AS pcw, count(*) AS total FROM teecurr2 "
             + "WHERE player3 REGEXP ? AND date >= ? AND date <= ? GROUP BY p3cw\n"
             + "UNION ALL \n"
             + "SELECT p4cw AS pcw, count(*) AS total FROM teecurr2 " 
             + "WHERE player4 REGEXP ? AND date >= ? AND date <= ? GROUP BY p4cw\n"
             + "UNION ALL\n"
             + "SELECT p5cw AS pcw, count(*) AS total FROM teecurr2 "
             + "WHERE player5 REGEXP ? AND date >= ? AND date <= ? GROUP BY p5cw\n"
             + "UNION ALL\n"
             + "SELECT p1cw AS pcw, count(*) AS total FROM teepast2 "
             + "WHERE player1 REGEXP ? AND date >= ? AND date <= ? GROUP BY p1cw\n"
             + "UNION ALL \n"
             + "SELECT p2cw AS pcw, count(*) AS total FROM teepast2 "
             + "WHERE player2 REGEXP ? AND date >= ? AND date <= ? GROUP BY p2cw\n"
             + "UNION ALL \n"
             + "SELECT p3cw AS pcw, count(*) AS total FROM teepast2 "
             + "WHERE player3 REGEXP ? AND date >= ? AND date <= ? GROUP BY p3cw\n"
             + "UNION ALL \n"
             + "SELECT p4cw AS pcw, count(*) AS total FROM teepast2 "
             + "WHERE player4 REGEXP ? AND date >= ? AND date <= ? GROUP BY p4cw\n"
             + "UNION ALL\n"
             + "SELECT p5cw AS pcw, count(*) AS total FROM teepast2 "
             + "WHERE player5 REGEXP ? AND date >= ? AND date <= ? GROUP BY p5cw) AS r1\n"
             + "GROUP BY pcw;";
    
    for (String guesttype : guestType) {

        try {
            pstmt = con.prepareStatement(sql);
            pstmt.clearParameters();
            pstmt.setString(1, "^" + guesttype);
            pstmt.setLong(2, sdate);
            pstmt.setLong(3, edate);
            pstmt.setString(4, "^" + guesttype);
            pstmt.setLong(5, sdate);
            pstmt.setLong(6, edate);
            pstmt.setString(7, "^" + guesttype);
            pstmt.setLong(8, sdate);
            pstmt.setLong(9, edate);
            pstmt.setString(10, "^" + guesttype);
            pstmt.setLong(11, sdate);
            pstmt.setLong(12, edate);
            pstmt.setString(13, "^" + guesttype);
            pstmt.setLong(14, sdate);
            pstmt.setLong(15, edate);
            pstmt.setString(16, "^" + guesttype);
            pstmt.setLong(17, sdate);
            pstmt.setLong(18, edate);
            pstmt.setString(19, "^" + guesttype);
            pstmt.setLong(20, sdate);
            pstmt.setLong(21, edate);
            pstmt.setString(22, "^" + guesttype);
            pstmt.setLong(23, sdate);
            pstmt.setLong(24, edate);
            pstmt.setString(25, "^" + guesttype);
            pstmt.setLong(26, sdate);
            pstmt.setLong(27, edate);
            pstmt.setString(28, "^" + guesttype);
            pstmt.setLong(29, sdate);
            pstmt.setLong(30, edate);
            Utilities.logDebug("WDH",""+pstmt);
            ...
            while (rs.next()) {
                ...
                sortedMap.put(guesttype + unique, rs.getString("pcw"));
                motCount.put(guesttype + unique, rs.getString("total"));
            ...
            //|||||||||||||||||||||||||||||||||||||||||||\\                              
           ///Loop through sorted map and output into row\\\
          ////|||||||||||||||||||||||||||||||||||||||||||\\\\
    for (Map.Entry<String,String> sortedType : sortedMap.entrySet()) {

        String playerType = sortedType.getKey();
        String MOT = sortedType.getValue();
        String countMOT = motCount.get(sortedType.getKey());

        while (playerType.endsWith("@")) {                    
            playerType = playerType.substring(0, playerType.length()-1);                                 
        }
        if (playerType.equals(lastPlayerType)) {
            playerType = "-";
        }
        out.println("<tr>");
        out.println("<td style=\"margin-left:auto; margin-right:auto;\">" + playerType + "</td>");
        out.println("<td style=\"margin-left:auto; margin-right:auto;\"><center>" + MOT + "</center></td>");
        out.println("<td style=\"margin-left:auto; margin-right:auto;\"><center>" + countMOT + "</center></td>");
        out.println("</tr>");
        lastPlayerType = (playerType.equals("-")) ? lastPlayerType : playerType;
    }

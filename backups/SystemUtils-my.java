 ...
 *    6/23/06   TEMP - noop call to doBlockers so the blockers are not rebuilt each night.  Manual 'unblocks'
 *                  via edit tee sheet are getting overwritten.  Must find permanent fix. (REMOVED) ****
...
public class SystemUtils {
   ...
   static long july3rd1 = ProcessConstants.july4;        // Monday 7/03 
         ...
         if (!player1.equals("") || !player2.equals("") || !player3.equals("") || !player4.equals("") || !player5.equals("")) {
            ...
            try {
               ...
               pstmt = con.prepareStatement (
                  ...
                  "hotelMod, orig_by, conf, notes, p91, p92, p93, p94, p95, pace_status_id, teecurr_id) " +
                  "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, " +
                  "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
               ...
               pstmt.setInt(57, p94);
               pstmt.setInt(58, p95);
               pstmt.setInt(59, pace_status_id);
               pstmt.setInt(60, teecurr_id);
...
*/
         ...
         if (xx != 0) {                                    // if course ready for tee sheets
            ...
            while (i < 366) {        // go 1 extra for today
               ...
               if (skip == 0) {             // temp - try checking all days !!!!! 6/14/2006
...
//      logError(errorMsg);                                       // log it
   ...
   if (con != null) {
      ...
      //
      // *************************************************************************************************
      //  Custom processing for North Ridge, Rogue Valley, Manito, The Lakes, Cherry Hills, Skaneateles
      // *************************************************************************************************
      //
                  ...
                  if (astat == 0 || bstat == 0) {   // if before or after still open to check
                     ...
                     if (error > 0) {                     // if error - try another time slot
                        ...
                        if (astat == 0 || bstat == 0) {   // if before or after still open to check
                           ...
                           //
                           if (parm.club.equals( "cherryxxxx" )) {    // just do them for now
      ...
      if (server_id == server_master) {
         ...
         while (rs.next()) {               // process each club
            ...
            if (con2 != null) {
               ...
               errorMsg = "Error in SystemUtils xTimer - unable to get con for club: " +club;
               logError(errorMsg);                                       // log it
            }
...
*/
 ...
 //************************************************************************
 // 
 // Display a standard database error message for ProShop side
 //
 //************************************************************************

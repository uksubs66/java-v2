...
public class Proshop_sheet extends HttpServlet {
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   //
   if (club.equals( "interlachen" ) && req.getParameter("caddynum") != null) {
   ...
   //
   //   if Pecan Plantation and call was to update the 'teed off' indicator for the group - go process
   //
   if (club.equals( "pecanplantation" ) && req.getParameter("teedOff") != null) {
   ...
   //
   if (club.equals( "medinahcc" ) || club.equals( "bellerive2006" )) {
   ...
   try {
      ...
      if (req.getParameter("print") == null) {
         ...
         //
         //  Custom for Cherry Hills - refresh the tee sheet every 9 minutes for proshop4 (bag room) between 7 AM and 7 PM.
         //
        ...
        if (fives == 0) {
            ...
            // query to count the total # of tee time player positions available
            sqlQuery2 = "SELECT SUM(subtotal) AS total FROM (" +
                ...
                ") AS gtotal;";
            // query to count the # of completly full tee times (all player positions are occupied)
            sqlQuery3 = "SELECT COUNT(date) AS total FROM teecurr2 WHERE " + courseClause + " date = ? AND blocker = '' AND restriction = '' AND player1 <> '' AND player2 <> '' AND player3 <> '' AND player4 <> '';";
        } else {
            // query  to count the total # of tee times available
            sqlQuery1 =  "SELECT COUNT(date) AS total FROM teecurr2 WHERE " + courseClause + " date = ? AND blocker = \"\" AND restriction = \"\" AND rest5 <> \"\";";
            // query to count the total # of tee time player positions available
           sqlQuery2 = "SELECT SUM(subtotal) AS total FROM (" +
                "SELECT COUNT(date) AS subtotal FROM teecurr2 WHERE " + courseClause + " date = ? AND player1 <> '' " +
                "UNION ALL " +
                ...
                ") AS gtotal;";
            // query to count the # of completly full tee times (all player positions are occupied)
            sqlQuery3 = "SELECT COUNT(date) AS total FROM teecurr2 WHERE " + courseClause + " date = ? AND blocker = '' AND restriction = '' AND player1 <> '' AND player2 <> '' AND player3 <> '' AND player4 <> '' AND (rest5 <> '' OR player5 <> '');";
        ...
        // 2nd query
        tmp_error = "SQL - Booked Tee Times";
        ...
        rs = pstmt1.executeQuery();
        tmp_error = "SQL - Booked Tee Times - Query ran";
        if (rs.next()) used_teetimes = rs.getInt("total");
        ...
        out.println("<tr><td align=\"center\" valign=\"top\">");
            ...
            //
            // if Abacus21, Jonas or Northstar, and today and not course=ALL
            //
         ...
         out.println("</form>");
            ...
            if (club.equals( "medinahcc" )) {
                out.println("<br>Member Times are indicated by the green time button.");
            }
            out.println("</font></td></tr></table>");
            ...
            //
            /*
            out.println("<table border=\"1\" cellspacing=\"3\" cellpadding=\"3\" bgcolor=\"8B8970\" align=\"center\">");
            out.println("<tr><td align=\"center\"><font color=\"#000000\" size=\"3\"><b>Tee Sheet Summary</b><br></font></td></tr>");
            
            out.println("<tr><td nowrap><font size=\"2\" color=\"#FFFFFF\">");
            out.println("Available Player Slots: " + available_slots);
            out.println("</font></td></tr>");
            
            out.println("<tr><td nowrap><font size=\"2\" color=\"#FFFFFF\">");
            out.println("Occupied Tee Times: " + used_teetimes);
            if (used_teetimes != 0 && available_slots != 0) {
                out.println(" &nbsp;(");
                if (used_teetimes / available_slots < 1) {
                    out.println("<1%");
                } else {
                    out.println(nf.format(used_teetimes / available_slots));
                }
                out.println(")");
            }
            out.println("</font></td></tr>");
            ...
            out.println("<tr><td nowrap><font size=\"2\" color=\"#FFFFFF\">");
            out.println("Available Tee Times: " + available_teetimes);
            out.println("</font></td></tr>");

            
            out.println("<tr><td nowrap><font size=\"2\" color=\"#FFFFFF\">");
            out.println("Full Tee Times: " + full_teetimes);
            out.println("</font></td></tr>");
            ...
            */
            ...
            /*
            out.println("<tr><td align=\"center\"><font color=\"#000000\" size=\"3\"><b>Tee Sheet Legend</b><br></font></td></tr>");

        //out.println("<tr><td align=\"center\"><font size=\"2\">Available Tee Times: </font></td></tr>");
            
            if (!event1.equals( "" )) {

               out.println("<tr><td align=\"center\"><font size=\"2\" color=\"#FFFFFF\">");
         ...
         out.println("<b>F/B:</b>&nbsp; &nbsp;<nobr>F = Front Nine,</nobr> &nbsp;&nbsp; <nobr>B = Back Nine,</nobr> &nbsp;&nbsp; <nobr>O = Open (for cross-overs)</nobr>,&nbsp;&nbsp; <nobr>S = Shotgun Event</nobr><hr>");
         ...
         for (int ic=0; ic<16; ic++) {
            ...
            if (!parmc.tmodea[ic].equals( "" )) {
               out.println("<nobr>" + parmc.tmodea[ic]+ " = " +parmc.tmode[ic]+ "</nobr>&nbsp; &nbsp;");
            }
         }
         out.println("<br>(9 = 9 holes)<hr><b>X:</b> Check In/Out Players");
         out.println("<hr><b>N:</b> Notes Attached to Reservation (click to view)");
           
         if (club.equals( "interlachen" )) {                  // if Interlachen

            out.println("<hr><b>CA:</b> # of Caddies Assigned to Group");
         }
         if (club.equals( "cordillera" )) {                  // if Cordillera 

            out.println("<hr><b>FC:</b> Forecaddie Requested");
         }
         if (club.equals( "pecanplantation" ) && index == 0) {        // if Pecan Plantation and today

            out.println("<hr><b>TO:</b> Teed Off");
         }
         if (club.equals( "medinahcc" )) {                  // if Medinah 

            out.println("<hr><b>P:</b> Print Tee Time");
         }
         out.println("</font></td></tr>");
        ...
        out.println("</td></tr></table>"); // end upper portion of tee sheet
        ...
        out.println("<font size=\"3\"><p><b><u>Tee Sheet Summary</b></u></font><br><font size=\"2\">");
        out.println("Available Tee Times Slots: " + available_teetimes);
        out.println(" &nbsp; &nbsp; &nbsp; &nbsp; Sold Tee Time Slots: " + used_teetimes);
        if (used_teetimes != 0 && available_teetimes != 0) {
            out.println(" &nbsp; &nbsp; &nbsp; &nbsp; Percent Sold:");
            if (used_teetimes / available_teetimes < 1) {
                out.println("<1%");
            } else {
                out.println(nf.format(used_teetimes / available_teetimes));
            }
        }
        out.println("</p></font>");
        ...
        //
         if (!event1.equals( "" ) || !rest1.equals( "" ) || !lott1.equals( "" )) {
            ...
            // legend title
            out.println("<br><b>Event / Restriction / Lottery  Legend</b> (click on buttons to view info)<br>");
         ...
         while (rs.next()) {
            ...
            p94 = rs.getInt("p94");
            p95 = rs.getInt("p95");
            if (club.equals( "interlachen" )) {         // If Interlachen - get # of caddies assigned
               numCaddies = rs.getInt("hotelNew");      // use this field since not used by Interlachen
            }
            if (club.equals( "pecanplantation" )) {     // If Pecan Plantation - get teed off indicator
            ...
            if (blocker.equals( "" ) && lskip == 0) {    // continue if tee time not blocked & not lottery - else skip
               ...
               if (in_use == 0 && noSubmit == 0) {                      // if not in use, then create submit button
                  ...
                  if (lstate > 0 && lstate < 5) {
                        ...
                        if (day_name.equals( "Friday" )) {       // if Friday
                           ...
                           if ((month == 4 && day == 20) || (month == 4 && day == 27) ||
                              ...
                              if (oaktime == false) {
                                 ...
                                 if (date == 20050706 && time > 1229 && fb == 1) {
                     ...
                     //
                     if (medinahMemTime == true || oaktime == true || mcctime == true) { // if Medinah, Oakmont or Milwaukee special time
               ...
               //
               buildPlayerTD(out, 1, show1, player1, bgcolor, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp1, g1, p1cw, disp_mnum, mnum1);
               buildPlayerTD(out, 2, show2, player2, bgcolor, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp2, g2, p2cw, disp_mnum, mnum2); 
               buildPlayerTD(out, 3, show3, player3, bgcolor, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp3, g3, p3cw, disp_mnum, mnum3); 
               buildPlayerTD(out, 4, show4, player4, bgcolor, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp4, g4, p4cw, disp_mnum, mnum4); 
               ...
               if (fivesALL != 0) {        // if 5-somes supported on any course
                  ...
                  if (fives != 0) {        // if 5-somes on this course
                    ...
                    buildPlayerTD(out, 5, show5, player5, bgcolor5, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp5, g5, p5cw, disp_mnum, mnum5); 
               ...
               if (club.equals( "pecanplantation" ) && index == 0) {   // if Pecan Plantation - add 'teed off' col
         ...
         if (!report_type.equals( "alpha" ) && !report_type.equals( "alphat" )) {   // if NOT alphabetical list
            ...
            while (rs.next()) {
               ...
               if (blocker.equals( "" )) {    // continue if tee time not blocked - else skip
                  ...
                  //  Process the F/B parm    0 = Front 9, 1 = Back 9, 9 = none (open for cross-over)
                  //
                  sfb = (fb == 1) ? "B" : (fb == 9) ? "0" : (type == shotgun) ? "S" : "F"; // changed by Paul S.
 ...
 // ********************************************************************
 //  Process the POS charges for this day - Abacus21, Jonas or NorthStar
 // ********************************************************************
 ...
 private void promptPOS2(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session, Connection con) 
   ...
   if (req.getParameter("POScontinue") == null) {
      ...
      if (parmp.posType.equals( "Abacus21" ) || parmp.posType.equals( "Jonas" )) {
         ...
         //
         resnum = date - 20000000;                     // resnum = yymmdd
      ...
      } else {         // NorthStar - date = mm/dd/yyyy
         ...
         tempNS = String.valueOf(day);

         if (day < 10) {

            tempNS = "0" + String.valueOf(day);
         }
 ...
 private void printTeeTime(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session, 
   ...
   out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
   if (min < 10) {
      out.println("<b>Time:</b>&nbsp;&nbsp; " + hr + ":0" + min + ampm);
   } else {
      out.println("<b>Time:</b>&nbsp;&nbsp; " + hr + ":" + min + ampm);
   }
   out.println("<br><br>");
 ...
 // *********************************************************
 //  Interlachen Custom - update # of caddies assigned to group
 // *********************************************************
 ...
 // *********************************************************
 //  Pecan Plantation Custom - update 'teed off' indicator for group
 // *********************************************************
 ...
 private void updateTeedOff(String stime, String sfb, long date, String teedOff, PrintWriter out, Connection con) {
   ...
   if (teedOff.equalsIgnoreCase( "x" )) {     // if 'teed off' = Yes requested
 ...
 public synchronized void addLineNS(String fname, String line) {

   int fail = 0;

   String dirname = "//home//northstar//";       // create directory name
 ...
 private void buildPlayerTD(PrintWriter out, int playern, short show, String player, String bgcolor, int time, 
                            ...
                            float hndcpn, int guest, String pcw, boolean disp_mnum, String mnum) {
    //
    //  Add Player
    //
    if (!player.equals("")) {

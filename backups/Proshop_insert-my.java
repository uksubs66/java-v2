...
public class Proshop_insert extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   if (req.getParameter("blockers") != null) {
      ...
      doBlockers(req, out, con, session);        // process block request
      out.close();
      return;
   }
   ...
   try {
      ...
      out.println("<tr><td align=\"left\" valign=\"middle\">");
         ...
         out.println("</font></td></tr><tr><td align=\"center\"><font size=\"2\"><br>");
         ...
         out.println("</font></td></tr><tr><td align=\"center\"><font size=\"2\"><br>");
         out.println("<a href=\"/" +rev+ "/servlet/Proshop_insert?index=" +index+ "&course=" +course+ "&blockers\" title=\"Return to Tee Sheet\" alt=\"Return\">");
...
//  ***************TEMP************
 ...
 private void moveWhole(parmSlot slotParms, long date, String prompt, int skip, HttpServletRequest req, 
   ...
   if (in_use != 0) {              // if time slot already in use
      ...
      //
      //  Check if this is a repsonse to a prompt after it was determined below that the 'TO' tee time is occupied
      //
      if (prompt.equals( "" )) {        // if return, then this is expected
   ...
   //
   //  If 'TO' tee time is in use and
   //  if this is NOT a repsonse to a prompt after it was determined below that the 'TO' tee time is occupied
   //
   if ((in_use != 0) && (prompt.equals( "" ))) {   // if 'TO' time slot already in use
 ...
 /***********************************************************
  * Expects the following pairs in the querystring
  *     index  =    days in advance of today that this tee-sheet is for
  *     course =    course name or -ALL- for all courses
  *     
  *
  ***********************************************************
  */
 ...
 private void doBlockers(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
    ...
    /*
     *  NOTE: CHANES TO THIS ARRAY NEED TO BE DUPLICATED IN Proshop_sheet:doPost().java
     */
...
out.println("<p>Done setting up varibales.</p>");
    ...
    */
    ...
    // start form the will be submitted to adjust blockers
    out.println("<form method=\"GET\" action=\"/" + rev + "/servlet/Proshop_insert\" name=frmBlockers>");
    ...
    try {
        ...
        while (rs.next()) {
            ...
            teetime_id = fb + delim + course + delim + hr + delim + min;
            ...
            out.println("<td bgcolor=\"white\" align=\"center\">");
            out.println("<font size=\"2\">");
            checked = (blocker.equals("")) ? "" : " checked";
            out.println("<input type=checkbox name=\"" + teetime_id + "\" value=\"1\" " + checked + ">");
            out.println("<input type=hidden name=\"tt_" + teetime_id + "\" value=\"1\"">");
            out.println("</font></td>");
 ...
 private void setBlockers(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
     ...
     /*
     String elem = "0}Mountain}10";
     String[] elemA;
     
     elemA = elem.split("}");
     
     for (int x = 0; x < elemA.length; x++) {
     ...
     */
     ...
     out.println("<p>" + i + " tee-times found</p>");

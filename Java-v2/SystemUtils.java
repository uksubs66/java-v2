 ...
 *   last updated:
 *
 *        6/15/14   Wynsotne GC (wynstone) add xDumpTees to create a csv file every hour of the current tee sheet (case 2444).
 *        4/14/14   Big Spring CC (bigsprings) - Custom weekend tee sheets and double tees will now only apply to the Big Spring CC course.
...
public class SystemUtils {
   ...
   //  (These shouldn't be needed -- currently only used by dbConn2 -- JGK 2015/04/30)
   //
   ...
   //
   //  Moved to com/foretees/common/LoginCredentials.java
   //
   //public static String support = LoginCredentials.support;            // class variables that never change
   //public static String supportpro = LoginCredentials.supportpro;
   //public static String sales = LoginCredentials.sales;
   //public static String admin = LoginCredentials.admin;
   //public static String proshop = LoginCredentials.proshop;
   //public static String id = LoginCredentials.id;
   //public static String passwordSup = LoginCredentials.passwordSup;       // password for support (dev) login...
   //public static String passwordSupPro = LoginCredentials.passwordSupPro; // password for support (prosupport) login...
   //public static String passwordSales = LoginCredentials.passwordSales;    // password for sales login...
   ...
   //
   public static String HOST = ProcessConstants.HOST;                 // mail.foretees.com - ForeTees mail server

   public static String PORT = ProcessConstants.PORT;

   public static String EFROM = ProcessConstants.EFROM;   // fake return address
   ...
   //
   public static int MEMBER_TIMEOUT = VerifyUser.MEMBER_TIMEOUT;
...
*/
 ...
 public static void getProshopSubMenu(HttpServletRequest req, PrintWriter out, int lottery) {
     ...
     out.print(Common_skin.getProshopSubMenuHtml(req, lottery));
...
public static HttpSession verifyMem(HttpServletRequest req, PrintWriter out) {
    return verifyMem(req, out, false);
}

public static HttpSession verifyMem(HttpServletRequest req, PrintWriter out, boolean allow_ext) {
    
    VerifyUser verify_user = VerifyUser.verifyMem(req, allow_ext);
    HttpSession session = verify_user.session;
    String htmlResponse = verify_user.htmlResponse;
    if(htmlResponse != null && htmlResponse.length() > 0){
       ...
       if( tpa_check != null && tpa_id != null && tpa_check.length() > 0 && !tpa_check.equals(tpa_id)){
           ...
           Common_skin.outputHeader(verify_user.club, verify_user.activity_id, "", true, out, req);
           out.println("</html>");
           out.close();
           session.invalidate();
 ...
 public static HttpSession verifyMem2(HttpServletRequest req, PrintWriter out) {
   ...
   String id = LoginCredentials.id;                   // session id
...
//      out.println("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\" \"http://www.w3.org/TR/html4/loose.dtd\">");
 ...
 // ****************************************************************************
  public static boolean verifyProAccess(HttpServletRequest req, String featureID, Connection con, PrintWriter out) {
      ...
      boolean result = verifyProAccess(req, featureID, con, message);
      if(message.length() > 0){
          out.print(message);
          out.close();
      }
 ...
 public static boolean verifyProAccess(HttpServletRequest req, String featureID, Connection con, StringBuilder message) {
 ...
 */
                ...
                message.append(SystemUtils.HeadTitle("DB Connection Error"));
                message.append("<BODY><CENTER><BR>");
                message.append("<BR><BR><H3>Database Connection Error</H3>");
                message.append("<BR><BR>Unable to connect to the Database.");
                message.append("<BR>Please try again later.");
                message.append("<BR><BR>If problem persists, contact customer support.");
                message.append("<BR><BR>");
                message.append("<a href=\"Proshop_announce\" target=\"bot\">Return</a>");
 ...
 public static HttpSession verifyPro(HttpServletRequest req, PrintWriter out) {
     ...
     return verify_user.session;
 }
 
 

 // *********************************************************
 // verifySupport - Check for illegal access by support user
 // *********************************************************

 public static boolean verifySupport(String user) {

   HttpSession session = null;

   String support = LoginCredentials.support;
   String supportpro = LoginCredentials.supportpro;
   String supportwill = LoginCredentials.supportwill;
   
   boolean allow = true;

   // Make sure user didn't enter illegally
   if (!user.equalsIgnoreCase(support) && !user.equalsIgnoreCase(supportpro) && !user.equalsIgnoreCase(supportwill)) {
 ...
 public static HttpSession verifyProAdm(HttpServletRequest req, PrintWriter out) {
   ...
   String admin = LoginCredentials.admin;
   String user = "";
...
//      out.println("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\" \"http://www.w3.org/TR/html4/loose.dtd\">");
 ...
 public static void restrictProshop(String featureID, PrintWriter out) {
     ...
     } else if (featureID.equals("TS_PACE_VIEW") || featureID.equals("TS_PACE_UPDATE") || featureID.equals("TS_NOTES_VIEW")) {
   ...
   try {
      ...
      if (xx != 0) {
         ...
         */
         ...
         //******************************************************************************************
         //
         if (club.equals( "bigsprings" ) && course.equalsIgnoreCase("Big Spring CC")) {
...
*/
        ...
        //    }
            ...
            try {
                ...
                // Gracefully catch the insert failure so that the entire process doesn't bomb out if one tee time fails to be inserted
                Utilities.logError("SystemUtils.buildTeeError(1) Building New Tee Sheets - error=" + e.getMessage());
            }
            ...
            if (fb == 2) {
               ...
               try {
                   ...
                   // Gracefully catch the insert failure so that the entire process doesn't bomb out if one tee time fails to be inserted
                   Utilities.logError("SystemUtils.buildTeeError(2) Building New Tee Sheets - error=" + e.getMessage());
               }
            ...
            if (club.equals( "piedmont" )) {       // if Piedmont Driving Club
               ...
               if (club.equals( "weeburn" )) {
                    ...
                    if (name.equals("Wednesday")) {       // only check 7/06 in 2006

                        if (shortDate >= 513 && shortDate <= 923) {       // April - Sept. (every Wed)

                            if (thistime == 830) {            // if we just did 8:00 AM

                                betwn = 6;                     // then switch to 6 min intervals

                            } else if (thistime == 1030) {        // if we just did 11:00 AM

                                betwn = betwn2;             // then return to normal intervals
                            }
                        }
                    } else if (name.equals("Tuesday")) {         // Tuesdays
                        ...
                        if ((shortDate >= 501 && shortDate <= 930)) {
                            ...
                            } else if (thistime == 1000) {        // if we just did 10:30 AM
...
//         last_mod_date = rs.getString("last_mod_date");   // replace these after fixing the date fields ***************
         ...
         // If event_signup_id isn't blank, this time is linked to an event signup, and player data should be pulled from there instead.
         if (event_signup_id != 0) {
             ...
             try {
                 ...
                 if (rs2.next()) {
                     ...
                     guest_id1 = rs2.getInt("guest_id1");
                     guest_id2 = rs2.getInt("guest_id2");
                     guest_id3 = rs2.getInt("guest_id3");
                     guest_id4 = rs2.getInt("guest_id4");
                     guest_id5 = rs2.getInt("guest_id5");
                     notes = rs2.getString("notes");
                 }
                 
             } catch (Exception e) {
                 Utilities.logError("SystemUtils.moveTee - " + club + " - Failed to look up linked event signup data for teecurr_id (" + teecurr_id + ") and event_signup_id (" + event_signup_id + ")");
             } finally {
         ...
         if (player1.equals("") && player2.equals("") && player3.equals("") && player4.equals("") && player5.equals("")) {
             ...
             try {
                 ...
                 logError("SystemUtils.moveTee Err:" + org.apache.commons.lang.exception.ExceptionUtils.getStackTrace(exc));
             } finally {
                 Connect.close(rs2, pstmt2);
             }
         }
...
//               pstmt.setInt(64, custom_int);
 ...
 private static void buildTimeSheets(int activity_id, String club, Connection con) {
    ...
    for (int x = 0; x < array.size(); x++) {
        ...
        //
        loopb1:
        for (int i = 0; i < 366; i++) {        // go 1 extra for today
    ...
    if (done_some) {
        ...
        try {
            ...
            //  call updateActivitySheets to add any restrictions, blockers, events, etc. to the new time sheets
            updateActivitySheets(con);

        } catch (Exception e3) {

            String errorMsg3 = "Error3 in SystemUtils.buildTimeSheets for " + club + ". Exception= ";
 ...
 */
 ...
 public static boolean scanTee(Connection con, String club)
   ...
   //
   if (hdcp_system.equalsIgnoreCase("CDGA"))  {
       
       dumpCDGA_sheets(club, con, null);

   } else if (hdcp_system.equalsIgnoreCase("HNCP"))  {
   
       dumpTeeSheetForHandicomp(club, con, true, false);
 ...
 public static boolean scanName(String name) {
    ...
    for(int i=0; i<name.length(); i++) {
       ...
       if (cs.compareTo( period ) != 0 && cs.compareTo( colon ) != 0 && cs.compareTo( dash ) != 0) {                             // if not a period (periods are ok)
 ...
 //************************************************************************
 // updateActivitySheets - Scans the block, event, restriction tables for
 //                 new/modified entries and updates activity_sheets if necessary.
 //
 //
 //   called by:  buildTee above after new tee times have been built
 //
 ...
 public static void updateActivitySheets(Connection con) {
...
*/
 ...
 public static void checkTime2(String club, long pdate, int ptime, String day_name) {
   ...
   if (con != null) {
      ...
      int adjusted_time = Utilities.adjustTime(con, ptime);    // Time-zone adjsuted time value for easier use in customs
      ...
      if (club.equals("waialae")) {

         if (adjusted_time >= 100 && adjusted_time <= 130) {       // do after 1am (6am CT) and make sure it gets done (more than once)
      ...
      if (club.equals( "ccjackson" )) {        // if CC of Jackson
         ...
         if (day_name.equals( "Thursday" ) && ptime > 858 && ptime < 903) {        // remove unfilled w/e tee times at 9:00 AM CT
      ...
      if (club.equals( "muirfield" ) && mmdd > 331 && mmdd < 1126) {               // Muirfield Village (4/01 - 12/01)
         ...
         if (ptime > 100 && ptime < 108) {                                     // release restrictions around 1:00 AM
            ...
            if (day_name.equals( "Tuesday" )) {
               ...
               releaseRestCommon("Tue 1PM Time Opens 7 Days Out", 8, con);          // go change the restriction start date
//               releaseRestCommon("Tue 2PM Time Opens 7 Days Out", 8, con);
            ...
            if (day_name.equals( "Wednesday" )) {
               ...
               releaseRestCommon("WED 1PM Time Opens 7 Days Out", 8, con);
//               releaseRestCommon("Wed 2PM Time Opens 7 Days Out", 8, con);
            ...
            if (day_name.equals( "Thursday" )) {
               ...
               releaseRestCommon("Thu 1PM Time Opens 7 Days Out", 8, con);
//               releaseRestCommon("Thur 2PM Time Opens 7 Days Out", 8, con);
            ...
            if (day_name.equals( "Friday" )) {
               ...
               releaseRestCommon("Fri 1PM Time Opens 7 Days Out", 8, con);
//               releaseRestCommon("Fri 2PM Time Opens 7 Days Out", 8, con);
      ...
      if (club.equals("mirasolcc") && ptime > 400 && ptime < 410 && (mmdd >= 1027 || mmdd <= 426)) {          // Mirasol - release restrictions around 4am CT - only run between 11/1 and 4/30, 5 days in advance of each (10/27 - 4/26)
      ...
      if (club.equals("ballenisles")) {
          
          if ((mmdd >= 1101 || mmdd <= 430) && ptime > 1459 && ptime < 1504) {    // release restriction at 4:00 PM ET, 7 days in advance
              releaseRestCommonGR("In Season No Guests B4 1130am", 8, con);
          }
      }
      ...
      // Desert Mountain - Remove X's 2 days in advance at 8am AZ time
      if (club.equals("desertmountain") && adjusted_time > 758 && adjusted_time < 803) {
      ...
      if (club.equals( "philcricket" ) || club.equals("atlantacountryclub")) {
       ...
       if (club.equalsIgnoreCase("ptarmigancc")) {
           if (Utilities.getDate(con) >= 20140925 && adjusted_time > 1157 && adjusted_time < 1204
                   && (day_name.equalsIgnoreCase("Wednesday") || day_name.equalsIgnoreCase("Thursday") || day_name.equalsIgnoreCase("Friday") || day_name.equalsIgnoreCase("Saturday"))) {
               xCustomTimer(club, day_name, con);
           }
       }
       
       if (club.equalsIgnoreCase("capitalcityclub")) {                                        //Remove X's the deay prior at noon
           if (adjusted_time > 1157 && adjusted_time < 1204) {
               xCustomTimer(club, day_name, con);
           }
       }
       
       if (club.equalsIgnoreCase("capecodnational")) {                                        //Remove X's the deay prior at 8 am ET
           if (adjusted_time > 657 && adjusted_time < 704) {
               xCustomTimer(club, day_name, con);
           }
       }
...
public static void reminderNotifyTimer() {
    ...
    if (Common_Server.SERVER_ID == TIMER_SERVER) {    // Only run this on the timer server!!
            ...
            if (con != null) {
                ...
                while (rs.next()) {
                    ...
                    try {
                        ...
                        con2 = Connect.getCon(club);
                        
                        if (con2 == null) {
                            Utilities.logError("SystemUtils.reminderNotifyTimer - Couldn't get connection to " + club + " database, reminders not sent");
                            continue;
                        }
                        
                        cur_date = Utilities.getDate(con2);
                        cur_time = Utilities.getTime(con2);
                        
                        stmt2 = con2.createStatement();
                        
                        rs2 = stmt2.executeQuery("SELECT remind_prefTime, remind_sent FROM club5");
                        
                        if (rs2.next()) {
                            
                            // We only want to send out reminder notifications the first time the 60 min timer runs after their preferred time, and only once per day.
                            if (rs2.getInt("remind_prefTime") > 0 && cur_date > rs2.getLong("remind_sent") && cur_time >= rs2.getInt("remind_prefTime")) {
                                sendEmail.sendReminderNotifications(club, con2);
                            }
                        }
                        
                    } catch (Exception e) {
                        Utilities.logError("SystemUtils.reminderNotifyTimer - " + club + " - Error looking up club reminder settings or running reminder job - Error=" + e.toString());
                    } finally {
                        Connect.close(rs2, stmt2, con2);
                    }
                }
                
            } else {
                Utilities.logError("SystemUtils.reminderNotifyTimer - Couldn't get connection to " + rev + " database to retrieve club list");
            }
            
        } catch (Exception e) {
            Utilities.logError("SystemUtils.reminderNotifyTimer - Error while looping over clubs - Error=" + e.toString());
        } finally {
 ...
 public static void xTimerClubs(String club, long hour) {
   ...
   try {
      ...
      if (con != null) {
         ...
         if (club.equals( "hudsonnatl" ) && hour == 6) {    //  Merion or Oakmont and around 6:00 AM CT or 7:00 AM ET
         ...
         */
         ...
         if (club.equals( "wynstone" ) && hour < 23 && hour > 4) {    //  Wynstone GC in IL and between 5:00 AM and 11:00 PM CT

            xDumpTees(club, con);                                     //  dump today's tee sheet into a csv file for ABDI (guest management company) to pick up
         }
 ...
 public static void xTees(String club, Connection con) {
   ...
   String clubName = "";
   String subject = "";
   String customQ = ""; //custom query for 
   ...
   try {
      ...
      if (rs.next()) {                     // one club5 table per club
         ...
         if ((x != 0) && (xhrs != 0)) {     // if club wants to remove X's
            ...
            if (club.equalsIgnoreCase("ptarmigancc")) {
                customQ = " AND (day != 'Saturday' AND day != 'Sunday' AND day != 'Friday')";
            }
            //
            //  Remove all 'X' players within 'xhrs' hours from now
            //
             pstmt2 = con.prepareStatement(
                     "SELECT teecurr_id, date, time, courseName, fb, day, player1, player2, player3, player4, player5, orig2, orig3, orig4, orig5 "
                     + "FROM teecurr2 WHERE date <= ? AND time <= ?" + customQ);
            ...
            pstmt2 = con.prepareStatement (
                      ...
                      "FROM teecurr2 WHERE date = ? AND time >= ? AND time <= ?"+customQ);
 ...
 //************************************************************************
 //  xDumpTees - subr of xTimer to dump today's tee sheet into a csv file 
 //              for Wynstone GC (case 2444)
 //************************************************************************

 public static void xDumpTees(String club, Connection con) {
   ...
   String user5 = "";
    ...
    try {
        ...
        //
        //  Create a new file (delete old content if file exists) - append = false
        //
        fout = new PrintWriter(new FileWriter("//srv//webapps//" + club + "//export//teetimes.csv", false));

        //
        //  Get today's tee sheet data
        //
        stmt = con.prepareStatement (
                "SELECT time, player1, player2, player3, player4, player5, orig_by, "
                       + "username1, username2, username3, username4, username5 " +
                "FROM teecurr2 " +
                "WHERE date = ? AND player1 != '' ORDER BY time, fb");
        ...
        while (rs.next()) {
            ...
            if (player1.equals("")) {    // use spaces when none
                ...
                ptype1 = "M";                         // Member
                if (user1.equals("")) ptype1 = "G";   // or Guest
            }
            if (player2.equals("")) {
                ...
                ptype2 = "M";                         // Member
                if (user2.equals("")) ptype2 = "G";   // or Guest
            }
            if (player3.equals("")) {
                ...
                ptype3 = "M";                         // Member
                if (user3.equals("")) ptype3 = "G";   // or Guest
            }
            if (player4.equals("")) {
                ...
                ptype4 = "M";                         // Member
                if (user4.equals("")) ptype4 = "G";   // or Guest
            }
            if (player5.equals("")) {
                ...
                ptype5 = "M";                         // Member
                if (user5.equals("")) ptype5 = "G";   // or Guest
            }
            
            //  Get name of user that originated the tee time
            
            orig_name = "unknown";
            
            if (!orig_by.equals("")) {
                
                if (orig_by.startsWith("proshop")) {
                
                    orig_name = "Golf Shop";
                
                } else {
                    
                    orig_name = Utilities.getFullNameFromUsername(orig_by, con);    // get the member's name
                    
                    if (orig_name.equals("")) orig_name = "unknown";
                }
            }
            
            //
            //  Build the record in the csv file
            //
            //   Record = date, time, name of originator, player1, type1, ..... (all 5 players) - Type: M=member, G=guest
            //
            fout.println(date+ "," +time+ "," +orig_name+ "," +player1+ "," +ptype1+ "," +player2+ "," +ptype2+ "," +player3+ "," +ptype3+ "," +player4+ "," +ptype4+ "," +player5+ "," +ptype5);

        }   // end of WHILE event

        fout.close();           // done - close the file
        
    }
    catch (Exception e1) {

        logError("Error in SystemUtils.xDumpTees - Exception: " + e1.getMessage());      // log the error
 ...
 public static void xMerionTees(String club, Connection con) {
   ...
   } else if (club.equals("oakhillcc")) {                   //send email reminders 2 days & 7 days in advance

       subject = "Reminder from Oak Hill Country Club";
       
       proEmail1 = "";       // Pro email address
       
       days = 3;         // days in advance to send emails
       
       days2 = 7;       // days in advance to send emails
          
   } else if (club.equals("hudsonnatl")) {

       subject = "Reminder from Hudson National Golf Club";
       
       proEmail1 = "";       // Pro email address
       
       days = 1;         // days in advance to send emails
   ...
   prepState = "SELECT date, time, player1, player2, player3, player4, username1, username2, username3, username4, fb, player5, username5, courseName " +
     ...
     if (club.equals("oakhillcc")) {
         ...
         cal2.add(Calendar.DATE, days2);                     // roll ahead 7 days

         year2 = cal2.get(Calendar.YEAR);
         month2 = cal2.get(Calendar.MONTH) + 1;
         day2 = cal2.get(Calendar.DAY_OF_MONTH);

         adv_date2 = year2 * 10000;                      // create a date field of yyyymmdd
         adv_date2 = adv_date2 + (month2 * 100);
         adv_date2 = adv_date2 + day2;                    // date = yyyymmdd (for comparisons)
         prepState = "SELECT date, time, player1, player2, player3, player4, username1, username2, username3, username4, fb, player5, username5, courseName "
                 + "FROM teecurr2 WHERE (date = ? OR date = ?) AND player1 != '' AND player1 != 'x'";
   ...
   try {
      ...
      pstmt2 = con.prepareStatement (prepState);
      ...
      while (rs2.next()) {
         ...
         if (club.equals("oakhillcc")) {   //if date found is 7 days out
             if (date == adv_date2) {
                edate = month2 + "/" + day2 + "/" + year2;       // date for email message
                days = 7;
             } else {
      ...
      // logError("TEST - email to be sent. Club=" +club+ " send=" +send+ ".");   // TEMP !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
               ...
               if (send != 0) {                 // shuld we send an email ?
                  ...
                  if (club.equals("oakhillcc")) {
                      enew += "TEE TIME REMINDER:  You are scheduled for the following tee time in "+days+" days.  If you do not intend to use this tee time, please cancel "
                              + "immediately as a courtesy to other members looking to bring guests.\n\n"
                              + "GUEST NAMES:  If you have not already done so, please enter the names of your guests in Foretees, or notify the golf shop of their "
                              + "names so the Oak Hill staff can properly greet and service them when they arrive.";
                  } else if (club.equals("oakmont") && cur_date <= 20150617) {
                      enew += "REMINDER:  The Hulton Bridge will be closed from June 11-24.  Please plan your travel accordingly and let your guests know about the closure.  "
                              + "Alternate routes could add an hour or more to your trip.";
                  }
 ...
 public static void xCustomTimer (String club, String day_name, Connection con) {
   ...
   try {
       ...
       rs = stmt.executeQuery("SELECT xCustomTimer_run FROM club5");
       
       if (rs.next()) {
           if (rs.getLong("xCustomTimer_run") == cur_date) {
               return;
           }
       }
       
   } catch (Exception e) {
       Utilities.logError("SystemUtils.xCustomTimer - " + club + " - Error trying to look up club5.xCustomTimer_run - Error=" + e.toString());
   } finally {
   ...
   Utilities.logError("Running xCustomTimer for " + club);
   ...
   try {
        ...
        if (club.equals("castlepines") || (club.equals("desertmountain") && date < 20151101)) {
            
            days = 2;
            
        } else if (club.equals("ironwood") || (club.equals("desertmountain") && date >= 20151101)) {
            
            days = 3;
            
        } else if (club.equals("discoverybay") || club.equalsIgnoreCase("ptarmigancc") || club.equalsIgnoreCase("capitalcityclub") || club.equalsIgnoreCase("capecodnational")) {
            
            days = 1;  
        }
        if (club.equalsIgnoreCase("ptarmigancc")) {
            customQ = " AND time < 1300";
        }
        if (club.equalsIgnoreCase("capecodnational")) {
            customQ = " AND time < 1200";
        }
       ...
       if (!club.equals("ptarmigancc") || !day_name.equalsIgnoreCase("wednesday")) {     

            //
            //  Remove all 'X' players within 'xhrs' hours from now
            //
            pstmt2 = con.prepareStatement (
                   "SELECT teecurr_id, date, time, courseName, fb, player1, player2, player3, player4, player5, orig2, orig3, orig4, orig5 "
                   + "FROM teecurr2 WHERE date <= ?" + customQ);
            ...
            while (rs2.next()) {
               ...
               // Desert Mountain - If tee time contains 3 or more X's, cancel the time instead of just clearing out the X's (starts 11/1/2015) (case 2685).
               if (club.equals("desertmountain") && xCount >= 3 && date >= 20151101) {
                   Proshop_dsheet.cancelOneTeeTime(date, time, fb, course, day_name, "no", "X-Removal", club, con);                   
                   change = 0;
               }
        ...
        //
        if (!club.equals("ptarmigancc") || !day_name.equals("saturday")) {      
            ...
            //
            pstmt2 = con.prepareStatement (
                      "SELECT mm, dd, yy, day, hr, min, player1, player2, player3, player4, player5, " +
                      "username1, username2, username3, username4, username5, fb, courseName " +
                      "FROM teecurr2 WHERE date = ? AND player1 <> '' AND (player1 = 'X' OR player2 = 'X' OR player3 = 'X' OR player4 = 'X' OR player5 = 'X')");
            ...
            while (rs2.next()) {
                ...
                player4 = rs2.getString("player4");
                player5 = rs2.getString("player5");
                username1 = rs2.getString("username1");
                username2 = rs2.getString("username2");
                username3 = rs2.getString("username3");
                username4 = rs2.getString("username4");
                username5 = rs2.getString("username5");
                fb = rs2.getInt("fb");
                course = rs2.getString("courseName");
                ...
                //  if any X's in the tee time, then send email to all players with email enabled
                if (player1.equalsIgnoreCase("x") || player2.equalsIgnoreCase("x") || player3.equalsIgnoreCase("x") || player4.equalsIgnoreCase("x") || player5.equalsIgnoreCase("x")) {   
                    ...
                    } else if (club.equals("discoverybay") || club.equalsIgnoreCase("ptarmigancc") || club.equalsIgnoreCase("capitalcityclub")) {

                        timeString = "12:00pm";

                    } else if (club.equals("desertmountain") || club.equals("capecodnational")) {
   ...
   try {
       pstmt2 = con.prepareStatement("UPDATE club5 SET xCustomTimer_run = ?");
       pstmt2.clearParameters();
       pstmt2.setLong(1, cur_date);
       
       pstmt2.executeUpdate();
       
   } catch (Exception e) {
       Utilities.logError("SystemUtils.xCustomTimer - " + club + " - Error trying to set club5.xCustomTimer_run to today's date - Error=" + e.toString());
   } finally {
...
/*
    ...
    return auth;
      * 
      */
     return Utilities.getAuthenticator();
 }
         ...
         try {
            ...
            while (rs1.next()) {
               ...
               try {
                   ...
                   // Martis Camp only wants one of the backup copies to go out. Skip the evening one so the one they get is as fresh as possible.
                   if (club.equals("martiscamp")) {
                       int temp_time = Utilities.getTime(con2);
...
*/
         ...
         b = stmt.execute("OPTIMIZE TABLE session_log");          // session_log
 ...
 public static void checkTeeTimesCCJ(Connection con) {
   ...
   try {
      ...
      long sdate = Utilities.getDate(con, 2);    // Get Saturday's date (running on Thurs)
      long edate = Utilities.getDate(con, 3);    // Get Sunday's date
 ...
 public static void releaseRestOahuCC(String restName, int days, Connection con) {
   ...
   try {
      ...
      pstmt = con.prepareStatement (
               "UPDATE restriction2 SET sdate = ?, start_mm = ?, start_dd = ?, start_yy = ? WHERE recurr <> 'Every Saturday' "
               + "AND name NOT IN ('Sunday Spouse Jr Restriction', 'Holiday Spouse Jr Rest Lab Day', 'Holiday Spouse Jr Restriction')");
      ...
      //
      pstmt = con.prepareStatement (
              "UPDATE teecurr2 SET restriction = '', rest_color = '' WHERE date < ? AND day <> 'Saturday' "
              + "AND restriction NOT IN ('Sunday Spouse Jr Restriction', 'Holiday Spouse Jr Rest Lab Day', 'Holiday Spouse Jr Restriction')");
...
*/
 ...
 public static void sessionLog(String msg, String user, String pw, String club, String caller, Connection con) {
     ...
     Connect.sessionLog(msg, user, pw, club, caller, con);
...
*/
 ...
 public static String getClubName(Connection con) {
    ...
    try {
       ...
       clubName = Utilities.getClubName(con);
        ...
        SystemUtils.logError("Error in SystemUtils.getClubName, Exception: " +e1.getMessage() + ", trace=" + Utilities.getStackTraceAsString(e1) );
    }
 ...
 public static void dumpCDGA_sheets(String club, Connection con, PrintWriter out) {
   ...
   */
    ...
    } else if (club.equals("idlewildcc")) {

        siteID = "1952";
        
    } else if (club.equals("oakparkcountryclub")) {
  ...
  ***************************************************************************************
  *
  * This method creates a string containing a csv of the current or past days tee sheet
  *
  * @param club the club we are running this for
  * @param con the existing database connection object for this club
  *
  ...
  **/
  ...
  public static String dumpTeeSheetForHandicomp(String club, Connection con, boolean previousRounds, boolean debug) {
    ...
    try {
        ...
        if (previousRounds) {
            sql = "" +
                "SELECT * " +
                "FROM teepast2 " +
                "WHERE date >= DATE_FORMAT(DATE_ADD(now(), INTERVAL -3 DAY), '%Y%m%d') AND player1 <> '' " +
                "ORDER BY date, time";
        } else {
            sql = "" +
                "SELECT * " +
        ...
        while (rs.next()) {
            ...
            /*  Record Formats (DO NOT CHANGE FOR THESE CLUBS):
            * 
            *    For Meadow Brook CC MI -
            * 
            "Date Time","Course Name","Event Name","Front/Back","Player1","Player2","Player3","Player4","Player5","p1cw","p2cw","p3cw","p4cw","p5cw","p1MemNum","p2MemNum","p3MemNum","p4MemNum","p5MemNum","p1HdcpNum","p2HdcpNum","p3HdcpNum","p4HdcpNum","p5HdcpNum","p91","p92","p93","p94","p95"
            * 
            * 
            *    For the Wisconsin Club - 
            * 
            "Date Time","Course Name","Event Name","Front/Back","Player1","Player2","Player3","Player4","Player5","p1cw","p2cw","p3cw","p4cw","p5cw","p91","p92","p93","p94","p95"
            */
    
            sb.append("\"" + rs.getInt("yy") + "-" + rs.getInt("mm") + "-" + rs.getInt("dd") + " " + rs.getInt("hr") + ":" + Utilities.ensureDoubleDigit(rs.getInt("min")) + ":00\",");
            sb.append("\"" + rs.getString("courseName") + "\",");
            sb.append("\"" + rs.getString("event") + "\",");
            sb.append("\"" + rs.getInt("fb") + "\",");
            sb.append("\"" + rs.getString("player1") + "\",");
            sb.append("\"" + rs.getString("player2") + "\",");
            sb.append("\"" + rs.getString("player3") + "\",");
            sb.append("\"" + rs.getString("player4") + "\",");
            sb.append("\"" + rs.getString("player5") + "\",");
            sb.append("\"" + rs.getString("p1cw") + "\",");
            sb.append("\"" + rs.getString("p2cw") + "\",");
            sb.append("\"" + rs.getString("p3cw") + "\",");
            sb.append("\"" + rs.getString("p4cw") + "\",");
            sb.append("\"" + rs.getString("p5cw") + "\",");
            
            if (club.equals("meadowbrookccmi")) {
            
                sb.append("\"" + rs.getString("username1") + "\","); // mNum1-5
                sb.append("\"" + rs.getString("username2") + "\",");
                sb.append("\"" + rs.getString("username3") + "\",");
                sb.append("\"" + rs.getString("username4") + "\",");
                sb.append("\"" + rs.getString("username5") + "\",");
                sb.append("\"" + ((!rs.getString("username1").equals("")) ? Utilities.getHdcpNum(rs.getString("username1"), con) : "") + "\",");
                sb.append("\"" + ((!rs.getString("username2").equals("")) ? Utilities.getHdcpNum(rs.getString("username2"), con) : "") + "\",");
                sb.append("\"" + ((!rs.getString("username3").equals("")) ? Utilities.getHdcpNum(rs.getString("username3"), con) : "") + "\",");
                sb.append("\"" + ((!rs.getString("username4").equals("")) ? Utilities.getHdcpNum(rs.getString("username4"), con) : "") + "\",");
                sb.append("\"" + ((!rs.getString("username5").equals("")) ? Utilities.getHdcpNum(rs.getString("username5"), con) : "") + "\",");
            }
            
            if (previousRounds) {

                sb.append("\"" + rs.getString("nopost1") + "\",");
                sb.append("\"" + rs.getString("nopost2") + "\",");
                sb.append("\"" + rs.getString("nopost3") + "\",");
                sb.append("\"" + rs.getString("nopost4") + "\",");
                sb.append("\"" + rs.getString("nopost5") + "\",");
            }
            
            sb.append("\"" + rs.getInt("p91") + "\",");
            sb.append("\"" + rs.getInt("p92") + "\",");
            sb.append("\"" + rs.getInt("p93") + "\",");
            sb.append("\"" + rs.getInt("p94") + "\",");
            sb.append("\"" + rs.getInt("p95") + "\"\n");
            /*
            fout.print("\"" + rs.getInt("yy") + "-" + rs.getInt("mm") + "-" + rs.getInt("dd") + " " + rs.getInt("hr") + ":" + Utilities.ensureDoubleDigit(rs.getInt("min")) + ":00\",");
            fout.print("\"" + rs.getString("event") + "\",");
            fout.print("\"" + rs.getInt("fb") + "\",");
            fout.print("\"" + rs.getString("player1") + "\",");
            fout.print("\"" + rs.getString("player2") + "\",");
            fout.print("\"" + rs.getString("player3") + "\",");
            fout.print("\"" + rs.getString("player4") + "\",");
        ...
        Utilities.logError("SystemUtils.dumpTeeSheetForHandicomp(): club=" + club + ", err=" + exc.getMessage());
    ...
    if (sb.length() == 0 && debug) sb.append("No data to return.");

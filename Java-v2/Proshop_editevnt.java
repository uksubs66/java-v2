 ...
 private void update1(HttpServletRequest req, PrintWriter out, HttpSession sess) {
   ...
   if (signUp.equals( "No" )) {
      ...
      if (req.getParameter("copyname") != null) {
         ...
         member_item_code = (req.getParameter("member_item_code") != null) ? req.getParameter("member_item_code") : "";
         member_fee = (req.getParameter("member_fee") != null) ? req.getParameter("member_fee") : "";
         member_tax = (req.getParameter("member_tax") != null) ? req.getParameter("member_tax") : "";
         guest_item_code = (req.getParameter("guest_item_code") != null) ? req.getParameter("guest_item_code") : "";
         guest_fee = (req.getParameter("guest_fee") != null) ? req.getParameter("guest_fee") : "";
         guest_tax = (req.getParameter("guest_tax") != null) ? req.getParameter("guest_tax") : "";
         ...
         ask_custom1 = (req.getParameter("ask_custom1") != null && req.getParameter("ask_custom1").equals("1")) ? 1 : 0;
         ...
         req_custom1 = (req.getParameter("req_custom1") != null && req.getParameter("req_custom1").equals("1")) ? 1 : 0;
...
*/
                ...
                for (i=0; i<16; i++) {
                   if (!tmode[i].equals( "" ) && !tmode[i].equals( null )) {
                      ...
                      if (tmodei[i] == 1) {   // if already selected
                         out.println("<label><input type=\"checkbox\" checked name=\"tmode" +i2+ "\" value=\"yes\">&nbsp;&nbsp;" +tmode[i]+ "</label>");
                      } else {
              ...
              out.println("<br><br>");
               ...
               out.println("Sign-up Date and Time (when members can begin to sign up):<br><br>");
                 ...
                 if (su_year < thisYear) {
                     Common_Config.buildOption(su_year, su_year, su_year, out);
                 }
                 for (int j = thisYear; j <= (thisYear + 5); j++) {
                     Common_Config.buildOption(j, j, su_year, out);
                 }
               ...
               out.println("Cut-off Date and Time (when members can no longer sign up):<br><br>");
                 ...
                 if (c_year < thisYear) {
                     Common_Config.buildOption(c_year, c_year, c_year, out);
                 }
                 for (int j = thisYear; j <= (thisYear + 5); j++) {
                     Common_Config.buildOption(j, j, c_year, out);
                 }
                 out.println("</select>");
               out.println("<br><br>");
               out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Hour &nbsp;");
                 ...
                 for (i=0; i<parm.MAX_Mems; i++) {
                    ...
                    if (!parm.mem[i].equals( "" )) {          // if supported and
                       ...
                       if (!mtype[i2].equals( "" )) {        //    already checked
                          ...
                          out.println("<label><input type=\"checkbox\" checked name=\"mem" +i2+ "\" value=\"" +parm.mem[i]+ "\">&nbsp;&nbsp;" +parm.mem[i] + "</label>");
                       } else {
                 ...
                 for (i=0; i<parm.MAX_Mships; i++) {
                    ...
                    if (!parm.mship[i].equals( "" )) {          // if supported and
                       ...
                       if (!mship[i2].equals( "" )) {        //    already checked
                          ...
                          out.println("<label><input type=\"checkbox\" checked name=\"mship" +i2+ "\" value=\"" +parm.mship[i]+ "\">&nbsp;&nbsp;" +parm.mship[i] + "</label>");
                       } else {
                 ...
                 if (!copyname.equals( "" )) {         // if a copy request
                    ...
                    out.println("</p>");
                     ...
                     out.println("<input type=\"hidden\" name=\"member_item_code\" value=" + member_item_code + ">");
                     out.println("<input type=\"hidden\" name=\"member_fee\" value=" + member_fee + ">");
                     out.println("<input type=\"hidden\" name=\"member_tax\" value=" + member_tax + ">");
                     out.println("<input type=\"hidden\" name=\"guest_item_code\" value=" + guest_item_code + ">");
                     out.println("<input type=\"hidden\" name=\"guest_fee\" value=" + guest_fee + ">");
                     out.println("<input type=\"hidden\" name=\"guest_tax\" value=" + guest_tax + ">");
                     ...
                     out.println("<input type=\"hidden\" name=\"ask_custom1\" value=\"" + ask_custom1 + "\">");
                     ...
                     out.println("<input type=\"hidden\" name=\"req_custom1\" value=\"" + req_custom1 + "\">");
 ...
 private void updateConf(HttpServletRequest req, PrintWriter out, HttpSession sess) {
   ...
   int ask_custom1 = (req.getParameter("ask_custom1") != null && req.getParameter("ask_custom1").equals("1")) ? 1 : 0;
   ...
   int req_custom1 = (req.getParameter("req_custom1") != null && req.getParameter("req_custom1").equals("1")) ? 1 : 0;
...
if (season == 0) {
 ...
 private void removeEvent(HttpServletRequest req, PrintWriter out, HttpSession sess) {
    ...
    Utilities.createEventLogEntry(user, event_id, 0, "CONFIG", "Event deleted from system", 0, con);
...
*/
   ...
   // Before applying updates, see if the year of the event is being changed, and if so, create a copy of the current event data to preserve the data for use with event reporting.
   if (year > oldDateYear && season == 0) {
       
       PreparedStatement pstmtx = null;
       
       try {
           pstmt = con.prepareStatement("SELECT * FROM events2b WHERE event_id = ?");
           pstmt.clearParameters();
           pstmt.setInt(1, event_id);
           ...
           if (rs.next()) {
               ...
               pstmtx = con.prepareStatement("INSERT INTO events2b "
                       + "(date, year, month, day, start_hr, start_min, stime, end_hr, end_min, etime, "
                       + "color, type, act_hr, act_min, courseName, signUp, max, itin, mc, pc, "
                       + "wa, ca, gstOnly, holes, fb, mempos, gstpos, name, stime2, etime2, "
                       + "fb2, minsize, gender, export_type, season, email1, email2, activity_id, locations, memedit, member_item_code, "
                       + "guest_item_code, member_fee, guest_fee, member_tax, guest_tax, inactive, format, pairings, size, guests, "
                       
                       + "memcost, gstcost, c_month, c_day, c_year, c_hr, c_min, c_date, c_time, x, "
                       + "xhrs, su_month, su_day, su_year, su_hr, su_min, su_date, su_time, ask_homeclub, ask_phone, "
                       + "ask_address, ask_hdcp, ask_email, ask_gender, ask_shirtsize, ask_shoesize, ask_otherA1, ask_otherA2, ask_otherA3, ask_custom1, "
                       + "req_guestname, req_homeclub, req_phone, req_address, req_hdcp, req_email, req_gender, req_shirtsize, req_shoesize, req_otherA1, "
                       + "req_otherA2, req_otherA3, req_custom1, otherQ1, otherQ2, otherQ3, tmode1, tmode2, "
                       
                       + "tmode3, tmode4, tmode5, tmode6, tmode7, tmode8, tmode9, tmode10, tmode11, tmode12, "
                       + "tmode13, tmode14, tmode15, tmode16, mem1, mem2, mem3, mem4, mem5, mem6, "
                       + "mem7, mem8, mem9, mem10, mem11, mem12, mem13, mem14, mem15, mem16, "
                       + "mem17, mem18, mem19, mem20, mem21, mem22, mem23, mem24, mship1, mship2, "
                       + "mship3, mship4, mship5, mship6, mship7, mship8, mship9, mship10, mship11, mship12, "
                       
                       + "mship13, mship14, mship15, mship16, mship17, mship18, mship19, mship20, mship21, mship22, "
                       + "mship23, mship24) "
                       + "VALUES ("
                       + "?,?,?,?,?,?,?,?,?,?,"
                       + "?,?,?,?,?,?,0,?,?,?,"
               ...
               pstmtx.setLong(ind++, oldDate);
               pstmtx.setInt(ind++, rs.getInt("year"));
               pstmtx.setInt(ind++, rs.getInt("month"));
               pstmtx.setInt(ind++, rs.getInt("day"));
               pstmtx.setInt(ind++, rs.getInt("start_hr"));
               pstmtx.setInt(ind++, rs.getInt("start_min"));
               pstmtx.setInt(ind++, rs.getInt("stime"));
               pstmtx.setInt(ind++, rs.getInt("end_hr"));
               pstmtx.setInt(ind++, rs.getInt("end_min"));
               pstmtx.setInt(ind++, rs.getInt("etime"));    // 10
               
               pstmtx.setString(ind++, rs.getString("color"));
               pstmtx.setInt(ind++, rs.getInt("type"));
               pstmtx.setInt(ind++, rs.getInt("act_hr"));
               pstmtx.setInt(ind++, rs.getInt("act_min"));
               pstmtx.setString(ind++, rs.getString("courseName"));
               pstmtx.setInt(ind++, rs.getInt("signUp"));
               pstmtx.setString(ind++, rs.getString("itin"));
               pstmtx.setInt(ind++, rs.getInt("mc"));
               pstmtx.setInt(ind++, rs.getInt("pc"));    // 19
               
               pstmtx.setInt(ind++, rs.getInt("wa"));
               pstmtx.setInt(ind++, rs.getInt("ca"));
               pstmtx.setInt(ind++, rs.getInt("gstOnly"));
               pstmtx.setInt(ind++, rs.getInt("holes"));
               pstmtx.setString(ind++, rs.getString("fb"));
               pstmtx.setString(ind++, rs.getString("mempos"));
               pstmtx.setString(ind++, rs.getString("gstpos"));
               pstmtx.setString(ind++, rs.getString("name"));
               pstmtx.setInt(ind++, rs.getInt("stime2"));
               pstmtx.setInt(ind++, rs.getInt("etime2"));    // 29
               
               pstmtx.setString(ind++, rs.getString("fb2"));
               pstmtx.setInt(ind++, rs.getInt("minsize"));
               pstmtx.setInt(ind++, rs.getInt("gender"));
               pstmtx.setInt(ind++, rs.getInt("export_type"));
               pstmtx.setInt(ind++, rs.getInt("season"));
               pstmtx.setString(ind++, rs.getString("email1"));
               pstmtx.setString(ind++, rs.getString("email2"));
               pstmtx.setInt(ind++, rs.getInt("activity_id"));
               pstmtx.setString(ind++, rs.getString("locations"));
               pstmtx.setInt(ind++, rs.getInt("memedit"));
               pstmtx.setString(ind++, rs.getString("member_item_code"));    // 40
               
               pstmtx.setString(ind++, rs.getString("guest_item_code"));
               pstmtx.setDouble(ind++, rs.getDouble("member_fee"));
               pstmtx.setDouble(ind++, rs.getDouble("guest_fee"));
               pstmtx.setDouble(ind++, rs.getDouble("member_tax"));
               pstmtx.setDouble(ind++, rs.getDouble("guest_tax"));
               pstmtx.setString(ind++, rs.getString("format"));
               pstmtx.setString(ind++, rs.getString("pairings"));
               pstmtx.setInt(ind++, rs.getInt("size"));
               pstmtx.setInt(ind++, rs.getInt("guests"));    // 49
               
               
               pstmtx.setString(ind++, rs.getString("memcost"));
               pstmtx.setString(ind++, rs.getString("gstcost"));
               pstmtx.setInt(ind++, rs.getInt("c_month"));
               pstmtx.setInt(ind++, rs.getInt("c_day"));
               pstmtx.setInt(ind++, rs.getInt("c_year"));
               pstmtx.setInt(ind++, rs.getInt("c_hr"));
               pstmtx.setInt(ind++, rs.getInt("c_min"));
               pstmtx.setInt(ind++, rs.getInt("c_date"));
               pstmtx.setInt(ind++, rs.getInt("c_time"));
               pstmtx.setInt(ind++, rs.getInt("x"));    // 59
               
               pstmtx.setInt(ind++, rs.getInt("xhrs"));
               pstmtx.setInt(ind++, rs.getInt("su_month"));
               pstmtx.setInt(ind++, rs.getInt("su_day"));
               pstmtx.setInt(ind++, rs.getInt("su_year"));
               pstmtx.setInt(ind++, rs.getInt("su_hr"));
               pstmtx.setInt(ind++, rs.getInt("su_min"));
               pstmtx.setInt(ind++, rs.getInt("su_date"));
               pstmtx.setInt(ind++, rs.getInt("su_time"));
               pstmtx.setInt(ind++, rs.getInt("ask_homeclub"));
               pstmtx.setInt(ind++, rs.getInt("ask_phone"));    // 69
               
               pstmtx.setInt(ind++, rs.getInt("ask_address")); 
               pstmtx.setInt(ind++, rs.getInt("ask_hdcp"));
               pstmtx.setInt(ind++, rs.getInt("ask_email"));
               pstmtx.setInt(ind++, rs.getInt("ask_gender"));
               pstmtx.setInt(ind++, rs.getInt("ask_shirtsize"));
               pstmtx.setInt(ind++, rs.getInt("ask_shoesize"));
               pstmtx.setInt(ind++, rs.getInt("ask_otherA1"));
               pstmtx.setInt(ind++, rs.getInt("ask_otherA2"));
               pstmtx.setInt(ind++, rs.getInt("ask_otherA3"));
               pstmtx.setInt(ind++, rs.getInt("ask_custom1"));    // 79
               
               pstmtx.setInt(ind++, rs.getInt("req_guestname"));
               pstmtx.setInt(ind++, rs.getInt("req_homeclub"));
               pstmtx.setInt(ind++, rs.getInt("req_phone"));
               pstmtx.setInt(ind++, rs.getInt("req_address"));
               pstmtx.setInt(ind++, rs.getInt("req_hdcp"));
               pstmtx.setInt(ind++, rs.getInt("req_email"));
               pstmtx.setInt(ind++, rs.getInt("req_gender"));
               pstmtx.setInt(ind++, rs.getInt("req_shirtsize"));
               pstmtx.setInt(ind++, rs.getInt("req_shoesize"));
               pstmtx.setInt(ind++, rs.getInt("req_otherA1"));    // 89
               
               pstmtx.setInt(ind++, rs.getInt("req_otherA2"));
               pstmtx.setInt(ind++, rs.getInt("req_otherA3"));
               pstmtx.setInt(ind++, rs.getInt("req_custom1"));
               pstmtx.setString(ind++, rs.getString("otherQ1"));
               pstmtx.setString(ind++, rs.getString("otherQ2"));
               pstmtx.setString(ind++, rs.getString("otherQ3"));    // 95
               
               for (int k = 1; k <= 16; k++) {
                   pstmtx.setInt(ind++, rs.getInt("tmode" + String.valueOf(k)));
               }    // 110
               for (int k = 1; k <= 24; k++) {
                   pstmtx.setString(ind++, rs.getString("mem" + String.valueOf(k)));
               }    // 134
               for (int k = 1; k <= 24; k++) {
                   pstmtx.setString(ind++, rs.getString("mship" + String.valueOf(k)));
               }    // 158
               ...
               if (pstmtx.executeUpdate() > 0) {
                   ...
                   // Duplicate created successfully, locate its event_id
                   try {
                       
                       pstmtx2 = con.prepareStatement("SELECT MAX(event_id) AS new_id FROM events2b WHERE name = ?");
                       pstmtx2.clearParameters();
                       pstmtx2.setString(1, rs.getString("name"));
                       
                       rsx = pstmtx2.executeQuery();
                       
                       if (rsx.next()){
                           
                           PreparedStatement pstmtx3 = null;
                           
                           // We now have the event_id for the duplicate copy of the original event. 
                           try {
                               
                               // Update the name of the duplicate event, and make sure it's set inactive
                               pstmtx3 = con.prepareStatement("UPDATE events2b SET name = CONCAT('*** ', name), inactive = 1 WHERE event_id = ?");
                               pstmtx3.clearParameters();
                               pstmtx3.setInt(1, rsx.getInt("new_id"));
                               
                               pstmtx3.executeUpdate();
                               
                           } catch (Exception e) {
                               Utilities.logError("Proshop_editevnt.updateEvent - " + club + " - Adjusting name on duplicate event record. event_id = " 
                                       + event_id + ", dup event_id = " + rsx.getInt("new_id") + " - Error = " + e.toString());
                           } finally {
                               Connect.close(pstmtx3);
                           } 
                           
                           try {
                               
                               // Migrate the old event's signups to this one, adjust the event name, and make sure they're set inactive
                               pstmtx3 = con.prepareStatement("UPDATE evntsup2b SET event_id = ?, name = CONCAT('*** ', name), inactive = 1 WHERE event_id = ?");
                               pstmtx3.clearParameters();
                               pstmtx3.setInt(1, rsx.getInt("new_id"));
                               pstmtx3.setInt(2, event_id);
                               
                               pstmtx3.executeUpdate();
                               
                           } catch (Exception e) {
                               Utilities.logError("Proshop_editevnt.updateEvent - " + club + " - Error migrating signups to duplicate event record. event_id = " 
                                       + event_id + ", dup event_id = " + rsx.getInt("new_id") + " - Error = " + e.toString());
                           } finally {
                               Connect.close(pstmtx3);
                           }
                           ...
                           try {
                               ...
                               // Migrate the old event's event log entries to the new event_id
                               pstmtx3 = con.prepareStatement("UPDATE event_log SET event_id = ? WHERE event_id = ?");
                               pstmtx3.clearParameters();
                               pstmtx3.setInt(1, rsx.getInt("new_id"));
                               pstmtx3.setInt(2, event_id);
                               ...
                               Utilities.logError("Proshop_editevnt.updateEvent - " + club + " - Error migrating event log entries to duplicate event_id. event_id = " 
                                       + event_id + ", dup event_id = " + rsx.getInt("new_id") + " - Error = " + e.toString());
                           } finally {
                               Connect.close(pstmtx3);
                           }
                       ...
                       Utilities.logError("Proshop_editevnt.updateEvent - " + club + " - Error getting event_id for duplicated event record. event_id = " + event_id + " - Error = " + e.toString());
                   } finally {
                       Connect.close(pstmtx2);
                   }
               }
           ...
           Utilities.logError("Proshop_editevnt.updateEvent - " + club + " - Error gathering event info to create copy for our recordxxxxxxs. event_id = " + event_id + " - Error = " + e.toString());
       } finally {
           Connect.close(pstmtx);
   ...
   try {
      ...
      if (signUp == 0) {     // only change the required parms in case they changed signup and want to come back later!!
         ...
         pstmt.setString(39, member_item_code);
         pstmt.setString(40, guest_item_code);
         pstmt.setDouble(41, member_fee);
         pstmt.setDouble(42, guest_fee);
         pstmt.setDouble(43, member_tax);
         pstmt.setDouble(44, guest_tax);

         pstmt.setString(45, oldName);
         ...
         pstmt = con.prepareStatement (
           ...
           "ask_homeclub = ?, ask_phone = ?, ask_address = ?, ask_hdcp = ?, ask_email = ?, ask_gender = ?, ask_shirtsize = ?, ask_shoesize = ?, " +
           "ask_otherA1 = ?, ask_otherA2 = ?, ask_otherA3 = ?, req_guestname = ?, " +
           "req_homeclub = ?, req_phone = ?, req_address = ?, req_hdcp = ?, req_email = ?, req_gender = ?, req_shirtsize = ?, req_shoesize = ?, " +
           "req_otherA1 = ?, req_otherA2 = ?, req_otherA3 = ?, otherQ1 = ?, otherQ2 = ?, otherQ3 = ?, " +
         ...
         pstmt.setInt(153, ask_custom1);
         pstmt.setInt(154, req_custom1);
         pstmt.setString(155, member_item_code);
         pstmt.setString(156, guest_item_code);
         pstmt.setDouble(157, member_fee);
         pstmt.setDouble(158, guest_fee);
         pstmt.setDouble(159, member_tax);
         pstmt.setDouble(160, guest_tax);
         
         pstmt.setString(161, oldName);
      }
   ...
   // Check if certain items have changed, and, if so, add an entry to the event log.
   if (!name.equals(oldName)) {    // Name of event has changed
       Utilities.createEventLogEntry(user, event_id, 0, "CONFIG", "Name of Event changed: \"" + oldName + "\" -&gt; \"" + name + "\"", 0, con);
   }
   
   if (date != oldDate) {    // Date of the event has changed
       
            long yy_old = oldDate / 10000;
            long mm_old = (oldDate - (yy_old * 10000)) / 100;
            long dd_old = oldDate - ((yy_old * 10000) + (mm_old * 100));
       
       Utilities.createEventLogEntry(user, event_id, 0, "CONFIG", "Date of Event changed: " + mm_old + "/" + dd_old + "/" + yy_old + " -&gt; " + month + "/" + day + "/" + year, 0, con);
   }

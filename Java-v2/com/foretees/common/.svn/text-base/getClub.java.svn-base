 ...
 *   last updated:
 *
 *      4/28/14  Check the inactive flag when getting the guest types.
 *      9/21/13  Init the parm strings in getPOS to prevent null errors.
 *      8/08/13  Ironwood CC (ironwood) - Added custom to use the sort_by field to order their guest types.
 *      5/21/13  Fairbanks Ranch CC (fairbanksranch) - Added custom to use the sort_by field to order their guest types.
 *      3/22/13  Updated getParms() so that it will only pull back the hidenames value from club5 if activity_id == 0.
 *      2/22/13  Fort Collins/Greeley/Fox Hill (fortcollins) - Updated their custom guest type processing so that it properly pulls the gOpt, revenue, and use_guestdb values for each guest type.
 *      7/17/12  Old Hawthorne (oldhawthorne) - Added custom to use the sort_by field to order their guest types.
 *      2/29/12  Fort Collins CC (fortcollins) - Updated custom guest type procedure to not apply to FlxRez.
 *      9/21/11  Las Campanas (lascampanas) - custom to display the guest types in order based on the sort_by field in guest5.
 *      6/02/11  The Concession (theconcession) - custom to display the guest types in order based on the sort_by field in guest5.
 *      6/01/11  Olympic Club - custom to display the guest types in order based on the sort_by field in guest5.
 *      3/16/11  Fox Hill CC (fortcollins) - Added guest36 to custom that gathers guest types specific to each course
 *      9/28/10  getPOS - Add salestax fields for CSG.
 *      8/05/10  Fox Hill CC (fortcollins) - Removed extra guest type that was not being used.  Was causing member event signups to be treated as guests.
 ...
 **/
 ...
 public static String getClubName(Connection con) {
    ...
    if (Connect.testConnection(con) == false) {
        ...
        Utilities.logError("Bad connection detected within getClub.getClubName!");
        ...
        try {
            ...
            Utilities.logError("Error getting database name - getClub.getClubName " + e.getMessage() + ", con was detected as good.");
 ...
 **/
    ...
    public static void loadParms(Connection con, parmClub parm, int activity_id) {
        try {
            getParms(con, parm, activity_id);
        } catch (Exception exc) {             // SQL Error
        }
    }
    
    public static parmClub getParms(HttpServletRequest req, int activity_id) {

        // check if we have this parmClub cached in the request
        parmClub parm = reqUtil.getRequestParmClub(req, ProcessConstants.RQA_PARMCLUB + activity_id, null);

        if(parm != null){
            return parm;
        }
        
        // Not cached, get the parm and cache it
        ...
        try {
            ...
            parm = new parmClub(activity_id, con);
            ...
            rs = stmt.executeQuery("SELECT SUM(fives) AS allfives FROM clubparm2");           // check all courses for 5-somes

            if (rs.next()) {
                    parm.fivesomes = rs.getInt("allfives") > 0;
            }

        } catch (Exception exc) {

             Utilities.logError("getClub.getParms: Err=" + org.apache.commons.lang.exception.ExceptionUtils.getStackTrace(exc));
        ...
        req.setAttribute(ProcessConstants.RQA_PARMCLUB + activity_id, parm);
 ...
 public static void getParms(Connection con, parmClub parm)
     ...
     getParms(con, parm, 0, false);      // default to golf and only active guest types
 }
 
 public static void getParms(Connection con, parmClub parm, int activity_id)
         throws Exception {

     getParms(con, parm, activity_id, false);    // get only active guest types
 }
    
 public static void getParms(Connection con, parmClub parm, int activity_id, boolean getAllGuests)
         throws Exception {
   ...
   String inactOpt = "";
   String club = getClubName(con);       // get this club's name
   
   if (getAllGuests == false) {
      
      inactOpt = " AND inactive = 0";   // only get the active guest types
   }
   ...
   try {
      ...
      Utilities.logError("getClub.getParms - init arrays: i = " +i+ ", Club = " +club+ ", Err=" + org.apache.commons.lang.exception.ExceptionUtils.getStackTrace(exc));
   ...
   try {
      ...
      if (rs.next()) {
         ...
         //
         if (parm.club.equals( "fortcollins" ) && activity_id == 0) {     // if Fort Collins or Greeley (shared site)
            ...
            if (parm.course.equals( "Greeley CC" )) {     // if Greeley
               ...
               */
               ...
               parm.guest[1] = rs.getString("guest3");      // Shared: 'Recip'
               parm.guest[2] = rs.getString("guest18");     // Shared: 'Prspt Mbr'
               parm.guest[3] = rs.getString("guest26");
               parm.guest[4] = rs.getString("guest9");
               parm.guest[5] = rs.getString("guest20");
               parm.guest[6] = rs.getString("guest21");
               parm.guest[7] = rs.getString("guest28");
               parm.guest[8] = rs.getString("guest29");
               parm.guest[9] = rs.getString("guest30");
               parm.guest[10] = rs.getString("guest31");
               parm.guest[11] = rs.getString("guest32");
               parm.guest[12] = rs.getString("guest33");
               parm.guest[13] = rs.getString("guest34");
               parm.guest[14] = rs.getString("guest35");
               parm.guest[15] = rs.getString("guest36");
               parm.guest[16] = "";
               parm.guest[17] = "";
               parm.guest[18] = "";
               parm.guest[19] = "";
            ...
            // Grab gOpt, revenue, and use_guestdb values for guest types being used for this club
            try {

                pstmt = con.prepareStatement("SELECT guest, gOpt, revenue, use_guestdb FROM guest5 "
                                           + "WHERE activity_id = ?" +inactOpt);
                ...
                for (int j = 0; j < 22; j++) {
                    ...
                    if (!parm.guest[j].equals("")) {
                        
                        rs2.beforeFirst();
                        while (rs2.next()) {
                            if (rs2.getString("guest").equals(parm.guest[j])) {
                                parm.gOpt[j] = rs2.getInt("gOpt");
                                parm.gRev[j] = rs2.getInt("revenue");
                                if (use_guestdb) parm.gDb[j] = rs2.getInt("use_guestdb");
                            }
                        }  
                ...
                Utilities.logError("getClub.getParms - fortcollins (" + parm.course + ") - Error looking up gOpt values for gtypes - ERR: " + exc.toString());
            } finally {
            ...
            if (parm.club.equals( "olyclub" ) || parm.club.equals("theconcession") || parm.club.equals("lascampanas") || parm.club.equals("oldhawthorne") 
                    || parm.club.equals("fairbanksranch") || parm.club.equals("ironwood") || parm.club.equals("semiahmoo")) {     // use specific order for Olympic Club

               pstmt = con.prepareStatement("SELECT guest, gOpt, revenue, use_guestdb FROM guest5 "
                                          + "WHERE activity_id = ?" +inactOpt+ " ORDER BY sort_by, guest"); 

            } else {
               
               pstmt = con.prepareStatement("SELECT guest, gOpt, revenue, use_guestdb FROM guest5 "
                                          + "WHERE activity_id = ?" +inactOpt);       //  LIMIT " + parm.MAX_Guests
            } 
         ...
         parm.use_memberPhotos = (rs.getInt("use_memberPhotos") == 1);
      ...
      Utilities.logError("getClub.getParms: i = " +i+ ", Club = " +club+ ", Err=" + org.apache.commons.lang.exception.ExceptionUtils.getStackTrace(e));
   ...
   // Hack for dining guests until proshop configuration of dining guests is complete
    if(i == 0 && activity_id == ProcessConstants.DINING_ACTIVITY_ID && parm.dining_guests.length > 0){
        for(int c = 0; c < parm.dining_guests.length; c++){
            parm.guest[c] = parm.dining_guests[c];
            parm.gOpt[c] = 0;
            parm.gRev[c] = 1;
            parm.gDb[c] = 0;
        }
        parm.MAX_Guests = parm.dining_guests.length;
    }
 ...
 **/
 ...
 public static void getPOS(Connection con, parmPOS parm, String course, int activity_id)
   ...
   //
   //  Init some parm strings to prevent null errors
   //
   for (i=0; i<parm.MAX_Guests; i++) {
   ...
   try {
      ...
      //
      // Now get the guest parms for each guest type specified (different table) - get ALL guests for POS charges (act & inact)
      //
      ...
      while (rs.next()) {
          ...
          parm.salestaxg[i] = rs.getDouble("salestax");
      ...
      while (rs.next()) {
          ...
          parm.salestaxm[i] = rs.getDouble("salestax");
      ...
      if (rs.next()) {
         ...
         parm.salestaxt[0] = rs.getDouble("tmode1_salestax");
         parm.salestaxt[1] = rs.getDouble("tmode2_salestax");
         parm.salestaxt[2] = rs.getDouble("tmode3_salestax");
         parm.salestaxt[3] = rs.getDouble("tmode4_salestax");
         parm.salestaxt[4] = rs.getDouble("tmode5_salestax");
         parm.salestaxt[5] = rs.getDouble("tmode6_salestax");
         parm.salestaxt[6] = rs.getDouble("tmode7_salestax");
         parm.salestaxt[7] = rs.getDouble("tmode8_salestax");
         parm.salestaxt[8] = rs.getDouble("tmode9_salestax");
         parm.salestaxt[9] = rs.getDouble("tmode10_salestax");
         parm.salestaxt[10] = rs.getDouble("tmode11_salestax");
         parm.salestaxt[11] = rs.getDouble("tmode12_salestax");
         parm.salestaxt[12] = rs.getDouble("tmode13_salestax");
         parm.salestaxt[13] = rs.getDouble("tmode14_salestax");
         parm.salestaxt[14] = rs.getDouble("tmode15_salestax");
         parm.salestaxt[15] = rs.getDouble("tmode16_salestax");
      }
      pstmtc.close();
 ...
 public static mTypeArrays getMtypes(mTypeArrays mArrays, int activity_id, Connection con)
   ...
   try {
      ...
      if(club.equalsIgnoreCase("lacumbrecc")) {          
          sql = "SELECT mship FROM mship5 WHERE activity_id = ? AND mship != 'Social Clubhouse' LIMIT ";
      } else {
          sql = "SELECT mship FROM mship5 WHERE activity_id = ? LIMIT ";
      }

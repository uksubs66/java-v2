 ...
 *   last updated:
 *
 *      2/07/14   Allow the generating of login page symlinks on dev server as well.
 *      2/06/14   Add golfnet source club id
 *      5/30/13   Updated symlink paths
 *      4/12/13   Add memberInact processing to allow us to disable member access from websites and our login page.
 *      3/29/13   Put the Insert club5 statement back in.  It was incorrectly removed.
 *      3/28/13   Add option to set FT Connect (Standard, not Premier) for club.
 *      1/30/13   Updated all calls to "support_main.htm" to point to "servlets/Support_main" instead.
 *      1/30/13   The "Create Symlinks" and "Sync Servers" options will now be visible on all nodes. (possibly temporarily)
 *      1/20/12   Removed the bounce nodes options from the page.
 *      5/06/11   Added flxrez_staging option for use prior to a club going live to their members with FlxRez.
...
public class Support_cluboptions extends HttpServlet {
 ...
 //*********************************************************************************
 // Process the request from servlet/Support_main
 //*********************************************************************************
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   if (exists == false ) {             // create entry if club5 does not exist yet
      ...
      try {
         ...
         PreparedStatement pstmt1 = con.prepareStatement (
            ...
            "rsync, seamless, zipcode, primaryif, mnum, mapping, stripzero, allow_responsive) " +
            "VALUES ('',0,0,'','', " +
            "'','','','','','','','','','','','','','','','','','','','','','','','', " +
            "'','','','','','','','','','','','','','','','','','','','','','','','', " +
            "0,0,'',0,0,0,0, " +
      ...
      try {
         ...
         if (rs.next()) {
            ...
            notification = rs.getInt("no_reservations");
            rsync = rs.getInt("rsync");
            seamless = rs.getInt("seamless");
            zipcode = rs.getString("zipcode");
            primaryif = rs.getInt("primaryif");
            mnum = rs.getInt("mnum");
            mapping = rs.getInt("mapping");
            stripzero = rs.getInt("stripzero");
            lottery_text = rs.getString("lottery_text");
            foretees_mode = rs.getInt("foretees_mode");
            genrez_mode = rs.getInt("genrez_mode");
            flxrez_staging = rs.getInt("flxrez_staging");
   ...
   out.println("<table border=\"1\" bgcolor=\"#F5F5DC\" width=\"550\">");
      ...
      out.println("<form action=\"Support_cluboptions\" method=\"post\">");
      ...
      out.println("<tr><td align=\"left\" width=\"300\">");
      out.println("<font size=\"2\"><br>");
      out.println("&nbsp;&nbsp;FlxRez Staging Mode (Hide 'Activities' Link)?:&nbsp;&nbsp;");             // GenRez
      out.println("</td><td align=\"left\">");
      out.println("<select size=\"1\" name=\"flxrez_staging\">");
      if (flxrez_staging == 0) {
         out.println("<option selected value=\"No\">No</option>");
         out.println("<option value=\"Yes\">Yes</option>");
      } else {
      ...
      out.println("<tr><td align=\"left\" width=\"300\">");
      out.println("<font size=\"2\"><br>");
      out.println("&nbsp;&nbsp;FT Connect Site?:&nbsp;&nbsp;");           // FT Connect Site
      out.println("</td><td align=\"left\">");
      out.println("<select size=\"1\" name=\"ftConnect\">");
      out.println("<option " + (ftConnect == 0 ? "selected " : "") +" value=\"0\">No</option>");
      out.println("<option " + (ftConnect == 1 ? "selected " : "") +"value=\"1\">Yes - Live</option>");
      out.println("<option " + (ftConnect == 2 ? "selected " : "") +"value=\"2\">Yes - Staging</option>");
      out.println("</select>");
      out.println("</td></tr>");

      out.println("<tr><td align=\"left\" width=\"300\">");
      out.println("<font size=\"2\"><br>");
      out.println("&nbsp;&nbsp;Allow Member Access?:&nbsp;&nbsp;");           // Member Access
      out.println("</td><td align=\"left\">");
      out.println("<select size=\"1\" name=\"memberInact\">");
      if (memberInact == 1) {
         out.println("<option selected value=\"No\">No</option>");
         out.println("<option value=\"Yes\">Yes</option>");
      } else {
      ...
      out.println("</select>&nbsp;&nbsp;NO blocks ALL members!");
      out.println("</td></tr>");
...
//      out.println("<tr><td align=\"left\" width=\"300\">");
//      out.println("<font size=\"2\"><br>");
//      out.println("&nbsp;&nbsp;Full Club Name:&nbsp;&nbsp;");
//      out.println("</td><td align=\"left\">");
//      out.println("<input type=\"text\" id=\"clubName\" name=\"clubName\" value=\"" +clubName+ "\" size=\"30\" maxlength=\"50\">");
//      out.println("</font></td></tr>");
      ...
      out.println("<tr><td align=\"left\" width=\"300\">");
      out.println("<font size=\"2\"><br>");
      out.println("&nbsp;&nbsp;Website:&nbsp;&nbsp; (example: www.foretees.com)");
      out.println("</td><td align=\"left\">");
      out.println("http://<input type=\"text\" id=\"website\" name=\"website\" value=\"" +website_url+ "\" size=\"20\" maxlength=\"100\" onchange=\"myFunction(this.form.website.value)\">");
      out.println("</font></td></tr>");
      ...
      out.println("<tr><td align=\"left\" width=\"300\">");
      out.println("<font size=\"2\"><br>");
      out.println("&nbsp;&nbsp;GolfNet Source Club ID:&nbsp;&nbsp;");
      out.println("</td><td align=\"left\">");
      out.println("<input type=\"text\" id=\"gn21_sourceClubId\" name=\"gn21_sourceClubId\" value=\"" +gn21_sourceClubId+ "\" size=\"16\" maxlength=\"16\">");
      out.println("</font></td></tr>");
      
      out.println("<tr><td colspan=2 align=\"center\">");
      out.println("<font size=\"2\"><form method=post action=\"Support_cluboptions\" name=\"supportform\">");
   ...
   out.println("<BR><BR> <A HREF=\"/" +rev+ "/servlet/Support_main\">Return - Cancel</A>");
   out.println("</CENTER></BODY></HTML>");
   out.close();
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   if (req.getParameter("symlinks") != null  || req.getParameter("syncwebapps") != null ||
       req.getParameter("doNodeBounce") != null || req.getParameter("doNodeBounceSkip1") != null) {

       // make sure we're on node 1 or notify user
       if ((Common_Server.SERVER_ID >= 11 && Common_Server.SERVER_ID <= 13) || ProcessConstants.isDevServer() || ProcessConstants.isTestingServer() 
               || ProcessConstants.isStagingServer()) {

           if (req.getParameter("symlinks") != null) {

               doSymlinks(club, out);
               return;

           } else if (req.getParameter("syncwebapps") != null) {

               doSyncwebapps(out);
               return;

           } else if (req.getParameter("doNodeBounce") != null) {

               doNodeBounce(false, out);
               return;

           } else if (req.getParameter("doNodeBounceSkip1") != null) {

               doNodeBounce(true, out);
               return;
           ...
           out.println("<p align=center><b>NOT ON NODE 1 - PLEASE USE THIS FEATURE ON NODE 1</b></P>");
   ...
   temp = req.getParameter("flxrez_staging");

   if (temp.equals( "Yes" )) {
      flxrez_staging = 1;
   }
   
   ftConnect = Integer.parseInt(req.getParameter("ftConnect"));
   
   temp = req.getParameter("memberInact");
   
   if (temp.equals( "No" )) {          // question is "Allow Member Access?"
      memberInact = 1;
   }   
...
//   clubName = req.getParameter("clubName");
   ...
   gn21_sourceClubId = req.getParameter("gn21_sourceClubId");
   ...
   try {
      ...
      PreparedStatement pstmt = con.prepareStatement (
         "UPDATE club5 SET "/*clubName = ?, */ + "no_reservations = ?, rsync = ?, seamless = ?, zipcode = ?, primaryif = ?, " +
         "mnum = ?, mapping = ?, stripzero = ?, lottery_text = ?, foretees_mode = ?, genrez_mode = ?," +
         "flxrez_staging = ?, allow_mobile = ?, website_url = ?, stripalpha = ?, stripdash = ?, " +
         "ftConnect = ?, memberInact = ?, gn21_sourceClubId = ?");

      int i = 1;
      pstmt.clearParameters();            // clear the parms
//      pstmt.setString(i++, clubName);
      pstmt.setInt(i++, notification);
      pstmt.setInt(i++, rsync);
      pstmt.setInt(i++, seamless);
      pstmt.setString(i++, zipcode);
      pstmt.setInt(i++, primaryif);
      pstmt.setInt(i++, mnum);
      pstmt.setInt(i++, mapping);
      pstmt.setInt(i++, stripzero);
      pstmt.setString(i++, lottery_text);
      pstmt.setInt(i++, foretees_mode);
      pstmt.setInt(i++, genrez_mode);
      pstmt.setInt(i++, flxrez_staging);
      pstmt.setInt(i++, mobile);
      pstmt.setString(i++, website_url);
      pstmt.setInt(i++, stripalpha);
      pstmt.setInt(i++, stripdash);
      pstmt.setInt(i++, ftConnect);
      pstmt.setInt(i++, memberInact);
      pstmt.setString(i++, gn21_sourceClubId);
 ...
 private void switchClubs(HttpServletRequest req, PrintWriter out) {
    ...
    if (req.getParameter("doSwitch") != null) {
        ...
        out.println("<p align=\"center\"><font size=\"2\" face=\"Arial, Helvetica, Sans-serif\"><a href=\"/" + rev + "/servlet/Support_main\" target=\"_top\">Main Menu</a></font></p>");
 ...
 private void doSymlinks(String club, PrintWriter out) {
    ...
    try {
        ...
        String cm = "ln -s ../ROOT/login.jsp /srv/webapps/" + club + "/";
        Process p = Runtime.getRuntime().exec(cm);
        out.println("<pre>" + cm + "</pre>");

        cm = "ln -s ../ROOT/mlogin.jsp /srv/webapps/" + club + "/";
        p = Runtime.getRuntime().exec(cm);
        out.println("<pre>" + cm + "</pre>");

        cm = "ls -lh /srv/webapps/" + club;
        p = Runtime.getRuntime().exec(cm);
        out.println("<pre>" + cm + "</pre>");
    ...
    out.println("<p align=\"center\"><font size=\"2\" face=\"Arial, Helvetica, Sans-serif\"><a href=\"Support_cluboptions\" target=\"_top\">Club Options</a></font></p>");
    out.println("<p align=\"center\"><font size=\"2\" face=\"Arial, Helvetica, Sans-serif\"><a href=\"/" + rev + "/servlet/Support_main\" target=\"_top\">Main Menu</a></font></p>");
 ...
 private void doNodeBounce(boolean skip1, PrintWriter out) {
    ...
    try {
        ...
        out.println("<html>");
        out.println("<head>");
        out.println("<title>Bounce Tomcat Nodes</title>");
        out.println("</head>");
        out.println("<body>");

        String cm = (skip1) ? "/root/scripts/bounce_skip1.sh" : "/root/scripts/bounce_nodes.sh";

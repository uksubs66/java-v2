...
*/           
            ...
            mNum = mNum.substring(0, mNum.length() - 2); // Strip the C# off the end so the mnum can be matched with the parents
            
            PreparedStatement mstmt = con.prepareStatement("" +
                    "SELECT m_ship FROM member2b where username = ?");
            mstmt.clearParameters();
            mstmt.setString(1, mNum);
            
            ResultSet mrs = mstmt.executeQuery();
            
            if (mrs.next()) {
                mship = mrs.getString("m_ship");
            } else {
                mship = "Unknown";
            }

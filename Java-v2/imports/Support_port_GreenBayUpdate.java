...
//            mtype = toTitleCase( mtype );
            ...
            // If username exists in system, update that member's record, otherwise insert a new one
            pstmt5 = con.prepareStatement("SELECT inact FROM member2b WHERE username = ? OR (username <> ? AND name_first = ? AND name_last = ? AND name_mi = ?)");
            pstmt5.clearParameters();
            pstmt5.setString(1, mem_id);
            pstmt5.setString(2, mem_id);
            pstmt5.setString(3, fname);
            pstmt5.setString(4, lname);
            pstmt5.setString(5, mi);
            ...
            if (rs5.next()) {
                ...
                try {
                    pstmt6 = con.prepareStatement("UPDATE member2b set name_first = ?, name_last = ?, name_mi = ?, memNum = ?, m_ship = ?, m_type = ?, " +
                            "email = ?, bag = ?, phone1 = ?, gender = ?, pri_indicator = ?, posid = ?, inact = 0 " +
                            "WHERE username = ? OR (username <> ? AND name_first = ? AND name_last = ? AND name_mi = ?)");
                    pstmt6.clearParameters();
                    pstmt6.setString(1, fname);
                    pstmt6.setString(2, lname);
                    pstmt6.setString(3, mi);
                    pstmt6.setString(4, mNum);
                    pstmt6.setString(5, mship);
                    pstmt6.setString(6, mtype);
                    pstmt6.setString(7, email);
                    pstmt6.setString(8, bag);
                    pstmt6.setString(9, phone);
                    pstmt6.setString(10, gender);
                    pstmt6.setInt(11, primary);
                    pstmt6.setString(12, posid);

                    pstmt6.setString(13, mem_id);
                    pstmt6.setString(14, mem_id);
                    pstmt6.setString(15, fname);
                    pstmt6.setString(16, lname);
                    pstmt6.setString(17, mi);

                    updatecount = pstmt6.executeUpdate();

                    pstmt6.close();

                    if (updatecount == 0) {
                        out.println("<br>Error updating user: " + mem_id + " " + lname + ", " + fname + " " + mi);
                    }
                } catch (Exception exc) {
                    out.println("<br>Error updating user: " + mem_id + " " + lname + ", " + fname + " " + mi + " ERR: " + exc.getMessage());
                }
            } else {

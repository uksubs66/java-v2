 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   //out.println("<font size=\"2\">");
      ...
      if (category_ids.size() > 0) {      // Only proceed if at least one event category was found
          ...
          for (int q = 0; q<selected_category_ids.size(); q++) {
          }
          ...
          out.println("<tr>");
          if (club.equalsIgnoreCase("beechmontcc")) {
              cols_per_row = 4;
          }
   ...
   try {
      ...
      String sql = "" +
         ...
         "AND activity_id = " + sess_activity_id + " AND signup_on_teesheet = 0 AND inactive = 0 ";
         if (sortBy.equals("sudate")) {         // sort by signup date, name
            ...
            sql += "ORDER BY " + (sortBySeason ? "season DESC, " : "") + "date2 " + ((sortDesc) ? "DESC" : "") + ", act_hr, act_min;";
      ...
      while (rs.next()) {
         ...
         if (signUp != 0 || max != 0 || gstOnly != 0) {
            ...
            if (!club.equals("tcclub") && !club.equals("merion") && !club.equals("ccrockies") && (sess_activity_id == 0 || sess_activity_id == dining_activity_id)) {

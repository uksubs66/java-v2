 ...
 *
 *    6/03/14   Oak Hill CC (oakhillcc) - add custom instructions to the Guest selection modal window.
...
public class Common_guestdb extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if(session != null) {
            ...
            user = (String) session.getAttribute("user");     // get username ('proshop' or member's username)
            //String club = (String)session.getAttribute("club");
            con = Connect.getCon(req);            // get DB connection

            if (ProcessConstants.isProshopUser(user)) {
                isProshop = true;
            } else if (con != null && Utilities.isHotelUser(user, con)) {
                isProshop = true;
                isHotel = true;
            } else {
                // Member - we need to use verify Mem if we're a member
                session = SystemUtils.verifyMem(req, out); 
                isMember = true;
            }
        }
...
*/
    ...
    public static void displayGuestConfig(HttpServletRequest req, HttpSession session, int activity_id, boolean isProshop, String user, String result, PrintWriter out, Connection con) {
        ...
        if (!Utilities.isGuestTrackingConfigured(activity_id, con)) {
            ...
            out.println("<div class=\"rwdOldWrap\" style=\"margin-left:auto; margin-right:auto; width:100%;\">");
    ...
    private void displayMergeForm(HttpServletRequest req, HttpSession session, int activity_id, boolean isProshop, String user, String result_msg, PrintWriter out, Connection con) {
        ...
        // **NOTE** Had to remove the #guest_id component of the link URL, since it causes the Return button to break after successfully merging a guest
//        out.println("<tr><td align=\"center\" colspan=\"2\"><br><button class=\"btnNorm\" onclick=\"location.href='Common_guestdb" + (guest_id != 0 ? "#" + guest_id : "") + "'\">Return</button></td></tr>");  
        out.println("<tr><td align=\"center\" colspan=\"2\"><br><button class=\"btnNorm\" onclick=\"location.href='Common_guestdb'\">Return</button></td></tr>");
        out.println("</table>");
    ...
    public static Map<String, Object> generateGuestInfoFields(HttpServletRequest req, boolean isProshop, boolean reqOnly, int activity_id, PrintWriter out, Connection con, boolean generateMap) {
        ...
        if (!use_email.equals("N") && (!reqOnly || use_email.equals("R"))) {
            if (generateMap) {
                message_list.add("Note: If an email address is entered, this guest will receive " + (sess_activity_id == 0 ? "tee time" : "activity") + " notifications.");
            } else {
                out.println("<tr><td colspan=\"2\" style=\"text-align:center; color:red; font-size:small;\">Note: If an email address is entered, this guest will receive " + (sess_activity_id == 0 ? "tee time" : "activity") + " notifications.<br><br></td></tr>");
            }
        }
            ...
            if (existingUser) {
                ...
                if (count > 0) {
                    ...
                    if (generateMap) {
                        ...
                        guest_map.put("first", name_first);
                        guest_map.put("middle", name_mi);
                        guest_map.put("last", name_last);
    ...
    private static Map<String, Object> loadModal(HttpServletRequest req, HttpSession session, int activity_id, String user, PrintWriter out, Connection con, boolean generateMap) {
        ...
        String club = Utilities.getClubDbName(con);      // get this club's name (db name)
...
/*
        ...
        if (club.equals("oakhillcc")) customInstruct = "In an effort to better serve your guests upon their arrival to Oak Hill, we are asking for your support in supplying us with the names of "
                                            + "your guests when booking tee times.<BR><BR>On behalf of the staff at Oak Hill, we appreciate your cooperation.<BR><BR>";
        
        if (generateMap) {
            if(rwd){
                message_list.add(customInstruct+ "Please select an existing guest from the Search Guests tab, or add a new guest using Add New Guest.");
            } else {
                message_list.add(customInstruct+ "Please select an existing guest from the list to the right, or add a new guest using the form below.");
            }
        } else {
            out.println(customInstruct+ "Please select an existing guest from the list to the right, or add a new guest using the form below.");
        }

 ...
 * @author Owner
 */
public class Dining_slot2 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest req, HttpServletResponse resp)
        ...
        slotPageParms.slot_url = "Dining_slot2";
        ...
        slotPageParms.page_title = "Dining Registration";
        slotPageParms.bread_crumb = "Dining Registration";
    ...
    private void processSlot(int id, int event_id, parmSlotPage slotPageParms, HttpServletRequest req, Connection con_d) {
        ...
        if (cparmD.stage == parmDining2.REMOVE_SELF) {
            ...
            if (parmD.id > 0 && my_index != null && my_releated_id != null && my_releated_id > 0 
                ...
                String reason = "Removed self: " + name;
        ...
        if (cparmD.stage == parmDining2.DELETE) {
            List<String> result = new ArrayList<String>();
            if (can_delete) {
                String reason = "Cancelled by " + name;
        ...
        if (cparmD.stage == parmDining2.PROCESS) {
            ...
            if (errors.isEmpty()) {
                ...
                if (result.isEmpty()) {
                    // removal sucessful
                    success.add("Thank you! Your reservation has been accepted and processed.");
                } else {
        ...
        //
        if (parmD.id > 0 && ( //parmD.member_created == false ||
                lockReason != null || 
                (
                !parmD.owner.equalsIgnoreCase(user) 
            ...
            slotPageParms.page_start_button_go_back = true;
            if(diningUtil.isUserReservee(user, parmD) && lockReason == null){
        ...
        if (cparmD.stage == parmDining2.PROMPT) {
            ...
            for (diningLocation loc : locations) {
                ...
                if (closed) {
                    closed_locations.add(formUtil.radioSelect("location_id_" + loc.id, "reservation[location_id]", loc.name + " <span class=\"smallSign\">Closed</span>", "", cparmD.location_id == loc.id || locations.size() == 1, "warning"));
                    closed_locations.add(formUtil.startRadioBlock("location_block_" + loc.id, "", ""));
                    closed_locations.add(formUtil.fieldHtml(
                            "<div class=\"sub_instructions\">Sorry, " + loc.name + " is closed on this date.  Please select a different date or location.</div>"));
                    ...
                    locations_object.add(formUtil.select("covers_" + loc.id, "covers", "Dining Party Size", Integer.toString(covers), partySizeMap));
    ...
    private List<String> getClubMessage(parmDining2 cparmD, parmDining2 parmD, HttpServletRequest req) {
        ...
        if (club.equals("mirabel")) {
            ...
            message.append("<ul>");
            message.append("<li>The Bar area of the Club is called Mulligans and the casual dining room is the Grill, please be cautious when making reservations.</li>");
            ...
            message.append("<ul>");
            message.append("<li>Thank you very much for arriving at the Club prior to your reservation time if you would like to enjoy a cocktail. In order to serve and provide a quality experience for as many members as possible, we will politely request your presence in the dining room at your reservation time.</li>");

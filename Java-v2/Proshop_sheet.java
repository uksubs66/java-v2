 ...
 *   last updated:
 * 
 *        6/30/14   Do not include the "Check In All' option in the Control Panel if using the Abacus21 Direct POS I/F.
 *        4/17/14   Allow for 'Jonas Generic' POS type.
 ...
 *        3/19/14   Interlachen (interlachen) - Removed custom to always display suspended restrictions in the tee sheet legend.
...
//             
            ...
            SystemUtils.logError("Proshop_sheet - exception from Proshop_sheet_reports. Error = " +exp.getMessage()+ ", and: " + exp.toString());
            return;
         }
      }
   }
   ...
   //
     if (req.getParameter("notes") != null || req.getParameter("event") != null || req.getParameter("rest") != null
             ...
             || req.getParameter("prtTeeTime") != null || req.getParameter("prtReceipt") != null || req.getParameter("prtStarterReceipt") != null 
             || req.getParameter("insert") != null || req.getParameter("prtcust") != null) {
   ...
   // Print a generic starter receipt with no POS information
   if (req.getParameter("prtStarterReceipt") != null) {
      Proshop_sheet_reports.printStarterReceipt(req, out, session);
      return;
   }
   ...
   if (req.getParameter("prtcust") != null) {
      String course = req.getParameter("course");
      String templott = (String)session.getAttribute("lottery");        // get lottery support indicator
      int lotteryS = Integer.parseInt(templott);           
      printCustomTee(req, resp, out, session, con);      // go prompt user
      return;
   }
   ...
   int isPosPayNow = Utilities.isPosPayNow(req);    // Set to 1 if using standard PayNow option, 2 if using Advanced PayNow options
   ...
   boolean prtRecpt = false;           // custom flag for print receipt option
   boolean prtStarterRecpt = false;
   boolean useEditTools = false;
   ...
   if (club.startsWith("demo") && updateAccess) useEditTools = true;     // just demo sites for now - turn on for all with update access after tested
   ...
   if (club.equalsIgnoreCase("dmgcc")) {
       ...
       colors.course_color[0] = "#1E90FF";
       colors.course_color[1] = "#DC143C";
   }
   ...
   //**********************************************************************************
   //
   int [] wedtimes = {820, 830, 840, 850, 900, 910, 920, 930, 940, 950, 1000, 1010, 1020, 1030, 1040, 1050, 1100, 1110, 1120, 1130, 1140, 1150, 1430, 1440, 1450, 1500};
...
//   int [] fritimes = { 830, 840, 910, 920, 940, 1010, 1020, 1040, 1110, 1120, 1140, 1150 };
         ...
         if (courseName1.equals( "" )) {
            ...
            if (club.equals( "foresthighlands" )) {         // change for Forest Highlands
               ...
               if (user.startsWith( "proshopa" )) {  
                  ...
                  courseName1 = "Cochise";      // actually does Geronimo too
   ...
   // restrict checkin access for Pro-ShopKeeper or PCS Group Check-In users that do not have POS access
   if (!posAccess && (parmp.posType.equalsIgnoreCase( "Pro-ShopKeeper" ) || parmp.posType.equals( "ClubProphetV3" ) || 
           parmp.posType.equals( "PCS Group Check-In" ))) {
   ...
   //
   //  Determine if the 'Print Receipt' option to be used
   //
   if ((club.equals("olyclub") || club.equals("pgavillage")) && index == 0) {
       prtRecpt = true;
   } else if (club.equals("mosscreek")) {
       prtStarterRecpt = true;
   }
   
    if (advance_days > 90 && club.equals("kiawahislandclub") && !user.equalsIgnoreCase("proshopkelleher") && !user.equalsIgnoreCase("proshopfrost") && !user.equalsIgnoreCase("proshoptownsend")) {
        Utilities.logDebug("WDH","");
        SystemUtils.restrictProshop("TS_VIEW", out);
    }
   ...
   if (club.equals("overlakegcc") || club.equals("bluehillscc") || club.equals("bloomfieldhillscc")) {
   ...
   try {
      ...
      // Get restrictions for this date and course
      try {
          ...
          while (rs.next() && i < count_rest) {
             ...
             rest = rs.getString("name");
             rest_recurr = rs.getString("recurr");
             rcolor = rs.getString("color");
          ...
          Utilities.logError("Proshop_sheet.doPost - " + club + " - Failed to load restriction data - Error = " + e.toString());
      } finally {
      ...
      try {
          ...
          pstmt2 = con.prepareStatement(
                  "SELECT name, color, act_hr, act_min "
                  + "FROM events2b "
          ...
          while (rs.next() && i < count_event) {
             ...
             event_start_hrA[i] = rs.getInt("act_hr");
             event_start_minA[i] = rs.getInt("act_min");
          ...
          Utilities.logError("Proshop_sheet.doPost - " + club + " - Failed to load event data - Error = " + e.toString());
      } finally {
          Connect.close(rs, pstmt2);
      }


      //  Get the lotteries for this date and course
      if (lotteryS > 0) {               // if lottery supported
          ...
          try {
              ...
              Utilities.logError("Proshop_sheet.doPost - " + club + " - Failed to load lottery data - Error = " + e.toString());
          } finally {
              Connect.close(rs, pstmt2);
          }
      ...
      */
      ...
      StatCounters.sheetCountsPro[hourCount]++;
      ...
      if (club.equals("waialae") && user.equalsIgnoreCase("proshop1")) {
          
         if (hourCount >= 10 && hourCount <= 22) {        // if between 10 AM and 6 PM CT (5 or 6 AM - 5 or 6 PM Hawaii Time)

            autoRefresh = true;                        // use auto refresh
            arSecs = 30;                               // every 30 secs
         }
      }
      
      if (club.equals("bloomcc") && user.equalsIgnoreCase("proshopstaff")) {
         if (hourCount >= 5 && hourCount <= 21) {        // if between 5 AM and 9 PM CT (6 AM - 10 PM ET)
     ...
     /*
     if (club.equals("desertmountain") && user.startsWith("proshopcg") && (courseName1.equals("Geronimo") || courseName1.equals("Cochise"))) {
     ...
     out.println(" var offset = 325;");
     out.println(" var duration = 250;");
     out.println(" jQuery(window).scroll(function() {");
     out.println("  if (jQuery(this).scrollTop() > offset) {");
     out.println("   jQuery('#scrollUp').fadeIn(duration);");
     out.println("  } else {");
     out.println("   jQuery('#scrollUp').fadeOut(duration);");
     out.println("  }");
     out.println(" });");

     out.println(" jQuery('#scrollUp').click(function(event) {");
     out.println("   event.preventDefault();");
     out.println("   jQuery('html, body').animate({scrollTop: 0}, duration);");
...
*/
     ...
     out.println("<tr valign=\"top\"><td align=\"center\" rowspan=\"2\" width=\"150\">");
         ...
         if (!club.equals("racebrook") || !user.equalsIgnoreCase("proshopview")) {
             ...
             out.println("<a href=\"Proshop_sheet?index=" +num+ "&course=" +courseName1+ "\" title=\"Refresh This Page\" alt=\"Refresh\">");
             ...
             if (posAccess) {
                 ...
                 if ((parmp.posType.equals( "Abacus21" ) || parmp.posType.startsWith( "Jonas" ) || parmp.posType.equals( "NorthStar" ) || parmp.posType.equals( "ClubSystems Group" ) ||
                      ...
                      parmp.posType.equals( "PCS Group Batch" )) &&
                      index == 0) {
             ...
             if (!club.equals( "demov4" ) || !user.equalsIgnoreCase( "proshop5" )) {     // if NOT proshop5 on demov4 site (ProshopKeeper Testing)
                ...
                if (index == 0) {            // only display when tee sheet is on current day and if user has reports access
                    ...
                    if (SystemUtils.verifyProAccess(req, "REPORTS", con, out)) {
                        ...
                        if (club.equals("carolinacc")) {
                            String font_color = "orangered";
                            
                            if (Utilities.checkDiaryEntry(date, con)) {
                                font_color = "Limegreen";
                            }
                            
                            out.println("<tr><td align=\"center\"><font size=\"2\"><a href=\"javascript:void(0)\" onclick=\"openDiaryWindow(); return false;\" style=\"color:" + font_color + "\" title=\"Diary\" alt=\"Diary\">");
                ...
                if (SystemUtils.verifyProAccess(req, "TOOLS_EMAIL", con, out) && enableAdvAssist) {
                    ...
                    if (Utilities.allowMobileApp(club, con)) {
                        out.println("<tr><td align=\"center\"><font size=\"2\">");
                        out.println("<a href=\"javascript:sendPush('Send_email', 'addToList')\" title=\"Send Push Notifications To All Members On This Tee Sheet\" alt=\"Push Notification\">");
                        out.println("Send Push Notification</a></td></tr>");
                    }
                }
                ...
                out.println("<input type=\"hidden\" name=\"initialRequest\" value=\"yes\">");
                ...
                if (index == 0 && checkinAccess && !parmp.posType.equals( "Pro-ShopKeeper" ) && !parmp.posType.equals( "ClubProphetV3" ) && 
                    !parmp.posType.equals( "Abacus21 Direct" ) && !parmp.posType.equals( "PCS Group Check-In" )) {    // only display when tee sheet is on current day and POS is not Pro-ShopKeeper and user has checkin access
             ...
             if (lotteryS > 0) {               // if lottery supported
                ...
                if (!lottA[0].equals("") && lottApproveAccess && ((club.equals("theforestcc") || club.equals("gallerygolf")) || club.equals("bhamcc") 
                        ...
                        || club.equals("dovecanyonclub")  || club.equals("ballenisles") || club.equals("wyndemere") || club.equals("desertmountain") || club.equals("rioverdecc") 
                        || club.equals("esterocc") || club.equals("terravita") || club.equals("fourhillscc") || club.startsWith("demo") || club.equals("kahkwa")
                        || club.equals("bonniebriar") || club.equals("reserveclubatwoodside") || club.equals("westchester"))) {     // if Pro has access and any lotteries defined
             ...
             if (club.equals("quailridgecc")){
                out.println("<a href=\"Proshop_sheet?prtcust=yes&date="+date+"&date=" + date + "&day=" + day_name + "&course="+ courseName1 +"\" title=\"Print Custom Sheet\" target=\"_\" alt=\"Print Custom Sheet\">");
                out.println("Print Custom Report</a>");
                out.println("</font></td></tr><tr><td align=\"center\"><font size=\"2\">");
             }
             if (club.equals("demowill") || club.equals("theoaksclub")){
                out.println("<a href=\"Proshop_sheet_reports?printShotgunTee=yes&date="+date+"&course="+ courseName1 +"\" title=\"Print Shotgun Sheet\" target=\"_\" alt=\"Print Shotgun Sheet\">");
                out.println("Print Shotgun Report</a>");
                out.println("</font></td></tr><tr><td align=\"center\"><font size=\"2\">");
             }
     ...
     out.println("<script type=\"text/javascript\">");
      ...
      out.println("w.creator = self;");
      out.println("}");
      out.println("function openTrashWindow(tid, index) {");
      out.println("w = window.open ('Proshop_cancel?tid=' +tid+ '&index=' +index,'pacePopup','width=560,height=320,scrollbars=1,dependent=0,directories=0,location=0,menubar=0,resizable=1,status=0,toolbar=0');");
      out.println("w.creator = self;");
      out.println("}");
     ...
     if (club.equals("demowill")) {
         String nameLast = "";
         String nameFirst = "";
         String nameMi = "";
         String userName = "";
         out.println("<br><br><table width=\"150\" border=\"1\" cellspacing=\"3\" cellpadding=\"3\" bgcolor=\"8B8970\" align=\"center\">");
         out.println("<tr><td align=\"center\"><font color=\"#000000\" size=\"3\"><b>Guest Quota</b><br></font></td></tr>");
         out.println("<tr><td>");
         ...
         try {
              ...
              ResultSet rsQuota = null;
              PreparedStatement prepQuota = con.prepareStatement(
                      "SELECT username, name_first, name_last, name_mi " +
                      "FROM member2b " +
                      "ORDER BY name_last;");

              prepQuota.clearParameters();        // clear the parms and check player 1
              rsQuota = prepQuota.executeQuery();      // execute the prepared stmt

              while (rsQuota.next()) {
                  nameFirst = rsQuota.getString("name_first");
                  nameMi = rsQuota.getString("name_last");
                  nameLast = rsQuota.getString("name_last");
                  userName = rsQuota.getString("username");
                  out.println("<option value=" + userName + ">" + nameLast + ", " + nameFirst + " " + nameMi + "</option>");
         ...
         out.println("<td nowrap style=\"padding:7px\" align=center>");
         out.println("<a href=\"javascript:void(0)\" onClick=\"window.open('Proshop_sheet_reports?date=" + date + "&course=" +courseName1+ "', 'newwindow', 'height=600, width=970, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no directories=no, status=no');return false;\">");
         out.println("<button type=\"button\" style=\"width:143px; height:27px; text-decoration:none; font-size: 10pt; font-weight:bold; background:#F5F5DC\">Veiw Quota</button></a></td>");
         out.println("</tr>");
         out.println("</div>");
...
*/
     ...
     errorMsg = "Error building tee sheet legend.";
      ...
      //
      if ((!eventA.get(0).equals( "" ) || !restA[0].equals( "" ) || !lottA[0].equals( "" ))) {
         ...
         for (i = 0; i < count_event; i++) {     // check for events
             ...
             if (!eventA.get(i).equals("")) {
                 ...
                 if (sysConfigEvent) {
                     out.println("<a href=\"javascript:void(0)\" onClick=\"window.open('Proshop_sheet?event=" + eventA.get(i) + "', 'newwindow', 'height=430, width=550, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no directories=no, status=no');return false;\">");
                     out.println("<button type=\"button\" style=\"text-decoration:none; background:" + ecolorA[i] + "\">" + eventA.get(i) + "</button></a>");
                 } else {
                     out.println("<button type=\"button\" style=\"text-decoration:none; background:" + ecolorA[i] + "\">" + eventA.get(i) + "</button>");
                 }
                 out.println("&nbsp;&nbsp;&nbsp;");
             }
         }
          ...
          for (i = 0; i < count_rest; i++) {     // check for restrictions
              ...
              if (!restA[i].equals("")) {
                  ...
                  if (sysConfigRest) {
                      ...
                      out.println("<button type=\"button\" style=\"text-decoration:none; background:" + rcolorA[i] + "\">" + restA[i] + "</button>");
                  }
                  out.println("&nbsp;&nbsp;&nbsp;");
              }
          }
            ...
            for (i=0; i<count_lott; i++) {     // check for restrictions
                ...
                if (!lottA[i].equals( "" ) && lstateA[i] < 5) {
                   ...
                   if (sysConfigLott) {
                       ...
                       out.println("<button type=\"button\" style=\"text-decoration:none; background:" +lcolorA[i]+ "\">" +lottA[i]+ "</button>");
                   }
                   out.println("&nbsp;&nbsp;&nbsp;");
                }
            }
      ...
      if (club.equals("desertforestgolfclub") || club.equals("sawgrass")) {
          ...
          if (prtRecpt || prtStarterRecpt) {      // if the Print Receipt option (Olympic Club & PGA Village and today)
      ...
      out.println("<table border=\"1\" bgcolor=\"#F5F5DC\" align=\"center\" width=\"90%\">");
         ...
         if (useEditTools) {         // if editing tools allowed

            out.println("<td align=\"center\">");
               out.println("<font color=\"#FFFFFF\" size=\"2\">");
               out.println("<u><b>Edit</b></u>");
            out.println("</font></td>");
         }
...
//         }
         ...
         //
         if (club.equals("columbine") && !courseName1.equalsIgnoreCase("Par 3")) {
         ...
         if (club.equals("estanciaclub")) {
            ...
            if (month_day <= 531 || month_day >= 1015) {
                fives = 0;
                fivesALL = 0;
            }
         }
       ...
       if (!club.equals("racebrook") || !user.equalsIgnoreCase("proshopview")) {
         ...
         if (club.equals("overlakegcc") || club.equalsIgnoreCase("bluehillscc") || club.equalsIgnoreCase("bloomfieldhillscc")) {
            out.println("<td align=\"center\">");
               out.println("<font color=\"#FFFFFF\" size=\"2\">");
      ...
      if (courseName1.equals( "-ALL-" )) {
         ...
         } /*else if (club.equals("desertmountain") && user.startsWith("proshopcg") && (courseName1.equals("Geronimo") || courseName1.equals("Cochise"))) {
      ...
      while (rs.next()) {
         ...
         if (skipTime == false) {        // if OK to include this time on tee sheet
            ...
            if (club.equals("sonnenalp")) {
               ...
               if (player1.startsWith("Club Corp Gues") || player2.startsWith("Club Corp Gues") || player3.startsWith("Club Corp Gues") ||
                   player4.startsWith("Club Corp Gues") || player5.startsWith("Club Corp Gues")) {
                  timecolor = "red";
               }
            }
            ...
            */
            ...
            if (!rest.equals("") && !rcolor.equals("")) {
                ...
                rest_loop:
                for (int ind = 0; ind < parmr.MAX && !parmr.restName[ind].equals(""); ind++) {
                ...
                if ((suspend || !defaultFound) && event.equals("") && lottery.equals("")) {
                    
                    if (bgcolor5.equals(bgcolor)) { // Reset bgcolor5 if color was that of the suspended restriction
                        bgcolor5 = "";
                    }
                    
                    // Search for the first non-suspended color to apply, or default if none found
                    ...
                    for (int ind2 = 0; ind2 < parmr.MAX && !parmr.restName[ind2].equals(""); ind2++) {
                        ...
                        // make sure it's not the default restriction/color, and has a non-blank, non-default color and applies to this time
                        if (!parmr.restName[ind2].equals(rest) && !parmr.color[ind2].equals("") && !parmr.color[ind2].equalsIgnoreCase("Default")
                            ...
                            for (int m = 0; m < parmr.MAX; m++) {
                                ...
                                if (parmr.susp[ind2][m][0] == 0 && parmr.susp[ind2][m][1] == 0) {
                                    break susp_loop;   // don't bother checking any more
                                        ...
                                        && (parmr.susp_locations[ind2][m][0].equals("-ALL-") || parmr.susp_locations[ind2][m][0].equals(courseNameT))) {    //time falls within a suspension and course matches
                                    suspend = true;
                                    break susp_loop;     // don't bother checking any more
                                }
                            }
                            ...
                            if (!suspend) {
                                ...
                                if (((parmr.courseName[ind2].equals("-ALL-")) || (parmr.courseName[ind2].equals(courseNameT))) 
                                        ...
                                        && !parmr.color[ind2].equals("Default")) {    // Make sure course and f/b match, and color isn't Default

                                    // Found a non-default colored match - apply the color and exit loop
                 ...
                 if (!defaultFound && !colorChanged && event.equals("") && lottery.equals("")) { 
            ...
            if (club.equals("racebrook") && user.equalsIgnoreCase("proshopview")) {
               ...
               if (lstate > 0 && lstate < 5) {
                  ...
                  if (event.equals("") && club.equals("oakmont") && oakshotgun == false && date != Hdate2b && (day_name.equals("Wednesday") || day_name.equals("Friday"))) {
                     ...
                     if (day_name.equals( "Friday" )) {       // if Friday
                        ...
                        oakloop1:
                        for (int oak = 0; oak < fritimes.length; oak++) {
                        ...
                        oakloop3:
                        for (int oak = 0; oak < wedtimes.length; oak++) {      // check normal Wed times
                  ...
                  if (medinahMemTime == true) {    // if Medinah special time
                     ...
                     if (oaktime == true || mcctime == true || beartime == true) { // if Bearpath, Oakmont or Milwaukee special time
                        ...
                        if (type == shotgun) {                             // if Shotgun Event
                           ...
                           } else if (club.equals("kiawahislandclub") && custom_int > 0 && !player1.equals("")) {
               ...
               //
               //   Add editing tools if any players in the tee time
               //
               if (useEditTools) {         // if editing tools allowed
                  
                   out.println("<td bgcolor=\"white\" align=\"center\" nowrap>");
               
                   if (!player1.equals("")) {      // if any players in tee time
                      
                       out.println("<a href=\"javascript:void(0)\" id=\"editTool\" onclick=\"showTOPopupLite(this)\" todo=\"edit\" tid=\"" +teecurr_id+ "\" style=\"color: black; text-decoration: underline\"><img src=/" +rev+ "/images/dts_edit.gif width=13 height=13 border=0></a>");
                      
                       out.println("<input type=\"image\" src=\"/" +rev+ "/images/dts_trash.gif\" width=\"13\" height=\"13\" border=\"0\" onclick=\"openTrashWindow(" + teecurr_id + "," + index + ");return false;\" title=\"Click here to CANCEL this tee time.\">");
               ...
               if (type == shotgun) {
                   ...
                   if (!shotgunLabel.equals("")) {       // if time gathered above
                       ...
                       String [] tempLabel = shotgunLabel.split(" ");      // delimiters are space (8:30 AM)

                       shotgunLabel = tempLabel[0];                            // get the time only (8:30)
                   }
            ...
            if (multi != 0) {                          // case 1509
               ...
               if (courseName1.equals( "-ALL-" )/* || (club.equals("desertmountain") && user.startsWith("proshopcg") && (courseName1.equals("Geronimo") || courseName1.equals("Cochise")))*/) {
...
//            }
            ...
            if (club.equals("pradera") || club.equals("pinery") || club.equals("canyonoakscc") || club.equals("beechmontcc") || club.equals("austincountryclub") 
                    || club.equalsIgnoreCase("rollinghillsccca") || club.equalsIgnoreCase("bloomfieldhillscc")) {
...
//            }
            ...
            if (club_group_name.equalsIgnoreCase("Bay Clubs")) {
                custom_disp1 = verifyCustom.getBayClubGreensFee(date, day_name, time, club, getMship(user1, club, con));
                custom_disp2 = verifyCustom.getBayClubGreensFee(date, day_name, time, club, getMship(user2, club, con));
                custom_disp3 = verifyCustom.getBayClubGreensFee(date, day_name, time, club, getMship(user3, club, con));
                custom_disp4 = verifyCustom.getBayClubGreensFee(date, day_name, time, club, getMship(user4, club, con));
                custom_disp5 = verifyCustom.getBayClubGreensFee(date, day_name, time, club, getMship(user5, club, con));
            }
            ...
            if (club.equals("mayfieldsr") && courseNameT.equalsIgnoreCase("sand ridge") && verifyCustom.checkMayfieldSR(date, time, day_name)) {
                ...
                buildPlayerTD(out, 1, show1, player1, bgcolor1, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp1, ghin1, g1, guest_id1, guest_uid1, p1cw, p91, disp_mnum, mnum1, disp_bag, bag1, courseName1, club, parmp, pos1, showcheckbox, checkinAccess, user1, teecurr_id, user, custom_disp1, isPosPayNow, club_group_name);
                buildPlayerTD(out, 2, show2, player2, bgcolor2, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp2, ghin2, g2, guest_id2, guest_uid2, p2cw, p92, disp_mnum, mnum2, disp_bag, bag2, courseName1, club, parmp, pos2, showcheckbox, checkinAccess, user2, teecurr_id, user, custom_disp2, isPosPayNow, club_group_name);
                ...
                buildPlayerTD(out, 1, show1, player1, bgcolor1, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp1, ghin1, g1, guest_id1, guest_uid1, p1cw, p91, disp_mnum, mnum1, disp_bag, bag1, courseName1, club, parmp, pos1, showcheckbox, checkinAccess, user1, teecurr_id, user, custom_disp1, isPosPayNow, club_group_name);
                buildPlayerTD(out, 2, show2, player2, bgcolor2, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp2, ghin2, g2, guest_id2, guest_uid2, p2cw, p92, disp_mnum, mnum2, disp_bag, bag2, courseName1, club, parmp, pos2, showcheckbox, checkinAccess, user2, teecurr_id, user, custom_disp2, isPosPayNow, club_group_name);
                buildPlayerTD(out, 3, show3, player3, bgcolor3, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp3, ghin3, g3, guest_id3, guest_uid3, p3cw, p93, disp_mnum, mnum3, disp_bag, bag3, courseName1, club, parmp, pos3, showcheckbox, checkinAccess, user3, teecurr_id, user, custom_disp3, isPosPayNow, club_group_name);
                buildPlayerTD(out, 4, show4, player4, bgcolor4, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp4, ghin4, g4, guest_id4, guest_uid4, p4cw, p94, disp_mnum, mnum4, disp_bag, bag4, courseName1, club, parmp, pos4, showcheckbox, checkinAccess, user4, teecurr_id, user, custom_disp4, isPosPayNow, club_group_name);
                ...
                if (fivesALL != 0) {        // if 5-somes supported on any course
                   ...
                   if (fives != 0) {        // if 5-somes on this course
                     ...
                     buildPlayerTD(out, 5, show5, player5, bgcolor5, time, num, courseNameT, fb, j, disp_hndcp, hndcp, hndcp5, ghin5, g5, guest_id5, guest_uid5, p5cw, p95, disp_mnum, mnum5, disp_bag, bag5, courseName1, club, parmp, pos5, showcheckbox, checkinAccess, user5, teecurr_id, user, custom_disp5, isPosPayNow, club_group_name);
            ...
            //
            if (tsNotesView && (!club.equals("racebrook") || !user.equalsIgnoreCase("proshopview"))) {
            ...
            if (club.equals( "medinahcc" )) {        // if Medinah - add Print Tee Time col
             ...
             if (prtRecpt || prtStarterRecpt) {      // if the Print Receipt option (Olympic Club & PGA Village and today)
                 ...
                 if (prtRecpt) {
                     recpt_type = "prtReceipt";
                 } else {
                     recpt_type = "prtStarterReceipt";
                 }
               ...
               if (!player1.equals( "" ) || !player2.equals( "" ) || !player3.equals( "" ) || !player4.equals( "" ) || !player5.equals("")) {
                  out.println("<a href=\"javascript:void(0)\" onClick=\"window.open ('Proshop_sheet?" + recpt_type + "&teecurr_id=" +teecurr_id+ "', 'newwindow', 'Height=440, width=400, toolbar=no, menubar=no, scrollbars=auto, resizable=no, location=no directories=no, status=no')\">");
                     out.println("PR</a>");
                 } else {
                     out.println("&nbsp;");
                 }
            ...
            */
            ...
            if (club.equals("overlakegcc") || club.equalsIgnoreCase("bluehillscc") || club.equalsIgnoreCase("bloomfieldhillscc")) {   // add 'Hole Number' col for Overlake G & CC
      ...
      out.println("<a href=\"#top\" id=\"scrollUp\" style=\"display: none;\"></a>");

       out.println("<div id=elTOPopup defaultValue=\"\" fb=\"\" nh=\"\" jump=\"\" tid=\"\">");
       out.println("<form name=\"frmTransOpt\">");
       out.println("<table id=elTOTable border=\"0\" cellpadding=\"0\" cellspacing=\"2\">"); // width=\"100%\" height=\"100%\" 
       ...
       if (isPosPayNow == 2) {
           out.println("<div id=elPNPopup defaultValue=\"\" fb=\"\" jump=\"\" tid=\"\">");
           out.println("<table width=\"100%\" height=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"2\">");
           out.println("<form name=\"frmPayNowOpt\">");
           out.println("<input type=\"hidden\" name=\"jump\" value=\"\">");

           out.println("<tr><td align=center class=smtext colspan=4><b><u>Select a Payment Method</u></b></td></tr>");

           out.println("<tr>");
           out.println("<td nowrap class=smtext><label title=\"Credit Card\"><input type=radio value=4 name=pn>Credit</label></td>");
           out.println("<td nowrap class=smtext><label title=\"Cash\"><input type=radio value=5 name=pn>Cash</label></td>");
           out.println("<td nowrap class=smtext><label title=\"Check\"><input type=radio value=6 name=pn>Check</label></td>");
           out.println("<td nowrap class=smtext><label title=\"Other\"><input type=radio value=7 name=pn>Other</label></td>");
           out.println("</tr>");
           out.println("<tr>");
           out.println("<td nowrap class=smtext><label title=\"Not Paid\"><input type=radio value=0 name=pn>Not Paid</label></td>");
           out.println("</tr>");

           out.println("<tr><td colspan=4 style=\"background-color:black\"></td></tr>");
           out.println("<tr><td colspan=4 style=\"background-color:black\"></td></tr>");
           out.println("<tr><td class=smtext colspan=4><label><input type=checkbox value=yes name=changeAll id=ca>Change All</label></td></tr>");
           out.println("<tr><td align=center colspan=4><a href=\"javascript: cancelPNPopup()\" class=smtext>cancel</a>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; "
                   + "<a href=\"javascript: savePNPopupLite()\" class=smtext>save</a>&nbsp;</td></tr>");
           out.println("</form>");
           out.println("</table>");
           out.println("</div>");
       }
...
*/
 ...
 private void buildPlayerTD(PrintWriter out, int playern, short show, String player, String bgcolor, int time,
                            ...
                            String ghin, int guest, int guest_id, String guest_uid, String pcw, int p9, boolean disp_mnum, String mnum,
                            boolean disp_bag, String bag, String courseName1, String club, parmPOS parmp, int pos,
                            boolean showcheckbox, boolean checkinAccess, String username, int teecurr_id, String user, 
                            String custom_disp, int isPosPayNow, String club_group_name) {
    ...
    if (!player.equals("")) {
      ...
      if (player.equalsIgnoreCase("x")) {
          ...
          if (club.equalsIgnoreCase("mpccpb")) {
              out.print("<td bgcolor=\"" + bgcolor + "\" align=\"center\">");
              out.print("<font size=\"2\">Member TBD</font></td>");
          } else {
              out.print("<td bgcolor=\"" + bgcolor + "\" align=\"center\">");
         ...
         //
         if ((club.equals("southview") || club.equals("dellwood") || club.equals("philcricket") || club.equalsIgnoreCase("ancalacc") || club.equalsIgnoreCase("engineerscc")) && player.startsWith("Join Me")) {
             bgcolor = "lime";
         }
         ...
         if (club.equalsIgnoreCase("thewoodlands") && player.startsWith("Guest Plan")) {
             bgcolor = "red";
         }
         
         if (club.equals("lomassantafecc")) {
             
             if (player.startsWith("Regular") || player.startsWith("Family") || player.startsWith("Soc-Ten")) {
                 bgcolor = "tomato";
         ...
         String p = player;                                       // copy name only
         ...
         if (guest == 0) {                                     // if not a guest
            ...
            if (disp_mnum && !mnum.equals( "" )) {        // add Member # if present and requested
            ...
            if (disp_bag && !bag.equals( "" )) {          // add Bag # if present and requested
               p = p + "  " +bag;
            }
            ...
            if (club_group_name.equalsIgnoreCase("Bay Clubs") && !custom_disp.equals("")) {
                p += " " + custom_disp;
            }
         } // end if not guest
         ...
         if (parmp.pos_paynow == 1 && num.equals("0") && pos != 1 && pos != 3) {
             ...
             if (isPosPayNow == 2) {               
                 ...
                 String pnImg = "pospaynow.gif";
                 String pnTitle = "Click here to select a Payment Method for this player.";
                 
                 if (pos != 0) {
                     pnImg = "pospaid.gif";
                     pnTitle = "Click here to adjust or remove the selected Payment Method for this player.";
                 }
                 
                 out.println("&nbsp;<a href=\"javascript:void(0)\" id=\"pn_"+teecurr_id+"_"+playern+"\" onclick=\"showPNPopupLite(this)\" value = \"" + pos + "\" "
                         + "playerSlot=\"" +playern+ "\" tid=\"" +teecurr_id+ "\" style=\"color: black; text-decoration: underline\">"
                         + "<img id=\"paybox_"+teecurr_id+"_"+playern+"\" src=\"/" +rev+ "/images/" + pnImg + "\" border=\"1\" name=\"paynow\" "
                         + "title=\"" + pnTitle + "\"></a>");
    ...
    if ((club.equals("foresthighlands") && pcw.equalsIgnoreCase("CD"))
            || ((club.equals("stalbans") || club.equals("tavistockcc") || club.equals("ozaukeecc") || club.equals("riverside")) && pcw.equalsIgnoreCase("CAD"))
            || (club.equals("sawgrass") && (pcw.equalsIgnoreCase("FC") || pcw.equalsIgnoreCase("C1B") || pcw.equalsIgnoreCase("C2B")))
            || (club.equals("pointowoods") && (pcw.equalsIgnoreCase("CD") || pcw.equalsIgnoreCase("4C")))
            || (club.equals("martiscamp") && (pcw.equalsIgnoreCase("CAD") || pcw.equalsIgnoreCase("FC")))) {
    ...
    if (club.equals("cherryhills") && !user.equals("proshop1")) {       // If Cherry Hills and not proshop1, don't allow them to click and change the MoT on the tee sheet
        ...
        pcws = "<a href=\"javascript:void(0)\" id=\"cw_"+teecurr_id+"_"+playern+"\" onclick=\"showTOPopupLite(this)\" value=\"" +pcw+ "\" playerSlot=\"" +playern+ "\" p9=\"" + p9 + "\" tid=\"" +teecurr_id+ "\" style=\"color: black; text-decoration: underline\">" +pcw+ "</a>";
 ...
 private void printCustomTee(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session,
                           Connection con) {
   out.println(SystemUtils.HeadTitle("Print Sheet"));
   ResultSet rs = null;
   ResultSet rs2 = null;
   ResultSet rs3 = null;
   ...
   PreparedStatement pstmtu1 = null;
   PreparedStatement pstmtu2 = null;
   PreparedStatement pstmtu3 = null;
   PreparedStatement pstmtu4 = null;
   PreparedStatement pstmtu5 = null;
   ...
   try {
       pstmt1 = con.prepareStatement("SELECT day, mm , dd FROM teecurr2 WHERE date = ? AND courseName = ?");
   ...
   String dday = Integer.toString(dd);
   if (dday.equals("0") || dday.equals("1") || dday.equals("2") || dday.equals("3") || dday.equals("4") || dday.equals("5") || dday.equals("6") || dday.equals("7") || dday.equals("8") || dday.equals("9")) {
       dday = "0"+dday;
   }
   ...
   try{
       ...
       pstmt = con.prepareStatement("SELECT time FROM teecurr2 WHERE DATE = ? AND courseName = ? and blocker = '' group by time");
   ...
   count = count * 2; //each tee time will have a front and a back
   ...
   try {
       if (excel.equals("yes")) {                // if user requested Excel Spreadsheet Format
           ...
           resp.setHeader("Content-Disposition", "attachment;filename=\"Proshop_sheet.xls\"");
       }
   } catch (Exception exc) {
   }   
     
   if (!excel.equals("yes") || req.getParameter("print") != null) {                // if not Excel Spreadsheet Format
       ...
       out.println("</td>");
       out.println("<td style=\"border:0px;\">");
       out.println("<form method=\"post\" action=\"Proshop_sheet\" name=frmRequestExcel id=frmRequestExcel target=\"_blank\">");
       out.println("<input type=\"hidden\" name=\"prtcust\" value=\"yes\">");
       out.println("<input type=\"hidden\" name=\"excel\" value=\"yes\">");
       out.println("<input type=\"hidden\" name=\"course\" value=" + course + ">");
       out.println("<input type=\"hidden\" name=\"date\" value=" + dates + ">");
   ...
   out.println("td, table { border:1px solid black; white-space:nowrap; text-align: center; border-collapse: collapse;}");
   ...
   out.println("<tr><td colspan=\"2\" align=\"center\" bgcolor=\"#C0C0C0\"><font size=\"5\">"+day+
              "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
              + ""+course+" COURSE&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
              + ""+months+"&nbsp;"+dday+"</font></td>");
   out.println("</tr>");
   out.println("<tr>");
   if (excel.equals("yes")) {
       out.println("<td align=\"center\" valign=\"top\" style=\"border:1px;\"><table width=\"100%\"><tr bgcolor=\"#C0C0C0\"><td align=\"center\">1ST TEE</td><td align=\"center\">10TH TEE</td></tr>");
   } else {
       out.println("<td align=\"center\" valign=\"top\" style=\"border:0px;\"><table width=\"100%\" style=\"border:0px;\"><tr bgcolor=\"#C0C0C0\"><td align=\"center\">1ST TEE</td><td align=\"center\">10TH TEE</td></tr>");
   }
   try{ 
       
      pstmt2 = con.prepareStatement("SELECT player1, player2, player3, player4, player5, username1, username2, username3, username4, username5, p1cw, p2cw, p3cw, p4cw, p5cw, hr, min, fb, time, day, mm , dd FROM teecurr2 WHERE date = ? AND courseName = ? AND time in("+timeString+") order by time, fb");
      ...
      while (rs.next()){
          ...
          time = rs.getInt("time");
          try {
              pstmtu1 = con.prepareStatement("SELECT name_first, name_last from member2b where username = ?");
              pstmtu1.clearParameters();
              pstmtu1.setString(1, username1);
              rs2 = pstmtu1.executeQuery();
              while (rs2.next()) {
                  first = rs2.getString("name_first");
                  last = rs2.getString("name_last");
                  player1 = first + " " + last;
                  first = "";
                  last = "";
              }
          ...
          try {
              ...
              pstmtu2 = con.prepareStatement("SELECT name_first, name_last from member2b where username = ?");
              pstmtu2.clearParameters();
              pstmtu2.setString(1, username2);
              rs2 = pstmtu2.executeQuery();
              while (rs2.next()) {
                  first = rs2.getString("name_first");
                  last = rs2.getString("name_last");
                  player2 = first + " " + last;
          ...
          try {
              ...
              pstmtu3 = con.prepareStatement("SELECT name_first, name_last from member2b where username = ?");
              pstmtu3.clearParameters();
              pstmtu3.setString(1, username3);
              rs2 = pstmtu3.executeQuery();
              while (rs2.next()) {
                  first = rs2.getString("name_first");
                  last = rs2.getString("name_last");
                  player3 = first + " " + last;
          ...
          try {
              ...
              pstmtu4 = con.prepareStatement("SELECT name_first, name_last from member2b where username = ?");
              pstmtu4.clearParameters();
              pstmtu4.setString(1, username4);
              rs2 = pstmtu4.executeQuery();
              while (rs2.next()) {
                  first = rs2.getString("name_first");
                  last = rs2.getString("name_last");
                  player4 = first + " " + last;
          ...
          try {
              ...
              pstmtu5 = con.prepareStatement("SELECT name_first, name_last from member2b where username = ?");
              pstmtu5.clearParameters();
              pstmtu5.setString(1, username5);
              rs2 = pstmtu5.executeQuery();
              while (rs2.next()) {
                  first = rs2.getString("name_first");
                  last = rs2.getString("name_last");
                  player5 = first + " " + last;
          ...
          if (p1cw.equals("") || p1cw == null) {
              p1cw = "&nbsp;&nbsp;&nbsp;";
          }
          if (p2cw.equals("") || p2cw == null) {
              p2cw = "&nbsp;&nbsp;&nbsp;";
          }
          if (p3cw.equals("") || p3cw == null) {
              p3cw = "&nbsp;&nbsp;&nbsp;";
          }
          if (p4cw.equals("") || p4cw == null) {
              p4cw = "&nbsp;&nbsp;&nbsp;";
          }
          if (p5cw.equals("") || p5cw == null) {
              p5cw = "&nbsp;&nbsp;&nbsp;";
          }
          String mmin = Integer.toString(min);
          if (mmin.equals("0") || mmin.equals("1") || mmin.equals("2") || mmin.equals("3") || mmin.equals("4") || mmin.equals("5") || mmin.equals("6") || mmin.equals("7") || mmin.equals("8") || mmin.equals("9")) {
              mmin = "0" + mmin;
          }

          if (hour == 13 || hour == 14 || hour == 15 || hour == 16 || hour == 17 || hour == 18 || hour == 19) {
              hour = hour - 12;
          }
       

       
          if (counter <= (count / 2) + odd) {
              if (time != tempTime && fb != tempfb) {
                  if (bgColor.equals("#C0C0C0")) {
                      bgColor = "#FFFFFF";
                  } else {
                      bgColor = "#C0C0C0";
                  }
              ...
              if (excel.equals("yes")) {
                  if (fb == 0) {
                      out.println("<tr>");
                      out.println("<td width=\"50%\" bgcolor=" + bgColor + " style=\"border:1px;\"><table align=\"center\" width=\"100%\" style=\"border:0px;\"><tr><td width=\"20%\">" + hour + ":" + mmin + "</td><td align=\"center\" width=\"60%\">" + player1 + "</td><td width=\"20%\" align=\"center\">" + p1cw + "</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">" + player2 + "</td><td>" + p2cw + "</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">" + player3 + "</td><td>" + p3cw + "</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">" + player4 + "</td><td>" + p4cw + "</td></tr>");
                  ...
                  if (fb == 0) {
                      out.println("<tr>");
                      out.println("<td width=\"50%\" bgcolor=" + bgColor + " style=\"border:0px;\"><table align=\"center\" width=\"100%\" style=\"border:0px;\"><tr><td width=\"20%\">" + hour + ":" + mmin + "</td><td align=\"center\" width=\"60%\">" + player1 + "</td><td width=\"20%\" align=\"center\">" + p1cw + "</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">" + player2 + "</td><td>" + p2cw + "</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">" + player3 + "</td><td>" + p3cw + "</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">" + player4 + "</td><td>" + p4cw + "</td></tr>");
                      out.println("</table></td>");
          ...
          if (counter == (count / 2) + odd) {
              ...
              if (excel.equals("yes")) {
                  out.println("</table></td><td align=\"center\" valign=\"top\" style=\"border:1px;\"><table width=\"100%\"><tr bgcolor=\"#C0C0C0\"><td align=\"center\">1ST TEE</td><td align=\"center\">10TH TEE</td></tr>");
              } else {
                  out.println("</table></td><td align=\"center\" valign=\"top\" style=\"border:0px;\"><table width=\"100%\" style=\"border:0px;\"><tr bgcolor=\"#C0C0C0\"><td align=\"center\">1ST TEE</td><td align=\"center\">10TH TEE</td></tr>");
              }


          }

          if (counter > (count / 2) + odd && counter <= count + odd) {
              if (((count / 2) + odd) + 1 == counter) {
                  bgColor = "#C0C0C0";
              }
              if (time != tempTime && fb != tempfb) {
                  if (bgColor.equals("#C0C0C0")) {
              ...
              if (counter == count && odd == 1) {
                  ...
                  if (excel.equals("yes")) {
                      ...
                      out.println("<tr>");
                      out.println("<td width=\"50%\" bgcolor=" + bgColor + " style=\"border:1px;\"><table align=\"center\" width=\"100%\" style=\"border:0px;\"><tr><td width=\"20%\">&nbsp;:&nbsp;</td><td align=\"center\" width=\"60%\">&nbsp;</td><td width=\"20%\" align=\"center\">&nbsp;</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">&nbsp</td><td>&nbsp</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">&nbsp</td><td>&nbsp</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">&nbsp</td><td>&nbsp</td></tr>");
                      out.println("</table></td>");
                      ...
                      out.println("<tr>");
                      out.println("<td width=\"50%\" bgcolor=" + bgColor + " style=\"border:0px;\"><table align=\"center\" width=\"100%\" style=\"border:0px;\"><tr><td width=\"20%\">&nbsp;:&nbsp;</td><td align=\"center\" width=\"60%\">&nbsp;</td><td width=\"20%\" align=\"center\">&nbsp;</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">&nbsp</td><td>&nbsp</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">&nbsp</td><td>&nbsp</td></tr>");
                      out.println("<tr align=\"center\"><td>&nbsp;</td><td align=\"center\">&nbsp</td><td>&nbsp</td></tr>");
                      out.println("</table></td>");
 ...
 private String promptCourse(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, HttpSession session, Connection con, int lotter)
         throws ServletException, IOException {
   ...
   //
   //  Get the existing course - if only one, then return it, else prompt for a selection
   //
   try {

      pstmt = con.prepareStatement (
              "SELECT courseName FROM teecurr2 group by courseName");
      ...
      while (rs.next()) {
         found = true;
         course = rs.getString("courseName");          // get the proid
         count++;
      }
   ...
   if (found == false) {        // if none yet
      ...
      out.println("<BODY>");
      SystemUtils.getProshopSubMenu(req, out, lot);        // required to allow submenus on this page
      out.println("<CENTER>");
      out.println("<BR><BR><H3>Setup Sequence Error</H3>");
      out.println("<BR><BR>Sorry, we are unable to process your request at this time.");
      out.println("<BR>There are no courses in the system yet.");
      ...
      course = "";
      return(course);            // return none
   }

   if (count == 1) {        // if only one pro
      return(course);                   // return it so we can continue using this course
   }
     

   //
   //  More than one course found - prompt user to select one
   //
   //  Build the html page to request the course info
   //
   out.println(SystemUtils.HeadTitle("Proshop - Select Course"));
   out.println("<body bgcolor=\"#FFFFFF\" text=\"#000000\">");
   SystemUtils.getProshopSubMenu(req, out, lot);            // required to allow submenus on this page
   ...
   out.println("<font size=\"2\">");
      ...
      out.println("<form action=\"Proshop_sheet\" method=\"post\" name=\"pform\">");
        
      if (req.getParameter("addltype") != null) {      // add lesson type?
        
         temp = req.getParameter("printCustomTee");       
         out.println("<input type=\"hidden\" name=\"printCustomTee\" value=\"" +temp+ "\">");
      }


      out.println("<b>Course:</b>&nbsp;&nbsp;");
      out.println("<select size=\"1\" name=\"course\" onChange=\"document.pform.submit()\">");
   ...
   course = "";           // return none so we know user was prompted

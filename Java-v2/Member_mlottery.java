...
public class Member_mlottery extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (club.equals("missionviejo") || club.equals("whitemanor") || club.equals("northridge") || club.equals("bonniebriar")
                ...
                || club.equals("lakewoodcc") || club.equals("coldspringcc") || club.startsWith("demo")) {
        ...
        if (club.equalsIgnoreCase("bhamcc") && lname.equalsIgnoreCase("Ladies Golf")) {  //only all members to to joing Ladies Golf event
            partialReqs = true;     // allow mems to join
        }
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (club.equalsIgnoreCase("oronoquecc")) {
                    for (int i = 0; i< luser.length; i++) {
                        if (!luser[i].equals(user)) {
                            continue;
                        }
                    }
                    if (!orig_by.equals(user)) {
                        continue;
                    }
                }

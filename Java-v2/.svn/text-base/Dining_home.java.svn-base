...
public class Dining_home extends HttpServlet {
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   resp.setHeader("Cache-Control", "no-store");    // for HTTP 1.1
   resp.setDateHeader("Expires", -1441);                   // prevents caching at the proxy server
...
*/
   ...
   if (req.getParameter("view_events") != null) {
       ...
       viewEventListing_new(organization_id, club, user, req, out, con, con_d);
...
*/
 ...
 private void viewSearchPage(String club, String user, HttpServletRequest req, PrintWriter out, Connection con) {
   ...
   // out.println("<div id=\"breadcrumb\">Home / Search Dining Reservations <a href=\"#\" onclick=\"window.location.href='Dining_home'\">&lt;&lt; Go Back</a></div>");
    ...
    if(rwd){
       ...
       out.print("<p>To locate the current reservations for an individual or group, select a member using the tool below.</p>");
       out.print("</div>"); // End instructions 

       out.print("<div class=\"memberDiningReservationSearch\"></div>");
 ...
 private void viewSearchResults(String club, String user, HttpServletRequest req, PrintWriter out, Connection con_d) {
    ...
    int person_id = Utilities.getParameterInteger(req, "search_data", 0);
    String search_user = Utilities.getUsernameFromPersonId(person_id, con);
    String search_user_name = Utilities.getFullNameFromUsername(search_user, con);

    List<reservationSignup> signups = diningUtil.getSignupsByUser(person_id, req, con_d);
    ...
    out.print(tags.getCaption(tags.getTag("h2", "Upcoming Reservations for " + search_user_name )));
    out.print(tags.openThead()+tags.openTr());
    out.print(tags.getTh("Date"));
    out.print(tags.getTh("Time"));
    ...
    try{
        for(reservationSignup signup : signups){
            ...
            out.print(tags.getTd(timeUtil.getStringDateMDYYYY(signup.date), "sDd"));
            if(signup.id != null && signup.id > 0){
                out.print(tags.getTd(tags.getFtLink(timeUtil.get12HourTime(signup.time), Utilities.getBaseUrl(req, dining_activity_id, club)+"Dining_slot?reservation_id="+signup.id, "standard_button"), "sT"));
            } else {
                out.print(tags.getTd(tags.getTag("div", timeUtil.get12HourTime(signup.time), "time_slot"), "sT"));
            }
            out.print(tags.getTd(signup.location, "sN"));
            if(signup.event_id != null && signup.event_id > 0){
                out.print(tags.getTd(tags.getJsonLink(signup.type, reservationUtil.linkJsonEsc(signup.event_id, dining_activity_id), "standard_link event_button"), "sP sTp"));
            } else {
        ...
        Utilities.logError("Dining_home.viewSearchResults: Err=" + org.apache.commons.lang.exception.ExceptionUtils.getStackTrace(exc));
    }
    out.print(tags.closeTbody());
    out.print(tags.closeTable());
    
    if(signups.isEmpty()){
        out.print(tags.getSubInst("No upcoming reservations found for " + search_user_name, "ftCenterText"));
    }
    
    out.println("<br><div style=\"text-align:center;\">");
    out.println("<input id=\"back\" name=\"back\" type=\"button\" value=\"Go Back\" onclick=\"window.history.back();\" />");
    out.println("</div>");
...
*/
 ...
 private void viewEventListing_new(int organization_id, String club, String user, HttpServletRequest req, PrintWriter out, Connection con,  Connection con_d) {
    ...
    List<parmEvent> events = diningUtil.getEvents(ProcessConstants.DINING_ACTIVITY_ID, user, req, con_d);
        
    int today = (int)Utilities.getDate(con);
    //int event_id = Utilities.getParameterInteger(req, "event_id", 0);
    
    int click_event_id = reqUtil.getParameterInteger(req, "event_id", 0);
    ...
    Common_skin.outputBreadCrumb(club, dining_activity_id, out, "Event Listing", req);
    ...
    out.print(tags.openTable("standard_list_table rwdCompactible rwdWideData"));
    out.print(tags.openThead()+tags.openTr());
    out.print(tags.getTh("Event Name"));
    out.print(tags.getTh("Location"));
    out.print(tags.getTh("Status"));
    out.print(tags.getTh("Date/Time"));
    out.print(tags.getTh("Sign-Up Starts"));
    out.print(tags.getTh("Sign-Up Ends"));
    out.print(tags.closeTr()+tags.closeThead());
    out.print(tags.openTbody());

    String status;
    String statusLinkType = null;
    int statusLinkId = 0;
    for (parmEvent event : events) {
        // output each event
        out.print(tags.openTr("ft-noFb"));
        out.print(tags.getTd(tags.getTag("a", event.name, "standard_button event_button"+(click_event_id==event.id?" defaultEvent":""), new String[]{"href=\"#\"","data-ftjson=\""+reservationUtil.linkJsonEsc(event.id, ProcessConstants.DINING_ACTIVITY_ID) +"\""}),"sT"));
        out.print(tags.getTd(event.locationName,"sN"));
        if(event.isSignedUp){
            status = "Registered";
            statusLinkType = "reservation_id";
            statusLinkId = event.reservationId;
        } else if (!event.canSignUp && event.inSignUpPeriod){
            status = "Please call the club";
        } else if(event.inSignUpPeriod){
            status = "Register Now";
            statusLinkType = "event_id";
            statusLinkId = event.id;
        } else {
            if(today < event.registrationStart){
                status = "Opens "+timeUtil.getStringDateMDYYYY(event.registrationStart);
            } else if(today > event.registrationEnd){
                status = "Closed "+timeUtil.getStringDateMDYYYY(event.registrationEnd);
            } else {
                status = "Not Open"; // Shouldn't get here.
            }
        }
      //if((event.isSignedUp || event.inSignUpPeriod) && (event.canSignUp || event.isSignedUp)){
        if(event.inSignUpPeriod && (event.canSignUp || event.isSignedUp)){
            out.print(tags.getTd(tags.getFtLink(status, Utilities.getBaseUrl(req, dining_activity_id, club)+"Dining_slot?"+statusLinkType+"="+statusLinkId, "standard_link"), "sP sSt"));
        } else {
            out.print(tags.getTd(status, "sP sSt"));
        }
        
        out.print(tags.getTd(
                timeUtil.getStringDateMDYYYY(event.date)
                + ", " + timeUtil.formatDate(req, event.date, event.startTime, ProcessConstants.RESERVATION_TIME_FORMAT)
                + " to " + timeUtil.formatDate(req, event.date, event.endTime, ProcessConstants.RESERVATION_TIME_FORMAT),"sDd"));
        out.print(tags.getTd(timeUtil.formatDate(req, event.registrationStart, ProcessConstants.RESERVATION_DATE_FORMAT),"sP startDate"));
        out.print(tags.getTd(timeUtil.formatDate(req, event.registrationEnd, ProcessConstants.RESERVATION_DATE_FORMAT),"sP endDate"));
        
        out.print(tags.closeTr());
     
    }
    out.print(tags.closeTbody()+tags.closeTable());
    if(events.isEmpty()){
        tags.getSubInst("No Dining Events Currently Available");
    }

 }

 private void viewEventListing_UNUSED(int organization_id, String club, String user, HttpServletRequest req, PrintWriter out, Connection con,  Connection con_d) {

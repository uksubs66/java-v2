 ...
 *   last updated:
 *
 *        6/05/14   Add a 'Cancel Tee Times' option to the Control Panel and add processing to cancel selected or All tee times for a specific day and course.
 *        5/07/14   Add 'Flip All F/B Options' to Control Panel and add processing to swap all the F/B's for day and course specified. 
 *        2/04/14   El Camino CC (elcaminocc) - Sort the tee sheet by fb first, then time (case 2362).
 *        1/09/14   Updated moveWhole and moveSingle when the to_time is part of a shotgun event to use the actual start time of the event for the time value in any email notifications sent.
 *       12/18/13   Updated moveWhole so that the lottery_email flag gets set to 0 when the "from" time is getting zeroed out after the move.
 *       11/15/13   Added an "Other course?" column to processed lottery requests that indicates whether or not the "Check other courses" option was selected for a particular signup.
 *        7/23/13   Get the actual assigned f/b value for each group when listing the lottery requests (custom added for Estero to assign both tees). Case 2286.
 *        4/22/13   Adjust the time for the club's time zone before getting the date from the index parm passed (allows for times near midnight).
 *        2/26/13   Updated covertLottRequest, auto_convert, and convert_all methods to generate history entries when lottery requests have been successfully converted into a tee time.
 *        2/21/13   When prompting user to send emails for lottery approval, also ask if they want to send emails to players that were unassigned.
 *        2/20/13   Update custom for Pecan Plantation - log a dummy assignment with 360 minutes (was 200) for any member that did not get assigned a time in a lottery.
 *        2/19/13   Allow pro to complete the lottery processing even when no requests existed - clear the tee sheet.
 *        1/11/13   Update the query in logLotteryReqs to grab all reqs (even unassigned) so custom for Pecan Plantation
 *                  works.  This custom logs an assignment with 200 mins for unassigned requests so members get a weight.
 *       12/18/12   moveWhole() method will now carry along orig1-5 values.
 *       12/14/12   Added a couple additional tweaks to the 12/13 changes to auto_convert and converLottReq.
 *       12/13/12   auto_convert method will now populate the orig1-5 fields when a lottery request is auto_converted (NOT convert_all!).
 *       12/13/12   convertLottReq method will now populate the orig1-5 fields when a lottery request is manually drug down to a time slot.
 *       12/13/12   Fixed hndcp indexes not getting populated when players moved via convertEventSgnUp, convertWaitListSgnUp, convertOld, and convertNotification.
 *       11/27/12   Add custom for Pecan Plantation - log a dummy assignment with 200 minutes for any member that did not get assigned a time in a lottery.
 *       11/23/12   Add x-ua-compatible meta tag specifying IE=EmulateIE8 to address IE10 (with the doctype the emulate mode should force Quirks mode)
 *       11/07/12   Change calls to SystemUtils.getFirstTime and getLastTime to Common_Lott (methods moved).
 *       10/30/12   Updated population of parmEmail so that fb values are included for groups 2-5 when sending lottery assignment emails.
 *        2/08/12   In logLotteryReqs check for multiple tee times assigned to each member in the lottery request (in case they played twice in the same lottery).
 ...
 *        6/14/11   Do not log the lottery assignments until done with lottery assignments (when sending emails).
 *        6/13/11   Display tmode abbr rather than full name inside the tmode popup - full name now avail when hovering over abbr
 *        5/17/11   Ramsey CC (ramseycountryclub) - Updated custom to move wait list when lottery has been processed so that it is setting the datetime value properly (case 1823).
 *        4/13/11   Make sure we prompt the pro to send emails (complete the approval) when returning to the tee sheet after approving a lottery - if only guests are in the lottery.
 *        4/11/11   Edison Club (edisonclub) - Adjustment to previous custom to accommodate for inserting off first tee time. Will now allow only 20 minutes prior, and up to 4hr after (case 1962).
 *        4/04/11   Edison Club (edisonclub) - When inserting a new tee time, allow selection of time values up to 4 hrs out (case 1962).
 *        3/21/11   Changed applicable sql update calls to teecurr2 to set the last_mod_date to now
 *        2/04/11   Change to prevent lottery_email flag from getting set when a lottery request is converted/drug into a time that is not a lottery time.
 *        1/05/11   Change the eway emails are sent when lotteries are approved.  Use HTML emails and include all groups from the lottery request so players know
 *                  when the others are playing.
 *       11/16/10   Added common methods in SystemUtils for lottery assignment calculations and added processing to log every assigned time for members in lassigns5.
 *       11/12/10   Added date to lottery assignment email subject line
 *       10/27/10   Added activity_id = 0 filter to event and restriction lookup queries so only golf-related ones are displayed
 *       10/20/10   Populate new parmEmail fields (Note: as of now the if there are guests in the moved time the guest indicator will not be set in parmEmail - look at slotParms2)
 *        9/15/10   Added guest_id values to convertWaitListSignUp
 *        8/24/10   Hazeltine CC Temp 2010 Site (hazeltine2010) - When inserting a new tee time, allow selection of time values up to 3 hrs out
...
public class Proshop_dsheet extends HttpServlet {
 ...
 //
    ...
    if (req.getParameter("flipallfb") != null && req.getParameter("flipallfb").equals("yes")) {
           
       flip_all_fbs(req, out, con, session, resp);    // user wants to swap all the F/B values on this tee sheet
       return;
    }
    
    if (req.getParameter("flipallfb2") != null) {
           
       flip_all_fbs2(req, out, con, session, resp);    // user confirmed he wants to swap all the F/B values on this tee sheet
    }
    ...
    if (req.getParameter("blockers") != null) {   // if Blockers or Cancel Tee Times from Control Panel
    ...
    if (req.getParameter("cancelTeeTimes") != null) {   // if request to cancel tee times (from displayBlockers method)

       ArrayList<Integer> timesInUseA = new ArrayList<Integer>();      // Will contain any tee times that were in use and couldn't be canceled

       timesInUseA = cancelTeeTimes(req, out, con, session);           // process cancel tee times request (those selected or all)
       
       if (timesInUseA.size() > 0) {          // if any tee times could not be canceled
          ...
          // inform user !!!!!!!!!!!!!!
           ...
           out.println(SystemUtils.HeadTitle("Cancel Tee Times"));
           out.println("<body bgcolor=\"#FFFFFF\" text=\"#000000\">");
           out.println("");
           out.println("<p align=\"center\"><BR><font size=5>Cancel Tee Times</font></p>");

           out.println("<br><p align=\"center\">");
           out.println("<font size=3>Not all requested tee times were canceled.  The following tee time(s)<br>were in use and "
                    + "could not be canceled.<br><br>");
                 
           for (int i=0; i<timesInUseA.size(); i++) {
               
               time = timesInUseA.get(i);    // get a time
               hr = time/100;
               min = time - (hr * 100);  
               
               out.println(hr+ ":" +SystemUtils.ensureDoubleDigit(min)+ "<br><br>");                 
           }
           
           out.println("</font></p>");      
    
           out.println("<center><p align=\"center\">");
           out.println("<form method=\"GET\" action=\"Proshop_dsheet\">");
           ...
           out.println("</form></p></center>");    
           out.println("</body></html>");
           out.close();
           return;         // Continue button will rebuild the edit tee sheet
       }
    }
...
*/
   ...
   if (club.equals("indianridgecc")) {      // Indian Ridge CC - Change colors for both courses
   ...
   long date = Utilities.getDate(con, index);   // get the date based on the index - use adjusted time
   ...
   //
   int cal_time = Utilities.getTime(con);
   ...
   if (req.getParameter("backToSheet") != null) {
       ...
       if (!mode.equals("")) {
           ...
           if (mode.equals("WAITLIST")) {
               ...
               whereClause = "date = ? AND lottery_email = ? AND lottery = ? AND " +
                             "(player1 <> '' || player2 <> '' || player3 <> '' || player4 <> '' || player5 <> '')";  // check for members or guests
           }
           ...
           //
           //  check for empty event or a lottery that has completed (processed but no requests/tee times - must be cleared
           //
           if (tmp_count == 0) {
               
               if (tmp_mode.equals("Lottery")) {
               
                  out.println("<center>");
                  out.println("<br><br><h2>No Lottery Requests Processed</h2>");
                  out.println("<p>There are no times for " + ((!name.equals("")) ? name : "") + " that need to have emails sent.</p>");
                  out.println("<p>Would you like to complete the lottery processing and open the tee sheet?</p>");
                  out.println("<form>");
                  ...
                  out.println("<input type=submit value=\"Yes - Complete the Lottery for this date\">");
                  out.println("</form>");
               ...
               } else if (tmp_mode.equals("Event") && handicapSys) {      // no emails to send for event and club uses a handicap system
                  
                  //  Prompt user to see if the scores for this event will not be posted
                  
                  out.println("<center>");
                  out.println("<br><br><h2>No Emails to Send for this Event</h2>");
                  out.println("<p>There are no times for " + ((!name.equals("")) ? name : "") + " that need to have emails sent.</p>");
                  out.println("<br><p>If the format of this event is such that members will not be required to post scores,<br>"
                            + "you can flag these rounds as 'No Post'.</p>");
                  out.println("<br><p>Would you like to flag these member rounds as 'No Post'?</p>");
                  out.println("<form>");
                  ...
                  out.println("<input type=hidden name=\"course\" value=\"" + course + "\">");
                  out.println("<input type=hidden name=\"returnCourse\" value=\"" + returnCourse + "\">");
                  out.println("<input type=hidden name=\"setNoPost\" value=\"1\">");

                  out.println("<input type=submit value=\"Yes - set member rounds to 'No Post'\">");
                  out.println("</form>");
       ...
       if (tmp_count == 0) {
           ...
           out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_jump?index=" + index + "&course=" + course + "\">");
            ...
            out.println("<p>There are " + tmp_count + " " + tmp_mode.toLowerCase() + " times for " + ((!name.equals("")) ? name : "") + " that may need to have emails sent<br>to notify the players of their assigned times.</p>");
            ...
            if (tmp_mode.equals("Lottery")) {
                ...
                out.println("<p><br><input type=\"checkbox\" name=\"sendUnass\" value=\"1\">&nbsp;&nbsp;<strong>Send emails to Unassigned players (if any)</strong><br>(if checked, emails will also be sent to those that did not get a time)</p>");
                  
            } else if (tmp_mode.equals("Event") && handicapSys) {      // no emails to send for event and club uses a handicap system
                  
                out.println("<p><b>Note:</b> &nbsp;If the format of this event is such that members will not be required to post scores,<br>"
                          + "you can flag these rounds as 'No Post' at this time. &nbsp;All member rounds in the event will be flagged.</p>");
                out.println("<p><br><input type=\"checkbox\" name=\"setNoPost\" value=\"1\">&nbsp;&nbsp;<strong>Set 'No Post' indicator for members in this event.</strong></p>");
            }
            ...
            out.println("<br>- OR -<br><br><input type=submit value=\"Send Later\">");
            
            out.println("</form>");
            
            out.println("<p>Note:&nbsp; <i>Emails will NOT be sent to players that do not have email addresses or have elected not to receive email notifications.</i></p>");
       ...
       if (req.getParameter("sendUnass") != null) sendUnass = Integer.parseInt(req.getParameter("sendUnass"));   // send unassigned emails too?
           
       if (req.getParameter("setNoPost") != null) setNoPost = Integer.parseInt(req.getParameter("setNoPost"));   // set No Post indicator in all event tee times too?
       ...
       if (mode.equals("WAITLIST")) {
           ...
           logLotteryReqs(club, clubName, name, (int)date, index, course, con); 
           
           sendLotteryEmails(club, clubName, name, (int)date, index, course, returnCourse, out, con, sendUnass); 
           
           setLotteryState5(clubName, club, name, (int)date, index, out, con);
           
       } else if (mode.equals("EVENT")) {
           
           sendEventEmails(clubName, name, (int)date, index, course, out, con, setNoPost);
       ...
       //
       //out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_dlott?index=" + index + "&course=" + course + "&lott_name=" + lott_name + "&hide="+hideUnavail+"&backToSheet\">");
   ...
   try {
      ...
      */
        ...
        try {
            getItem.getEvents(date, course, parmEvents, 0, con);
        } catch (Exception e) {
            dbError(out, e, index, course, "");
        }
      ...
      */
      ...
      //
      if (course.equals( "-ALL-" )) {
         string7b = "SELECT name, recurr, color, id, stime, etime FROM restriction2 WHERE sdate <= ? AND edate >= ? AND activity_id = 0 " +
                    "AND showit = 'Yes'";
      } else {
         string7b = "SELECT name, recurr, color, id, stime, etime FROM restriction2 WHERE sdate <= ? AND edate >= ? AND activity_id = 0 " +
      ...
      if (course.equals( "-ALL-" )) {
         string7c = "SELECT name, color FROM events2b WHERE date = ? AND activity_id = 0 AND inactive = 0";
      } else {
         string7c = "SELECT name, color FROM events2b WHERE date = ? AND activity_id = 0 AND inactive = 0 " +
      ...
      int [] ucol_width = new int [10];
      int ucol_start[] = new int[10]; 
      ...
      if (mode.equals("WAITLIST")) {
          ...
          ucol_width[7] = (multi_lot == 1 && courseCount > 1) ? 110 : 0;
          ucol_width[8] = 40;                                       // weight
          ucol_width[9] = (fivesALL == 0) ? 410 : 560;              // members
          ...
          ucol_start[7] = ucol_start[6] + ucol_width[6];
          ucol_start[8] = ucol_start[7] + ucol_width[7];
          ucol_start[9] = ucol_start[8] + ucol_width[8];
    ...
    //out.println(SystemUtils.DOCTYPE);
      ...
      out.println("<head>");
      out.println("<meta http-equiv=\"X-UA-Compatible\" content=\"IE=EmulateIE8\">");  // force IE9 rendering mode - fix for IE10 drag-n-drop page
      ...
      out.println("<tr><td align=\"left\" valign=\"middle\">");
         ...
         out.println("<a href=\"Proshop_dsheet?mode=" + mode + "&index=" +index+ "&name=" +name+ "&course=" +course+ "&returnCourse=" +returnCourse+ "&email=" +emailOpt+ "&hide=" + hideUnavail + "\" class=cpLink title=\"Refresh This Page\" alt=\"Refresh\">");
         ...
         if (index >= 0) {
             ...
             out.println("</font></td></tr><tr><td align=\"center\"><font size=\"2\">");
             out.println("<a href=\"Proshop_dsheet?mode=" + mode + "&index=" +index+ "&name=" +name+ "&course=" +course+ "&returnCourse=" +returnCourse+ "&blockers&email=" +emailOpt+ "\" class=cpLink title=\"Block or Unblock Tee Times\" alt=\"Blockers\">");
             ...
             if (mode.equals("EVENT") || mode.equals("")) {
                 ...
                 rs = stmt.executeQuery("SELECT name FROM events2b WHERE activity_id = 0 AND inactive = 0 AND type = 1 AND date = " +date+ " LIMIT 1"); // shotgun event on this day?                
                 if (rs.next()) {
                    out.println("</font></td></tr><tr><td align=\"center\"><font size=\"2\">");
                    out.println("<a href=\"Proshop_dsheet?mode=" + mode + "&index=" +index+ "&name=" +name+ "&course=" +course+ "&sha&email=" +emailOpt+ "\" class=cpLink title=\"Shotgun Hole Assignments\" alt=\"Shotgun\">");
         ...
         out.println("<a href=\"Proshop_dsheet?mode=" + mode + "&index=" +index+ "&name=" +name+ "&course=" +course+ "&returnCourse=" +returnCourse+ "&email=" +emailOpt+ ((hideUnavail.equals("1") ? "" : "&hide=1")) + "\" class=cpLink title=\"Toggle Hiding Tee Times\" alt=\"Toggle Hiding Tee Timess\">");
         ...
         if (mode.equals("LOTT") && !course.equals("-ALL-")) {
             out.println("</font></td></tr><tr><td align=\"center\" nowrap><font size=\"2\">");
             out.println("<a href=\"Proshop_dsheet?mode=" + mode + "&index=" +index+ "&date=" + date + "&course=" +course+ "&returnCourse=" +returnCourse+ "&name=" + name + "&hide=" + hideUnavail + "&convert=all&email=" +emailOpt+ "\" title=\"Convert All Requests\" alt=\"Convert All Requests\">");
         ...
         if (IS_TLT && index >= 0) {
             out.println("</font></td></tr><tr><td align=\"center\"><font size=\"2\">");
             out.println("<a href=\"Proshop_jump?newnotify=1&course=" +course+ "&mm=" + month + "&dd=" + day + "&yy=" + year + "\" class=cpLink title=\"Make New Notification\" alt=\"Make New Notification\">");
             out.println("<nobr>Make New Notification</nobr></a><br>");
         }
         
         // show flip all f/b options and cancel tee times if NOT in lott mode and NOT all courses
         if (!mode.equals("LOTT") && !course.equals("-ALL-") && index >= 0 && mode.equals("")) {
            
             out.println("</font></td></tr><tr><td align=\"center\" nowrap><font size=\"2\">");
             out.println("<a href=\"Proshop_dsheet?mode=" + mode + "&index=" +index+ "&date=" + date + "&course=" +course+ "&returnCourse=" +returnCourse+ "&flipallfb=yes&email=" +emailOpt+ "\" title=\"Flip All F/B Options\" alt=\"Flip All F/B Options\">");
             out.println("Flip All F/B Options</a><br>");
             
             // cancel tee times
             out.println("</font></td></tr><tr><td align=\"center\"><font size=\"2\">");
             out.println("<a href=\"Proshop_dsheet?mode=" + mode + "&index=" +index+ "&course=" +course+ "&returnCourse=" +returnCourse+ "&blockers&canceltees&email=" +emailOpt+ "\" class=cpLink title=\"Cancel Tee Times\" alt=\"Cancel Tee Times\">");
             out.println("Cancel Tee Times</a><br>");
         }
         ...
         if (index >= 0) {
             ...
             out.println("<a href=\"Proshop_dsheet?mode=" + mode + "&index=" +index+ "&course=" +course+ "&returnCourse=" +returnCourse+ "&name=" + name + "&date=" + date + "&backToSheet\" title=\"Return to Tee Sheet\" alt=\"Return\">");

             //out.println("<a href=\"Proshop_jump?index=" +index+ "&course=" +course+ "\" class=cpLink title=\"Return to Tee Sheet\" alt=\"Return\">");
             
             
             //out.println("<a href=\"Proshop_dlott?index=" +index+ "&course=" +course+ "&lott_name=" + lott_name + "&lott_date=" + date + "&backToSheet\" title=\"Return to Tee Sheet\" alt=\"Return\">");
             //out.println("<a href=\"Proshop_devnt?index=" +index+ "&course=" +course+ "&name=" + event_name + "&event_date=" + date + "&backToSheet\" title=\"Return to Tee Sheet\" alt=\"Return\">");
             out.println("<nobr>Return to Tee Sheet</nobr></a><br>");
         } else {
             // links for old tee sheets
             out.println("</font></td></tr><tr><td align=\"center\"><font size=\"2\">");
             out.println("<a href=\"Proshop_jump?index=" +index+ "&course=" +course+ "&oldsheets\" class=cpLink title=\"Return to Date Selection\" alt=\"Return\">");
             out.println("<nobr>Return to Date Selection</nobr></a><br>");
             
             out.println("</font></td></tr><tr><td align=\"center\"><font size=\"2\">");
             out.println("<a href=\"Proshop_jump?oldsheets&course=" + course + "&calDate=" + month + "/" + day + "/" + year + "\" class=cpLink title=\"Back\" alt=\"Back\" onclick=\"return checkCourse()\">");
      ...
      if (parm.multi != 0) {           // if multiple courses supported for this club
         ...
         if (courseCount < 5) {        // if < 5 courses, use buttons
            ...
            for (i=0; i<courseA.size(); i++) {
               ...
               out.print("<a href=\"Proshop_dsheet?mode="+mode+"&index=" +index+ "&name=" +name+ "&course=" +courseName+ "&returnCourse=" +returnCourse+ "&hide=" + hideUnavail + "\" style=\"color:blue\" target=\"_top\" title=\"Switch to new course\" alt=\"" +courseName+ "\">");
            ...
            out.println("<form action=\"Proshop_dsheet\" method=\"get\" name=\"cform\" target=\"_top\">");
            out.println("<input type=\"hidden\" name=\"mode\" value=\"" + mode + "\">");
            out.println("<input type=\"hidden\" name=\"name\" value=\"" + name + "\">");
          ...
          if (SystemUtils.verifyProAccess(req, "EVNTSUP_MANAGE", con, out)) {
              for (z = 0; z < parmEvents.count; z++) {
                 ...
                 out.println("<button type=\"button\" style=\"background:" + parmEvents.color[z] + "\" " +
                         "onclick=\"location.href='Proshop_dsheet?mode=EVENT&name=" + parmEvents.name[z] + "&index=" + index + "&course=" + course + "&hide=" + hideUnavail + "&email=" + emailOpt + "'\">" + parmEvents.name[z] + "</button>");
                 out.println("&nbsp;&nbsp;&nbsp;&nbsp;");
          ...
          if (SystemUtils.verifyProAccess(req, "LOTT_APPROVE", con, out)) {
              for (z = 0; z < parmLotteries.count; z++) {
                 ...
                 out.println("<button type=\"button\" style=\"background:" + parmLotteries.color[z] + "\" " +
                         "onclick=\"location.href='Proshop_dsheet?mode=LOTT&name=" + parmLotteries.name[z] + "&index=" + index + "&course=" + course + "&hide=" + hideUnavail + "&email=" + emailOpt + "'\">" + parmLotteries.name[z] + "</button>");
                 out.println("&nbsp;&nbsp;&nbsp;&nbsp;");
          ...
          if (SystemUtils.verifyProAccess(req, "WAITLIST_MANAGE", con, out)) {
              for (z = 0; z < parmWaitLists.count; z++) {
                 ...
                 out.println("<button type=\"button\" style=\"background:" + parmWaitLists.color[z] + "\" " +
                         "onclick=\"location.href='Proshop_dsheet?mode=WAITLIST&name=" + parmWaitLists.name[z] + "&index=" + index + "&course=" + course + "&hide=" + hideUnavail + "&email=" + emailOpt + "'\">" + parmWaitLists.name[z] + "</button>");
                 out.println("&nbsp;&nbsp;&nbsp;&nbsp;");
     ...
     /*
      if (!event1.equals( "" )) {
         ...
         out.println("<button type=\"button\" style=\"background:" + ecolor1 + "\" " +
                 "onclick=\"location.href='Proshop_dsheet?mode=EVENT&name=" + event1 + "&index=" + index + "&hide=" + hideUnavail + "&email=" + emailOpt + "'\">" + event1 + "</button>");
         out.println("&nbsp;&nbsp;&nbsp;&nbsp;");
     ...
     /*
      if (!lott1.equals( "" )) {
         ...
         out.println("<button type=\"button\" style=\"background:" + lcolor1 + "\" " +
                 "onclick=\"location.href='Proshop_dsheet?mode=LOTT&name=" + lott1 + "&index=" + index + "&hide=" + hideUnavail + "&email=" + emailOpt + "'\">" + lott1 + "</button>");
         out.println("&nbsp;&nbsp;&nbsp;&nbsp;");
     ...
     */
      ...
      errMsg = "Build Main Sheet Table.";
       ...
       String dts_defaultF3Color = ""; // default
       ...
       if (mode.equals("WAITLIST")) {
          ...
          while (rs.next()) {
              ...
              if (in_use == 0) {
                  ...
                  out.print("<a href=\"Proshop_waitlist_slot?index=" +index+ "&signupId=" +wait_list_signup_id+ "&returnCourse=" +course+ "&email=" +emailOpt+ "&name=" +name+ "\" title=\"Edit signup\" alt=\"Edit signup\">");
                  ...
                  out.print("<a href=\"Proshop_dsheet?mode=" +mode+ "&index=" +index+ "&wlsId=" +wait_list_signup_id+ "&returnCourse=" +course+ "&email=" +emailOpt+ "&name=" +name+ "&hide=" +hideUnavail+ "&delete=yes\" title=\"Delete signup\" alt=\"Remove signup\" onclick=\"return confirm('Are you sure you want to permanently delete this signup?');\">");
             ...
             */
              ...
              if (!notes.equals("")) {
                  ...
                  out.println("&nbsp; &nbsp; <img src=\"/"+rev+"/images/notes.gif\" width=10 height=12 border=0 title=\"" + StringEscapeUtils.escapeHtml(notes) + "\">");
              }
   ...
   //       if (course.equals( "-ALL-" )) {
          if (multi_lot == 1) {              // if multi courses
              ...
              if (courseCount > 1) {
                  out.println(" class=header style=\"left: " +ucol_start[7]+ "px; width: " +ucol_width[7]+ "px\">Other Course?</span><span ");
              }
          }
          out.println(" class=header style=\"left: " +ucol_start[8]+ "px; width: " +ucol_width[8]+ "px\">W</span><span id=widthMarker2 ");
          out.println(" class=header style=\"left: " +ucol_start[9]+ "px; width: " +ucol_width[9]+ "px\">Members</span>");
          out.print("</div>\n");
      ...
      */    
          ...
          */
          ...
          String order_by = "r_date ASC, r_time ASC"; // default sorting
          
          if (club.equals("loxahatchee")) {
              order_by = "player1, player2, player3, player4, player5";    // They want to sort alphabetically instead
          }
          ...
          while (rs.next()) {
              ...
              if (!player1.equals("")) {
                  ...
                  //out.print(notes);
                  if (!rs.getString("notes").equals("")) out.println("<img src=\"/"+rev+"/images/notes.gif\" width=10 height=12 border=0 title=\"" + StringEscapeUtils.escapeHtml(rs.getString("notes")) + "\">");
                  out.println("</span>");
          ...
          while (rs.next()) {
              ...
              if (in_use == 0) {
                  ...
                  // not in use
                  out.print("<a href=\"ProshopTLT_slot?index=" +index+ "&notifyId=" +notification_id+ "&returnCourse=" +course+ "&email=" +emailOpt+ "\" title=\"Edit notification\" alt=\"Edit notification\">");
                  out.print("<img src=/" +rev+ "/images/dts_edit.gif width=13 height=13 border=0></a>");
                  out.print("<img src=/" +rev+ "/images/shim.gif width=5 height=1 border=0>");
                  out.print("<a href=\"Proshop_dsheet?mode=" +mode+ "&index=" +index+ "&notifyId=" +notification_id+ "&returnCourse=" +course+ "&email=" +emailOpt+ "&delete=yes\" title=\"Delete notification\" alt=\"Remove notification\" onclick=\"return confirm('Are you sure you want to permanently delete this notification?');\">");
                  out.print("<img src=/" +rev+ "/images/dts_trash.gif width=13 height=13 border=0></a>");
       ...
       if (index < 0) {
           ...
           if (club.equals( "edisonclub" )) {
               ...
               if (club.equals("elcaminocc")) {
                   stringTee += "ORDER BY fb, time, courseName";
               } else {
            ...
            if ( blocker.equals( "" ) && blnHide == false) {      // continue if tee time not blocked - else skip
               ...
               */
               ...
               if (in_use == 0 && index >= 0) {
                   ...
                   // not in use 
                   out.print("<a href=\"Proshop_dsheet?mode=" +mode+ "&name=" +name+ "&index=" +index+ "&course=" +courseT+ "&returnCourse=" +course+ "&time=" +time+ "&fb=" +fb+ "&jump=" +j+ "&email=" +emailOpt+ "&first=" +first+ "&insert=yes\" title=\"Insert a time slot\" alt=\"Insert a time slot\">");
                   out.print("<img src=/" +rev+ "/images/dts_newrow.gif width=13 height=13 border=0></a>");
                   out.print("<img src=/" +rev+ "/images/shim.gif width=5 height=1 border=0>");
                   out.print("<a href=\"Proshop_dsheet?mode=" +mode+ "&name=" +name+ "&index=" +index+ "&course=" +courseT+ "&returnCourse=" +course+ "&time=" +time+ "&fb=" +fb+ "&jump=" +j+ "&email=" +emailOpt+ "&delete=yes\" title=\"Delete time slot\" alt=\"Remove time slot\">");
                   out.print("<img src=/" +rev+ "/images/dts_trash.gif width=13 height=13 border=0></a>");
       ...
       //  Nothing seems to work - can't get blank lines after the tee sheet !!!!           
       //  out.println("<div><p>&nbsp;</p><p>&nbsp;</p><p>&nbsp;</p></div>"); // spacer at bottom of tee sheet
       ...
       out.println("<form name=frmSendAction method=POST action=Proshop_dsheet>");
       ...
       out.println("<div id=elFBOPopup defaultValue=\"\" jump=\"\">"); //  style=\"visibility: hidden\"
       out.println("<form name=frmFBO>");
       out.println("<input type=hidden name=jump value=\"\">");
       out.println("<table id=elFBOTable border=0 cellpadding=0 cellspacing=2 style=\"width: 100%; height: 100%\">"); //width='120' height='100' 
       ...
       out.println("<tr><td style=\"text-align: center;\"><a href=\"javascript: cancelFBOPopup()\" class=smtext>cancel</a>&nbsp;&nbsp;&nbsp;<a href=\"javascript: saveFBOPopup()\" class=smtext>save</a>&nbsp;</td></tr>");
       out.println("</table>");
       out.println("</form>");
       out.println("</div>");
       ...
       out.println("<table id=elTOTable border=0 cellpadding=0 cellspacing=2>"); // width='280' height='130' 
       out.println("<input type=hidden name=jump value=\"\">");
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   if (date == 0) {
      ...
      //  Get today's date and then use the value passed to locate the requested date
      //
      date = Utilities.getDate(con, slotParms.ind);    // Get the current date (adjusted to time zone) 
      ...
      cal.set(slotParms.yy, slotParms.mm - 1, slotParms.dd);
   ...
   out.println("<BR><BR>");
   out.println("<font size=\"2\">");
   out.println("<a href=\"Proshop_jump?index=" +slotParms.ind+ "&course=" +slotParms.returnCourse+ "\">");
 ...
 private void doInsert(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
   ...
   if (req.getParameter("insertSubmit") == null) {      // if not an insert 'submit' request (from self)
      ...
      if (first.equalsIgnoreCase( "yes" )) {
         ...
         if (club.equals("edisonclub")) {       // reset maxtimes to default for calculation of earliest time
             maxtimes = 20;
         }
         ...
         if (club.equals("edisonclub")) {
             maxtimes = 260;        // Set maxtimes to 20 + 240, so it will allow 20 minutes before, and up to 4 hrs after
         } else {
      ...
      try {
         ...
         SystemUtils.updateHist(date, day_name, time, fb, course, "Tee Time Inserted", "", "", "", "", user, "Proshop User", 1, 0, 0, con);
      ...
      out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_dsheet?mode=" +mode+ "&name=" +name+ "&index=" +indexs+ "&course=" +returnCourse+ "&jump=" +jump+ "&email=" +emailOpt+ "\">");
      out.println("</HEAD>");
      out.println("<BODY bgcolor=\"#FFFFFF\" text=\"#000000\">");
...
//      SystemUtils.doBlockers(con);
 ...
 private void doDelete(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
   ...
   if (req.getParameter("deleteSubmit") == null) {      // if this is the first request
      ...
      try {
         ...
         if (count > 0) {
             SystemUtils.updateHist(date, day_name, time, fb, course, "Tee Time Deleted", "", "", "", "", user, "Proshop User", 1, 0, 0, con);
         }
      ...
      out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_dsheet?mode=" +mode+ "&name=" +name+ "&index=" +indexs+ "&course=" +returnCourse+ "&jump=" +jump+ "&email=" +emailOpt+ "&jump=" +jump+ "\">");
      out.println("</HEAD>");
      out.println("<BODY bgcolor=\"#FFFFFF\" text=\"#000000\">");
      out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
      out.println("<center>");
 ...
 private void doDeleteNotifyReq(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
    ...
    out.println("<HTML><HEAD><title>Proshop Delete Confirmation</title>");
    out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_dsheet?index=" +index+ "&course=" +returnCourse+ "&jump=" +jump+ "&email=" +emailOpt+ "&jump=" +jump+ "\">");
    out.println("</HEAD>");
    out.println("<BODY bgcolor=\"#FFFFFF\" text=\"#000000\">");
    out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
    out.println("<center>");
 ...
 private void doDeleteLottReq(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
    ...
    out.println("<HTML><HEAD><title>Proshop Delete Confirmation</title>");
    out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_dsheet?mode=LOTT&index=" +index+ "&course=" +returnCourse+ "&email=" +emailOpt+ "&jump=" +jump+ "&hide=" +hide+ "&name=" +name+ "\">");
    out.println("</HEAD>");
    out.println("<BODY bgcolor=\"#FFFFFF\" text=\"#000000\">");
    out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
    out.println("<center>");
 ...
 private void doDeleteEventReq(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
    ...
    out.println("<HTML><HEAD><title>Proshop Delete Confirmation</title>");
    out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_dsheet?mode=EVENT&index=" +index+ "&course=" +returnCourse+ "&email=" +emailOpt+ "&jump=" +jump+ "&hide=" +hide+ "&name=" +name+ "\">");
    out.println("</HEAD>");
    out.println("<BODY bgcolor=\"#FFFFFF\" text=\"#000000\">");
    out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
    out.println("<center>");
 ...
 private void doDeleteWaitReq(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
    ...
    out.println("<HTML><HEAD><title>Proshop Delete Confirmation</title>");
    out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_dsheet?mode=WAITLIST&index=" +index+ "&course=" +returnCourse+ "&email=" +emailOpt+ "&jump=" +jump+ "&hide=" +hide+ "&name=" +name+ "\">");
    out.println("</HEAD>");
    out.println("<BODY bgcolor=\"#FFFFFF\" text=\"#000000\">");
    out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
    out.println("<center>");
 ...
 private void changeCW(parmSlot slotParms, String changeAll, String ninehole, long date, HttpServletRequest req, PrintWriter out, Connection con, HttpServletResponse resp) {
   ...
   if (date == 0 || slotParms.from_time == 0 || slotParms.from_course == null || slotParms.user.equals( "" ) || slotParms.user == null) {
      ...
      try {
         ...
         in_use = verifySlot.checkInUse(date, slotParms.from_time, slotParms.from_fb, slotParms.from_course, slotParms.user, slotParms, req);
   ...
   try {
      ...
      PreparedStatement pstmt1 = con.prepareStatement (
         "UPDATE teecurr2 SET last_mod_date = now(), " +
 ...
 private void changeFB(parmSlot slotParms, long date, HttpServletRequest req, PrintWriter out, Connection con, HttpServletResponse resp) {
   ...
   if (date == 0 || slotParms.from_time == 0 || slotParms.from_course == null || slotParms.user.equals( "" ) || slotParms.user == null) {
      ...
      try {
         ...
         dbError(out, e1, slotParms.ind, slotParms.returnCourse, "Error 1 in changeFB. ");
         return;
      }
   }
   ...
   try {
      ...
      PreparedStatement pstmt1 = con.prepareStatement (
         "UPDATE teecurr2 SET last_mod_date = now(), in_use = 0, fb = ? " +
         "WHERE date = ? AND time = ? AND fb = ? AND courseName = ?");
      ...
      dbError(out, e1, slotParms.ind, slotParms.returnCourse, "Error 2 in changeFB. ");
      return;
   }
 ...
 private void moveWhole(parmSlot slotParms, long date, String prompt, int skip, HttpServletRequest req, 
   ...
   String orig5 = "";
   String customMsg = "";
   String lottery = "";           // lottery name from the 'from' tee time (in case it was part of a lottery)
   ...
   int guest_id4 = 0;
   int guest_id5 = 0;
   long teecurr_id_from = 0;         // 'from' teecurr id
   long teecurr_id_to = 0;           // 'to' teecurr id
   int lottery_email = 0;
   int shotgun_time = 0;
   int suppress_emails = 0;
   int custom_int = 0;
   ...
   teecurr_id_from = slotParms.teecurr_id;
   lottery = slotParms.lottery;

   //
   //  If the From time is part of a lottery, get its lottery_email value so we can transfer that to the To time
   //
   if (!slotParms.lottery.equals("")) {
       
      lottery_email = getLotteryEmail(date, slotParms.from_time, slotParms.from_fb, slotParms.from_course, con);    
   }
   ...
   if (slotParms.sendEmail.equalsIgnoreCase( "yes" )) {
       suppress_emails = 2;
   } else {
       suppress_emails = 1;
   }
   ...
   if (date == 0 || slotParms.to_time == 0 || slotParms.to_course == null || slotParms.user.equals( "" ) || slotParms.user == null) {
      ...
      try {
         ...
         if (!prompt.equals( "" )) {        // if return, tee time already busy
            ...
            in_use = verifySlot.checkInUse(date, slotParms.to_time, slotParms.to_fb, slotParms.to_course, slotParms.user, slotParms, req);
         }
   ...
   // If the to_time is a shotgun event, look up the actual start time of the event
   if (!slotParms.event.equals("") && slotParms.event_type == 1) {
       shotgun_time = Utilities.getEventTime(slotParms.event, con);
   }
   ...
   if (prompt.equals( "return" )) {        // if prompt specified a return
      ...
      out.println("<meta http-equiv=\"Refresh\" content=\"0; url=Proshop_dsheet?mode=" +mode+ "&name=" +slotParms.name+ "&index=" + slotParms.ind + "&course=" + slotParms.returnCourse + "&email=" + slotParms.sendEmail + "&jump=" + slotParms.jump + "\">");
      out.println("</HEAD>");
      out.println("<BODY bgcolor=\"#FFFFFF\" text=\"#000000\">");
      out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
      ...
      teecurr_id_to = slotParms.teecurr_id;
   ...
   if (!player1.equals( "" ) || !player2.equals( "" ) || !player3.equals( "" ) || !player4.equals( "" ) || !player5.equals( "" )) {
      ...
      try {
        ...
        String sqlString = "UPDATE teecurr2 SET last_mod_date = now(), player1 = ?, player2 = ?, player3 = ?, player4 = ?, " +
            ...
            "orig1 = ?, orig2 = ?, orig3 = ?, orig4 = ?, orig5 = ?, custom_int = ?";
         if (lottery_email > 0) {
            sqlString += ", lottery_email = ?";
         }
         sqlString += " WHERE date = ? AND time = ? AND fb = ? AND courseName = ?";

         
         PreparedStatement pstmt6 = con.prepareStatement (sqlString);
         ...
         pstmt6.setString(66, orig1);
         pstmt6.setString(67, orig2);
         pstmt6.setString(68, orig3);
         pstmt6.setString(69, orig4);
         pstmt6.setString(70, orig5);
         pstmt6.setInt(71, custom_int);

         if (lottery_email > 0) {

            pstmt6.setInt(72, lottery_email);
            pstmt6.setLong(73, date);
            pstmt6.setInt(74, slotParms.to_time);
            pstmt6.setInt(75, slotParms.to_fb);
            pstmt6.setString(76, slotParms.to_course);

         } else {
            
            pstmt6.setLong(72, date);
            pstmt6.setInt(73, slotParms.to_time);
            pstmt6.setInt(74, slotParms.to_fb);
            pstmt6.setString(75, slotParms.to_course);
         }
      ...
      //
      //  Create group bindings for this entry
      //
      // I don't trust teecurr_id_from and to from above, so I'll just get them again
      long teecurr_id_from2 = SystemUtils.getTeeCurrId(date, slotParms.from_time, slotParms.from_fb, slotParms.from_course, con);
      long teecurr_id_to2 = SystemUtils.getTeeCurrId(date, slotParms.to_time, slotParms.to_fb, slotParms.to_course, con);
      // There is an edge case where a single teecurr is bound to multiple groups, so we get them all and re-bind them all.
      // (This edge case is a result from the drag and drop of lottery requests, and may need to be better managed)
      List<Long> group_ids = verifySlot.getTeeCurrGroupIdsFromTeeCurrId(teecurr_id_from2, con);
      for(long group_id : group_ids){
          verifySlot.addTeeCurrGroupDetail(teecurr_id_to2, group_id, con);
      }
      ...
      SystemUtils.updateHist(date, slotParms.day, slotParms.to_time, slotParms.to_fb, slotParms.to_course, player1, player2, player3,
                             player4, player5, slotParms.user, fullName, 0, suppress_emails, 0, con);
      ...
      try {
         ...
         PreparedStatement pstmt5 = con.prepareStatement (
            ...
            "mNum1 = '', mNum2 = '', mNum3 = '', mNum4 = '', mNum5 = '', " +
            "userg1 = '', userg2 = '', userg3 = '', userg4 = '', userg5 = '', orig_by = '', conf = '', " +
            "pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0, pos5 = 0, custom_int = 0, " +
            "custom_disp1 = '', custom_disp2 = '', custom_disp3 = '', custom_disp4 = '', custom_disp5 = '', " +
            "tflag1 = '', tflag2 = '', tflag3 = '', tflag4 = '', tflag5 = '', " +
            "guest_id1 = 0, guest_id2 = 0, guest_id3 = 0, guest_id4 = 0, guest_id5 = 0, lottery_email = 0 " +
            "WHERE date = ? AND time = ? AND fb = ? AND courseName = ?");
      ...
      SystemUtils.updateHist(date, slotParms.day, slotParms.from_time, slotParms.from_fb, slotParms.from_course, empty, empty, empty,
                             empty, empty, slotParms.user, fullName, 1, suppress_emails, 0, con);
      
      
      
      //
      //  Lastly, if the 'from' tee time was part of a lottery and was already assigned & approved, then change the assigned teecurr_id so it is identified correctly in the email notification
      //
      if (!lottery.equals("")) {
         
         try {

             PreparedStatement pstmt = con.prepareStatement("UPDATE lreqs3 SET teecurr_id1 = ? WHERE teecurr_id1 = ? AND name = ?");
             pstmt.clearParameters();
             pstmt.setLong(1, teecurr_id_to2);
             pstmt.setLong(2, teecurr_id_from2);
             pstmt.setString(3, lottery);
             pstmt.executeUpdate();
             pstmt.close();

             pstmt = con.prepareStatement("UPDATE lreqs3 SET teecurr_id2 = ? WHERE teecurr_id2 = ? AND name = ?");
             pstmt.clearParameters();
             pstmt.setLong(1, teecurr_id_to2);
             pstmt.setLong(2, teecurr_id_from2);
             pstmt.setString(3, lottery);
             ...
             pstmt = con.prepareStatement("UPDATE lreqs3 SET teecurr_id3 = ? WHERE teecurr_id3 = ? AND name = ?");
             pstmt.clearParameters();
             pstmt.setLong(1, teecurr_id_to2);
             pstmt.setLong(2, teecurr_id_from2);
             pstmt.setString(3, lottery);
             ...
             pstmt = con.prepareStatement("UPDATE lreqs3 SET teecurr_id4 = ? WHERE teecurr_id4 = ? AND name = ?");
             pstmt.clearParameters();
             pstmt.setLong(1, teecurr_id_to2);
             pstmt.setLong(2, teecurr_id_from2);
             pstmt.setString(3, lottery);
             ...
             pstmt = con.prepareStatement("UPDATE lreqs3 SET teecurr_id5 = ? WHERE teecurr_id5 = ? AND name = ?");
             pstmt.clearParameters();
             pstmt.setLong(1, teecurr_id_to2);
             pstmt.setLong(2, teecurr_id_from2);
             pstmt.setString(3, lottery);
   ...
   if (sendemail != 0) {
      ...
      //
      parme.activity_id = 0;
      parme.club = slotParms.club;          // use orig parm block
      //parme.guests = slotParms2.guests;     // use the new parm block for guest count
      parme.type = "moveWhole";             // type = Move Whole tee time
      parme.date = date;
      parme.time = 0;    
      
      if (!slotParms.event.equals("") && slotParms.event_type == 1) {
          parme.to_time = shotgun_time;
          parme.from_time = shotgun_time;
      } else {
 ...
 private void moveSingle(parmSlot slotParms, long date, String prompt, int skip, HttpServletRequest req, 
   ...
   String moveP1 = "UPDATE teecurr2 SET last_mod_date = now(), player" +to+ " = ?, username" +to+ " = ?, p" +to+ "cw = ?, in_use = 0, hndcp" +to+ " = ?, show" +to+ " = ?, " +
           "mNum" +to+ " = ?, userg" +to+ " = ?, p9" +to+ " = ?, pos" +to+ " = ?, custom_disp" +to+ " = ?, tflag" +to+ " = ?, guest_id" +to+ " = ?";
   
   if (lottery_email > 0) {
      
      moveP1 += ", lottery_email = ?";
   }
   moveP1 += " WHERE date = ? AND time = ? AND fb = ? AND courseName = ?";
   ...
   try {
      ...
      if (lottery_email > 0) {
         ...
         pstmt6.setInt(13, lottery_email);
         
         pstmt6.setLong(14, date);
         pstmt6.setInt(15, slotParms.to_time);
         pstmt6.setInt(16, slotParms.to_fb);
         pstmt6.setString(17, slotParms.to_course);
      ...
      SystemUtils.updateHist(date, slotParms.day, slotParms.to_time, slotParms.to_fb, slotParms.to_course, player[0], player[1], player[2],
                             player[3], player[4], slotParms.user, fullName, 1, suppress_emails, 0, con);
   ...
   //
     String moveP2 = "UPDATE teecurr2 SET last_mod_date = now(), player" + fr + " = '', username" + fr + " = '', p" + fr + "cw = '', hndcp" + fr + " = 0, in_use = 0, show" + fr + " = 0, "
             + "mNum" + fr + " = '', userg" + fr + " = '', pos" + fr + " = 0, custom_disp" + fr + " = '', tflag" + fr + " = '', guest_id" + fr + " = 0, orig_by = IF(player1 = '' AND player2 = '' "
             + "AND player3 = '' AND player4 = '' AND player5 = '', '', orig_by), custom_int = IF(player1 = '' AND player2 = '' AND player3 = '' AND player4 = '' AND player5 = '', 0, custom_int)  "
             + "WHERE date = ? AND time = ? AND fb = ? AND courseName = ?";
 ...
 **/
 ...
 private void editDone(PrintWriter out, parmSlot slotParms, HttpServletResponse resp, String mode) {
   ...
   try {
       ...
       String url="Proshop_dsheet?mode=" +mode+ "&name=" +slotParms.name+ "&index=" +slotParms.ind+ "&course=" + slotParms.returnCourse + "&jump=" + slotParms.jump + "&email=" + slotParms.sendEmail + "&hide=" + slotParms.hideTimes;
 ...
 private void dbError(PrintWriter out, Exception e1, int index, String course, String eMsg) {
      ...
      out.println("<BR><BR>");
      out.println("<font size=\"2\">");
      out.println("<a href=\"Proshop_jump?index=" +index+ "&course=" +course+ "\">");
      out.println("Return to Tee Sheet</a></font>");
      out.println("</CENTER></BODY></HTML>");
      out.close();
 }
 ...
 private void displayBlockers(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
    ...
    boolean cancelTees = false;
    
    if (req.getParameter("canceltees") != null) {     // if this is for Cancel Tee Times rather than Blockers
   ...
   int colorMax = colors.colorMax;             // max number of colors defined
    ...
    out.println("}");
      out.println("function doCancel(emailOpt, index, course) {");
      out.println(" if (confirm('This will cancel ALL the tee times in the table shown.  ARE YOU SURE THAT YOU WANT TO DO THIS?')) {");
      out.println("   window.location.href = 'Proshop_dsheet?cancelTeeTimes&cancelAll&email=' + emailOpt + '&index=' + index + '&course=' + course;");
      out.println(" }");
      out.println("}");
   out.println("// -->");
   out.println("</script>");
    ...
    if (cancelTees == false) {
       ...
       out.println("<center><p><font size=5>Cancel Tee Times</font></p>");
    }
    ...
    if (cancelTees == true) {    // if we are here for Cancel Tee Times
       
       out.println("<br><table align=\"center\" bgcolor=\"#F5F5DC\" border=\"1\" cellpadding=\"5\"><tr><td align=\"center\">");
       out.println("<font size=3>To cancel one or more tee times, check the box in the Cancel column of each "
               + "tee time that you wish to cancel.<br>Once all tee times have been checked, select the 'Cancel Selected Tee Times' button below."
               + "<br><br>If you would like to cancel ALL the tee times shown below, then select the 'Cancel All Tee Times' button.<br><br>");      
               
       if (emailOpt.equals("yes")) {
            out.println("<strong>NOTE:</strong> &nbsp;This WILL send Email Notifications to the Members in the Canceled Times.");
       } else {
            out.println("<strong>NOTE:</strong> &nbsp;This WILL NOT send Email Notifications to the Members in the Canceled Times.");
       }
       out.println("</font></td></tr></table>");
    }
    ...
    // build tee sheet header
        ...
        out.println("<br><br><table width=\"80%\" align=\"center\"><tr valign=\"top\">");
        
        out.println("<td align=\"center\">");
        out.println("<form method=\"GET\" action=\"Proshop_dsheet\" name=frmRefresh>");
         out.println("<input type=submit value=\" Refresh Sheet \">");
         out.println("<input type=hidden name=blockers value=true>");
         if (cancelTees == true) {
            out.println("<input type=hidden name=canceltees value=true>");            
         }
         out.println("<input type=hidden name=mode value=\"" + mode + "\">");
         out.println("<input type=hidden name=name value=\"" + name + "\">");
         out.println("<input type=hidden name=index value=\"" + index + "\">");
        ...
        out.println("<td align=\"center\">");
        out.println("<form method=\"GET\" action=\"Proshop_jump\" name=frmBack>");
        ...
        out.println("<td align=\"center\">");
        out.println("<form method=\"GET\" action=\"Proshop_dsheet\" name=frmBackEdit>");
        ...
        if (cancelTees == false) {    // are we here for blockers or to cancel tee times?
        
            out.println("<form method=\"POST\" action=\"Proshop_dsheet\" name=frmBlockers>");
            out.println("<td align=\"center\">");
               ...
               out.println("<input type=hidden name=blockers value=true>");
               out.println("<input type=hidden name=index value=\"" + index + "\">");
               out.println("<input type=hidden name=course value=\"" + course + "\">");   // form ends after the table below
               
        } else {
           
            out.println("<form method=\"GET\" action=\"Proshop_dsheet\" name=frmCancel>");
            out.println("<td align=\"center\">");
               out.println(" <input type=\"button\" name=\"cancelAll\" value=\" Cancel ALL Tee Times \" onclick=\"doCancel('" + emailOpt + "','" + index + "','" + course + "')\">");
               out.println("</td>");
               out.println("<input type=hidden name=cancelTeeTimes value=true>");
               out.println("<input type=hidden name=cancelAll value=true>");
               out.println("<input type=hidden name=email value=\"" + emailOpt + "\">");
               out.println("<input type=hidden name=index value=\"" + index + "\">");
               out.println("<input type=hidden name=course value=\"" + course + "\">");
            out.println("</form></td>");
           
            out.println("<form method=\"GET\" action=\"Proshop_dsheet\" name=frmBlockers>");
            out.println("<td align=\"center\">");
            
               out.println("<input type=submit value=\" Cancel Selected Tee Times \" " +               
                           "onclick=\"return confirm('Are you sure you want to cancel the selected tee times? &nbsp;This action cannot be undone!');\">" +
                           "</td>");          
               out.println("<input type=hidden name=cancelTeeTimes value=true>");
               out.println("<input type=hidden name=email value=\"" + emailOpt + "\">");
               out.println("<input type=hidden name=index value=\"" + index + "\">");
    ...
    try {
        ...
        while (rs.next()) {
            ...
            } else {     // Cancel Tee Times process
               
               //
               //  Cancel Tee Time Check Box
               //
               out.println("<td align=\"center\">");

               // do not check the box inititally
               checked = false;
               tmp_tag = " onclick=\"return false\" onmousedown=\"selectRange(event, " + row_id + ")\"";

               // allow access to the checkbox if player1 is present 
               if ( !player1.equals("") && !player1.equals("&nbsp;") ) {
                  
                  // tee time is occupied - allow Cancel checkbox
                  out.println("<input type=checkbox name=\"" + teetime_id + "\" id=\"chk" + row_id + "\" value=\"1\" " + tmp_tag + ">");
                  out.println("<input type=hidden name=\"ID_" + teetime_id + "\" value=\"" + ((checked) ? "1" : "0") + "\">");

               } else {

                  out.println("<input type=checkbox name=\"" + teetime_id + "\" id=\"chk" + row_id + "\" value=\"1\" disabled>");
               }
               out.println("</td>");
            }
 ...
 private void setHoleAssignments(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
    ...
    out.println("<td><form method=\"GET\" action=\"Proshop_jump\" name=frmBack>");
    out.println("<input type=submit value=\" Back To Tee Sheet \">");
    out.println("<input type=hidden name=index value=\"" + index + "\">");
    out.println("<input type=hidden name=course value=\"" + course + "\">");
    out.println("</form></td>");
     
    out.println("<td><form method=\"GET\" action=\"Proshop_dsheet\" name=frmBackEdit>");
    ...
    out.println("<td><form method=\"GET\" action=\"Proshop_dsheet\" name=frmBackHoleAssign>");
 ...
 private void displayHoleAssignments(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
    ...
    if (course.equals( "-ALL-" )) {
     sql = "SELECT name, color FROM events2b WHERE date = ? AND activity_id = 0 AND inactive = 0 ORDER BY stime";
    } else {
     sql = "SELECT name, color FROM events2b WHERE date = ? AND activity_id = 0 AND inactive = 0 " +
    ...
    out.println("<table align=center width=640 bgcolor=#EAEAEA border=1 cellpadding=8><tr><td width=640>");
    out.println("<b>Instructions:</b>  Using the text boxes in the first column of each row, enter the " +
            "hole assignment for each group/slot.  Once finshed click the 'Submit Assignments' button.</td></tr></table>");
    out.println("<br>");
             
    // build tee sheet header
    
        out.println("<br><table width=\"50%\" align=\"center\"><tr valign=middle>");
        ...
        out.println("<td align=\"center\">");
        out.println("<form method=\"POST\" action=\"Proshop_dsheet\" name=frmHoleAssign>");
        ...
        out.println("</tr></table><br>");
         ...
         if (!event1.equals( "" )) {
            ...
            out.println("<a href=\"javascript:void(0)\" onClick=\"window.open('Proshop_sheet?event=" +event1+ "', 'newwindow', 'height=430, width=550, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no directories=no, status=no');return false;\">");
            out.println("<button type=\"button\" style=\"text-decoration:none; background:" + ecolor1 + "\">" + event1 + "</button></a>");
            out.println("&nbsp;&nbsp;&nbsp;");

            if (!event2.equals( "" )) {

                out.println("<a href=\"javascript:void(0)\" onClick=\"window.open('Proshop_sheet?event=" +event2+ "', 'newwindow', 'height=430, width=550, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no directories=no, status=no');return false;\">");
                out.println("<button type=\"button\" style=\"text-decoration:none; background:" + ecolor2 + "\">" + event2 + "</button></a>");
                out.println("&nbsp;&nbsp;&nbsp;");

                if (!event3.equals( "" )) {

                    out.println("<a href=\"javascript:void(0)\" onClick=\"window.open('Proshop_sheet?event=" +event3+ "', 'newwindow', 'height=430, width=550, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no directories=no, status=no');return false;\">");
                    out.println("<button type=\"button\" style=\"text-decoration:none; background:" + ecolor3 + "\">" + event3 + "</button></a>");
                    out.println("&nbsp;&nbsp;&nbsp;");

                    if (!event4.equals( "" )) {

                        out.println("<a href=\"javascript:void(0)\" onClick=\"window.open('Proshop_sheet?event=" +event4+ "', 'newwindow', 'height=430, width=550, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no directories=no, status=no');return false;\">");
         ...
         out.println("<br>");
         out.println("<table border=\"1\" bgcolor=\"#F5F5DC\" align=\"center\" width=720>");

         out.println("<tr bgcolor=\"#336633\">");
            out.println("<td align=\"center\" width=120>");
               out.println("<font color=\"#FFFFFF\" size=\"2\">");
    ...
    */
    ...
    if (course.equals( "-ALL-" )) {
        ...
        // select all tee times for all courses
        stringTee = "SELECT * " +
                    "FROM teecurr2 WHERE date = ? AND event != '' AND event_type = 1 ORDER BY time, courseName, fb";

    } else {

        // select all tee times for a particular course
        stringTee = "SELECT * " +
                    "FROM teecurr2 WHERE date = ? AND courseName = ? AND event != '' AND event_type = 1 ORDER BY time, fb";
    ...
    try {
        ...
        while (rs.next()) {
           ...
           // if (!player1.equals("")) {   // if any players in tee time (remove this so we can assign holes prior to moving players to tee sheet!!)
                ...
                //
                out.println("<td bgcolor=\"#F5F5DC\" align=\"center\" width=120>");
                out.println("<font size=\"2\">");
 ...
 //
 //   Process the request to cancel tee times as specified in displayBlockers
 //
 private ArrayList<Integer> cancelTeeTimes(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
   ...
   String value = "";
   String tmp = "";
   String courseName = "";    // course name of individual tee time 
   ...
   long date = (year * 10000) + (month * 100) + day;    // create a date field of yyyymmdd for date of tee sheet(s)
   
   String day_name = day_table[day_num];         // get name for day

    
   if (req.getParameter("cancelAll") != null) {
      
      // request is to cancel all the tee times on the tee sheet(s)

      //Utilities.logDebug("BP", "Proshop_dsheet.cancelTeeTimes - Cancel All received. Course = " +course+ ", index = " +index+ ", Date = " +date);  // log it
       
      timesInUseA = cancelAllTeeTimes(timesInUseA, date, course, day_name, emailOpt, user, club, con);      // cancel all tee times for date and course
            
   } else {    // cancel only those that were selected (checkboxes)
      
      // the form should be here with each tee-time and its 'cancel' setting
      // loop thru each element and locate the respective tee-time and cancel it
      ...
      while (elems.hasMoreElements()) {
         ...
         if(name.startsWith("ID_") == true) {   // ID_fb_course_hr_min

               tmp = name.substring(3);         // grab everything after the 3rd char (after ID_) - result should = 'fb_course_hr_min' (name of the checkbox)

               if (req.getParameter(tmp) != null && req.getParameter(tmp).equals("1")) {
                  
                  String [] tempA = tmp.split("_");       // get all 4 parts

                  fb = Integer.parseInt(tempA[0]);
                  courseName = tempA[1];
                  hr = Integer.parseInt(tempA[2]);
                  min = Integer.parseInt(tempA[3]);
                  
                  time = (hr * 100) + min;        // create time value
                  
                  timeInUse = cancelOneTeeTime(date, time, fb, courseName, day_name, emailOpt, user, club, con);      // cancel this tee time

                  //Utilities.logDebug("BP", "Proshop_dsheet.cancelTeeTimes - Cancel Select Times received. FB = " +fb+ ", CourseName = " +courseName+ ", Hr = " +hr+ ", Min = " +min+ ", Time = " +time+ ", Course = " +course+ ", index = " +index+ ", Date = " +date);  // log it
                  
                  if (timeInUse > 0) {     // if tee was already in use
                      
                      timesInUseA.add(timeInUse);       //  save this to show user !!!!!
                  }
               }
               
         }    // end of IF tee time with checkbox (ID_)
         
      }       // end of WHILE elements
 ...
 //
 //   Process the request to set blockers as specified in displayBlockers
 //
 private ArrayList<Integer> cancelAllTeeTimes(ArrayList<Integer> timesInUseA, long date, String course, String day, String emailOpt, String user, String club, Connection con) {
   ...
   //  get each tee time and cancel it
   
   try {
       
       pstmt6 = con.prepareStatement ("SELECT time, fb "
                                       + "FROM teecurr2 "
                                       + "WHERE date = ? AND courseName = ? AND player1 != ''");

       pstmt6.clearParameters();     
       pstmt6.setLong(1, date);
       pstmt6.setString(2, course);
       rs = pstmt6.executeQuery();  
       ...
       while (rs.next()) {
          ...
          int timeInUse = cancelOneTeeTime(date, time, fb, course, day, emailOpt, user, club, con);      // cancel this tee time
        ...
        Utilities.logError("Proshop_dsheet.cancelAllTeeTimes - Getting tee times error. Club = " +club+ ", Course = " +course+ ", Date = " +date+ ", Error=" +exp.toString());  // log it

   } finally {

      Connect.close(pstmt6);
   }
        
   return(timesInUseA);
     
 }  // end cancelAllTeeTimes
 ...
 //   Process the request to set blockers as specified in displayBlockers
 //
 public static int cancelOneTeeTime(long date, int time, int fb, String course, String day, String emailOpt, String user, String club, Connection con) {
   ...
   try {
       ...
       count = pstmt6.executeUpdate();              
        ...
        Utilities.logError("Proshop_dsheet.cancelOneTeeTime - Checking tee time error. Club = " +club+ ", Course = " +course+ ", Date = " +date+ ", Error=" +exp.toString());  // log it
   ...
   if (count == 0) {
       ...
       timeInUse = time;
        ...
        if (emailOpt.equalsIgnoreCase("yes")) {     // if we will be sending emails

            try {

                //
                //   Gather the current tee time info for the email message below
                //
                pstmt6 = con.prepareStatement ("SELECT mm, dd, yy, orig_by, p91, p92, p93, p94, p95, notes, hideNotes, event, event_type, "
                                             + "player1, player2, player3, player4, player5, username1, username2, username3, username4, username5, "
                                             + "p1cw, p2cw, p3cw, p4cw, p5cw, guest_id1, guest_id2, guest_id3, guest_id4, guest_id5 "
                                             + "FROM teecurr2 "
                                             + "WHERE date = ? AND time = ? AND fb = ? AND courseName = ?");
                ...
                if (rs.next()) {
                    ...
                    Utilities.logError("Proshop_dsheet.cancelOneTeeTime - Get Tee Time info failed. Club = " +club+ ", Course = " +course+ ", Date = " +date+ ", Error=" +exp.toString());  // log it
        ...
        try {
            ...
            //
            //   Cancel the tee time(s)
            //
            pstmt6 = con.prepareStatement (
                "UPDATE teecurr2 " +
                "SET last_mod_date = now(), player1 = '', player2 = '', player3 = '', player4 = '', " +
                    "username1 = '', username2 = '', username3 = '', username4 = '', p1cw = '', " +
                    "p2cw = '', p3cw = '', p4cw = '',  in_use = 0, hndcp1 = 0, hndcp2 = 0, hndcp3 = 0, " +
                    "hndcp4 = 0, player5 = '', username5 = '', " +
                    "p5cw = '', hndcp5 = 0, notes = '', hideNotes = 0, proMod = proMod+1, " +
                    "mNum1 = '', mNum2 = '', mNum3 = '', mNum4 = '', mNum5 = '', " +
                    "userg1 = '', userg2 = '', userg3 = '', userg4 = '', userg5 = '', " +
                    "guest_id1 = 0, guest_id2 = 0, guest_id3 = 0, guest_id4 = 0, guest_id5 = 0, orig_by = '', conf = '', " +
                    "p91 = 0, p92 = 0, p93 = 0, p94 = 0, p95 = 0, pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0, pos5 = 0, " +
                    "custom_disp1 = '', custom_disp2 = '', custom_disp3 = '', custom_disp4 = '', custom_disp5 = '', " +
                    "custom_string = '', custom_int = 0, " +
                    "tflag1 = '', tflag2 = '', tflag3 = '', tflag4 = '', tflag5 = '', " +
                    "orig1 = '', orig2 = '', orig3 = '', orig4 = '', orig5 = '', " +
                    "nopost1 = 0, nopost2 = 0, nopost3 = 0, nopost4 = 0, nopost5 = 0, make_private = 0 " +
                "WHERE date = ? AND time = ? AND fb = ? AND courseName = ?");
            ...
            //
            //  Make a history entry to track this change
            //
            int email_suppressed = 1;     // default to emails suppressed

            if (emailOpt.equalsIgnoreCase("yes")) {
                email_suppressed = 2;                // not suppressed
            }

            //  new tee time
            SystemUtils.updateHist(date, day, time, fb, course, "", "", "", "", "", user, "Edit Tsheet Cancel", 1, email_suppressed, 0, con);
            ...
            if (emailOpt.equalsIgnoreCase("yes")) {
                ...
                parme.user = user;
                parme.orig_by = orig_by;
                parme.emailNew = 0;
                parme.emailMod = 0;
                parme.emailCan = 1;       // email is for a cancel
                ...
                parme.oldplayer1 = player1;
                parme.oldplayer2 = player2;
                parme.oldplayer3 = player3;
                parme.oldplayer4 = player4;
                parme.oldplayer5 = player5;
                ...
                parme.olduser1 = user1;
                parme.olduser2 = user2;
                parme.olduser3 = user3;
                parme.olduser4 = user4;
                parme.olduser5 = user5;
                ...
                parme.oldguest_id1 = guest_id1;
                parme.oldguest_id2 = guest_id2;
                parme.oldguest_id3 = guest_id3;
                parme.oldguest_id4 = guest_id4;
                parme.oldguest_id5 = guest_id5;
                ...
                Utilities.logError("Proshop_dsheet.cancelOneTeeTime - Cancel Tee Time error. Club = " +club+ ", Course = " +course+ ", Date = " +date+ ", Error=" +exp.toString());  // log it
 ...
 private void setBlockers(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session) {
    ...
    try {
        ...
        SystemUtils.buildDatabaseErrMsg("Proshop_dsheet.setBlockers - Fatal error updating tee time blockers.", exp.toString(), out, false);
    }
 ...
 */
    ...
    try {
        ...
        if (rs.next()) {
            ...
            teeTimeFull = addPlayer(slotParms, rs.getString( "player_name" ), usert, rs.getString( "mNum" ), rs.getString( "cw" ), userg, rs.getInt( "9hole" ), rs.getInt("guest_id"), allowFives, con);
    ...
    if ( !slotParms.player5.equals("") && ((!slotParms.rest5.equals("") && overRideFives == false) || fives == 0)) {
        ...
        if (!overrideAccess) {
            ...
            out.println("<form action=\"Proshop_dsheet\" method=\"post\" target=\"_top\">");
            out.println("<input type=\"hidden\" name=\"mode\" value=\"" + mode + "\">");
   ...
   if (slotParms.oldPlayer1.equals( "" ) && slotParms.oldPlayer2.equals( "" ) && slotParms.oldPlayer3.equals( "" ) &&
    ...
    out.println("<meta http-equiv=\"Refresh\" content=\"0; url=Proshop_dsheet?mode=" +mode+ "&index=" + slotParms.ind + "&course=" + slotParms.returnCourse + "&jump=" + slotParms.jump + "\">");
 ...
 //
 // TODO: if there are 5 players in the signup and it's dragged on to a course with no 5-some option - the 5th player is dropped and disappears
 //       the user is not explictly told this but they do see the players listed and the 5th player will be empty but they may not notice that
 //
 ...
 */
    ...
    try {
        ...
        if (rs.next()) {
            ...
            teeTimeFull = addPlayer(slotParms, rs.getString( "player_name" ), rs.getString( "username" ), rs.getString( "mNum" ), rs.getString( "cw" ), userg, rs.getInt( "9hole" ), rs.getInt("guest_id"), allowFives, con);
            if (!teeTimeFull) tmp_added++;
    ...
    try {
        ...
        out.println("<!-- slotParms.guest_id1="+slotParms.guest_id1+" -->");
        out.println("<!-- slotParms.guest_id2="+slotParms.guest_id2+" -->");
        out.println("<!-- slotParms.guest_id3="+slotParms.guest_id3+" -->");
        out.println("<!-- slotParms.guest_id4="+slotParms.guest_id4+" -->");
        out.println("<!-- slotParms.guest_id5="+slotParms.guest_id5+" -->");
    ...
    if (skip < 99) {
        ...
        out.println("<form action=\"Proshop_dsheet\" method=\"post\" target=\"_top\" name=frmConfirm>");
        out.println("<input type=\"hidden\" name=\"mode\" value=\"WAITLIST\">");
        out.println("<input type=\"hidden\" name=\"convert\" value=\"yes\">");
        out.println("<input type=\"hidden\" name=\"overRideFives\" value=\"yes\">");
        out.println("<input type=\"hidden\" name=\"wlsId\" value=\"" + wls_id + "\">");
    ...
    out.println("<meta http-equiv=\"Refresh\" content=\"5; url=Proshop_dsheet?mode=WAITLIST&name=" + name + "&index=" + slotParms.ind + "&course=" + slotParms.returnCourse + "&jump=" + slotParms.jump + "&hide=" + hide + "\">");
...
*/
    ...
    try {
        ...
        if (rs.next()) {
            ...
            teeTimeFull = addPlayer(slotParms, rs.getString( "player_name" ), rs.getString( "username" ), rs.getString( "mNum" ), rs.getString( "cw" ), "", rs.getInt( "9hole" ), rs.getInt("guest_id"), allowFives, con);
            if (!teeTimeFull) tmp_added++;
            userg = rs.getString( "username" );
        }
   ...
   if (slotParms.oldPlayer1.equals( "" ) && slotParms.oldPlayer2.equals( "" ) && slotParms.oldPlayer3.equals( "" ) &&
    ...
    out.println("<meta http-equiv=\"Refresh\" content=\"0; url=Proshop_dsheet?mode=&index=" + slotParms.ind + "&course=" + slotParms.returnCourse + "&jump=" + slotParms.jump + "\">");
 ...
 private static boolean addPlayer(parmSlot slotParms, String player_name, String username, String mNum, String cw, String userg, int p9hole, int guest_id, boolean allowFives, Connection con) {
 ...
 //
 private static boolean addPlayer(parmSlot slotParms, String player_name, String username, String cw, String userg, int p9hole, int guest_id, String orig, boolean allowFives, Connection con) {
 ...
 */
 ...
 private void buildRow(int slotIndex, int group, String course, String course_color, String bgcolor, int max_players, int courseCount, int lcol_start[], int lcol_width[], int index, String emailOpt, int j, String dts_defaultF3Color, boolean child, String hideUnavail, String name, ResultSet rs, PrintWriter out, int multi) {
    ...
    if (rs.getInt("in_use") == 0) {
        ...
        if (!child) {
            ...
            out.print("<img src=/" +rev+ "/images/shim.gif width=5 height=1 border=0>");
            out.print("<a href=\"Proshop_dsheet?mode=LOTT&index=" +index+ "&lotteryId=" + rs.getInt("id") + "&returnCourse=" +course+ "&email=" +emailOpt+ "&name=" +name+ "&hide=" +hideUnavail+ "&delete=yes\" title=\"Delete Request\" alt=\"Remove Request\" onclick=\"return confirm('Are you sure you want to permanently delete this lottery request?');\">");
            out.print("<img src=/" +rev+ "/images/dts_trash.gif width=13 height=13 border=0></a>");
        } else {
            out.print("<img src=/" +rev+ "/images/shim.gif width=13 height=13 border=0>");
            out.print("<img src=/" +rev+ "/images/shim.gif width=5 height=1 border=0>");
    ...
    if (before > 0) {
        ftime = getTime(Common_Lott.getFirstTime(rs.getInt("time"), before));    // get earliest time for this request
    } else {
        ftime = getTime(rs.getInt("time")); 
    }
    if (after > 0) {
        ltime = getTime(Common_Lott.getLastTime(rs.getInt("time"), after));     // get latest time for this request
    } else {
    ...
    if (multi == 1) { // only display these cols if multi course club - assigned course & requested course
        ...
        if (courseCount > 1) {
            out.print("<span id=lottery_slot_" + slotIndex + "_checkothers class=cellDataC style=\"cursor: default; left: " + lcol_start[7] + "px; width: " + lcol_width[7] + "px;\">");
            out.print(rs.getInt("checkothers") == 1 ? "Yes" : "No");
            out.print("</span>");
        }
    }
    ...
    // Weight
    //
    out.print("<span id=lottery_slot_" + slotIndex + "_weight class=cellData style=\"cursor: default; left: " + lcol_start[8] + "px; width: " + lcol_width[8] + "px;\">");
    ...
    // Players
    //
    out.print("<span id=lottery_slot_" + slotIndex + "_members class=cellDataB value=\"\" style=\"cursor: default; left: " + lcol_start[9] + "px; width: " + lcol_width[9] + "px; text-align: left\">&nbsp;");
    ...
    if (!rs.getString("notes").trim().equals("")) {
        ...
        // this won't work in rl but is meant to show a dynamic popup, or we can spawn a small popup window that will show the notes
        out.println("&nbsp; &nbsp; <img src=\"/"+rev+"/images/notes.gif\" width=10 height=12 border=0 title=\"" + StringEscapeUtils.escapeHtml(rs.getString("notes")) + "\">");
    }
 ...
 private void convert_all(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session, HttpServletResponse resp) {
    ...
    SystemUtils.moveReqs(name, date, course, user, false, con);
    ...
    if (count > 0) {
        ...
        // debug !!!!!
        //out.println("Count = " +count+ ", Course = " +course+ ", Date = " +date+ ", Name = " +name+ "<BR><BR>");
        ...
        out.println("<meta http-equiv=\"Refresh\" content=\"0; url=Proshop_dsheet?mode=LOTT&index=" + index + "&name=" + name + "&course=" + course + "&hide=" + hide + "\">");
 ...
 /*
  *   Get control here when a lottery request is dragged onto the tee sheet.
  * 
  */
...
*/
    ...
    try {
        ...
        if (rs.next()) {
            ...
            blocker = rs.getString("blocker");
            lottery = rs.getString("lottery");
            slotParms.date = rs.getLong("date");
            slotParms.day = rs.getString("day");
            slotParms.fb = rs.getInt("fb");
...
*/  
    ...
    int [] weightA = new int [5];            // array to hold user weights
    ...
    try {
        ...
        if ( rs.next() ) {
            ...
            weightA[0] = rs.getInt(193 + field_offset);      // get individual weights
            weightA[1] = rs.getInt(194 + field_offset);
            weightA[2] = rs.getInt(195 + field_offset);
            weightA[3] = rs.getInt(196 + field_offset);
            weightA[4] = rs.getInt(197 + field_offset);
            
            // Set orig values to the main orig_by value for the lottery request. If starts with "proshop" or blank, user user1 instead.
            if (rs.getString("orig_by").equals("") || rs.getString("orig_by").startsWith("proshop")) {
                if (!user1.equals("")) {
                    orig = user1;
                } else {
                    orig = "";
                ...
                orig = rs.getString("orig_by");    // Used for setting the orig1-5 values for players moved out of the lott req.
            }
            
            courseReq = rs.getString("courseReq");             // get the requested course
            grp_weight = rs.getInt("weight");                  // get the group weight
        ...
        if (!player1.equals("")) {
            group_players++;
            teeTimeFull = addPlayer(slotParms, player1, user1, p1cw, userg1, p91, guest_id1, orig, allowFives, con);
            if (!teeTimeFull) { tmp_added++; }
        }
        
        if (!player2.equals("")) {
            group_players++;
            teeTimeFull = addPlayer(slotParms, player2, user2, p2cw, userg2, p92, guest_id2, orig, allowFives, con);
            if (!teeTimeFull) { tmp_added++; }
        }
        
        if (!player3.equals("")) {
            group_players++;
            teeTimeFull = addPlayer(slotParms, player3, user3, p3cw, userg3, p93, guest_id3, orig, allowFives, con);
            if (!teeTimeFull) { tmp_added++; }
        }
        
        if (!player4.equals("")) {
            group_players++;
            teeTimeFull = addPlayer(slotParms, player4, user4, p4cw, userg4, p94, guest_id4, orig, allowFives, con);
            if (!teeTimeFull) { tmp_added++; }
        }
        
        if (!player5.equals("") && allowFives && group_size == 5) {
            group_players++;
            teeTimeFull = addPlayer(slotParms, player5, user5, p5cw, userg5, p95, guest_id5, orig, allowFives, con);
            if (!teeTimeFull) { tmp_added++; }
        }
    ...
    try {
        ...
        pstmt6 = con.prepareStatement (
             ...
             "mNum1 = ?, mNum2 = ?, mNum3 = ?, mNum4 = ?, mNum5 = ?, " +
             "userg1 = ?, userg2 = ?, userg3 = ?, userg4 = ?, userg5 = ?, orig_by = ?, conf = ?, " +
             "p91 = ?, p92 = ?, p93 = ?, p94 = ?, p95 = ?, pos1 = ?, pos2 = ?, pos3 = ?, pos4 = ?, pos5 = ?, lottery_email = ?, " +
        ...
        pstmt6.setInt(45, slotParms.pos4);
        pstmt6.setInt(46, slotParms.pos5);
        pstmt6.setInt(47, lottery_email);
        ...
        pstmt6.setString(60, slotParms.orig1);
        pstmt6.setString(61, slotParms.orig2);
        pstmt6.setString(62, slotParms.orig3);
        pstmt6.setString(63, slotParms.orig4);
        pstmt6.setString(64, slotParms.orig5);

        pstmt6.setInt(65, teecurr_id);
    ...
    // if the tee time was updated then update the request
    int tmp_time = atime1;
    ...
    if (count == 1) {
        ...
        // Bind the newly inserted request
       long member_id = memberUtil.getMemberIdFromUsername(slotParms.orig_by, con);

       if(member_id > 0){
           long teecurr_group_id = verifySlot.createTeeCurrGroup(member_id, id, con);
           if(teecurr_group_id > 1){
                   verifySlot.addTeeCurrGroupDetail(teecurr_id, teecurr_group_id, con);
           } else {
               Utilities.logError("Proshop_dsheet.convertLottReq: Err=Unable to create teecurr binding.  Can't create Teecurr2_group for lreq:"+id);
           }
       } else {
           //Utilities.logError("Proshop_dsheet.convertLottReq: Err=Unable to create teecurr binding.  Can't find member ID for:"+slotParms.orig_by);
       }

        
        // process according to how many groups this request had
        if (groups == 1) {
           
            if (atime1 == 0) tmp_time = time;      // use tee time if requested unassigned (necessary for req to be deleted later)
            
            // there was one group in the request - just change the state and save the teecurr_id
            try {
                
                pstmt = con.prepareStatement("UPDATE lreqs3 SET state = 4, atime1 = ?, teecurr_id1 = ? WHERE id = ?");
                pstmt.clearParameters();
                pstmt.setInt(1, tmp_time);
                pstmt.setInt(2, teecurr_id);
                pstmt.setInt(3, lottery_id);
                pstmt.executeUpdate();
                pstmt.close();
            ...
            // add teecurr_id setting to fields so we know where this group was moved
            fields = fields + " teecurr_id" + (group) + " = ?";
            
            if (group == 1 && atime1 == 0) {    // if this is the first group and its unassigned
               
               fields = fields + ", atime1 = ?";
               tmp_time = time;                   // use tee time if requested unassigned (necessary for req to be deleted later) 
            }
            ...
            try {
                ...
                pstmt.setInt(1, teecurr_id);
                if (group == 1 && atime1 == 0) {
                   pstmt.setInt(2, tmp_time);
                   pstmt.setInt(3, lottery_id);
                } else {
                   pstmt.setInt(2, lottery_id);
                }
                pstmt.executeUpdate();
                pstmt.close();
            ...
            try {
                ...
                pstmt = con.prepareStatement ("" +
                        "SELECT state FROM lreqs3 WHERE id = ? AND " + 
        ...
        String fullName = "Lottery Request Conversion (Manual Drag)";
        int hist_type = 3;
        
        // Create a tee time history entry
        SystemUtils.updateHist(slotParms.date, slotParms.day, time, slotParms.fb, course, slotParms.player1, slotParms.player2, slotParms.player3,
                slotParms.player4, slotParms.player5, user, fullName, hist_type, con);
        
        
        /*    MOVED to logLotteryReqs 6/14/2011
        //
        //  Log the assignments in lassigns5
        //
        int interval = SystemUtils.getCourseInterval(courseReq, con);     // get the tee time interval for the requested course
               
        rtime = SystemUtils.calcRtime(rtime, group, interval);            // calculate the actual requested time for this group
                  
        int mins = SystemUtils.calcProxTime(rtime, time);                 // calculate the minutes between time req'd and time assgnd 

        for (int i=0; i<5; i++) {

           if (!userA[i].equals( "" )) {

              SystemUtils.logAssign(userA[i], name, date, mins, rtime, courseReq, time, course, weightA[i], grp_weight, lottery_id, con);  // log the assigned time in lassigns5 for this member
           }
        }
         */
...
*/
    ...
    // Completed update - reload page
    //
    out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_dsheet?mode=LOTT&index=" + slotParms.ind + "&course=" + slotParms.returnCourse + "&name=" + name + "&hide="+hideUnavail+"\">");
    out.close();
    return;
 }
 ...
 private void auto_convert(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session, HttpServletResponse resp) {
   ...
   int [] weightA = new int [25];                // array to hold individual weights
   ...
   try {
      ...
      while (rs.next()) {
         ...
         grp_weight = rs.getInt("weight");              // get the group weight and individual weights
         weightA[0] = rs.getInt("weight1");
         weightA[1] = rs.getInt("weight2");
         weightA[2] = rs.getInt("weight3");
         weightA[3] = rs.getInt("weight4");
         weightA[4] = rs.getInt("weight5");
         weightA[5] = rs.getInt("weight6");
         weightA[6] = rs.getInt("weight7");
         weightA[7] = rs.getInt("weight8");
         weightA[8] = rs.getInt("weight9");
         weightA[9] = rs.getInt("weight10");
         weightA[10] = rs.getInt("weight11");
         weightA[11] = rs.getInt("weight12");
         weightA[12] = rs.getInt("weight13");
         weightA[13] = rs.getInt("weight14");
         weightA[14] = rs.getInt("weight15");
         weightA[15] = rs.getInt("weight16");
         weightA[16] = rs.getInt("weight17");
         weightA[17] = rs.getInt("weight18");
         weightA[18] = rs.getInt("weight19");
         weightA[19] = rs.getInt("weight20");
         weightA[20] = rs.getInt("weight21");
         weightA[21] = rs.getInt("weight22");
         weightA[22] = rs.getInt("weight23");
         weightA[23] = rs.getInt("weight24");
         weightA[24] = rs.getInt("weight25");
         ...
         if (atime1 != 0) {          // only process if its assigned
            ...
            if (ok == true) {             // if ok to proceed (no tee times are in use)
               ...
               //
               //  Get the minutes between the requested time and the assigned time
               //
               int mins = SystemUtils.calcProxTime(rtime, time);                 // calculate the minutes between time req'd and time assgnd 
               ...
               errorMsg = "Error in Proshop_dsheet.auto_convert (get mem# and hndcp): ";
               ...
               errorMsg = "Error in Proshop_dsheet.auto_convert (put group 1 in tee sheet): ";
               ...
               PreparedStatement pstmt6 = con.prepareStatement (
                  ...
                  "p91 = ?, p92 = ?, p93 = ?, p94 = ?, p95 = ?, lottery_email = ?, " +
                  "tflag1 = ?, tflag2 = ?, tflag3 = ?, tflag4 = ?, tflag5 = ?, " +
                  "guest_id1 = ?, guest_id2 = ?, guest_id3 = ?, guest_id4 = ?, guest_id5 = ?, " +
                  "orig1 = ?, orig2 = ?, orig3 = ?, orig4 = ?, orig5 = ? " +
                  "WHERE date = ? AND time = ? AND fb = ? AND courseName = ?");
               ...
               pstmt6.setInt(43, lottery_email);
               pstmt6.setString(44, slotParms.tflag1);
               pstmt6.setString(45, slotParms.tflag2);
               pstmt6.setString(46, slotParms.tflag3);
               pstmt6.setString(47, slotParms.tflag4);
               pstmt6.setString(48, slotParms.tflag5);
               pstmt6.setInt(49, g1guest_id1);
               pstmt6.setInt(50, g1guest_id2);
               pstmt6.setInt(51, g1guest_id3);
               pstmt6.setInt(52, g1guest_id4);
               pstmt6.setInt(53, g1guest_id5);
               pstmt6.setString(54, g1orig1);
               pstmt6.setString(55, g1orig2);
               pstmt6.setString(56, g1orig3);
               pstmt6.setString(57, g1orig4);
               pstmt6.setString(58, g1orig5);

               pstmt6.setLong(59, date);
               pstmt6.setInt(60, time);
               pstmt6.setInt(61, afb);
               pstmt6.setString(62, course);
               ...
               if (count > 0) {
                  ...
                  // Create a tee time history entry
                  String fullName = "Lottery Request Conversion (Auto-Convert)";
                  int hist_type = 3;
               ...
               group_ids.add(teecurr_id);

               pstmt6 = con.prepareStatement("UPDATE lreqs3 SET teecurr_id1 = ? WHERE id = ?");
               pstmt6.clearParameters();
               pstmt6.setLong(1, teecurr_id);
               pstmt6.setInt(2, lottery_id);
               pstmt6.executeUpdate();
               pstmt6.close();
               ...
               if (groups > 1 && count != 0) {
                  ...
                  //
                  //  Determine the requested time for this group based on group 1's requested time
                  //
                  rtime2 = SystemUtils.calcRtime(rtime, 2, interval);            // calculate the actual requested time for this group
                  ...
                  errorMsg = "Error in Proshop_dsheet.auto_convert (put group 2 in tee sheet): ";
                  ...
                  pstmt6.setString(47, slotParms.tflag4);
                  pstmt6.setString(48, slotParms.tflag5);
                  pstmt6.setInt(49, g2guest_id1);
                  pstmt6.setInt(50, g2guest_id2);
                  pstmt6.setInt(51, g2guest_id3);
                  pstmt6.setInt(52, g2guest_id4);
                  pstmt6.setInt(53, g2guest_id5);
                  pstmt6.setString(54, g2orig1);
                  pstmt6.setString(55, g2orig2);
                  pstmt6.setString(56, g2orig3);
                  pstmt6.setString(57, g2orig4);
                  pstmt6.setString(58, g2orig5);

                  pstmt6.setLong(59, date);
                  pstmt6.setInt(60, time);
                  pstmt6.setInt(61, afb2);
                  pstmt6.setString(62, course);
               ...
               if (groups > 2 && count != 0) {
                  ...
                  //  Determine the requested time for this group based on group 1's requested time
                  //
                  rtime3 = SystemUtils.calcRtime(rtime, 3, interval);            // calculate the actual requested time for this group
                  ...
                  errorMsg = "Error in Proshop_dsheet.auto_convert (put group 3 in tee sheet): ";
                  ...
                  pstmt6.setString(47, slotParms.tflag4);
                  pstmt6.setString(48, slotParms.tflag5);
                  pstmt6.setInt(49, g3guest_id1);
                  pstmt6.setInt(50, g3guest_id2);
                  pstmt6.setInt(51, g3guest_id3);
                  pstmt6.setInt(52, g3guest_id4);
                  pstmt6.setInt(53, g3guest_id5);
                  pstmt6.setString(54, g3orig1);
                  pstmt6.setString(55, g3orig2);
                  pstmt6.setString(56, g3orig3);
                  pstmt6.setString(57, g3orig4);
                  pstmt6.setString(58, g3orig5);

                  pstmt6.setLong(59, date);
                  pstmt6.setInt(60, time);
                  pstmt6.setInt(61, afb3);
                  pstmt6.setString(62, course);
               ...
               if (groups > 3 && count != 0) {
                  ...
                  //  Determine the requested time for this group based on group 1's requested time
                  //
                  rtime4 = SystemUtils.calcRtime(rtime, 4, interval);            // calculate the actual requested time for this group
                  ...
                  errorMsg = "Error in Proshop_dsheet.auto_convert (put group 4 in tee sheet): ";
                  ...
                  pstmt6.setString(47, slotParms.tflag4);
                  pstmt6.setString(48, slotParms.tflag5);
                  pstmt6.setInt(49, g4guest_id1);
                  pstmt6.setInt(50, g4guest_id2);
                  pstmt6.setInt(51, g4guest_id3);
                  pstmt6.setInt(52, g4guest_id4);
                  pstmt6.setInt(53, g4guest_id5);
                  pstmt6.setString(54, g4orig1);
                  pstmt6.setString(55, g4orig2);
                  pstmt6.setString(56, g4orig3);
                  pstmt6.setString(57, g4orig4);
                  pstmt6.setString(58, g4orig5);

                  pstmt6.setLong(59, date);
                  pstmt6.setInt(60, time);
                  pstmt6.setInt(61, afb4);
                  pstmt6.setString(62, course);
               ...
               if (groups > 4 && count != 0) {
                  ...
                  //  Determine the requested time for this group based on group 1's requested time
                  //
                  rtime5 = SystemUtils.calcRtime(rtime, 5, interval);            // calculate the actual requested time for this group
                  ...
                  errorMsg = "Error in Proshop_dsheet.auto_convert (put group 5 in tee sheet): ";
                  ...
                  pstmt6.setString(47, slotParms.tflag4);
                  pstmt6.setString(48, slotParms.tflag5);
                  pstmt6.setInt(49, g5guest_id1);
                  pstmt6.setInt(50, g5guest_id2);
                  pstmt6.setInt(51, g5guest_id3);
                  pstmt6.setInt(52, g5guest_id4);
                  pstmt6.setInt(53, g5guest_id5);
                  pstmt6.setString(54, g5orig1);
                  pstmt6.setString(55, g5orig2);
                  pstmt6.setString(56, g5orig3);
                  pstmt6.setString(57, g5orig4);
                  pstmt6.setString(58, g5orig5);

                  pstmt6.setLong(59, date);
                  pstmt6.setInt(60, time);
                  pstmt6.setInt(61, afb5);
                  pstmt6.setString(62, course);
               ...
               if (count == 0) {
                  ...
                  return;
                   ...
                   // Add bindings
                   long member_id = memberUtil.getMemberIdFromUsername(orig_by, con);
                   long teecurr_group_id = verifySlot.getTeeCurrGroup(member_id, id, con, true); // Check if we already have this
               
                   if((group_ids.size() > 1 || teecurr_group_id > 0) && member_id > 0 && group_ids.size() > 0){
                       if(teecurr_group_id < 1){
                           teecurr_group_id = verifySlot.createTeeCurrGroup(member_id, id, con);
                       }
                       if(teecurr_group_id > 1){
                           for(Long teecurr2_id : group_ids){
                               verifySlot.addTeeCurrGroupDetail(teecurr2_id, teecurr_group_id, con);
                           }
                       } else {
                           Utilities.logError("Proshop_dsheet.auto_convert: Err=Unable to create teecurr binding.  Can't get teecurr_group_id for member_id:"+member_id+", lreq_id:"+id);
                       }
                   }
               ...
               //
               //  Log each member that was assigned a time - in lassigns5
               //
               for (i=0; i<25; i++) {          // check all 25 possible players
                  ...
                  if (!userA[i].equals( "" )) {     // if player is a member
                     ...
                     if (p5.equals( "Yes" )) {    // if groups of 5

                        if (i > 4 && i < 10) {    // if player 6 thru 10
                           
                           time = atime2;
                           rtime = rtime2;
                           
                        } else if (i > 9 && i < 15) {    // if player 11 thru 15
                     
                           time = atime3;
                           rtime = rtime3;
                           
                        } else if (i > 14 && i < 20) {    // if player 16 thru 20
                     
                           time = atime4;
                           rtime = rtime4;
                           
                        } else if (i > 19) {    // if player 21 thru 25
                        ...
                        if (i > 3 && i < 8) {    // if player 5 thru 8
                           
                           time = atime2;
                           rtime = rtime2;
                           
                        } else if (i > 7 && i < 12) {    // if player 9 thru 12
                     
                           time = atime3;
                           rtime = rtime3;
                           
                        } else if (i > 11 && i < 16) {    // if player 13 thru 16
                     
                           time = atime4;
                           rtime = rtime4;
                           
                        } else if (i > 15) {    // if player 17 thru 20
    ...
    // Completed update - reload page
    //
    out.println("<meta http-equiv=\"Refresh\" content=\"0; url=Proshop_dsheet?mode=LOTT&index=" + index + "&course=" + returnCourse + "&name=" + name + "&hide="+hideUnavail+"\">");
 ...
 private void convertEventSgnUp(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session, HttpServletResponse resp) {
    ...
    try {
        ...
        if (!player1.equals("")) {
            ...
            teeTimeFull = addPlayer(slotParms, player1, user1, memNum1, p1cw, userg1, p9, guest_id1, allowFives, con);
            if (!teeTimeFull) { tmp_added++; }
        }
        ...
        if (!player2.equals("")) {
            ...
            teeTimeFull = addPlayer(slotParms, player2, user2, memNum2, p2cw, userg2, p9, guest_id2, allowFives, con);
            if (!teeTimeFull) { tmp_added++; }
        }
        ...
        if (!player3.equals("")) {
            ...
            teeTimeFull = addPlayer(slotParms, player3, user3, memNum3, p3cw, userg3, p9, guest_id3, allowFives, con);
            if (!teeTimeFull) { tmp_added++; }
        }
        ...
        if (!player4.equals("")) {
            ...
            teeTimeFull = addPlayer(slotParms, player4, user4, memNum4, p4cw, userg4, p9, guest_id4, allowFives, con);
            if (!teeTimeFull) { tmp_added++; }
        }
        ...
        if (!player5.equals("") && allowFives && group_size == 5) {
            ...
            teeTimeFull = addPlayer(slotParms, player5, user5, memNum5, p5cw, userg5, p9, guest_id5, allowFives, con);
            if (!teeTimeFull) { tmp_added++; }
        }
    ...
    if (count == 1) {
        ...
        // Add history entry
        SystemUtils.updateHist(slotParms.date, slotParms.day, slotParms.time, slotParms.fb, slotParms.course, slotParms.player1, slotParms.player2, slotParms.player3,
                slotParms.player4, slotParms.player5, user, "Edit Tsheet Move From Event", 1, 0, 0, con);
    ...
    // Completed update - reload page
    //
    out.println("<meta http-equiv=\"Refresh\" content=\"0; url=Proshop_dsheet?mode=EVENT&index=" + slotParms.ind + "&course=" + slotParms.returnCourse + "&name=" + name + "&hide="+hideUnavail+"&limit=" + limit + "&jump=777\">");
 ...
 //
 //  User selected the Flip All F/Bs link in the Control Panel 
 //
 private void flip_all_fbs(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session, HttpServletResponse resp) {
    ...
    try {
        ...
        // make sure the tee sheet only has one type fo F/B specified (can't flip if they are mixed)
        pstmt = con.prepareStatement (
            "SELECT COUNT(*) " + 
            "FROM teecurr2 " +
            "WHERE date = ? AND courseName = ? AND fb = 0");
        ...
        pstmt = con.prepareStatement (
            "SELECT COUNT(*) " + 
            "FROM teecurr2 " +
            "WHERE date = ? AND courseName = ? AND fb = 1");
    ...
    if ((countf > 0 && countb > 0) || counto > 0) {    // if NOT all one FB type (F or B)
        
        out.println(SystemUtils.HeadTitle("Flip FB Error"));
        out.println("<body bgcolor=\"#FFFFFF\" text=\"#000000\">");
        out.println("<font size=\"2\" face=\"Arial, Helvetica, Sans-serif\">");
        out.println("<CENTER><BR><BR><H2>Mixed F/B Types Found</H2>");
        out.println("<BR><BR>Sorry, we are not able to swap the F/B options for this tee sheet.<BR>");
        out.println("<BR>The F/B values must ALL be F or B.  &nbsp;There cannot be a mixture and there cannot be any cross-over times.");
        out.println("<BR><BR>You must change the F/B values individually on the Edit Tee Sheet page.&nbsp; Just click on the F/B value in the tee times that you wish to change.");

        out.println("<font size=\"2\">");
        out.println("<form action=\"Proshop_dsheet\" method=\"get\" target=\"_top\">");
        out.println("<input type=\"hidden\" name=\"mode\" value=\"" + mode + "\">");
        out.println("<input type=\"hidden\" name=\"index\" value=\"" + index + "\">");
        out.println("<input type=\"hidden\" name=\"date\" value=\"" + sdate + "\">");
        out.println("<input type=\"hidden\" name=\"course\" value=\"" + course + "\">");
        out.println("<input type=\"hidden\" name=\"email\" value=\"" + email + "\">");
        out.println("<input type=\"hidden\" name=\"returnCourse\" value=\"" + returnCourse + "\">");
        out.println("<input type=\"submit\" value=\"Return\" name=\"return\" style=\"text-decoration:underline; background:#8B8970\">");
        ...
        out.println("<CENTER><BR><BR><H2>Flip The F/B Values</H2>");
        out.println("<BR><BR>You have elected to flip the F/B options for this tee sheet.<BR><BR>");
        if (countf > 0) {           
           out.println("All tee times are <strong>currently on the Front (F)</strong>, and will be <strong>changed to the Back (B)</strong>. &nbsp;");
        } else {
           out.println("All tee times are <strong>currently on the Back (B)</strong>, and will be <strong>changed to the Front (F)</strong>. &nbsp;");
        }
        out.println("<br><br>Are you sure you want to change them?<BR><BR>");
        ...
        if (email.equals("yes")) {
            ...
            out.println("<input type=\"checkbox\" checked name=\"send_emails\" value=\"yes\">");  // default to Yes
           
        } else {
        
            out.println("<input type=\"checkbox\" name=\"send_emails\" value=\"yes\">");
        }

        out.println("&nbsp;&nbsp;&nbsp;Send Email Notifications to the Members on this Tee Sheet<br>(check the box if Yes)");
        
        if (countf > 0) {           
           out.println("<input type=\"hidden\" name=\"currentFB\" value=\"Front\">");
        } else {
           out.println("<input type=\"hidden\" name=\"currentFB\" value=\"Back\">");
        }
        out.println("<BR><BR><input type=\"submit\" value=\"Yes - Flip Them\" name=\"flipallfb2\" style=\"text-decoration:underline; background:#8B8970\">");

        out.println("<BR><BR><input type=\"submit\" value=\"No - Return\" name=\"return\" style=\"text-decoration:underline; background:#8B8970\">");
        out.println("</form>");
 ...
 //
 //  User has confirmed that he wants to flip all the F/Bs on the specified tee sheet - flip them now
 //
 private void flip_all_fbs2(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session, HttpServletResponse resp) {
    ...
    String email = req.getParameter("email");
    String returnCourse = req.getParameter("returnCourse");
    String currentFB = req.getParameter("currentFB");
    
    boolean sendEmails = false;
    
    if (req.getParameter("send_emails") != null && req.getParameter("send_emails").equals("yes")) {
    ...
    int fb = 0;                        // default new F/Bs to Front

    if (currentFB.equals("Front")) {   // if F/Bs are already Front
       
       fb = 1;                         // change them to Back
    }
    ...
    try {
        ...
        pstmt.executeUpdate();       // flip all the F/Bs
    ...
    if (sendEmails == true) {
       ...
       if (currentFB.equals("Front")) {   // if F/Bs were on Front
          
          fb = 0;                         // Front (for emails) - previous FB
       ...
       sendFlipFBemails(date, club, fb, course, user, con);
    }    
    
 }   // end of flip_all_fbs2
 
 
 
 
 //
 //   Lottery approval is done - delete the requests OR set lottery state to 5 so tee sheet will open up
 //
 private void setLotteryState5(String clubName, String club, String lott_name, int lott_date, int index, PrintWriter out, Connection con) {
    ...
    if (club.equals("ramseycountryclub")) {
       ...
       try {
          ...
          pstmtd = con.prepareStatement ("" +
               "SELECT *, " +
                    "DATE_FORMAT(sdatetime, '%T') AS stime " +
               "FROM wait_list " +
               "WHERE name = ?;");
 ...
 //
 //   Log the lottery request assignments (save time requested and time assigned) - called when pro is done with lottery and ready to send emails
 //
 private void logLotteryReqs(String club, String clubName, String lott_name, int lott_date, int index, String course, Connection con) {
    ...
    //
    //  Locate every member in the lottery requests and find the tee time they were assigned to
    //
    try {
        ...
        pstmt = con.prepareStatement ("" +
            "SELECT * " +
            "FROM lreqs3 " +
            "WHERE date = ? AND name = ?;");        // make sure we get the unassigned reqs too (BP 1/11/13 for Pecan Plantation)
            //"WHERE date = ? AND name = ? AND state = 4;");
        ...
        while (rs.next()) {
            ...
            max = 25;         // max # of players
            
            if (p5.equalsIgnoreCase("No")) {      // if 4-some groups
            
               max = 20;         // 5 groups of 4
            }
                              
           //
            //  Locate each member in this request and determine when and where he/she was assigned.  Then log it.
            //
            for (int i=0; i<max; i++) {    // check all players
             
               if (!userA[i].equals("")) {     // if member
                  
                  //
                  //  Get member's request information
                  //
                  weight = weightA[i];                  // this member's weight
                  ...
                  if (p5.equalsIgnoreCase("No")) {      // if 4-some groups
                     ...
                     if (i < 4) {
                        ...
                        timeReq = SystemUtils.calcRtime(rtime, 2, interval);            // calculate the actual requested time for this group                  
                        
                     } else if (i > 7 && i < 12) {
                        
                        timeReq = SystemUtils.calcRtime(rtime, 3, interval);            // calculate the actual requested time for this group                  
                        
                     } else if (i > 11 && i < 16) {
                        
                        timeReq = SystemUtils.calcRtime(rtime, 4, interval);            // calculate the actual requested time for this group                  
                        
                     } else if (i > 15) {
                        
                        timeReq = SystemUtils.calcRtime(rtime, 5, interval);            // calculate the actual requested time for this group                  
                     }                        
                  ...
                  //
                  //  Check teecurr for this member (search lottery times only at this point)
                  //
                  pstmt2 = con.prepareStatement ("" +
                     "SELECT time, courseName " +
                     "FROM teecurr2 " +
                     "WHERE date = ? AND lottery = ? AND (username1 = ? || username2 = ? || username3 = ? || username4 = ? || username5 = ?);");

                  pstmt2.clearParameters();
                  pstmt2.setInt(1, lott_date);
                  pstmt2.setString(2, lott_name);
                  pstmt2.setString(3, userA[i]);
                  pstmt2.setString(4, userA[i]);
                  pstmt2.setString(5, userA[i]);
                  pstmt2.setString(6, userA[i]);
                  pstmt2.setString(7, userA[i]);
                  ...
                  while (rs2.next()) {                // find the closest time if more than one
                     
                     atime = rs2.getInt("time");
                     courseAsgnd = rs2.getString("courseName");
                     
                     mins = SystemUtils.calcProxTime(timeReq, atime);    // calculate the minutes between time req'd and time assgnd 
                  
                     if (last_mins == 9999) {   // if this is the first tee time found
                         
                         last_mins = mins;
                     
                     } else {
                         
                         if (mins < last_mins) {     // if this tee time is closer to the time requested
                             
                             last_mins = mins;       //  save it
                             
                         } else {
                             
                             mins = last_mins;       // if not, then use the last one (the assigned time should be the closest to the time requested)
                         }
                     }
                  }          // end of WHILE tee times found
                  
                  if (mins < 9999) {       // if time found
                      
                     SystemUtils.logAssign(userA[i], lott_name, lott_date, mins, timeReq, courseReq, atime, courseAsgnd, weight, grp_weight, id, con);  // log the assigned time in lassigns5 for this member                           
                      
                  } else {    
                     
                     //
                     //  Member not found in lottery times on this date - check all tee times for this date
                     //
                     pstmt2 = con.prepareStatement ("" +
                        "SELECT time, courseName " +
                        "FROM teecurr2 " +
                        "WHERE date = ? AND (username1 = ? || username2 = ? || username3 = ? || username4 = ? || username5 = ?);");

                     pstmt2.clearParameters();
                     pstmt2.setInt(1, lott_date);
                     pstmt2.setString(2, userA[i]);
                     pstmt2.setString(3, userA[i]);
                     pstmt2.setString(4, userA[i]);
                     pstmt2.setString(5, userA[i]);
                     pstmt2.setString(6, userA[i]);
                  ...
                  //
                  //  Custom for Pecan Plantation - if member still unassigned then add an entry in the assignment with 360 mins so they get a better weight next time
                  //
                  if (club.equals("pecanplantation") && mins == 9999) {
                      
                     //Utilities.logDebug("BP", "Proshop_dsheet.logLotteryReqs - unassigned req. Club = " +clubName+ ". Lott = " +lott_name+ ", Req Id = " +id);         // log it
                      
                     SystemUtils.logAssign(userA[i], lott_name, lott_date, 360, timeReq, courseReq, 0, "", weight, grp_weight, id, con);  // log the assigned time in lassigns5 for this member     
                  }
      ...
      SystemUtils.logError("Proshop_dsheet.logLotteryReqs error. Club = " +clubName+ ". Error = " +exp.toString());         // log it
    } finally {     
         if (rs != null) {
             try {
                 rs.close();
 ...
 //   Revised 1/05/2011
 //
 private void sendLotteryEmails(String club, String clubName, String lott_name, int lott_date, int index, String course, String returnCourse, 
                                PrintWriter out, Connection con, int sendUnass) {
    ...
    //
    //  parm block to hold the email parms
    //
    parmEmail parme = new parmEmail();          // allocate an Email parm block

    //
    //  Setup the Email Parm Block
    //
    parme.activity_id = 0;
    parme.club = club;
    parme.date = lott_date;
    parme.type = "lassign";          // indicate from lottery assign
    parme.user = "proshop";
    parme.emailNew = 1;              // indicate New email
    ...
    try {
        ...
        pstmt = con.prepareStatement ("" +
            "SELECT teecurr_id1, teecurr_id2, teecurr_id3, teecurr_id4, teecurr_id5 " +
            "FROM lreqs3 " +
            "WHERE date = ? AND name = ? AND state = 4;");
        ...
        while (rs.next()) {
           ...
           teecurr_id1 = rs.getInt("teecurr_id1");
           teecurr_id2 = rs.getInt("teecurr_id2");
           teecurr_id3 = rs.getInt("teecurr_id3");
           teecurr_id4 = rs.getInt("teecurr_id4");
           teecurr_id5 = rs.getInt("teecurr_id5");
           
           found = false;          // init group found flag
           req_count++;
            
           parme.player1 = "";     // init the email parm fields
           parme.player2 = "";
           parme.player3 = "";
           parme.player4 = "";
           parme.player5 = "";
           ...
           parme.wuser4 = "";
           parme.wuser5 = "";
           parme.course = "";     // we store the course for each individual tee time (see below)
           parme.p91 = 0;
           parme.p92 = 0;
           parme.p93 = 0;
           parme.p94 = 0;
           ...
           //
           //  Now get each tee time that has been assigned and gather the player info
           //
           if (teecurr_id1 > 0) {
              ...
              pstmt2.clearParameters();
              pstmt2.setInt(1, teecurr_id1);       // get this tee time
              ...
              if (rs2.next()) {
                 ...
                 parme.day = rs2.getString("day");
                 parme.dd = rs2.getInt("dd");
                 parme.mm = rs2.getInt("mm");
                 parme.yy = rs2.getInt("yy");
                 parme.fb = rs2.getInt("fb");
                 parme.time = rs2.getInt("time");
                 
                 parme.course = rs2.getString("courseName");       // save course name in course for first tee time only
                 
                 parme.player1 = rs2.getString("player1");
                 parme.player2 = rs2.getString("player2");
                 parme.player3 = rs2.getString("player3");
                 parme.player4 = rs2.getString("player4");
                 parme.player5 = rs2.getString("player5");

                 parme.pcw1 = rs2.getString("p1cw");
                 parme.pcw2 = rs2.getString("p2cw");
                 parme.pcw3 = rs2.getString("p3cw");
                 parme.pcw4 = rs2.getString("p4cw");
                 parme.pcw5 = rs2.getString("p5cw");

                 parme.p91 = rs2.getInt("p91");
                 parme.p92 = rs2.getInt("p92");
                 parme.p93 = rs2.getInt("p93");
                 parme.p94 = rs2.getInt("p94");
                 parme.p95 = rs2.getInt("p95");

                 /*
                 parme.guest_id1 = rs2.getInt("guest_id1");
                 parme.guest_id2 = rs2.getInt("guest_id2");
                 parme.guest_id3 = rs2.getInt("guest_id3");
                 parme.guest_id4 = rs2.getInt("guest_id4");
                 parme.guest_id5 = rs2.getInt("guest_id5");
                  */
            
                 event = rs2.getString("event");
                 type = rs2.getInt("event_type");          
            
                 lott_email = rs2.getInt("lottery_email");
                 
                 if (lott_email == 1) {              // ok to send an email for this tee time?
                  
                    parme.user1 = rs2.getString("username1");        // yes, provide usernames so we can get email addresses
                    parme.user2 = rs2.getString("username2");        //    otherwise we include these players in the other groups' emails
                    parme.user3 = rs2.getString("username3");
                    parme.user4 = rs2.getString("username4");
                    parme.user5 = rs2.getString("username5");
                    
                    found = true;                                // indicate at least one group to send
                    count++;                                     // track number of tee times to send emails for
                 }
                  ...
                  if (!event.equals( "" ) && type == 1) {
                     ...
                     try {
                        ...
                        pstmtev.clearParameters();        // clear the parms
                        pstmtev.setString(1, event);
                        rs3 = pstmtev.executeQuery();      // execute the prepared stmt
                     ...
                     //  convert time to hour and minutes for email msg
                     //
                     parme.wuser1 = hr + ":" + SystemUtils.ensureDoubleDigit(min) + ampm + " (Shotgun)";    // put time string in wuser !!!!!!!!!!!!!!!!!!
                      ...
                      //  convert time to hour and minutes for email msg
                      //
                      time = parme.time;              // time for this tee time
                      hr = time / 100;
                      min = time - (hr * 100);
                      ampm = " AM";
                      if (hr > 12) {
                      ...
                      parme.wuser1 = hr + ":" + SystemUtils.ensureDoubleDigit(min) + ampm;       // put time string in wuser !!!!!!!!!!!!!!!!!!
           ...
           if (teecurr_id2 > 0) {
              ...
              pstmt2.clearParameters();
              pstmt2.setInt(1, teecurr_id2);       // get this tee time
              ...
              if (rs2.next()) {
                 ...
                 parme.mm = rs2.getInt("mm");
                 parme.yy = rs2.getInt("yy");
                 parme.fb2 = rs2.getInt("fb");
                 parme.time2 = rs2.getInt("time");
                 
                 parme.oldplayer2 = rs2.getString("courseName");       // save course name in oldplayer since it is not used for these emails
                 
                 parme.player6 = rs2.getString("player1");
                 parme.player7 = rs2.getString("player2");
                 parme.player8 = rs2.getString("player3");
                 parme.player9 = rs2.getString("player4");
                 parme.player10 = rs2.getString("player5");

                 parme.pcw6 = rs2.getString("p1cw");
                 parme.pcw7 = rs2.getString("p2cw");
                 parme.pcw8 = rs2.getString("p3cw");
                 parme.pcw9 = rs2.getString("p4cw");
                 parme.pcw10 = rs2.getString("p5cw");

                 parme.p96 = rs2.getInt("p91");
                 parme.p97 = rs2.getInt("p92");
                 parme.p98 = rs2.getInt("p93");
                 parme.p99 = rs2.getInt("p94");
                 parme.p910 = rs2.getInt("p95");
                 
                 /*
                 parme.guest_id6 = rs2.getInt("guest_id1");
                 parme.guest_id7 = rs2.getInt("guest_id2");
                 parme.guest_id8 = rs2.getInt("guest_id3");
                 parme.guest_id9 = rs2.getInt("guest_id4");
                 parme.guest_id10 = rs2.getInt("guest_id5");
                  */
                 ...
                 if (lott_email == 1) {              // ok to send an email for this tee time?
                    ...
                    parme.user6 = rs2.getString("username1");        // yes, provide usernames so we can get email addresses
                    parme.user7 = rs2.getString("username2");        //    otherwise we include these players in the other groups' emails
                    parme.user8 = rs2.getString("username3");
                    parme.user9 = rs2.getString("username4");
                    parme.user10 = rs2.getString("username5");
                  ...
                  if (!event.equals( "" ) && type == 1) {
                     ...
                     //  convert time to hour and minutes for email msg
                     //
                     parme.wuser2 = hr + ":" + SystemUtils.ensureDoubleDigit(min) + ampm + " (Shotgun)";    // put time string in wuser !!!!!!!!!!!!!!!!!!
                      ...
                      //  convert time to hour and minutes for email msg
                      //
                      time = parme.time2;              // time for this tee time
                      hr = time / 100;
                      min = time - (hr * 100);
                      ampm = " AM";
                      if (hr > 12) {
                      ...
                      parme.wuser2 = hr + ":" + SystemUtils.ensureDoubleDigit(min) + ampm;       // put time string in wuser !!!!!!!!!!!!!!!!!!
           ...
           if (teecurr_id3 > 0) {
              ...
              pstmt2.clearParameters();
              pstmt2.setInt(1, teecurr_id3);       // get this tee time
              ...
              if (rs2.next()) {
                 ...
                 parme.mm = rs2.getInt("mm");
                 parme.yy = rs2.getInt("yy");
                 parme.fb3 = rs2.getInt("fb");
                 parme.time3 = rs2.getInt("time");
                 
                 parme.oldplayer3 = rs2.getString("courseName");       // save course name in oldplayer since it is not used for these emails
                 
                 parme.player11 = rs2.getString("player1");
                 parme.player12 = rs2.getString("player2");
                 parme.player13 = rs2.getString("player3");
                 parme.player14 = rs2.getString("player4");
                 parme.player15 = rs2.getString("player5");

                 parme.pcw11 = rs2.getString("p1cw");
                 parme.pcw12 = rs2.getString("p2cw");
                 parme.pcw13 = rs2.getString("p3cw");
                 parme.pcw14 = rs2.getString("p4cw");
                 parme.pcw15 = rs2.getString("p5cw");

                 parme.p911 = rs2.getInt("p91");
                 parme.p912 = rs2.getInt("p92");
                 parme.p913 = rs2.getInt("p93");
                 parme.p914 = rs2.getInt("p94");
                 parme.p915 = rs2.getInt("p95");
                 
                 /*
                 parme.guest_id11 = rs2.getInt("guest_id1");
                 parme.guest_id12 = rs2.getInt("guest_id2");
                 parme.guest_id13 = rs2.getInt("guest_id3");
                 parme.guest_id14 = rs2.getInt("guest_id4");
                 parme.guest_id15 = rs2.getInt("guest_id5");
                  */
                 ...
                 if (lott_email == 1) {              // ok to send an email for this tee time?
                    ...
                    parme.user11 = rs2.getString("username1");        // yes, provide usernames so we can get email addresses
                    parme.user12 = rs2.getString("username2");        //    otherwise we include these players in the other groups' emails
                    parme.user13 = rs2.getString("username3");
                    parme.user14 = rs2.getString("username4");
                    parme.user15 = rs2.getString("username5");
                  ...
                  if (!event.equals( "" ) && type == 1) {
                     ...
                     //  convert time to hour and minutes for email msg
                     //
                     parme.wuser3 = hr + ":" + SystemUtils.ensureDoubleDigit(min) + ampm + " (Shotgun)";    // put time string in wuser !!!!!!!!!!!!!!!!!!
                      ...
                      //  convert time to hour and minutes for email msg
                      //
                      time = parme.time3;              // time for this tee time
                      hr = time / 100;
                      min = time - (hr * 100);
                      ampm = " AM";
                      if (hr > 12) {
                      ...
                      parme.wuser3 = hr + ":" + SystemUtils.ensureDoubleDigit(min) + ampm;       // put time string in wuser !!!!!!!!!!!!!!!!!!
           ...
           if (teecurr_id4 > 0) {
              ...
              pstmt2.clearParameters();
              pstmt2.setInt(1, teecurr_id4);       // get this tee time
              ...
              if (rs2.next()) {
                 ...
                 parme.mm = rs2.getInt("mm");
                 parme.yy = rs2.getInt("yy");
                 parme.fb4 = rs2.getInt("fb");
                 parme.time4 = rs2.getInt("time");
                 
                 parme.oldplayer4 = rs2.getString("courseName");       // save course name in oldplayer since it is not used for these emails
                 
                 parme.player16 = rs2.getString("player1");
                 parme.player17 = rs2.getString("player2");
                 parme.player18 = rs2.getString("player3");
                 parme.player19 = rs2.getString("player4");
                 parme.player20 = rs2.getString("player5");

                 parme.pcw16 = rs2.getString("p1cw");
                 parme.pcw17 = rs2.getString("p2cw");
                 parme.pcw18 = rs2.getString("p3cw");
                 parme.pcw19 = rs2.getString("p4cw");
                 parme.pcw20 = rs2.getString("p5cw");

                 parme.p916 = rs2.getInt("p91");
                 parme.p917 = rs2.getInt("p92");
                 parme.p918 = rs2.getInt("p93");
                 parme.p919 = rs2.getInt("p94");
                 parme.p920 = rs2.getInt("p95");
                 
                 /*
                 parme.guest_id16 = rs2.getInt("guest_id1");
                 parme.guest_id17 = rs2.getInt("guest_id2");
                 parme.guest_id18 = rs2.getInt("guest_id3");
                 parme.guest_id19 = rs2.getInt("guest_id4");
                 parme.guest_id20 = rs2.getInt("guest_id5");
                  */
                 ...
                 if (lott_email == 1) {              // ok to send an email for this tee time?
                    ...
                    parme.user16 = rs2.getString("username1");        // yes, provide usernames so we can get email addresses
                    parme.user17 = rs2.getString("username2");        //    otherwise we include these players in the other groups' emails
                    parme.user18 = rs2.getString("username3");
                    parme.user19 = rs2.getString("username4");
                    parme.user20 = rs2.getString("username5");
                  ...
                  if (!event.equals( "" ) && type == 1) {
                     ...
                     //  convert time to hour and minutes for email msg
                     //
                     parme.wuser4 = hr + ":" + SystemUtils.ensureDoubleDigit(min) + ampm + " (Shotgun)";    // put time string in wuser !!!!!!!!!!!!!!!!!!
                      ...
                      //  convert time to hour and minutes for email msg
                      //
                      time = parme.time4;              // time for this tee time
                      hr = time / 100;
                      min = time - (hr * 100);
                      ampm = " AM";
                      if (hr > 12) {
                      ...
                      parme.wuser4 = hr + ":" + SystemUtils.ensureDoubleDigit(min) + ampm;       // put time string in wuser !!!!!!!!!!!!!!!!!!
           ...
           if (teecurr_id5 > 0) {
              ...
              pstmt2.clearParameters();
              pstmt2.setInt(1, teecurr_id5);       // get this tee time
              ...
              if (rs2.next()) {
                 ...
                 parme.mm = rs2.getInt("mm");
                 parme.yy = rs2.getInt("yy");
                 parme.fb5 = rs2.getInt("fb");
                 parme.time5 = rs2.getInt("time");
                 
                 parme.oldplayer5 = rs2.getString("courseName");       // save course name in oldplayer since it is not used for these emails !!
                 
                 parme.player21 = rs2.getString("player1");
                 parme.player22 = rs2.getString("player2");
                 parme.player23 = rs2.getString("player3");
                 parme.player24 = rs2.getString("player4");
                 parme.player25 = rs2.getString("player5");

                 parme.pcw21 = rs2.getString("p1cw");
                 parme.pcw22 = rs2.getString("p2cw");
                 parme.pcw23 = rs2.getString("p3cw");
                 parme.pcw24 = rs2.getString("p4cw");
                 parme.pcw25 = rs2.getString("p5cw");

                 parme.p921 = rs2.getInt("p91");
                 parme.p922 = rs2.getInt("p92");
                 parme.p923 = rs2.getInt("p93");
                 parme.p924 = rs2.getInt("p94");
                 parme.p925 = rs2.getInt("p95");
                 
                 /*
                 parme.guest_id21 = rs2.getInt("guest_id1");
                 parme.guest_id22 = rs2.getInt("guest_id2");
                 parme.guest_id23 = rs2.getInt("guest_id3");
                 parme.guest_id24 = rs2.getInt("guest_id4");
                 parme.guest_id25 = rs2.getInt("guest_id5");
                  */
                 ...
                 if (lott_email == 1) {              // ok to send an email for this tee time?
                    ...
                    parme.user21 = rs2.getString("username1");        // yes, provide usernames so we can get email addresses
                    parme.user22 = rs2.getString("username2");        //    otherwise we include these players in the other groups' emails
                    parme.user23 = rs2.getString("username3");
                    parme.user24 = rs2.getString("username4");
                    parme.user25 = rs2.getString("username5");
                  ...
                  if (!event.equals( "" ) && type == 1) {
                     ...
                     //  convert time to hour and minutes for email msg
                     //
                     parme.wuser5 = hr + ":" + SystemUtils.ensureDoubleDigit(min) + ampm + " (Shotgun)";    // put time string in wuser !!!!!!!!!!!!!!!!!!
                      ...
                      //  convert time to hour and minutes for email msg
                      //
                      time = parme.time5;              // time for this tee time
                      hr = time / 100;
                      min = time - (hr * 100);
                      ampm = " AM";
                      if (hr > 12) {
                      ...
                      parme.wuser5 = hr + ":" + SystemUtils.ensureDoubleDigit(min) + ampm;       // put time string in wuser !!!!!!!!!!!!!!!!!!
            ...
            //***********************************************
            //
            if (found == true) {           // if any to send
           
               sendEmail.sendIt(parme, con);      // send the emails - in common
               
               //
               //   reset the lottery fields in the tee times
               //
               if (teecurr_id1 > 0) {
            ...
            }     // end of IF any emails sent
           
        } // end while loop for lreqs3
    ...
    //
    //  Now see if there are any tee times in this lottery that contain players that have not been sent emails yet.  This can occur
    //  when a player or a group has been moved into a tee time that is part of the lottery, but not associated with a lottery request.
    //
    try {
        ...
        pstmt = con.prepareStatement ("" +
            "SELECT * " +
            "FROM teecurr2 " +
            "WHERE date = ? AND lottery = ? AND lottery_email = 1 AND " +
            "(player1 <> '' OR player2 <> '' OR player3 <> '' OR player4 <> '' OR player5 <> '');");
        ...
        while (rs2.next()) {
           ...
           teecurr_id1 = rs2.getInt("teecurr_id");
           ...
           parme.course = rs2.getString("courseName");   
           ...
           parme.user1 = rs2.getString("username1");        //   provide usernames so we can get email addresses
           parme.user2 = rs2.getString("username2");       
           parme.user3 = rs2.getString("username3");
           parme.user4 = rs2.getString("username4");
           parme.user5 = rs2.getString("username5");
        ...
        SystemUtils.buildDatabaseErrMsg("Fatal error in Proshop_dsheet.sendLotteryEmails - sending misc emails.", exp.toString(), out, false);
    }

    

    if (sendUnass == 1) {     // if pro opted to send emails to unassigned players too
    
        //
        //  See if there are any unassigned players and send them an email to notify them that they did not get a time.
        //
        try {
            ...
            pstmt = con.prepareStatement ("" +
                    "SELECT * " +
                    "FROM lreqs3 " +
                    "WHERE date = ? AND name = ? AND state = 1;");
            ...
            while (rs2.next()) {
                ...
                parme.mm = rs2.getInt("mm");
                parme.yy = rs2.getInt("yy");
                parme.course = rs2.getString("courseReq");     
                ...
                parme.player4 = rs2.getString("player4");
                parme.player5 = rs2.getString("player5");
                parme.player6 = rs2.getString("player6");
                parme.player7 = rs2.getString("player7");
                parme.player8 = rs2.getString("player8");
                parme.player9 = rs2.getString("player9");
                parme.player10 = rs2.getString("player10");
                parme.player11 = rs2.getString("player11");
                parme.player12 = rs2.getString("player12");
                parme.player13 = rs2.getString("player13");
                parme.player14 = rs2.getString("player14");
                parme.player15 = rs2.getString("player15");
                parme.player16 = rs2.getString("player16");
                parme.player17 = rs2.getString("player17");
                parme.player18 = rs2.getString("player18");
                parme.player19 = rs2.getString("player19");
                parme.player20 = rs2.getString("player20");
                parme.player21 = rs2.getString("player21");
                parme.player22 = rs2.getString("player22");
                parme.player23 = rs2.getString("player23");
                parme.player24 = rs2.getString("player24");
                parme.player25 = rs2.getString("player25");

                parme.user1 = rs2.getString("user1");       
                parme.user2 = rs2.getString("user2");       
                parme.user3 = rs2.getString("user3");
                parme.user4 = rs2.getString("user4");
                parme.user5 = rs2.getString("user5");
                parme.user6 = rs2.getString("user6");
                parme.user7 = rs2.getString("user7");
                parme.user8 = rs2.getString("user8");
                parme.user9 = rs2.getString("user9");
                parme.user10 = rs2.getString("user10");
                parme.user11 = rs2.getString("user11");       
                parme.user12 = rs2.getString("user12");       
                parme.user13 = rs2.getString("user13");
                parme.user14 = rs2.getString("user14");
                parme.user15 = rs2.getString("user15");
                parme.user16 = rs2.getString("user16");
                parme.user17 = rs2.getString("user17");
                parme.user18 = rs2.getString("user18");
                parme.user19 = rs2.getString("user19");
                parme.user20 = rs2.getString("user20");
                parme.user21 = rs2.getString("user21");       
                parme.user22 = rs2.getString("user22");       
                parme.user23 = rs2.getString("user23");
                parme.user24 = rs2.getString("user24");
                parme.user25 = rs2.getString("user25");
                ...
                parme.pcw4 = rs2.getString("p4cw");
                parme.pcw5 = rs2.getString("p5cw");
                parme.pcw6 = rs2.getString("p6cw");
                parme.pcw7 = rs2.getString("p7cw");
                parme.pcw8 = rs2.getString("p8cw");
                parme.pcw9 = rs2.getString("p9cw");
                parme.pcw10 = rs2.getString("p10cw");
                parme.pcw11 = rs2.getString("p11cw");
                parme.pcw12 = rs2.getString("p12cw");
                parme.pcw13 = rs2.getString("p13cw");
                parme.pcw14 = rs2.getString("p14cw");
                parme.pcw15 = rs2.getString("p15cw");
                parme.pcw16 = rs2.getString("p16cw");
                parme.pcw17 = rs2.getString("p17cw");
                parme.pcw18 = rs2.getString("p18cw");
                parme.pcw19 = rs2.getString("p19cw");
                parme.pcw20 = rs2.getString("p20cw");
                parme.pcw21 = rs2.getString("p21cw");
                parme.pcw22 = rs2.getString("p22cw");
                parme.pcw23 = rs2.getString("p23cw");
                parme.pcw24 = rs2.getString("p24cw");
                parme.pcw25 = rs2.getString("p25cw");
                ...
                parme.p94 = rs2.getInt("p94");
                parme.p95 = rs2.getInt("p95");
                parme.p96 = rs2.getInt("p96");
                parme.p97 = rs2.getInt("p97");
                parme.p98 = rs2.getInt("p98");
                parme.p99 = rs2.getInt("p99");
                parme.p910 = rs2.getInt("p910");
                parme.p911 = rs2.getInt("p911");
                parme.p912 = rs2.getInt("p912");
                parme.p913 = rs2.getInt("p913");
                parme.p914 = rs2.getInt("p914");
                parme.p915 = rs2.getInt("p915");
                parme.p916 = rs2.getInt("p916");
                parme.p917 = rs2.getInt("p917");
                parme.p918 = rs2.getInt("p918");
                parme.p919 = rs2.getInt("p919");
                parme.p920 = rs2.getInt("p920");
                parme.p921 = rs2.getInt("p921");
                parme.p922 = rs2.getInt("p922");
                parme.p923 = rs2.getInt("p923");
                parme.p924 = rs2.getInt("p924");
                parme.p925 = rs2.getInt("p925");

                parme.wuser1 = "Unassigned";          // put time string in wuser !!!!!!!!!!!!!!!!!!
            ...
            }                // end of WHILE unassigned requests
            pstmt.close();

        }
        catch (Exception exp) {
            SystemUtils.buildDatabaseErrMsg("Fatal error in Proshop_dsheet.sendLotteryEmails - sending Unassigned emails.", exp.toString(), out, false);
        }
    }         // end of IF sendUnass
    ...
    out.println("<p>We sent emails for " + count + " tee times.</p>");
    
   // out.println("<p>Requests found = " + req_count + ", tee times found = " + tee_count + ".</p>");
 ...
 }       // end of sendLotteryEmails
 

 
 private void sendEventEmails(String clubName, String event_name, int event_date, int index, String returnCourse, PrintWriter out, Connection con, int setNoPost) {
...
*/    
    try {
        ...
        SystemUtils.buildDatabaseErrMsg("Proshop_dsheet.sendEventEmails - Fatal error sending emails.", exp.toString(), out, false);
   ...
   if (setNoPost > 0) {         // if we are to set the No Post indicators too

      try {
      
          pstmt = con.prepareStatement ("UPDATE teecurr2 SET nopost1 = 1 " +
                                        "WHERE date = ? && event = ? && username1 != ''");

          pstmt.clearParameters();
          pstmt.setInt(1, event_date);
          pstmt.setString(2, event_name);
          countNoPost += pstmt.executeUpdate();
         
          pstmt = con.prepareStatement ("UPDATE teecurr2 SET nopost2 = 1 " +
                                        "WHERE date = ? && event = ? && username2 != ''");
          ...
          pstmt = con.prepareStatement ("UPDATE teecurr2 SET nopost3 = 1 " +
                                        "WHERE date = ? && event = ? && username3 != ''");
          ...
          pstmt = con.prepareStatement ("UPDATE teecurr2 SET nopost4 = 1 " +
                                        "WHERE date = ? && event = ? && username4 != ''");
          ...
          pstmt = con.prepareStatement ("UPDATE teecurr2 SET nopost5 = 1 " +
                                        "WHERE date = ? && event = ? && username5 != ''");
         ...
         SystemUtils.buildDatabaseErrMsg("Proshop_dsheet.sendEventEmails - Fatal error setting No Post.", exp.toString(), out, false);
   ...
   if (setNoPost > 0) {         // if we are to set the No Post indicators too
        ...
        out.println("<p>We also set " + countNoPost + " players to 'No Post' status.</p>");
   }
 ...
 }      // end of sendEventEmails
 
 

 private void sendFlipFBemails(long date, String club, int oldFB, String course, String user, Connection con) {
    ...
    parme.activity_id = 0;
    parme.club = club;          
    parme.type = "moveWhole";    // type = Move Whole tee time - this works well for this purpose too
    parme.date = date;
    parme.time = 0;    
    ...
    try {
        ...
        rs = pstmt.executeQuery();      //  get all the tee times for this day
        ...
        while ( rs.next() ) {
           ...
           guest_id5 = rs.getInt("guest_id5");
            ...
            if (!event.equals("") && event_type == 1) {
               ...
               shotgun_time = Utilities.getEventTime(event, con);
               ...
               parme.to_time = time;
               parme.from_time = time;
            }
            parme.fb = 0;
            parme.to_fb = fb;
            parme.from_fb = oldFB;
            parme.to_course = course;
            parme.from_course = course;
            parme.mm = mm;
            parme.dd = dd;
            parme.yy = yy;
        ...
        }     // end of WHILE tee times for this day
        
        pstmt.close();
    
    } catch (Exception exp) {
                
       Utilities.logError("Error getting tee times in Proshop_dsheet.sendFlipFBemails.  Error=" +exp.toString());                                
    }
        
 }    // end of sendFlipFBemails
...
*/    
    ...
    out.println("<h3>Done!</h3>");
    out.println("<p>We sent " + count3 + " emails to " + count2 + " members for " + count1 + " tee times.</p>");
    //out.println("<a href=\"Proshop_jump?index=" +index+ "&course=" +returnCourse+ "\" title=\"Return to Tee Sheet\" alt=\"Return\">Back To Tee Sheet</a>");
 ...
 private static int getLotteryEmail(long date, int time, int fb, String course, Connection con) {
    ...
    int lottery_email = 0;
     ...
     try {
         ...
         pstmt1b = con.prepareStatement (
            "SELECT lottery_email FROM teecurr2 " +
            "WHERE date = ? AND time = ? AND fb = ? AND courseName = ?");
         ...
         pstmt1b.setInt(2, time);
         pstmt1b.setInt(3, fb);
         pstmt1b.setString(4, course);

         rs = pstmt1b.executeQuery();      // execute the prepared stmt

         if (rs.next()) {

            lottery_email = rs.getInt(1);          // get the first time found - use as upper limit for display
         }
         pstmt1b.close();

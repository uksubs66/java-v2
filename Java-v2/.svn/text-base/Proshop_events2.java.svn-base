 ...
 *   last updated:
 *
 *        4/24/14   Add email address to Golf Genius export.
 *        4/03/14   Changes to Golf Genius export - add mNum as Member and modify the handicap index values.
 *        4/02/14   Add a heading row to the export file for Golf Genius.
 *        2/25/14   Interlachen - add a question for the tees that member will play from (custom).
 *        1/16/14   Ballen Isles CC (ballenisles) - Added control panel option to generate lottery assignments for all members registered for a given event (case 2347).
 *        1/08/14   Add Golf Genius as an export type.  Same as EventMan except that we include the handicap index.
 *        9/23/13   Fixed positive handicaps so they show with the proper "+" ahead of them.
 *        7/12/13   Added an additional column to event log display to indicate whether emails were suppressed for event log item.
 *        7/15/13   Bald Peak CC (baldpeak) - Added custom to display the C/W cell in red for "CAD" modes of transportation.
 *        7/12/13   Handicap indexes will now be displayed behind player names on the Detailed View under the Print List option, if disp_hndcp is enabled.
 *        4/05/13   Handicap values will now only be displayed for golf events.
 ...
 *        1/21/13   In notes section, change signup id form short to int to handle larger numbers
 ...
 *        8/26/11   When exporting to TPP, do not strip the decimal portion of a player's handicap index when stripping off a "-" in the value.
 *        8/26/11   When exporting to TPP, if a player is not entered as a guest, but the username is blank, treat them as a guest.
 *        6/28/11   Add Ecel option for the detailed print list option (case 1933 & 1636).
 *        3/12/11   Added ability to view the event signup log - viewEventLog()
...
//
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   // if here to view event log - jump there now
   if (req.getParameter("log") != null) {
       
      SystemUtils.getProshopSubMenu(req, out, lottery);        // required to allow submenus on this page

      viewEventLog(req, user, out, con);
      return;
   }
   ...
   // If requested to generate lottery assignments for all members registered for this event
   if (req.getParameter("genlottassigns") != null) {
       
       int temp_event_id = 0;
       int result = 0;

       try {
           temp_event_id = Integer.parseInt(req.getParameter("id"));
       } catch (Exception ignore) { }

       if (temp_event_id > 0) {
           
           result = verifyCustom.generateLottAssignsFromEvent(user, temp_event_id, con);
           
           out.println("<div style=\"margin-left:auto; margin-right:auto; text-align:center;\">");
           out.println("<br><h3>Generate Lottery Assignments - " + req.getParameter("name") + "</h3>");
           
           if (result == 0) {
               out.println("<br>No lottery assignments generated. Either no members registered, or all registered members have already had assignments generated for this event.");
           } else {
               out.println("<br><br>Lottery assignments were successfully generated for " + result + " members. "
                       + "Please check the <a href=\"Proshop_events2?id=" + temp_event_id + "&name=" + name + "&course=" + course + "&log\" title=\"Event Log\" alt=\"Event Log\">Event Log</a> for additional details.");
           }
      ...
      if (req.getParameter("print") == null) {
         ...
         if (gstOnly == 0 && signup != 0) {     // if not guest only event and members can signup
            ...
            if (updateAccess || manageAccess) {
                out.println("<a href=\"Proshop_events2?name=" +name+ "&course=" +course+ "&print=print\" target=\"_blank\" title=\"Print This Page\" alt=\"Print\">");
                out.println("Print List</a><br><br>");

                if (activity_id == 0) {
                    out.println("<a href=\"Proshop_events2?name=" +name+ "&course=" +course+ "&print=bag\" target=\"_blank\" title=\"Print Bag Room Report\" alt=\"Bag Report\">");
                    out.println("Print Bag Report</a><br><br>");
                }
                out.println("<a href=\"Proshop_events2?name=" +name+ "&course=" +course+ "&print=mnum\" target=\"_blank\" title=\"Print Member Number Report\" alt=\"Member Id Report\">");
                out.println("Print Member Id Report</a><br><br>");

                out.println("<a href=\"Proshop_events2?name=" +name+ "&course=" +course+ "&print=notes\" target=\"_blank\" title=\"List All Player Notes\" alt=\"Player Notes Report\">");
            ...
            if (manageAccess) {
                if (okToMove1 == true) {       // if ok to move and checkin players
                   ...
                   out.println("<a href=\"Proshop_evntChkAll?name=" +name+ "&course=" +course+ "\" title=\"Move Players to " + ((activity_id == 0) ? "Tee" : "Time") + " Sheet & Check Them In\" alt=\"Check All In\">");
                   out.println("Move Players & Check All In</a><br><br>");
                }

                if (okToMove2 == true) {       // if ok to move players w/o checkin 

                   out.println("<a href=\"Proshop_evntChkAll?name=" +name+ "&course=" +course+ "&nocheck=yes\" title=\"Move Players to " + ((activity_id == 0) ? "Tee" : "Time") + " Sheet w/o Check In\" alt=\"Move All w/o Check In\">");
                   out.println("Move Players & Do Not Check In</a><br><br>");
                }
            }

            if (USE_NEW_SIGNUP && export_type != 0 && activity_id == 0) {
                out.println("<a href=\"Proshop_events2?name=" +name+ "&course=" +course+ "&export\" target=\"_blank\" title=\"Export Players\" alt=\"Export Players\">");
            ...
            if (manageAccess && activity_id == 0) {
                ...
                if (tmp_index >= 0) {
                    out.println("<a href=\"Proshop_dsheet?mode=EVENT&index=" + tmp_index + "&name=" +name+ "&course=" +course+ "&hide=1\" title=\"Manually Move Players to Tee Sheet\" alt=\"Drag-n-Drop\" target=\"_top\">");
                    out.println("Drag-n-Drop</a><br><br>");
                }
            }

            // event signup log
            out.println("<a href=\"Proshop_events2?id=" +event_id+ "&name="+name+"&course=" +course+ "&log\" title=\"Event Log\" alt=\"Event Log\">");
            out.println("View Event Log</a><br><br>");

            if (club.equals("ballenisles") && sess_activity_id == 0) {
                
                // If Ballen Isles, add option to generate mock lottery requests for members registered for this event.
                out.println("<a href=\"Proshop_events2?id=" +event_id+ "&name="+name+"&course=" +course+ "&genlottassigns\" title=\"Generate Lottery Assignments\" alt=\"Generate Lottery Assignments\" "
                        + "onclick=\"return confirm('This will generate a lottery assignment for each member registered for this event. "
                        + "\\n\\nMembers who previously had a lottery assignment generated will be skipped. "
                        + "\\n\\nAre you sure you want to continue?');\">");
                out.println("Generate Lottery Assignments</a><br><br>");
            }
               ...
               if (updateAccess) {
                   ...
                   if (signup != 0) {
                      ...
                      if ( (club.equals("tcclub")) && signup == 0) {
                          ...
                          out.println("<form action=\"Proshop_evntChkAll\" method=\"post\" target=\"bot\">");
                          out.println("<input type=\"hidden\" name=\"name\" value=\"" + name + "\">");
                          out.println("<input type=\"hidden\" name=\"course\" value=\"" + course + "\">");
            ...
            if (gstOnly == 0) {     // if not guest only event
               ...
               if (count_reg == 0) {      // if no one signed up
                  ...
                  if (teams_reg > 5) {
                     ...
                     out.println("<form method=\"get\" action=\"Proshop_events1\">");
                     out.println("<input type=\"submit\" value=\"Return\" style=\"text-decoration:underline; background:#8B8970\">");
                     out.println("</form>");
                  }
                  ...
                  PreparedStatement pstmte = con.prepareStatement (
                     ...
                     "WHERE name = ? AND inactive = 0 ORDER BY wait, r_date, r_time");
                  ...
                  out.println("<tr bgcolor=\"#336633\">");
                     ...
                     if (disp_hndcp == true && sess_activity_id == 0) {
                  ...
                  while (rs2.next()) {
                     ...
                     if (in_use == 0) {
                        ...
                        if (!player1.equals("")) {
                           ...
                           if (player1.equalsIgnoreCase("x")) {   // if 'x'
                              ...
                              if (disp_hndcp == false || sess_activity_id > 0) {
                                 out.println(player1);
                              } else {
                     ...
                     if (season == 0 && activity_id == 0) {
                         ...
                         if (club.equals("baldpeak") && p1cw.equalsIgnoreCase("CAD")) {
                             out.println("<td bgcolor=\"red\" align=\"center\">");
                         } else {
                     ...
                     if (size > 1) {
                        ...
                        if (season == 0 && activity_id == 0) {
                            ...
                            if (club.equals("baldpeak") && p2cw.equalsIgnoreCase("CAD")) {
                                out.println("<td bgcolor=\"red\" align=\"center\">");
                            } else {
                                out.println("<td bgcolor=\"white\" align=\"center\">");
                            }
                     ...
                     if (size > 2) {
                        ...
                        if (season == 0 && activity_id == 0) {
                            ...
                            if (club.equals("baldpeak") && p3cw.equalsIgnoreCase("CAD")) {
                                out.println("<td bgcolor=\"red\" align=\"center\">");
                            } else {
                                out.println("<td bgcolor=\"white\" align=\"center\">");
                            }
                     ...
                     if (size > 3) {
                        ...
                        if (season == 0 && activity_id == 0) {
                            ...
                            if (club.equals("baldpeak") && p4cw.equalsIgnoreCase("CAD")) {
                                out.println("<td bgcolor=\"red\" align=\"center\">");
                            } else {
                                out.println("<td bgcolor=\"white\" align=\"center\">");
                            }
                     ...
                     if (size > 4) {
                        ...
                        if (season == 0 && activity_id == 0) {
                            ...
                            if (club.equals("baldpeak") && p5cw.equalsIgnoreCase("CAD")) {
                                out.println("<td bgcolor=\"red\" align=\"center\">");
                            } else {
                                out.println("<td bgcolor=\"white\" align=\"center\">");
                            }
                        ...
                        //
                        out.println("<form method=\"post\" action=\"Proshop_events2\" target=\"_blank\">");
                        out.println("<td align=\"center\">");
                        out.println("<font size=\"2\">");
                        if (!notes.equals("")) {
         ...
         if (excel == false) {     // if normal request
            ...
            if (printType.equals( "print" )) { 
               ...
               out.println("<td align=\"center\"><font size=\"2\">");
                  out.println("<form method=\"post\" action=\"Proshop_events2\">");
                  out.println("<input type=\"hidden\" name=\"name\" value=\"" +name+ "\">");
                  out.println("<input type=\"hidden\" name=\"course\" value=\"" +course+ "\">");
         ...
         if (count_reg == 0) {      // if no one signed up
            ...
            if (printType.equals( "alpha" )) {
               ...
               if (printType.equals( "full" )) {
                  ...
                  if (club.equals("interlachen") && ask_custom1 == 1) out.println("<td>Tees to Play</td>");
                  ...
                  int team_num = 1;
                  int colspan = 2 + ask_gender + ask_hdcp + ask_homeclub + ask_hdcp + ask_address + ask_email + ask_shirtsize + ask_shoesize + ask_otherA1 + ask_otherA2 + ask_otherA3 + ask_custom1;
                  String tmp_wait = "";
                    ...
                    if (rs2.getString("player1") != null && !rs2.getString("player1").equals("")) {
                        ...
                        for (int p = 1; p <= size; p++) { // 5
                            ...
                            if (rs2.getString("player"+p) != null && !rs2.getString("player"+p).equals("")) {
                                ...
                                String playerName = rs2.getString("player" + p);
                                float hndcp = rs2.getFloat("hndcp" + p);
                                
                                if (hndcp < 0) {
                                    hndcp = hndcp * -1;
                                }
                                
                                if (disp_hndcp && hndcp != 99) {
                                    playerName += " " + hndcp;
                                }
                                ...
                                if (club.equals("interlachen") && ask_custom1 == 1) out.println("<td>" + rs2.getString("customQuest" + p) + "&nbsp;</td>");
    ...
    try {
        ...
        while (rs.next()) {
            ...
            if (export_type == 1) {                
                ...
                golfGeniusPrintPlayer(rs.getString("player1"), rs.getString("username1"), rs.getString("homeclub1"), rs.getString("ghin1"), rs.getString("gender1"), rs.getFloat("hndcp1"), rs.getString("email1"), team_num, 1, firstRecord, con, out);
                
                firstRecord = false;       // Golf Genius requires a heading row in the file
                
                if (!rs.getString("player2").equals("")) golfGeniusPrintPlayer(rs.getString("player2"), rs.getString("username2"), rs.getString("homeclub2"), rs.getString("ghin2"), rs.getString("gender2"), rs.getFloat("hndcp2"), rs.getString("email2"), team_num, 2, firstRecord, con, out);
                if (!rs.getString("player3").equals("")) golfGeniusPrintPlayer(rs.getString("player3"), rs.getString("username3"), rs.getString("homeclub3"), rs.getString("ghin3"), rs.getString("gender3"), rs.getFloat("hndcp3"), rs.getString("email3"), team_num, 3, firstRecord, con, out);
                if (!rs.getString("player4").equals("")) golfGeniusPrintPlayer(rs.getString("player4"), rs.getString("username4"), rs.getString("homeclub4"), rs.getString("ghin4"), rs.getString("gender4"), rs.getFloat("hndcp4"), rs.getString("email4"), team_num, 4, firstRecord, con, out);
                if (!rs.getString("player5").equals("")) golfGeniusPrintPlayer(rs.getString("player5"), rs.getString("username5"), rs.getString("homeclub5"), rs.getString("ghin5"), rs.getString("gender5"), rs.getFloat("hndcp5"), rs.getString("email5"), team_num, 5, firstRecord, con, out);
 ...
 private void tppPrintPlayer(String player, String username, String clubName, String ghin, String gender, String hndcp, Connection con, PrintWriter out) {
    ...
    if (!isGuest && username.equals("")) {      // If not entered as a guest, but the username is blank, treat them as a guest.
        isGuest = true;
    }
 ...
 private void golfGeniusPrintPlayer(String player, String username, String clubName, String ghin, String gender, float hndcp, 
                                    String gstEmail, int team_num, int seq_num, boolean firstRecord, Connection con, PrintWriter out) {
    ...
    // determine if this is a guest or a member
    try {
    ...
    if (!username.equals("")) {
       ...
       clubName = "";     // no Home Club for members
       
      // get mNum, etc
      try {

         pstmt = con.prepareStatement("SELECT memNum, ghin, gender, email FROM member2b WHERE username = ?");
         pstmt.clearParameters();
         pstmt.setString(1, username);
         ...
         if (rs.next()) {
             ...
             // if the ghin # is not here (old events) try to look it up.
             if (ghin.equals("")) ghin = rs.getString("ghin");

             // if the gender is not here (old events) try to look it up.
             if (gender.equals("")) gender = rs.getString("gender");
         ...
         out.println("Error occurred getting member number for player: " + player + " Error= " + exc.getMessage());
       ...
       email = gstEmail;      // get guest email if provided
    }
    
    // Determine the index value to include
    ...
    if (hndcp == 99 || hndcp == -99) {
       ...
       tmp_index = String.valueOf(0 - hndcp);   // -8.4 = 8.4
       ...
       tmp_index = "p" +hndcp;   // p1.5 (plus)       
    }
    
    
    //  output a heading row if this is the 1st record
    if (firstRecord) {
    ...
    // output the row
    out.println("\"" + mNum + "\"," + 
                "\"" + username + "\"," +    // unique member id
                "\"" + clubName + "\"," +    // home club, if guest
                "\"" + email + "\"," +       // email address if member and specified
                "\"" + ghin + "\"," +        // handicap #
                "\"" + lname + "\"," +       // last name
                "\"" + fname + "\"," +       // first name
                "\"" + tmp_index + "\"," +   // hdcp index 
                "\"" + gender + "\"," +      // gender
                "\"" + team_num + "\"," +    // team #
                "\"" + seq_num + "\"," +     // player pos within team #
                "\"" + tmp_memGst + "\"");   // member or guest
     
 }   // end of Golf Genius Export
 ...
 private void viewEventLog(HttpServletRequest req, String pro_user, PrintWriter out, Connection con) {
    ...
    String email_suppressed = "";
    String sign_up_date = "";

    String order_by = "e1.event_signup_id, event_log_id"; // default to sory by entry
    
    String sort = (req.getParameter("sort") == null) ? "" : req.getParameter("sort");

    if (sort.equals("reg")) {

        order_by = "e2.r_date, e2.r_time";

    } else if (sort.equals("origby")) {

        order_by = "full_name";

    } else if (sort.equals("entry")) {

        order_by = "entry_timestamp";

    }

    int event_id = Utilities.parseInt(req.getParameter("id"));
    int signup_num = 0;
    String tmp = "";

    out.println("<br><h3 align=center>" + name + " Sign-Up Log</h3><br>");
    ...
    try {
        ...
        pstmt = con.prepareStatement(""
                + "SELECT e1.*, e3.season, "
                + "DATE_FORMAT(e1.date_time, '%c/%d/%Y') AS pretty_date, "
                + "DATE_FORMAT(e1.date_time, '%l:%i %p') AS pretty_time, "
                + "e2.r_date, e2.r_time, CONCAT(m1.name_last, ', ', m1.name_first, ' ', m1.name_mi) AS full_name "
                + "FROM event_log e1 "
                + "LEFT OUTER JOIN evntsup2b e2 ON e1.event_signup_id = e2.id "
                + "LEFT OUTER JOIN events2b e3 ON e1.event_id = e3.event_id "
                + "LEFT OUTER JOIN member2b m1 ON e1.user = m1.username "
                + "WHERE e1.event_id = ? AND (e3.season = 0 || DATEDIFF(now(), e1.date_time) < 183) "
                + "ORDER BY " + order_by);
        ...
        tmp = "<a style=\"color:darkGreen\" href=\"?id=" + event_id + "&name=" + name + "&log&sort=";
        
        out.println(
                "<div style=\"text-align: center\">" +
                "Sort By: &nbsp;" +
                ((sort.equals("")) ? "<b>" : (tmp + "\">")) + "Sign-Up Number" + ((sort.equals("")) ? "</b>" : "</a>") +
                "&nbsp&nbsp;&nbsp;&nbsp;" +
                ((sort.equals("entry")) ? "<b>" : (tmp + "entry\">")) + "Log Entry Date" + ((sort.equals("entry")) ? "</b>" : "</a>") +
                "&nbsp&nbsp;&nbsp;&nbsp;" +
                ((sort.equals("reg")) ? "<b>" : (tmp + "reg\">")) + "Sign-Up Date" + ((sort.equals("reg")) ? "</b>" : "</a>") +
                "&nbsp&nbsp;&nbsp;&nbsp;" +
                ((sort.equals("origby")) ? "<b>" : (tmp + "origby\">")) + "Reserved By Name" + ((sort.equals("origby")) ? "</b>" : "</a>") +
                "</div><br><br>");
        
        /*
        out.println(
                "<div style=\"text-align: center\">" +
                "<a href=\"?id=" + event_id + "&name=" + name + "&log&sort=default\" style=\"color:darkGreen\">Order By Sign-Up #</a>" +
                "&nbsp&nbsp;&nbsp;&nbsp;" +
                "<a href=\"?id=" + event_id + "&name=" + name + "&log&sort=entry\" style=\"color:darkGreen\">Log Entry Date</a>" +
                "&nbsp&nbsp;&nbsp;&nbsp;" +
                "<a href=\"?id=" + event_id + "&name=" + name + "&log&sort=reg\" style=\"color:darkGreen\">Registration Date</a>" +
                "&nbsp&nbsp;&nbsp;&nbsp;" +
                "<a href=\"?id=" + event_id + "&name=" + name + "&log&sort=user\" style=\"color:darkGreen\">Reserved By Name</a>" +
                "</div><br><br>");
        */
        
        out.println("<table align=center border=1 cellspacing=5 cellpadding=3>");

        int last_signup_id = 0;
        String row_color = "";
        ...
        while ( rs.next() ) {
            ...
            if (h == false) {
                ...
                if (rs.getInt("e3.season") == 1) {
                    seasonLong = true;
                }
                
                if (sort.equals("origby")) {

                    out.println(
                            "<tr bgcolor=\"#336633\" style=\"color: white\">" +
                              "<td><b>User</b></td>" +
                              "<td><b>ID</b></td>" +
                              "<td nowrap><b>Log Entry Date</b></td>" +
                              "<td><b>Action</b></td>" +
                              "<td><b>Sign-Up Date</b></td>" +
                              "<td><b>Details</b></td>" +
                              "<td><b>Email Suppressed?</b></td>" +
                            "</tr>");
            ...
            if (rs.getString("action").equalsIgnoreCase("CONFIG") && !Utilities.isFTProshopUser(pro_user)) {
                continue log_entry_loop;
            }
            
            signup_num++;

            // if member full_name will contain their name, if proshop user then full_name will be null and the
            // proshop username will be present in user - assign accordingly
            user = (rs.getString("full_name") == null) ? rs.getString("user") : rs.getString("full_name");

            // handle row shading
            if (rs.getInt("event_signup_id") != last_signup_id) {

                last_signup_id = rs.getInt("event_signup_id");
                x = (x==false);
                row_color = ((x==true) ? "#F9F9F9" : "#F2F2F2");
            }
            
            if (rs.getString("action").equalsIgnoreCase("CREATE")) {
                bgcolor = "green";
                font_color = "white";
            } else if (rs.getString("action").equalsIgnoreCase("UPDATE")) {
                bgcolor = "yellow";
                font_color = "darkGreen";
            } else if (rs.getString("action").equalsIgnoreCase("CANCEL")) {
                bgcolor = "red";
                font_color = "white";
            } else if (rs.getString("action").equalsIgnoreCase("CONFIG")) {
                bgcolor = "darkorange";
                font_color = "black";
            } else {
                bgcolor = row_color;
            ...
            if (rs.getInt("email_suppressed") == 1) {
                email_suppressed = "Yes";
            } else if (rs.getInt("email_suppressed") == 2) {
                email_suppressed = "No";
            } else {
                email_suppressed = "N/A";
            }
            
            out.println("<tr bgcolor=\"" + row_color + "\">");
            
            if (rs.getString("action").equalsIgnoreCase("CONFIG")) {
                sign_up_date = "N/A";
            } else {
                sign_up_date = Utilities.getDateFromYYYYMMDD(rs.getInt("r_date"), 2) + " at "  + Utilities.getSimpleTime(rs.getInt("r_time"));
            }

            if (sort.equals("origby")) {

                out.print("<td nowrap>" + user + "</td>"); // member name or proshop username
                out.print("<td>" + rs.getInt("event_signup_id") + "</td>");
                out.print("<td nowrap>" + rs.getString("pretty_date") + " at "  + rs.getString("pretty_time") + "</td>"); // entry date/time
                out.print("<td bgcolor=\"" + bgcolor + "\" style=\"color:" + font_color + "\">" + rs.getString("action") + "</td>");
                out.print("<td nowrap style=\"text-align: center;\">" + sign_up_date + "</td>"); // registered date/time

            } else if (sort.equals("reg")) {
            
                out.print("<td nowrap style=\"text-align: center;\">" + sign_up_date + "</td>"); // registered date/time
                out.print("<td nowrap>" + user + "</td>");
                out.print("<td>" + rs.getInt("event_signup_id") + "</td>");
                out.print("<td bgcolor=\"" + bgcolor + "\" style=\"color:" + font_color + "\">" + rs.getString("action") + "</td>");
                out.print("<td nowrap>" + rs.getString("pretty_date") + " at "  + rs.getString("pretty_time") + "</td>");
            ...
            out.print("<td nowrap>" + rs.getString("detail") + "</td>");
            out.print("<td nowrap style=\"text-align: center;\">" + email_suppressed + "</td>");
            out.println("</tr>");
        ...
        if (!h) {
            ...
            out.println("<tr><td align=center colspan=5><i><b><font color=red>No history found for this event.</font></b></i></td></tr>");
            out.println("</table>");

        } else {
            
            out.println("</table>");
            out.println("<center><p><i><b>&nbsp;Found "+ signup_num +" historical entries " + (seasonLong ? "in the past 6 months" : "") + " for this event.</b></i></p></center>");
        }

    } catch (Exception exc) {

        out.println("Proshop_events2.viewEventLog() Error occurred displaying event log for event_id #" + event_id + ". Error= " + exc.getMessage());
    ...
    out.println("<p align=center><font size=2><i>This report was generated on " + df_full.format(cal.getTime()) + "</i></font></p>");

    //out.println("<p align=center><button onclick=\"javascript:history.back(1)\">Return</button></p>");
    // style="text-decoration:underline; background:#8B8970"

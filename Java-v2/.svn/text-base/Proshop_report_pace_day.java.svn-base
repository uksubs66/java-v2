...
public class Proshop_report_pace_day extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   out.println("</font></td></tr></table><br>");
      ...
      out.println("<form action=\"Proshop_report_pace_day\" target=\"bot\" method=\"post\" name=\"frmLoadDay\">");
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   try {
      ...
      if (!excel.equals("yes")) {                              // if NOT Excel Format
         ...
         out.println("<td align=\"center\">");
         out.println("<form action=\"Proshop_report_pace_day\" method=\"post\" target=\"_blank\">");
         ...
         out.println("<td align=\"center\">");
         out.println("<form action=\"Proshop_report_pace_day\" method=\"get\" target=\"bot\">");
      ...
      out.println("<span style=\"font-size: smaller\"><span style=\"font-weight: bold\">NOTE</span>: Lag refers to the number of minutes that a group finished after the group ahead of them.  "
              + "It does not factor in the actual start time or the tee time intervals.</span>");
      ...
      while (rs.next()) {
         ...
         try {
             pstmt2 = con.prepareStatement(
                     ...
                     + "IFNULL(HOUR(SUBTIME((SELECT hole_timestamp FROM pace_entries WHERE teecurr_id = ? AND hole_number = 18), (SELECT hole_timestamp FROM pace_entries WHERE teecurr_id = ? AND hole_number = 18))), -1) AS lag_hr, "
                     + "IFNULL(MINUTE(SUBTIME((SELECT hole_timestamp FROM pace_entries WHERE teecurr_id = ? AND hole_number = 18), (SELECT hole_timestamp FROM pace_entries WHERE teecurr_id = ? AND hole_number = 18))), -1) AS lag_min, "
             ...
             pstmt2.setInt(6, prev_teecurr_id);
             pstmt2.setInt(7, teecurr_id);
             pstmt2.setInt(8, prev_teecurr_id);
             pstmt2.setInt(9, teecurr_id);
             ...
             if (rs2.next()) {
                 if (!(rs2.getInt("hr") == 0 && rs2.getInt("min") == 0)) {
                     ...
                     if (invert != 18) {
                         pace_time += "<br><font size=1>(thru " + hole_num + ")</font>";
                     }
                     lag_hr = rs2.getInt("lag_hr");
                     lag_min = rs2.getInt("lag_min");
                 }
             }
             
         } catch (Exception e) {
             Utilities.logError("Proshop_report_pace_day.doPost - " + club + " - Error looking up pace_status info for teecurr_id=" + teecurr_id + " - Error=" + e.toString());
         } finally {
             Connect.close(rs2, pstmt2);
         }

         prev_teecurr_id = teecurr_id;    // Save this teecurr_id so we can use it next iteration for determining lag time
         ...
         if (lag_hr == -1 || lag_min == -1 || (lag_hr == 0 && lag_min == 0)) {    // If either is missing (-1), or both are zero, it means we couldn't calculate a lag time for this group
             out.println("<td style=\"text-align:center\"><font size=\"2\">N/A</font></td>");
         } else {
             String lag_color = "yellowgreen";
             String lag_disp = "";
             
             if (lag_hr > 0 || lag_min >= 14) {
                 lag_color = "pink";
             }
             ...
             if (lag_min > 0) {
                 lag_disp += lag_min + "min";
             }
             out.println("<td style=\"text-align:center; background-color:" + lag_color + ";\"><font size=\"2\">" + lag_disp + "</font></td>");
         }
         out.println("</tr>");

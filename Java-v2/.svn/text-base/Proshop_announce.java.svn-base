...
public class Proshop_announce extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   if (req.getParameter("backups") != null) {
       ...
       Connection con = Connect.getCon(req, newClub);
...
*/
    ...
    boolean new_skin = true; // (user.equals(DINING_USER)) || Utilities.isNewSkinActive(club, Connect.getCon(req));
   ...
   // add club selection drop-down if specific club/user
   
   String group_name = "";

   if (Utilities.isFTProshopUser(user)) {   // FT universal proshop user
       
       group_name = "ALL";
       
   } if (Utilities.isClubInGroup(club, "AGC") && user.equalsIgnoreCase("proshopngc")) {
       
       group_name = "AGC";
       
   } if (Utilities.isClubInGroup(club, "TPC") && (user.equalsIgnoreCase("proshoppassport") || user.equalsIgnoreCase("proshopsales") || user.equalsIgnoreCase("proshopregional"))) {
       
       group_name = "TPC";
       
   } else if (Utilities.isClubInGroup(club, "Columbia Hospitality") && user.equalsIgnoreCase("proshoppatrick")) {
       
       group_name = "Columbia Hospitality";
   } 
   
   if (!group_name.equals("")) {    // if club is part of a group and a specific user
      
      //  add drop down list of all clubs - for switch
      out.println("<p><br>");    
      ...
      if (group_name.equals("ALL")) group_name = "";   // list all clubs for FT proshop user
      ...
      try {
         ...
         rs = stmt.executeQuery("SELECT clubname, fullname, group_name, inactive FROM clubs" + (!group_name.equals("") ? " WHERE group_name = '" + group_name + "'" : "") + " ORDER BY fullname");
         ...
         while (rs.next()) {
               ...
               if (group_name.equals("") || inactive == 0) {
                   out.println("<option" + (clubname.equals(club) ? " selected" : "") + " value=\"" + clubname + "\">" + fullname + (group_name.equals("") ? " - " + clubname : "") + (inactive > 0 ? " (INACT)" : "") + "</option>");
               }
         }  

      } catch (Exception e) {
          Utilities.logError("Proshop_announce.doGet - " + club + " - user: " + user + " - Error loading club selection drop-down - Error: " + e.toString());
      } finally {
          Connect.close(rs, stmt, con2);
      }          

      out.println("</select><br /></p></form>");
   }    // end of IF group
   
     if (user.equals(DINING_USER) || new_skin || user.equals("proshopfb")) {
         out.println(diningPrefix);
     }
...
@SuppressWarnings({"unchecked","deprecation"})
 private void manageListStagingPages(HttpServletRequest req, HttpServletResponse resp, final String club, String user, final int activity_id, Connection con, PrintWriter out) {
    ...
    boolean new_skin = true; //Utilities.isNewSkinActive(club, con);
...
*/
    ...
    for (File f : files) {
        ...
        if (club.equals("palmvalley-cc") || club.equals("mg-cc")) {  // needs custom handling due to the "-" in their clubname
...
*/
         ...
         if (isDining == true) {       // if Dining Admin
            ...
            out.println("<br><br>");
            out.println("<a href=\"Proshop_announce?manage&list\" target=\"bot\">Manage Non-Active Announcement Pages</a>");
            out.println("</p>");
            out.println("</font></td>");
            out.println("</tr>");
            out.println("<tr>");

 ...
 *        1/31/13   Fixed anchor tag so it is properly closed.
 *        1/21/11   Add tflag to member list and use yellow background for Inact or Excluded values.
 *       10/25/10   Added capability to sort by 'Status' column
...
public class Admin_memlist extends HttpServlet {
  ...
  **/
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (action != null && action.equals("report"))
      ...
      printMemberList(out, con, req, resp, rsync, club, user);
  ...
  ***************************************************************************************
  **/
 protected void printMemberList(PrintWriter out, Connection con, HttpServletRequest req, HttpServletResponse res, int rsync, String club, String user)
 {
    ...
    try {                            // Get all columns from member table for names requested
      ...
      String order_by = "last_only, name_last, name_first, name_mi";
      ...
      PreparedStatement stmt = con.prepareStatement (
               "SELECT SUBSTRING_INDEX(name_last, '_', 1) AS last_only, DATE_FORMAT(last_sync_date, '%c/%e%/%Y') AS last_sync_date, m.* FROM member2b m WHERE name_last LIKE ? ORDER BY " + order_by);
      ...
      users.addColumn(new Column("tflag", "Flag"));
      users.addColumn(new Column("inactive", "<A class=\"btnHref\" href=\"" + versionId + "servlet/Admin_memlist?letter=" + orig_letter + "&sortby=inact\">Status</A>"));
      if (rsync > 0) {                                          // if Roster Sync club
          
          users.addColumn(new Column("exclude", "Exclude"));
         
          if (Utilities.isFTAdminUser(user)) {
              users.addColumn(new Column("last_sync_date", "Last Sync Date"));
          }
      }
      ...
      while(rs.next()) {
         ...
         if (username.equalsIgnoreCase("save4foretees")) {
             continue;
         }
         
         deleted = rs.getInt("deleted");    // deleted flag (1 = deleted) - we now flag them instead of actually removing the record
         
         if (!user.equalsIgnoreCase("admin4tea") && deleted == 1) {     // only display delted members for admin4tea
             continue;
         }
         ...
         if (deleted == 1) {
            deleteUrl = "javascript:deleteUser('" + versionId + "servlet/Admin_editmem?letter=" + orig_letter + "')";
         }
         ...
         if (deleted == 1) {                                 // deleted?
             row.add(username, "rwDtaY");                    //  use yellow bg to make more obvious      
         } else {
             row.add(username);
         }
         ...
         if (deleted == 1) {                                       // deleted?
            row.add(" DEL", "rwDtaY");                             // yes, highlight in yellow
         } else if (inact == 1) {                                  // inactive?
            row.add(" Inact", "rwDtaY");                           // yes, highlight in yellow
         } else {
            row.add(" A");
         }           
         ...
         if (rsync > 0) {                                          // if Roster Sync club
            ...
            if (deleted == 1) {                                       // deleted?
               row.add(" DEL", "rwDtaY");                             // yes, highlight in yellow
            } else if (billable == 1) {                               // billable ?
               row.add(" In");                                        // Yes
            } else {
               row.add(" Ex", "rwDtaY");                              // excluded - yellow background
            }
            
            if (Utilities.isFTAdminUser(user)) {
                row.add(rs.getString("last_sync_date"));
            }
         }
  ...
  protected void printReport(PrintWriter out, Connection con)
  {
      ...
      try {
        ...
        stmt1 = con.prepareStatement (
                 "SELECT COUNT(*) FROM member2b WHERE deleted = 0");
        ...
        //
        stmt1 = con.prepareStatement (
           "SELECT m_ship, COUNT(*) FROM member2b WHERE deleted = 0 GROUP BY m_ship");
        ...
        while(rs.next()) {
           ...
           stmt2 = con.prepareStatement (
                 "SELECT m_type, COUNT(*) FROM member2b WHERE m_ship = ? AND deleted = 0 GROUP BY m_type");

 ...
 *   last updated:
 *
 *        7/22/13   Prompt user before building tee times if holes > 18 - see if they want multiple tee times created per group.
 *        4/11/11   Do not allow null values in userg fields when inserting a new tee time.
 *        2/08/11   Fixed issue with moved flag not getting set to 1 when moving all event signups to tee sheets.
...
//
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   if (req.getParameter("continue") == null) {
      ...
      out.println("<tr><td align=\"center\" valign=\"top\">");
      out.println("<BR><BR><H2>Event Move Players Confirmation</H2><BR>");
      
      out.println("<form method=\"post\" action=\"Proshop_evntChkAll\">");
      out.println("<input type=\"hidden\" name=\"name\" value=\"" + name + "\">");
      out.println("<input type=\"hidden\" name=\"course\" value=\"" + course + "\">");
      
      out.println("<table border=\"1\" valign=\"top\" width=\"500\" bgcolor=\"#F5F5DC\">");       // table for messages
      out.println("<tr><td align=\"center\" valign=\"top\">");
      out.println("<font size=\"2\">");
      
      out.println("<BR>You have requested that all players currently signed up for");
      ...
      out.println("<BR><BR>Players on the wait list will not be included.");
        
      if (parmc.posType.equals( "Pro-ShopKeeper" ) && noCheck == false) {

         out.println("<BR><BR></font></td></tr><tr><td align=\"center\" valign=\"top\">");
         out.println("<font size=\"2\">");
         out.println("<BR><b>NOTE:</b> POS charges, if any, will be accumulated for the members in this event.");
         if (holes > 18) {
      ...
      if (course.equals( "-ALL-" )) {
         ...
         out.println("<BR><BR></font></td></tr><tr><td align=\"center\" valign=\"top\">");
         out.println("<font size=\"2\">");
         out.println("<BR><b>NOTE:</b> The event was configured to use ALL courses.");
      ...
      //
      //  If more than 18 holes then make sure the user wants to create multiple tee times per group
      //
      if (activity_id == 0 && holes > 18) {

         out.println("<BR><BR></font></td></tr><tr><td align=\"center\" valign=\"top\" bgcolor=\"yellow\">");
         out.println("<font size=\"2\">");
         out.println("<BR><b>IMPORTANT:</b> The event was configured for " +holes+ " holes.");
         out.println("<BR><BR>Would you like to create multiple tee times per team to accommodate<BR>the number of holes specified for this event?&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
      
         out.println("<input type=\"checkbox\" value=\"yes\" name=\"multipleTees\" id=\"multipleTees\"><label for=\"multipleTees\">Yes</label>");
      }
      
      out.println("<BR><BR></font></td></tr>");
      out.println("</table>");                   // end of message table
   ...
   //
   //  User wants to proceed with the Move and/or Check-In
   //
   if (activity_id == 0) {    // if Golf
       ...
       boolean setMoved = false;        // Value should be set to true immediately prior to inserting the final tee times for signups
       ...
       //
       //  Check if user only wants one tee time per group
       //
       if (holes > 18 && req.getParameter("multipleTees") == null) {
          
          holes = 18;     // force to one tee time per group
       }
       ...
       if (holes == 9) {
          ...
          doPOS = pskeeper;
          setMoved = true;
          error = add9(con, parm, parmp, parmc, doPOS, setMoved);   // add 9 hole tee time entries

       } else if (holes == 18) {

          doPOS = pskeeper;
          setMoved = true;
          error = add18(con, parm, parmp, parmc, doPOS, setMoved);   // add 18 hole tee time entries
...
*/
 ...
 private int add9(Connection con, parmTee parm, parmPOS parmp, parmClub parmc, boolean doPOS, boolean setMoved) {
      ...
      while (rs2.next()) {
         ...
         if (wait == 0 && moved == 0) {     // process entry if not on wait list and not already moved
            ...
            if (!parm.player1.equals( "" ) || !parm.player2.equals( "" ) || !parm.player3.equals( "" ) ||
               ...
               //
               //  Do not allow null userg's
               //
               if (parm.userg1 == null) parm.userg1 = "";
               if (parm.userg2 == null) parm.userg2 = "";
               if (parm.userg3 == null) parm.userg3 = "";
               if (parm.userg4 == null) parm.userg4 = "";
               if (parm.userg5 == null) parm.userg5 = "";
               ...
               //
               //  Mark this signup moved
               //
               if (setMoved) {
                   try {
                       pstmt = con.prepareStatement("UPDATE evntsup2b SET moved = 1 WHERE id = ?");
                       pstmt.clearParameters();
                       ...
                       Utilities.logError("Error in Proshop_evntChkAll add9 - flagging signup as moved: " + exc.getMessage());
 ...
 private int add18(Connection con, parmTee parm, parmPOS parmp, parmClub parmc, boolean doPOS, boolean setMoved) {
      ...
      while (rs2.next()) {
         ...
         if (wait == 0 && moved == 0) {     // process entry if not on wait list and not already moved
            ...
            if (!parm.player1.equals( "" ) || !parm.player2.equals( "" ) || !parm.player3.equals( "" ) ||
               ...
               //
               //  Mark this signup moved if this is the last tee time to be inserted for this signup
               //
               if (setMoved) {
                   try {
                       pstmt = con.prepareStatement("UPDATE evntsup2b SET moved = 1 WHERE id = ?");
                       ...
                       Utilities.logError("Error in Proshop_evntChkAll add18 - flagging signup as moved: " + exc.getMessage());

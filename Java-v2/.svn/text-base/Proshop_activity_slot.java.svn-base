 ...
 *        12/06/11  Fixed issue where we were only checking for conflicting reservations if the activity had a limit on number of rounds per day.
 ...
 *         9/24/11  Pine Brook CC (pinebrookcc) - Added custom to prevent guests from being included in Sat/Sun 9-12 times until 4pm the day before (case 2027).
 ...
 *         4/13/11  Fixed issue with slotParms.time getting rolled forward while checking restrictions, but not getting reset to the original time
 *                  if we hit a restriction, which caused the reservation time to be incorrect in the email notifications.
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   if (req.getParameter("suppressEmails") != null) {   // user wish to suppress emails? (option on this page only)
    ...
    if (req.getParameter("force_singles") != null) {   // user wish to disallow other from joining their reservation
        ...
        try { force_singles = Integer.parseInt(sid); }
        catch (NumberFormatException e) {}
    }
...
*/
   ...
   if ((req.getParameter("letter") != null) || (req.getParameter("return") != null) ||
      ...
      slotParms.force_singles = force_singles;
      ...
      if (req.getParameter("in_slots") != null) {
          ...
          if (selectedSlots == 0) selectedSlots = tempSlots.length;
          ...
          if (selectedSlots < tempSlots.length) {
              ...
              for (int i=selectedSlots; i<tempSlots.length; i++) {
                  ...
                  try {
                      ...
                      Utilities.logError("Proshop_activity_slot.doPost - " + club + " - Error releasing unselected time slots - Err: " + exc.toString());
                  } finally {
      ...
      if (!memNotice.equals( "" ) && req.getParameter("memNotice") == null) {      // if message to display
         ...
         out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
            ...
            out.println("<table border=\"1\" cols=\"1\" bgcolor=\"#F5F5DC\" cellpadding=\"3\">");
               ...
               out.println("<font size=\"2\">");
               out.println("<form action=\"Proshop_activity_slot\" method=\"post\" name=\"can\">");
               ...
               out.println("<td align=\"center\">");
               out.println("<font size=\"2\">");
                  out.println("<form action=\"Proshop_activity_slot\" method=\"post\">");
                  out.println("<input type=\"hidden\" name=\"slot_id\" value=\"" + slot_id + "\">");
...
*/
      ...
      if (req.getParameter("altSelectedSlots") == null) {
          ...
          if (slot_id == 0) {
             ...
             if (slotParms.slots < 2) {
                 ...
                 // We need information populated during checkInUseM to get the consec_csv list, run this after checkInUse
                 ...
                 */
                 ...
                 if (in_use == 9 || ((in_use == 1 || in_use == 2 || slotParms.layout_mode == 4) && consec_csv.size() > 1 
                     ...
                     promptOtherTime(out, slotParms, run_mode, con);
                     return;
                 }
             }
          }
      ...
      if (slotParms.search_other_times == 1 && (req.getParameter("altSelectedSlots") != null || ((in_use == 1 || in_use == 2) 
          ...
          if (in_use != 0) {
              out.println("<br>Sorry, but we were unable to find enough consecutive time slots at the selected time <br>and location to fulfill your request.<br>");
          }
          ...
          try {
              ...
              Utilities.logError("Proshop_activity_slot.doPost - " + club + " - Error running checkInUseM - ERR: " + exc.toString());
          }
             
          if (in_use == 1) {
              
              out.println("<br>Please return to the time sheets and select another time or location.");
              ...
              try {
                  ...
                  pstmt = con.prepareStatement(
                          "SELECT activity_name, DATE_FORMAT(date_time, '%H%i') as time, a.interval, DATE_FORMAT(date_time, '%l:%i %p') AS display_time "
                  ...
                  if (rs.next()) { 
                      ...
                      out.println("<br>The following <span style=\"font-weight:bold;\">alternate time</span> was found. "
                              + "<br><br>Please click <span style=\"font-weight:bold\">Continue</span> to accept this time and continue, or return to the time sheet.");
                      out.println("</font><font size=\"2\">");
                      out.println("<br><br><span style=\"font-weight:bold;\">DO NOT use you browser's BACK button!</span><br>");
                      out.println("</font><br>");

                      out.println("<hr width=\"75%\" size=\"1\" color=\"#dfeda3\"><br>");

                      out.println("Location: <span style=\"font-weight:bold\">" + rs.getString("activity_name") + "</span><br>");
                      out.println("Start Time: <span style=\"font-weight:bold\">" + rs.getString("display_time") + "</span><br>");
                      out.println("Length: <span style=\"font-weight:bold\">" + resLength + " minutes</span><br><br>");
                      ...
                      out.println("<input type=\"hidden\" name=\"date\" value=\"" + slotParms.date + "\">");
                      out.println("<input type=\"hidden\" name=\"time\" value=\"" + rs.getInt("time") + "\">");
                      out.println("<input type=\"hidden\" name=\"day\" value=\"" + slotParms.day + "\">");
                      out.println("<input type=\"hidden\" name=\"selectedSlots\" value=\"" + slotParms.slots + "\">");
                      out.println("<input type=\"hidden\" name=\"promptOtherTime\" value=\"yes\">");
                      out.println("<input class=\"btnNorm\" type=\"submit\" value=\"Continue\" title=\"Accept times and continue\">");
                      out.println("</form></td>");
                  ...
                  Utilities.logError("Proshop_activity_slot.doPost - " + club + " - Error looking up slot details - ERR: " + exc.toString());
              } finally {
      ...
      if (in_use != 0) {              // if time slot already in use
         ...
         if (index.equals( "888" )) {       // if originated from Proshop_main
            ...
            out.println("<button onclick=\"window.history.go(-1);\">Go Back</button>");
         }
         out.println("</font></CENTER></BODY></HTML>");
         out.close();
         return;
      ...
      time = slotParms.time;
      date = slotParms.date;
      force_singles = slotParms.force_singles;
...
//                out.println("<input type=\"hidden\" name=\"index\" value=\"" + index + "\">");
   ...
   boolean hybrid_slot = Utilities.checkProshopHybridSlot(con);
     ...
     if (hybrid_slot) {
         ...
         out.print(Common_skin.getHeader(club, slotParms.activity_id, "Proshop Activity Booking", false, req));
     } else {
   ...
   out.println("<tr><td align=\"center\"><br>");
      ...
      out.println("<form action=\"Proshop_activity_slot\" method=\"post\" name=\"playerform\">");
      out.println("<input type=\"hidden\" name=\"slot_id\" value=" + slot_id + ">");
      out.println("<input type=\"hidden\" name=\"group_id\" value=" + group_id + ">");
      out.println("<td align=\"center\" valign=\"top\">");
         ...
         if (assign == 0) {      // if normal time slot
            ...
            for (int x = 1; x <= parmAct.max_players; x++) {
                ...
                style = (x > 2 && slotParms.force_singles == 1) ? " style=\"background-color: yellow\"" : "";
            ...
            if (parmAct.force_singles == 1) {
                ...
                out.println("<font size=\"2\"><br><br>Singles Match?:&nbsp;&nbsp; ");
                out.println("<input type=\"checkbox\" name=\"force_singles\" value=\"1\"" + (( force_singles == 1 ) ? " checked" : "") + ">");
   ...
   if (req.getParameter("remove") != null) {
      ...
      if (skip < 2) {
        ...
        if (!slotParms.player3.equals("") && slotParms.force_singles != 0) {      // if singles match and more than 2 players
              ...
              out.println("<BR><BR>Sorry, you cannot include more than 2 players when you select 'Yes'<BR>");
              out.println("for the 'Singles Match?' option. Please limit your group to 2 players.");
              out.println("<BR><BR>");
         ...
         if (club.equals("sunsethillscc") && sess_activity_id == 1) {
             
             if (slotParms.slots > 2 && (slotParms.members + slotParms.guests) < 4) {
                 errorMsg = "<BR><H3>Not Enough Players Found</H3><BR>"
                         + "<BR><BR>Sorry, 90 minute reservations must include 4 players at all times."
                         + "<BR><BR>Please add additional players to this reservation or return to the time sheets.";
                 
             } else if ((slotParms.members + slotParms.guests) < 2) {
                 errorMsg = "<BR><H3>Not Enough Players Found</H3><BR>"
                         + "<BR><BR>Sorry, all reservations must include at least 2 players at all times."
                         + "<BR><BR>Please add additional players to this reservation or return to the time sheets.";
             }
         }
      ...
      if (skip < 6) {
         ...
         if (club.equals("pinebrookcc")) {
             ...
             if (error == true) {
                 ...
                 out.println("<BR>Sorry, <b>" + slotParms.player + "</b> may only play 1 round per month on the indoor courts.<br><br>");
         ...
         if (club.equals("philcricket")) {
             ...
             if (verifyCustom.checkSubtypeActivities(slotParms, sess_activity_id, con)) {
                  ...
                  out.println(SystemUtils.HeadTitle("Member Not Allowed"));
                  out.println("<BODY><CENTER><img src=\"/" +rev+ "/images/FlxRezWebLogo.gif\"><BR>");
                  out.println("<hr width=\"40%\">");
                  out.println("<BR><BR><H3>Member Not Allowed</H3><BR>");
                  out.println("<BR>Sorry, " + slotParms.player + " is not authorized to be a part of this reservation.<br>");
                  out.println("<BR>This member does not have access to this activity. If you would like to include them in this reservation, please add them as a guest.<br>");
         ...
         if (club.equals("quecheeclubtennis")) {
             ...
             if (verifyCustom.checkSubtypeActivities(slotParms, sess_activity_id, con)) {
                  ...
                  out.println("<BR><BR><H3>Member Not Allowed</H3><BR>");
                  out.println("<BR>Sorry, " + slotParms.player + " is not authorized to be a part of this reservation.<br>");
                  out.println("<BR>This member does not have access to this activity. Please remove this member from the reservation, or return to the time sheets.<br>");
      ...
      //  If this skip is set, then we've already been through these tests.
      //
      //  only do this check if this activity enforces a per day limit (removed this portion since we're also checking here to see if that member has other reservations that overlap)
      //
      if (skip < 7) { // && parmAct.rndsperday != 0 && parmAct.minutesbtwn != 0
         ...
         try {
            ...
            dbError(out, e21);
            Utilities.logError("verifyActSlot.checkSched - club=" + club + ", activity_id=" + slotParms.activity_id + ", slot_id=" + slotParms.slot_id);
            return;
         }
    ...
    for (int i = 0; i < slotParms.sheet_ids.size(); i++) {
        ...
        try {
            ...
            PreparedStatement pstmt6 = con.prepareStatement (
                ...
                "SET notes = ?, hideNotes = ?, force_singles = ?, " +
                "last_mod_by = ?, last_mod_date = now(), orig_by = ?, " +
                "proNew = ?, proMod = ?, related_ids = ?, report_ignore = ?, reserved_at = ? " +
                "WHERE sheet_id = ?");

            pstmt6.setString(1, slotParms.notes);
            pstmt6.setInt(2, hide);
            pstmt6.setInt(3, slotParms.force_singles);
            pstmt6.setString(4, user);
            pstmt6.setString(5, slotParms.orig_by);
            pstmt6.setInt(6, proNew);
            pstmt6.setInt(7, proMod);
            pstmt6.setString(8, slotParms.in_slots);
            pstmt6.setInt(9, slotParms.report_ignore);
            pstmt6.setString(10, reserved_at);
            pstmt6.setInt(11, slot_id);
...
*/
   ...
   if (index.equals( "888" )) {         // if came from proshop_searchmain
      ...
      out.println("<link rel=\"stylesheet\" href=\"/" +rev+ "/web utilities/foretees2.css\" type=\"text/css\">");
      out.println("<title>Proshop Slot Page</title>");
      out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_jump?activity=yes&index=" + index + "&last_tab=" +activity_id+ "&activity_id=" +group_id+ "&date=" +date+ "&jump=" + slotParms.jump + "&layout_mode=" + slotParms.layout_mode + "\">");
      out.println("</HEAD>");
      out.println("<BODY bgcolor=\"#FFFFFF\" text=\"#000000\">");
      out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
      out.println("<CENTER><img src=\"/" +rev+ "/images/FlxRezWebLogo.gif\"><BR>");
   ...
   if (sendemail != 0 && suppressEmails.equalsIgnoreCase( "no" )) {
      ...
      try { activity_email_name = getActivity.getActivityEmailName(slot_activity_id, con); }
      catch (Exception ignore) {}
...
*/
    ...
    if (index.equals( "888" )) {       // if originated from Proshop_main
        ...
        out.println("<link rel=\"stylesheet\" href=\"/" +rev+ "/web utilities/foretees2.css\" type=\"text/css\">");
        out.println("<Title>Proshop Slot Page</Title>");
        out.println("<meta http-equiv=\"Refresh\" content=\"1; url=Proshop_jump?activity=&date=" + date + "&index=" + index + "&layout_mode=" + layout + "&sheet_id=" + slot_id + "&activity_id=" + group_id + "\">");
        out.println("</HEAD>");
        out.println("<BODY><CENTER><img src=\"/" +rev+ "/images/FlxRezWebLogo.gif\"><BR>");
        out.println("<hr width=\"40%\">");
        out.println("<BR><BR><H3>Return/Cancel Requested</H3>");
 ...
 private void returnToSlot(PrintWriter out, parmSlot slotParms, boolean allowOverride, String user, int skip) {
   ...
   out.println("<font size=\"2\">");
   out.println("<form action=\"Proshop_activity_slot\" method=\"post\" target=\"_top\">");
   out.println("<input type=\"hidden\" name=\"slot_id\" value=\"" + slotParms.slot_id + "\">");
   out.println("<input type=\"hidden\" name=\"in_slots\" value=\"" + slotParms.in_slots + "\">");
   out.println("<input type=\"hidden\" name=\"group_id\" value=\"" + slotParms.group_id + "\">");
   out.println("<input type=\"hidden\" name=\"activity_id\" value=\"" + slotParms.activity_id + "\">");
   ...
   out.println("<input type=\"hidden\" name=\"force_singles\" value=\"" + slotParms.force_singles + "\">");
   out.println("<input type=\"hidden\" name=\"layout_mode\" value=\"" + slotParms.layout_mode + "\">");
 ...
 private void promptOtherTime(PrintWriter out, parmSlot slotParms, int run_mode, Connection con) {
    ...
    // If doing a standard time lookup returned fewer than expected times, or user is using summary view.
    if (run_mode == 1) {
        ...
        try {
            ...
            Utilities.logError("Proshop_activity_slot.promptOtherTime - " + slotParms.club + " - Error looking up interval data for activity_id (" + slotParms.activity_id + "): Error=" + exc.getMessage());
        ...
        if (interval > 0 && consec_csv.size() > 0) {
            ...
            out.println("<table border=\"1\" cols=\"1\" bgcolor=\"#f5f5dc\" cellpadding=\"5\">");
            out.println("<tr>");
            out.println("<td width=\"85%\" align=\"center\">");
            out.println("<font size=\"3\">");
            out.println("<br><b>NOTICE</b><br></font>");
            out.println("<font size=\"2\">");
            ...
            if (slotParms.sheet_ids.size() == 0) {
                ...
                out.println("<br>Sorry, but the time you selected is currently occupied.");
                
                if (slotParms.search_other_times == 1) {
                    out.println("<br><br>Please select from the other options below, or return to the time sheets.<br><br>");
                } else {
                    out.println("<br><br>Please return to the time sheets and select another time or location.<br><br>");
                }
            } else {
            ...
            // If we couldn't offer them the max amount of time, print buttons to search for the next available set of times.
            if (slotParms.sheet_ids.size() < slotParms.slots && (slotParms.search_other_times == 1/* || slotParms.club.startsWith("demo")*/)) {
                
                out.println("<hr width=\"75%\" size=\"1\" color=\"#dfeda3\">");

                out.println("<table border=\"0\" cellpadding=\"5\">");
                out.println("<tr><td colspan=\"" + (consec_csv.size() - slotParms.sheet_ids.size()) + "\" align=\"center\">Find next available " + common_name + " for:</td></tr>");
                out.println("<tr>");
                
                for (int i = slotParms.sheet_ids.size(); i < consec_csv.size(); i++) {
                    
                    out.println("<form action=\"Proshop_activity_slot\" method=\"post\">");
                    out.println("<td align=\"center\">");
                    out.println("<input type=\"hidden\" name=\"slot_id\" value=\"" + (slotParms.sheet_ids.size() > 0 ? slotParms.sheet_ids.get(0) : slotParms.slot_id) + "\">");
                    out.println("<input type=\"hidden\" name=\"group_id\" value=\"" + slotParms.group_id + "\">");
                    out.println("<input type=\"hidden\" name=\"activity_id\" value=\"" + slotParms.activity_id + "\">");
                    out.println("<input type=\"hidden\" name=\"in_slots\" value=\"" + slotParms.in_slots + "\">");
                    out.println("<input type=\"hidden\" name=\"layout_mode\" value=\"" + slotParms.layout_mode + "\">");
                    ...
                    out.println("<input type=\"hidden\" name=\"day\" value=\"" + slotParms.day + "\">");
                    out.println("<input type=\"hidden\" name=\"selectedSlots\" value=\"" + consec_csv.get(i) + "\">");
                    out.println("<input type=\"hidden\" name=\"altSelectedSlots\" value=\"" + consec_csv.get(i) + "\">");
                    out.println("<input class=\"btnNorm\" type=\"submit\" value=\"" + (consec_csv.get(i) * interval) + " min\" title=\"Find next available time for " + (consec_csv.get(i) * interval) + " minutes\">");
                    out.println("</td>");
                    out.println("</form>");
                }

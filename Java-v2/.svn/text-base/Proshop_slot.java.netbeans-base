 ...
 * 
 *        5/02/14   Oak Hill CC (oakhillcc) - add custom for user to specify if tee time is an advance guest time and track these (case 2361).
 *        4/29/14   Interlachen - adjust 5-some guest restriction - change the date range.
 *        3/28/14   Long Cove Club (longcove) - Updated custom to restrict members to only one time per day with new dates for 2014 (case 1038).
 ...
 *       12/13/13   Silverleaf Club (silverleaf) - Updated caddie master email custom to also send emails for the "FOR" MoT, and made it so emails will be sent for unaccompanied guest groups (case 2329).
 ...
 *        7/12/13   Updated SystemUtils.updateHist call to include an indicator of whether or not emails were suppressed.
 ...
 *        6/06/13   Add the pro's username at the end of the notes when they add or change the tee time notes (so staff can determine who entered the notes).
 ...
 *       12/21/12   Desert Mountain (desertmountain) - allow pro to type username in player slot to get player's name (like olyclub) - case 2210.
 ...
 *        6/19/12   CC of Naples (ccnaples) - Added custom to prompt proshop users if they don't include a guest name.
 ...
 *        3/13/12   Pinery CC (pinery) - Added custom to allow proshop users to select a cart number for each player, which will be displayed on the tee sheet (case 2134).
 ...
 *        3/08/12   Marshfield CC (marshfieldcc) - Added their club to the custom that will prompt Proshop users to enter guest names if they weren't (case 2129).
 ...
 *        1/17/12   In skip 9 processing in verify, clear the userg values if no guests in the tee time.  This is done in case there had been one or more
 *                  guests in the tee time, and they were then removed.
 *       11/30/11   Updated processing so that slotParms.teecurr_id gets populated during verify.
 *       11/22/11   Club at Pradera (pradera) - Added custom to allow proshop users to select a cart number for each player, which will be displayed on the tee sheet (case 2020).
 *       11/08/11   Meadow Club (meadowclub) - Added custom to skip all prompts on day of (set skip value to 8 or 15 off the bat, depending on guests) (case 2055).
 ...
 *       10/18/11   Fixed issue with Staff List short name not getting passed through when assigning a member to unaccompanied guests.
 ...
 *        6/14/11   Olympic Club - Use new memNum lookup inside the player name textbox.
 *        6/13/11   The Olympic Club - do not show the 9-hole option.
 ...
 *       12/23/10   Dataw Island Club (dataw) - Default Suppress Emails box to checked (case 1921).
 *       12/17/10   Dataw Island Club (dataw) - Set default MoT depending on guest type.
 *       11/30/10   Added userg1-5 values to parme population code for sending emails to sponsors of unaccompanied guest times (case 1410).
 *       10/26/10   Monterey Peninsula CC (mpccpb) - Added custom checkbox to designate a time as a walk-in time (stored as a 1 in the custom_int field of teecurr2)
 *       10/20/10   Omaha CC (omahacc) - Set default MoT to 'GC' for all guest types.
 *       10/20/10   Populate new parmEmail fields
 *       10/11/10   The Estancia Club (estanciaclub) - index now stored in custom_int field if greater than 30 at time of booking.  Members may only have 4 such times on the books
 *                  at any given time, and these times require at least one member and one guest (case 1897).
 *       10/07/10   The custom_int field will now be cleared when a tee time is cancelled
 *        9/09/10   The Estancia Club (estanciaclub) - Set default MoT to 'GC' for all guest types.
...
public class Proshop_slot extends HttpServlet {
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   // If reqeusted, display help information for the "Make tee time private?" option
   if (req.getParameter("makePrivateHelp") != null) {
       Common_Config.makePrivateHelp(out);
       return;
   }
   ...
   if (req.getParameter("trashCan") != null) {     // if request from Proshop_cancel (via the trash can image on tee sheet)
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   boolean displayCartNumbers = false;
   ...
   //  flag to determine if club wants to use the auto type feature where pro types the username into the player slot and the name appears
   boolean userToName = false;
   
   if (club.equals("olyclub") || club.equals("desertmountain")) {
   ...
   if (req.getParameter("suppressEmails") != null) {   // user wish to suppress emails? (option on this page only)
      ...
      if (club.equals("charlottecc") || club.equals("shadowridgecc") || club.equals("desertforestgolfclub") || club.equals("thereserveclub") ||
          club.equals("dataw") || club.equals("bhamcc") || club.equals("aronimink") || club.equals("overlakegcc") || club.equals("jupiterhillsclub") 
      || (club.equals("tcclub") && index.equals("0")) || club.equals("lacumbrecc")) {         // always set to Yes initially for Charlotte
   ...
   if (req.getParameter("custom_int") != null) {
       custom_int = Integer.parseInt(req.getParameter("custom_int"));
   }
   ...
   if ((req.getParameter("letter") != null) || (req.getParameter("return") != null) ||
      ...
      if (req.getParameter("nopost1") != null) nopost1 = Integer.parseInt(req.getParameter("nopost1"));
      if (req.getParameter("nopost2") != null) nopost2 = Integer.parseInt(req.getParameter("nopost2"));
      if (req.getParameter("nopost3") != null) nopost3 = Integer.parseInt(req.getParameter("nopost3"));
      if (req.getParameter("nopost4") != null) nopost4 = Integer.parseInt(req.getParameter("nopost4"));
      if (req.getParameter("nopost5") != null) nopost5 = Integer.parseInt(req.getParameter("nopost5"));
      if (req.getParameter("make_private") != null) make_private = Integer.parseInt(req.getParameter("make_private"));
      if (req.getParameter("custom_int") != null) custom_int = Integer.parseInt(req.getParameter("custom_int"));
...
//            if (newreq == true && (club.equals( "pelicansnest" ) || club.equals( "lakewoodranch" ) || club.startsWith( "demo" ))) {
      ...
      nopost1 = slotParms.nopost1;          // get Handicap No-Post flags
      nopost2 = slotParms.nopost2;
      nopost3 = slotParms.nopost3;
      nopost4 = slotParms.nopost4;
      nopost5 = slotParms.nopost5;
      make_private = slotParms.make_private;
...
//          }
          ...
          if (custom_int == -1) {
              ...
              memNotice += "Notice: There is no crossover period available for this tee time. Only a 9-hole round can be booked.";
      ...
      if (!memNotice.equals( "" )) {      // if message to display
         ...
         out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
            ...
            out.println("<table border=\"1\" cols=\"1\" bgcolor=\"#f5f5dc\" cellpadding=\"3\">");
               ...
               out.println("<tr><td align=\"center\">");
               out.println("<font size=\"2\">");
               out.println("<form action=\"Proshop_slot\" method=\"post\" name=\"can\">");
               out.println("<input type=\"hidden\" name=\"index\" value=" + index + ">");
               out.println("<input type=\"hidden\" name=\"course\" value=\"" + course + "\">");
               out.println("<input type=\"hidden\" name=\"returnCourse\" value=\"" + returnCourse + "\">");
               out.println("<input type=\"hidden\" name=\"date\" value=" + date + ">");
               ...
               out.println("<font size=\"2\">");
                  ...
                  out.println("<input type=\"hidden\" name=\"showlott\" value=\"" + showlott + "\">");
                  out.println("<input type=\"hidden\" name=\"nopost1\" value=\"" + nopost1 + "\">");
                  out.println("<input type=\"hidden\" name=\"nopost2\" value=\"" + nopost2 + "\">");
                  out.println("<input type=\"hidden\" name=\"nopost3\" value=\"" + nopost3 + "\">");
                  out.println("<input type=\"hidden\" name=\"nopost4\" value=\"" + nopost4 + "\">");
                  out.println("<input type=\"hidden\" name=\"nopost5\" value=\"" + nopost5 + "\">");
   ...
   if (club.equals("pradera") || club.equals("pinery") || club.equals("canyonoakscc") || club.equals("beechmontcc") || club.equals("austincountryclub") || club.equalsIgnoreCase("rollinghillsccca") || club.equalsIgnoreCase("bloomfieldhillscc")) {
       
       displayCartNumbers = true;
   }
   ...
   // If Club at Pradera, load custom_disp values into gp ints used by customs for other clubs.
   if (displayCartNumbers) {
       ...
       if (!custom1.equals("")) {
           gp1 = Integer.parseInt(custom1);
       }
       if (!custom2.equals("")) {
           gp2 = Integer.parseInt(custom2);
       }
       if (!custom3.equals("")) {
           gp3 = Integer.parseInt(custom3);
       }
       if (!custom4.equals("")) {
           gp4 = Integer.parseInt(custom4);
       }
       if (!custom5.equals("")) {
           gp5 = Integer.parseInt(custom5);
       }
   }
   ...
   boolean hybrid_slot = Utilities.checkProshopHybridSlot(con);
     ...
     if (hybrid_slot) {
         ...
         out.print(Common_skin.getHeader(club, 0, "Proshop Tee Slot Page", false, req));
     } else {
...
//      out.println("alert(x);");
   ...
   if (assign == 0) {   // if normal tee time
      ...
      if (club.equals( "ranchobernardo" )) {
       ...
       if (club.equals("pinery") || club.equals("trophyclubcc") || club.equals("pmarshgc") || club.equals("midpacific") || club.equals("estanciaclub")
               ...
               || parm.club.equals("dmgcc") || parm.club.equals("bayclubstonetree")) {
      ...
      if (club.equalsIgnoreCase("winchestercc")) {
          out.println("defCW = 'INC';");
      }
      
      if (club.equalsIgnoreCase("philcricket")) {
          out.println("defCW = '';");
      }
      
      if (club.equalsIgnoreCase("blackdiamondranch")) {
          out.println("defCW = 'CRT';");
      }
      
      if (club.equalsIgnoreCase("mosscreek")) {
          out.println("defCW = 'CRT';");
      }
      
      if (club.equalsIgnoreCase("reserveclubatwoodside")) {
          if (course.equals("Nicklaus Course")) {
              out.println("defCW = 'CT';");
          }
          if (course.equals("Zoeller Course")) {
              out.println("defCW = 'CRT';");
          }
      }
      ...
      } else if (club.equalsIgnoreCase("interlachen")) {
          out.println("if (name == 'X - No Fivesome') {");
          out.println("  defCW = 'MC';}");
      } else if (club.equalsIgnoreCase("stcloudcc")) {
          out.println("if (name == 'High School Apollo' || name == 'College $$$' || name == 'College - SCSU') {");
          out.println("  defCW = 'WLK';}");
      ...
      } else if (club.equals("lakewood")) {
          
          out.println("if (name == 'Family Guest' || name == 'Off Season Guest') {");
          out.println("  defCW = 'WA';");
          out.println("}");
          
      } else if (club.equals("marincountryclub")) {
          out.println("defCW = 'N/A';");
      } else if (club.equals("mosscreek")) {
          out.println("if (name == '10 Round' || name == '30 Day Renter' || name == '365 Day Renter' || name == 'Comp' "
                  + "|| name == 'Discovery Package' || name == 'Guest for a Day' || name == 'M4D Dinner' || name == 'M4D Lunch' "
                  + "|| name == 'Sunday Supper' || name == 'Tournament') {");
          out.println("  defCW = 'ACP';");
          out.println("} else if (name == 'Accompanied' || name == 'Junior' || name == 'Local Reciprocal' || name == 'Renter' || name == 'Unaccom') {");
          out.println("  defCW = 'CRT';");
          out.println("} else if (name == 'Afternoon Rate') {");
          out.println("  defCW = 'PM';");
          out.println("}");
          
      } else if (club.equals("gallerygolf")) {
          
          out.println("if (name == 'Accompanied') {");
          out.println("  defCW = 'AG';");
          out.println("} else if (name == 'Unaccompanied') {");
          out.println("  defCW = 'UG';");
          out.println("} else if (name == 'Extended Family') {");
          out.println("  defCW = 'FAM';");
          out.println("} else if (name == 'Afternoon') {");
          out.println("  defCW = 'AFN';");
          out.println("} else if (name == 'Referred Guest') {");
          out.println("  defCW = 'WS';");
          out.println("} else if (name == 'Comp Golf') {");
          out.println("  defCW = 'COM';");
          out.println("} else if (name == 'Event') {");
          out.println("  defCW = 'EVE';");
          out.println("} else if (name == 'Outside Special') {");
          out.println("  defCW = 'OS';");
          out.println("} else if (name == 'Standard') {");
          out.println("  defCW = 'STD';");
          out.println("} else if (name == 'PGA/CartFee') {");
          out.println("  defCW = 'CF';");
          out.println("} else if (name == 'Troon Program') {");
          out.println("  defCW = 'TRN';");
          out.println("}");
      }
      ...
      if (club.equals("coldspringcc")) {
          
          out.println("defCW = 'GLF';");
      }
   ...
   out.println("<body onLoad=\"movenotes()\" bgcolor=\"#FFFFFF\" text=\"#000000\" link=\"#000000\" vlink=\"#000000\" topmargin=\"0\">");
   out.println("<font face=\"Arial, Helvetica, Sans-serif\"><center>");
   ...
   out.println("<tr><td align=\"center\"><br>");
      ...
      out.println("<form action=\"Proshop_slot\" method=\"post\" name=\"can\" autocomplete=\"yes\">");
      out.println("<input type=\"hidden\" name=\"index\" value=" + index + ">");
      out.println("<input type=\"hidden\" name=\"course\" value=\"" + course + "\">");
      out.println("<input type=\"hidden\" name=\"returnCourse\" value=\"" + returnCourse + "\">");
      out.println("<input type=\"hidden\" name=\"day\" value=" + day_name + ">");
      ...
      out.println("<form action=\"Proshop_slot\" method=\"post\" name=\"playerform\">");
      out.println("<td align=\"center\" valign=\"top\">");
         ...
         //Custom to Restrict 'CDY' MOT for springhillgc weekends after 12:59 and weekdays after 2:59
         if (club.equals("springhillgc")) {
             if (day_name.equals("Saturday") || day_name.equals("Sunday")) {
                 if (time <= 1259) {
                     for (i = 0; i < parmc.tmode_limit; i++) {
                         if (!parmc.tmodea[i].equalsIgnoreCase("CDY")) {
         ...
         if (assign == 0) {      // if normal tee time
            ...
            if (handicapSys == true) {
                ...
                if (club.equals("oaklandhills")) tdwidth = 605;
                
            } else {
                
                if (club.equals("oaklandhills")) tdwidth = 525;              
            }
            ...
            out.println("<table border=\"1\" bgcolor=\"#F5F5DC\" align=\"center\" width=\"" +tdwidth+ "\">");  // table for player selection
            ...
            } else if (displayCartNumbers) {
               out.println("Cart Number");
            }
            ...
            out.println("1:&nbsp;&nbsp;<input type=\"text\" name=\"player1\" value=\"" + player1 + "\" size=\"20\" maxlength=\"43\" onkeypress=\"return DYN_disableEnterKey(event)\"" + ((userToName) ? " onchange=\"memTOname(this, 1)\"" : "") + ">");
              ...
              if (club.equals("bishopsgategc") && custom_int == -1) {
                 out.println("&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" checked disabled>");
              ...
              // If Club at Pradera, print a select box containing 1-72 as selectable cart number options
              if (displayCartNumbers) {
                  
                  out.println("<select name=\"gp1\">");
                  ...
                  for (int j=0; j<=72; j++) {
                      ...
                      if (j == 0) {
                          ...
                          out.println("<option value=\"" + String.valueOf(j) + "\"" + (gp1 == j ? " selected" : "") + ">" + j + "</option>");
                      }
                  }
              ...
              if (handicapSys) {          // if Handicap System - include 'No Post' column
                 if (nopost1 == 1) {
                     out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" checked name=\"nopost1\" value=\"1\">");
                 } else {
                     out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" name=\"nopost1\" value=\"1\">");
                 }
              }

            out.println("<br><img src=\"/" +rev+ "/images/erase.gif\" onclick=\"erasename('player2')\" style=\"cursor:hand\">");
            out.println("2:&nbsp;&nbsp;<input type=\"text\" name=\"player2\" value=\"" + player2 + "\" size=\"20\" maxlength=\"43\" onkeypress=\"return DYN_disableEnterKey(event)\"" + ((userToName) ? " onchange=\"memTOname(this, 2)\"" : "") + ">");
              ...
              if (displayCartNumbers) {
                  ...
                  for (int j=0; j<=72; j++) {
                      ...
                      if (j == 0) {
                          ...
                          out.println("<option value=\"" + String.valueOf(j) + "\"" + (gp2 == j ? " selected" : "") + ">" + j + "</option>");
                      }
                  }
              ...
              if (handicapSys) {          // if Handicap System - include 'No Post' column
                 if (nopost2 == 1) {
                      out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" checked name=\"nopost2\" value=\"1\">");
                 } else {
                      out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" name=\"nopost2\" value=\"1\">");
                 }
              }

            out.println("<br><img src=\"/" +rev+ "/images/erase.gif\" onclick=\"erasename('player3')\" style=\"cursor:hand\">");
            out.println("3:&nbsp;&nbsp;<input type=\"text\" name=\"player3\" value=\"" + player3 + "\" size=\"20\" maxlength=\"43\" onkeypress=\"return DYN_disableEnterKey(event)\"" + ((userToName) ? " onchange=\"memTOname(this, 3)\"" : "") + ">");
              ...
              if (displayCartNumbers) {
                  ...
                  for (int j=0; j<=72; j++) {
                      ...
                      if (j == 0) {
                          ...
                          out.println("<option value=\"" + String.valueOf(j) + "\"" + (gp3 == j ? " selected" : "") + ">" + j + "</option>");
                      }
                  }
              ...
              if (handicapSys) {          // if Handicap System - include 'No Post' column
                  if (nopost3 == 1) {
                      out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" checked name=\"nopost3\" value=\"1\">");
                  } else {
                      out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" name=\"nopost3\" value=\"1\">");
                  }
              }

            out.println("<br><img src=\"/" +rev+ "/images/erase.gif\" onclick=\"erasename('player4')\" style=\"cursor:hand\">");
            out.println("4:&nbsp;&nbsp;<input type=\"text\" name=\"player4\" value=\"" + player4 + "\" size=\"20\" maxlength=\"43\" onkeypress=\"return DYN_disableEnterKey(event)\"" + ((userToName) ? " onchange=\"memTOname(this, 4)\"" : "") + ">");
              ...
              if (displayCartNumbers) {
                  ...
                  for (int j=0; j<=72; j++) {
                      ...
                      if (j == 0) {
                          ...
                          out.println("<option value=\"" + String.valueOf(j) + "\"" + (gp4 == j ? " selected" : "") + ">" + j + "</option>");
                      }
                  }
              ...
              if (handicapSys) {          // if Handicap System - include 'No Post' column
                  if (nopost4 == 1) {
                        out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" checked name=\"nopost4\" value=\"1\">");
                  } else {
                        out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" name=\"nopost4\" value=\"1\">");
                  }
              }
            ...
            if (p5.equals( "Yes" )) {
              ...
              out.println("5:&nbsp;&nbsp;<input type=\"text\" id=\"player5\" name=\"player5\" value=\"" + player5 + "\" size=\"20\" maxlength=\"43\" onkeypress=\"return DYN_disableEnterKey(event)\"" + ((userToName) ? " onchange=\"memTOname(this, 5)\"" : "") + ">");
              ...
              if (displayCartNumbers) {
                  ...
                  for (int j=0; j<=72; j++) {
                      ...
                      if (j == 0) {
                          ...
                          out.println("<option value=\"" + String.valueOf(j) + "\"" + (gp5 == j ? " selected" : "") + ">" + j + "</option>");
                      }
                  }
              ...
              if (handicapSys) {          // if Handicap System - include 'No Post' column
                  if (nopost5 == 1) {
                        out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" checked name=\"nopost5\" value=\"1\">");
                  } else {
                        out.println("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" name=\"nopost5\" value=\"1\">");
                  }
              }
            ...
            if (tsNotesView) {
                out.println("<br><br>");
                if (tsNotesUpdate) out.println("<img src=\"/" +rev+ "/images/erase.gif\" onClick=\"erasetext('notes')\" style=\"cursor:hand\">");
                out.println("Notes:&nbsp;<textarea name=\"notes\" " + (!tsNotesUpdate ? "disabled " : "") + "value=\"\" id=\"notes\" cols=\"28\" rows=\"2\">" + notes + "</textarea>");

                out.println("<br><br>&nbsp;&nbsp;Hide Notes from Members?:&nbsp;&nbsp; ");
                out.println("<input type=\"checkbox\" " + (hide != 0 ? "checked " : "") + "name=\"hide\" " + (!tsNotesUpdate ? "disabled " : "") + "value=\"Yes\">");
                out.println("</font><font size=\"1\">&nbsp;(checked = yes)</font><font size=\"2\">");
            } else {
                out.println("<br>");
            }
            
            if (!tsNotesView || !tsNotesUpdate) {
                out.println("<input type=\"hidden\" name=\"notes\" value=\"" + notes + "\">");
            ...
            if (hidenames == 1) {
                out.println("<input type=\"radio\" id=\"mp1\" name=\"make_private\" value=\"1\"" + (make_private <= 1 ? " checked" : "") + "><label for=\"mp1\">Hide Names</label>&nbsp;");
            } else {
                out.println("<input type=\"radio\" id=\"mp0\" name=\"make_private\" value=\"0\"" + (make_private == 0 ? " checked" : "") + "><label for=\"mp0\">No</label>&nbsp;");
                out.println("<input type=\"radio\" id=\"mp1\" name=\"make_private\" value=\"1\"" + (make_private == 1 ? " checked" : "") + "><label for=\"mp1\">Hide Names</label>&nbsp;");
            }
            out.println("<input type=\"radio\" id=\"mp2\" name=\"make_private\" value=\"2\"" + (make_private == 2 ? " checked" : "") + "><label for=\"mp2\">Hide Tee Time</label>&nbsp;");
            ...
            if (club.equals("mpccpb")) {
                ...
                out.println("<input type=\"checkbox\" " + (custom_int == 1 ? "checked " : "") + " name=\"custom_int\" value=\"1\">");
                out.println("</font><font size=\"1\">&nbsp;(checked = yes)</font><font size=\"2\">");
            }
            ...
            //
            //  Oak Hill CC - custom for Advance Guest Time - add a checkbox
            //
//            if (club.equals("oakhillcc")) {
//               
//                out.println("<br><br>Advance Guest Time $50:&nbsp;&nbsp; ");
//                out.println("<input type=\"checkbox\" " + (custom_int == 1 ? "checked " : "") + " name=\"custom_int\" value=\"1\">");
//                out.println("</font><font size=\"1\">&nbsp;&nbsp;(checked = yes)</font><font size=\"2\">");
//            }           
         ...
         if (!club.equals("oaklandhills") && !displayCartNumbers) {
             out.println("<input type=\"hidden\" name=\"custom1\" value=\"" + custom1 + "\">");
             out.println("<input type=\"hidden\" name=\"custom2\" value=\"" + custom2 + "\">");
             out.println("<input type=\"hidden\" name=\"custom3\" value=\"" + custom3 + "\">");
             out.println("<input type=\"hidden\" name=\"custom4\" value=\"" + custom4 + "\">");
         ...
         if (assign == 0) {
            ...
            out.println("<input type=\"hidden\" name=\"nopost4\" value=\"" + nopost4 + "\">");
            out.println("<input type=\"hidden\" name=\"nopost5\" value=\"" + nopost5 + "\">");
            out.println("<input type=\"hidden\" name=\"make_private\" value=\"" + make_private+ "\">");
         }
 ...
 private void verify(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session, HttpServletResponse resp) {
   ...
   if (req.getParameter("trashCan") != null) {     // if request from Proshop_cancel (via the trash can image on tee sheet)
       ...
       trashCan = true;        // set flag for return
   }
   ...
   if (req.getParameter("nopost1") != null) slotParms.nopost1 = Integer.parseInt(req.getParameter("nopost1"));
   if (req.getParameter("nopost2") != null) slotParms.nopost2 = Integer.parseInt(req.getParameter("nopost2"));
   if (req.getParameter("nopost3") != null) slotParms.nopost3 = Integer.parseInt(req.getParameter("nopost3"));
   if (req.getParameter("nopost4") != null) slotParms.nopost4 = Integer.parseInt(req.getParameter("nopost4"));
   if (req.getParameter("nopost5") != null) slotParms.nopost5 = Integer.parseInt(req.getParameter("nopost5"));
   
   if (req.getParameter("make_private") != null) slotParms.make_private = Integer.parseInt(req.getParameter("make_private"));
   ...
   slotParms.showlott = showlott;
   slotParms.proShortName = proShortName;
   ...
   if (!proShortName.equals("")) fullName = proShortName;       // use short name of pro if they use this feature (Pro Id)
   ...
   try {
      ...
      PreparedStatement pstmt = con.prepareStatement (
         ...
         "orig1, orig2, orig3, orig4, orig5, teecurr_id, notes, make_private " +
      ...
      if (club.equals("bishopsgategc")) {  // if bishops bay
      ...
      if ((/*club.equals("oakhillcc") || */club.equals("kiawahislandclub")) && req.getParameter("remove") == null) {  // if bishops bay OR (oak hill AND NOT a Cancel Tee Time) 
          
          if (req.getParameter("custom_int") != null) {
              custom_int = Integer.parseInt(req.getParameter("custom_int"));
          }
          slotParms.custom_int = custom_int;      // get custom_int from tee time if not passed (see directly above)
      }
   ...
   if (req.getParameter("proNameReqd") != null) {           // if name is required
      ...
      if (proShortName.equals("") || proShortName == null) {
   ...
   //
   if (club.equals( "congressional" ) || club.equals("estanciaclub")) {
...
// Test code to detect what could be causing issues when submitting times with tracked guest (primarily at TPC Boston) - *REMOVE ONCE PROBLEM FIXED*
Utilities.logError("*TEST* club: " + club + ", teetime: " + slotParms.date + " - " + slotParms.time + " - " + slotParms.fb + " - " + slotParms.course + ", errPlayer: " + slotParms.gplayer + ", " +
        "p1|gid1: " + slotParms.player1 + "|" + slotParms.guest_id1 + ", p2|gid2: " + slotParms.player2 + "|" + slotParms.guest_id2 + ", p3|gid3: " + slotParms.player3 + "|" + slotParms.guest_id3 + ", " +
        "p4|gid4: " + slotParms.player4 + "|" + slotParms.guest_id4 + ", p5|gid5: " + slotParms.player5 + "|" + slotParms.guest_id5);
      ...
      //
      if ((club.equals("stdavidsgc") || club.equals("pelicansnest") || club.equals("marshfieldcc") || club.equals("ccnaples")) && !slotParms.gplayer.equals( "" ) && slotParms.hit3 == true) {
      ...
      //
      if (club.equals("mayfieldsr") && slotParms.course.equals("Sand Ridge") && verifyCustom.checkMayfieldSR(slotParms.date, slotParms.time, slotParms.day)) {
      ...
      //
      if (club.equals("woodway") && verifySlot.checkWoodway(slotParms.date, slotParms.time, slotParms.fb)) {
      ...
      //
      //  Check for twoSomeOnly times for Huntingdon Valley CC and prompt for override if true
      //
      if (club.equals("huntingdonvalleycc") && verifySlot.checkHuntingdonValley(slotParms.date, slotParms.time, slotParms.course, slotParms.fb)) {
      ...
      // If Meadow Club and day-of, skip all checks and just book the time.
      if (club.equals("meadowclub") && ind == 0) {
          
          // We only need to look at guests if there are both guests and members and there's a guest in player slot 1, or if it's an unaccompanied guest time
          if (skip == 0 && ((slotParms.members != 0 && slotParms.guests != 0 && !slotParms.g1.equals("")) || (slotParms.members == 0 && slotParms.guests != 0))) {
              skip = 8;
          } else {
              skip = 15;
          }
      ...
      if (skip == 20) {
          ...
          try {
             error = verifySlot.checkMemberAccess(slotParms, con);
          }
          catch (Exception ignore) {
          }
          ...
          if (error && skip == 0) {          // if problem
             ...
             out.println("<BODY><CENTER><img src=\"/" +rev+ "/images/foretees.gif\"><BR>");
             out.println("<hr width=\"40%\">");
             out.println("<BR><H3>Membership Restricted</H3><BR>");
             out.println("<BR><BR>Sorry, <b>" +slotParms.player+ "</b> is not allowed to be a part of Golf reservations due to membership privileges.");
             out.println("<BR><BR>Please remove this player and submit the reservation again.");

             //  Return to _slot 
             returnToSlot(out, slotParms, overrideAccess, "", 20);                // use unique skip id for this!!!!!!!!!!
             return;
          }
      }*/
      ...
      if (skip < 2) {
         ...
         //***************************************************************************************************
         //
         String customGstMsg = verifyCustom.checkCustoms1(slotParms, req);     // go check for customs
         ...
         if (!customGstMsg.equals( "" )) {         // if error encountered - reject
            ...
            if (club.equals("kiawahislandclub")) {
                slotParms.custom_int = Utilities.getDateDiff(slotParms.date, con);
            }
      ...
      if (skip < 3) {
         ...
         if (slotParms.guests != 0) {      // if any guests were included
            ...
            if (club.equals("miravista")) {
                ...
                error = verifySlot.checkGuestTimes(slotParms, con, 6, "month", 0, 0);
                ...
                if (error == true) {      // max guest times exceeded this hour
                    ...
                    out.println("<hr width=\"40%\">");
                    out.println("<BR><BR><H3>Maximum Number of Guest Times Exceeded</H3>");
                    out.println("<BR>Sorry, but one of the guest players has already played or is scheduled to play 6 rounds this month.");
                    out.println("<BR><BR>Please remove this guest.");
         ...
         if (club.equals( "congressional" )) {           // if congressional
           ...
           if (ind > 9 || (ind == 9 && thisTime < congTime)) {     // if a special guest time
              ...
              if (slotParms.player2.startsWith( "30 Day Advance Guest" ) || slotParms.player3.startsWith( "30 Day Advance Guest" ) ||
                 ...
                 out.println("<hr width=\"40%\">");
                 out.println("<BR><H3>Invalid Guest Time Request</H3><BR>");
                 out.println("<BR><BR>Sorry, you must include at least one 30 Day Advance Guest in the group<BR>");
      ...
      if (club.equals( "colletonriverclub" )) {
         ...
         if (club.equals( "oakmont" )) {      // if Oakmont CC
            ...
            if (((slotParms.day.equals("Saturday") || slotParms.day.equals("Sunday") || slotParms.day.equals("Monday")) && slotParms.ind > 7) || 
                ((slotParms.day.equals("Tuesday") || slotParms.day.equals("Wednesday") || slotParms.day.equals("Thursday") || slotParms.day.equals("Friday")) && slotParms.ind > 14)) {
               //
...
//         }
            ...
            if (slotParms.rnds > 1) {       // if multiple rounds per day supported
               ...
               if (slotParms.hit3 == true) {       // if rounds too close together
                  ...
                  out.println(slotParms.player + " is already scheduled to play on this date at <b>" +tmsg+ "</b>.<br><br>");
                    ...
                    if (club.equals("longcove") && slotParms.date >= 20150413 && slotParms.date <= 20150419) {
                       out.println("<BR>Sorry, <b>" + slotParms.player + "</b> is already scheduled to play on this date. A member can only be scheduled once per day from April 6th - 12th.");
                    } else {
      ...
      // Skip over the days in advance check for this club
      if (club.equals("kiawahislandclub") && skip < 8) {
          skip = 8;
      }
      ...
      if (skip < 9) {     // *****NOTE:  skip=10 is set in doPost above from 'Assign' here ********
         ...
         //***********************************************************************************************
         //
         if (slotParms.guests == 0) {      // if no guests included in this tee time
...
//             
//            //  At least 1 guest requested in tee time.  If Interlachen, check for a 5-some request (May 1st - Oct 15th - per Sarah on 4/29/14)
//            //  Only 2 Guests are allowed in any 5-some group.
//            //
//            if (club.equals( "interlachen" ) && shortDate >= 501 && shortDate <= 1015) {      // if Interlachen
//
...
//            }     // end of IF Fort Collins
            ...
            if (!slotParms.g1.equals( "" ) || slotParms.members > 1) {  // if slot 1 is a guest OR more than 1 member
               ...
               out.println("<input type=\"hidden\" name=\"showlott\" value=\"" + showlott + "\">");
               out.println("<input type=\"hidden\" name=\"proShortName\" value=\"" + proShortName + "\">");
               out.println("<input type=\"hidden\" name=\"nopost1\" value=\"" +slotParms.nopost1+ "\">");
               out.println("<input type=\"hidden\" name=\"nopost2\" value=\"" +slotParms.nopost2+ "\">");
               out.println("<input type=\"hidden\" name=\"nopost3\" value=\"" +slotParms.nopost3+ "\">");
               out.println("<input type=\"hidden\" name=\"nopost4\" value=\"" +slotParms.nopost4+ "\">");
               out.println("<input type=\"hidden\" name=\"nopost5\" value=\"" +slotParms.nopost5+ "\">");
               out.println("<input type=\"hidden\" name=\"make_private\" value=\"" + slotParms.make_private+ "\">");

               if (slotParms.club.equals("oaklandhills") || displayCartNumbers) {       // Include custom_disp values if present for oaklandhills
               ...
               if (slotParms.club.equals("oakhillcc") || club.equals("kiawahislandclub")) {       // Include custom_int 

                   out.println("<input type=\"hidden\" name=\"custom_int\" value=\"" + slotParms.custom_int + "\">");
               }
            ...
            //
            if (slotParms.guests != 0 && !club.equals( "sonnenalp")
                    && (!club.equals("tetonpines") || ((!slotParms.player1.equals("") && !slotParms.player1.startsWith("Resort") && !slotParms.player1.startsWith("Teton Club") 
                            && !slotParms.player1.startsWith("Powderhorn") && !slotParms.player1.startsWith("Headwaters") && !slotParms.player1.startsWith("TCC"))
                        || (!slotParms.player2.equals("") && !slotParms.player2.startsWith("Resort") && !slotParms.player2.startsWith("Teton Club") 
                            && !slotParms.player2.startsWith("Powderhorn") && !slotParms.player2.startsWith("Headwaters") && !slotParms.player2.startsWith("TCC"))
                        || (!slotParms.player3.equals("") && !slotParms.player3.startsWith("Resort") && !slotParms.player3.startsWith("Teton Club") 
                            && !slotParms.player3.startsWith("Powderhorn") && !slotParms.player3.startsWith("Headwaters") && !slotParms.player3.startsWith("TCC"))
                        || (!slotParms.player4.equals("") && !slotParms.player4.startsWith("Resort") && !slotParms.player4.startsWith("Teton Club") 
                            && !slotParms.player4.startsWith("Powderhorn") && !slotParms.player4.startsWith("Headwaters") && !slotParms.player4.startsWith("TCC"))
                        || (!slotParms.player5.equals("") && !slotParms.player5.startsWith("Resort") && !slotParms.player5.startsWith("Teton Club") 
                            && !slotParms.player5.startsWith("Powderhorn") && !slotParms.player5.startsWith("Headwaters") && !slotParms.player5.startsWith("TCC"))))) {      // if all guests and NOT Sonnenalp and NOT Teton Pines and Resort/Teton Club/Powderhorn/Headwaters/TCC guests
               ...
               out.println("<input type=\"hidden\" name=\"orig_by\" value=\"" + slotParms.orig_by + "\">");
               out.println("<input type=\"hidden\" name=\"jump\" value=\"" + slotParms.jump + "\">");
               out.println("<input type=\"hidden\" name=\"userg1\" value=\"\">");
               out.println("<input type=\"hidden\" name=\"userg2\" value=\"\">");
               out.println("<input type=\"hidden\" name=\"userg3\" value=\"\">");
               out.println("<input type=\"hidden\" name=\"userg4\" value=\"\">");
               out.println("<input type=\"hidden\" name=\"userg5\" value=\"\">");
//               out.println("<input type=\"hidden\" name=\"userg1\" value=\"" + slotParms.userg1 + "\">");
//               out.println("<input type=\"hidden\" name=\"userg2\" value=\"" + slotParms.userg2 + "\">");     If they opt to not assign members, we want to clear out any existing userg values!!!
//               out.println("<input type=\"hidden\" name=\"userg3\" value=\"" + slotParms.userg3 + "\">");
//               out.println("<input type=\"hidden\" name=\"userg4\" value=\"" + slotParms.userg4 + "\">");
//               out.println("<input type=\"hidden\" name=\"userg5\" value=\"" + slotParms.userg5 + "\">");
               out.println("<input type=\"hidden\" name=\"guest_id1\" value=\"" + slotParms.guest_id1 + "\">");
...
//               }
      ...
      //
      if (((!slotParms.player1.equals( "" ) && !slotParms.player1.equalsIgnoreCase( "x" ) && (slotParms.g1.equals( "" ) || !slotParms.userg1.equals(""))) || slotParms.guest_id1 != 0) ||
          ((!slotParms.player2.equals( "" ) && !slotParms.player2.equalsIgnoreCase( "x" ) && (slotParms.g2.equals( "" ) || !slotParms.userg2.equals(""))) || slotParms.guest_id2 != 0) ||
          ((!slotParms.player3.equals( "" ) && !slotParms.player3.equalsIgnoreCase( "x" ) && (slotParms.g3.equals( "" ) || !slotParms.userg3.equals(""))) || slotParms.guest_id3 != 0) ||
          ((!slotParms.player4.equals( "" ) && !slotParms.player4.equalsIgnoreCase( "x" ) && (slotParms.g4.equals( "" ) || !slotParms.userg4.equals(""))) || slotParms.guest_id4 != 0) ||
          ((!slotParms.player5.equals( "" ) && !slotParms.player5.equalsIgnoreCase( "x" ) && (slotParms.g5.equals( "" ) || !slotParms.userg5.equals(""))) || slotParms.guest_id5 != 0)) {
      ...
      if ((!slotParms.player1.equals( "" ) && !slotParms.player1.equalsIgnoreCase( "x" )) ||
         ...
         if (tmp_pci != 2 && club.equals("quailridgecc")) {
             int timelocal = Utilities.getTime(con);
             cal_pci.add(cal_pci.DAY_OF_MONTH, +1);
             tmp_pci = (parm.precheckin == 1
                     && mm == (cal_pci.get(cal_pci.MONTH) + 1)
                     && dd == cal_pci.get(cal_pci.DAY_OF_MONTH)
                     && yy == cal_pci.get(cal_pci.YEAR)
                     && timelocal >= 1700) ? (short) 2 : (short) 0;
         }
         ...
         if (tmp_pci != 2 && club.equals("ccstalbans")) {
             int timecurr = Utilities.getTime(con);
             long datecurr = Utilities.getDate(con, 1);
             tmp_pci = (parm.precheckin == 1
             && slotParms.date == datecurr && timecurr >= 1800) ? (short) 2 : (short) 0;
         }
      ...
      //
      //  Check the handicap no-post flags - only set if player is a member
      //
      if (slotParms.user1.equals("")) slotParms.nopost1 = 0;
      if (slotParms.user2.equals("")) slotParms.nopost2 = 0;
      if (slotParms.user3.equals("")) slotParms.nopost3 = 0;
      if (slotParms.user4.equals("")) slotParms.nopost4 = 0;
      if (slotParms.user5.equals("")) slotParms.nopost5 = 0;


      //
      //  If new notes added or if previous notes were changed, then add this pro's username to the end for identification purposes.
      //
      if (!slotParms.notes.equals("") && !slotParms.notes.equals(oldNotes) && !slotParms.notes.endsWith("(Staff User)")) {

         slotParms.notes = slotParms.notes + " (Staff User)";     
      }           
      ...
      if (club.equals("oakhillcc") && custom_int > 0) {
         ...
         int cust_hist_type = 2;     // Update tee time request
         
         if (slotParms.oldPlayer1.equals( "" ) && slotParms.oldPlayer2.equals( "" ) && slotParms.oldPlayer3.equals( "" ) &&
            slotParms.oldPlayer4.equals( "" ) && slotParms.oldPlayer5.equals( "" )) {

            cust_hist_type = 1;     // New tee time request
         }
   
         verifyCustom.logOakhillAdvGst(slotParms.teecurr_id, slotParms.date, slotParms.day, slotParms.time, slotParms.fb, slotParms.course, slotParms.player1, slotParms.player2, slotParms.player3,
                          slotParms.player4, slotParms.player5, user, fullName, slotParms.notes, cust_hist_type, con);
      }
   ...
   try {
      ...
      PreparedStatement pstmt6 = con.prepareStatement (
         ...
         "SET last_mod_date = now(), player1 = ?, player2 = ?, player3 = ?, player4 = ?, " +
             ...
             "nopost1 = ?, nopost2 = ?, nopost3 = ?, nopost4 = ?, nopost5 = ?, make_private = ? " +
      ...
      pstmt6.setInt(69, slotParms.nopost1);
      pstmt6.setInt(70, slotParms.nopost2);
      pstmt6.setInt(71, slotParms.nopost3);
      pstmt6.setInt(72, slotParms.nopost4);
      pstmt6.setInt(73, slotParms.nopost5);
      
      pstmt6.setInt(74, slotParms.make_private);
         
      pstmt6.setLong(75, slotParms.date);
      pstmt6.setInt(76, slotParms.time);
      pstmt6.setInt(77, slotParms.fb);
      pstmt6.setString(78, slotParms.course);
      ...
      Utilities.logError("Proshop_slot.verify - " + club + " - Error updating tee time - ERR: " + e1.toString());
      ...
      if (trashCan) {   // if caller is the trashcan window on tee sheet
   ...
   //
   int hist_type = 1;                   // default to updated tee time
   int email_suppressed = 0;
   
   if (slotParms.oldPlayer1.equals( "" ) && slotParms.oldPlayer2.equals( "" ) && slotParms.oldPlayer3.equals( "" ) &&
       slotParms.oldPlayer4.equals( "" ) && slotParms.oldPlayer5.equals( "" )) {
      
      hist_type = 0;     // new tee time request
   }
   ...
   SystemUtils.updateHist(slotParms.date, slotParms.day, slotParms.time, slotParms.fb, slotParms.course, slotParms.player1, slotParms.player2, slotParms.player3,
                          slotParms.player4, slotParms.player5, user, fullName, hist_type, email_suppressed, slotParms.make_private, con);
...
*/
   ...
   if (index.equals( "888" )) {         // if came from proshop_searchmain
      ...
      if (req.getParameter("remove") != null) {
         ...
         out.println("<p>&nbsp;</p><p>&nbsp;<b>Thank you!</b>&nbsp;&nbsp;The reservation has been canceled.</p>");
      ...
      //  debug
      //Utilities.logError("Proshop_slot.verify - return: posSent=" +posSent+ ", skipDining=" +skipDining);
       
      if (trashCan) {   // if caller is the trashcan window on tee sheet
                
         out.println("<CENTER><BR><H2>Tee Time Canceled</H2>");
         out.println("<p><b>Thank you!</b>&nbsp;&nbsp;The reservation has been canceled.</p>");
         out.println("<BR>You may need to refresh the tee sheet to see the current values.<BR><BR>");
         out.println("<form>");
         out.println("<input type=button value=\" Return \" onclick=\"window.close()\" style=\"background-color: #8B8970\">");
         out.println("</form>");
         out.println("</CENTER>");
   ...
   //
   if (club.equals("hallbrookcc") || club.equals("riverside")) {
       ...
       if ((slotParms.p1cw.equals("CAD") && !slotParms.oldp1cw.equals("CAD"))
           ...
           sendCustomEmail = true;      // send an email to Caddie Master
   ...
   //***********************************************
   //
   if ((sendemail != 0 && suppressEmails.equalsIgnoreCase( "no" )) || sendCustomEmail) {
      ...
      //  Send the email
      //
      if (sendemail != 0 && suppressEmails.equalsIgnoreCase( "no" )) sendEmail.sendIt(parme, con);      // in common
      

      // Used to send emails for various customs determined prior to the email block of code
      if (sendCustomEmail) {
   ...
   try {
    ...
    if (club.equals("bishopsgategc")) {
        ...
        try {
            ...
            Utilities.logError("Proshop_slot.cancel - " + club + " - Error looking up teecurr_id - ERR: " + exc.toString());
        } finally {
 ...
 private void returnToSlot(PrintWriter out, parmSlot slotParms, boolean allowOverride, String user, int skip) {
   ...
   if (slotParms.club.equals("pradera") || slotParms.club.equals("pinery") || slotParms.club.equals("canyonoakscc") || slotParms.club.equals("beechmontcc") || slotParms.club.equals("austincountryclub") || slotParms.club.equalsIgnoreCase("rollinghillsccca") || slotParms.club.equalsIgnoreCase("bloomfieldhillscc")) {
   ...
   out.println("<input type=\"hidden\" name=\"proShortName\" value=\"" + slotParms.proShortName + "\">");
   out.println("<input type=\"hidden\" name=\"nopost1\" value=\"" +slotParms.nopost1+ "\">");
   out.println("<input type=\"hidden\" name=\"nopost2\" value=\"" +slotParms.nopost2+ "\">");
   out.println("<input type=\"hidden\" name=\"nopost3\" value=\"" +slotParms.nopost3+ "\">");
   out.println("<input type=\"hidden\" name=\"nopost4\" value=\"" +slotParms.nopost4+ "\">");
   ...
   if (!allowOverride || (slotParms.club.equals("mayfieldsr") && slotParms.course.equals("Sand Ridge") && verifyCustom.checkMayfieldSR(slotParms.date, slotParms.time, slotParms.day) && skip == 0)) {
       out.println("<input type=\"submit\" value=\"Return\" name=\"return\" style=\"text-decoration:underline; background:#8B8970\">");
   } else {
       out.println("<input type=\"submit\" value=\"No - Return\" name=\"return\" style=\"text-decoration:underline; background:#8B8970\">");
   }
   ...
   if (allowOverride && (!slotParms.club.equals( "lakewoodranch" ) || skip != 7) && (!slotParms.club.equals("mayfieldsr") || !slotParms.course.equals("Sand Ridge") || !verifyCustom.checkMayfieldSR(slotParms.date, slotParms.time, slotParms.day) || skip != 0)) {    // skip if lakewood ranch and player already playing within x hrs

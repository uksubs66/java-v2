 ...
 *   last updated:
 *
 *       10/24/11  BSK  Updated TeeTimer to clear entries more than 7 days old from the debuglog table.
...
public class TeeTimer {
   ...
   class timeTask extends TimerTask {
    ...
    public void run() {
        ...
        // Run login stat collection (will only log stats on the 1st of each month)
        try {
            
            Support_counts.run_Logreports();
        ...
        try {
            ...
            stmt.executeUpdate("DELETE FROM debuglog WHERE debug_timestamp < DATE_ADD(now(), INTERVAL -7 DAY)");

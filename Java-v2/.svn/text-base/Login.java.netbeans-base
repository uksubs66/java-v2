 ...
 *
 *        6/05/14   Add proshop login message for new message board feature.
 *        5/23/14   Change the message in promptRWD to mention 'mobile friendly or touch style devices' to be more inclusive.
 *        5/17/14   Comment out the old Flexscape SSO methods - we now use Common_webapi.
 *        4/17/14   Add calls to countMobile for RWD mobile logins.
 *        4/16/14   Add RWD message to promptMobileUser to inform mobile user that RWD is available.
 *        4/14/14   Added temp support login for Will.
 *        4/09/14   Add GENMOBILE caller so websites can add a mobile link for their members (if web provider is not CE, MF, etc).
 *        3/31/14   Comment out the logError ALERT message for remote users coming into doGet until we get some websites to stop.
 *        3/31/14   Used username in place of user when calling recordLogin from remoteUser and flexwebUser.  Enhanced the logError message in recordLogin.
...
import com.foretees.api.ApiTasks;
...
public class Login extends HttpServlet {
 ...
 //String passwordSupwill = "foreteeswill";
 ...
 public void init()
   ...
   //
   //  Start Api Task timers.
   //
   try {
      ApiTasks.start();
   }
   catch (Exception e) {
      SystemUtils.logError("ApiTasks: Unable to start ApiTasks from Login.");   // log it and continue
   }
 ...
 public void processExtLogin(HttpServletRequest req, HttpServletResponse resp) {
    ...
    String mship = "";
    String subtype = "";
    String caller = reqUtil.getParameterString(req,"caller",""); 
    String jumpTo = "";
    String name = "";
    String wc = "";
    String errMsg = "";
    ...
    int date = reqUtil.getParameterInteger(req,"date",0);         // contains the date passed from the email dining link (date of booking)
    int customId =  reqUtil.getParameterInteger(req,"customId",0);   // would contain the custom messsage id from the dining link that was clicked on
    int event_id = reqUtil.getParameterInteger(req,"event",0);    // if link to an event - the event id
    
    int activity_id = reqUtil.getParameterInteger(req,"act_id",0);  
    ...
    if (caller.equals("dining")) {       // Dining Request system       
        ...
        //if (req.getParameter("event") != null && !req.getParameter("event").equals("")) {
            ...
            //event_id = reqUtil.getParameterInteger(req,"event",null);
        //}
    ...
    */
    ...
    if (!user.equals("proshop")) {       // if NOT proshop user
       ...
       try {
           ...
           stmt = con.prepareStatement (
               "SELECT CONCAT(name_first, IF(LENGTH(name_mi) = 0, '', CONCAT(' ', name_mi)), ' ', name_last) AS fullName, "
                   + "m_ship, m_type, wc, msub_type, username, id "
           ...
           if (!rs.next()) {          // if user not found
               ...
               user = rs.getString("username"); // get the real user name (case could be different from user entry)
    ...
    if(req.getParameter("s_a") == null && req.getParameter("ext_rdr") == null){
        // We are not currently using the proper base url -- fix it
        // Force us in to RWD mode, and block us from switching
        ...
        resp.setStatus(303);
        resp.setHeader( "Location",  Utilities.getBaseUrl(req, activity_id, club)+"Login?ext_rdr=true&"+req.getQueryString());
    ...
    session = Utilities.getNewSession(req);     // Create a new session object

    //session.setAttribute("sess_id", id);          // set session id for validation ("foretees")
    session.setAttribute("ext-user", user);         // save username as ext-user (so that verifyMem would fail, unless we pass allowext=true)
    session.setAttribute("name", name);             // save user's full name
    session.setAttribute("member_id", member_id);   // save user's member_id
    ...
    // set up the request and make sure the session validates with ext-user
    if(SystemUtils.verifyMem(req, out, true) == null){
        Connect.close(rs,stmt,con);
    ...
    /*if (caller.equals("dining")) {
       ...
       // jumpTo = base_url + "Member_dining";       // Dining Request system
        ...
        StringBuilder unsub_url = new StringBuilder(base_url);
        unsub_url.append("Member_dining?ext-dReq&dReq&caller=email");
        unsub_url.append("&customId=");
        unsub_url.append(customId);
        unsub_url.append("&date=");
        unsub_url.append(date);
        //unsub_url.append("&usr=");
        //unsub_url.append(user);
        unsub_url.append("&ecusr=");
        unsub_url.append(Utilities.URLEncode(Utilities.encryptTTdata(user)));
        resp.setStatus(303);
        resp.setHeader( "Location",  unsub_url.toString());
        resp.setHeader( "Connection", "close" );

    } else */
    if (caller.equals("event")|| caller.equals("dining")) {
        ...
        if(caller.equals("event")){
            out.print("<a href=\"#\" data-ftjson=\"" + reservationUtil.linkJsonEsc(event_id, activity_id) + "\" class=\"standard_button event_button\" style=\"color:black; background-color:lightblue;\">Continue</a>");
        } else {
            // New Dining A la carte
            out.print("<a href=\"Dining_slot?action=new&date="+timeUtil.getStringDateMMDDYYYY(date)+"\" class=\"standard_button\" style=\"color:black; background-color:lightblue;\">New Dining Reservation</a>");
        }
        ...
        StringBuilder unsub_url = new StringBuilder(base_url);
        unsub_url.append("Unsubscribe");
        if(activity_id == ProcessConstants.DINING_ACTIVITY_ID){
            unsub_url.append("?dining=ft");
        }
        resp.setStatus(303);
        resp.setHeader( "Location",  unsub_url.toString());
        resp.setHeader( "Connection", "close" );
 ...
 public void doGet(HttpServletRequest req, HttpServletResponse resp)
   ...
   if (req.getParameter("help") != null) {
      ...
      //
      if (caller.equalsIgnoreCase("GENID79623") || caller.equalsIgnoreCase("GENMOBILE")) {
...
//   resp.setHeader("P3P","CP=\"ALL DSP COR NID CURa OUR STP PUR\"");       // old version
   ...
   // If an app mode was passed, set it (normally we do thin in verifyMem, but we don't do that here)
   SystemUtils.setAppModeFromUri(req, 0);
   // Get RWD (responsive) state
   boolean rwd = Utilities.getRequestBoolean(req, ProcessConstants.RQA_RWD, false);
   // Store the RWD selected in a cookie
   if(req.getParameter("store_rwd_cookie") != null){
   ...
   if (username.startsWith(LoginCredentials.support)) {
   ...
   if (username.startsWith(LoginCredentials.sales)) {
   ...
   if (req.getParameter("caller") != null) {
       ...
       if (req.getParameter("caller").equals(ProcessConstants.FT_PREMIER_CALLER)) {  // ForeTees Website (Flexscape)
           ...
           Utilities.logError("Login.doPost: Invalid caller (FT_Premier). No longer used. Club=" + club);
           
       } else {
           
           remoteUser(req, out, username, club, resp);   // All others - go process 'remote' user......
       }
       Connect.close(rs,pstmt,con);
       return;
   }

   if (username.startsWith( LoginCredentials.admin )) {
   ...
   if (username.startsWith( LoginCredentials.proshop )) {
 ...
 private void supportUser(HttpServletRequest req, PrintWriter out, String pw, String user, String club) {
   ...
   if ((user.equals(LoginCredentials.support) && pw.equals(LoginCredentials.passwordSup)) || (user.equals(LoginCredentials.supportwill) && pw.equals(LoginCredentials.passwordSupwill)) || (user.equals(LoginCredentials.supportpro) && pw.equals(LoginCredentials.passwordSupPro))) {
 ...
 private void salesUser(HttpServletRequest req, PrintWriter out, String pw, String user, String club) {
   ...
   if (pw.equals( LoginCredentials.passwordSales )) {
 ...
 private void proshopUser(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, String pw, String user, String club) {
   ...
   try {
      con = Connect.getCon(req,club);           // get connection to this club's db
   }
   catch (Exception exc) {
   ...
   if (!user.equalsIgnoreCase("proshop4tea")) {
       ...
       if (construction) {
           Connect.close(con);
           return;
       }       // exit if site under construction
   }
...
//   }
   ...
   boolean upload_pdf = (req.getParameter("upload_pdf") != null && req.getParameter("upload_pdf").equals("1"));
   boolean dining_reports = (req.getParameter("dining_reports") != null && req.getParameter("dining_reports").equals("1"));
   boolean auto_dining = false;

   if ((edit_announce == true || manage_email_content == true || manage_member_prompts == true || send_email == true || upload_pdf == true || dining_reports == true) && 
   ...
   if (auto_dining == true) {      // if this is the dining admin user and dining is configured, go directly to the appropriate page (no frames).
       ...
       if (send_email == true || upload_pdf == true) {
       ...
       } else if (upload_pdf == true) {
          out.println("<meta http-equiv=\"Refresh\" content=\"0; url=Proshop_upload_pdf\">");
       } else if (dining_reports == true) {
          out.println("<meta http-equiv=\"Refresh\" content=\"0; url=Proshop_report_dining\">");
       }
       out.println("</head>");
       ...
       if (edit_announce == true) {
          ...
          out.println("<form method=\"get\" action=\"Proshop_report_dining\">");
       } else {
 ...
 private void memberUser(HttpServletRequest req,
   ...
   PreparedStatement pstmth1 = null;
   ...
   try {
      ...
      if ( allow_member4tea ) {
          ...
          pstmt = con.prepareStatement (
             "SELECT password, name_last, name_first, name_mi, m_ship, m_type, email, count, wc, message, "
                  + "email2, email_bounced, email2_bounced, username, iCal1, iCal2, default_activity_id, "
                  + "mobile_count, mobile_iphone, msub_type, id, allow_mp " +
             "FROM member2b " +
         ...
         if (mobile == 0 && !club.equals("roccdallas")) {                // if NOT a Mobile user
             ...
             pstmt = con.prepareStatement (
                "SELECT password, name_last, name_first, name_mi, m_ship, m_type, email, count, wc, message, "
                     + "email2, email_bounced, email2_bounced, iCal1, iCal2, default_activity_id, "
                     + "mobile_count, mobile_iphone, msub_type, username, id, allow_mp " + 
                "FROM member2b WHERE username = ? AND inact = 0 AND billable = 1");

         } else {                   // Old Mobile user (req was from mlogin.jsp) - check mobile credentials
            
             pstmt = con.prepareStatement (
                "SELECT mobile_pass, name_last, name_first, name_mi, m_ship, m_type, email, count, wc, message, "
                     + "email2, email_bounced, email2_bounced, iCal1, iCal2, default_activity_id, "
                     + "mobile_count, mobile_iphone, msub_type, username, id, allow_mp " +
      ...
      if (rs.next()) {
         ...
         if (pw.equalsIgnoreCase(rs.getString(1)) && !error) {        // password MUST be the first field in query above !!
            ...
            // Get the member data from the resultset
            mship = rs.getString("m_ship");       // Get mship type
            mtype = rs.getString("m_type");       // Get member type
            user = rs.getString("username");
            member_id = rs.getLong("id");
            ...
            //allow_mp = rs.getInt("allow_mp") == 1;
            
            // This is how we can set up test members for clubs who are in staging mode for ForeTees Connect, and want specific members to see the Connect UI.
            if (default_activity_id == 8000) {
                ftConnect = 1;
            }
            ...
            */
            ...
            if (error == false) {
              ...
              // logMsg = "Member Login Successful. UA=[" +req.getHeader("User-Agent")+ "] ";     // use this one when we need to see the browser/OS info
               ...
               session.setAttribute("member_id", member_id);           // save members full name
               session.setAttribute("club", club);           // save club name
               session.setAttribute("caller", "none");       // save caller's name
               session.setAttribute("mship", mship);         // save member's mship type
               session.setAttribute("mtype", mtype);         // save member's mtype
               ...
               //session.setAttribute("allow_mp", allow_mp); // Don't use session to set/check MP access.  Use: User ua = ApiAccess.getUser(req); (Or User ua = User.getMemberUser(member_id, [club_id or club_con])), then use ua.access.manager_portal
               ...
               if (rwd && isMobileClient) {    // if RWD mobile
               ...
               //
               if (mobile == 0) {        // if NOT the old mobile site
                  ...
                  if (enableAdvAssist == false && allowMobile == true && !rwd) {   // if mobile device detected and mobile ok for this site
                     ...
                     Connect.close(rs, pstmth1);
                     Connect.close(rs, stmt2);
                     Connect.close(rs, pstmt, con);
                     return;
                  }
                  ...
                  if ((count == 0 || email.equals( "" ) || email_bounced > 0 || email2_bounced > 0 || iCal1 == -1 || iCal2 == -1) && !club.equals("tcclub")) {
                  ...
                  if (new_skin) {
                      ...
                      out.println("<h2 style=\"padding:10px\">Welcome " + ((count > 1) ? "back " : "") + Utilities.filterDisplayName(name, club) + ".</h2>");
...
*/                   
                  ...
                  out.close();
                    ...
                    //  Mobile Device
                    //
                    promptMobileUser(club, name, session, out, req, con);   // prompt mobile user to see how they want to proceed
 ...
 private void remoteUser(HttpServletRequest req,
                 PrintWriter out, String user, String club, HttpServletResponse resp) {
   ...
   if (fatalError == true) {
    ...
    if (caller.equals( "MYCLUB" ) || (mobileApp == true && !caller.equals("GJONAS74912")) || caller.equals( "CLUBSTERWEB" )) { 
...
//        stripAlpha = 1;   // not sure about this one!!
   ...
   //  GENMOBILE = Generic id for Mobile devices added Apr 2014 so we can block doGet calls
   //
   if (caller.equals( "AMO" ) || caller.equals( "MEMFIRST" ) || caller.equals( "GCWH" ) ||
       ...
       caller.equals( "GENMOBILE" ) || caller.equals("CLUBSTERWEB") || mobileApp) {
      ...
      //
      if ( (caller.equals("MYCLUB") && req.getParameter("activity") == null) || caller.equals( "MFMOBILE" ) || 
            caller.equals( "GENMOBILE" ) || CEmobile || mobileApp) {
      ...
      // Pad out passed username values to ensure they are 4 characters (e.g. A1 -> A001, A12 -> A012, A123 -> A123)
      if (club.equals("indianvalleycc") && caller.toUpperCase().startsWith("CLUBSTER")) {
          username = padUsername(username, "0", 4);
      }
...
//            }
            ...
            */
            ...
            recordLogin(username, "", club, remote_ip, 1);
            ...
            // Utilities.logError("*TEST* Login Check For Mobile UA=" + req.getHeader("User-Agent"));  // for testing different browsers
            ...
            //
            boolean doPromptPrimary = false;
            if (primary.equalsIgnoreCase( "yes" ) && !mNum.equals( "" )) {
                doPromptPrimary = true;
            }
            if (doPromptPrimary && !allowRwd) {
                // Non responsive, we'll prompt primary here, for now.
                // (Responsive primary prompt is down below)
           ...
           // stmt.setString(2, message);       // put in the message displayed (now done in Member_msg!!)
            ...
            if(allowRwd){
                ...
                int tempMobile = mobile;          // save mobile value for promptPrimary
                boolean softForceRwd = false;
                
                if (mobile > 0) {
                    // If mobile requested, and has RWD enabled
                    // use RWD instead of old mobile
                    session.setAttribute("mobile", 0); // RWD does not ever use old mobile
                    mobile = 0;
                    softForceRwd = true; // Skip any prompting for mobile, just do it -- but allow user to switch, if club allows.
                    Utilities.setBitInRequest(req, ProcessConstants.RQA_APPMODE, ProcessConstants.APPMODE_RWD);
                }
    
                Integer passedAppMode = reqUtil.getParameterInteger(req, "set_mode", null);
                
                if(passedAppMode == null){
                    ...
                    boolean rwdCookieResult = false;
                    //if(rwdStateCookie != null && !forceRwd){
                    if(rwdStateCookie != null && !forceRwd && !softForceRwd){  // do not change if user came in on mobile or club is forced to rwd
                        rwdCookieResult = rwdStateCookie.getValue().equals("ON");
                        Utilities.setBitInRequest(req, ProcessConstants.RQA_APPMODE, ProcessConstants.APPMODE_RWD, rwdCookieResult);
                    ...
                    String unsupportedBrowserMessage = uaUtil.getUnsupportedMessage(req);

                    if(unsupportedBrowserMessage != null){
                        // Browser doesn't support RWD
                        promptUnsupported(unsupportedBrowserMessage, default_activity_id, out, req, doPromptPrimary);
                        Connect.close(rs,pstmt,con);
                        return;

                    //} else if(true){
                    } else if(!softForceRwd && !forceRwd && isMobileClient && (!rwdCookieResult || rwdStateCookie == null)){
                        // If we're on a mobile client, and no cookie has been set, or the cookie is set to desktop
                        // Prompt the mobile user where to go
                        // (We will always prompt mobile users if they previously selected Standard/Desktop mode, 
                        //   because we want to encourage them to use RWD.  Non-RWD doesn't work well on touch devices.)
                        //
                        promptRwd(default_activity_id, out, req, doPromptPrimary);
                        Connect.close(rs,pstmt,con);
                        return;
                    } else if (!softForceRwd && !forceRwd && !isMobileClient ){
                        // Non mobile clients will get pushed to standard/desktop mode without prompting
                        Utilities.clearBitInRequest(req, ProcessConstants.RQA_APPMODE, ProcessConstants.APPMODE_RWD);
                        req.setAttribute(ProcessConstants.RQA_RWD, false);
                    }
                } else {
                    req.setAttribute(ProcessConstants.RQA_APPMODE, passedAppMode);
                    boolean is_rwd = Utilities.getBit(passedAppMode, ProcessConstants.APPMODE_RWD);
                    req.setAttribute(ProcessConstants.RQA_RWD, is_rwd);
                    if(is_rwd){
                        // If it's RWD, remember in cookie
                        Utilities.setRwdCookie(resp, req, is_rwd);
                    }
                }
                
                if (doPromptPrimary) {

                   //  mobile is turned off just above because we are in RWD mode - however we need to prompt in mobile mode/format
                   //boolean primaryDone = promptPrimary(mobile, mNum, message, club, new_skin, default_activity_id, out, req, con);
                   boolean primaryDone = promptPrimary(tempMobile, mNum, message, club, new_skin, default_activity_id, out, req, con);
                ...
                //
                //  Check for IE 6 browser ******** TODO: THIS SHOULD BE UPDATED - OLD SKIN IS DEAD SO UNSUPPORTED BROWSERS SHOULD BE WARNED AND LET IN TO NEW SKIN ANYWAYS
                //
                if (new_skin) {
           ...
           // if (oldemail1.equals( "" ) && (!caller.equals( "MEMFIRST" ) || club.equals("mesaverdecc"))) {  // if email not present and NOT MF
            if (oldemail1.equals( "" ) && (rsync == false || club.equals("mesaverdecc") || club.equals("charlottecc") || club.equals("hillwoodcc")) 
                    && !club.equals("tcclub") && (!club.equals("rehobothbeachcc") || count <= 2)){  // if email not present and NOT MF

               autoRefresh = false;                                        // do not refresh
            }

            if ((!emailErr.equals( "" ) || !email2Err.equals( "" ) || iCal1 == -1 || iCal2 == -1 || email_bounced == 1 || email2_bounced == 1) 
                    && eventName.equals("") && !club.equals("tcclub")) {
...
//               if (new_skin) {
               ...
               // Set APPMODE bit so we know we're in seemless mode by url (will change base_url)
               Utilities.setBitInRequest(req, ProcessConstants.RQA_APPMODE, ProcessConstants.APPMODE_SEEMLESS);
               ...
               if(skipMsgs && delay == 0){
                   // If no delay, and no message,
                   // just 303 redirect
                   resp.setStatus(303);
                   resp.setHeader( "Location", refresh_url);
                   resp.setHeader( "Connection", "close" );
                   Connect.close(rs,pstmt,con);
                   return;
               } else {
...
//               }  // end of Old Skin
               ...
               if ((email_bounced == 1 || email2_bounced == 1) && !club.equals("tcclub")) {
...
//
//                    out.println("<form method=\"get\" action=\"" + Utilities.getBaseUrl(req, 0, club) + "Member_announce\">");
//                    out.println("<input type=\"submit\" value=\"Continue to Golf\" class=\"login_button_lg\">");
//                    out.println("</form><br>");
//
//                    out.println("<form method=\"get\" action=\"" + Utilities.getBaseUrl(req, ProcessConstants.DINING_ACTIVITY_ID, club) + "Member_announce\">");
//                    out.println("<input type=\"submit\" value=\"Continue to Dining\" class=\"login_button_lg\">");
//                    out.println("</form><br>");
//                               
               } else {     // emails not bounced

                  if ((rsync == false || club.equals("mesaverdecc") || club.equals("charlottecc") || club.equals("hillwoodcc")) 
                          && !club.equals("tcclub") && (!club.equals("rehobothbeachcc") || count <= 2)) {       // if not a roster sync club
                     ...
                     } else if ((iCal1 == -1 || iCal2 == -1) && !club.equals("tcclub")) {        
...
//                      }
            ...
            recordLogin(username, "", club, remote_ip, 0);
 ...
 */
 ...
 private boolean promptPrimary(int mobile, String mNum, String message, String club, boolean new_skin, int activity_id, 
   ...
   String base_url = Utilities.getBaseUrl(req, temp_activity_id, club);
   /*
     String base_url = "../" + club + "_golf/";
         ...
         while (rs.next()) {
            ...
            if (addit == true) {
               ...
               if (mobile == 0) {
                  ...
                  if(rwd){
                        out.print("<p class=\"primaryUserSelectItem\"><a href=\"" + base_url + "Member_msg?user=" +user+ "&name=" +url_name+ "&message=" + message + eventName_param + "\" class=\"standard_button\" alt=\"" +name+ "\">");
                        out.print(name + "</a></p>");
                  } else {
                  ...
                  out.println("<li><a href=\"" + base_url + "Member_msg?user=" +user+ "&name=" +url_name+ "&message=" +message+ "\" style=\"color:#336633\" alt=\"" +name+ "\">" +name+ ". . . . . . . .</a></li>");
               }
            }
         }
 ...
 private void userHelp(HttpServletRequest req,
   ...
   //
   try {
      con = Connect.getCon(club);          // get a connection
   }
   catch (Exception exc) {
 ...
 public static void countLogin(String type, Connection con) {
    ...
    if (type.equals( "pro" ) || type.equals( "mem" )) {      // skip admin and hotel users
        ...
        if (StatCounters.startDate.equals( "" )) {             // if first login since bounce

            StatCounters.startDate = String.valueOf( new java.util.Date() );   // set new date & time                  
        }
        ...
        if (type.equals( "pro" )) {   
            ...
            StatCounters.loginCountsPro[hour]++;
        }

        if (type.equals( "mem" )) {

            StatCounters.loginCountsMem[hour]++;
        }
    }
...
*/
 ...
 public static String getPOS(Connection con) {
    return ApiCommon.getClubPOS(con);
 }
 ...
 */
 ...
 private void invalidLogin(String errMsg, HttpServletRequest req, PrintWriter out, Connection con, int mobile) {
   ...
   if ( club.equals("newcanaan") && user.startsWith("proshop") ) {
      ...
      out.close();
        ...
        Common_skin.outputHeader(club, 0, "Invalid Login", true, out, req, con);
        ...
        out.println("<div id=\"main_login\">");
        out.println("<h1>Login Rejected</h1>");
        out.println("<div class=\"sub_instructions\">");
        out.println("<p>The login information you submitted was either missing or invalid.</p>");
        out.println("<p>Error: <b>" +errMsg+"</b></p>");
        out.println("</div>");    // end message box                    
        out.println("<input type=\"button\" value=\"Please Try Again\" class=\"login_button_lg\" onclick=\"history.back(1);\">");
        out.println("</form>");
        out.println("<div class=\"clearfloat\"></div>");
        out.println("</div>");    // end wrapper_login 
 ...
 // ***************************************************************************
 //  Prompt use about unsupported browser
 // ***************************************************************************
    private void promptUnsupported(String message, int activity_id, PrintWriter out, HttpServletRequest req, boolean doPromptPrimary) {


        String club = reqUtil.getSessionString(req, "club", "");
        String name = reqUtil.getSessionString(req, "name", "");
        String clubName = Utilities.getClubName(req);
        ...
        String std_base_url = Utilities.getBaseUrl(req, activity_id, club);

        // Responive or standard?
        Common_skin.outputHeader(club, activity_id, "ForeTees Mobile Prompt", true, out, req);
        Common_skin.outputBody(club, activity_id, out, req);
        ...
        if (doPromptPrimary) {
            // Create callback to login, passing some extra details
            out.print("<form method=\"post\" action=\"Login\">");
            Enumeration parmNames = req.getParameterNames();
            // Output any existing parameters
            while (parmNames.hasMoreElements()) {
                String parameterName = (String) parmNames.nextElement();
                out.print("<input type=\"hidden\" name=\""+parameterName+"\" value=\""+reqUtil.getParameterString(req, parameterName, "") +"\">");
            }
            out.print("<input type=\"hidden\" value=\"true\" name=\"did_unsupported_prompt\">");
            out.print("<input type=\"hidden\" value=\"" + app_mode_desktop +"\" name=\"set_mode\">");
            out.println("<input type=\"submit\" value=\"Continue\" class=\"login_button_lg\">");
            out.print("</form>");
        } else {
            out.println("<a class=\"login_button_lg\" href=\"" + std_base_url+ "Member_msg?useStandard\">Continue</a>");
        }
 ...
 // ***************************************************************************
 //  Prompt Mobile user if they want RWD or Standard
 // ***************************************************************************
    private void promptRwd(int activity_id, PrintWriter out, HttpServletRequest req, boolean doPromptPrimary) {


        String club = reqUtil.getSessionString(req, "club", "");
        String name = reqUtil.getSessionString(req, "name", "");
        //String clubName = Utilities.getClubName(req);      // may not have con yet

        int app_mode_current = Utilities.getRequestInteger(req, ProcessConstants.RQA_APPMODE, 0);
        int app_mode_rwd = Utilities.setBit(app_mode_current, ProcessConstants.APPMODE_RWD);
        int app_mode_desktop = Utilities.clearBit(app_mode_current, ProcessConstants.APPMODE_RWD);
        req.setAttribute(ProcessConstants.RQA_APPMODE, app_mode_rwd); // Temporarily set app mode to RWD
        req.setAttribute(ProcessConstants.RQA_RWD, true);

        String rwd_base_url = Utilities.getBaseUrl(req, activity_id, club, app_mode_rwd);
        String std_base_url = Utilities.getBaseUrl(req, activity_id, club, app_mode_desktop);
        ...
        out.println("<div id=\"wrapper_login\">");
        out.println("<div id=\"title\">ForeTees Mobile Prompt</div>");
        //out.println("<div id=\"title\">" + clubName + "</div>");
        ...
        out.println("<h1>Welcome " + name + "</h1>");
        out.println("<div class=\"main_instructions\">");
        out.println("<p>We have detected that you are using a modern touch style device to access ForeTees.</p>");
        out.println("<p>ForeTees' Mobile Mode supports all of the same features as our Standard Desktop Mode, "
                   + "but is easier to use on touch or mobile devices. &nbsp;We recommend using Mobile Mode.</p>");
        out.println("<p>Please select the mode you would like to use:</p>");
        out.println("</div>");    // end message box   
        if (doPromptPrimary) {
            // Create callback to login, passing some extra details
            out.print("<form method=\"post\" action=\"Login\">");
            ...
            out.print("<input type=\"hidden\" value=\"true\" name=\"did_rwd_prompt\">");
            
            out.print("<div class=\"radioBarGroup\">");
            out.print("<label onclick=\";\"><input name=\"set_mode\" type=\"radio\" value=\"" + app_mode_desktop +"\"" + (!Utilities.getBit(app_mode_current, ProcessConstants.APPMODE_RWD) ? " checked" : "") + "/><span>Desktop <span>"+ProcessConstants.LOGIN_DESKTOP_TOOLTIP+"</span></span></label>");
            out.print("<label onclick=\";\"><input name=\"set_mode\" type=\"radio\" value=\"" + app_mode_rwd +"\"" + (Utilities.getBit(app_mode_current, ProcessConstants.APPMODE_RWD) ? " checked" : "") + "/><span>Mobile <span>"+ProcessConstants.LOGIN_MOBILE_TOOLTIP+"</span></span></label>");
            out.print("</div>");
            out.println("<input type=\"submit\" value=\"Continue\" class=\"login_button_lg\">");
            out.print("</form>");
        } else {
            out.print("<div class=\"radioBarGroup\">");
            out.print("<label onclick=\";\"><input name=\"s_m\" type=\"radio\" value=\"" + std_base_url + "Member_msg?useStandard\"" + (!Utilities.getBit(app_mode_current, ProcessConstants.APPMODE_RWD) ? " checked" : "") + "/><span>Desktop <span>"+ProcessConstants.LOGIN_DESKTOP_TOOLTIP+"</span></span></label>");
            out.print("<label onclick=\";\"><input name=\"s_m\" type=\"radio\" value=\"" + rwd_base_url + "Member_msg?useRwd&saveRwd\"" + (Utilities.getBit(app_mode_current, ProcessConstants.APPMODE_RWD) ? " checked" : "") + "/><span>Mobile <span>"+ProcessConstants.LOGIN_MOBILE_TOOLTIP+"</span></span></label>");
            out.print("</div>");
            out.println("<input type=\"button\" value=\"Continue\" class=\"login_button_lg follow_selected\" data-ftselector=\"input[name=s_m]\">");
        }
 ...
 // ***************************************************************************
 public static void promptMobileUser(String club, String name, HttpSession session, PrintWriter out, HttpServletRequest req, Connection con) {
    ...
    if (foretees_mode == 1 && count == 0 && (dining_mode == 0 || (dining_mode == 1 && dining_staging == 1))) {   // if Golf ONLY
        ...
        if (allowRwd == false) {       // if RWD not supported for this club
        ...
        out.println("<BR><BR>");
        if (allowRwd == true) {       // if RWD is supported for this club
            out.println("<strong>NOTE:</strong>&nbsp; You can now access the entire ForeTees site from your mobile device.&nbsp; Simply select the 'Go To Full Mobile Site' option"
                      + " on the mobile menu (next page).<BR><BR>");
        }
        ...
        if (allowRwd == true) {       // if RWD is supported for this club
           ...
           // ?????????????????????????????????????  
           //  Add menu from below here but use responsive styling and change the links to RWD mode!!!!
           ...
           // ???????????????????? CHANGE THIS FOR RWD ?????????????????  
            ...
            out.println("<div id=\"wrapper_login\">");
            out.println("<div id=\"title\">" + Utilities.getClubName(con, true) + "</div>");

            //Common_skin.outputPageStart(club, 0, out, req);
            out.println("<div id=\"main_login\">");
            out.println("<h1>Welcome " + name + "</h1>");
            out.println("<div class=\"main_instructions\">");
            out.println("<p>Some instructions here.</p>");
            out.println("<p>How would you like to proceed?</p>");
            out.println("</div>");    // end message box  
            ...
            if (foretees_mode == 1) {
                  ...
                  out.println("<li><a href=\"Login?mobile=yes&reroute=yes&activity=0\">Golf (Old Mobile Version) . . . . . . . . . . .</a></li>"); 
                  out.println("<li><a href=\"Login?mobile=no&rwd=yes&reroute=yes&activity=0\">Golf (New Mobile Version) . . . . . . . . . . .</a></li>");                
            }

            if (dining_mode == 1 && dining_staging == 0) {   // if Dining and ready for members

                  out.println("<li><a href=\"Login?mobile=no&reroute=yes&rwd=yes&activity=" +dining_activity_id+ "\">Dining . . . . . . . . . . . . . . . . .</a></li>");
            }
            ...
            if (count > 0) {
                  ...
                  for (i=0; i< count; i++) {
                     out.println("<li><a href=\"Login?mobile=no&reroute=yes&rwd=yes&activity=" +activity_ids[i]+ "\">" +activity_names[i]+ " . . . . . . . . . . .</a></li>");                
                  }   
            }

            out.println("</ul></div>");
            
            //out.println("<input type=\"button\" value=\"Continue\" class=\"login_button_lg\">");
            

            out.println("<div class=\"clearfloat\"></div>");
            out.println("</div>");    // end main_login 
            out.println("</div>");    // end wrapper_login 
...
/*        
            ...
            out.println("</ul></div>");
            out.println("<div><p>&nbsp;</p><p>&nbsp;</p></div>");  
            out.println("</body></html>");           
*/
           
        } else {      // old mobile - responsive not enabled
 ...
 private void mobileFlexRoute(HttpServletRequest req, HttpServletResponse resp, PrintWriter out) {
   ...
   boolean goRwd = req.getParameter("rwd") != null;
   ...
   // flash a quick message and route to destination
   if (goRwd) {
        ...
        out.println("<HTML xmlns='http://www.w3.org/1999/xhtml'><HEAD><Title>Member Welcome Page</Title>");
        out.println("<meta http-equiv=\"Refresh\" content=\"0; url=" + Utilities.getBaseUrl(req, activity_id, club, rwdOn) + activityURL + "\">");
        //out.println("<meta name=\"viewport\" id=\"viewport\" content=\"width=device-width, user-scalable=no\">");
        out.println("<LINK REL=StyleSheet HREF=\"/" +rev+ "/mobile/style.css\" TYPE=\"text/css\" MEDIA=screen></HEAD>");
        out.println("<BODY><CENTER>");
        out.println("<BR><BR><div class=\"headertext\">Thank you.  We will now route you to your desired activity.");
        out.println("<BR><BR></div>");
        out.println("<form method=\"get\" action=\"" + Utilities.getBaseUrl(req, activity_id, club, rwdOn) + activityURL + "\">");
        out.println("<input type=\"submit\" value=\"Continue\" style=\"text-decoration:underline; background:#8B8970\">");
        out.println("</form>");
        out.println("</CENTER></BODY></HTML>");
 ...
 private void recordLoginStat(int user_type_id) {
    ...
    try {
        ...
        stmt = con.createStatement();
        stmt.executeUpdate("INSERT INTO login_stats (entry_date, hour, node, user_type_id, login_count) VALUES (now(), DATE_FORMAT(now(), \"%H\"), \"" + Common_Server.SERVER_ID + "\", \"" + user_type_id + "\", 1) ON DUPLICATE KEY UPDATE login_count = login_count + 1");
 ...
 private static void countMobile(int mobile_count, int mobile_iphone, String user, HttpServletRequest req, Connection con) {
   ...
   if (ua.indexOf("iphone") > -1 || ua.indexOf("ipod") > -1) {
	...
	// found an iPad - count as iPhone (Apple)
      iphone = true;   
 ...
 private static String getMessageBoard(int today_date, String club, int activity_id, String last_message, Connection con) {
     ...
     //
     //   ************ NOTICE - WE ARE NOW USING A DIFFERENT MESSAGE TOOL - REFER TO PROSHOP_ANNOUNCEMENTS AND PROSHOP_MAINTOP ************************
     //
     ...
     //
     if (today_date < 20140610 && !last_message.equals("messageboard")) {        // change msg name in Stop form below !!!!!!!!!!!!!!!!!!!!!
         ...
         message = "<style type=\"text/css\">"
                 ...
                 + "<div style=\"width:700px; background-color:#f1f1f1; padding:0px 50px 17px 50px; font-family:Georgia, Times New Roman, Times, serif; color:#333333; font-size:26px; text-align:center;\">"
                 + "<BR><i><strong>NOTICE: &nbsp;NEW MESSAGE BOARD AND NEW FEATURE</strong></i></div>"
                 
                 + "<div style=\"width:700px; background-color:#f1f1f1; padding:0px 50px 0px 50px; font-family:Georgia, Times New Roman, Times, serif; color:#333333; font-size:18px; text-align:left;\">"
                 + "<br>In an attempt to improve our communications with you, our valued users, we have added a message board that will be available after you have logged in. &nbsp;We will post important notices to inform "
                 + "you of new products and features, as well as other important information that will assist you in using our system.<br><br>"
                 + "When we have posted a message or announcement you will notice a mailbox image <img style=\"display:block; margin-left:auto; margin-right:auto;\" src=\"http://dev.foretees.com/v5/images/ForeTees_Msg_sm.png\"> "
                 + "in the upper right area of the ForeTees screen. &nbsp;Just click the image to open the message. "
                 + "&nbsp;The mailbox image will only appear when there is a new message and will remain until the message expires. &nbsp;";
         
          if (activity_id == 0) {
              
             message += "<br><br><strong>NOTE: &nbsp;We have a new feature to announce now</strong>, so you will see the image once you proceed from this page. &nbsp;Please click the mailbox image to read about the new feature.";
          }
          message += "<br><br>"
                 + "This message will expire on 6/09/2014."
                 + "<br><br>"
                 ...
                 + "<input type=hidden name=\"login_message\" value=\"messageboard\">"           //  MESSAGE NAME to STOP IT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     /*
         message = "<style type=\"text/css\">"
                 ...
                 + "<table cellpadding=0 cellspacing=0 border=0 bgcolor=\"#f1f1f1\" width=\"760\"><tr><td>"
                 + "<div style=\"width:700px; background-color:#f1f1f1; padding:0px 50px 17px 50px; font-family:Georgia, Times New Roman, Times, serif; color:#333333; font-size:26px; text-align:center;\">"
                 + "<BR><i><strong>NOTICE: &nbsp;THE NEW FULL-FEATURED MOBILE IS NOW ACTIVE</strong></i></div>"
                 + "<div style=\"width:700px; background-color:#f1f1f1; padding:0px 50px 0px 50px; font-family:Georgia, Times New Roman, Times, serif; color:#333333; font-size:18px; text-align:left;\">"
                 + "<br>If the new full-featured mobile system had not been previously activated for your club, it has now.&nbsp Please review "
                 + "the previous notices below to ensure that your members have access to ForeTees from their mobile devices, and "
                 + "they are aware of its features and functionality.<br><br>We have added several instructional videos to the member 'Help' page "
                 + "on ForeTees.&nbsp; Please direct your members there for assistance.<br><br></div>"
                 + "<HR width=\"75%\"></HR><br><br>"
                 + "<div style=\"width:700px; background-color:#f1f1f1; padding:0px 50px 17px 50px; font-family:Georgia, Times New Roman, Times, serif; color:#333333; font-size:20px; text-align:left;\">"
                 + "<i><strong>Member Information for New Full-Featured Mobile For ForeTees</strong></i></div>"
                 + "<div style=\"width:700px; background-color:#f1f1f1; padding:0px 50px 0px 50px; font-family:Georgia, Times New Roman, Times, serif; color:#333333; font-size:18px; text-align:left;\">"
                 + "In the newsletter referenced below, we informed you that we would follow up with a sample letter that you could use to inform your members of this new Mobile enhancement. "
                 + "&nbsp;The 'Sample Letter' link below will open the letter in a new window. &nbsp;We have also provided a link to the short introduction video referenced in the sample letter for your members.<br><br>"
                 + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"http://www.foretees.com/documents/ForeTees Introduces RWD.pdf\" target=\"_blank\">Sample Letter</a>"
                 + "<br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"https://vimeo.com/user12200989/review/92281843/0ea6deef80\" target=\"_blank\">Introduction Video for Members</a>"
                 + "<br><br><br></div>"
                 + "<div style=\"width:700px; background-color:#f1f1f1; padding:0px 50px 17px 50px; font-family:Georgia, Times New Roman, Times, serif; color:#333333; font-size:20px; text-align:left;\">"
                 + "<i><strong>Previous Message Regarding the Newsletter</strong></i></div>"
                 + "<div style=\"width:700px; background-color:#f1f1f1; padding:0px 50px 0px 50px; font-family:Georgia, Times New Roman, Times, serif; color:#333333; font-size:18px; text-align:left;\">"
                 + "We sent a newsletter on Tuesday (4/15) announcing this great new feature, but this is so important that we want to make sure you see it.&nbsp; The link below will open the "
                 + "newsletter in a new window.&nbsp; Please read the newsletter and forward the address (URL) to others at the club and/or print it. &nbsp;Be sure to read all the Q&As and view the video.<br><br>Let us know if you have any questions.<br><br>"
                 + "</div>"
                 + "<div style=\"width:700px; background-color:#f1f1f1; padding:0px 50px 0px 50px; font-family:Georgia, Times New Roman, Times, serif; color:#333333; font-size:18px; text-align:left;\">"
                 + "<strong>ForeTees New Mobile Solution: </strong>"
                 + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"http://www.foretees.com/index.php/newsletter/\" target=\"_blank\">Click Here For Newsletter</a>"
                 + "<br><br>"
                 + "Thank you for using ForeTees!</span></p>"
                 + "<p style=\"margin-bottom: 0;\"> <span style=\"font-family:Georgia, 'Times New Roman', Times, serif;\">Please "
                 + "contact your sales representative at <a href=\"mailto:sales@foretees.com\">sales@foretees.com</a> if you need assistance.<br><br></span>"
                 + "</div>"
                 + "</td></tr></table>"
                 + "<br><form method=\"post\" action=\"/" + rev + "/servlet/Login\">"
                 
                 + "<input type=hidden name=\"login_message\" value=\"newfeature2\">"           //  MESSAGE NAME to STOP IT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 */
 ...
 /**
  * Pads username out with the specified character to reach a desired minimum length
  * @param username Username value to be padded with extra characters
  * @param padChar Character to be added to usernames to extend their length (e.g. "0")
  * @param length Desired length for username (including any leading letters)
  * @return String - username padded out to the desired length
  */
 private String padUsername(String username, String padChar, int length) {
     
     String result = username;
     
     if (result.length() < length) {
         
         if (username.matches("^[a-zA-Z].*")) {    // Username starts with letter. Padding zeros must come after letter.
             
             while (result.length() < length) {
                 result = result.substring(0,1) + padChar + result.substring(1);
             }
             
         } else {    // Username doesn't start with letter, pad out with leading zeros

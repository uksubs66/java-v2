 ...
 *   Revisions:  
 *
 *       09/22/10   Added support for Activities and fixed the excel output
 *
 ***************************************************************************************
 */
...
public class Proshop_search_guests extends HttpServlet {
 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    try{
        if (excel.equals("yes")) {                // if user requested Excel Spreadsheet Format
            ...
            resp.setHeader("Content-Disposition", "attachment;filename=\"Proshop_search_guests.xls\"");
        } else {
            resp.setContentType("text/html");
        }
    }
   ...
   try {
    ...
    if (!excel.equals("yes")) {     // if normal request
        ...
        out.println("<center><form method=post action=\"Proshop_search_guests\" name=frmRequestExcel id=frmRequestExcel target=\"_blank\">");
    ...
    if (sess_activity_id == 0) {
        ...
        out.println("<tr bgcolor=\"#336633\">");
          ...
          if (club.equals("tpctc")) {
             ...
             out.println("<td><font color=white><b>Trans Mode</b></font></td>");
          }
        out.println("</tr>");
        ...
        try {
            ...
            pstmt = con.prepareStatement("" +
                    "SELECT " +
                        "teepast_id, mm, dd, yy, hr, min, player1, player2, player3, player4, player5, " +
                        "show1, show2, show3, show4, show5, p1cw, p2cw, p3cw, p4cw, p5cw, " +
                        "IFNULL(userg1,''), IFNULL(userg2,''), " +
            ...
            while (rs.next()) {
                ...
                pcwA[0] = rs.getString("p1cw");
                pcwA[1] = rs.getString("p2cw");
                pcwA[2] = rs.getString("p3cw");
                pcwA[3] = rs.getString("p4cw");
                pcwA[4] = rs.getString("p5cw");
                usergA[0] = rs.getString(22);
                usergA[1] = rs.getString(23);
                usergA[2] = rs.getString(24);
                usergA[3] = rs.getString(25);
                usergA[4] = rs.getString(26);
                  ...
                  if (!gtypeA[i].equals("") && !gtypeA[i].equals( null ) && showA[i] == 1) {     // if guest and checked in
                     ...
                     if (rs2.next()) {
                         ...
                         out.println("<td>" +mm+ "/" + SystemUtils.ensureDoubleDigit(dd) + "/" +yy+ "</td>" +
                                  "<td>" + ((hr > 12) ? hr - 12 : hr) + ":" + SystemUtils.ensureDoubleDigit(min) + " " + ((hr > 12) ? "PM" : "AM") + "</td>");
                         if (multi > 0) {
            ...
            // if not excel then display the return button
            if (excel.equals("")) {

                out.println("<center><BR><form method=get action=\"Proshop_search_guests\">");
       ...
       //     SystemUtils.buildDatabaseErrMsg(exc.toString(), "", out, false);
              SystemUtils.buildDatabaseErrMsg("Error in Proshop_search_guests. teepast_id = " +teepast_id, exc.getMessage(), out, false);
            return;
        }
        ...
        out.println("<br><table bgcolor=\"#F5F5DC\" align=center border=1 cellpadding=6>");
        out.println("<tr bgcolor=\"#336633\">");
          out.println("<td><font color=white><b>Date / Time</b></font></td>");
          out.println("<td><font color=white><b>Location</b></font></td>" +
                      "<td><font color=white><b>Event</b></font></td>" +
                      "<td><font color=white><b>Hosting Member</b></font></td>" +
                      "<td><font color=white><b>Guest <font size=2>(from reservation)</font></b></font></td>" +
        ...
        try {
            ...
            pstmt = con.prepareStatement("" +
                    "SELECT " +
                        "asp.player_name, asp.userg, asp.show, t1.activity_id, t1.event_id, t2.activity_name, " +
                        "m.name_first, m.name_last, DATE_FORMAT(t1.date_time, '%c/%e/%Y %h:%i %p') AS date_time " +
                    "FROM activity_sheets_players asp " + 
                    "LEFT OUTER JOIN activity_sheets t1 ON t1.sheet_id = asp.activity_sheet_id " + 
                    "LEFT OUTER JOIN member2b m ON m.username = asp.userg " + 
                    "LEFT OUTER JOIN activities t2 ON t2.activity_id = t1.activity_id " + 
                    "WHERE " +
                        "DATE_FORMAT(t1.date_time, '%Y%m%d') >= ? AND " +
                        "DATE_FORMAT(t1.date_time, '%Y%m%d') <= ? AND " +
                        "player_name LIKE ? " + 
            ...
            while (rs.next()) {
                ...
                if (rs.getString("userg").equals("") && rs.getString("name_last") == null) {

                    // userg is empty and name is null then it's an unaccompanied guest
                    host = "Unaccompanied";

                } else if (!rs.getString("userg").equals("") && rs.getString("name_last") == null) {

                    // userg is NOT empty and name is null then it's an unknown member that is assigned to this guest (member record likely was deleted)
                    host = "Unknown";

                } else {

                    host = rs.getString("name_first") + " " + rs.getString("name_last");
    ...
    // if not excel then close out properly
    if (excel.equals("")) {
 ...
 private void getCustomDate(HttpServletRequest req, PrintWriter out, Connection con) {
   ...
   oldest_dd = oldest_date - ((oldest_yy * 10000) + (oldest_mm * 100));
    ...
    out.println("<form action=\"Proshop_search_guests\" method=\"post\">");

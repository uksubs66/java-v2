 ...
 public void doPost(HttpServletRequest req, HttpServletResponse resp)
   ...
   try {
      ...
      if (req.getParameter("new") != null) {     // if 'new' request
         ...
         //
         id = getNewId(con, event_id, name, course, c_date, c_time);      // allocate a new entry
   ...
   if ((req.getParameter("letter") != null) || (req.getParameter("return") != null) ||
      ...
      //
      String memNotice = verifySlot.checkMemNotice(date, time, time, fb, course, day_name, "event", true, sess_activity_id, locations, con);  // use date of event and actual start time
         ...
         out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
            ...
            out.println("<tr><td valign=\"top\" align=\"center\">");
               ...
               if (club.equals("mediterra") && (sess_activity_id == 1 || sess_activity_id == 2 || sess_activity_id == 3)) {
                   out.println("<b>Notice from your Sports Club</b><br><br><br></font>");
               } else {
   ...
   //
   //  Data for Custom Question - Interlachen uses this for Tee Selection (Men Black, Men Forward, etc)
   //
   //String[] teeSelections = new String[0]; 
   List<String> teeListOptions = new ArrayList<String>();
    
   if (ask_custom1 > 0 && sess_activity_id == 0 && club.equals("interlachen")) {     // custom question for Interlachen - tee selection (white, blue, etc)

       teeListOptions = Utilities.getTeeOptionNames(course, req);
   ...
   boolean hybrid_slot = Utilities.checkProshopHybridSlot(con);
     ...
     if (hybrid_slot) {
         ...
         out.print(Common_skin.getHeader(club, sess_activity_id, "Proshop Tee Slot Page", false, req));
     } else {
   ...
   if (ask_custom1 == 1) out.println(" eval('f.customQuest'+pos+'.selectedIndex = -1');");
      ...
      out.println("<table border=\"0\" cellpadding=\"5\" cellspacing=\"5\" align=\"center\">"); // table to contain 4 tables below
         ...
         out.println("<td align=\"center\" valign=\"top\">");
           ...
           out.println("<br><br></font>");
            ...
            if (ask_custom1 == 1) {
               if (sess_activity_id == 0 && club.equals("interlachen")) {
                  out.println("<td>&nbsp;</td>" +
                              "<td>Tees to Play&nbsp;</td>");
               }
            }
            ...
            */
            ...
            out.println("<input type=\"hidden\" name=\"guest_id5\" value=\"" + guest_id5 + "\">");
               ...
               if (ask_custom1 > 0) {                   
                   if (sess_activity_id == 0 && club.equals("interlachen") && !teeListOptions.isEmpty()) {                      
                      out.println("<td></td><td><select size=\"1\" name=\"customQuest1\" id=\"customQuest1\">");
                      if (customQuest1.equals("") || customQuest1.equals("-select one-")) {            // if tee name not selected yet
                         Common_Config.buildOption("-select one-", "-select one-", "-select one-", out);     // add first option so they have to pick one
                      }
                      for (String teeListOption : teeListOptions) {   
                         Common_Config.buildOption(teeListOption, teeListOption, customQuest1, out);
                      }
                      out.println("</select><td>");
                   }
               }
           ...
           if (size > 1) {
               ...
               if (ask_custom1 > 0) {                   
                   if (sess_activity_id == 0 && club.equals("interlachen") && !teeListOptions.isEmpty()) {                      
                      out.println("<td></td><td><select size=\"1\" name=\"customQuest2\" id=\"customQuest2\">");
                      if (customQuest2.equals("") || customQuest2.equals("-select one-")) {            // if tee name not selected yet
                         Common_Config.buildOption("-select one-", "-select one-", "-select one-", out);     // add first option so they have to pick one
                      }
                      for (String teeListOption : teeListOptions) {   
                         Common_Config.buildOption(teeListOption, teeListOption, customQuest2, out);
                      }
                      out.println("</select><td>");
                   }
               }
           ...
           if (size > 2) {
               ...
               if (ask_custom1 > 0) {                   
                   if (sess_activity_id == 0 && club.equals("interlachen") && !teeListOptions.isEmpty()) {                      
                      out.println("<td></td><td><select size=\"1\" name=\"customQuest3\" id=\"customQuest3\">");
                      if (customQuest3.equals("") || customQuest3.equals("-select one-")) {            // if tee name not selected yet
                         Common_Config.buildOption("-select one-", "-select one-", "-select one-", out);     // add first option so they have to pick one
                      }
                      for (String teeListOption : teeListOptions) {   
                         Common_Config.buildOption(teeListOption, teeListOption, customQuest3, out);
                      }
                      out.println("</select><td>");
                   }
               }
           ...
           if (size > 3) {
               ...
               if (ask_custom1 > 0) {                   
                   if (sess_activity_id == 0 && club.equals("interlachen") && !teeListOptions.isEmpty()) {                      
                      out.println("<td></td><td><select size=\"1\" name=\"customQuest4\" id=\"customQuest4\">");
                      if (customQuest4.equals("") || customQuest4.equals("-select one-")) {            // if tee name not selected yet
                         Common_Config.buildOption("-select one-", "-select one-", "-select one-", out);     // add first option so they have to pick one
                      }
                      for (String teeListOption : teeListOptions) {   
                         Common_Config.buildOption(teeListOption, teeListOption, customQuest4, out);
                      }
                      out.println("</select><td>");
                   }
               }
           ...
           if (size > 4) {
               ...
               if (ask_custom1 > 0) {                   
                   if (sess_activity_id == 0 && club.equals("interlachen") && !teeListOptions.isEmpty()) {                      
                      out.println("<td></td><td><select size=\"1\" name=\"customQuest5\" id=\"customQuest5\">");
                      if (customQuest5.equals("") || customQuest5.equals("-select one-")) {            // if tee name not selected yet
                         Common_Config.buildOption("-select one-", "-select one-", "-select one-", out);     // add first option so they have to pick one
                      }
                      for (String teeListOption : teeListOptions) {   
                         Common_Config.buildOption(teeListOption, teeListOption, customQuest5, out);
                      }
                      out.println("</select><td>");
                   }
               }
...
*/              
         ...
         if (hybrid_slot) {
             ...
             out.print("<td><div class=\"hybridSlotMemberSelect slotMemberSelect\" data-ftparmc=\"" + StringEscapeUtils.escapeHtml(new Gson().toJson(parmc)) + "\"></div><div class=\"hybridSlotMemberOther\">");
...
// private synchronized int getNewId(Connection con, String name, String course, long date, int time) {
 public static int getNewId(Connection con, int event_id, String name, String course, long date, int time) {
    ...
    try {
        ...
        pstmt = con.prepareStatement (
                "INSERT INTO evntsup2b (" +
                    "event_id, name, courseName, c_date, c_time, r_date, r_time, " +
        ...
        pstmt.setLong(6, Utilities.getDate(con));             // date registered
        pstmt.setInt(7, Utilities.getTime(con));              // time registered
 ...
 private void verify(HttpServletRequest req, PrintWriter out, Connection con, HttpSession session, HttpServletResponse resp) {
   ...
   if (req.getParameter("remove") != null) {
      ...
      for (i=0; i < 5; i++) {
          ...
          if (customQuest[i] == null) customQuest[i] = "";
      ...
      // If no players present, reject.  They must enter at least one player, no exceptions.
      if (player1.equals("") && player2.equals("") && player3.equals("") && player4.equals("") && player5.equals("")) {
            ...
            out.println("<BR><BR><H3>Data Entry Error</H3>");
            out.println("<BR><BR>Required field has not been completed, or is invalid.");
            out.println("<BR><BR>At least one player field must contain a valid entry.");
            out.println("<BR><BR>If you wish to remove all players from this registration, please use the <span style=\"font-weight:bold;\">'Cancel Entry'</span> button.");
            out.println("<BR><BR>");
            out.println("<font size=\"2\">");
...
//            return;
      ...
      if ((!player2.equals( "" )) && (g2.equals( "" )) && !invData) {                  // specified but not guest
...
//            return;
      ...
      if ((!player3.equals( "" )) && (g3.equals( "" )) && !invData) {                  // specified but not guest
...
//            return;
      ...
      if ((!player4.equals( "" )) && (g4.equals( "" )) && !invData) {                  // specified but not guest
...
//            return;
      ...
      if ((!player5.equals( "" )) && (g5.equals( "" )) && !invData) {                  // specified but not guest
...
//            return;
      ...
      if (invData) {
            ...
            out.println("<center>");
            out.println("<BR><BR><H3>Data Entry Error</H3>");
            out.println("<BR><BR>Sorry, a name you entered is invalid.");
            out.println("<BR><BR>You entered:&nbsp&nbsp'" + player1 + (!player2.equals("") ? "', '" + player2 + "'" : "") + (!player3.equals("") ? "', '" + player3 + "'" : "") 
                    + (!player4.equals("") ? "', '" + player4 + "'" : "") + (!player5.equals("") ? "', '" + player5 + "'" : "") + ".");
            out.println("<BR><BR>Please check the names and try again.");
            out.println("<BR><BR>");
            out.println("<font size=\"2\">");
      ...
      memA[4] = user5;
       ...
       if (club.equals("rollinghillsgc")) {
           ...
           if (!err_player.equals("")) {
               out.println(SystemUtils.HeadTitle("Access Error"));
               out.println("<body bgcolor=\"#ccccaa\" text=\"#000000\" link=\"#0000FF\" vlink=\"#0000FF\" alink=\"#FF0000\">");
               out.println("<font face=\"Arial, Helvetica, Sans-serif\">");
               out.println("<center>");
               out.println("<BR><BR><H3>Access Error</H3>");
               out.println("<BR><BR>Sorry, but <b>'" + err_player + "'</b> does not have a current USGA handicap in ForeTees.");
            ...
            try {
               ...
               if (guestError == true) {      // if # of guests exceeded
                  ...
                  continueForm(player1, player2, player3, player4, player5, p1cw, p2cw, p3cw, p4cw, p5cw, 
                               ...
                               address1, address2, address3, address4, address5, email1, email2, email3, email4, email5, 
                               shirt1, shirt2, shirt3, shirt4, shirt5, shoe1, shoe2, shoe3, shoe4, shoe5, 
                               customQuest1, customQuest2, customQuest3, customQuest4, customQuest5, otherA1, otherA2, otherA3, 
                               userg1, userg2, userg3, userg4, userg5,
                               guest_id1, guest_id2, guest_id3, guest_id4, guest_id5,
      ...
      if (club.equals("ccnaples")) {
       ...
       if (club.equals("estanciaclub") && (name.contains("Choose Up") || name.contains("Grimm Game"))) {
           ...
           if (teeTimeCount >= teeTimeLimit) {
               ...
               out.println("<BR>Sorry, but <span style=\"font-weight:bold;\">" + parms.player + "</span> is only allowed to have " + teeTimeLimit + " tee times booked at any "
                       + "given time (including <span style=\"font-weight:bold;\">Choose Up</span> or <span style=\"font-weight:bold;\">Grimm Game</span> Events)");
       ...
       if (club.equals("merion")) {
           ...
           if (skip < 5) {
               ...
               int custom_event_category_id_2 = 3;    // "Mens Member Guest Events"
               int custom_event_category_id_3 = 4;    // "Mens Member Guest Events"
...
//                    On June 21st, registration for our September Member-Guest will be open to all members.
      ...
      if (req.getParameter("sp") == null && 
          ...
          extraInfoForm(player1, player2, player3, player4, player5, p1cw, p2cw, p3cw, p4cw, p5cw, 
                        ...
                        ask_shoesize + req_shoesize, ask_custom1 + req_custom1, ask_otherA1 + req_otherA1, ask_otherA2 + req_otherA2, ask_otherA3 + req_otherA3, 
                        otherQ1, otherQ2, otherQ3, who_shirtsize, who_shoesize, who_otherQ1, who_otherQ2, who_otherQ3, players, guests, 
                        userg1, userg2, userg3, userg4, userg5,
                        guest_id1, guest_id2, guest_id3, guest_id4, guest_id5,
                        notes, hides, course, name, suppressEmails, 7, club, rev, signup_id, event_id, out, con); // using highest skip value prior to this check
      ...
      if (req_otherA1 == 1 && skip < 14) {
          ...
          // Check to make sure any players that are present have an answer for Other specified
          i = 0;
          loop1:
          while (i < 5) {
      ...
      if (req_otherA2 == 1 && skip < 15) {
          ...
          // Check to make sure any guests that are present have an answer for Other2 specified
          i = 0;
          loop1:
          while (i < 5) {
      ...
      if (req_otherA3 == 1 && skip < 16) {
          ...
          // Check to make sure any guests that are present have an answer for Other3 specified
          i = 0;
          loop1:
          while (i < 5) {
      ...
      // If other A3 is required for this event then test for it
      //
      if (sess_activity_id == 0 && club.equals("interlachen") && req_custom1 == 1 && skip < 17) {
          
          // Check to make sure any players that are present have an answer for the Custom Question specified
          i = 0;
          loop1:
          while (i < 5) {
              ...
              if (!player[i].equals( "" )) {    // if player present
                 ...
                 if (customQuest[i].equals("") || customQuest[i].equals("-select one-")) {   // if custom answer not specified
                    ...
                    out.println("<BR><BR><b>" + player[i] + "</b> is missing a Tee Selection.");
                    //out.println("<BR><BR>All participants are required to specify an answer to Question #3.");
                    out.println("<BR><BR>Would you like to override this and allow the event registration?");
                    out.println("<BR><BR>");
                    out.println("<font size=\"2\">");
                    ...
                    continueForm(player1, player2, player3, player4, player5, p1cw, p2cw, p3cw, p4cw, p5cw, 
                                 ...
                                 userg1, userg2, userg3, userg4, userg5,
                                 guest_id1, guest_id2, guest_id3, guest_id4, guest_id5,
                                 notes, hides, course, name, suppressEmails, 17, true, season, rev, signup_id, event_id, out);
   ...
   try {
      ...
      pstmt6.setString(89, customQuest1);
      pstmt6.setString(90, customQuest2);
      pstmt6.setString(91, customQuest3);
      pstmt6.setString(92, customQuest4);
      pstmt6.setString(93, customQuest5);
      
      pstmt6.setInt(94, signup_id);
 ...
 private static void extraInfoForm(
                        ...
                        int ask_homeclub, int ask_phone, int ask_address, int ask_email, int ask_shirtsize, int ask_shoesize, int ask_custom1, int ask_otherA1, int ask_otherA2, int ask_otherA3, 
                        ...
                        int guest_id1, int guest_id2, int guest_id3, int guest_id4, int guest_id5,
                        String notes, String hides, String course, String name, String suppressEmails, 
                        int skip, String club, String rev, int signup_id, int event_id, PrintWriter out, Connection con) {

 ...
 *   last updated:
 *
 *        5/27/2014  Login counters were not working because we passed the wrong user (flexid instead of username).
 *        5/22/2014  Move login counters back to outside the 'isCreate' check - wasn't catching any.
 *        5/21/2014  Move the login counter to after we detect that a new session is needed, and 
 *                   add a counter for mobile devices.
 ...
 * 
 * 
 *          http://dev.foretees.com/v5/servlet/Common_webapi?type=caldata&club_id=229&user=1719&activity_id=9999
 *
 ***************************************************************************************
 */
...
import com.foretees.api.reportItems.CalendarEntry;
...
public class Common_webapi extends HttpServlet {
    ...
    final static String SESSION_ID = LoginCredentials.id;
    final static String STATUS_WAITLIST = "Wait List";
    final static String STATUS_REGISTERED = "Registered";
    ...
    throws ServletException, IOException {
        ...
        if (!is_error) {
            ...
            if (type.equalsIgnoreCase("SSO")) { //  && req.getMethod().equals("POST")
                ...
                result = encodeMap(performSSO(req, resp, activity_id, user, username, member_id, club, ProcessConstants.FT_PREMIER_CALLER, mship, mtype, msubtype, name, wc, remote_ip, benchmark_start, con));
            }
...
*/
    ...
    private static List<String> decryptUID (Long keyTime, String base64_iv, String uid) {
        ...
        return VerifyUser.decryptUID(null, null, null, keyTime, base64_iv, uid);
    }
    
    
    private static List<String> decryptUID (HttpServletRequest req, String clubid, String user, Long keyTime, String base64_iv, String uid) {
        
        return VerifyUser.decryptUID(req, clubid, user, keyTime, base64_iv, uid);

    }
    
    public static Map<String, Object> loadUser(HttpServletRequest req, String type, String clubid, String user){
        
        return VerifyUser.loadUser(req, type, clubid, user);
...
*/
 ...
 private final static String cleanRecord4( String s ) {
    ...
    public static Map<String,Object> performSSO(HttpServletRequest req, HttpServletResponse resp, int activity_id, final String user, final String username, final long member_id, final String club, String caller, final String mship, final String mtype, final String msubtype, final String name, final String wc, final String remote_ip, final long benchmark_start, Connection con)
        ...
        return VerifyUser.performSSO(req, resp, activity_id, user, username, member_id, club, caller, mship, mtype, msubtype, name, wc, remote_ip, benchmark_start, con);

    }



    private Map<String, Object> getActivitities(HttpServletRequest req, HttpServletResponse resp, int activity_id, final String user, final String username, final String club, String caller, String mship, String mtype, final long benchmark_start, Connection con)
            throws ServletException, IOException {
    ...
    public static Map<String, Object> getCalendarData(HttpServletRequest req, HttpServletResponse resp, int activity_id, final String user, String username, final String club, final String caller, String mship, String mtype, final long benchmark_start, Connection con)
            throws ServletException, IOException {
        return getCalendarData(req, resp, activity_id, user, username, club, caller, mship, mtype, benchmark_start, null, con);
    }

    public static Map<String, Object> getCalendarData(HttpServletRequest req, HttpServletResponse resp, int activity_id, final String user, String username, final String club, final String caller, String mship, String mtype, final long benchmark_start, ApiGetCommand cmd, Connection con)
            throws ServletException, IOException {
        ...
        boolean alternate_model = cmd != null;
        boolean local_urls = cmd != null;
        
        boolean eventsOnly = false;
        boolean clubCalendar = false;
        
        Integer gender_filter = null;
        
        
        List<String> stringActivityList = null;
        ...
        if(caller.equals(ProcessConstants.FT_PREMIER_CALLER)){
            ...
            app_mode = Utilities.setBit(app_mode,ProcessConstants.APPMODE_RWD);
            app_mode = Utilities.setBit(app_mode,ProcessConstants.APPMODE_BLOCK_RWD_SWITCH);
        }

        if(cmd != null){
            eventsOnly = cmd.events_only != null;
            clubCalendar = cmd.club_cal != null;
            if(cmd.req_year != null){
                req_year = cmd.req_year;
            }
            if(cmd.req_year != null){
                req_month = cmd.req_month;
            }
            if(cmd.date_start != null){
                req_sdate = timeUtil.getClubDateTimeFromDb(req, cmd.date_start)[timeUtil.DATE];
            }
            if(cmd.date_end != null){
                req_edate = timeUtil.getClubDateTimeFromDb(req, cmd.date_end)[timeUtil.DATE];
            }
            if(cmd.string_list != null){
                stringActivityList = cmd.string_list;
            }
            if(cmd.activity_id != null){
                activity_id = cmd.activity_id;
            }
            if(cmd.member_id != null){
                ClubMember mbr = new ClubMember(cmd.member_id, con);
                if(mbr.last_error == null){
                    username = mbr.username;
                    mtype = mbr.m_type;
                    mship = mbr.m_ship;
                }
            } else {
                username = "no_user_specified"; // A user name that won't exist
                eventsOnly = true; // if no username was passed, only load events
            }
            
        } else if(req != null) {
            String gender_filter_type = reqUtil.getParameterString(req, "gender", "").toLowerCase();
            if(gender_filter_type.equals("m")){
                gender_filter = 3; // If filtering male, we set the filter to 3 (female) to exclude female (all, but female)
            } else if(gender_filter_type.equals("f")){
                gender_filter = 2; // If filtering female, we set the filter to 2 (male) to exclude male (all, but male)
            }
            try {
            ...
            if (req.getParameterValues("activity_id") != null) {
                stringActivityList = Arrays.asList(req.getParameterValues("activity_id"));
            }
            eventsOnly = req.getParameter("events_only") != null;
            clubCalendar = req.getParameter("clubcal") != null;
            if(req.getParameter("list_mode") != null){
                // Force requesting alternate response model
        ...
        String dining_base = dining_base = local_urls?Utilities.getBaseUrl(req, ProcessConstants.DINING_ACTIVITY_ID, club, app_mode):Utilities.getExternalBaseUrl(req, ProcessConstants.DINING_ACTIVITY_ID, club, app_mode);
        String golf_base = golf_base = local_urls?Utilities.getBaseUrl(req, 0, club, app_mode):Utilities.getExternalBaseUrl(req, 0, club, app_mode);
        String flxrez_base = flxrez_base = local_urls?Utilities.getBaseUrl(req, activity_id, club, app_mode):Utilities.getExternalBaseUrl(req, activity_id, club, app_mode); // what about 999?
        ...
        if (stringActivityList != null) {
            //String[] tmpActivityList = req.getParameterValues("activity_id");
            activityList = new int[stringActivityList.size()];
            for (i = 0; i < stringActivityList.size(); i++) {
                try {
                    activityList[i] = Integer.parseInt(stringActivityList.get(i));
        ...
        sysLingo.setLingo("Lesson Book", club, activity_id);
...
*/
        ...
        List<Integer[]> month_dates = new ArrayList<Integer[]>();
        if(alternate_model && custom_dates){
            months = timeUtil.partialMonthsBetween(req_sdate, req_edate) + 1;
            for(i2 = 0; i2 < months; i2++){
                DateTime dt_s = timeUtil.getClubJodaDateTime(req, req_sdate).plusMonths(i2);
                int i_sdate = 0, i_edate = 0;
                if(i2>0){
                    i_sdate = timeUtil.buildIntDate(dt_s.getYear(), dt_s.getMonthOfYear(), 1);
                } else {
                    i_sdate = timeUtil.buildIntDate(dt_s);
                }
                if(i2+1 == months){
                    i_edate = req_edate;
                } else {
                    i_edate = timeUtil.buildIntDate(dt_s.getYear(), dt_s.getMonthOfYear(), dt_s.dayOfMonth().getMaximumValue());
                }
                month_dates.add(new Integer[]{i_sdate, i_edate});
            }
        }
        ...
        if (clubCalendar) {
            container_map.put("foreTeesClubCalendarData", response_map);
        } else {
        ...
        for (i2 = 0; i2 < months; i2++) {                 // do each month
            ...
            if (custom_dates) {
                ...
                if(!month_dates.isEmpty()){
                    month_da = month_dates.get(i2);
                    sdate = month_da[0];
                    edate = month_da[1];
                    req_sdate = month_da[0];
                    req_edate = month_da[1];
                    DateTime dt_s = timeUtil.getClubJodaDateTime(req, (int)sdate);
                    DateTime dt_e = timeUtil.getClubJodaDateTime(req, (int)edate);
                    days_in_month = dt_s.dayOfMonth().getMaximumValue();
                    day = dt_s.getDayOfMonth();
                    month = dt_s.getMonthOfYear();
                    year = dt_s.getYear();
                    numDays = Days.daysBetween(dt_s, dt_e).getDays();
                    //dbg.put("dbg1startDay",day);
                    //dbg.put("dbg1sdate",sdate);
                    //dbg.put("dbg1edate",edate);
                } else {
            ...
            //
            if (!clubCalendar && !eventsOnly && parm.foretees_mode == 1 && (activity_id == 0 || activity_id == ProcessConstants.ALL_ACTIVITIES)) {
            ...
            //
            if (!clubCalendar && !eventsOnly && activity_id != ProcessConstants.DINING_ACTIVITY_ID) {
            ...
            if (activity_id != ProcessConstants.DINING_ACTIVITY_ID) {
                ...
                try {
                    ...
                    if(activity_id != ProcessConstants.ALL_ACTIVITIES){
                    ...
                    if (!club.equals("tcclub")) {
                        sql.append(" AND gstOnly = 0 ");
                    }
                    
                    int p = 1;
                    pstmt1 = con.prepareStatement(sql.toString());
                    pstmt1.setLong(p++, sdate);
                    pstmt1.setLong(p++, edate);
                    if(activity_id != ProcessConstants.ALL_ACTIVITIES){
                        pstmt1.setInt(p++, activity_id);
                    }
                    if(gender_filter != null){
                        pstmt1.setInt(p++, gender_filter);
                    }
            ...
            if (!clubCalendar && !eventsOnly && parm.foretees_mode == 1 && (activity_id == 0 || activity_id == ProcessConstants.ALL_ACTIVITIES)) {
                ...
                try {
                    ...
                    pstmt1 = con.prepareStatement(
                            ...
                            + "WHERE wls.date BETWEEN ? AND ? "
                            + "  AND wlp.username = ? "
                            + "  AND converted = 0 "
                            + "ORDER BY wls.date");

                    pstmt1.clearParameters();
                    pstmt1.setString(1, timeUtil.getDbDateTime((int)sdate, 0));
                    pstmt1.setString(2, timeUtil.getDbDateTime((int)edate, 2359));
                    pstmt1.setString(3, username);
            ...
            if (!clubCalendar && !eventsOnly && parm.genrez_mode != 0 && (flxRezAct_selected || activity_id == ProcessConstants.ALL_ACTIVITIES)) {      // if any activites defined for this club
                ...
                try {
                    ...
                    pstmt1 = con.prepareStatement(
                            ...
                            + "  AND ap.username = ? " + (selected_act_id != ProcessConstants.ALL_ACTIVITIES ? ""
                            + "  AND a.activity_id IN (" + selected_act_children + ") " : "")
            ...
            while ( day <= ((custom_dates) ? 31 : numDays) ) {
                ...
                boolean skip_this_day = false;
                
                if (alternate_model) {
                    DateTime testdt = timeUtil.getClubJodaDateTime(req, year, month, 1, 0, 0, 0);
                    if(day > testdt.dayOfMonth().getMaximumValue()){
                        skip_this_day = true;
                    }
                }

                // skip all past days if processing the current month
                if (!skip_this_day && ((month == today_month && day >= today) || month != today_month)) {
                ...
                if (alternate_model) {
                    calendar_day.put("dayOfMonth", day);
                } else {
                ...
                if(alternate_model){
                    calendar_day.put("entries", entries);
                }
                
                //calendar_day.put("debug_monthdates", month_dates);
                //calendar_day.put("debug_months", months);
                //calendar_day.put("debug_month", month);
                ...
                if (!clubCalendar && max > 0 && activity_id != ProcessConstants.ALL_ACTIVITIES && (activity_id == 0 || activity_id == ProcessConstants.DINING_ACTIVITY_ID || !getActivity.isChildlessActivity(activity_id, con))) {
                    ...
                    if (activity_id == 0) {          // if Golf
                        ...
                        calendar_day.put("sheetLink", dining_base + "Dining_slot?action=new&date=" + date);
                ...
                if (IS_TLT) {
                    ...
                    try {
                        ...
                        pstmt1 = con.prepareStatement(
                                ...
                                + "AND np.username = ? "
                                + "AND req_datetime = BETWEEN ? AND ? ");

                        pstmt1.clearParameters();
                        pstmt1.setString(1, username);
                        pstmt1.setString(2, timeUtil.getDbDateTime((int)date, 0));
                        pstmt1.setString(3, timeUtil.getDbDateTime((int)date, 2359));
                        ...
                        while (rs.next()) {
                            ...
                            if (alternate_model) {
                                CalendarEntry cen = new CalendarEntry();
                                cen.label = buildItemDescription("Golf", rs.getString("pretty_time"));
                                cen.activity_id = 0;
                                cen.sub_category = "teetime";
                                cen.time = rs.getString("pretty_time");
                                cen.css_class = "tee_list_tlt_time";
                                cen.data = data_map;
                                entries.add(cen);
                            } else {
                                Map<String, String> calendar_entry = new HashMap<String, String>();
                                calendar_entry.put("activityId", "0");
                                calendar_entry.put("subCategory", "teetime"); // was going to be notification but shouldn't really matter
                                calendar_entry.put("time", rs.getString("pretty_time"));
                        ...
                        try {
                            ...
                            while (rs.next()) {
                                ...
                                if (!ename.equals("") && etype == 1) {
                                    ...
                                    if (alternate_model) {
                                        CalendarEntry cen = new CalendarEntry();
                                        cen.label = buildItemDescription("Shotgun Start", stime2);
                                        cen.activity_id = 0;
                                        cen.sub_category = "shotgun";
                                        cen.registered = true;
                                        cen.time = stime2;
                                        cen.css_class = "shotgun";
                                        entries.add(cen);
                                    } else {
                                        Map<String, String> calendar_entry = new HashMap<String, String>();
                                        calendar_entry.put("activityId", "0");
                                        calendar_entry.put("subCategory", "shotgun");
                                        calendar_entry.put("registered", "true");
                                        calendar_entry.put("time", stime2);
                                    ...
                                    if (!restricted && !cutoff && !(date == todayDate && time <= ctime) && (activity_id == 0 || activity_id == ProcessConstants.ALL_ACTIVITIES)) {
                                        ...
                                        if (alternate_model) {
                                            CalendarEntry cen = new CalendarEntry();
                                            cen.label = buildItemDescription("Tee Time", stime);
                                            cen.activity_id = 0;
                                            cen.sub_category = "teetime";
                                            cen.registered = true;
                                            cen.time = stime;
                                            cen.css_class = "tee_list_color_4";
                                            cen.data = data_map;
                                            entries.add(cen);
                                        } else {
                                            Map<String, String> calendar_entry = new HashMap<String, String>();
                                            calendar_entry.put("activityId", "0");
                                            calendar_entry.put("subCategory", "teetime");
                                            calendar_entry.put("registered", "true");
                                            calendar_entry.put("time", stime);
            ...
            } // end if not club calednar

            
                //*******************************************************************************
                //  Check for any lotteries for this day (all will be zero if not supported or if here for events only or club calendar)
                //*******************************************************************************
                //
                            ...
                            if (lstate == 2) {       // if still ok to process lottery requests
                                ...
                                */
                                ...
                                if (alternate_model) {
                                    ...
                                    cen.label = item_name;
                                    cen.activity_id = 0;
                                    cen.sub_category = "lottery";
                                    cen.registered = true;
                                    cen.time = stime2;
                                    cen.css_class = "tee_list_color_6";
                                    cen.data = data_map;
                                    entries.add(cen);
                                } else {
                                    Map<String, String> calendar_entry = new HashMap<String, String>();
                                    calendar_entry.put("activityId", "0"); // all lotteries are golf
                                    calendar_entry.put("subCategory", "lottery");
                                    calendar_entry.put("time", stime);
                                    calendar_entry.put("registered", "true");
...
//               if (rs.next()) {               // just get the first one as each lesson can have multiple entries
                        while (rs.next()) {              // get all lesson times that have a num greater than zero (not a subsequent time)
                            ...
                            if (alternate_model) {
                                CalendarEntry cen = new CalendarEntry();
                                cen.label = item_name;
                                cen.activity_id = returned_activity_id;
                                cen.sub_category = "lesson";
                                cen.registered = true;
                                cen.time = stime2 + ampm;
                                cen.css_class = "tee_list_color_5";
                                cen.data = data_map;
                                entries.add(cen);
                            } else {
                                Map<String, String> calendar_entry = new HashMap<String, String>();
                                calendar_entry.put("activityId", String.valueOf(returned_activity_id));
                                calendar_entry.put("subCategory", "lesson");
                                calendar_entry.put("registered", "true");
                                calendar_entry.put("time", stime2 + ampm);
                                calendar_entry.put("displayText", item_name);
                                calendar_entry.put("data-ftjson", gson_obj.toJson(data_map));
...
*/
                            ...
                            if (returned_activity_id == activity_id) {
                                ...
                                if (alternate_model) {
                                    ...
                                    Map<String, String> calendar_entry = new HashMap<String, String>();
                                    calendar_entry.put("activityId", String.valueOf(returned_activity_id));
                                    calendar_entry.put("subCategory", "lessongrp");
                                    calendar_entry.put("registered", "true");
                                    calendar_entry.put("time", stime2 + ampm);
                                    calendar_entry.put("displayText", item_name);
                                    calendar_entry.put("data-ftjson", gson_obj.toJson(data_map));
                ...
                if (eventA[day] == 1) {        // if any events  exist for this day
                    ...
                    try {
                        ...
                        sql.append(""
                                + "SELECT "
                                + "    name, coursename, signup, activity_id, event_id, act_hr, act_min, type, gender "
                                + "  FROM events2b "
                                + "  WHERE "
                                + "    date = ? "
                                + "    AND inactive = 0 ");
                        ...
                        sql.append(" ORDER BY act_hr, act_min ");

                        pstmt1 = con.prepareStatement(sql.toString());
                        pstmt1.clearParameters();
                        int p = 1;
                        pstmt1.setLong(p++, date);
                        if (!allAct) {
                            pstmt1.setInt(p++, selected_act_id);
                        }
                        if (gender_filter != null) {
                            pstmt1.setInt(p++, gender_filter);
                        }
                        ...
                        while (rs.next()) {
                            ...
                            if (!clubCalendar || signUp != 0) {           // if members can signup
                                ...
                                if (signedup == true) {     // if member is registered
                                    ...
                                    if (returned_activity_id == activity_id || returned_activity_id == selected_act_id || selected_act_id == ProcessConstants.ALL_ACTIVITIES) {
                                        ...
                                        if (alternate_model) {
                                            CalendarEntry cen = new CalendarEntry();
                                            cen.label = buildItemDescription(ename + (selected_act_id == ProcessConstants.ALL_ACTIVITIES ? " - (" + activity_name + ")" : "") + " - " + ((wait == 0) ? STATUS_REGISTERED : STATUS_WAITLIST), stime);
                                            cen.activity_id = returned_activity_id;
                                            cen.sub_category = "event";
                                            cen.waitlisted = wait != 0;
                                            cen.registered = true;
                                            cen.time = stime;
                                            cen.css_class = "tee_list_color_1";
                                            cen.gender = ProcessConstants.getEventGender(event_gender);
                                            cen.data = data_map;
                                            entries.add(cen);
                                        } else {
                                            Map<String, String> calendar_entry = new HashMap<String, String>();
                                            calendar_entry.put("activityId", String.valueOf(returned_activity_id));
                                            calendar_entry.put("subCategory", "event");
                                            calendar_entry.put("registered", "true");
                                            calendar_entry.put("waitlisted", String.valueOf(wait != 0));
                                            calendar_entry.put("time", stime);
                                            calendar_entry.put("displayText", buildItemDescription(ename + (selected_act_id == ProcessConstants.ALL_ACTIVITIES ? " - (" + activity_name + ")" : "") + " - " + ((wait == 0) ? STATUS_REGISTERED : STATUS_WAITLIST), stime));
                                            calendar_entry.put("cssClass", "tee_list_color_1");
                                            calendar_entry.put("data-ftjson", gson_obj.toJson(data_map));
                                            calendar_entry.put("gender", ProcessConstants.getEventGender(event_gender));
                                            calendar_day.put("entry_" + entry, calendar_entry);
                                        }
...
*/
                                    if (signedup == false) {     // if dependents not registered either - add link to event info
                                        ...
                                        if (returned_activity_id == activity_id || returned_activity_id == selected_act_id || selected_act_id == ProcessConstants.ALL_ACTIVITIES) {
                                            ...
                                            if (alternate_model) {
                                                CalendarEntry cen = new CalendarEntry();
                                                cen.label =  buildItemDescription(ename + (selected_act_id == ProcessConstants.ALL_ACTIVITIES ? " - (" + activity_name + ")" : ""), stime);
                                                cen.activity_id = returned_activity_id;
                                                cen.sub_category = "event";
                                                cen.waitlisted = false;
                                                cen.registered = false;
                                                cen.time = stime;
                                                cen.css_class = "tee_list_color_2";
                                                cen.gender = ProcessConstants.getEventGender(event_gender);
                                                cen.data = data_map;
                                                entries.add(cen);
                                            } else {
                                ...
                                if (allAct || returned_activity_id == activity_id) {
                                    ...
                                    if (alternate_model) {
                                        ...
                                        cen.time = stime;
                                        cen.css_class = "tee_list_color_3";
                                        cen.data = data_map;
                                        entries.add(cen);
                                    } else {
                                        Map<String, String> calendar_entry = new HashMap<String, String>();
                    ...
                    try {
                        ...
                        pstmt1 = con.prepareStatement(
                                "SELECT wl.wait_list_id, wl.course, wls.date, wls.ok_stime, wls.ok_etime, wls.wait_list_signup_id, wl.color, " +
                                    ...
                                    "DATE_FORMAT(wls.ok_stime, '%l:%i %p') AS pretty_stime" +
                                "FROM wait_list_signups wls " +
                        ...
                        if (rs.next()) {
                            ...
                            if (activity_id == 0) {
                                ...
                                if (alternate_model) {
                                    CalendarEntry cen = new CalendarEntry();
                                    cen.label = buildItemDescription("Wait List Sign-up", rs.getString("pretty_stime"));
                                    cen.activity_id = 0;
                                    cen.sub_category = "waitlist";
                                    cen.registered = true;
                                    cen.time = rs.getString("pretty_stime");
                                    cen.css_class = "tee_list_color_7";
                                    cen.data = data_map;
                                    entries.add(cen);
                                } else {
                                    Map<String, String> calendar_entry = new HashMap<String, String>();
                                    calendar_entry.put("activityId", "0"); // wait lists are all golf
                                    calendar_entry.put("subCategory", "waitlist");
                                    calendar_entry.put("registered", "true");
                                    calendar_entry.put("time", rs.getString("pretty_stime"));
                                    calendar_entry.put("displayText", buildItemDescription("Wait List Sign-up", rs.getString("pretty_stime")));
                ...
                if (!clubCalendar && parm.genrez_mode != 0 && activity_id != 0 && activity_id != ProcessConstants.DINING_ACTIVITY_ID) {
                    ...
                    if (activitiesA[day] != 0) {
                        ...
                        try {
                            ...
                            pstmt1.clearParameters();
                            pstmt1.setString(1, timeUtil.getDbDateTime((int)date, 0));
                            pstmt1.setString(2, timeUtil.getDbDateTime((int)date, 2359));
                            pstmt1.setString(3, username);
                            rs = pstmt1.executeQuery();
                            ...
                            while (rs.next()) {
                                ...
                                if (root_id == activity_id || activity_id == 999) {
                                    ...
                                    if (alternate_model) {
                                        CalendarEntry cen = new CalendarEntry();
                                        cen.label = item_name;
                                        cen.activity_id = root_id;
                                        cen.sub_category = "reservation";
                                        cen.registered = true;
                                        cen.time = rs.getString("pretty_time");
                                        cen.css_class = "tee_list_color_6";
                                        cen.data = data_map;
                                        ...
                                        calendar_entry.put("subCategory", "reservation");
                                        calendar_entry.put("registered", "true");
                                        calendar_entry.put("time", rs.getString("pretty_time"));
                                        calendar_entry.put("displayText", item_name);
                                        calendar_entry.put("cssClass", "tee_list_color_6"); // is this right?
                                        calendar_entry.put("data-ftjson", gson_obj.toJson(data_map));
                                        calendar_day.put("entry_" + entry, calendar_entry);
                                    }
                            ...
                            Utilities.logError("Common_webapi.getCalendarData() Error loading activity times: " + club + ", Day=" + day + ", username: " + username + ", Err=" + org.apache.commons.lang.exception.ExceptionUtils.getStackTrace(e));
                    ...
                    try {
                        ...
                        if (con_d != null) {
                            ...
                            while (rs.next()) {
                                ...
                                //state = ""; // reset (not used here?)
                                dtime = Integer.parseInt(rs.getString("stime")); // first set to the event start time

                                if (!clubCalendar) { // skip checking for reservation if clubcal
                                    ...
                                    if (rs2.next()) {
                                        ...
                                        waitlisted = rs2.getString("state").equals("waitlisted");
                                        dtime = Integer.parseInt(rs2.getString("stime")); // override dtime to time of actual reservation if found

                                    }
                                } // end if clubCalendar
                                
                                item_name = buildItemDescription(rs.getString("name") + ((waitlisted) ? " (" + STATUS_WAITLIST + ")" : ((signedup) ? " (" + STATUS_REGISTERED + ")" : "")), Utilities.getSimpleTime(dtime));

                                Map<String, Object> data_map = new LinkedHashMap<String, Object>();
                                data_map.put("url", "Member_events2");
                                data_map.put("base_url", dining_base);
                                data_map.put("data", reservationUtil.linkData(rs.getInt("id"), ProcessConstants.DINING_ACTIVITY_ID));
                                ...
                                if (alternate_model) {
                                    CalendarEntry cen = new CalendarEntry();
                                    cen.label = item_name;
                                    cen.activity_id = ProcessConstants.DINING_ACTIVITY_ID;
                                    cen.sub_category = "event";
                                    if (!clubCalendar) {
                                        cen.registered = signedup;
                                        cen.waitlisted = waitlisted;
                                    }
                                    cen.time = Utilities.getSimpleTime(dtime);
                                    cen.css_class = "tee_list_color_10";
                                    cen.gender = ProcessConstants.getEventGender(ProcessConstants.EVENT_GENDER_UNCATOREGIZED);
                                    cen.data = data_map;
                                    entries.add(cen);
                                } else {
                                    Map<String, String> calendar_entry = new HashMap<String, String>();
                                    ...
                                    if (!clubCalendar) {
                                        calendar_entry.put("registered", signedup ? "true" : "false");
                                        calendar_entry.put("waitlisted", waitlisted ? "true" : "false");
                                    }
                                    calendar_entry.put("time", Utilities.getSimpleTime(dtime));
                                    calendar_entry.put("displayText", item_name);
                                    calendar_entry.put("cssClass", "tee_list_color_10");
                                    calendar_entry.put("data-ftjson", gson_obj.toJson(data_map));
                                    calendar_entry.put("gender", ProcessConstants.getEventGender(ProcessConstants.EVENT_GENDER_UNCATOREGIZED));
                                    calendar_day.put("entry_" + entry, calendar_entry);
                                }
                                entry++;
                ...
                //
                if (!clubCalendar && organization_id != 0 && diningR[day] != 0 && (selected_act_id == ProcessConstants.ALL_ACTIVITIES || selected_act_id == ProcessConstants.DINING_ACTIVITY_ID)) {
                    ...
                    try {
                        ...
                        if (con_d != null) {
                            ...
                            pstmt1 = con_d.prepareStatement(""
                                    + "SELECT id, to_char(time, 'HH24MI') AS stime, state "
                                    + "FROM reservations "
                                    + "WHERE "
                                    + " category = 'dining' AND state <> 'cancelled' AND "
                                    + " organization_id = ? AND "
                            ...
                            while (rs.next()) {
                                ...
                                boolean waitlisted = rs.getString("state").trim().equalsIgnoreCase("waitlisted");
                                item_name = buildItemDescription("Dining Reservation" + ((waitlisted) ? " (" + STATUS_WAITLIST + ")" : ""), Utilities.getSimpleTime(dtime));
                                
                                Map<String, Object> data_map = new LinkedHashMap<String, Object>();
                                data_map.put("url", "Dining_slot?action=edit&reservation_id=" + rs.getInt("id"));
                                data_map.put("base_url", dining_base);
                                data_map.put("data", "");
                                ...
                                if (alternate_model) {
                                    ...
                                    cen.waitlisted = waitlisted;
                                    cen.time = Utilities.getSimpleTime(dtime);
                                    cen.css_class = "tee_list_color_9";
                                    cen.data = data_map;
                                    entries.add(cen);
                                } else {
                                    Map<String, String> calendar_entry = new HashMap<String, String>();
                ...
                if (month_dates.isEmpty() && custom_dates && day > days_in_month && days_done < numDays) {
...
/*
   ...
   //
   private static void countMobile(String user, String club, HttpServletRequest req, Connection con) {
      ...
      if (iphone == true) {
         ...
         iphoneAdd = ", mobile_iphone = mobile_iphone + 1";    // bump iphone counter if its an iPhone
      }

      try {

         stmt = con.prepareStatement (
            "UPDATE member2b SET mobile_count = mobile_count + 1" +iphoneAdd+ " WHERE username = ?");
         ...
         Utilities.logError("Common_webapi.countMobile - error setting mobile count in member2b.  User=" + user + ", club=" + club + ", Err=" + exc.toString());
...
/*
   private static void countMobileApp(String user, String club, HttpServletRequest req, Connection con) {
       
       StatCounters.countMobileApp(user, club, req, con);
   ...
   //
   private static void countLogins(String user, String club, HttpServletRequest req, Connection con) {
       
       StatCounters.countLogins(user, club, req, con);

   }       // end of countLogins

*/
 
    private static String buildItemDescription(String name, String time) {
...
/*
  ...
  */  
    private static Map<String, Object> errorMessageObj(String errorCode, String errorMessage) {
        ...
        return VerifyUser.errorMessageObj(errorCode, errorMessage);

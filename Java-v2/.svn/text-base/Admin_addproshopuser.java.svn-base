    ...
    HttpSession session = SystemUtils.verifyAdmin(req, out);       // check for intruder
     ...
     try {
         ...
         PreparedStatement stmt = con.prepareStatement (
                 ...
                 + "SYSCONFIG_WAITLIST, TS_CHECKIN, TS_PRINT, TS_POS, TS_NOTES_VIEW, TS_NOTES_UPDATE, TS_UPDATE, TS_VIEW, TS_PAST_UPDATE, TS_PAST_VIEW, "
                 + "TS_CTRL_TSEDIT, TS_CTRL_FROST, TS_PACE_UPDATE, TS_PACE_VIEW, TOOLS_ANNOUNCE, TOOLS_HDCP, TOOLS_SEARCHTS, TOOLS_EMAIL, SYSCONFIG_MANAGECONTENT, WAITLIST_UPDATE, "
                 + "WAITLIST_MANAGE, WAITLIST_VIEW"
                 + ") "
                 + "VALUES ("
                 + "?,?,?,?,0,?,?,?,'',?,"
                 + "?,?,"
